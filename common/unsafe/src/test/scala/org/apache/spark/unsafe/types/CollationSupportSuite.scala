...
class CollationSupportSuite extends AnyFunSuite with Matchers { // scalastyle:ignore funsuite
  ...
  private def assertUpper(target: String, collationName: String, expected: String): Unit = {
    ...
    assert(expected_utf8 === CollationSupport.Upper.exec(target_utf8, true, collationId))
    // Testing the old JVM-based implementation of the Upper function.
    assert(expected_utf8 === CollationSupport.Upper.exec(target_utf8, false, collationId))
  ...
  private def assertLower(target: String, collationName: String, expected: String): Unit = {
    ...
    assert(expected_utf8 === CollationSupport.Lower.exec(target_utf8, true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expected_utf8 === CollationSupport.Lower.exec(target_utf8, false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }
  ...
  private def assertInitCap(target: String, collationName: String, expected: String): Unit = {
    ...
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Lower function.
    assert(expected_utf8 === CollationSupport.InitCap.exec(target_utf8, true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expected_utf8 === CollationSupport.InitCap.exec(target_utf8, false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }
  ...
  private def assertInitCap(
    ...
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Lower function.
    assert(expectedICU_utf8 === CollationSupport.InitCap.exec(target_utf8, true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expectedNonICU_utf8 === CollationSupport.InitCap.exec(target_utf8, false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }
