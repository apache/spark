...
class CollationSupportSuite extends AnyFunSuite with Matchers { // scalastyle:ignore funsuite
   ...
   */
  val testSupportedCollations: Seq[String] =
    Seq("UTF8_BINARY", "UTF8_LCASE", "UNICODE", "UNICODE_CI")

  /**
   * Collation-aware UTF8String lowercase conversion.
   */
  private def assertLowerCaseCodePoints(string: String, expected: String, useCodePoints: Boolean): Unit = {
    val str = UTF8String.fromString(string)
    if (useCodePoints) {
      val result = CollationAwareUTF8String.lowerCaseCodePoints(str)
      assert(UTF8String.fromString(expected) === result)
    }
    else {
   ...
   */
  private def assertCompare(s1: String, s2: String, collationName: String, expected: Int): Unit = {
    val l = UTF8String.fromString(s1)
    val r = UTF8String.fromString(s2)
    // Test the comparator, which is the most general way to compare strings with collations.
    val compare = CollationFactory.fetchCollation(collationName).comparator.compare(l, r)
    assert(Integer.signum(expected) === Integer.signum(compare))
    // Test the equals function, which may be faster than the comparator for equality checks.
    val equals = CollationFactory.fetchCollation(collationName).equalsFunction.apply(l, r)
    assert((expected == 0) === equals)
  }

  /**
   * Verify the behaviour of the `Contains` collation support class.
   */
  private def assertContains(pattern: String, target: String, collationName: String, expected: Boolean): Unit = {
    val l = UTF8String.fromString(pattern)
    val r = UTF8String.fromString(target)
    val collationId = CollationFactory.collationNameToId(collationName)
    assert(expected === CollationSupport.Contains.exec(l, r, collationId))
  }

  /**
   * Verify the behaviour of the `StartsWith` collation support class.
   */
  private def assertStartsWith(
      pattern: String,
      prefix: String,
      collationName: String,
      expected: Boolean): Unit = {
    val l = UTF8String.fromString(pattern)
    val r = UTF8String.fromString(prefix)
    val collationId = CollationFactory.collationNameToId(collationName)
    assert(expected === CollationSupport.StartsWith.exec(l, r, collationId))
  }

  /**
   * Verify the behaviour of the `EndsWith` collation support class.
   */
  private def assertEndsWith(
      pattern: String,
      suffix: String,
      collationName: String,
      expected: Boolean): Unit = {
    val l = UTF8String.fromString(pattern)
    val r = UTF8String.fromString(suffix)
    val collationId = CollationFactory.collationNameToId(collationName)
    assert(expected === CollationSupport.EndsWith.exec(l, r, collationId))
  }

  /**
   * Verify the behaviour of the `StringSplitSQL` collation support class.
   */
  private def assertStringSplitSQL(
      str: String,
      delimiter: String,
      collationName: String,
      expected: Array[UTF8String]): Unit = {
    val s = UTF8String.fromString(str)
    val d = UTF8String.fromString(delimiter)
    val collationId = CollationFactory.collationNameToId(collationName)
    val result = CollationSupport.StringSplitSQL.exec(s, d, collationId)
   ...
   */
  private def assertUpper(target: String, collationName: String, expected: String): Unit = {
    val target_utf8 = UTF8String.fromString(target)
    val expected_utf8 = UTF8String.fromString(expected)
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Upper function.
    assert(expected_utf8 === CollationSupport.Upper.exec(target_utf8, useICU = true, collationId))
    // Testing the old JVM-based implementation of the Upper function.
    assert(expected_utf8 === CollationSupport.Upper.exec(target_utf8, useICU = false, collationId))
   ...
   */
  private def assertLower(target: String, collationName: String, expected: String): Unit = {
    val target_utf8 = UTF8String.fromString(target)
    val expected_utf8 = UTF8String.fromString(expected)
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Lower function.
    assert(expected_utf8 === CollationSupport.Lower.exec(target_utf8, useICU = true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expected_utf8 === CollationSupport.Lower.exec(target_utf8, useICU = false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }
   ...
   */
  private def assertInitCap(target: String, collationName: String, expected: String): Unit = {
    val target_utf8 = UTF8String.fromString(target)
    val expected_utf8 = UTF8String.fromString(expected)
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Lower function.
    assert(expected_utf8 ===
      CollationSupport.InitCap.exec(target_utf8, useICU = true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expected_utf8 ===
      CollationSupport.InitCap.exec(target_utf8, useICU = false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }

  private def assertInitCap(
      target: String,
      collationName: String,
      expectedICU: String,
      expectedNonICU: String): Unit = {
    val target_utf8 = UTF8String.fromString(target)
    val expectedICU_utf8 = UTF8String.fromString(expectedICU)
    val expectedNonICU_utf8 = UTF8String.fromString(expectedNonICU)
    val collationId = CollationFactory.collationNameToId(collationName)
    // Testing the new ICU-based implementation of the Lower function.
    assert(expectedICU_utf8 ===
      CollationSupport.InitCap.exec(target_utf8, useICU = true, collationId))
    // Testing the old JVM-based implementation of the Lower function.
    assert(expectedNonICU_utf8 ===
      CollationSupport.InitCap.exec(target_utf8, useICU = false, collationId))
    // Note: results should be the same in these tests for both ICU and JVM-based implementations.
  }
   ...
   */
  private def assertStringInstr(
      string: String,
      substring: String,
      collationName: String,
      expected: Int): Unit = {
    val str = UTF8String.fromString(string)
    val substr = UTF8String.fromString(substring)
    val collationId = CollationFactory.collationNameToId(collationName)
    assert(expected === CollationSupport.StringInstr.exec(str, substr, collationId) + 1)
  }

  /**
   * Verify the behaviour of the `FindInSet` collation support class.
   */
  private def assertFindInSet(
      word: String,
      set: UTF8String,
      collationName: String,
      expected: Int): Unit = {
    val w = UTF8String.fromString(word)
    val collationId = CollationFactory.collationNameToId(collationName)
    val result = CollationSupport.FindInSet.exec(w, set, collationId)
    assert(expected === result)
  }
   ...
   */
  private def assertStringReplace(
       source: String,
       search: String,
       replace: String,
       collationName: String,
       expected: String): Unit = {
    val src = UTF8String.fromString(source)
    val sear = UTF8String.fromString(search)
    val repl = UTF8String.fromString(replace)
    val collationId = CollationFactory.collationNameToId(collationName)
    val result = CollationSupport.StringReplace.exec(src, sear, repl, collationId)
    assert(UTF8String.fromString(expected) === result)
  }
   ...
   */
  private def assertStringLocate(
      substring: String,
      string: String,
      start: Int,
      collationName: String,
    ...
    val str = UTF8String.fromString(string)
    val collationId = CollationFactory.collationNameToId(collationName)
    val result = CollationSupport.StringLocate.exec(str, substr, start - 1, collationId) + 1
    assert(expected === result)
  }
   ...
   */
  private def assertSubstringIndex(
      string: String,
      delimiter: String,
      count: Int,
      collationName: String,
      expected: String): Unit = {
    val str = UTF8String.fromString(string)
    val delim = UTF8String.fromString(delimiter)
    val collationId = CollationFactory.collationNameToId(collationName)
    val result = CollationSupport.SubstringIndex.exec(str, delim, count, collationId)
    assert(UTF8String.fromString(expected) === result)
  }
  ...
  private def assertStringTrim(
    ...
    val collationId = CollationFactory.collationNameToId(collationName)
    val src = UTF8String.fromString(sourceString)
    val trim = UTF8String.fromString(trimString)
    var result: UTF8String = null
    var resultTrimLeftRight: UTF8String = null
    var resultTrimRightLeft: UTF8String = null
    if (trimString == null) {
      // Trim string is ASCII space.
      result = CollationSupport.StringTrim.exec(src)
      val trimLeft = CollationSupport.StringTrimLeft.exec(src)
      resultTrimLeftRight = CollationSupport.StringTrimRight.exec(trimLeft)
      val trimRight = CollationSupport.StringTrimRight.exec(src)
      ...
      val trimLeft = CollationSupport.StringTrimLeft.exec(src, trim, collationId)
      resultTrimLeftRight = CollationSupport.StringTrimRight.exec(trimLeft, trim, collationId)
      val trimRight = CollationSupport.StringTrimRight.exec(src, trim, collationId)
    ...
    assert(resultTrimLeftRight === result)
    assert(resultTrimRightLeft === result)
  }

  /**
   * Verify the behaviour of the `StringTrimLeft` collation support class.
   */
  private def assertStringTrimLeft(
      collationName: String,
      sourceString: String,
      trimString: String,
      expected: String): Unit = {
   ...
   */
  private def assertStringTrimRight(
      collationName: String,
      sourceString: String,
      trimString: String,
      expected: String): Unit = {
   ...
   */
  private def assertStringTranslate(
      inputString: String,
      ...
      expected: String): Unit = {
    val collationId = CollationFactory.collationNameToId(collationName)
    val dict = buildDict(matchingString, replaceString)
    val source = UTF8String.fromString(inputString)
    val result = CollationSupport.StringTranslate.exec(source, dict, collationId)
    assert(UTF8String.fromString(expected) === result)
  }

  private def buildDict(matching: String, replace: String): java.util.Map[String, String] = {
    val dict = new util.HashMap[String, String]
    var i: Int = 0
    var j: Int = 0
    while (i < matching.length) {
      var rep: String = "\u0000"
      if (j < replace.length) {
        val repCharCount: Int = Character.charCount(replace.codePointAt(j))
        rep = replace.substring(j, j + repCharCount)
        j += repCharCount
      }
      val matchCharCount: Int = Character.charCount(matching.codePointAt(i))
      val matchStr: String = matching.substring(i, i + matchCharCount)
  ...
  test("compare") {
    ...
    // Maximum code point.
    val maxCodePoint: Int = Character.MAX_CODE_POINT
    val maxCodePointStr: String = new String(Character.toChars(maxCodePoint))
    var i: Int = 0
    ...
    // Minimum code point.
    val minCodePoint: Int = Character.MIN_CODE_POINT
    val minCodePointStr: String = new String(Character.toChars(minCodePoint))
    i = minCodePoint + 1
    while (i <= maxCodePoint && Character.isValidCodePoint(i)) {
  ...
  test("lowercase code points") {
    // Empty strings.
    assertLowerCaseCodePoints("", "", useCodePoints = false)
    assertLowerCaseCodePoints("", "", useCodePoints = true)
    // Basic tests.
    assertLowerCaseCodePoints("xyz", "xyz", useCodePoints = false)
    assertLowerCaseCodePoints("xyz", "xyz", useCodePoints = true)
    assertLowerCaseCodePoints("abcd", "abcd", useCodePoints = false)
    assertLowerCaseCodePoints("abcd", "abcd", useCodePoints = true)
    // Advanced tests.
    assertLowerCaseCodePoints("你好", "你好", useCodePoints = false)
    assertLowerCaseCodePoints("你好", "你好", useCodePoints = true)
    assertLowerCaseCodePoints("Γειά", "γειά", useCodePoints = false)
    assertLowerCaseCodePoints("Γειά", "γειά", useCodePoints = true)
    assertLowerCaseCodePoints("Здраво", "здраво", useCodePoints = false)
    assertLowerCaseCodePoints("Здраво", "здраво", useCodePoints = true)
    // Case variation.
    assertLowerCaseCodePoints("xYz", "xyz", useCodePoints = false)
    assertLowerCaseCodePoints("xYz", "xyz", useCodePoints = true)
    assertLowerCaseCodePoints("AbCd", "abcd", useCodePoints = false)
    assertLowerCaseCodePoints("aBcD", "abcd", useCodePoints = true)
    // Accent variation.
    assertLowerCaseCodePoints("äbć", "äbć", useCodePoints = false)
    assertLowerCaseCodePoints("äbć", "äbć", useCodePoints = true)
    assertLowerCaseCodePoints("AbĆd", "abćd", useCodePoints = false)
    assertLowerCaseCodePoints("aBcΔ", "abcδ", useCodePoints = true)
    // One-to-many case mapping (e.g. Turkish dotted I).
    assertLowerCaseCodePoints("i\u0307", "i\u0307", useCodePoints = false)
    assertLowerCaseCodePoints("i\u0307", "i\u0307", useCodePoints = true)
    assertLowerCaseCodePoints("I\u0307", "i\u0307", useCodePoints = false)
    assertLowerCaseCodePoints("I\u0307", "i\u0307", useCodePoints = true)
    assertLowerCaseCodePoints("İ", "i\u0307", useCodePoints = false)
    assertLowerCaseCodePoints("İ", "i\u0307", useCodePoints = true)
    assertLowerCaseCodePoints("İİİ", "i\u0307i\u0307i\u0307", useCodePoints = false)
    assertLowerCaseCodePoints("İİİ", "i\u0307i\u0307i\u0307", useCodePoints = true)
    assertLowerCaseCodePoints("İiIi\u0307", "i\u0307iii\u0307", useCodePoints = false)
    assertLowerCaseCodePoints("İiIi\u0307", "i\u0307iii\u0307", useCodePoints = true)
    assertLowerCaseCodePoints("İoDiNe", "i\u0307odine", useCodePoints = false)
    assertLowerCaseCodePoints("İodInE", "i\u0307odine", useCodePoints = true)
    assertLowerCaseCodePoints("Abi\u0307o12", "abi\u0307o12", useCodePoints = false)
    assertLowerCaseCodePoints("aBi\u0307o12", "abi\u0307o12", useCodePoints = true)
    // Conditional case mapping (e.g. Greek sigmas).
    assertLowerCaseCodePoints("ς", "ς", useCodePoints = false)
    assertLowerCaseCodePoints("ς", "σ", useCodePoints = true)
    assertLowerCaseCodePoints("σ", "σ", useCodePoints = false)
    assertLowerCaseCodePoints("σ", "σ", useCodePoints = true)
    assertLowerCaseCodePoints("Σ", "σ", useCodePoints = false)
    assertLowerCaseCodePoints("Σ", "σ", useCodePoints = true)
    assertLowerCaseCodePoints("ςΑΛΑΤΑ", "ςαλατα", useCodePoints = false)
    assertLowerCaseCodePoints("ςΑΛΑΤΑ", "σαλατα", useCodePoints = true)
    assertLowerCaseCodePoints("σΑΛΑΤΑ", "σαλατα", useCodePoints = false)
    assertLowerCaseCodePoints("σΑΛΑΤΑ", "σαλατα", useCodePoints = true)
    assertLowerCaseCodePoints("ΣΑΛΑΤΑ", "σαλατα", useCodePoints = false)
    assertLowerCaseCodePoints("ΣΑΛΑΤΑ", "σαλατα", useCodePoints = true)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟς", "θαλασσινος", useCodePoints = false)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟς", "θαλασσινοσ", useCodePoints = true)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟσ", "θαλασσινοσ", useCodePoints = false)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟσ", "θαλασσινοσ", useCodePoints = true)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟΣ", "θαλασσινος", useCodePoints = false)
    assertLowerCaseCodePoints("ΘΑΛΑΣΣΙΝΟΣ", "θαλασσινοσ", useCodePoints = true)
    // Surrogate pairs.
    assertLowerCaseCodePoints("a🙃b🙃c", "a🙃b🙃c", useCodePoints = false)
    assertLowerCaseCodePoints("a🙃b🙃c", "a🙃b🙃c", useCodePoints = true)
    assertLowerCaseCodePoints("😀😆😃😄😄😆", "😀😆😃😄😄😆", useCodePoints = false)
    assertLowerCaseCodePoints("😀😆😃😄😄😆", "😀😆😃😄😄😆", useCodePoints = true)
    assertLowerCaseCodePoints("𐐅", "𐐭", useCodePoints = false)
    assertLowerCaseCodePoints("𐐅", "𐐭", useCodePoints = true)
    assertLowerCaseCodePoints("𝔸", "𝔸", useCodePoints = false)
    assertLowerCaseCodePoints("𝔸", "𝔸", useCodePoints = true)
  }

  test("contains") {
    testSupportedCollations.foreach { collationName =>
      // Empty strings.
      assertContains("", "", collationName, expected = true)
      assertContains("a", "", collationName, expected = true)
      assertContains("", "x", collationName, expected = false)
      // Basic tests.
      assertContains("a", "a", collationName, expected = true)
      assertContains("_a_", "_a_", collationName, expected = true)
      assertContains("_a_", "a", collationName, expected = true)
      assertContains("%a%", "%a%", collationName, expected = true)
      assertContains("%a%", "a", collationName, expected = true)
      assertContains("*a*", "*a*", collationName, expected = true)
      assertContains("*a*", "a", collationName, expected = true)
      ...
      assertContains("/a/", "/a/", collationName, expected = true)
      assertContains("/a/", "a", collationName, expected = true)
      assertContains("abcde", "xyz", collationName, expected = false)
      assertContains("abcde", "bcd", collationName, expected = true)
      assertContains("abcde", "abc", collationName, expected = true)
      assertContains("abcde", "cde", collationName, expected = true)
      assertContains("abcde", "abcde", collationName, expected = true)
      assertContains("你好", "x", collationName, expected = false)
      assertContains("你好", "你", collationName, expected = true)
      assertContains("你好", "好", collationName, expected = true)
      assertContains("你好", "你好", collationName, expected = true)
      assertContains("Γειά", "x", collationName, expected = false)
      assertContains("Γειά", "ειά", collationName, expected = true)
      assertContains("Γειά", "Γει", collationName, expected = true)
      assertContains("Γειά", "Γειά", collationName, expected = true)
      assertContains("Здраво", "x", collationName, expected = false)
      assertContains("Здраво", "драво", collationName, expected = true)
      assertContains("Здраво", "Здрав", collationName, expected = true)
      assertContains("Здраво", "Здраво", collationName, expected = true)
    }
    // Advanced tests.
    assertContains("abcde", "bcd", "UTF8_BINARY", expected = true)
    assertContains("abcde", "bde", "UTF8_BINARY", expected = false)
    assertContains("abcde", "fgh", "UTF8_BINARY", expected = false)
    assertContains("abcde", "abcde", "UNICODE", expected = true)
    assertContains("abcde", "aBcDe", "UNICODE", expected = false)
    assertContains("abcde", "fghij", "UNICODE", expected = false)
    assertContains("abcde", "C", "UTF8_LCASE", expected = true)
    assertContains("abcde", "AbCdE", "UTF8_LCASE", expected = true)
    assertContains("abcde", "X", "UTF8_LCASE", expected = false)
    assertContains("abcde", "c", "UNICODE_CI", expected = true)
    assertContains("abcde", "bCD", "UNICODE_CI", expected = true)
    assertContains("abcde", "123", "UNICODE_CI", expected = false)
    assertContains("ab世De", "b世D", "UTF8_BINARY", expected = true)
    assertContains("ab世De", "B世d", "UTF8_BINARY", expected = false)
    assertContains("äbćδe", "bćδ", "UTF8_BINARY", expected = true)
    assertContains("äbćδe", "BcΔ", "UTF8_BINARY", expected = false)
    assertContains("ab世De", "ab世De", "UNICODE", expected = true)
    assertContains("ab世De", "AB世dE", "UNICODE", expected = false)
    assertContains("äbćδe", "äbćδe", "UNICODE", expected = true)
    assertContains("äbćδe", "ÄBcΔÉ", "UNICODE", expected = false)
    assertContains("ab世De", "b世D", "UTF8_LCASE", expected = true)
    assertContains("ab世De", "B世d", "UTF8_LCASE", expected = true)
    assertContains("äbćδe", "bćδ", "UTF8_LCASE", expected = true)
    assertContains("äbćδe", "BcΔ", "UTF8_LCASE", expected = false)
    assertContains("ab世De", "ab世De", "UNICODE_CI", expected = true)
    assertContains("ab世De", "AB世dE", "UNICODE_CI", expected = true)
    assertContains("äbćδe", "ÄbćδE", "UNICODE_CI", expected = true)
    assertContains("äbćδe", "ÄBcΔÉ", "UNICODE_CI", expected = false)
    assertContains("The Kelvin.", "Kelvin", "UTF8_LCASE", expected = true)
    assertContains("The Kelvin.", "Kelvin", "UTF8_LCASE", expected = true)
    assertContains("The KKelvin.", "KKelvin", "UTF8_LCASE", expected = true)
    assertContains("2 Kelvin.", "2 Kelvin", "UTF8_LCASE", expected = true)
    assertContains("2 Kelvin.", "2 Kelvin", "UTF8_LCASE", expected = true)
    assertContains("The KKelvin.", "KKelvin,", "UTF8_LCASE", expected = false)
    // Case variation.
    assertContains("aBcDe", "bcd", "UTF8_BINARY", expected = false)
    assertContains("aBcDe", "BcD", "UTF8_BINARY", expected = true)
    assertContains("aBcDe", "abcde", "UNICODE", expected = false)
    assertContains("aBcDe", "aBcDe", "UNICODE", expected = true)
    assertContains("aBcDe", "bcd", "UTF8_LCASE", expected = true)
    assertContains("aBcDe", "BCD", "UTF8_LCASE", expected = true)
    assertContains("aBcDe", "abcde", "UNICODE_CI", expected = true)
    assertContains("aBcDe", "AbCdE", "UNICODE_CI", expected = true)
    // Accent variation.
    assertContains("aBcDe", "bćd", "UTF8_BINARY", expected = false)
    assertContains("aBcDe", "BćD", "UTF8_BINARY", expected = false)
    assertContains("aBcDe", "abćde", "UNICODE", expected = false)
    assertContains("aBcDe", "aBćDe", "UNICODE", expected = false)
    assertContains("aBcDe", "bćd", "UTF8_LCASE", expected = false)
    assertContains("aBcDe", "BĆD", "UTF8_LCASE", expected = false)
    assertContains("aBcDe", "abćde", "UNICODE_CI", expected = false)
    assertContains("aBcDe", "AbĆdE", "UNICODE_CI", expected = false)
    // One-to-many case mapping (e.g. Turkish dotted I).
    assertContains("i\u0307", "i", "UNICODE_CI", expected = false)
    assertContains("i\u0307", "\u0307", "UNICODE_CI", expected = false)
    assertContains("i\u0307", "İ", "UNICODE_CI", expected = true)
    assertContains("İ", "i", "UNICODE_CI", expected = false)
    assertContains("adi̇os", "io", "UNICODE_CI", expected = false)
    assertContains("adi̇os", "Io", "UNICODE_CI", expected = false)
    assertContains("adi̇os", "i\u0307o", "UNICODE_CI", expected = true)
    assertContains("adi̇os", "İo", "UNICODE_CI", expected = true)
    assertContains("adİos", "io", "UNICODE_CI", expected = false)
    assertContains("adİos", "Io", "UNICODE_CI", expected = false)
    assertContains("adİos", "i\u0307o", "UNICODE_CI", expected = true)
    assertContains("adİos", "İo", "UNICODE_CI", expected = true)
    assertContains("i\u0307", "i", "UTF8_LCASE", expected = true) // != UNICODE_CI
    assertContains("İ", "\u0307", "UTF8_LCASE", expected = false)
    assertContains("İ", "i", "UTF8_LCASE", expected = false)
    assertContains("i\u0307", "\u0307", "UTF8_LCASE", expected = true) // != UNICODE_CI
    assertContains("i\u0307", "İ", "UTF8_LCASE", expected = true)
    assertContains("İ", "i", "UTF8_LCASE", expected = false)
    assertContains("adi̇os", "io", "UTF8_LCASE", expected = false)
    assertContains("adi̇os", "Io", "UTF8_LCASE", expected = false)
    assertContains("adi̇os", "i\u0307o", "UTF8_LCASE", expected = true)
    assertContains("adi̇os", "İo", "UTF8_LCASE", expected = true)
    assertContains("adİos", "io", "UTF8_LCASE", expected = false)
    assertContains("adİos", "Io", "UTF8_LCASE", expected = false)
    assertContains("adİos", "i\u0307o", "UTF8_LCASE", expected = true)
    assertContains("adİos", "İo", "UTF8_LCASE", expected = true)
    // Conditional case mapping (e.g. Greek sigmas).
    assertContains("σ", "σ", "UTF8_BINARY", expected = true)
    assertContains("σ", "ς", "UTF8_BINARY", expected = false)
    assertContains("σ", "Σ", "UTF8_BINARY", expected = false)
    assertContains("ς", "σ", "UTF8_BINARY", expected = false)
    assertContains("ς", "ς", "UTF8_BINARY", expected = true)
    assertContains("ς", "Σ", "UTF8_BINARY", expected = false)
    assertContains("Σ", "σ", "UTF8_BINARY", expected = false)
    assertContains("Σ", "ς", "UTF8_BINARY", expected = false)
    assertContains("Σ", "Σ", "UTF8_BINARY", expected = true)
    assertContains("σ", "σ", "UTF8_LCASE", expected = true)
    assertContains("σ", "ς", "UTF8_LCASE", expected = true)
    assertContains("σ", "Σ", "UTF8_LCASE", expected = true)
    assertContains("ς", "σ", "UTF8_LCASE", expected = true)
    assertContains("ς", "ς", "UTF8_LCASE", expected = true)
    assertContains("ς", "Σ", "UTF8_LCASE", expected = true)
    assertContains("Σ", "σ", "UTF8_LCASE", expected = true)
    assertContains("Σ", "ς", "UTF8_LCASE", expected = true)
    assertContains("Σ", "Σ", "UTF8_LCASE", expected = true)
    assertContains("σ", "σ", "UNICODE", expected = true)
    assertContains("σ", "ς", "UNICODE", expected = false)
    assertContains("σ", "Σ", "UNICODE", expected = false)
    assertContains("ς", "σ", "UNICODE", expected = false)
    assertContains("ς", "ς", "UNICODE", expected = true)
    assertContains("ς", "Σ", "UNICODE", expected = false)
    assertContains("Σ", "σ", "UNICODE", expected = false)
    assertContains("Σ", "ς", "UNICODE", expected = false)
    assertContains("Σ", "Σ", "UNICODE", expected = true)
    assertContains("σ", "σ", "UNICODE_CI", expected = true)
    assertContains("σ", "ς", "UNICODE_CI", expected = true)
    assertContains("σ", "Σ", "UNICODE_CI", expected = true)
    assertContains("ς", "σ", "UNICODE_CI", expected = true)
    assertContains("ς", "ς", "UNICODE_CI", expected = true)
    assertContains("ς", "Σ", "UNICODE_CI", expected = true)
    assertContains("Σ", "σ", "UNICODE_CI", expected = true)
    assertContains("Σ", "ς", "UNICODE_CI", expected = true)
    assertContains("Σ", "Σ", "UNICODE_CI", expected = true)
    assertContains("ΣΑΛΑΤΑ", "Σ", "UTF8_BINARY", expected = true)
    assertContains("ΣΑΛΑΤΑ", "σ", "UTF8_BINARY", expected = false)
    assertContains("ΣΑΛΑΤΑ", "ς", "UTF8_BINARY", expected = false)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UTF8_BINARY", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UTF8_BINARY", expected = false)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UTF8_BINARY", expected = false)
    assertContains("ΣΑΛΑΤΑ", "Σ", "UTF8_LCASE", expected = true)
    assertContains("ΣΑΛΑΤΑ", "σ", "UTF8_LCASE", expected = true)
    assertContains("ΣΑΛΑΤΑ", "ς", "UTF8_LCASE", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UTF8_LCASE", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UTF8_LCASE", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UTF8_LCASE", expected = true)
    assertContains("ΣΑΛΑΤΑ", "Σ", "UNICODE", expected = true)
    assertContains("ΣΑΛΑΤΑ", "σ", "UNICODE", expected = false)
    assertContains("ΣΑΛΑΤΑ", "ς", "UNICODE", expected = false)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UNICODE", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UNICODE", expected = false)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UNICODE", expected = false)
    assertContains("ΣΑΛΑΤΑ", "Σ", "UNICODE_CI", expected = true)
    assertContains("ΣΑΛΑΤΑ", "σ", "UNICODE_CI", expected = true)
    assertContains("ΣΑΛΑΤΑ", "ς", "UNICODE_CI", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UNICODE_CI", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UNICODE_CI", expected = true)
    assertContains("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UNICODE_CI", expected = true)
    // Surrogate pairs.
    assertContains("a🙃b🙃c", "x", "UTF8_BINARY", expected = false)
    assertContains("a🙃b🙃c", "x", "UTF8_LCASE", expected = false)
    assertContains("a🙃b🙃c", "x", "UNICODE", expected = false)
    assertContains("a🙃b🙃c", "x", "UNICODE_CI", expected = false)
    assertContains("a🙃b🙃c", "b", "UTF8_BINARY", expected = true)
    assertContains("a🙃b🙃c", "b", "UTF8_LCASE", expected = true)
    assertContains("a🙃b🙃c", "b", "UNICODE", expected = true)
    assertContains("a🙃b🙃c", "b", "UNICODE_CI", expected = true)
    assertContains("a🙃b🙃c", "a🙃b", "UTF8_BINARY", expected = true)
    assertContains("a🙃b🙃c", "a🙃b", "UTF8_LCASE", expected = true)
    assertContains("a🙃b🙃c", "a🙃b", "UNICODE", expected = true)
    assertContains("a🙃b🙃c", "a🙃b", "UNICODE_CI", expected = true)
    assertContains("a🙃b🙃c", "b🙃c", "UTF8_BINARY", expected = true)
    assertContains("a🙃b🙃c", "b🙃c", "UTF8_LCASE", expected = true)
    assertContains("a🙃b🙃c", "b🙃c", "UNICODE", expected = true)
    assertContains("a🙃b🙃c", "b🙃c", "UNICODE_CI", expected = true)
    assertContains("a🙃b🙃c", "a🙃b🙃c", "UTF8_BINARY", expected = true)
    assertContains("a🙃b🙃c", "a🙃b🙃c", "UTF8_LCASE", expected = true)
    assertContains("a🙃b🙃c", "a🙃b🙃c", "UNICODE", expected = true)
    assertContains("a🙃b🙃c", "a🙃b🙃c", "UNICODE_CI", expected = true)
    assertContains("😀😆😃😄", "😄😆", "UTF8_BINARY", expected = false)
    assertContains("😀😆😃😄", "😄😆", "UTF8_LCASE", expected = false)
    assertContains("😀😆😃😄", "😄😆", "UNICODE", expected = false)
    assertContains("😀😆😃😄", "😄😆", "UNICODE_CI", expected = false)
    assertContains("😀😆😃😄", "😆😃", "UTF8_BINARY", expected = true)
    assertContains("😀😆😃😄", "😆😃", "UTF8_LCASE", expected = true)
    assertContains("😀😆😃😄", "😆😃", "UNICODE", expected = true)
    assertContains("😀😆😃😄", "😆😃", "UNICODE_CI", expected = true)
    assertContains("😀😆😃😄", "😀😆", "UTF8_BINARY", expected = true)
    assertContains("😀😆😃😄", "😀😆", "UTF8_LCASE", expected = true)
    assertContains("😀😆😃😄", "😀😆", "UNICODE", expected = true)
    assertContains("😀😆😃😄", "😀😆", "UNICODE_CI", expected = true)
    ...
    assertContains("😀😆😃😄", "😀😆😃😄", "UNICODE", expected = true)
    assertContains("😀😆😃😄", "😀😆😃😄", "UNICODE_CI", expected = true)
    assertContains("𐐅", "𐐅", "UTF8_BINARY", expected = true)
    assertContains("𐐅", "𐐅", "UTF8_LCASE", expected = true)
    assertContains("𐐅", "𐐅", "UNICODE", expected = true)
    assertContains("𐐅", "𐐅", "UNICODE_CI", expected = true)
    assertContains("𐐅", "𐐭", "UTF8_BINARY", expected = false)
    assertContains("𐐅", "𐐭", "UTF8_LCASE", expected = true)
    assertContains("𐐅", "𐐭", "UNICODE", expected = false)
    assertContains("𐐅", "𐐭", "UNICODE_CI", expected = true)
    assertContains("𝔸", "𝔸", "UTF8_BINARY", expected = true)
    assertContains("𝔸", "𝔸", "UTF8_LCASE", expected = true)
    assertContains("𝔸", "𝔸", "UNICODE", expected = true)
    assertContains("𝔸", "𝔸", "UNICODE_CI", expected = true)
  }

  test("startsWith") {
    testSupportedCollations.foreach { collationName =>
      // Empty strings.
      assertStartsWith("", "", collationName, expected = true)
      assertStartsWith("a", "", collationName, expected = true)
      assertStartsWith("", "x", collationName, expected = false)
      // Basic tests.
      assertStartsWith("a", "a", collationName, expected = true)
      assertStartsWith("_a_", "_a", collationName, expected = true)
      assertStartsWith("_a_", "a", collationName, expected = false)
      assertStartsWith("%a%", "%a", collationName, expected = true)
      assertStartsWith("%a%", "a", collationName, expected = false)
      assertStartsWith("*a*", "*a", collationName, expected = true)
      assertStartsWith("*a*", "a", collationName, expected = false)
      assertStartsWith("?a?", "?a", collationName, expected = true)
      assertStartsWith("?a?", "a", collationName, expected = false)
      assertStartsWith("/a/", "/a", collationName, expected = true)
      assertStartsWith("/a/", "a", collationName, expected = false)
      assertStartsWith("abcde", "xyz", collationName, expected = false)
      assertStartsWith("abcde", "bcd", collationName, expected = false)
      assertStartsWith("abcde", "abc", collationName, expected = true)
      assertStartsWith("abcde", "cde", collationName, expected = false)
      assertStartsWith("abcde", "abcde", collationName, expected = true)
      assertStartsWith("你好", "x", collationName, expected = false)
      assertStartsWith("你好", "你", collationName, expected = true)
      assertStartsWith("你好", "好", collationName, expected = false)
      assertStartsWith("你好", "你好", collationName, expected = true)
      assertStartsWith("Γειά", "x", collationName, expected = false)
      assertStartsWith("Γειά", "ειά", collationName, expected = false)
      assertStartsWith("Γειά", "Γει", collationName, expected = true)
      assertStartsWith("Γειά", "Γειά", collationName, expected = true)
      assertStartsWith("Здраво", "x", collationName, expected = false)
      assertStartsWith("Здраво", "драво", collationName, expected = false)
      assertStartsWith("Здраво", "Здрав", collationName, expected = true)
      assertStartsWith("Здраво", "Здраво", collationName, expected = true)
    }
  }
  ...
  test("ends with") {
    testSupportedCollations.foreach { collationName =>
      // Empty strings.
      assertEndsWith("", "", collationName, expected = true)
      assertEndsWith("a", "", collationName, expected = true)
      assertEndsWith("", "x", collationName, expected = false)
      // Basic tests.
      assertEndsWith("a", "a", collationName, expected = true)
      assertEndsWith("_a_", "a_", collationName, expected = true)
      assertEndsWith("_a_", "a", collationName, expected = false)
      assertEndsWith("%a%", "a%", collationName, expected = true)
      assertEndsWith("%a%", "a", collationName, expected = false)
      assertEndsWith("*a*", "a*", collationName, expected = true)
      assertEndsWith("*a*", "a", collationName, expected = false)
      assertEndsWith("?a?", "a?", collationName, expected = true)
      assertEndsWith("?a?", "a", collationName, expected = false)
      assertEndsWith("/a/", "a/", collationName, expected = true)
      assertEndsWith("/a/", "a", collationName, expected = false)
      assertEndsWith("abcde", "xyz", collationName, expected = false)
      assertEndsWith("abcde", "bcd", collationName, expected = false)
      assertEndsWith("abcde", "abc", collationName, expected = false)
      assertEndsWith("abcde", "cde", collationName, expected = true)
      assertEndsWith("abcde", "abcde", collationName, expected = true)
      assertEndsWith("你好", "x", collationName, expected = false)
      assertEndsWith("你好", "你", collationName, expected = false)
      assertEndsWith("你好", "好", collationName, expected = true)
      assertEndsWith("你好", "你好", collationName, expected = true)
      assertEndsWith("Γειά", "x", collationName, expected = false)
      assertEndsWith("Γειά", "ειά", collationName, expected = true)
      assertEndsWith("Γειά", "Γει", collationName, expected = false)
      assertEndsWith("Γειά", "Γειά", collationName, expected = true)
      assertEndsWith("Здраво", "x", collationName, expected = false)
      assertEndsWith("Здраво", "драво", collationName, expected = true)
      assertEndsWith("Здраво", "Здрав", collationName, expected = false)
      assertEndsWith("Здраво", "Здраво", collationName, expected = true)
    }
    // Advanced tests.
    assertEndsWith("abcde", "cde", "UTF8_BINARY", expected = true)
    assertEndsWith("abcde", "bde", "UTF8_BINARY", expected = false)
    assertEndsWith("abcde", "fgh", "UTF8_BINARY", expected = false)
    assertEndsWith("abcde", "abcde", "UNICODE", expected = true)
    assertEndsWith("abcde", "aBcDe", "UNICODE", expected = false)
    assertEndsWith("abcde", "fghij", "UNICODE", expected = false)
    assertEndsWith("abcde", "E", "UTF8_LCASE", expected = true)
    assertEndsWith("abcde", "AbCdE", "UTF8_LCASE", expected = true)
    assertEndsWith("abcde", "X", "UTF8_LCASE", expected = false)
    assertEndsWith("abcde", "e", "UNICODE_CI", expected = true)
    assertEndsWith("abcde", "CDe", "UNICODE_CI", expected = true)
    assertEndsWith("abcde", "bcd", "UNICODE_CI", expected = false)
    assertEndsWith("abcde", "123", "UNICODE_CI", expected = false)
    assertEndsWith("ab世De", "世De", "UTF8_BINARY", expected = true)
    assertEndsWith("ab世De", "世dE", "UTF8_BINARY", expected = false)
    assertEndsWith("äbćδe", "ćδe", "UTF8_BINARY", expected = true)
    assertEndsWith("äbćδe", "cΔé", "UTF8_BINARY", expected = false)
    assertEndsWith("ab世De", "ab世De", "UNICODE", expected = true)
    assertEndsWith("ab世De", "AB世dE", "UNICODE", expected = false)
    assertEndsWith("äbćδe", "äbćδe", "UNICODE", expected = true)
    assertEndsWith("äbćδe", "ÄBcΔÉ", "UNICODE", expected = false)
    assertEndsWith("ab世De", "世De", "UTF8_LCASE", expected = true)
    assertEndsWith("ab世De", "世dE", "UTF8_LCASE", expected = true)
    assertEndsWith("äbćδe", "ćδe", "UTF8_LCASE", expected = true)
    assertEndsWith("äbćδe", "cδE", "UTF8_LCASE", expected = false)
    assertEndsWith("ab世De", "ab世De", "UNICODE_CI", expected = true)
    assertEndsWith("ab世De", "AB世dE", "UNICODE_CI", expected = true)
    assertEndsWith("äbćδe", "ÄbćδE", "UNICODE_CI", expected = true)
    assertEndsWith("äbćδe", "ÄBcΔÉ", "UNICODE_CI", expected = false)
    assertEndsWith("The Kelvin", "Kelvin", "UTF8_LCASE", expected = true)
    assertEndsWith("The Kelvin", "Kelvin", "UTF8_LCASE", expected = true)
    assertEndsWith("The KKelvin", "KKelvin", "UTF8_LCASE", expected = true)
    assertEndsWith("The 2 Kelvin", "2 Kelvin", "UTF8_LCASE", expected = true)
    assertEndsWith("The 2 Kelvin", "2 Kelvin", "UTF8_LCASE", expected = true)
    assertEndsWith("The KKelvin", "KKelvin,", "UTF8_LCASE", expected = false)
    // Case variation.
    assertEndsWith("aBcDe", "cde", "UTF8_BINARY", expected = false)
    assertEndsWith("aBcDe", "cDe", "UTF8_BINARY", expected = true)
    assertEndsWith("aBcDe", "abcde", "UNICODE", expected = false)
    assertEndsWith("aBcDe", "aBcDe", "UNICODE", expected = true)
    assertEndsWith("aBcDe", "cde", "UTF8_LCASE", expected = true)
    assertEndsWith("aBcDe", "CDE", "UTF8_LCASE", expected = true)
    assertEndsWith("aBcDe", "abcde", "UNICODE_CI", expected = true)
    assertEndsWith("aBcDe", "AbCdE", "UNICODE_CI", expected = true)
    // Accent variation.
    assertEndsWith("aBcDe", "ćde", "UTF8_BINARY", expected = false)
    assertEndsWith("aBcDe", "ćDe", "UTF8_BINARY", expected = false)
    assertEndsWith("aBcDe", "abćde", "UNICODE", expected = false)
    assertEndsWith("aBcDe", "aBćDe", "UNICODE", expected = false)
    assertEndsWith("aBcDe", "ćde", "UTF8_LCASE", expected = false)
    assertEndsWith("aBcDe", "ĆDE", "UTF8_LCASE", expected = false)
    assertEndsWith("aBcDe", "abćde", "UNICODE_CI", expected = false)
    assertEndsWith("aBcDe", "AbĆdE", "UNICODE_CI", expected = false)
    // One-to-many case mapping (e.g. Turkish dotted I).
    assertEndsWith("i\u0307", "\u0307", "UNICODE_CI", expected = false)
    assertEndsWith("i\u0307", "İ", "UNICODE_CI", expected = true)
    assertEndsWith("İ", "i", "UNICODE_CI", expected = false)
    assertEndsWith("İİİ", "i̇i̇", "UNICODE_CI", expected = true)
    assertEndsWith("İİİ", "ii̇", "UNICODE_CI", expected = false)
    assertEndsWith("İi̇İ", "İi̇", "UNICODE_CI", expected = true)
    assertEndsWith("i̇İi̇i̇", "\u0307İi̇İ", "UNICODE_CI", expected = false)
    assertEndsWith("the i\u0307o", "io", "UNICODE_CI", expected = false)
    assertEndsWith("the i\u0307o", "Io", "UNICODE_CI", expected = false)
    assertEndsWith("the i\u0307o", "i\u0307o", "UNICODE_CI", expected = true)
    assertEndsWith("the i\u0307o", "İo", "UNICODE_CI", expected = true)
    assertEndsWith("the İo", "io", "UNICODE_CI", expected = false)
    assertEndsWith("the İo", "Io", "UNICODE_CI", expected = false)
    assertEndsWith("the İo", "i\u0307o", "UNICODE_CI", expected = true)
    assertEndsWith("the İo", "İo", "UNICODE_CI", expected = true)
    assertEndsWith("i\u0307", "\u0307", "UTF8_LCASE", expected = true) // != UNICODE_CI

    assertEndsWith("i\u0307", "İ", "UTF8_LCASE", expected = true)
    assertEndsWith("İ", "\u0307", "UTF8_LCASE", expected = false)
    assertEndsWith("İİİ", "i̇i̇", "UTF8_LCASE", expected = true)
    assertEndsWith("İİİ", "ii̇", "UTF8_LCASE", expected = false)
    assertEndsWith("İi̇İ", "İi̇", "UTF8_LCASE", expected = true)
    assertEndsWith("i̇İi̇i̇", "\u0307İi̇İ", "UTF8_LCASE", expected = true) // != UNICODE_CI

    assertEndsWith("i̇İi̇i̇", "\u0307İİ", "UTF8_LCASE", expected = false)
    assertEndsWith("the i\u0307o", "io", "UTF8_LCASE", expected = false)
    assertEndsWith("the i\u0307o", "Io", "UTF8_LCASE", expected = false)
    assertEndsWith("the i\u0307o", "i\u0307o", "UTF8_LCASE", expected = true)
    assertEndsWith("the i\u0307o", "İo", "UTF8_LCASE", expected = true)
    assertEndsWith("the İo", "io", "UTF8_LCASE", expected = false)
    assertEndsWith("the İo", "Io", "UTF8_LCASE", expected = false)
    assertEndsWith("the İo", "i\u0307o", "UTF8_LCASE", expected = true)
    assertEndsWith("the İo", "İo", "UTF8_LCASE", expected = true)
    assertEndsWith("İo", "İo", "UTF8_LCASE", expected = true)
    assertEndsWith("İo", "i̇o", "UTF8_LCASE", expected = true)
    // Conditional case mapping (e.g. Greek sigmas).
    assertEndsWith("σ", "σ", "UTF8_BINARY", expected = true)
    assertEndsWith("σ", "ς", "UTF8_BINARY", expected = false)
    assertEndsWith("σ", "Σ", "UTF8_BINARY", expected = false)
    assertEndsWith("ς", "σ", "UTF8_BINARY", expected = false)
    assertEndsWith("ς", "ς", "UTF8_BINARY", expected = true)
    assertEndsWith("ς", "Σ", "UTF8_BINARY", expected = false)
    assertEndsWith("Σ", "σ", "UTF8_BINARY", expected = false)
    assertEndsWith("Σ", "ς", "UTF8_BINARY", expected = false)
    assertEndsWith("Σ", "Σ", "UTF8_BINARY", expected = true)
    assertEndsWith("σ", "σ", "UTF8_LCASE", expected = true)
    assertEndsWith("σ", "ς", "UTF8_LCASE", expected = true)
    assertEndsWith("σ", "Σ", "UTF8_LCASE", expected = true)
    assertEndsWith("ς", "σ", "UTF8_LCASE", expected = true)
    assertEndsWith("ς", "ς", "UTF8_LCASE", expected = true)
    assertEndsWith("ς", "Σ", "UTF8_LCASE", expected = true)
    assertEndsWith("Σ", "σ", "UTF8_LCASE", expected = true)
    assertEndsWith("Σ", "ς", "UTF8_LCASE", expected = true)
    assertEndsWith("Σ", "Σ", "UTF8_LCASE", expected = true)
    assertEndsWith("σ", "σ", "UNICODE", expected = true)
    assertEndsWith("σ", "ς", "UNICODE", expected = false)
    assertEndsWith("σ", "Σ", "UNICODE", expected = false)
    assertEndsWith("ς", "σ", "UNICODE", expected = false)
    assertEndsWith("ς", "ς", "UNICODE", expected = true)
    assertEndsWith("ς", "Σ", "UNICODE", expected = false)
    assertEndsWith("Σ", "σ", "UNICODE", expected = false)
    assertEndsWith("Σ", "ς", "UNICODE", expected = false)
    assertEndsWith("Σ", "Σ", "UNICODE", expected = true)
    assertEndsWith("σ", "σ", "UNICODE_CI", expected = true)
    assertEndsWith("σ", "ς", "UNICODE_CI", expected = true)
    assertEndsWith("σ", "Σ", "UNICODE_CI", expected = true)
    assertEndsWith("ς", "σ", "UNICODE_CI", expected = true)
    assertEndsWith("ς", "ς", "UNICODE_CI", expected = true)
    assertEndsWith("ς", "Σ", "UNICODE_CI", expected = true)
    assertEndsWith("Σ", "σ", "UNICODE_CI", expected = true)
    assertEndsWith("Σ", "ς", "UNICODE_CI", expected = true)
    assertEndsWith("Σ", "Σ", "UNICODE_CI", expected = true)
    assertEndsWith("ΣΑΛΑΤΑ", "Σ", "UTF8_BINARY", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "σ", "UTF8_BINARY", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "ς", "UTF8_BINARY", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UTF8_BINARY", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UTF8_BINARY", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UTF8_BINARY", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "Σ", "UTF8_LCASE", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "σ", "UTF8_LCASE", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "ς", "UTF8_LCASE", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UTF8_LCASE", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UTF8_LCASE", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UTF8_LCASE", expected = true)
    assertEndsWith("ΣΑΛΑΤΑ", "Σ", "UNICODE", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "σ", "UNICODE", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "ς", "UNICODE", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UNICODE", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UNICODE", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UNICODE", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "Σ", "UNICODE_CI", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "σ", "UNICODE_CI", expected = false)
    assertEndsWith("ΣΑΛΑΤΑ", "ς", "UNICODE_CI", expected = false)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "Σ", "UNICODE_CI", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "σ", "UNICODE_CI", expected = true)
    assertEndsWith("ΘΑΛΑΣΣΙΝΟΣ", "ς", "UNICODE_CI", expected = true)
    // Surrogate pairs.
    assertEndsWith("a🙃b🙃c", "x", "UTF8_BINARY", expected = false)
    assertEndsWith("a🙃b🙃c", "x", "UTF8_LCASE", expected = false)
    assertEndsWith("a🙃b🙃c", "x", "UNICODE", expected = false)
    assertEndsWith("a🙃b🙃c", "x", "UNICODE_CI", expected = false)
    assertEndsWith("a🙃b🙃c", "b", "UTF8_BINARY", expected = false)
    assertEndsWith("a🙃b🙃c", "b", "UTF8_LCASE", expected = false)
    assertEndsWith("a🙃b🙃c", "b", "UNICODE", expected = false)
    assertEndsWith("a🙃b🙃c", "b", "UNICODE_CI", expected = false)
    assertEndsWith("a🙃b🙃c", "a🙃b", "UTF8_BINARY", expected = false)
    assertEndsWith("a🙃b🙃c", "a🙃b", "UTF8_LCASE", expected = false)
    assertEndsWith("a🙃b🙃c", "a🙃b", "UNICODE", expected = false)
    assertEndsWith("a🙃b🙃c", "a🙃b", "UNICODE_CI", expected = false)
    assertEndsWith("a🙃b🙃c", "b🙃c", "UTF8_BINARY", expected = true)
    assertEndsWith("a🙃b🙃c", "b🙃c", "UTF8_LCASE", expected = true)
    assertEndsWith("a🙃b🙃c", "b🙃c", "UNICODE", expected = true)
    assertEndsWith("a🙃b🙃c", "b🙃c", "UNICODE_CI", expected = true)
    assertEndsWith("a🙃b🙃c", "a🙃b🙃c", "UTF8_BINARY", expected = true)
    assertEndsWith("a🙃b🙃c", "a🙃b🙃c", "UTF8_LCASE", expected = true)
    assertEndsWith("a🙃b🙃c", "a🙃b🙃c", "UNICODE", expected = true)
    assertEndsWith("a🙃b🙃c", "a🙃b🙃c", "UNICODE_CI", expected = true)
    assertEndsWith("😀😆😃😄", "😄😆", "UTF8_BINARY", expected = false)
    assertEndsWith("😀😆😃😄", "😄😆", "UTF8_LCASE", expected = false)
    assertEndsWith("😀😆😃😄", "😄😆", "UNICODE", expected = false)
    assertEndsWith("😀😆😃😄", "😄😆", "UNICODE_CI", expected = false)
    assertEndsWith("😀😆😃😄", "😆😃", "UTF8_BINARY", expected = false)
    assertEndsWith("😀😆😃😄", "😆😃", "UTF8_LCASE", expected = false)
    assertEndsWith("😀😆😃😄", "😆😃", "UNICODE", expected = false)
    assertEndsWith("😀😆😃😄", "😆😃", "UNICODE_CI", expected = false)
    ...
    assertEndsWith("😀😆😃😄", "😀😆", "UNICODE", expected = false)
    assertEndsWith("😀😆😃😄", "😀😆", "UNICODE_CI", expected = false)
    assertEndsWith("😀😆😃😄", "😃😄", "UTF8_BINARY", expected = true)
    assertEndsWith("😀😆😃😄", "😃😄", "UTF8_LCASE", expected = true)
    assertEndsWith("😀😆😃😄", "😃😄", "UNICODE", expected = true)
    assertEndsWith("😀😆😃😄", "😃😄", "UNICODE_CI", expected = true)
    assertEndsWith("😀😆😃😄", "😀😆😃😄", "UTF8_BINARY", expected = true)
    assertEndsWith("😀😆😃😄", "😀😆😃😄", "UTF8_LCASE", expected = true)
    assertEndsWith("😀😆😃😄", "😀😆😃😄", "UNICODE", expected = true)
    assertEndsWith("😀😆😃😄", "😀😆😃😄", "UNICODE_CI", expected = true)
    assertEndsWith("𐐅", "𐐅", "UTF8_BINARY", expected = true)
    assertEndsWith("𐐅", "𐐅", "UTF8_LCASE", expected = true)
    assertEndsWith("𐐅", "𐐅", "UNICODE", expected = true)
    assertEndsWith("𐐅", "𐐅", "UNICODE_CI", expected = true)
    assertEndsWith("𐐅", "𐐭", "UTF8_BINARY", expected = false)
    assertEndsWith("𐐅", "𐐭", "UTF8_LCASE", expected = true)
    assertEndsWith("𐐅", "𐐭", "UNICODE", expected = false)
    assertEndsWith("𐐅", "𐐭", "UNICODE_CI", expected = true)
    assertEndsWith("𝔸", "𝔸", "UTF8_BINARY", expected = true)
    assertEndsWith("𝔸", "𝔸", "UTF8_LCASE", expected = true)
    assertEndsWith("𝔸", "𝔸", "UNICODE", expected = true)
    assertEndsWith("𝔸", "𝔸", "UNICODE_CI", expected = true)
  }

  test("string split sql") {
    // Possible splits
    val empty_match = Array(UTF8String.fromString(""))
    val array_abc = Array(UTF8String.fromString("abc"))
    val array_1a2 = Array(UTF8String.fromString("1a2"))
    val array_AaXbB = Array(UTF8String.fromString("AaXbB"))
    val array_aBcDe = Array(UTF8String.fromString("aBcDe"))
    val array_special = Array(UTF8String.fromString("äb世De"))
    val array_abcde = Array(UTF8String.fromString("äbćδe"))
    val full_match = Array(UTF8String.fromString(""), UTF8String.fromString(""))
    val array_1_2 = Array(UTF8String.fromString("1"), UTF8String.fromString("2"))
    val array_A_B = Array(UTF8String.fromString("A"), UTF8String.fromString("B"))
    val array_a_e = Array(UTF8String.fromString("ä"), UTF8String.fromString("e"))
    val array_Aa_bB = Array(UTF8String.fromString("Aa"), UTF8String.fromString("bB"))
    val array_Turkish_uppercase_dotted_I = Array(UTF8String.fromString("İ"))
    val array_Turkish_lowercase_dotted_i = Array(UTF8String.fromString("i\u0307"))
    val array_i = Array(UTF8String.fromString("i"), UTF8String.fromString(""))
    val array_dot = Array(UTF8String.fromString(""), UTF8String.fromString("\u0307"))
    val array_AiB = Array(UTF8String.fromString("Ai\u0307B"))
    val array_AIB = Array(UTF8String.fromString("AİB"))
    val array_small_nonfinal_sigma = Array(UTF8String.fromString("σ"))
    val array_small_final_sigma = Array(UTF8String.fromString("ς"))
    val array_capital_sigma = Array(UTF8String.fromString("Σ"))
    val array_a_b_c = Array(
      UTF8String.fromString("a"), UTF8String.fromString("b"), UTF8String.fromString("c"))
    val array_emojis = Array(UTF8String.fromString("😀"), UTF8String.fromString("😄"))
    val array_AOB = Array(UTF8String.fromString("A𐐅B"))
    val array_AoB = Array(UTF8String.fromString("A𐐭B"))
    // Empty strings.
  ...
  test("init cap") {
    ...
    assertInitCap(UTF8String.fromBytes(Array[Byte](0xC0.toByte, 0xC1.toByte, 0xF5.toByte)).toString,
      "UTF8_BINARY", "���", "���")
    assertInitCap(UTF8String.fromBytes(
      Array[Byte](0xC0.toByte, 0xC1.toByte, 0xF5.toByte, 0x20.toByte, 0x61.toByte,
        0x41.toByte, 0xC0.toByte)).toString,
      "UTF8_BINARY",
      "��� Aa�", "��� Aa�")
    assertInitCap(UTF8String.fromBytes(Array[Byte](0xC2.toByte, 0xC2.toByte)).toString,
      "UTF8_BINARY", "��", "��")
    assertInitCap(UTF8String.fromBytes(
      Array[Byte](0x61.toByte, 0x41.toByte, 0xC2.toByte, 0xC2.toByte, 0x41.toByte)).toString,
      "UTF8_BINARY",
      "Aa��a", "Aa��a")
  }
  ...
  test("find in set") {
    ...
    assertFindInSet("C", UTF8String.fromBytes(
      Array[Byte]( 0x41.toByte, 0xC2.toByte, 0x2C.toByte, 0x42.toByte, 0x2C.toByte, 0x43.toByte,
        0x2C.toByte, 0x43.toByte, 0x2C.toByte, 0x56.toByte )),
      "UTF8_BINARY", 3)
    assertFindInSet("c", UTF8String.fromBytes(
      Array[Byte]( 0x41.toByte, 0xC2.toByte, 0x2C.toByte, 0x42.toByte, 0x2C.toByte, 0x43.toByte,
        0x2C.toByte, 0x43.toByte, 0x2C.toByte, 0x56.toByte )),
