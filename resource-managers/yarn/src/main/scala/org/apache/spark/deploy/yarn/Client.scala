...
private[spark] class Client(
  ...
  private def createConfArchive(confsToOverride: Map[String, String]): File = {
    ...
    try {
      ...
      // Upload $SPARK_CONF_DIR/log4j2.properties file to the distributed cache to make sure that
      // the executors will use the latest configurations instead of the default values. This is
      // required when user changes log4j2.properties directly to set the log configurations. If
