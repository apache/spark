...
private[spark] class Client(
  ...
  private val amKeytabFileName: Option[String] = if (keytab != null && isClusterMode) {
    ...
    logInfo(log"Kerberos credentials: principal = ${MDC(LogKey.PRINCIPAL, principal)}, " +
    ...
    def cleanupStagingDirInternal(): Unit = {
      try {
        ...
        if (fs.delete(stagingDirPath, true)) {
          logInfo(log"Deleted staging directory ${MDC(LogKey.PATH, stagingDirPath)}")
        }
      } catch {
        case ioe: IOException =>
          logWarning(log"Failed to cleanup staging dir ${MDC(LogKey.PATH, stagingDirPath)}", ioe)
      }
    }
  ...
  def createApplicationSubmissionContext(
    ...
    sparkConf.get(ROLLED_LOG_INCLUDE_PATTERN).foreach { includePattern =>
      try {
        ...
        case NonFatal(e) =>
          logWarning(log"Ignoring ${MDC(LogKey.CONFIG, ROLLED_LOG_INCLUDE_PATTERN.key)}} " +
  ...
  private def verifyClusterResources(newAppResponse: GetNewApplicationResponse): Unit = {
    ...
    logInfo(log"Verifying our application has not requested more than the maximum memory " +
      log"capability of the cluster (${MDC(LogKey.MAX_MEMORY_SIZE, maxMem)} MB per container)")
    val executorMem =
    ...
    logInfo(log"Will allocate AM container, with ${MDC(LogKey.MEMORY_SIZE, amMem)} MB memory " +
      log"including ${MDC(LogKey.OVERHEAD_MEMORY_SIZE, amMemoryOverhead)} MB overhead")
        ...
        case None =>
          ...
          logWarning(
            log"Neither ${MDC(LogKey.CONFIG, SPARK_JARS.key)} nor " +
              log"${MDC(LogKey.CONFIG2, SPARK_ARCHIVE.key)}} is set, falling back to uploading " +
    ...
    confDirs.foreach { envKey =>
      sys.env.get(envKey).foreach { path =>
        ...
        if (dir.isDirectory()) {
          ...
          if (files == null) {
            logWarning(log"Failed to list files under directory ${MDC(LogKey.PATH, dir)}")
          } else {
            files.foreach { file =>
  ...
  private def createContainerLaunchContext(): ContainerLaunchContext = {
    ...
    if (isClusterMode) {
      ...
      if (sparkConf.get(AM_JAVA_OPTIONS).isDefined) {
        logWarning(log"${MDC(LogKey.CONFIG, AM_JAVA_OPTIONS.key)} will not take effect " +
          log"in cluster mode")
      }
    } else {
  ...
  private def getDriverLogsLink(appReport: ApplicationReport): IMap[String, String] = {
    ...
    try {
      ...
      case e: Exception =>
        logWarning(log"Unable to get driver log links for ${MDC(LogKey.APP_ID, appId)}: ", e)
