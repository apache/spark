  ...
  private[spark] val AM_TRUST_PROXY_USER_COOKIE =
    ConfigBuilder("spark.yarn.am.trustProxyUserCookie")
      .doc("When true (default), the YARN AM UI filter uses the 'proxy-user' cookie set by the " +
        ...
        "separate authentication filter set in spark.ui.filters runs after it and supplies the " +
        "real user. That filter must reject unauthenticated requests, not merely wrap them: one " +
        "that wraps the request but leaves the user null (as Hadoop's AuthenticationFilter does " +
        "for anonymous requests) replaces the sentinel with a null user, and a null user passes " +
        "every ACL check. In cluster mode the AM replaces spark.ui.filters with its own filter, " +
        "so no other filter runs and, with the cookie not trusted, all proxied requests are " +
        "denied while ACLs are enabled. Only set it to false in client mode together with such " +
        "an authentication filter that rejects unauthenticated requests.")
      .version("4.3.0")
      .booleanConf
      .createWithDefault(true)
