...
private[spark] object Config extends Logging {
  ...
  val KUBERNETES_JOB_MIN_CPU = ConfigBuilder("spark.kubernetes.job.min.cpu")
    .doc("The minimum CPU for running the job. This only applicable when you enable " +
      "`VolcanoFeatureStep` feature step in `spark.kubernetes.driver.pod.featureSteps`.")
    .version("3.3.0")
    .doubleConf
