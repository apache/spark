...
import io.fabric8.kubernetes.client.dsl.{FilterWatchListDeletable, MixedOperation, PodResource}
...
class KubernetesClusterSchedulerBackendSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  @Mock
  private var lifecycleEventHandler: ExecutorPodsLifecycleEventHandler = _
  ...
  test("Stop all components") {
    ...
    verify(eventQueue).stopProcessingEvents()
