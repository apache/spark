...
class ExecutorPodsAllocatorSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  test("backoff: enters on pod failures and creation timeout, " +
    ...
    podsAllocatorUnderTest = new ExecutorPodsAllocator(
      backoffConf, secMgr, executorBuilder, kubernetesClient, snapshotsStore,
      waitForExecutorPodsClock)
    podsAllocatorUnderTest.start(TEST_SPARK_APP_ID, schedulerBackend)

    val registry = podsAllocatorUnderTest.metricsSource.get.metricRegistry
    val normalGauge = registry.getGauges.get("backoffState.Normal")
    val backoffGauge = registry.getGauges.get("backoffState.Backoff")
    assert(normalGauge.getValue == 1)
    assert(backoffGauge.getValue == 0)
    ...
    assert(normalGauge.getValue == 0)
    assert(backoffGauge.getValue == 1)
    assert(podsAllocatorUnderTest.invokePrivate(numOutstandingPods).get() == 1) // pending exec 5
