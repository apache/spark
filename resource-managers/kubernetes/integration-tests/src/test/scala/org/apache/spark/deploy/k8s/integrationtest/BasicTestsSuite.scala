...
private[spark] trait BasicTestsSuite { k8sSuite: KubernetesSuite =>
  ...
  ignore("SPARK-42190: Run SparkPi with local[*]", k8sTestTag) {
  ...
  ignore("Run SparkPi with no resources", k8sTestTag) {
    runSparkPiAndVerifyCompletion()
  }

  ignore("Run SparkPi with no resources & statefulset allocation", k8sTestTag) {
  ...
  ignore("Run SparkPi with a very long application name.", k8sTestTag) {
    sparkAppConf.set("spark.app.name", "long" * 40)
    runSparkPiAndVerifyCompletion()
  }

  ignore("Use SparkLauncher.NO_RESOURCE", k8sTestTag) {
    sparkAppConf.setJars(Seq(containerLocalSparkDistroExamplesJar))
    runSparkPiAndVerifyCompletion(
      appResource = SparkLauncher.NO_RESOURCE)
  }

  ignore("Run SparkPi with a master URL without a scheme.", k8sTestTag) {
  ...
  ignore("Run SparkPi with an argument.", k8sTestTag) {
  ...
  ignore("Run SparkPi with custom labels, annotations, and environment variables.", k8sTestTag) {
    sparkAppConf
  ...
  ignore("All pods have the same service account by default", k8sTestTag) {
    runSparkPiAndVerifyCompletion(
      executorPodChecker = (executorPod: Pod) => {
  ...
  ignore("Run extraJVMOptions check on driver", k8sTestTag) {
    sparkAppConf
  ...
  ignore("SPARK-42474: Run extraJVMOptions JVM GC option check - G1GC", k8sTestTag) {
    sparkAppConf
  ...
  ignore("SPARK-42474: Run extraJVMOptions JVM GC option check - Other GC", k8sTestTag) {
    sparkAppConf
  ...
  ignore("Run SparkRemoteFileTest using a remote data file", k8sTestTag, localTestTag) {
  ...
  ignore("SPARK-42769: All executor pods have SPARK_DRIVER_POD_IP env variable", k8sTestTag) {
    runSparkPiAndVerifyCompletion(
      executorPodChecker = (executorPod: Pod) => {
        doBasicExecutorPodCheck(executorPod)
        assert {
