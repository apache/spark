...
package object config {
  ...
  private[spark] class MesosSecretConfig private[config](taskType: String) {
    private[spark] val SECRET_NAMES =
      buildConf(s"spark.mesos.$taskType.secret.names")
    ...
    private[spark] val SECRET_VALUES =
      buildConf(s"spark.mesos.$taskType.secret.values")
    ...
    private[spark] val SECRET_ENVKEYS =
      buildConf(s"spark.mesos.$taskType.secret.envkeys")
    ...
    private[spark] val SECRET_FILENAMES =
      buildConf(s"spark.mesos.$taskType.secret.filenames")
  ...
  private[spark] val SHUFFLE_CLEANER_INTERVAL_S =
    buildConf("spark.shuffle.cleaner.interval")
      .timeConf(TimeUnit.SECONDS)
      .createWithDefaultString("30s")

  private[spark] val RECOVERY_MODE =
    buildConf("spark.deploy.recoveryMode")
      .stringConf
      .createWithDefault("NONE")

  private[spark] val DISPATCHER_WEBUI_URL =
    buildConf("spark.mesos.dispatcher.webui.url")
  ...
  private[spark] val ZOOKEEPER_URL =
    buildConf("spark.deploy.zookeeper.url")
  ...
  private[spark] val HISTORY_SERVER_URL =
    buildConf("spark.mesos.dispatcher.historyServer.url")
  ...
  private[spark] val DRIVER_LABELS =
    buildConf("spark.mesos.driver.labels")
  ...
  private[spark] val DRIVER_FAILOVER_TIMEOUT =
    buildConf("spark.mesos.driver.failoverTimeout")
  ...
  private[spark] val NETWORK_NAME =
    buildConf("spark.mesos.network.name")
  ...
  private[spark] val NETWORK_LABELS =
    buildConf("spark.mesos.network.labels")
  ...
  private[spark] val DRIVER_CONSTRAINTS =
    buildConf("spark.mesos.driver.constraints")
