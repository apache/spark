...
test_that("structType and structField", {
  ...
  expect_error(structType("A stri"), ".*Syntax error at or near 'stri'.*")
})
...
test_that("Specify a schema by using a DDL-formatted string when reading", {
  ...
  expect_error(read.df(jsonPath, "json", "name stri"), ".*Syntax error at or near 'stri'.*")
  ...
  expect_error(loadDF(jsonPath, "json", "name stri"), ".*Syntax error at or near 'stri'.*")
})
