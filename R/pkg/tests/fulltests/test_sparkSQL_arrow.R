...
test_that("createDataFrame/collect Arrow optimization", {
  ...
  arrowEnabled <- sparkR.conf("spark.sql.sparkr.execution.arrow.enabled")[[1]]

  callJMethod(conf, "set", "spark.sql.sparkr.execution.arrow.enabled", "false")
  tryCatch({
    expected <- collect(createDataFrame(mtcars))
  },
  finally = {
    callJMethod(conf, "set", "spark.sql.sparkr.execution.arrow.enabled", arrowEnabled)
  })

  callJMethod(conf, "set", "spark.sql.sparkr.execution.arrow.enabled", "true")
  tryCatch({
