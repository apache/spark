...
class v09DirectKafkaStreamSuite
  ...
  test("offset recovery") {
    ...
    def sendDataAndWaitForReceive(data: Seq[Int]) {
      ...
      eventually(timeout(10 seconds), interval(50 milliseconds)) {
        assert(strings.forall {
          v09DirectKafkaStreamSuite.collectedData.contains
        })
      }
    }
    ...
    kafkaStream.foreachRDD { (rdd: RDD[(String, String)], time: Time) =>
      ...
      v09DirectKafkaStreamSuite.collectedData.appendAll(data)
    }

    // This is ensure all the data is eventually receiving only once
    stateStream.foreachRDD { (rdd: RDD[(String, Int)]) =>
      rdd.collect().headOption.foreach { x => v09DirectKafkaStreamSuite.total = x._2 }
    }
    ssc.start()
    ...
    eventually(timeout(10 seconds), interval(50 milliseconds)) {
      assert(v09DirectKafkaStreamSuite.total === (1 to 20).sum)
    }
    ssc.stop()
  }
  ...
  test("Direct Kafka stream report input information") {
    ...
    import v09DirectKafkaStreamSuite._
    ssc = new StreamingContext(sparkConf, Milliseconds(200))
...
object v09DirectKafkaStreamSuite {
  val collectedData = new mutable.ArrayBuffer[String]() with mutable.SynchronizedBuffer[String]
