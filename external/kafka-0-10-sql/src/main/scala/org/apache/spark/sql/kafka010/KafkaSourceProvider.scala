...
with DataSourceRegister with Logging {
  ...
  private def validateOptions(parameters: Map[String, String]): Unit = {
    ...
    if (caseInsensitiveParams.contains(s"kafka.${ConsumerConfig.AUTO_OFFSET_RESET_CONFIG}")) {
      val supportedValues = Set("earliest", "latest")
      val value = caseInsensitiveParams(s"kafka.${ConsumerConfig.AUTO_OFFSET_RESET_CONFIG}")
      if (!supportedValues.contains(value)) {
        throw new IllegalArgumentException(s"Value $value for Kafka option " +
          s"'${ConsumerConfig.AUTO_OFFSET_RESET_CONFIG}' is not supported")
      }
    }
