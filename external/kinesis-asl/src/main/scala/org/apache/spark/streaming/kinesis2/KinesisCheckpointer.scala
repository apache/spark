 ...
 */
private[kinesis2] class KinesisCheckpointer(receiver: KinesisReceiver[_],
   ...
   *
   * If a checkpointer is provided, e.g. on IRecordProcessor.shutdown [[ShutdownReason]],
   * we will use that to make the final checkpoint. If `null` is provided, we will not make the
   * checkpoint, e.g. in case of [[ShutdownReason]].
   */
