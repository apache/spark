 ...
 */
private[kinesis2] class KinesisTestUtils(streamShardCount: Int = 2) extends Logging {

  val endpointUrl = new URI(KinesisTestUtils.endpointUrl)
  ...
  def createStream(): Unit = {
    ...
    val createStreamRequest = CreateStreamRequest.builder()
      ...
      .shardCount(streamShardCount).build()
  ...
  def getShards(): Seq[Shard] = {
    val stream = DescribeStreamRequest.builder().streamName(_streamName).build()
    kinesisClient.describeStream(stream).streamDescription().shards().asScala
  }
  ...
  def splitShard(shardId: String): Unit = {
    val splitShardRequest = SplitShardRequest.builder()
      ...
      .newStartingHashKey("170141183460469231731687303715884105728").build()
  ...
  def mergeShard(shardToMerge: String, adjacentShardToMerge: String): Unit = {
    val mergeShardRequest = MergeShardsRequest.builder()
      ...
      .adjacentShardToMerge(adjacentShardToMerge).build()
  ...
  def deleteStream(): Unit = {
    try {
      if (streamCreated) {
        val deleteStream = DeleteStreamRequest.builder().streamName(streamName).build()
        kinesisClient.deleteStream(deleteStream)
      }
    } catch {
      case e: Exception =>
  ...
  def deleteDynamoDBTable(tableName: String): Unit = {
    try {
      val delTableReq = DeleteTableRequest.builder().tableName(tableName).build()
      val table = dynamoDB.deleteTable(delTableReq)
    } catch {
      case e: Exception =>
...
private[kinesis2] object KinesisTestUtils {
  ...
  def getRegionNameByEndpoint(endpoint: URI): String = {

    RegionUtils.getRegionsForService(endpoint.toString)
      .asScala
      .find(_.getAvailableEndpoints.asScala.toSeq.contains(endpoint.getHost))
      .map(_.getName)
      .getOrElse(
  ...
  def getAWSCredentials(): AwsCredentialsProvider = {
    ...
    Try {
      SparkAWSCredentials.builder.basicCredentials("xxx", "xxx").build().provider
    } match {
...
private[kinesis2] trait KinesisDataGenerator {
...
private[kinesis2] class SimpleDataGenerator(client: KinesisClient) extends KinesisDataGenerator {
  override def sendData(streamName: String, data: Seq[Int]): Map[String, Seq[(Int, String)]] = {
    ...
    data.foreach { num =>
      ...
      val data = SdkBytes.fromString(str, StandardCharsets.UTF_8)
