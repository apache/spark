 ...
 */
private[kinesis2] sealed trait SparkAWSCredentials extends Serializable {
  /**
...
private[kinesis2] final case object DefaultCredentials extends SparkAWSCredentials {

  val credentials = AwsBasicCredentials.create("xxx", "xxx")

  def provider: AwsCredentialsProvider = StaticCredentialsProvider.create(credentials)
}
...
object SparkAWSCredentials {
  ...
  class Builder {
     ...
     */
    def build(): SparkAWSCredentials = longLivedCreds
