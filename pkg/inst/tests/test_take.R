...
test_that("take() gives back the original elements in correct count and order", {
  ...
  expect_equal(take(numVectorRRDD, 1), head(as.list(numVector), n = 1))
  expect_equal(take(numVectorRRDD, 3), head(as.list(numVector), n = 3))
  ...
  expect_equal(take(numListRRDD, 1), head(as.list(numList), n = 1))
  expect_equal(take(numListRRDD2, 999), as.list(numList))
  ...
  expect_equal(take(strVectorRRDD2, 2), head(as.list(strVector), n = 2))

  strListRRDD <- parallelize(jsc, strList, 4)
  strListRRDD2 <- parallelize(jsc, strList, 1)
  expect_equal(take(strListRRDD, 3), head(as.list(strList), n = 3))
  expect_equal(take(strListRRDD2, 1), head(as.list(strList), n = 1))
