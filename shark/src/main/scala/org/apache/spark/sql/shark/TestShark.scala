...
object TestShark
  extends TestSharkContext(new SparkContext("local", "TestSqlContext", new SparkConf()))
 ...
 * in a "clean" state.
 *
 * TestShark is singleton object version of this class because instantiating multiple copies of the
 * hive metastore seems to lead to weird non-deterministic failures.  Therefore, the execution of
 * testcases that rely on TestShark must be serialized.
 */
class TestSharkContext(sc: SparkContext) extends SharkContext(sc) {
  self =>
  ...
  val inRepoTests = new File("shark/src/test/hive/")
