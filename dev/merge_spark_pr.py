...
def choose_jira_assignee(issue, asf_jira):
    ...
    including the original reporter and all commenters
    """
    while True:
        try:
            reporter = issue.fields.reporter
            commenters = list(map(lambda x: x.author, issue.fields.comment.comments))
            candidates = set(commenters)
