...
def assign_issue(client: jira.client.JIRA, issue: int, assignee: str) -> bool:
    """
    Assign an issue to a user. Shorthand for jira.client.JIRA.assign_issue, which will searches users again and only
    chooses the assignee from 20 candidates. If it's unmatched, it picks the head blindly.
    In our case, the assignee is already resolved.
    """
