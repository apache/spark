...
EOF
  ...
  REPO_ID=$(curl --retry 10 --retry-connrefused -s -u "$ASF_USERNAME:$ASF_PASSWORD" \
  ...
  curl --retry 10 --retry-connrefused -s -u "$APACHE_USERNAME:$APACHE_PASSWORD" \
  ...
  while true; do
    STATUS=$(curl --retry 10 --retry-connrefused -s -u "$APACHE_USERNAME:$APACHE_PASSWORD" \
...
if [[ "$1" == "publish-release" ]]; then
  ...
  if ! is_dry_run; then
    ...
    out=$(curl --retry 10 --retry-connrefused --fail -X POST -d "$repo_request" -u $ASF_USERNAME:$ASF_PASSWORD \
  ...
  if ! is_dry_run; then
    ...
    xargs -P 4 -n 1 -I {} bash -c '
      ...
      if curl --retry 10 --retry-connrefused --fail -u "$ASF_USERNAME:$ASF_PASSWORD" \
