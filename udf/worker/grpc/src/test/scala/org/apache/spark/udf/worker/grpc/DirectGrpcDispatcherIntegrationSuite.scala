...
// scalastyle:on funsuite
  ...
  test("responses larger than gRPC's default inbound limit round-trip successfully") {
    newDispatcher()
    ...
    withSession { session =>
      ...
      val results = session.process(Iterator.single(input), emptyFinish).toList
      assert(results.map(_.getData) == List(payload))
    }
  }
  ...
  test("a socket path alone does not make a worker ready") {
    ...
    val runner = ProcessCallable.newBuilder()
      .addCommand("bash").addCommand("-c").addCommand(socketOnlyWorker).addCommand("--")
      .build()
    val properties = UDFWorkerProperties.newBuilder()
      .setConnection(WorkerConnectionSpec.newBuilder()
        .setUnixDomainSocket(UnixDomainSocket.getDefaultInstance)
        .build())
      .setInitializationTimeoutMs(1000)
      .setGracefulTerminationTimeoutMs(1000)
      .build()
    val spec = UDFWorkerSpecification.newBuilder()
      .setCapabilities(capabilities)
      .setDirect(DirectWorker.newBuilder().setRunner(runner).setProperties(properties).build())
      .build()
    dispatcher = new DirectGrpcDispatcher(spec)

    val error = intercept[org.apache.spark.udf.worker.core.direct.DirectWorkerTimeoutException] {
    ...
    assert(error.getMessage.contains("socket-created"),
      s"expected worker output in timeout, got: ${error.getMessage}")
  }
