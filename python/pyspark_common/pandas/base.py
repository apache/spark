...
try:
    ...
    from pyspark_connect.sql import functions as F, Column, Window

from pyspark_common.sql.types import LongType, BooleanType, NumericType
from pyspark_common import pandas as ps  # For running doctests and reference resolution in PyCharm.
from pyspark_common.pandas._typing import Axis, Dtype, IndexOpsLike, Label, SeriesOrIndex
from pyspark_common.pandas.config import get_option, option_context
...
if TYPE_CHECKING:
    ...
    from pyspark_common.pandas.data_type_ops.base import DataTypeOps
...
class IndexOpsMixin(object, metaclass=ABCMeta):
    ...
    def __array_ufunc__(
        ...
        from pyspark_common.pandas import numpy_compat
    ...
    def isnull(self: IndexOpsLike) -> IndexOpsLike:
        ...
        """
        from pyspark_common.pandas.indexes import MultiIndex
        ...
        if max_compute_count is not None:
            ...
            if len(uniq_pdf) > max_compute_count:
                raise ValueError(
                    ...
                    "Please set 'compute.max_rows' by using 'pyspark_common.pandas.config.set_option' "
...
def _test() -> None:
    ...
    import pyspark_common.pandas.base

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.base.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]").appName("pyspark_common.pandas.base tests").getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.base,
        globs=globs,
