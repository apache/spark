...
from pyspark_common.pandas.data_type_ops.categorical_ops import _to_cat
try:
...
def _test() -> None:
    ...
    import pyspark_common.pandas.categorical

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.categorical.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.categorical tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.categorical,
        globs=globs,
