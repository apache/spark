...
from pyspark_common.pandas.missing.indexes import MissingPandasLikeDatetimeIndex
...
def _test() -> None:
    ...
    import pyspark_common.pandas.indexes.datetimes

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.indexes.datetimes.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.indexes.datetimes tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.indexes.datetimes,
        globs=globs,
