...
from pyspark_common.pandas.internal import InternalField
from pyspark_common.pandas.series import Series
from pyspark_common.sql.types import StructField
...
def _test() -> None:
    ...
    import pyspark_common.pandas.indexes.category

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.indexes.category.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.indexes.category tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.indexes.category,
        globs=globs,
