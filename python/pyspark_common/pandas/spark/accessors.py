...
try:
    ...
    from pyspark_connect.sql import Column as PySparkColumn, DataFrame as PySparkDataFrame

from pyspark_common.storagelevel import StorageLevel
from pyspark_common.sql.types import DataType, StructType
from pyspark_common.pandas._typing import IndexOpsLike
from pyspark_common.pandas.internal import InternalField
# For Supporting Spark Connect
from pyspark_common.sql.utils import get_column_class, get_dataframe_class
...
if TYPE_CHECKING:
    ...
    from pyspark_common.pandas.frame import CachedDataFrame
...
class SparkIndexOpsMethods(Generic[IndexOpsLike], metaclass=ABCMeta):
    ...
    def transform(self, func: Callable[[PySparkColumn], PySparkColumn]) -> IndexOpsLike:
        ...
        Name: a, dtype: int64
        """
        from pyspark_common.pandas import MultiIndex
...
class SparkSeriesMethods(SparkIndexOpsMethods["ps.Series"]):
    def apply(self, func: Callable[[PySparkColumn], PySparkColumn]) -> "ps.Series":
        ...
        from pyspark_common.pandas.internal import HIDDEN_COLUMNS
...
class SparkFrameMethods:
    ...
    def schema(self, index_col: Optional[Union[str, List[str]]] = None) -> StructType:
        ...
        Returns
        -------
        pyspark_common.sql.types.StructType
...
def _test() -> None:
    ...
    import pyspark_common.pandas.spark.accessors

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.spark.accessors.__dict__.copy()
    globs["np"] = numpy
    globs["pd"] = pandas
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.spark.accessors tests")
        .getOrCreate()
    )
    ...
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.spark.accessors,
        globs=globs,
