...
import pyspark_common.pandas.groupby as psg
import pyspark_common.pandas.window as psw
...
.. currentmodule:: pyspark_common.pandas
...
latest/api/python/reference/pyspark_common.pandas/api/pyspark_common.pandas.DataFrame.to_numpy.html>`__
...
def _create_supported_by_module(
    ...
    ps_module_group: Any
        Specific path of importable pyspark_common.pandas module.
    """
...
def _organize_by_implementation_status(
    ...
    ps_funcs: Dict[str, Callable]
        function name and function object mapping of pyspark_common.pandas module.
    pd_module_group : Any
        Specific path of importable pandas module.
    ps_module_group: Any
        Specific path of importable pyspark_common.pandas module.
...
def _transform_missing(
    ...
    ps_module_path : str
        Path string of pyspark_common.pandas module.

    Examples
    --------
    >>> _transform_missing("DataFrame", "add", {"axis", "fill_value", "level"},
    ...                     "pandas.DataFrame", "pyspark_common.pandas.DataFrame")
    ...
    if len(missing_set) > MAX_MISSING_PARAMS_SIZE:
        ...
        additional_str = (
            ...
            + "<https://spark.apache.org/docs/latest/api/python/reference/pyspark_common.pandas/api/"
...
def _test() -> None:
    ...
    import pyspark_common.pandas.supported_api_gen

    globs = pyspark_common.pandas.supported_api_gen.__dict__.copy()
    (failure_count, test_count) = doctest.testmod(pyspark_common.pandas.supported_api_gen, globs=globs)
    if failure_count:
        sys.exit(-1)
