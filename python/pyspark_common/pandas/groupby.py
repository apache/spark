...
try:
    ...
    from pyspark_connect.sql import Column, DataFrame as SparkDataFrame, Window, functions as F
...
from pyspark_common.pandas._typing import Axis, FrameLike, Label, Name
...
from pyspark_common.pandas.exceptions import DataError

if TYPE_CHECKING:
    from pyspark_common.pandas.window import RollingGroupby, ExpandingGroupby, ExponentialMovingGroupby
...
class GroupBy(Generic[FrameLike], metaclass=ABCMeta):
    ...
    def mean(self, numeric_only: Optional[bool] = True) -> FrameLike:
        ...
        Returns
        -------
        pyspark_common.pandas.Series or pyspark_common.pandas.DataFrame
    ...
    def quantile(self, q: float = 0.5, accuracy: int = 10000) -> FrameLike:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.quantile
        pyspark_common.pandas.DataFrame.quantile
    ...
    def sem(self, ddof: int = 1) -> FrameLike:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.sem
        pyspark_common.pandas.DataFrame.sem
        """
        if not isinstance(ddof, int):
            raise TypeError("ddof must be integer")
    ...
    def rolling(
        ...
        DataFrame.groupby
        """
        from pyspark_common.pandas.window import RollingGroupby
    ...
    def expanding(self, min_periods: int = 1) -> "ExpandingGroupby[FrameLike]":
        ...
        DataFrame.groupby
        """
        from pyspark_common.pandas.window import ExpandingGroupby
    ...
    def ewm(
        ...
        """
        from pyspark_common.pandas.window import ExponentialMovingGroupby
...
class SeriesGroupBy(GroupBy[Series]):
    ...
    def nsmallest(self, n: int = 5) -> Series:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.nsmallest
        pyspark_common.pandas.DataFrame.nsmallest
    ...
    def nlargest(self, n: int = 5) -> Series:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.nlargest
        pyspark_common.pandas.DataFrame.nlargest
    ...
    def unique(self) -> Series:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.unique
        pyspark_common.pandas.Index.unique
...
def _test() -> None:
    ...
    import pyspark_common.pandas.groupby

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.groupby.__dict__.copy()
    globs["np"] = numpy
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.groupby tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.groupby,
        globs=globs,
