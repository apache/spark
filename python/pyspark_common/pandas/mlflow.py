...
from pyspark_common.pandas._typing import Label, Dtype
from pyspark_common.pandas.utils import lazy_property, default_session
from pyspark_common.pandas.frame import DataFrame
from pyspark_common.pandas.series import Series, first_series
from pyspark_common.pandas.typedef import as_spark_type
...
def load_model(
    ...
    >>> from pyspark_common.pandas.mlflow import load_model
...
def _test() -> None:
    ...
    import pyspark_common.pandas.mlflow

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.mlflow.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]").appName("pyspark_common.pandas.mlflow tests").getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.mlflow,
        globs=globs,
