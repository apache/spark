...
try:
    ...
    from pyspark_connect.sql import Column, functions as F
...
from pyspark_common.pandas.missing.resample import (
...
from pyspark_common.pandas.spark.functions import timestampdiff
...
class Resampler(Generic[FrameLike], metaclass=ABCMeta):
    ...
    def min(self) -> FrameLike:
        ...
        See Also
        --------
        pyspark_common.pandas.Series.groupby
        pyspark_common.pandas.DataFrame.groupby
...
def _test() -> None:
    ...
    import pyspark_common.pandas.resample

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.resample.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.resample tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.resample,
        globs=globs,
