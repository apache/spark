...
class PandasNotImplementedError(NotImplementedError):
    def __init__(
        ...
        if method_name is not None:
            ...
            msg = (
                "The scalar `{0}.{1}` is not reimplemented in pyspark_common.pandas;"
...
def _test() -> None:
    ...
    import pyspark_common.pandas.exceptions

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.exceptions.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.exceptions tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.exceptions,
        globs=globs,
