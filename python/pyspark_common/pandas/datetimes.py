...
import pyspark_common.pandas as ps
from pyspark_common.sql.types import DateType, TimestampType, TimestampNTZType, LongType, IntegerType
...
def _test() -> None:
    ...
    import pyspark_common.pandas.datetimes

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.datetimes.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.datetimes tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.datetimes,
        globs=globs,
