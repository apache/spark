...
from pyspark_common._globals import _NoValue, _NoValueType
...
#     >>> from pyspark_common.pandas.config import show_options
#     >>> show_options()
...
def _test() -> None:
    ...
    import pyspark_common.pandas.config

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.config.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]").appName("pyspark_common.pandas.config tests").getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.config,
        globs=globs,
