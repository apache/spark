...
from pyspark_common.sql.types import DoubleType, LongType, BooleanType
...
def _test() -> None:
    ...
    import pyspark_common.pandas.numpy_compat

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.numpy_compat.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.numpy_compat tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.numpy_compat,
        globs=globs,
