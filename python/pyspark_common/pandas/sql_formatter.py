...
def _test() -> None:
    ...
    import sys
    from pyspark.sql import SparkSession
    import pyspark_common.pandas.sql_formatter

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.sql_formatter.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.sql_formatter tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.sql_formatter,
        globs=globs,
