...
from pyspark_common.sql.types import StringType, BinaryType, ArrayType, LongType, MapType
...
def _test() -> None:
    ...
    import pyspark_common.pandas.strings

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.strings.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    spark = (
        SparkSession.builder.master("local[4]")
        .appName("pyspark_common.pandas.strings tests")
        .getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.strings,
        globs=globs,
