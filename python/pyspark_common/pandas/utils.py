...
try:
    ...
    from pyspark_connect.sql import functions as F, Column, DataFrame as PySparkDataFrame, SparkSession

from pyspark_common.sql.types import DoubleType
from pyspark_common.sql.utils import is_remote, get_dataframe_class
...
def _test() -> None:
    ...
    import pyspark_common.pandas.utils

    os.chdir(os.environ["SPARK_HOME"])

    globs = pyspark_common.pandas.utils.__dict__.copy()
    globs["ps"] = pyspark_common.pandas
    globs["sf"] = F
    spark = (
        SparkSession.builder.master("local[4]").appName("pyspark_common.pandas.utils tests").getOrCreate()
    )
    (failure_count, test_count) = doctest.testmod(
        pyspark_common.pandas.utils,
        globs=globs,
