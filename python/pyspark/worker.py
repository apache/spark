...
# checks). Historically each eval type spelled this out inline in ``read_udfs``,
# growing a single central if/elif dispatcher. ``EvalTypeHandler`` makes the
# lifecycle explicit and lets a new eval type attach itself as a self-contained
# subclass -- no edit to a central branch is required.
#
# A handler declares its ``eval_type`` and is registered automatically via
# ``__init_subclass__``; ``read_udfs`` looks it up in ``_EVAL_TYPE_HANDLERS`` and,
# when present, delegates to ``handler.run`` instead of walking the if/elif chain.
# ---------------------------------------------------------------------------
