...
class ArrowToPandasConversionTests(unittest.TestCase):
    def test_convert_column_numpy_ser_name_survives_preprocess_time(self):
        # _convert_column_numpy reads the Arrow field name before preprocess_time, because the
        ...
        for pa_type in [pa.timestamp("us", tz="UTC"), pa.timestamp("s"), pa.timestamp("ns")]:
            ...
            result = ArrowToPandasConversion._convert_column_numpy(col, spark_type, timezone="UTC")
            self.assertEqual(result.name, "tscol", f"name lost for {pa_type}")

    def test_udt_convert_column_numpy(self):
        import pyarrow as pa
        ...
        result = ArrowToPandasConversion._convert_column_numpy(arr, udt, ser_name="my_point")
        ...
        # empty
        result = ArrowToPandasConversion._convert_column_numpy(
    ...
    def test_udt_chunked_array(self):
        ...
        result = ArrowToPandasConversion._convert_column_numpy(chunked, ExamplePointUDT())
        self.assertEqual(result.iloc[0], ExamplePoint(1.0, 2.0))
        self.assertEqual(result.iloc[1], ExamplePoint(3.0, 4.0))

    def test_variant_convert_column_numpy(self):
        import pyarrow as pa
        ...
        result = ArrowToPandasConversion._convert_column_numpy(arr, VariantType(), ser_name="v")
    ...
    def test_geography_convert_column_numpy(self):
        import pyarrow as pa
    ...
    def test_geometry_convert_column_numpy(self):
        import pyarrow as pa
        ...
        result = ArrowToPandasConversion._convert_column_numpy(arr, GeometryType(0), ser_name="g")
