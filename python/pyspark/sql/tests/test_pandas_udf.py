...
class PandasUDFTests(ReusedSQLTestCase):
    ...
    def test_pandas_udf_detect_unsafe_type_conversion(self):
        ...
        with self.sql_conf({
                "spark.sql.execution.pandas.arrowSafeTypeConversion.enabled": True}):
        ...
        with self.sql_conf({
                "spark.sql.execution.pandas.arrowSafeTypeConversion.enabled": False}):
            df.select(['A']).withColumn('udf', udf('A')).collect()
    ...
    def test_pandas_udf_arrow_overflow(self):
        ...
        with self.sql_conf({"spark.sql.execution.pandas.arrowSafeTypeConversion.enabled": False}):
            df.withColumn('udf', udf('id')).collect()
