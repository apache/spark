...
class FunctionsTestsMixin:
    ...
    def test_kll_sketch_get_quantile_bigint(self):
        ...
        quantile = sketch_df.select(
            F.kll_sketch_get_quantile_bigint("sketch", F.lit(0.5))
        ).first()[0]
    ...
    def test_kll_sketch_get_quantile_bigint_array(self):
        ...
        quantiles = sketch_df.select(
            F.kll_sketch_get_quantile_bigint("sketch", F.array(F.lit(0.25), F.lit(0.5), F.lit(0.75)))
        ).first()[0]
    ...
    def test_kll_sketch_get_rank_bigint(self):
        ...
        rank = sketch_df.select(
            F.kll_sketch_get_rank_bigint("sketch", F.lit(3))
        ).first()[0]
    ...
    def test_kll_sketch_float_variants(self):
        ...
        merged = sketch_df.select(
            F.kll_sketch_merge_float("sketch", "sketch")
        ).first()[0]
        self.assertIsNotNone(merged)
        
        # Test get_quantile
        quantile = sketch_df.select(
            F.kll_sketch_get_quantile_float("sketch", F.lit(0.5))
        ).first()[0]
        self.assertIsNotNone(quantile)
        
        # Test get_rank
        rank = sketch_df.select(
            F.kll_sketch_get_rank_float("sketch", F.lit(3.0))
        ).first()[0]
        self.assertGreaterEqual(rank, 0.0)
        self.assertLessEqual(rank, 1.0)
    ...
    def test_kll_sketch_double_variants(self):
        ...
        # Test merge
        merged = sketch_df.select(
            F.kll_sketch_merge_double("sketch", "sketch")
        ).first()[0]
        self.assertIsNotNone(merged)
        
        # Test get_quantile
        quantile = sketch_df.select(
            F.kll_sketch_get_quantile_double("sketch", F.lit(0.5))
        ).first()[0]
        self.assertIsNotNone(quantile)
        
        # Test get_rank
        rank = sketch_df.select(
            F.kll_sketch_get_rank_double("sketch", F.lit(3.0))
        ).first()[0]
        self.assertGreaterEqual(rank, 0.0)
        self.assertLessEqual(rank, 1.0)
