...
class MapInPandasTestsMixin:
        ...
        for safely in [True, False]:
            with self.subTest(convertToArrowArraySafely=safely), self.sql_conf(
                ...
                with self.subTest(convert="string to double"):
                    ...
                    expected = (
                        rf"Cannot convert column 'id' of pandas type '{pandas_type_name}' "
                ...
                with self.subTest(convert="float to int precision loss"):
                    ...
                    if safely:
                        expected = (
                            r"Cannot convert column 'id' of pandas type 'float64' "
                            r"to Arrow type 'int32'\."
                            " Please verify the data values are compatible"
    ...
    def test_map_in_pandas_type_mismatch(self):
        ...
        with self.assertRaisesRegex(
            PythonException,
            f"Cannot convert column 'id' of pandas type '{pandas_type_name}' "
            "to Arrow type 'int32'\\.",
        ):
            df.collect()
