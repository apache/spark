...
class ArrowTests(ReusedSQLTestCase):
    ...
    def setUpClass(cls):
        ...
        assert cls.spark.conf.get("spark.sql.pyspark.execution.arrow.enabled") == "false"
        cls.spark.conf.set("spark.sql.execution.arrow.enabled", "true")
        assert cls.spark.conf.get("spark.sql.pyspark.execution.arrow.enabled") == "true"

        cls.spark.conf.set("spark.sql.execution.arrow.fallback.enabled", "true")
        assert cls.spark.conf.get("spark.sql.pyspark.execution.arrow.fallback.enabled") == "true"
        cls.spark.conf.set("spark.sql.execution.arrow.fallback.enabled", "false")
        assert cls.spark.conf.get("spark.sql.pyspark.execution.arrow.fallback.enabled") == "false"

        # Enable Arrow optimization in this tests.
        cls.spark.conf.set("spark.sql.pyspark.execution.arrow.enabled", "true")
        # Disable fallback by default to easily detect the failures.
        cls.spark.conf.set("spark.sql.pyspark.execution.arrow.fallback.enabled", "false")
    ...
    def test_toPandas_fallback_enabled(self):
        with self.sql_conf({"spark.sql.pyspark.execution.arrow.fallback.enabled": True}):
    ...
    def _toPandas_arrow_toggle(self, df):
        with self.sql_conf({"spark.sql.pyspark.execution.arrow.enabled": False}):
            pdf = df.toPandas()
