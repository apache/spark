...
class SparkConnectCloneSessionTest(SparkConnectSQLTestCase):
    ...
    def test_temp_views_independence_after_cloning(self):
        ...
        original_df = self.connect.createDataFrame([(2, "modified_original"), (3, "another")], ["id", "type"])
        original_df.createOrReplaceTempView("shared_view")
        
        # Replace the view in the cloned session
        cloned_session.sql("CREATE OR REPLACE TEMPORARY VIEW shared_view AS SELECT 4 as id, 'cloned_data' as type")
    ...
    def test_invalid_session_id_format(self):
        ...
        self.assertIn("INVALID_CLONE_SESSION_REQUEST.TARGET_SESSION_ID_FORMAT", str(context.exception))
