...
class SubqueryConnectTests(ReusedConnectTestCase):
    @property
    def df1(self):
    ...
    def test_unanalyzable_expression(self):
        sub = self.spark.range(1).alias("r").where(sf.col("l.id") == sf.col("r.id"))
        ...
        self.check_error(
            ...
            messageParameters={'objectName': '`l`.`id`', 'proposal': '`r`.`id`'},
        ...
        with self.tempView("t1", "t2"):
            ...
            assertDataFrameEqual(
                self.spark.table("t1").select(
                    ...
                    (
                        self.spark.table("t2")
                        .where(sf.col("t1.c2") == sf.col("t2.c2"))
                        .select(sf.max("c1"))
                        .scalar()
                    ),
                ),
        ...
        with self.tempView("l", "r"):
                ...
                assertDataFrameEqual(
                    ...
                    .select(
                        ...
                        (
                            self.spark.table("l")
                            .alias("l2")
                            .where(sf.col("l2.a").eqNullSafe(sf.col("l1.a")))
                            .select(sf.sum("b"))
                            .scalar()
                            .alias("sum_b")
                        ),
            ...
            with self.subTest("non-aggregated"):
                with self.assertRaises(SparkRuntimeException) as pe:
                    self.spark.table("l").alias("l1").select(
                        "a",
                        (
                            self.spark.table("l")
                            .alias("l2")
            ...
            with self.subTest("non-equal"):
                assertDataFrameEqual(
                    ...
                    self.spark.sql(
                        """select a, (select sum(b) from l l2 where l2.a < l1.a) from l l1"""
                    ),
                )
            ...
            with self.subTest("disjunctive"):
                assertDataFrameEqual(
                    ...
                    .where(
                        self.spark.table("r")
                        .where(
                            ((sf.col("a") == sf.col("c")) & (sf.col("d") == sf.lit(2.0)))
                            | ((sf.col("a") == sf.col("c")) & (sf.col("d") == sf.lit(1.0)))
                        )
    ...
    def test_exists_subquery(self):
        with self.tempView("l", "r"):
            ...
            with self.subTest("EXISTS"):
                assertDataFrameEqual(
                    self.spark.table("l").where(
                        self.spark.table("r").where(sf.col("a") == sf.col("c")).exists()
                    ),
                    self.spark.sql(
            ...
            with self.subTest("NOT EXISTS"):
                ...
                assertDataFrameEqual(
                    self.spark.table("l").where(
                        ~(
                            self.spark.table("r")
                            .where((sf.col("a") == sf.col("c")) & (sf.col("b") < sf.col("d")))
                            .exists()
                        )
                    ),
                    self.spark.sql(
...
if __name__ == "__main__":
    from pyspark.sql.tests.connect.test_connect_subquery import *  # noqa: F401
