...
class CogroupedMapInPandasTests(ReusedSQLTestCase):
    ...
    def test_complex_group_by(self):
        ...
        def merge_pandas(lft, r):
            return pd.merge(lft[["k", "v"]], r[["k", "v2"]], on=["k"])
    ...
    def _test_with_key(left, right, isLeft):
        def right_assign_key(key, lft, r):
            return lft.assign(key=key[0]) if isLeft else r.assign(key=key[0])
