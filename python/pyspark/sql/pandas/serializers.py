...
from pyspark.sql.pandas.types import (
    to_arrow_type,
    type_require_conversion,
    recursive_convert_outputs,
    recursive_convert_inputs,
)
...
class ArrowStreamPandasSerializer(ArrowStreamSerializer):
    ...
    def pandas_to_arrow(pser, arrow_column_type) -> pa.array:
        if type_require_conversion(arrow_column_type):
            # We have to do per-value conversion for nested types.
            return pa.array(pser.apply(lambda v: recursive_convert_outputs(v, arrow_column_type)))
        else:
            return pa.array(pser)

    def _create_batch(self, series) -> pa.RecordBatch:
        """
        ...
        pa_arrs = [self.pandas_to_arrow(s, t) for s, t in series]

        return pa.RecordBatch.from_arrays(pa_arrs, ["_%d" % i for i in range(len(pa_arrs))])
