...
def hll_sketch_agg(
    ...
    if lgConfigK is not None and tgtHllType is not None:
        return _invoke_function("hll_sketch_agg", _to_java_column(col), lgConfigK, tgtHllType)
...
def hll_union_agg(col: "ColumnOrName", lgMaxK: Optional[int] = None) -> Column:
    ...
    via a Datasketches Union instance configured with lgMaxK arg.
    ...
    if lgMaxK is not None:
        return _invoke_function("hll_union_agg", _to_java_column(col), lgMaxK)
    else:
...
def hll_union(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    ...
    Datasketches Union object configured with an lgMaxK equal to the min of the
