...
@try_remote_functions
def approx_count_distinct_sketch(col: "ColumnOrName", rsd: Optional[float] = None) -> Column:
    """Aggregate function: returns a new :class:`~pyspark.sql.Column` containing the
    aggregateable HyperLogLog++ sketch of column `col`.
    ...
    -------
    :class:`~pyspark.sql.Column`
        The aggregateable HyperLogLog++ sketch as a binary column.
    ...
    """
    if rsd is None:
        return _invoke_function_over_columns("approx_count_distinct_sketch", col)
    else:
        return _invoke_function("approx_count_distinct_sketch", _to_java_column(col), rsd)


@try_remote_functions
def approx_count_distinct_eval_sketch(col: "ColumnOrName") -> Column:
    """Returns a new :class:`~pyspark.sql.Column` containing the
    approximate distinct count generated by the sketch in column `col`.
    ...
    >>> df_agg.withColumn(approx_distinct_count_eval_skecth("hll_sketch")).alias('distinct_count').show()
    +---------------+---------------+
    ...
    +---------------+---------------+
    """
    return _invoke_function_over_columns("approx_count_distinct_eval_sketch", col)


@try_remote_functions
def approx_count_distinct_agg_sketch(col: "ColumnOrName", rsd: Optional[float] = None) -> Column:
    """Aggregate function: returns a new :class:`~pyspark.sql.Column` containing the
    approximate distinct count generated by the sketches in column `col`.
    ...
    >>> df = spark.createDataFrame([1,2,2,3], "INT")
    >>> df_agg = df.agg(approx_count_distinct_sketch("value")).alias('hll_sketch'))
    >>> df_agg.agg(approx_distinct_count_agg_skecth("hll_sketch")).alias('distinct_count').show()
    +---------------+
    |distinct_values|
    +---------------+
    |              3|
    ...
    """
    if rsd is None:
        return _invoke_function_over_columns("approx_count_distinct_agg_sketch", col)
    else:
        return _invoke_function("approx_count_distinct_agg_sketch", _to_java_column(col), rsd)
