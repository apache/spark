...
class SQLTests(ReusedPySparkTestCase):
    ...
    def test_stream_save_options(self):
        df = self.spark.readStream.format('parquet') \
            .load('python/test_support/sql/parquet_partitioned')
        ...
        q = df.writeStream.option('checkpointLocation', chk).queryName('this_query') \
            .format('parquet').partitionBy('year').outputMode('append').option('path', out).start()
        try:
