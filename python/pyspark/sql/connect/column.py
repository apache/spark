...
class Column(ParentColumn):
    ...
    def otherwise(self, value: Any) -> ParentColumn:
        ...
        return Column(CaseWhen(branches=self._expr._branches, else_value=F.lit(value)._expr))
    ...
    def isin(self, *cols: Any) -> ParentColumn:
        ...
        return Column(UnresolvedFunction("in", [self._expr] + [F.lit(c)._expr for c in _cols]))
    ...
    def __getitem__(self, k: Any) -> ParentColumn:
        if isinstance(k, slice):
            ...
            return Column(UnresolvedExtractValue(self._expr, F.lit(k)._expr))
