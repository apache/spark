...
class SparkSession:
    ...
    def createDataFrame(
        ...
        def convert_to_arrow_data(cols: List[str], data: Iterable[Any]):
            def convert(data: Iterable[Any]):
                if isinstance(data, list):
                    ...
                    return dict(zip(cols, [convert(item) for item in data]))
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if _schema is None and isinstance(_data[0], (Row, dict, tuple)):
            ...
            def get_arrow_schema() -> Optional[pa.Schema]:
                if _schema is not None and isinstance(_schema, StructType):
                    try:
                        return to_arrow_schema(_schema)
                    except TypeError:
                        return None
                elif _inferred_schema is not None and isinstance(_inferred_schema, StructType):
                    try:
                        return to_arrow_schema(_inferred_schema)
                    except TypeError:
                        return None
                else:
                    return None
