...
class ReadChanges(LogicalPlan):
    ...
    def plan(self, session: "SparkConnectClient") -> proto.Relation:
        plan = self._create_proto_relation()
        plan.read_changes.unparsed_identifier = self.table_name
        if self._is_streaming is not None:
            plan.read_changes.is_streaming = self._is_streaming
        for k, v in self.options.items():
            plan.read_changes.options[k] = v
        return plan

    def print(self, indent: int = 0) -> str:
        return f"{' ' * indent}<ReadChanges table_name={self.table_name}>\n"
