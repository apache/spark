...
class LazyOuterReference(Expression):
    def __init__(self, unparsed_identifier: str):
        assert isinstance(unparsed_identifier, str)
        super().__init__()
        self._unparsed_identifier = unparsed_identifier
    ...
    def to_plan(self, session: "SparkConnectClient") -> proto.Expression:
        """Returns the Proto representation of the expression."""
        expr = self._create_proto_expression()
        expr.lazy_outer_reference.unparsed_identifier = self._unparsed_identifier
        return expr

    def __repr__(self) -> str:
        return f"outer({self._unparsed_identifier})"

    def __eq__(self, other: Any) -> bool:
        return (
            other is not None
            and isinstance(other, LazyOuterReference)
            and other._unparsed_identifier == self._unparsed_identifier
        )
