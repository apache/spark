...
class HasMaxIter(Params):
    ...
    maxIter = IntParam(Params._dummy(), "maxIter", "max number of iterations (>= 0).", ParamValidators.gtEq(0))
...
class HasRegParam(Params):
    ...
    regParam = FloatParam(Params._dummy(), "regParam", "regularization parameter (>= 0).", ParamValidators.gtEq(0))
...
class HasFeaturesCol(Params):
    ...
    featuresCol = StringParam(Params._dummy(), "featuresCol", "features column name.", None)
...
class HasLabelCol(Params):
    ...
    labelCol = StringParam(Params._dummy(), "labelCol", "label column name.", None)
...
class HasPredictionCol(Params):
    ...
    predictionCol = StringParam(Params._dummy(), "predictionCol", "prediction column name.", None)
...
class HasProbabilityCol(Params):
    ...
    probabilityCol = StringParam(Params._dummy(), "probabilityCol", "Column name for predicted class conditional probabilities. Note: Not all models output well-calibrated probability estimates! These probabilities should be treated as confidences, not precise probabilities.", None)
...
class HasRawPredictionCol(Params):
    ...
    rawPredictionCol = StringParam(Params._dummy(), "rawPredictionCol", "raw prediction (a.k.a. confidence) column name.", None)
...
class HasInputCol(Params):
    ...
    inputCol = StringParam(Params._dummy(), "inputCol", "input column name.", None)
...
class HasInputCols(Params):
    ...
    inputCols = ListStringParam(Params._dummy(), "inputCols", "input column names.", None)
...
class HasOutputCol(Params):
    ...
    outputCol = StringParam(Params._dummy(), "outputCol", "output column name.", None)
...
class HasNumFeatures(Params):
    ...
    numFeatures = IntParam(Params._dummy(), "numFeatures", "number of features.", ParamValidators.gtEq(0))
...
class HasCheckpointInterval(Params):
    ...
    checkpointInterval = IntParam(Params._dummy(), "checkpointInterval", "set checkpoint interval (>= 1) or disable checkpoint (-1). E.g. 10 means that the cache will get checkpointed every 10 iterations.", lambda interval: (interval == -1) or (interval >= 1))
...
class HasSeed(Params):
    ...
    seed = IntParam(Params._dummy(), "seed", "random seed.", None)
...
class HasStepSize(Params):
    ...
    stepSize = FloatParam(Params._dummy(), "stepSize", "Step size to be used for each iteration of optimization.", None)
...
class HasHandleInvalid(Params):
    ...
    handleInvalid = StringParam(Params._dummy(), "handleInvalid", "how to handle invalid entries. Options are skip (which will filter out rows with bad values), or error (which will throw an errror). More options may be added later.", ParamValidators.inList(['skip', 'error']))
...
class HasElasticNetParam(Params):
    ...
    elasticNetParam = FloatParam(Params._dummy(), "elasticNetParam", "the ElasticNet mixing parameter, in range [0, 1]. For alpha = 0, the penalty is an L2 penalty. For alpha = 1, it is an L1 penalty.", ParamValidators.inRange(0, 1))
...
class HasFitIntercept(Params):
    ...
    fitIntercept = BooleanParam(Params._dummy(), "fitIntercept", "whether to fit an intercept term.", None)
...
class HasStandardization(Params):
    ...
    standardization = BooleanParam(Params._dummy(), "standardization", "whether to standardize the training features before fitting the model.", None)
...
class HasThresholds(Params):
    ...
    thresholds = ListFloatParam(Params._dummy(), "thresholds", "Thresholds in multi-class classification to adjust the probability of predicting each class. Array must have length equal to the number of classes, with values >= 0. The class with largest value p/t is predicted, where p is the original probability of that class and t is the class' threshold.", lambda lst: all(map(lambda t: t >= 0, lst)))
...
class HasWeightCol(Params):
    ...
    weightCol = StringParam(Params._dummy(), "weightCol", "weight column name. If this is not set or empty, we treat all instance weights as 1.0.", None)
...
class HasSolver(Params):
    ...
    solver = StringParam(Params._dummy(), "solver", "the solver algorithm for optimization. If this is not set or empty, default value is 'auto'.", None)
...
class DecisionTreeParams(Params):
    ...
    maxDepth = IntParam(Params._dummy(), "maxDepth", "Maximum depth of the tree. (>= 0) E.g., depth 0 means 1 leaf node; depth 1 means 1 internal node + 2 leaf nodes.", ParamValidators.gtEq(0))
    maxBins = IntParam(Params._dummy(), "maxBins", "Max number of bins for discretizing continuous features.  Must be >=2 and >= number of categories for any categorical feature.", ParamValidators.gtEq(2))
    minInstancesPerNode = IntParam(Params._dummy(), "minInstancesPerNode", "Minimum number of instances each child must have after split. If a split causes the left or right child to have fewer than minInstancesPerNode, the split will be discarded as invalid. Should be >= 1.", ParamValidators.gtEq(1))
    minInfoGain = FloatParam(Params._dummy(), "minInfoGain", "Minimum information gain for a split to be considered at a tree node.", None)
    maxMemoryInMB = IntParam(Params._dummy(), "maxMemoryInMB", "Maximum memory in MB allocated to histogram aggregation.", ParamValidators.gtEq(0))
    cacheNodeIds = BooleanParam(Params._dummy(), "cacheNodeIds", "If false, the algorithm will pass trees to executors to match instances with nodes. If true, the algorithm will cache node IDs for each instance. Caching can speed up training of deeper trees. Users can set how often should the cache be checkpointed or disable it by setting checkpointInterval.", None)
