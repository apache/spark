...
class Frame(object, metaclass=ABCMeta):
    ...
    def _build_fallback_driver_method(self, method: str) -> Callable:
        def _internal_fallback_function(*args: Any, **kwargs: Any) -> Any:
            log_advice(
                f"`{method}` is executed in fallback mode. It loads all data into the "
                f"driver's memory. It should only be used if the pandas DataFrame is "
                f"expected to be small."
            )
            return getattr(self._to_pandas(), method)(*args, **kwargs)

        return _internal_fallback_function

    def _to_hdf_fallback(self, *args: Any, **kwargs: Any) -> None:
        _f = self._build_fallback_driver_method("to_hdf")
