...
class BooleanExtensionOpsTest(PandasOnSparkTestCase, TestCasesUtils):
    ...
    def test_floordiv(self):
        ...
        # float is always returned in pandas-on-Spark
        self.assert_eq((pser // 1).astype("Float64"), psser // 1)
    ...
    def test_pow(self):
        ...
        # float is always returned in pandas-on-Spark
        self.assert_eq((pser ** 1).astype("Float64"), psser ** 1)
        self.assert_eq((pser ** 0.1).astype("Float64"), self.psser ** 0.1)
        self.assert_eq((pser ** pser.astype(float)).astype("Float64"), psser ** psser.astype(float))
        self.assertRaises(TypeError, lambda: psser ** psser)
        self.assertRaises(TypeError, lambda: psser ** True)
    ...
    def test_rpow(self):
        self.assert_eq(pd.Series([1, 1, 1], dtype="Float64"), 1 ** self.psser)
        self.assert_eq((0.1 ** self.pser).astype("Float64"), 0.1 ** self.psser)
        self.assertRaises(TypeError, lambda: "x" ** self.psser)
        self.assertRaises(TypeError, lambda: True ** self.psser)
        self.assertRaises(TypeError, lambda: datetime.date(1994, 1, 1) ** self.psser)
        self.assertRaises(TypeError, lambda: datetime.datetime(1994, 1, 1) ** self.psser)
    ...
    def test_rmod(self):
        self.assert_eq(ps.Series([0, np.nan, np.nan], dtype="Int64"), 1 % self.psser)
        self.assert_eq(
