...
class DataFrameTestsMixin:
    ...
    def test_validate_test_names(self):
        # All test cases MUST be in pattern 'test_[a-z].*',
        # because CI will actually skip 'DataFrameTests', and executed the split tests
        # in 'test_dataframe_splits' for better parallelism.
        for name in [name for name in dir(self) if name.startswith("test_")]:
            self.assertTrue(len(name) > 5)
            self.assertTrue("a" <= name[5] <= "z")
...
# instead we divide it into several splits in 'test_dataframe_splits' and so that the testing
