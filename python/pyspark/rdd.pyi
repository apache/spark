...
class RDD(Generic[T_]):
    ...
    def cache(self) -> RDD[T_]: ...
    def persist(self, storageLevel: StorageLevel = ...) -> RDD[T_]: ...
    def unpersist(self, blocking: bool = ...) -> RDD[T_]: ...
    ...
    def map(self, f: Callable[[T_], U], preservesPartitioning: bool = ...) -> RDD[U]: ...
    def flatMap(
        self, f: Callable[[T_], Iterable[U]], preservesPartitioning: bool = ...
    ) -> RDD[U]: ...
    def mapPartitions(
    ...
    def filter(self, f: Callable[[T_], bool]) -> RDD[T_]: ...
    def distinct(self, numPartitions: Optional[int] = ...) -> RDD[T_]: ...
    def sample(
    ...
    def union(self, other: RDD[U]) -> RDD[Union[T_, U]]: ...
    def intersection(self, other: RDD[T_]) -> RDD[T_]: ...
    def __add__(self, other: RDD[T_]) -> RDD[T_]: ...
    @overload
    ...
    def sortBy(
        self,
        keyfunc: Callable[[T_], O],
        ascending: bool = ...,
        numPartitions: Optional[int] = ...,
    ) -> RDD[T_]: ...
    def glom(self) -> RDD[List[T_]]: ...
    def cartesian(self, other: RDD[U]) -> RDD[Tuple[T_, U]]: ...
    def groupBy(
        self,
        f: Callable[[T_], K],
        numPartitions: Optional[int] = ...,
        partitionFunc: Callable[[K], int] = ...,
    ) -> RDD[Tuple[K, Iterable[T_]]]: ...
    def pipe(
        self, command: str, env: Optional[Dict[str, str]] = ..., checkCode: bool = ...
    ) -> RDD[str]: ...
    def foreach(self, f: Callable[[T_], None]) -> None: ...
    def foreachPartition(self, f: Callable[[Iterable[T_]], None]) -> None: ...
    def collect(self) -> List[T_]: ...
    def collectWithJobGroup(
        self, groupId: str, description: str, interruptOnCancel: bool = ...
    ) -> List[T_]: ...
    def reduce(self, f: Callable[[T_, T_], T_]) -> T_: ...
    def treeReduce(self, f: Callable[[T_, T_], T_], depth: int = ...) -> T_: ...
    def fold(self, zeroValue: T, op: Callable[[T_, T_], T_]) -> T_: ...
    def aggregate(
        self, zeroValue: U, seqOp: Callable[[U, T_], U], combOp: Callable[[U, U], U]
    ) -> U: ...
    def treeAggregate(
        self,
        zeroValue: U,
        seqOp: Callable[[U, T_], U],
    ...
    @overload
    def max(self, key: Callable[[T_], O]) -> T_: ...
    @overload
    def min(self: RDD[O]) -> O: ...
    @overload
    def min(self, key: Callable[[T_], O]) -> T_: ...
    def sum(self: RDD[NumberOrArray]) -> NumberOrArray: ...
    def count(self) -> int: ...
    def stats(self: RDD[NumberOrArray]) -> StatCounter: ...
    def histogram(self, buckets: Union[int, List[T_], Tuple[T_, ...]]) -> Tuple[List[T_], List[int]]: ...
    ...
    @overload
    def top(self, num: int, key: Callable[[T_], O]) -> List[T_]: ...
    @overload
    def takeOrdered(self: RDD[O], num: int) -> List[O]: ...
    @overload
    def takeOrdered(self, num: int, key: Callable[[T_], O]) -> List[T_]: ...
    def take(self, num: int) -> List[T_]: ...
    def first(self) -> T_: ...
    ...
    def subtract(
        self, other: RDD[T_], numPartitions: Optional[int] = ...
    ) -> RDD[T_]: ...
    def keyBy(self, f: Callable[[T_], K]) -> RDD[Tuple[K, T_]]: ...
    def repartition(self, numPartitions: int) -> RDD[T_]: ...
    def coalesce(self, numPartitions: int, shuffle: bool = ...) -> RDD[T_]: ...
    def zip(self, other: RDD[U]) -> RDD[Tuple[T_, U]]: ...
    def zipWithIndex(self) -> RDD[Tuple[T_, int]]: ...
    def zipWithUniqueId(self) -> RDD[Tuple[T_, int]]: ...
    def name(self) -> str: ...
    def setName(self, name: str) -> RDD[T_]: ...
    ...
    def toLocalIterator(self, prefetchPartitions: bool = ...) -> Iterator[T_]: ...
    def barrier(self) -> RDDBarrier[T_]: ...
    def withResources(self, profile: ResourceProfile) -> RDD[T_]: ...
