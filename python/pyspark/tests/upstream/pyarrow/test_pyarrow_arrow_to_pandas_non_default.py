...
class PyArrowArrayToPandasZeroCopyTests(_PyArrowToPandasTestBase):
    ...
    in ``ArrowToPandasConversion._convert_column_numpy``
...
class PyArrowArrayToPandasIntegerObjectNullsTests(_PyArrowToPandasTestBase):
    ...
    PySpark passes it in ``ArrowToPandasConversion._convert_column_legacy``
    ...
    full ``pandas_options`` dict ``_convert_column_legacy`` passes.  The last is not a
    ...
    def _build_source_arrays(self):
        ...
        # These are also the types _convert_column_legacy still serves.
    ...
    # Output column for all three arguments as _convert_column_legacy passes them together.
