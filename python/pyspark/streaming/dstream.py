...
from typing import Generic, TypeVar, Hashable, Callable, Iterable, Optional, Tuple, overload, Union, Any, List, \
    TYPE_CHECKING
...
class DStream(Generic[T_co]):
    ...
    def mapPartitions(
        self,
        f: Callable[[Iterable[T_co]], Iterable[U]],
        preservesPartitioning: bool = False
    ) -> "DStream[U]":
        """
    ...
    def foreachRDD(
        self,
        func: Union[Callable[[RDD[T_co]], None], Callable[[datetime, RDD[T_co]], None]]
    ) -> None:
        """
        ...
        if func.__code__.co_argcount == 1:
            ...
            def func(_: Any, rdd: RDD[T_co]) -> None:
                return old_func(rdd)
    ...
    def flatMapValues(self: "DStream[Tuple[K, V]]", f: Callable[[V], Iterable[U]]) -> "DStream[Tuple[K, U]]":
        ...
        def flat_map_fn(kv: T_co) -> Iterable[U]:
    ...
    def transform(
        self,
        func: Callable[[datetime, RDD[T_co]], RDD[U]]
    ...
    def transformWith(
        self,
        func: Union[Callable[[RDD[T_co], RDD[U]], RDD[V]],
                    Callable[[datetime, RDD[T_co], RDD[U]], RDD[V]]],
        other: RDD[U],
        keepSerializer: bool = False) -> "DStream[V]":
        """
        Return a new DStream in which each RDD is generated by applying a function
        ...
        jfunc = TransformFunction(
            self._sc, func, self._jrdd_deserializer, other._jrdd_deserializer)  # type: ignore[attr-defined]
    ...
    def updateStateByKey(
        ...
        if initialRDD:
            initialRDD = initialRDD._reserialize(self._jrdd_deserializer)  # type: ignore[attr-defined]
            dstream = self._sc._jvm.PythonStateDStream(
                self._jdstream.dstream(), jreduceFunc, initialRDD._jrdd  # type: ignore[attr-defined]
            )
        else:
