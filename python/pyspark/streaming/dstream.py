...
class DStream(Generic[T_co]):
    ...
    def __init__(
        ...
        self._ssc = ssc
        self._sc = ssc._sc  # type: ignore[attr-defined]
    ...
    def flatMap(
        ...
        def func(s: int, iterator: Iterable[T_co]) -> Iterable[U]:
            return chain.from_iterable(map(f, iterator))

        return self.mapPartitionsWithIndex(func, preservesPartitioning)

    def map(
        self: "DStream[T_co]", f: Callable[[T_co], U], preservesPartitioning: bool = False
    ) -> "DStream[U]":
        """
    ...
    def reduce(self, func: Callable[[T_co, T_co], T_co]) -> "DStream[T_co]":
        ...
        """
        return (
            self.map(lambda x: (None, x))  # type: ignore[misc]
            .reduceByKey(func, 1)  # type: ignore[arg-type]
            .map(lambda x: cast(T_co, x[1]))
        )
    ...
    @overload
    def foreachRDD(self, func: Callable[[datetime, RDD[T_co]], None]) -> None:
        ...

    def foreachRDD(
        self, func: Union[Callable[[RDD[T_co]], None], Callable[[datetime, RDD[T_co]], None]]
    ) -> None:
        """
        Apply a function to each RDD in this DStream.
        """
        if func.__code__.co_argcount == 1:
            old_func = cast(Callable[[RDD[T_co]], None], func)

            def func(_: datetime, rdd: RDD[T_co]) -> None:
                return old_func(rdd)
    ...
    def pprint(self, num: int = 10) -> None:
        ...
        def takeAndPrint(time: datetime, rdd: RDD[T_co]) -> None:
    ...
    def countByValue(self) -> "DStream[Tuple[T_co, int]]":
        ...
        """
        return self.map(lambda x: (x, 1)).reduceByKey(  # type: ignore[return-value, misc]
            lambda x, y: x + y
        )
    ...
    def saveAsTextFiles(self, prefix: str, suffix: Optional[str] = None) -> None:
        ...
        def saveAsTextFile(t: Optional[datetime], rdd: RDD[T_co]) -> None:
    ...
    def transform(
        self, func: Union[Callable[[RDD[T_co]], RDD[U]], Callable[[datetime, RDD[T_co]], RDD[U]]]
        ...
        """
        if func.__code__.co_argcount == 1:
            oldfunc = cast(Callable[[RDD[T_co]], RDD[U]], func)

            def func(_: datetime, rdd: RDD[T_co]) -> RDD[U]:
                return oldfunc(rdd)
    ...
    def transformWith(
        self,
        func: Callable[[datetime, RDD[T_co], RDD[U]], RDD[V]],
        other: RDD[U],
        keepSerializer: bool = ...,
    ) -> "DStream[V]":
        ...
    ...
    def transformWith(
        self,
        func: Union[
            Callable[[RDD[T_co], RDD[U]], RDD[V]], Callable[[datetime, RDD[T_co], RDD[U]], RDD[V]]
        ],
        other: RDD[U],
        ...
        if func.__code__.co_argcount == 2:
            oldfunc = cast(Callable[[RDD[T_co], RDD[U]], RDD[V]], func)

            def func(_: datetime, a: RDD[T_co], b: RDD[U]) -> RDD[V]:
                return oldfunc(a, b)
    ...
    def union(self, other: "DStream[U]") -> "DStream[Union[T_co, U]]":
        ...
        return self.transformWith(
            lambda a, b: a.union(b), other, True
    ...
    def cogroup(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.cogroup(b, numPartitions), other
        )  # type: ignore[call-overload]
    ...
    def join(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.join(b, numPartitions), other
        )  # type: ignore[call-overload]
    ...
    def leftOuterJoin(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.leftOuterJoin(b, numPartitions), other
        )  # type: ignore[call-overload]
    ...
    def fullOuterJoin(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.fullOuterJoin(b, numPartitions), other
        )  # type: ignore[call-overload]
    ...
    def slice(self, begin: Union[datetime, int], end: Union[datetime, int]) -> List[RDD[T_co]]:
        """
    ...
    def window(self, windowDuration: int, slideDuration: Optional[int] = None) -> "DStream[T_co]":
        """
    ...
    def reduceByWindow(
        ...
        slideDuration : int
            ...
            DStream's batching interval
        """
        keyed = self.map(lambda x: (1, x))  # type: ignore[misc]
        ...
        return reduced.map(lambda kv: cast(T_co, kv[1]))

    def countByWindow(self, windowDuration: int, slideDuration: int) -> "DStream[Tuple[T_co, int]]":
        """
        ...
        """
        return self.map(lambda x: 1).reduceByWindow(  # type: ignore[misc, return-value]
    ...
    def countByValueAndWindow(
        ...
        """
        keyed = self.map(lambda x: (x, 1))  # type: ignore[misc]
    ...
    def updateStateByKey(
        ...
        def reduceFunc(t: datetime, a: DStream, b: DStream) -> DStream:
            ...
            state = g.mapValues(lambda vs_s: updateFunc(vs_s[0], vs_s[1]))  # type: ignore[arg-type]
