...
class RDD(object):
    ...
    def histogram(self, bucketCount, buckets=None, evenBuckets=False):
        """
        ...
        If buckets is not given, it will be generated by using bucketCount
        number of buckets evenly spaced between the minimum and maximum
        of the RDD. For example if the min value is 0 and the max is 100
        ...
        if buckets is None:
            if bucketCount < 1:
                raise ValueError("bucketCount should be greater than 1")

            r = jdrdd.histogram(bucketCount)
            return list(r._2())

        elif len(buckets) != bucketCount + 1:
            raise ValueError("bucketCount does not match with "
                             "length of buckets")
