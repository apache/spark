...
Spark Connect uses a new gRPC and HTTP/2 interface, which allows Spark to communicate with different systems in a more efficient and scalable way.
...
While Spark Connect does not have built-in authentication, it is designed to work seamlessly with your existing authentication infrastructure.
The gRPC and HTTP/2 interface allows for the use of authenticating proxies, which makes it possible to secure Spark Connect without having to implement authentication logic in Spark directly.
