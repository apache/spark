...
* In Spark 4.1, Python UDFs are arrow-optimized by default, as the default configuration of ``spark.sql.execution.pythonUDF.arrow.enabled`` is ``true``. To restore the legacy behavior, set ``spark.sql.execution.pythonUDF.arrow.enabled`` to ``false``.
