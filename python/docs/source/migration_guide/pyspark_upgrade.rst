...
* In Spark 4.0, items other than functions (e.g. ``DataFrame``, ``StructType``) have been removed from the wildcard import ``from pyspark.sql.functions import *``, you should import these stuffs from proper modules (e.g. ``from pyspark.sql import DataFrame``, ``from pyspark.sql.types import StructType``).
