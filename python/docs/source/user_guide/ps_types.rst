======================
Type Support In Pandas on Spark
======================
...
Type casting between PySpark and pandas on Spark
---------------------------------------
...
.. code-block:: python
    ...
    >>> pdf = ps.DataFrame({"int8": [1], "bool": [True], "float32": [1.0], "float64": [1.0], "int32": [1], "int64": [1], "int16": [1], "datetime": [datetime.datetime(2020, 10, 27)], "object_string": ["1"], "object_decimal": [decimal.Decimal("1.1")], "object_date": [datetime.date(2020, 10, 27)]})

    # 2. Type casting by using `astype`
    >>> pdf['int8'] = pdf['int8'].astype('int8')
    >>> pdf['int16'] = pdf['int16'].astype('int16')
    >>> pdf['int32'] = pdf['int32'].astype('int32')
    >>> pdf['float32'] = pdf['float32'].astype('float32')
...
Type casting between pandas and pandas on Spark
--------------------------------------
...
.. code-block:: python
    ...
    # pd.Catrgorical type is not supported in pandas on Spark yet.
...
These kind of pandas specific data types below are not currently supported in pandas on Spark but planned to be supported.
...
The pandas specific data types below are not planned to be supported in pandas on Spark yet.
...
The table below shows which NumPy data types are matched to which PySpark data types internally in pandas on Spark.
...
The table below shows which Python data types are matched to which PySpark data types internally in pandas on Spark.
...
For decimal type, pandas on Spark uses Spark's system default precision and scale.
...
.. code-block:: python
    ...
    >>> ps.Series(["welcome", "to", "Pandas_on_Spark"]).spark.data_type
    StringType

    >>> ps.Series([[False, True, False]]).spark.data_type
    ArrayType(BooleanType,true)

    >>> ps.DataFrame({"d": [0.3, 0.1, 0.8], "s": ["welcome", "to", "Pandas_on_Spark"], "b": [False, True, False]}).spark.print_schema()
    root
...
.. note::
    ...
    Pandas on Spark currently does not support multiple types of data in single column.
