...
.. code-block:: python
   ...
   >>> pdf = pss.DataFrame({'a': [1,2,3], 'b':[4,5,6]})
   >>> def pandas_plus(pser):
   ...     return pser + 1  # should always return the same length as input.
   ...
   >>> pdf.transform(pandas_plus)

.. code-block:: python

   >>> pdf = ps.DataFrame({'a': [1,2,3], 'b':[5,6,7]})
   >>> def pandas_plus(pser):
   ...     return pser[pser % 2 == 1]  # allows an arbitrary length
   ...
   >>> pdf.apply(pandas_plus)

In this case, each function takes a pandas Series, and pandas on spark computes the functions in a distributed manner as below.
...
.. code-block:: python
   ...
   ...
   >>> pdf.apply(pandas_plus, axis='columns')
...
In :func:`DataFrame.pandas_on_spark.transform_batch`, :func:`DataFrame.pandas_on_spark.apply_batch`,
:func:`Series.pandas_on_spark.transform_batch`, etc., the ``batch``
...
.. code-block:: python
   ...
   ...
   >>> pdf.pandas_on_spark.transform_batch(pandas_plus)
...
.. code-block:: python
   ...
   ...
   >>> pdf.pandas_on_spark.apply_batch(pandas_plus)
...
pandas on Spark combines the pandas DataFrames as a pandas on Spark DataFrame.
...
.. code-block:: python
   ...
   ...     return pser + 1  # should always return the same length as input.
   ...
   >>> pdf.a.pandas_on_spark.transform_batch(pandas_plus)
