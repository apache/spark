...
class MockProtocolWriter:
    ...
    @staticmethod
    def write_utf8(s: str, buf: io.BytesIO) -> None:
    ...
    @staticmethod
    def write_bool(val: bool, buf: io.BytesIO) -> None:
        buf.write(struct.pack("!?", val))

    @staticmethod
    def build_preamble(buf: io.BytesIO) -> None:
        """Write everything ``main()`` reads before ``eval_type``."""
        write_int(0, buf)  # split_index
        MockProtocolWriter.write_utf8(
            f"{sys.version_info[0]}.{sys.version_info[1]}", buf
        )  # python version
        MockProtocolWriter.write_utf8(
            json.dumps(
                {
        ...
        MockProtocolWriter.write_utf8("/tmp", buf)  # spark_files_dir
        write_int(0, buf)  # num_python_includes
        MockProtocolWriter.write_bool(False, buf)  # needs_broadcast_decryption_server
        write_int(0, buf)  # num_broadcast_variables

    @staticmethod
    def build_udf_payload(
        ...
        for offset in arg_offsets:
            ...
            MockProtocolWriter.write_bool(False, buf)  # is_kwarg
    ...
    def write_arrow_ipc_batches(
        batch_iter: Iterator[pa.RecordBatch], buf: io.BufferedIOBase
    ) -> None:
    ...
    def write_worker_input(
        ...
        MockProtocolWriter.build_preamble(buf)
    ...
    @staticmethod
    def run_worker_from_replayed_file(
        write_input: Callable[[io.BufferedIOBase], None],
    ) -> None:
        ...
        for group in groups:
            ...
            for df_batch in group:
                if max_records_per_batch and df_batch.num_rows > max_records_per_batch:
                    ...
                    MockProtocolWriter.write_arrow_ipc_batches(iter(sub_batches), buf)
                else:
                    MockProtocolWriter.write_arrow_ipc_batches(iter([df_batch]), buf)
        write_int(0, buf)  # end of groups

    @staticmethod
    def build_grouped_arg_offsets(n_cols, n_keys=0):
    ...
    @staticmethod
    def make_grouped_arg_offsets(num_key_cols: int, num_value_cols: int) -> list[int]:
...
class MockDataFactory:
    ...
    @staticmethod
    def make_typed_batch(rows: int, n_cols: int) -> tuple[pa.RecordBatch, IntegerType]:
        """Columns cycling through int64, string, binary, boolean."""
        type_cycle = [
        ...
        fields = [
            StructField(f"col_{i}", type_cycle[i % len(type_cycle)][1]) for i in range(n_cols)
        ]
        return (
    ...
    @staticmethod
    def make_grouped_batch(rows_per_group, n_cols):
    ...
    @staticmethod
    def make_mixed_batch(rows_per_group):
    ...
    @staticmethod
    def make_pure_batch(rows, n_cols, make_array, spark_type):
    ...
    def make_grouped_batches(
        ...
        for g in range(num_groups):
            arrays = [
                type_cycle[i % len(type_cycle)][0](rows_per_group) for i in range(n_cols)
            ]
    ...
    @staticmethod
    def wrap_batch_in_struct(batch: pa.RecordBatch) -> pa.RecordBatch:
...
class _PeakmemBenchBase:
    ...
    def peakmem_worker(self, *args):
        MockProtocolWriter.run_worker_from_replayed_file(lambda buf: self._write_scenario(*self._args, buf))
...
def _build_non_grouped_scenarios():
    ...
    for name, (rows, n_cols, num_batches) in {
        ...
        batch, col0_type = MockDataFactory.make_typed_batch(rows, n_cols)
    ...
    scenarios["pure_ints"] = (
        MockDataFactory.make_pure_batch(
            _PURE_ROWS,
            _PURE_COLS,
            lambda r: pa.array(np.random.randint(0, 1000, r, dtype=np.int64)),
            IntegerType(),
    ...
    scenarios["mixed_types"] = (
        MockDataFactory.make_typed_batch(_PURE_ROWS, _PURE_COLS)[0],
        _PURE_BATCHES,
        IntegerType(),
    )
...
def _build_arrow_batched_scenarios():
    ...
    for scenario_name, make_array, spark_type in [
        ...
        batch = MockDataFactory.make_pure_batch(_PURE_ROWS, _PURE_COLS, make_array, spark_type)
    ...
    # mixed types
    batch, col0_type = MockDataFactory.make_typed_batch(_PURE_ROWS, _PURE_COLS)
    type_cycle = [IntegerType(), StringType(), BinaryType(), BooleanType()]
    input_struct = StructType(
...
class _ArrowBatchedBenchMixin:
    ...
    def _write_scenario(self, scenario, udf_name, buf):
        ...
        def write_command(b):
            ...
            MockProtocolWriter.write_utf8(input_struct.json(), b)
            MockProtocolWriter.build_udf_payload(udf_func, ret_type, arg_offsets, b)

        MockProtocolWriter.write_worker_input(
            PythonEvalType.SQL_ARROW_BATCHED_UDF,
            write_command,
            lambda b: MockProtocolWriter.write_arrow_ipc_batches((batch for _ in range(num_batches)), b),
            buf,
        )
...
def _build_grouped_agg_arrow_scenarios():
    ...
    for name, (num_groups, rows_per_group, n_cols) in {
        ...
        groups = MockDataFactory.make_agg_arrow_groups(num_groups, rows_per_group, n_cols)
...
class _GroupedAggArrowBenchMixin:
    ...
    def _write_scenario(self, scenario, udf_name, buf):
        ...
        def write_command(b):
            MockProtocolWriter.build_udf_payload(udf_func, return_type, arg_offsets, b)

        MockProtocolWriter.write_worker_input(
            PythonEvalType.SQL_GROUPED_AGG_ARROW_UDF,
            write_command,
            lambda b: MockProtocolWriter.write_grouped_arrow_data(groups, num_dfs=1, buf=b),
            buf,
        )
...
def _build_grouped_map_arrow_scenarios():
    ...
    for name, (num_groups, rows_per_group, num_key_cols, num_value_cols) in {
        ...
        groups, return_type = MockDataFactory.make_grouped_batches(
            num_groups, rows_per_group, num_key_cols, num_value_cols
        )
        arg_offsets = MockProtocolWriter.make_grouped_arg_offsets(num_key_cols, num_value_cols)
...
class _GroupedMapArrowBenchMixin:
    ...
    def _write_scenario(self, scenario, udf_name, buf):
        ...
        def write_command(b):
            ...
            for offset in arg_offsets:
                write_int(offset, b)
                MockProtocolWriter.write_bool(False, b)  # is_kwarg
...
def _build_grouped_map_pandas_scenarios():
    ...
    for name, (rows, n_cols, num_groups, max_rpb) in {
        ...
        batch, schema = MockDataFactory.make_grouped_batch(rows, n_cols)
        scenarios[name] = (batch, num_groups, schema, max_rpb)

    # mixed column types, small groups
    batch, schema = MockDataFactory.make_mixed_batch(3)
...
class _GroupedMapPandasBenchMixin:
    ...
    def _write_scenario(self, scenario, udf_name, buf):
        ...
        arg_offsets = MockProtocolWriter.build_grouped_arg_offsets(n_cols, n_keys=n_args - 1)
        MockProtocolWriter.write_worker_input(
            PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF,
            lambda b: MockProtocolWriter.build_udf_payload(udf_func, ret_type, arg_offsets, b),
            lambda b: MockProtocolWriter.write_grouped_arrow_data(
                [(batch,)] * num_groups,
                num_dfs=1,
                buf=b,
...
class _MapArrowIterBenchMixin:
    ...
    def _write_scenario(self, scenario, udf_name, buf):
        ...
        if ret_type is None:
            ret_type = col0_type
        wrapped = MockDataFactory.wrap_batch_in_struct(batch)
        MockProtocolWriter.write_worker_input(
            PythonEvalType.SQL_MAP_ARROW_ITER_UDF,
            lambda b: MockProtocolWriter.build_udf_payload(udf_func, ret_type, arg_offsets, b),
            lambda b: MockProtocolWriter.write_arrow_ipc_batches((wrapped for _ in range(num_batches)), b),
            buf,
        )
