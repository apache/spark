...
<tr>
  ...
  <td>false</td>
  <td>
    If set to true, file fetching will use a local cache that is shared by executors
    that belong to the same application, which can improve task launching performance when
    running many executors on the same host. If set to false (default), these caching optimizations will
    be disabled and all executors will fetch their own copies of files. This optimization may not be
    enabled in order to use Spark local directories that reside on NFS filesystems (see
