...
<tr><td> <code>local-cluster[N,C,M]</code> </td><td> Run Spark cluster locally with N number of workers, C cores per a worker and M memory per a worker (only for unit test purpose).</td></tr>
