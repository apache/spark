...
* `--conf`: Arbitrary Spark configuration property in key=value format. For values that contain spaces wrap "key=value" in quotes (as shown). Multiple configurations shall be passed individually.(e.g --conf <key>=<value> --conf <key>=<value>)
