...
[machine learning](mllib-guide.md) and
[graph processing](graphx-programming-guide.md) algorithms on data streams.
...
If you have already [downloaded](index.md#downloading) and [built](index.md#building) Spark,
...
distributed dataset (see [Spark Programming Guide](programming-guide.md#resilient-distributed-datasets-rdds) for more details). Each RDD in a DStream contains data from a certain interval,
...
- When running a Spark Streaming program locally, do not use "local" or "local[1]" as the master URL.
  ...
  (see [Spark Properties](configuration.md#spark-properties) for information on how to set
  the master).
...
the`spark.default.parallelism` [configuration property](configuration.md#spark-properties). You
...
[configuration property](configuration.md#spark-properties) to change the default.
...
In both cases, using Kryo serialization can reduce both CPU and memory overheads. See the [Spark Tuning Guide](tuning.md#data-serialization)) for more details. Consider registering custom classes, and disabling object reference tracking for Kryo (see Kryo-related configurations in the [Configuration Guide](configuration.md#compression-and-serialization)).
...
* **Execution mode**: Running Spark in Standalone mode or coarse-grained Mesos mode leads to
  ...
  [Running on Mesos guide](running-on-mesos.md) for more details.
...
in the [Tuning Guide](tuning.md#memory-tuning). It is strongly recommended that you read that. In this section, we discuss a few tuning parameters specifically in the context of Spark Streaming applications.
...
consistent batch processing times. Make sure you set the CMS GC on both the driver (using `--driver-java-options` in `spark-submit`) and the executors (using [Spark configuration](configuration.md#runtime-environment) `spark.executor.extraJavaOptions`).

* **Other tips**: To further reduce GC overheads, here are some more tips to try.
    - Use Tachyon for off-heap storage of persisted RDDs. See more detail in the [Spark Programming Guide](programming-guide.md#rdd-persistence).
...
the [custom receiver guide](streaming-custom-receivers.md) for more details.
...
* Additional guides
    - [Kafka Integration Guide](streaming-kafka-integration.md)
    - [Flume Integration Guide](streaming-flume-integration.md)
    - [Kinesis Integration Guide](streaming-kinesis-integration.md)
    - [Custom Receiver Guide](streaming-custom-receivers.md)
* API documentation
  - Scala docs
