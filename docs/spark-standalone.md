...
Note that if you are running spark-shell from one of the spark cluster machines, the `bin/spark-shell` script
will automatically set master according to `spark.master` in `spark-defaults.conf`.
