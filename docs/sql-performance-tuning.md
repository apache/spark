...
Whole-stage code generation combines operators into generated Java methods. On HotSpot JVMs with
`-XX:+DontCompileHugeMethods` enabled (the default), methods larger than 8000 bytes are not JIT
compiled. If a generated method is too large, consider lowering the following limit to `8000` so
Spark runs the affected subtree without whole-stage code generation. Compare performance on your
workload before retaining the setting; disabling whole-stage code generation can also reduce
performance.

<table class="spark-config">
<thead><tr><th>Property Name</th><th>Default</th><th>Meaning</th><th>Since Version</th></tr></thead>
<tr>
  <td><code>spark.sql.codegen.hugeMethodLimit</code></td>
  <td>65535</td>
  <td>
    Maximum bytecode size of a compiled Java method generated by whole-stage code generation.
    If a compiled method exceeds this limit, Spark disables whole-stage code generation for that
    subtree of the query plan. This internal setting does not change the JVM's JIT compilation limit.
  </td>
  <td>2.3.0</td>
</tr>
</table>
