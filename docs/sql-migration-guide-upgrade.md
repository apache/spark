...
## Upgrading From Spark SQL 2.4 to 3.0
  ...
  - In Spark version 2.4 and earlier, the `from_json` function produces `null`s if there is no valid root JSON token in its input (` ` for example). Since Spark 3.0, such input is treated as a bad record and handled according to specified mode. For example, in the `PERMISSIVE` mode the ` ` input is converted to `Row(null, null)` if specified schema is `key STRING, value INT`. 
