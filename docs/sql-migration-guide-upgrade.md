...
## Upgrading From Spark SQL 2.4 to 3.0
  ...
  - Refreshing a cached table would trigger a table uncache operation and then a table cache (lazily) operation. In Spark version 2.4 and earlier, the cache name and cache storage level are not preserved before the uncache operation. Therefore, the cache name and cache storage level could be changed unexpectedly. Since Spark 3.0, cache name and cache storage level will be first preserved for cache recreation. It helps to maintain a consistent cache behavior upon table refreshing.
