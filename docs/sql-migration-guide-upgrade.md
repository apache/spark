...
## Upgrading From Spark SQL 2.4 to 3.0
  ...
  - Since Spark 3.0, JSON datasource infers TimestampType from string values if they matches to the pattern defined by the JSON option `timestampFormat`. Set JSON option `inferTimestamp` to `false` to disable such type inferring.
