...
- Since Spark 4.3, `spark.sql.execution.combineAdjacentAggregation` defaults to `true`. Spark now merges an adjacent partial/final aggregate pair (with no shuffle between them) into a single complete-mode aggregate. To restore the previous behavior, set `spark.sql.execution.combineAdjacentAggregation` to `false`.
