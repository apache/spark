...
- Since Spark 4.0, the `sentences` function, when `language` parameter is not null and `country` parameter is null, we no longer use `Locale.US`, instead use `Locale(language)`.
