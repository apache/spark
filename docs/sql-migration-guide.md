...
- Since Spark 4.4, when the result of casting a decimal value to timestamp falls outside the Long range after scaling and truncating sub-microsecond digits, Spark returns null (non-ANSI mode) or raises `CAST_OVERFLOW` error (ANSI mode) instead of a wrapping value. In non-ANSI mode and `TRY_CAST`, a decimal type that may overflow is treated as nullable, so casts into map keys or non-nullable array elements and struct fields that previously analyzed successfully may now fail type checking. Decimal types that cannot overflow retain the previous complex-type cast behavior.
