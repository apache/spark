...
- Since Spark 4.4, when `flatten` is asked to build an array larger than the maximum supported array length, generated code raises the same error as interpreted evaluation, `COLLECTION_SIZE_LIMIT_EXCEEDED.FUNCTION`, instead of the internal error `_LEGACY_ERROR_TEMP_2176`. `flatten` raises an error under exactly the same conditions as before; only the reported error condition changes.
