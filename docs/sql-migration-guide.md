...
- Since Spark 4.3, the Spark Thrift Server logs a warning at startup when `hive.server2.enable.doAs` is `true` and `hive.server2.authentication` verifies user identities (any recognized type other than `NONE`/`NOSASL`), because executor-side data access still runs as the server's own service identity rather than the connecting user (SPARK-5159). Set `spark.sql.hive.thriftServer.allowIneffectiveDoAs` to `true` to acknowledge the limitation and silence the warning. Spark 5.0 is expected to refuse to start on this configuration unless that option is set.
