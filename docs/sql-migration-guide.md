...
- Since Spark 4.4, `spark.sql.sources.v2.bucketing.partition.filter.enabled` defaults to `true`. In a storage-partitioned join, the partition key groups pushed down to both sides are now narrowed to those that can produce output for the join type, instead of always taking the union of the two sides' groups: an inner or semi join keeps only the groups present on both sides, a join that preserves the left side's rows keeps the left side's groups, a right outer join keeps the right side's, and a full outer join is unaffected. A group that is dropped is not scanned at all, so a query may read fewer files. To restore the previous behavior, set `spark.sql.sources.v2.bucketing.partition.filter.enabled` to `false`.
