...
- Since Spark 4.3, `unix_seconds`, `unix_millis`, and `unix_micros` accept `TIMESTAMP_NTZ` (and the nanosecond-precision timestamp types) directly and read them with no time-zone shift. Previously a `TIMESTAMP_NTZ` argument was not rejected but was implicitly cast to `TIMESTAMP_LTZ`, which applies a session-time-zone shift, so under a non-UTC session time zone these functions now return a different (zone-correct) result for `TIMESTAMP_NTZ` inputs. To get the previous value, cast the argument to `TIMESTAMP_LTZ` explicitly, for example `unix_seconds(CAST(ntz_col AS TIMESTAMP_LTZ))`.
