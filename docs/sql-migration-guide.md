...
- Since Spark 4.4, looking a key up in a map never matches a null key, so `m[k]` and `element_at(m, k)` return NULL rather than the null key's value. Map keys cannot normally be null, but the file-format readers build maps directly and do not reject them, so a null key can reach a query from sources such as Parquet or a Hive text table. Previously a lookup of a primitive key type's zero value could match a null key and return its value -- including when the map also held a real entry for that key, whose value was then shadowed. Lookups against such a map could also fail with a `NullPointerException` for non-primitive key types; they now return NULL as well.
