...
## Upgrading from Spark SQL 3.0 to 3.1
  ...
  - In Spark 3.1, temporary view created via `CACHE TABLE ... AS SELECT` will also have the same behavior with permanent view. In addition, when the temporary view is dropped, Spark will invalidate all cache dependents of the view, as opposed to just uncache the temporary view itself. To restore the behavior before Spark 3.1, you can set `spark.sql.legacy.storeAnalyzedPlanForView` to `true`.
