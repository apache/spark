...
- Since Spark 4.0, reading from a file data source will correctly respect query options, e.g. delimiters. Previously, the first query plan was cached and subsequent option changes ignored. To restore the previous behavior, set `spark.sql.legacy.readDataSourceCacheIgnoreOptions` to `true`.
