...
- Since Spark 4.4, when overflowing during casting numeric types (double, float) to timestamp,
  Spark will return null under non-ansi mode instead of an incorrect value caused by numeric
  overflow. In ANSI mode, the exact positive boundary is now recognized as overflow, so `CAST`
  raises `CAST_OVERFLOW` and `TRY_CAST` returns null instead of a clamped timestamp.
