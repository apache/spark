...
- Since Spark 4.2, duplicate Common Table Expression (CTE) names within a single `WITH` clause are detected case-insensitively, so names that differ only in case (for example, `WITH a AS (...), A AS (...)`) are rejected. Rename the conflicting CTEs.
...
- Since Spark 4.2, when a user-defined function (UDF) has the same name as a built-in parameterless function, the built-in function takes precedence during name resolution. Rename or qualify the UDF to keep invoking the user-defined version.
