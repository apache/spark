...
## Upgrading from Spark SQL 3.1 to 3.2
  ...
  - In Spark 3.2, a `SHOW TBLPROPERTIES` clause's output both show `key` and `value` columns whether `propertyKey` is defined. In Spark 3.1 and earlier, a `SHOW TBLPROPERTIES` clause's output only show a `value` column when `propertyKey` is defined.
