...
Spark SQL CLI supports running SQL from initialization script file(`-i`) or normal SQL file(`-f`), If path is not absolute, the path will be handled as local file.
