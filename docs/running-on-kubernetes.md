...
request are increased by `spark.kubernetes.executor.resizeFactor`. There is no configured upper
bound, so the memory keeps growing while the usage stays above the threshold. The pod is patched
through the `resize` subresource, so the container is not restarted.
...
grows, never shrinks, and has no configured upper bound; a PVC whose resize failed is skipped for the
rest of the application. While an expansion is still in progress, the plugin waits instead of
requesting another one. When PVCs are owned and reused by the driver (see
