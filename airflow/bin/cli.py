...
def backfill(args):
    ...
    if args.dry_run:
        ...
        dag.run(
            ...
            local=args.local,
            donot_pickle=(args.donot_pickle or configuration.conf.getboolean('core', 'donot_pickle')),
...
def run(args):
    ...
    if configuration.conf.get('core', 'S3_LOG_FOLDER').startswith('s3:'):
        import boto
        s3_log = filename.replace(log, configuration.conf.get('core', 'S3_LOG_FOLDER'))
...
def webserver(args):
    ...
    app = cached_app(configuration.conf)
    threads = args.threads or configuration.conf.get('webserver', 'threads')
    if args.debug:
        print(
...
def serve_logs(args):
    ...
    WORKER_LOG_SERVER_PORT = \
        int(configuration.conf.get('celery', 'WORKER_LOG_SERVER_PORT'))
    flask_app.run(
...
def initdb(args):
    print("DB: " + configuration.conf.get('core', 'SQL_ALCHEMY_CONN'))
    utils.initdb()
    print("Done.")
...
def flower(args):
    broka = configuration.conf.get('celery', 'BROKER_URL')
    args.port = args.port or configuration.conf.get('celery', 'FLOWER_PORT')
    port = '--port=' + args.port
    api = ''
    if args.broker_api:
...
def get_parser():
    ...
    parser_webserver.add_argument(
        "-p", "--port",
        default=configuration.conf.get('webserver', 'WEB_SERVER_PORT'),
        type=int,
        help="Set the port on which to run the web server")
    parser_webserver.add_argument(
        "-w", "--threads",
        default=configuration.conf.get('webserver', 'THREADS'),
        type=int,
        help="Number of threads to run the webserver on")
    parser_webserver.add_argument(
        "-hn", "--hostname",
        default=configuration.conf.get('webserver', 'WEB_SERVER_HOST'),
    ...
    parser_worker.add_argument(
        ...
        default=configuration.conf.get('celery', 'DEFAULT_QUEUE'))
    parser_worker.add_argument(
        "-c", "--concurrency",
        type=int,
        help="The number of worker processes",
        default=configuration.conf.get('celery', 'celeryd_concurrency'))
    ...
    parser_kerberos.add_argument(
        ...
        nargs='?', default=configuration.conf.get('kerberos', 'keytab'))
    parser_kerberos.add_argument(
        "principal", help="kerberos principal",
        nargs='?', default=configuration.conf.get('kerberos', 'principal'))
