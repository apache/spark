...
class S3FileTransformOperator(BaseOperator):
    ...
    def execute(self, context):
        logging.info("Downloading source S3 file {0}".format(self.source_s3_key))
        if not self.source_s3.check_for_key(self.source_s3_key):
            raise Exception("The source key {0} does not exist".format(self.source_s3_key))
        ...
        with NamedTemporaryFile("w") as f_source, NamedTemporaryFile("w") as f_dest:
            logging.info("Dumping S3 file {0}"
                " contents to local file {1}".format(self.source_s3_key, f_source.name))
