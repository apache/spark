...
class DAGSchedulerSuite extends SparkFunSuite with TempLocalSparkContext with TimeLimits {
  ...
  test("pipelined shuffle: a finishOnly replay landing on a torn-down stage does not re-post " +
    ...
    // stage/job bookkeeping is re-posted (finishOnly=true) once the producer finishes. If the
    // group is torn down (a sibling aborts) between that re-post and its dequeue, the replayed
    // event lands on an already-removed stage and hits the cancelled-stage guard in
    // handleTaskCompletion. That guard must NOT re-post TaskEnd for a finishOnly event -- the
    // per-task TaskEnd already fired, so re-posting would double-count it in listeners
    // (AppStatusListener's active-task count could even go negative). This drives that exact path:
    // buffer a consumer completion (TaskEnd fires once), tear the stage down, then deliver the
    // finishOnly replay and assert it adds no further TaskEnd.
    val taskEndCount = new java.util.concurrent.atomic.AtomicInteger(0)
    val countingListener = new SparkListener {
      override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = taskEndCount.incrementAndGet()
    }
    ...
    try {
      ...
      val consumerStageId = consumerTaskSet.stageId

      // Consumer partition 0 succeeds -> its TaskEnd fires inline (count 1); bookkeeping deferred.
      val replayEvent = makeCompletionEvent(consumerTaskSet.tasks(0), Success, 42)
      runEvent(replayEvent)
      sc.listenerBus.waitUntilEmpty()
      assert(taskEndCount.get() === 1, "the consumer task's TaskEnd fires inline exactly once")

      // Tear the group down (producer FetchFailed on the consumer's other task -> group abort),
      // which removes the consumer stage from stageIdToStage. (This legitimately posts its OWN
      // TaskEnd for the failed task, so we snapshot the count AFTER teardown and assert the replay
      // below adds nothing on top -- isolating the finishOnly replay from the abort's own events.)
      runEvent(makeCompletionEvent(
      ...
      assert(!scheduler.stageIdToStage.contains(consumerStageId),
        "the consumer stage must have been torn down by the abort")
      sc.listenerBus.waitUntilEmpty()
      val countBeforeReplay = taskEndCount.get()

      // Now deliver the finishOnly replay of partition 0's completion, as the release path would
      // have posted it -- but it arrives after teardown. The cancelled-stage guard must swallow it
      // WITHOUT re-posting TaskEnd, so the count must not change.
      runEvent(replayEvent.copy(finishOnly = true))
      sc.listenerBus.waitUntilEmpty()
      assert(taskEndCount.get() === countBeforeReplay,
        s"a finishOnly replay on a torn-down stage must not re-post TaskEnd; count went from " +
          s"$countBeforeReplay to ${taskEndCount.get()}")
    } finally {
      sc.removeSparkListener(countingListener)
    }
  }
