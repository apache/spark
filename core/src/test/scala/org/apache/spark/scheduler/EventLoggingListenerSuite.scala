      ...
      case executorMetrics: SparkListenerStageExecutorMetrics =>
          expectedEvents.get((stageId, executorMetrics.execId)) match {
            case Some(expectedMetrics) =>
              ...
              assert(executorMetrics.executorMetrics.length ===
                expectedMetrics.executorMetrics.length)
              (0 until MetricGetter.values.length).foreach { idx =>
                assert(executorMetrics.executorMetrics(idx) ===
                  expectedMetrics.executorMetrics(idx))
              }
            case None =>
              assert(false)
        }
