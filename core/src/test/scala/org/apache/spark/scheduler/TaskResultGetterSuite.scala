...
class TaskResultGetterSuite extends SparkFunSuite with BeforeAndAfter with LocalSparkContext {
  ...
  test("task result size is set on the driver, not the executors") {
    ...
    val resSizeBefore = resBefore.accumUpdates.find(_.name == RESULT_SIZE).flatMap(_.update)
    val resSizeAfter = resAfter.accumUpdates.find(_.name == RESULT_SIZE).flatMap(_.update)
