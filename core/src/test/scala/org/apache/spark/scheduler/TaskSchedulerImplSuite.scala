...
class TaskSchedulerImplSuite extends SparkFunSuite with LocalSparkContext
  ...
  test("SPARK-57878: Binpack assignment concentrates tasks onto a part of executors") {
    ...
    val perExecutor = taskDescriptions.groupBy(_.executorId).map { case (id, ts) => id -> ts.size }
  ...
  test("SPARK-57878: Rejects an unknown assignment strategy") {
    ...
    assert(error.getMessage.contains(
      "The value of spark.scheduler.taskAssignmentStrategy should be one of " +
        "roundrobin, binpack, balance, none, but was does-not-exist"))
  }
