...
class TaskSetManagerSuite extends SparkFunSuite with LocalSparkContext with Logging {
  ...
  test("TaskSet with no preferences") {
    ...
    val taskOption = manager.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
  ...
  test("multiple offers with no preferences") {
    ...
    assert(manager.resourceOffer("exec1", "host1", NO_PREF, Map.empty) === None)
  ...
  test("skip unsatisfiable locality levels") {
    ...
    assert(manager.resourceOffer("execC", "host2", ANY, Map.empty) === None)
    ...
    assert(manager.resourceOffer("execA", "host1", ANY, Map.empty).get.index === 0)
  }
  ...
  test("basic delay scheduling") {
    ...
    assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty).get.index === 0)
    assert(manager.resourceOffer("exec1", "host1", PROCESS_LOCAL, Map.empty) == None)
    ...
    assert(manager.resourceOffer("exec1", "host1", NODE_LOCAL, Map.empty).get.index == 2)

    // Offer host2, exec2, at NODE_LOCAL level: we should choose task 2
    assert(manager.resourceOffer("exec2", "host2", NODE_LOCAL, Map.empty).get.index == 1)

    // Offer host2, exec2 again, at NODE_LOCAL level: we should get noPref task
    // after failing to find a node_Local task
    assert(manager.resourceOffer("exec2", "host2", NODE_LOCAL, Map.empty) == None)
    clock.advance(LOCALITY_WAIT_MS)
    assert(manager.resourceOffer("exec2", "host2", NO_PREF, Map.empty).get.index == 3)
  }
  ...
  test("we do not need to delay scheduling when we only have noPref tasks in the queue") {
    ...
    val manager = new TaskSetManager(sched, taskSet, MAX_TASK_FAILURES, clock = clock)
    // First offer host1, exec1: first task should be chosen
    assert(manager.resourceOffer("exec1", "host1", PROCESS_LOCAL, Map.empty).get.index === 0)
    assert(manager.resourceOffer("exec3", "host2", PROCESS_LOCAL, Map.empty).get.index === 1)
    assert(manager.resourceOffer("exec3", "host2", NODE_LOCAL, Map.empty) == None)
    assert(manager.resourceOffer("exec3", "host2", NO_PREF, Map.empty).get.index === 2)
  }
  ...
  test("delay scheduling with fallback") {
    ...
    assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty) === None)

    clock.advance(LOCALITY_WAIT_MS)

    // Offer host1 again: second task (on host2) should get chosen
    assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty).get.index === 1)

    // Offer host1 again: third task (on host2) should get chosen
    assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty).get.index === 2)

    // Offer host2: fifth task (also on host2) should get chosen
    assert(manager.resourceOffer("exec2", "host2", ANY, Map.empty).get.index === 4)

    // Now that we've launched a local task, we should no longer launch the task for host3
    assert(manager.resourceOffer("exec2", "host2", ANY, Map.empty) === None)

    clock.advance(LOCALITY_WAIT_MS)

    // After another delay, we can go ahead and launch that task non-locally
    assert(manager.resourceOffer("exec2", "host2", ANY, Map.empty).get.index === 3)
  }
  ...
  test("repeated failures lead to task set abortion") {
    ...
    (1 to manager.maxTaskFailures).foreach { index =>
      val offerResult = manager.resourceOffer("exec1", "host1", ANY, Map.empty)
  ...
  test("executors should be blacklisted after task failure, in spite of locality preferences") {
    ...
    {
      val offerResult = manager.resourceOffer("exec1", "host1", PROCESS_LOCAL, Map.empty)
      ...
      assert(manager.resourceOffer("exec1", "host1", PROCESS_LOCAL, Map.empty).isEmpty)
      assert(manager.resourceOffer("exec1", "host1", NODE_LOCAL, Map.empty).isEmpty)
      assert(manager.resourceOffer("exec1", "host1", RACK_LOCAL, Map.empty).isEmpty)
      assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty).isEmpty)
    }

    // Run the task on exec1.1 - should work, and then fail it on exec1.1
    {
      val offerResult = manager.resourceOffer("exec1.1", "host1", NODE_LOCAL, Map.empty)
      assert(offerResult.isDefined,
      ...
      assert(manager.resourceOffer("exec1.1", "host1", NODE_LOCAL, Map.empty).isEmpty)
    }

    // Run the task on exec2 - should work, and then fail it on exec2
    {
      val offerResult = manager.resourceOffer("exec2", "host2", ANY, Map.empty)
      assert(offerResult.isDefined, "Expect resource offer to return a task")
      ...
      assert(manager.resourceOffer("exec2", "host2", ANY, Map.empty).isEmpty)
    }
    ...
    {
      val offerResult = manager.resourceOffer("exec3", "host3", ANY, Map.empty)
      assert(offerResult.isDefined)
      assert(offerResult.get.index === 0)
      assert(offerResult.get.executorId === "exec3")

      assert(manager.resourceOffer("exec3", "host3", ANY, Map.empty).isEmpty)
  ...
  test("Executors exit for reason unrelated to currently running tasks") {
    ...
    sched.addExecutor("execC", "host2")
    manager.executorAdded()
    assert(manager.resourceOffer("exec1", "host1", ANY, Map.empty).isDefined)
    ...
    assert(!sched.taskSetsFailed.contains(taskSet.id))
    assert(manager.resourceOffer("execC", "host2", ANY, Map.empty).isDefined)
    sched.removeExecutor("execC")
    manager.executorLost(
  ...
  test("test RACK_LOCAL tasks") {
    ...
    assert(manager.resourceOffer("execC", "host3", RACK_LOCAL, Map.empty) === None)
    // Task 0 can be scheduled with ANY
    assert(manager.resourceOffer("execC", "host3", ANY, Map.empty).get.index === 0)
    // Offer host2
    // Task 1 can be scheduled with RACK_LOCAL
    assert(manager.resourceOffer("execB", "host2", RACK_LOCAL, Map.empty).get.index === 1)
  }
  ...
  test("Not serializable exception thrown if the task cannot be serialized") {
    ...
    intercept[TaskNotSerializableException] {
      manager.resourceOffer("exec1", "host1", ANY, Map.empty)
    }
  ...
  test("[SPARK-13931] taskSetManager should not send Resubmitted tasks after being a zombie") {
    ...
    val task1 = manager.resourceOffer("execA", "host1", TaskLocality.PROCESS_LOCAL, Map.empty).get
    ...
    val task2 = manager.resourceOffer("execB", "host2", TaskLocality.ANY, Map.empty).get
  ...
  test("[SPARK-22074] Task killed by other attempt task should not be resubmitted") {
    ...
    for ((exec, host) <- Seq(
      ...
      "exec3" -> "host3",
      "exec2" -> "host2")) {
      val taskOption = manager.resourceOffer(exec, host, NO_PREF, Map.empty)
      assert(taskOption.isDefined)
      val task = taskOption.get
    ...
    val taskOption = manager.resourceOffer("exec2", "host2", ANY, Map.empty)
    assert(taskOption.isDefined)
  ...
  test("speculative and noPref task should be scheduled after node-local") {
    ...
    assert(manager.resourceOffer("execA", "host1", PROCESS_LOCAL, Map.empty).get.index === 0)
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty) == None)
    assert(manager.resourceOffer("execA", "host1", NO_PREF, Map.empty).get.index == 1)

    manager.speculatableTasks += 1
    clock.advance(LOCALITY_WAIT_MS)
    // schedule the nonPref task
    assert(manager.resourceOffer("execA", "host1", NO_PREF, Map.empty).get.index === 2)
    // schedule the speculative task
    assert(manager.resourceOffer("execB", "host2", NO_PREF, Map.empty).get.index === 1)
    clock.advance(LOCALITY_WAIT_MS * 3)
    // schedule non-local tasks
    assert(manager.resourceOffer("execB", "host2", ANY, Map.empty).get.index === 3)
  }
  ...
  test("node-local tasks should be scheduled right away " +
    ...
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty).get.index === 0)
    assert(manager.resourceOffer("execA", "host2", NODE_LOCAL, Map.empty).get.index === 1)
    assert(manager.resourceOffer("execA", "host3", NODE_LOCAL, Map.empty).get.index === 3)
    assert(manager.resourceOffer("execA", "host3", NODE_LOCAL, Map.empty) === None)

    // schedule no-preference after node local ones
    assert(manager.resourceOffer("execA", "host3", NO_PREF, Map.empty).get.index === 2)
  }
  ...
  test("SPARK-4939: node-local tasks should be scheduled right after process-local tasks finished")
  {
    ...
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty).get.index === 2)
    assert(manager.resourceOffer("execB", "host2", NODE_LOCAL, Map.empty).get.index === 3)
    // node-local tasks are scheduled without delay
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty).get.index === 0)
    assert(manager.resourceOffer("execB", "host2", NODE_LOCAL, Map.empty).get.index === 1)
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty) == None)
    assert(manager.resourceOffer("execB", "host2", NODE_LOCAL, Map.empty) == None)
  }
  ...
  test("SPARK-4939: no-pref tasks should be scheduled after process-local tasks finished") {
    ...
    // process-local tasks are scheduled first
    assert(manager.resourceOffer("execA", "host1", PROCESS_LOCAL, Map.empty).get.index === 1)
    assert(manager.resourceOffer("execB", "host2", PROCESS_LOCAL, Map.empty).get.index === 2)
    // no-pref tasks are scheduled without delay
    assert(manager.resourceOffer("execA", "host1", PROCESS_LOCAL, Map.empty) == None)
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL, Map.empty) == None)
    assert(manager.resourceOffer("execA", "host1", NO_PREF, Map.empty).get.index === 0)
    assert(manager.resourceOffer("execA", "host1", ANY, Map.empty) == None)
  }
  ...
  test("Ensure TaskSetManager is usable after addition of levels") {
    ...
    assert(manager.resourceOffer("execB.2", "host2", ANY, Map.empty) !== None)
    sched.removeExecutor("execA")
    ...
    assert(manager.resourceOffer("execC", "host3", ANY, Map.empty) !== None)
  }
  ...
  test("Kill other task attempts when one attempt belonging to the same task succeeds") {
    ...
    for ((k, v) <- List(
        ...
        "exec2" -> "host2",
        "exec2" -> "host2")) {
      val taskOption = manager.resourceOffer(k, v, NO_PREF, Map.empty)
      assert(taskOption.isDefined)
      val task = taskOption.get
    ...
    val taskOption5 = manager.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
  ...
  test("Killing speculative tasks does not count towards aborting the taskset") {
    ...
    (0 until 3).foreach { attempt =>
      Seq(3, 4).foreach { index =>
        ...
        assert(!manager.isZombie)
        val nextTask = manager.resourceOffer(s"exec2", s"host2", NO_PREF, Map.empty)
    ...
    val taskOpt6 = manager.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
  ...
  test("don't update blacklist for shuffle-fetch failures, preemption, denied commits, " +
    ...
    val taskDescs = Seq(
      ...
      (0 until 2).flatMap{ _ => tsmSpy.resourceOffer(exec, host, TaskLocality.ANY, Map.empty)}
    }
  ...
  test("update application blacklist for shuffle-fetch") {
    ...
    val taskDescs = Seq(
      ...
      // offer each executor twice (simulating 2 cores per executor)
      (0 until 2).flatMap{ _ => tsm.resourceOffer(exec, host, TaskLocality.ANY, Map.empty)}
    }
    assert(taskDescs.size === 4)
  ...
  test("update blacklist before adding pending task to avoid race condition") {
    ...
    val taskDesc = taskSetManagerSpy.resourceOffer(exec, host, TaskLocality.ANY, Map.empty)
  ...
  test("SPARK-21563 context's added jars shouldn't change mid-TaskSet") {
    ...
    val taskOption1 = manager1.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
    assert(taskOption1.get.addedJars === addedJarsPreTaskSet)
    val taskOption2 = manager1.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
    ...
    val taskOption3 = manager1.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
    ...
    val taskOption4 = manager2.resourceOffer("exec1", "host1", NO_PREF, Map.empty)
