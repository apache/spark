...
class TaskSetManagerSuite extends SparkFunSuite with LocalSparkContext with Logging {
  ...
  test("Fetch failed task should not have success completion event") {
    sc = new SparkContext("local", "test")
    // Set the speculation multiplier to be 0 so speculative tasks are launched immediately
    sc.conf.set("spark.speculation.multiplier", "0.0")
    sc.conf.set("spark.speculation", "true")
    ...
    sched.initialize(new FakeSchedulerBackend() {
      override def killTask(taskId: Long,
          ...
          reason: String): Unit = {
        // Check the only one killTask event in this case, which triggered by
        // task 3.1 fetch failed.
        assert(taskId === 3)
        assert(executorId === "exec2")
        assert(interruptThread)
        assert(reason === "another attempt fetch failed")
        killTaskCalled = true
      }
    })

    // Keep track of the original tasks will not has SUCCESS event.
    var originTaskSuccess = false
    val dagScheduler = new FakeDAGScheduler(sc, sched) {
      override def taskEnded(task: Task[_],
        ...
        super.taskEnded(task, reason, result, accumUpdates, taskInfo)
        reason match {
          case Success if taskInfo.taskId == 3 &&
              taskInfo.attemptNumber == 0 && taskInfo.index == 3 =>
            originTaskSuccess = true
          case _ =>
        }
      }
    }
    ...
    // The speculative task raise a FetchFailed
    manager.handleFailedTask(4, TaskState.FAILED,
      FetchFailed(BlockManagerId("exec1", "host1", 12345), 0, 0, 0, "ignored"))
    // Check the original task killed by FetchFailed.
    assert(!originTaskSuccess && killTaskCalled)
  }
