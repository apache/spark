...
class TaskSetManagerSuite extends SparkFunSuite with LocalSparkContext with Logging {
  ...
  test("delay scheduling with failed hosts") {
    ...
    manager.executorLost("exec2", "host2", new ExecutorLossReason("Unknown"))
  ...
  test("new executors get added and lost") {
    ...
    manager.executorLost("execC", "host2", new ExecutorLossReason("Unknown"))
    assert(manager.myLocalityLevels.sameElements(Array(NODE_LOCAL, NO_PREF, ANY)))
    sched.removeExecutor("execD")
    manager.executorLost("execD", "host1", new ExecutorLossReason("Unknown"))
    assert(manager.myLocalityLevels.sameElements(Array(NO_PREF, ANY)))
  }
  ...
  test("Ensure TaskSetManager is usable after addition of levels") {
    ...
    manager.executorLost("execA", "host1", new ExecutorLossReason("Unknown"))
    manager.executorLost("execB.2", "host2", new ExecutorLossReason("Unknown"))
    clock.advance(LOCALITY_WAIT_MS * 4)
