...
class TaskSetManagerSuite extends SparkFunSuite with LocalSparkContext with Logging {
  ...
  test("Blacklisting individual tasks") {
    ...
    sc = new SparkContext("local", "test")
    sched = new FakeTaskScheduler(sc, ("exec1", "hostA"), ("exec2", "hostA"))
    val clock = new ManualClock
    val blacklist = mock(classOf[BlacklistTracker])
    when(blacklist.BLACKLIST_TIMEOUT_MILLIS).thenReturn(BlacklistTracker.getBlacklistTimeout(conf))

    val taskSet = FakeTask.createTaskSet(numTasks = 10)
    val tsm = new TaskSetManager(sched, taskSet, 4, Some(blacklist), clock)
    clock.setTime(0)
    tsm.updateBlacklistForFailedTask("hostA", exec = "exec1", index = 0)
    for {
    ...
    assert(!tsm.isExecutorBlacklistedForTaskSet("exec1"))
    ...
    tsm.updateBlacklistForFailedTask("hostA", exec = "exec1", index = 1)
    assert(tsm.isExecutorBlacklistedForTaskSet("exec1"))
    assert(!tsm.isNodeBlacklistedForTaskSet("hostA"))
    tsm.updateBlacklistForFailedTask("hostA", exec = "exec2", index = 0)
    assert(tsm.isExecutorBlacklistedForTaskSet("exec1"))
    assert(!tsm.isExecutorBlacklistedForTaskSet("exec2"))
    assert(!tsm.isNodeBlacklistedForTaskSet("hostA"))
    tsm.updateBlacklistForFailedTask("hostA", exec = "exec2", index = 1)
    assert(tsm.isExecutorBlacklistedForTaskSet("exec1"))
    assert(tsm.isExecutorBlacklistedForTaskSet("exec2"))
    assert(tsm.isNodeBlacklistedForTaskSet("hostA"))
    for {
    ...
    assert(tsm.isNodeBlacklistedForTaskSet("hostA"))
    val execToFailures = tsm.execToFailures
  ...
  test("executors should be blacklisted after task failure, in spite of locality preferences") {
    ...
    val conf = new SparkConf().
      set(config.BLACKLIST_ENABLED.key, "true").
      set(config.BLACKLIST_TIMEOUT_CONF.key, rescheduleDelay.toString).
