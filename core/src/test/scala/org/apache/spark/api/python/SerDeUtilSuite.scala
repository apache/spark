...
class SerDeUtilSuite extends FunSuite with SharedSparkContext {
    ...
    test("Check converting an empty pair RDD to python does not throw an exception (SPARK-5441)") {
        val emptyRdd = sc.makeRDD(Seq[(Any, Any)]())
        SerDeUtil.pairRDDToPython(emptyRdd, 10)
    }

    test("Check converting an empty python RDD to pair RDD does not throw an exception (SPARK-5441)") {
        val emptyRdd = sc.makeRDD(Seq[(Any, Any)]())
