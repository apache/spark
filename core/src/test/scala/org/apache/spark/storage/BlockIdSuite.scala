...
class BlockIdSuite extends SparkFunSuite {
  ...
  test("shuffle") {
    ...
    assert(id.name === "shuffle_1_2_3_0")
  ...
  test("shuffle data") {
    ...
    assert(id.name === "shuffle_4_5_6_0.data")
    assert(id.asRDDId === None)
  ...
  test("shuffle index") {
    ...
    assert(id.name === "shuffle_7_8_9_0.index")
    assert(id.asRDDId === None)
  ...
  test("extend shuffle") {
    ...
    assertDifferent(id, ShuffleBlockId(3, 2, 3, 5))
    assert(id.name === "shuffle_1_2_3_4")
    assert(id.asRDDId === None)
    assert(id.shuffleId === 1)
    assert(id.mapId === 2)
    assert(id.reduceId === 3)
    assert(id.stageAttemptId === 4)
    assert(id.isShuffle)
    assertSame(id, BlockId(id.toString))
  }

  test("extend shuffle data") {
    val id = ShuffleDataBlockId(4, 5, 6, 7)
    assertSame(id, ShuffleDataBlockId(4, 5, 6, 7))
    assertDifferent(id, ShuffleDataBlockId(6, 5, 6, 8))
    assert(id.name === "shuffle_4_5_6_7.data")
    assert(id.asRDDId === None)
    assert(id.shuffleId === 4)
    assert(id.mapId === 5)
    assert(id.reduceId === 6)
    assert(id.stageAttemptId === 7)
    assert(!id.isShuffle)
    assertSame(id, BlockId(id.toString))
  }

  test("extend shuffle index") {
    val id = ShuffleIndexBlockId(7, 8, 9, 10)
    assertSame(id, ShuffleIndexBlockId(7, 8, 9, 10))
    assertDifferent(id, ShuffleIndexBlockId(9, 8, 9, 11))
    assert(id.name === "shuffle_7_8_9_10.index")
    assert(id.asRDDId === None)
    assert(id.shuffleId === 7)
    assert(id.mapId === 8)
    assert(id.reduceId === 9)
    assert(id.stageAttemptId === 10)
    assert(!id.isShuffle)
    assertSame(id, BlockId(id.toString))
  }
