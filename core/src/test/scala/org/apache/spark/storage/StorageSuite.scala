...
class StorageSuite extends FunSuite {
  ...
  test("storage status add/remove RDD blocks") {
    ...
    assert(status.rddIds.toSet === Seq(0, 1, 2).toSet)
    assert(status.rddBlocks(0).size === 1)
    assert(status.rddBlocks(0).head._1 === RDDBlockId(0, 0))
    assert(status.rddBlocks(0).head._2 === BlockStatus(memAndDisk, 10L, 20L, 0L))
    assert(status.rddBlocks(1).size === 1)
    assert(status.rddBlocks(1).head._1 === RDDBlockId(1, 1))
    assert(status.rddBlocks(1).head._2 === BlockStatus(memAndDisk, 100L, 200L, 0L))
    assert(status.rddBlocks(2).size === 3)
    assert(status.rddBlocks(2).head._1 === RDDBlockId(2, 2))
    assert(status.rddBlocks(2).head._2 === BlockStatus(memAndDisk, 10L, 20L, 0L))
    ...
    assert(status.rddBlocks(0).size === 1)
    assert(status.rddBlocks(0).head._1 === RDDBlockId(0, 0))
    assert(status.rddBlocks(0).head._2 === BlockStatus(memAndDisk, 0L, 0L, 0L))
    assert(status.rddBlocks(2).size === 3)
    assert(status.rddBlocks(2).head._1 === RDDBlockId(2, 2))
    assert(status.rddBlocks(2).head._2 === BlockStatus(memAndDisk, 0L, 1000L, 0L))
    ...
    assert(status.rddIds.toSet === Seq(0, 2).toSet)
    assert(status.rddBlocks(0).size === 1)
    assert(status.rddBlocks(1).size === 0)
    assert(status.rddBlocks(2).size === 1)
    assert(status.memUsedByRDD(0) === 0L)
    ...
    assert(status.rddIds.toSet === Seq(0, 2, 3).toSet)
    assert(status.rddBlocks(0).size === 1)
    assert(status.rddBlocks(1).size === 0)
    assert(status.rddBlocks(2).size === 2)
    assert(status.rddBlocks(3).size === 1)
    assert(status.rddBlocks(3).head._1 === RDDBlockId(3, 5))
    assert(status.rddBlocks(3).head._2 === BlockStatus(memAndDisk, 10L, 200L, 0L))
    assert(status.memUsedByRDD(0) === 0L)
    assert(status.memUsedByRDD(1) === 0L)
    assert(status.memUsedByRDD(2) === 20L)
  ...
  test("StorageUtils.rddInfoFromStorageStatus") {
    val storageStatuses = stockStorageStatuses
    val rddInfos = stockRDDInfos
    StorageUtils.rddInfoFromStorageStatus(storageStatuses, rddInfos)
  ...
  test("StorageUtils.blockLocationsFromStorageStatus") {
    val storageStatuses = stockStorageStatuses
    val blockLocations1 = StorageUtils.blockLocationsFromStorageStatus(storageStatuses)
    assert(blockLocations1.contains(RDDBlockId(0, 0)))
    assert(blockLocations1.contains(RDDBlockId(0, 1)))
    assert(blockLocations1.contains(RDDBlockId(0, 2)))
    assert(blockLocations1.contains(RDDBlockId(0, 3)))
    assert(blockLocations1.contains(RDDBlockId(0, 4)))
    ...
    assert(blockLocations1.size === 8)
    assert(blockLocations1(RDDBlockId(0, 0)) === Seq("dog:1"))
    assert(blockLocations1(RDDBlockId(0, 1)) === Seq("dog:1"))
    assert(blockLocations1(RDDBlockId(0, 2)) === Seq("duck:2"))
    assert(blockLocations1(RDDBlockId(0, 3)) === Seq("duck:2"))
    assert(blockLocations1(RDDBlockId(1, 0)) === Seq("duck:2"))
    assert(blockLocations1(RDDBlockId(1, 1)) === Seq("duck:2"))
    assert(blockLocations1(RDDBlockId(0, 4)) === Seq("cat:3"))
    ...
    val blockLocations2 = StorageUtils.blockLocationsFromStorageStatus(storageStatuses)
    assert(blockLocations2.contains(RDDBlockId(0, 0)))
    assert(blockLocations2.contains(RDDBlockId(0, 1)))
    assert(blockLocations2.contains(RDDBlockId(0, 2)))
    assert(blockLocations2.contains(RDDBlockId(0, 3)))
    assert(blockLocations2.contains(RDDBlockId(0, 4)))
    assert(blockLocations2.contains(RDDBlockId(1, 0)))
    assert(blockLocations2.contains(RDDBlockId(1, 1)))
    assert(blockLocations2.contains(RDDBlockId(1, 2)))
    assert(blockLocations2.size === 8)
    assert(blockLocations2(RDDBlockId(0, 0)) === Seq("dog:1", "cat:3"))
    assert(blockLocations2(RDDBlockId(0, 1)) === Seq("dog:1"))
    assert(blockLocations2(RDDBlockId(0, 2)) === Seq("duck:2"))
    assert(blockLocations2(RDDBlockId(0, 3)) === Seq("duck:2"))
    assert(blockLocations2(RDDBlockId(1, 0)) === Seq("dog:1", "duck:2"))
    assert(blockLocations2(RDDBlockId(1, 1)) === Seq("duck:2"))
    assert(blockLocations2(RDDBlockId(0, 4)) === Seq("dog:1", "cat:3"))
    assert(blockLocations2(RDDBlockId(1, 2)) === Seq("cat:3"))
  }

  test("StorageUtils.filterStorageStatusByRDD") {
    val storageStatuses = stockStorageStatuses
    val filteredStorageStatuses0 = StorageUtils.filterStorageStatusByRDD(storageStatuses, 0)
    val filteredStorageStatuses1 = StorageUtils.filterStorageStatusByRDD(storageStatuses, 1)
    assert(filteredStorageStatuses0.size === 3)
    assert(filteredStorageStatuses1.size === 2)
  }
