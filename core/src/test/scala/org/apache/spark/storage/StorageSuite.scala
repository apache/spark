...
class StorageSuite extends FunSuite {
  ...
  test("storage status memUsed, diskUsed, extBlkStoreUsed") {
    val status = storageStatus2
    ...
    def actualOffHeapUsed: Long = status.blocks.values.map(_.extBlkStoreSize).sum
  ...
  test("StorageUtils.updateRddInfo") {
    ...
    assert(rddInfos(0).extBlkStoreSize === 0L)
    ...
    assert(rddInfos(1).extBlkStoreSize === 0L)
  }
