...
class StorageSuite extends FunSuite {
  ...
  test("storage status memUsed, diskUsed, offheapUsed") {
    val status = storageStatus2
    ...
    def actualOffHeapUsed: Long = status.blocks.values.map(_.offHeapSize).sum
  ...
  test("StorageUtils.updateRddInfo") {
    ...
    assert(rddInfos(0).offHeapSize === 0L)
    ...
    assert(rddInfos(1).offHeapSize === 0L)
  }
