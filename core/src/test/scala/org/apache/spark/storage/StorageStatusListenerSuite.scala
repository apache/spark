...
class StorageStatusListenerSuite extends FunSuite {
  ...
  test("task end without updated blocks") {
    ...
    assert(listener.executorIdToStorageStatus("big").numBlocks === 0)
    assert(listener.executorIdToStorageStatus("fat").numBlocks === 0)
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo1, taskMetrics))
    assert(listener.executorIdToStorageStatus("big").numBlocks === 0)
    assert(listener.executorIdToStorageStatus("fat").numBlocks === 0)
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo2, taskMetrics))
    assert(listener.executorIdToStorageStatus("big").numBlocks === 0)
    assert(listener.executorIdToStorageStatus("fat").numBlocks === 0)
  }
  ...
  test("task end with updated blocks") {
    ...
    assert(listener.executorIdToStorageStatus("big").numBlocks === 0)
    assert(listener.executorIdToStorageStatus("fat").numBlocks === 0)
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo1, taskMetrics1))
    ...
    assert(listener.executorIdToStorageStatus("fat").numBlocks === 0)
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo2, taskMetrics2))
    assert(listener.executorIdToStorageStatus("big").numBlocks === 2)
  ...
  test("unpersist RDD") {
    ...
    taskMetrics2.updatedBlocks = Some(Seq(block3))
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo1, taskMetrics1))
    listener.onTaskEnd(SparkListenerTaskEnd(1, 0, "obliteration", TaskSucceeded, taskInfo1, taskMetrics2))
