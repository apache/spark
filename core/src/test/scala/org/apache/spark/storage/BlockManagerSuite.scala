...
class BlockManagerSuite extends SparkFunSuite with Matchers with BeforeAndAfterEach
  ...
  test("block failed to unroll should not be replicated for MEMORY_ONLY_2") {
    ...
    store.putIterator("someBlock1", bigList.iterator, StorageLevel.MEMORY_ONLY_2)
    val memoryStore = store.memoryStore
    val memoryStore2 = store2.memoryStore
    // check to see if block is replicated
    val memSize = if (memoryStore.contains("someBlock1")) memoryStore.getSize("someBlock1") else 0L
    val memSize2 = if (memoryStore2.contains("someBlock1")) memoryStore2.getSize("someBlock1") else 0L
    assert(memSize == 0)
    assert(memSize2 == 0)
  }
  ...
  test("block failed to unroll will be replicated for MEMORY_AND_DISK_2") {
    ...
    store.putIterator("someBlock1", bigList.iterator, StorageLevel.MEMORY_AND_DISK_2)
    val memoryStore = store.memoryStore
    val memoryStore2 = store2.memoryStore
    // check to see if block is replicated
    val memSize = if (memoryStore.contains("someBlock1")) memoryStore.getSize("someBlock1") else 0L
    val diskSize = if (diskStore.contains("someBlock1")) diskStore.getSize("someBlock1") else 0L
    val memSize2 = if (memoryStore2.contains("someBlock1")) memoryStore2.getSize("someBlock1") else 0L
    assert(memSize == 0)
    assert(diskSize > 0)
    assert(memSize2 > 0)
