...
import org.apache.spark.scheduler.{LiveListenerBus, MapStatus, MapTaskResult, SparkListenerBlockUpdated}
...
class BlockManagerSuite extends SparkFunSuite with Matchers with BeforeAndAfterEach
  ...
  val mapOutputTracker = new MapOutputTrackerMaster(
    new SparkConf(false), new NoOpShuffleOutputTracker, bcastManager, true)
  ...
  test("test migration of shuffle blocks during decommissioning") {
    ...
    try {
      mapOutputTracker.registerMapOutput(
        ...
        0,
        MapTaskResult(
          MapStatus(bm1.blockManagerId, Array(blockSize), 0),
          None))
      ...
      mapOutputTracker.unregisterShuffle(0, true)
      // Avoid thread leak
