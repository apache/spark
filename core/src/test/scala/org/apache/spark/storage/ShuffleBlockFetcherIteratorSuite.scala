...
class ShuffleBlockFetcherIteratorSuite extends SparkFunSuite with PrivateMethodTester {
  ...
  private def verifyFetchBlocksInvocationCount(
  ...
  test("successful 3 local + 4 host local + 2 remote reads") {
    ...
    verifyFetchBlocksInvocationCount(transfer, 1)
  ...
  test("Hit maxBytesInFlight limitation before maxBlocksInFlightPerAddress") {
    ...
    val iterator = createShuffleBlockIteratorWithDefaults(
      transfer,
      blocksByAddress,
      maxBytesInFlight = 1000L // allow 1 FetchRequests at most at the same time
    )
    ...
    verifyFetchBlocksInvocationCount(transfer, 2)
    assert(iterator.hasNext)
    iterator.next()
    assert(!iterator.hasNext)
  }
  ...
  test("fetch continuous blocks in batch should respect maxBytesInFlight") {
    ...
    // be sent.
    verifyFetchBlocksInvocationCount(transfer, 5)
    ...
    verifyFetchBlocksInvocationCount(transfer, 6)
    assert(numResults == 6)
  }
  ...
  test("big blocks are also checked for corruption") {
    ...
    val blockLengths1 = Seq((shuffleBlockId1, corruptBuffer1.size(), 1))
    ...
    val blockLengths2 = Seq((shuffleBlockId2, corruptBuffer2.size(), 2))
  ...
  test("Blocks should be shuffled to disk when size of the request is above the" +
    ...
    val blocksByAddress1 = getBlocksByAddressForSingleBM(remoteBmId, remoteBlocks.keys, 100L, 0)
    ...
    val blocksByAddress2 = getBlocksByAddressForSingleBM(remoteBmId, remoteBlocks.keys, 300L, 0)
