...
class SparkConfSuite extends SparkFunSuite with LocalSparkContext with ResetSystemProperties {
  ...
  test("conf entry: int") {
    val conf = new SparkConf()
    val iConf = SparkConf.intConf("spark.int", defaultValue = Some(1))
  ...
  test("conf entry: long") {
    val conf = new SparkConf()
    val lConf = SparkConf.longConf("spark.long")
  ...
  test("conf entry: double") {
    val conf = new SparkConf()
    val dConf = SparkConf.doubleConf("spark.double")
    conf.set(dConf, 20.0)
  ...
  test("conf entry: boolean") {
    val conf = new SparkConf()
    val bConf = SparkConf.booleanConf("spark.boolean", defaultValue = Some(false))
  ...
  test("conf entry: required") {
    val conf = new SparkConf()
    val requiredConf = SparkConf.longConf("spark.noDefault")
  ...
  test("conf entry: optional") {
    val conf = new SparkConf()
    val optionalConf = SparkConf.intConf("spark.optional").optional
  ...
  test("conf entry: fallback") {
    val conf = new SparkConf()
    val parentConf = SparkConf.intConf("spark.int", defaultValue = Some(1))
    val confWithFallback = SparkConf.fallbackConf("spark.fallback", parentConf)
  ...
  test("conf entry: time") {
    val conf = new SparkConf()
    val time = SparkConf.timeConf("spark.time", TimeUnit.SECONDS, defaultValue = Some("1h"))
  ...
  test("conf entry: bytes") {
    val conf = new SparkConf()
    val bytes = SparkConf.bytesConf("spark.bytes", ByteUnit.KiB, defaultValue = Some("1m"))
  ...
  test("conf entry: string seq") {
    val conf = new SparkConf()
    val seq = SparkConf.stringSeqConf("spark.seq")
  ...
  test("conf entry: enum") {
    val conf = new SparkConf()
    val enum = SparkConf.enumConf("spark.enum", v => v, Set("a", "b", "c"),
      defaultValue = Some("a"))
  ...
  test("conf entry: conversion error") {
    val conf = new SparkConf()
    val conversionTest = SparkConf.doubleConf("spark.conversionTest")
