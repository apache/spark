...
private[memory] trait MemoryManagerSuite extends SparkFunSuite {
  ...
  test("single task requesting execution memory") {
    ...
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(100L) === 0L)
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(100L) === 0L)

    taskMemoryManager.releaseOnHeapExecutionMemory(500L)
    assert(taskMemoryManager.getMemoryConsumptionForThisTask === 500L)
    assert(manager.executionMemoryUsed === 500L)
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(300L) === 300L)
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(300L) === 200L)
    ...
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(1000L) === 1000L)
    assert(taskMemoryManager.acquireOnHeapExecutionMemory(100L) === 0L)
  }
  ...
  test("two tasks requesting full execution memory") {
    ...
    val t1Result1 = Future { t1MemManager.acquireOnHeapExecutionMemory(500L) }
    val t2Result1 = Future { t2MemManager.acquireOnHeapExecutionMemory(500L) }
    ...
    // both now at 1 / N
    val t1Result2 = Future { t1MemManager.acquireOnHeapExecutionMemory(500L) }
    val t2Result2 = Future { t2MemManager.acquireOnHeapExecutionMemory(500L) }
  ...
  test("two tasks cannot grow past 1 / N of execution memory") {
    ...
    val t1Result1 = Future { t1MemManager.acquireOnHeapExecutionMemory(250L) }
    val t2Result1 = Future { t2MemManager.acquireOnHeapExecutionMemory(250L) }
  ...
  test("tasks can block to get at least 1 / 2N of execution memory") {
    ...
    val t1Result1 = Future { t1MemManager.acquireOnHeapExecutionMemory(1000L) }
    ...
    Thread.sleep(300)
    t1MemManager.releaseOnHeapExecutionMemory(250L)
    ...
    val t2Result2 = Future { t2MemManager.acquireOnHeapExecutionMemory(100L) }
    assert(Await.result(t2Result2, 200.millis) === 0L)
  }
  ...
  test("TaskMemoryManager.cleanUpAllAllocatedMemory") {
    ...
    val t2Result3 = Future { t2MemManager.acquireOnHeapExecutionMemory(500L) }
  ...
  test("tasks should not be granted a negative amount of execution memory") {
    ...
    val t1Result1 = Future { t1MemManager.acquireOnHeapExecutionMemory(700L) }
    assert(Await.result(t1Result1, futureTimeout) === 700L)

    val t2Result1 = Future { t2MemManager.acquireOnHeapExecutionMemory(300L) }
    assert(Await.result(t2Result1, futureTimeout) === 300L)

    val t1Result2 = Future { t1MemManager.acquireOnHeapExecutionMemory(300L) }
    assert(Await.result(t1Result2, 200.millis) === 0L)
  }
}
