...
class CountEvaluatorSuite extends SparkFunSuite {
  ...
  test("test count >= 1") {
    val evaluator = new CountEvaluator(10, 0.95)
    evaluator.merge(1, 1)
    assert(new BoundedDouble(10.0, 0.95, 5.0, 16.0) == evaluator.currentResult())
    evaluator.merge(1, 3)
    assert(new BoundedDouble(20.0, 0.95, 13.0, 28.0) == evaluator.currentResult())
    evaluator.merge(1, 8)
    assert(new BoundedDouble(40.0, 0.95, 30.0, 51.0) == evaluator.currentResult())
