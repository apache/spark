...
class ExecutorAllocationManagerSuite extends SparkFunSuite {
  ...
  // ============================================================================================
  // SPARK-59561 (relates to SPARK-58935): tests for reconciling ExecutorAllocationManager's
  // bookkeeping against AppStatusStore's ground-truth stage state after a dropped
  // SparkListenerStageSubmitted event. Reconciled stages are never patched into
  // stageAttemptToTaskIndices/stageAttemptToNumRunningTask -- instead they are read fresh
  // from AppStatusStore for as long as they remain "recovered" (until stage completion).
  // ============================================================================================

  test("SPARK-59561: dropped StageSubmitted self-heals via AppStatusStore reconciliation") {
  ...
  test("SPARK-59561: reconcile() does not resurrect a stage already completed on our side") {
    val conf = createConf(0, 5, 0)
  ...
  // Covers the "double drop" scenario: the same executorManagement queue congestion that
  // dropped a stage's SparkListenerStageSubmitted could later also drop that same stage
  // attempt's SparkListenerStageCompleted. Without the terminal-stage sweep in reconcile(),
  // such an attempt would never be cleaned up: it would stay in recoveredStageAttempts and
  // keep querying AppStatusStore for it on every future need-calculation, for the remaining
  // lifetime of the application.
  test("SPARK-59561: reconcile() cleans up a recovered stage whose own StageCompleted is " +
      "also dropped") {
    val conf = createConf(0, 5, 0)
    val statusStore = AppStatusStore.createLiveStore(conf)
    ...
    val stageInfo = createStageInfo(0, 2)
    stageInfo.submissionTime = Some(System.currentTimeMillis())
    // StageSubmitted dropped on our side, delivered normally to AppStatusStore.
    statusStore.listener.get.onStageSubmitted(SparkListenerStageSubmitted(stageInfo))
    manager.listener.reconcile(statusStore)
    assert(manager.maxNumExecutorsNeededPerResourceProfile(defaultProfile.id) === 2,
      "sanity check: recovered as usual")

    // Now the stage finishes. StageCompleted is delivered normally to AppStatusStore (so its
    // ground truth correctly reflects completion)...
    statusStore.listener.get.onStageCompleted(SparkListenerStageCompleted(stageInfo))
    assert(!statusStore.activeStages().exists(_.stageId == 0),
      "test setup: AppStatusStore should no longer list this stage as active")
    // ...but manager.listener.onStageCompleted is deliberately never called here, simulating
    // the *same* StageCompleted event also being dropped from the executorManagement queue.

    // The computed value is still correct, because it's read fresh from AppStatusStore, whose
    // ground truth is accurate:
    assert(manager.maxNumExecutorsNeededPerResourceProfile(defaultProfile.id) === 0,
      "value should still be correct (0) even though our own StageCompleted never arrived, " +
        "because ground truth already reflects completion")

    // The bookkeeping entry is not cleaned up by this same reconcile() call (the sweep below
    ...
    // Now trigger a second reconcile() call (e.g. as if another, unrelated drop happened
    // later) and confirm the sweep actually cleans this attempt up this time.
    manager.listener.reconcile(statusStore)
    assert(recoveredSize() === 0,
      "expected reconcile()'s terminal-stage sweep to clean up an attempt whose own " +
        "StageCompleted was also dropped, once ground truth confirms it's done")
    assert(manager.maxNumExecutorsNeededPerResourceProfile(defaultProfile.id) === 0,
      "value should remain correct (0) after cleanup")
  }

  test("SPARK-59561: reconcile() attributes a recovered stage to its own resource profile, " +
      "not the default one") {
    ...
    val rprof1 = new ResourceProfileBuilder().require(execReqs).require(taskReqs).build()
    rpManagerLocal.addResourceProfile(rprof1)
    ...
    // A default-profile stage that the manager already knows about normally (not dropped)...
    val normalStage = createStageInfo(0, 3, rp = defaultProfile)
    normalStage.submissionTime = Some(System.currentTimeMillis())
    manager.listener.handleStageSubmitted(normalStage)

    // ...and a *different-resource-profile* stage that only AppStatusStore knows about
    // (simulating its SparkListenerStageSubmitted being dropped on the manager's side).
    val droppedStage = createStageInfo(1, 5, rp = rprof1)
    droppedStage.submissionTime = Some(System.currentTimeMillis())
    statusStore.listener.get.onStageSubmitted(SparkListenerStageSubmitted(droppedStage))
    // AppStatusStore must also know about the normal-path stage, or reconcile() would
    // (correctly) try to recover it too, confusing what this test is isolating.
    statusStore.listener.get.onStageSubmitted(SparkListenerStageSubmitted(normalStage))

    assert(manager.maxNumExecutorsNeededPerResourceProfile(defaultProfile.id) === 3,
      "the default-profile stage was delivered normally and should already be counted")
    assert(manager.maxNumExecutorsNeededPerResourceProfile(rprof1.id) === 0,
      "the non-default-profile stage was dropped and should not be counted yet")

    manager.listener.reconcile(statusStore)

    assert(manager.maxNumExecutorsNeededPerResourceProfile(defaultProfile.id) === 3,
      "recovering the other profile's stage must not affect the default profile's count")
    // rprof1's request shape (4 cores/4 gpu per executor, 1 cpu/1 gpu per task) gives
    // maxTasksPerExecutor = 4, so 5 pending tasks need ceil(5 / 4) = 2 executors -- this also
    // confirms the recovered stage's need-calculation goes through the *correct* resource
    // profile's tasksPerExecutor, not the default profile's (which would wrongly give 5).
    assert(manager.maxNumExecutorsNeededPerResourceProfile(rprof1.id) === 2,
      "expected the dropped stage to be recovered and attributed to its own " +
  ...
  test("SPARK-59561: the shrink (updateAndSyncNumExecutorsTarget) path also reflects a " +
    ...
    val manager = new ExecutorAllocationManager(client, listenerBus, conf,
      clock = clock, resourceProfileManager = rpManagerLocal, reliableShuffleStorage = false,
      statusStoreProvider = () => Some(statusStore))
    managers += manager
    manager.start()
    ...
    val stageInfo = createStageInfo(0, 2)
    stageInfo.submissionTime = Some(System.currentTimeMillis())
    // Dropped on our side, delivered normally to AppStatusStore.
    statusStore.listener.get.onStageSubmitted(SparkListenerStageSubmitted(stageInfo))
    manager.listener.reconcile(statusStore)

    // Ramp-up is incremental (starts at 1, doubles after sustainedSchedulerBacklogTimeout) --
    // advance well past both backlog timeouts and call this enough times to reach the full
    // 2 needed, same pattern as other tests here.
    (1 to 5).foreach { _ =>
      clock.advance(TimeUnit.SECONDS.toMillis(sustainedSchedulerBacklogTimeout + 1))
      manager invokePrivate _updateAndSyncNumExecutorsTarget(clock.nanoTime())
    }
    assert(numExecutorsTarget(manager, defaultProfile.id) === 2,
      "sanity check: the target grew to satisfy the recovered stage's 2 pending tasks, " +
        "going through the exact same shrink/grow decision path used for non-recovered stages")

    // The stage finishes; ground truth reflects it (this manager's own onStageCompleted is
    // never called, so the only reason this is picked up is that getPendingTaskSum/
    // totalRunningTasksPerResourceProfile read AppStatusStore fresh on every call).
    statusStore.listener.get.onStageCompleted(SparkListenerStageCompleted(stageInfo))

    clock.advance(1000)
    manager invokePrivate _updateAndSyncNumExecutorsTarget(clock.nanoTime())
    assert(numExecutorsTarget(manager, defaultProfile.id) === 0,
      "expected the shrink path to bring the target back down to the configured minimum " +
        "(0) once ground truth shows the recovered stage's tasks are all done -- this is " +
        "not just a read-only need-calculation check, but the actual target the cluster " +
        "manager would be told to scale down to")
  }

  test("SPARK-59561: reconcile() recovers multiple stages dropped in the same burst") {
    val conf = createConf(0, 20, 0)
      .set("spark.scheduler.listenerbus.eventqueue.executorManagement.capacity", "1")
    val customBus = new LiveListenerBus(conf)
    val statusStore = AppStatusStore.createLiveStore(conf)
  ...
  test("SPARK-59561: pre-existing TaskStart progress is correctly reflected via ground truth, " +
      "not double-counted") {
    val conf = createConf(0, 20, 0)
      .set("spark.scheduler.listenerbus.eventqueue.executorManagement.capacity", "1")
    val customBus = new LiveListenerBus(conf)
  ...
  test("SPARK-59561: a task that fails and needs retry after reconciliation is correctly " +
  ...
  test("SPARK-59561: a real StageSubmitted arriving after reconciliation already recovered the " +
  ...
  test("SPARK-59561: a failed reconcile attempt is retried on the next schedule() tick, even " +
      "without a new drop") {
    val conf = createConf(0, 5, 0)
      .set("spark.scheduler.listenerbus.eventqueue.executorManagement.capacity", "1")
  ...
  test("SPARK-59561: a single maxNumExecutorsNeededPerResourceProfile() call queries " +
