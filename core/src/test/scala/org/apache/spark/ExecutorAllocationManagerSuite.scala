...
class ExecutorAllocationManagerSuite
  ...
  def testAllocationRatio(cores: Int, divisor: Double, expected: Int): Unit = {
    ...
    val stages = Seq(createStageInfo(0, 20))
    // Submit the job and stage start/submit events
    manager.listener.onJobStart(SparkListenerJobStart(0, 0, stages, null))
  ...
  test("ignore task end events from completed stages") {
    sc = createSparkContext(0, 10, 0)
    val manager = sc.executorAllocationManager.get
    val stages = Seq(createStageInfo(0, 5))
    post(sc.listenerBus, SparkListenerJobStart(0, 0, stages, sc.getLocalProperties))
    post(sc.listenerBus, SparkListenerStageSubmitted(stages(0)))
  ...
  test("SPARK-23365 Don't update target num executors when killing idle executors") {
    ...
    manager.listener.onStageSubmitted(SparkListenerStageSubmitted(stages(0)))
    clock.advance(1000)
