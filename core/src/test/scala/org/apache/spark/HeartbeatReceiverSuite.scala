...
class HeartbeatReceiverSuite extends FunSuite with LocalSparkContext {
  ...
  test("HeartbeatReceiver") {
    ...
    sc.env.rpcEnv.setupEndpoint("heartbeat", new HeartbeatReceiver(sc.env.rpcEnv, scheduler))
    val receiverRef = sc.env.rpcEnv.setupDriverEndpointRef("heartbeat")

    val metrics = new TaskMetrics
    metrics.jvmGCTime = 100
