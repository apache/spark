...
class ExternalOrderingAppendOnlyMapSuite extends FunSuite with LocalSparkContext {
  ...
  private def createExternalMap[T: Ordering] = new ExternalOrderingAppendOnlyMap[T, T, ArrayBuffer[T]](
    createCombiner[T], mergeValue[T], () => ArrayBuffer[T](), mergeCombiners[T])
