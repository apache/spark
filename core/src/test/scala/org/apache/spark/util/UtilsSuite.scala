...
class UtilsSuite extends SparkFunSuite with ResetSystemProperties {
  ...
  test("shutdown hook manager") {
    ...
    val hook1 = manager.add(1, "TestShutdownHook", () => output += 1)
    manager.add(3, "TestShutdownHook", () => output += 3)
    manager.add(2, "TestShutdownHook", () => output += 2)
    manager.add(4, "TestShutdownHook", () => output += 4)
    manager.add(Int.MinValue, "TestShutdownHook", () => output += Int.MinValue)
    manager.add(Int.MinValue, "TestShutdownHook", () => output += Int.MinValue)
    manager.add(Int.MaxValue, "TestShutdownHook", () => output += Int.MaxValue)
    manager.add(Int.MaxValue, "TestShutdownHook", () => output += Int.MaxValue)
