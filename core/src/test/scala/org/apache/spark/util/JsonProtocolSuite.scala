...
class JsonProtocolSuite extends SparkFunSuite {
  ...
  private def assertEquals(reason1: TaskEndReason, reason2: TaskEndReason) {
    (reason1, reason2) match {
      ...
      case (ExecutorLostFailure(execId1, exit1UnrelatedToRunningTasks),
          ExecutorLostFailure(execId2, exit2UnrelatedToRunningTasks)) =>
        assert(execId1 === execId2)
        assert(exit1UnrelatedToRunningTasks === exit2UnrelatedToRunningTasks)
