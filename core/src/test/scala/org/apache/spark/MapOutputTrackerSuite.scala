...
class MapOutputTrackerSuite extends SparkFunSuite {
  ...
  test("zero-sized blocks should be excluded when getMapSizesByExecutorId") {
    ...
    assert(tracker.containsShuffle(10))
    assert(tracker.getMapSizesByExecutorId(10, 0, 4, "c", true).toSeq ===
      Seq(
  ...
  test("fetch contiguous partitions") {
    ...
    val statuses1 = tracker.getMapSizesByExecutorId(10, 0, 2, "a", true)
    assert(statuses1.toSet ===
      Seq((BlockManagerId("a", "hostA", 1000),
        ...
        .toSet)
    val statuses2 = tracker.getMapSizesByExecutorId(10, 2, 3, "a", true)
