...
class SparkThrowableSuite extends SparkFunSuite {
  ...
  test("Check if error class is missing") {
    val ex1 = intercept[IllegalArgumentException] {
      getMessage("", None, Array.empty)
    }
    assert(ex1.getMessage == "Cannot find error class ''")

    val ex2 = intercept[IllegalArgumentException] {
      getMessage("LOREM_IPSUM", None, Array.empty)
    }
  ...
  test("Check if message parameters match message format") {
    ...
    intercept[IllegalFormatException] {
      getMessage("MISSING_COLUMN", None, Array.empty)
    }

    // Does not fail with too many args (expects 0 args)
    assert(getMessage("DIVIDE_BY_ZERO", None, Array("foo", "bar", "baz")) ==
  ...
  test("Error message is formatted") {
    assert(getMessage("MISSING_COLUMN", None, Array("foo", "bar, baz")) ==
