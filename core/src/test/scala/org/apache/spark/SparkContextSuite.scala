...
class SparkContextSuite extends SparkFunSuite with LocalSparkContext with Eventually {
  ...
  test("SPARK-28106: check jar exists when case of malformed url") {
    try {
      sc = new SparkContext(new SparkConf().setAppName("test").setMaster("local"))
      Seq("http", "https", "ftp").foreach { scheme =>
        val badURL = s"$scheme://user:pwd/path"
        val e1 = intercept[MalformedURLException] {
          sc.addJar(badURL, true)
        }
        assert(e1.getMessage.contains(badURL))
        assert(sc.addedFiles.isEmpty)
        assert(sc.addedJars.isEmpty)
  ...
  test("SPARK-28106: check jar exists when case of file schema ") {
    val jarPath = Thread.currentThread().getContextClassLoader.getResource("TestUDTF.jar")
    try {
      sc = new SparkContext(new SparkConf().setAppName("test").setMaster("local"))
      sc.addJar(jarPath.toString, true)
      assert(sc.listJars().filter(_.contains("TestUDTF.jar")).size == 1)
      sc.addJar(jarPath.toString, true)
      assert(sc.listJars().filter(_.contains("TestUDTF.jar")).size == 1)
    } finally {
  ...
  test("SPARK-28106 addJar can be called twice with same file ") {
    for (
      ...
      sc = new SparkContext(new SparkConf().setAppName("test").setMaster(master))
      method match {
        case "addJar" =>
          sc.addJar(jarPath)
          sc.addJar(jarPath)
  ...
  test("SPARK-28106 With check jar path, add jar with invalid path") {
    withTempDir { tmpDir =>
      ...
      sc.addJar("dummy.jar", true)
      sc.addJar("", true)
      sc.addJar(tmpDir.getAbsolutePath, true)
  ...
  test("SPARK-28106 with check jar exists," +
    ...
    withTempDir { dir =>
      ...
      sc.addJar(tmpJar.getAbsolutePath, true)
      assert(sc.listJars().size === 1)
    }
  }
