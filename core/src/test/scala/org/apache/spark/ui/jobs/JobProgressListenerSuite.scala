...
class JobProgressListenerSuite extends FunSuite with LocalSparkContext with Matchers {
  ...
  test("test update metrics") {
    ...
    def makeTaskMetrics(base: Int) = {
      ...
      val shuffleReadSpillMetrics = taskMetrics.getOrCreateShuffleReadSpillMetrics()
      val shuffleWriteSpillMetrics = taskMetrics.getOrCreateShuffleWriteSpillMetrics()
      shuffleReadSpillMetrics.incShuffleBytesWritten(base + 5)
      shuffleReadSpillMetrics.setMemorySize(base + 6)
      shuffleWriteSpillMetrics.incShuffleBytesWritten(base + 5)
      shuffleWriteSpillMetrics.setMemorySize(base + 6)
    ...
    assert(stage0Data.shuffleReadDiskSpillBytes == 110)
    assert(stage0Data.shuffleWriteDiskSpillBytes == 110)
    assert(stage1Data.shuffleReadDiskSpillBytes == 205)
    assert(stage1Data.shuffleWriteDiskSpillBytes == 205)
    assert(stage0Data.shuffleReadMemorySpillBytes == 112)
    assert(stage0Data.shuffleWriteMemorySpillBytes == 112)
    assert(stage1Data.shuffleReadMemorySpillBytes == 206)
    assert(stage1Data.shuffleWriteMemorySpillBytes == 206)
    ...
    assert(stage0Data.shuffleReadDiskSpillBytes == 410)
    assert(stage0Data.shuffleWriteDiskSpillBytes == 410)
    assert(stage1Data.shuffleReadDiskSpillBytes == 610)
    assert(stage1Data.shuffleWriteDiskSpillBytes == 610)
    assert(stage0Data.shuffleReadMemorySpillBytes == 412)
    assert(stage0Data.shuffleWriteMemorySpillBytes == 412)
    assert(stage1Data.shuffleReadMemorySpillBytes == 612)
    assert(stage1Data.shuffleWriteMemorySpillBytes == 612)
