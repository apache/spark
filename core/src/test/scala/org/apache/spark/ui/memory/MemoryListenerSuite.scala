...
class MemoryListenerSuite extends SparkFunSuite with LocalSparkContext with Matchers {
  ...
  test("test first stage with no executor metrics update") {
    ...
    listener.onStageSubmitted(createStageStartEvent(1))
    listener.onStageCompleted(createStageEndEvent(1))

    // stage 2, with one metrics update
    listener.onStageSubmitted(createStageStartEvent(2))
    val execMetrics = createExecutorMetrics("host-1", 0L, 20, 10)
    listener.onExecutorMetricsUpdate(createExecutorMetricsUpdateEvent(
      execId1, execMetrics))
    listener.onStageCompleted(createStageEndEvent(2))
  ...
  test("test multiple executors metrics updated in one stage") {
    ...
    listener.onStageSubmitted(createStageStartEvent(1))
    val execMetrics1 = createExecutorMetrics("host-1", 0L, 20, 10)
    listener.onExecutorMetricsUpdate(createExecutorMetricsUpdateEvent(
      execId1, execMetrics1))
    val execMetrics2 = createExecutorMetrics("host-1", 0L, 30, 5)
    listener.onExecutorMetricsUpdate(createExecutorMetricsUpdateEvent(
      execId1, execMetrics2))
    val execMetrics3 = createExecutorMetrics("host-1", 0L, 15, 15)
    listener.onExecutorMetricsUpdate(createExecutorMetricsUpdateEvent(
  ...
  test("test multiple executors") {
    ...
    // two executors running in one stage and one executor is removed before stage complete
    listener.onStageSubmitted(createStageStartEvent(1))
    val exec1Metrics = createExecutorMetrics("host-1", 1446336000L, 20, 10)
    listener.onExecutorMetricsUpdate(createExecutorMetricsUpdateEvent(
