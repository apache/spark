...
class StorageTabSuite extends SparkFunSuite with LocalSparkContext with BeforeAndAfter {
  ...
  test("stage submitted / completed") {
    ...
    bus.postToAllSync(SparkListenerStageSubmitted(stageInfo1))
    ...
    bus.postToAllSync(SparkListenerStageSubmitted(stageInfo0Cached))
    assert(storageListener._rddInfoMap.size === 4)
    assert(storageListener.rddInfoList.size === 2)

    // We only keep around the RDDs that are cached
    bus.postToAllSync(SparkListenerStageCompleted(stageInfo0))
    assert(storageListener._rddInfoMap.size === 2)
    assert(storageListener.rddInfoList.size === 2)
  }
  ...
  test("unpersist") {
    ...
    assert(storageListener._rddInfoMap.size === 2)
    assert(storageListener.rddInfoList.size === 2)
    bus.postToAllSync(SparkListenerUnpersistRDD(0))
    assert(storageListener._rddInfoMap.size === 1)
    assert(storageListener.rddInfoList.size === 1)
    bus.postToAllSync(SparkListenerUnpersistRDD(4)) // doesn't exist
    assert(storageListener._rddInfoMap.size === 1)
    assert(storageListener.rddInfoList.size === 1)
    bus.postToAllSync(SparkListenerUnpersistRDD(1))
  ...
  test("block update") {
    ...
    bus.postToAllSync(SparkListenerBlockManagerAdded(1L, bm1, 1000L))
    bus.postToAllSync(SparkListenerStageSubmitted(stageInfo0))
  ...
  test("verify StorageTab contains all cached rdds") {
    ...
    postUpdateBlocks(bus, blockUpdateInfos2)
    assert(storageListener.rddInfoList.size === 2)
    bus.postToAllSync(SparkListenerStageCompleted(stageInfo1))
    assert(storageListener.rddInfoList.size === 2)
  }
  ...
  private def postUpdateBlocks(
    ...
    blockUpdateInfos.foreach { blockUpdateInfo =>
      bus.postToAllSync(SparkListenerBlockUpdated(blockUpdateInfo))
    }
  }
}
