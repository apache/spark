...
class ClassLoaderIsolationSuite extends SparkFunSuite with LocalSparkContext  {
  ...
  // Dynamically generate JARs that mimic Scala's `object Hello { def test(): Int = N }`
  // The test code accesses Hello$ class via MODULE$ field and test() method.
    ...
    val source = Map(
      "com.example.Hello$" ->
        s"""package com.example;
           ...
           |  static { MODULE$$ = new Hello$$(); }
           |  private Hello$$() {}
           |  public int test() { return $returnValue; }
           |}""".stripMargin)
    // scalastyle:on line.size.limit
    val jarFile = File.createTempFile(s"TestHelloV$returnValue", ".jar")
    jarFile.deleteOnExit()
    TestUtils.createJarWithJavaSources(source, jarFile)
  }
