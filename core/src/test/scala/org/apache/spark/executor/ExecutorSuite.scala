...
class ExecutorSuite extends SparkFunSuite
  ...
  test("SPARK-33587: isFatalError") {
    ...
    def testThrowable(
      ...
      def beEqualToIsFatal(isFatal: Boolean) = new BeEqualToIsFatal(isFatal)

      // `e`'s depth is 1 so `depthToCheck` needs to be at least 3 to detect fatal errors.
      e should beEqualToIsFatal(depthToCheck >= 1 && isFatal)
      // `e`'s depth is 2 so `depthToCheck` needs to be at least 3 to detect fatal errors.
      errorInThreadPool(e) should beEqualToIsFatal(depthToCheck >= 2 && isFatal)
      errorInGuavaCache(e) should beEqualToIsFatal(depthToCheck >= 2 && isFatal)
      new SparkException("foo", e) should beEqualToIsFatal(depthToCheck >= 2 && isFatal)
      ...
      errorInGuavaCache(errorInThreadPool(e)) should beEqualToIsFatal(depthToCheck >= 3 && isFatal)
      new SparkException("foo", new SparkException("foo", e)) should beEqualToIsFatal(
        depthToCheck >= 3 && isFatal)
    }
