...
class NettyMultiSecretMasterNetSuite extends AbstractNettyMasterNet {

  masterConf.set("spark.authenticate", "true")
  masterConf.set("spark.authenticate.secret", "1q2w3e")
  masterConf.set("spark.authenticate.secrets.testUser1", "abc123")
  masterConf.set("spark.authenticate.secrets.testUser2", "def123")
  ...
  test("registering app with user secret") {
    val clientEnv = makeEnv(user1SecretConf)
    val masterRef = connectToMaster(clientEnv)
    testAppRegistration(clientEnv, masterRef)
  }

  test("registering app with user 2 secret") {
    shouldFailBecauseOfAuthz {
      val clientEnv = makeEnv(user1SecretConf)
      val masterRef = connectToMaster(clientEnv)
      testAppRegistrationWithAuthz(clientEnv, masterRef)
    }
  }

  test("registering app with user secret - master failure scenario") {
    val clientEnv = makeEnv(user1SecretConf)
    val masterRef = connectToMaster(clientEnv)
    testAppRegistrationWithMasterFailure(clientEnv, masterRef)
  }

  test("sending scheduling msg with user secret") {
    shouldFailBecauseOfAuthz {
      val clientEnv = makeEnv(user1SecretConf)
      val masterRef = connectToMaster(clientEnv)
      testSchedulingMsgAsk(masterRef)
  ...
  test("sending submission msg with user secret") {
    val clientEnv = makeEnv(user1SecretConf)
    val masterRef = connectToMaster(clientEnv)
    testSubmissionMsgAsk(masterRef)
  }
