...
class JsonProtocolSuite extends SparkFunSuite with JsonTestUtils {
  ...
  test("writeExecutorRunner") {
    val output = JsonProtocol.writeExecutorRunner(createExecutorRunner(1))
    assertValidJson(output)
  ...
  test("writeWorkerState") {
    ...
    val finishedExecutors = List[ExecutorRunner](createExecutorRunner(1), createExecutorRunner(2))
