...
class CommandUtilsSuite extends FunSuite with Matchers {
  ...
  test("set java.library.path correctly") {
    ...
    val envs = Map(Utils.libraryPath -> "libraryPathToA")
    val cmd = CommandUtils.buildCommandSeq(new Command("mainClass", Seq(), envs, Seq(),
      Seq("libraryPathToB"), Seq()), 512, sparkHome)
    cmd.filter(_.contains("libraryPathToA")).size shouldBe 1
  }
}
