...
import org.apache.spark.deploy.DeployMessages.{DecommissionWorkersOnHosts, KillDriverResponse, MasterStateResponse, RequestKillDriver, RequestMasterState}
import org.apache.spark.deploy.DeployTestUtils._
import org.apache.spark.deploy.master._
import org.apache.spark.internal.config.DECOMMISSION_ENABLED
import org.apache.spark.internal.config.UI.{MASTER_UI_DECOMMISSION_ALLOW_MODE,
  UI_REVERSE_PROXY, UI_REVERSE_PROXY_URL}
...
class MasterWebUISuite extends SparkFunSuite {
  ...
  test("state-changing master UI endpoints reject a request without the CSRF token") {
    ...
    Seq("app/kill", "driver/kill").foreach { path =>
  ...
  test("the master page embeds the CSRF token in the form it renders") {
    ...
    assert(rendered.contains("app/kill/"))
  ...
  test("SPARK-58893: kill application redirect location with reverse proxy") {
    ...
    try {
      ...
      val body = convPostDataToString(Map(
        ("id", activeApp.id), ("terminate", "true"), ("csrfToken", webUI.csrfToken)))
