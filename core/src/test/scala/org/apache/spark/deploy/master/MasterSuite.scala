...
class MasterSuite extends SparkFunSuite
  ...
  test("master/worker web ui available") {
    ...
    try {
      eventually(timeout(5.seconds), interval(100.milliseconds)) {
        val json = Utils
          .tryWithResource(Source.fromURL(s"$masterUrl/json"))(_.mkString)
        val JArray(workers) = (parse(json) \ "workers")
        workers.size should be (2)
        workers.foreach { workerSummaryJson =>
          val JString(workerWebUi) = workerSummaryJson \ "webuiaddress"
          val workerResponse = parse(Utils
            .tryWithResource(Source.fromURL(s"$workerWebUi/json"))(_.mkString))
          (workerResponse \ "cores").extract[Int] should be (2)
        }

        val html = Utils
          .tryWithResource(Source.fromURL(s"$masterUrl/"))(_.mkString)
        ...
        workerLinks foreach { case WORKER_LINK_RE(workerUrl, workerId) =>
          val workerHtml = Utils
            .tryWithResource(Source.fromURL(workerUrl))(_.mkString)
      ...
      eventually(timeout(5.seconds), interval(100.milliseconds)) {
        ...
        workers.foreach { workerSummaryJson =>
          ...
          val workerResponse = parse(
            Utils.tryWithResource(Source.fromURL(url))(_.mkString))
          (workerResponse \ "cores").extract[Int] should be (2)
        }
    ...
    try {
      eventually(timeout(5.seconds), interval(100.milliseconds)) {
        ...
        workers.foreach { workerSummaryJson =>
          ...
          val url = s"$masterUrl/proxy/${workerId}/json"
          val workerResponse = parse(Utils
            .tryWithResource(Source.fromURL(url))(_.mkString))
          (workerResponse \ "cores").extract[Int] should be (2)
