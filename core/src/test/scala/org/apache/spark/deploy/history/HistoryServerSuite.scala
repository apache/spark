 ...
 * cases should be added to this test suite.  The expected outcomes can be generated by enabling
 * HistoryServerSuiteRegenerate
 * and running:
 * core/testOnly org.apache.spark.deploy.history.HistoryServerSuiteRegenerate
 *
...
abstract class HistoryServerSuite extends SparkFunSuite with BeforeAndAfter with Matchers
  ...
  val logDir = getTestResourcePath("spark-events")
  val expRoot = getTestResourceFile("HistoryServerExpectations")
  val storeDir = Utils.createTempDir(namePrefix = "history")
...
/*
class HistoryServerSuiteRegenerate extends org.scalatest.funsuite.AnyFunSuite {
  test("regenerate") {
    ...
    class GenerateSuite extends LevelDBBackendHistoryServerSuite  {
      override val expRoot = Utils.createTempDir(namePrefix = "expRoot")
      override val storeDir = Utils.createTempDir(namePrefix = "storeDir")
    }
    val suite = new GenerateSuite
    try {
      suite.init()
