...
private[spark] object JoinType {
  ...
  def inner[K: ClassTag, L: ClassTag, R: ClassTag] = new JoinType[K, L, R, (L, R)] {
  ...
  def leftOuter[K: ClassTag, L: ClassTag, R: ClassTag] = new JoinType[K, L, R, (L, Option[R])] {
  ...
  def rightOuter[K: ClassTag, L: ClassTag, R: ClassTag] = new JoinType[K, L, R, (Option[L], R)] {
...
private[spark] class SortMergeJoinRDD[K, L, R, PAIR <: Product2[_, _]](
  ...
  private def internalCompute[V, W](leftIter: Iterator[(K, V)], rightIter: Iterator[(K, W)])
    ...
    new Iterator[(K, (Iterator[V], Iterator[W]))] {
      ...
      var leftNext: (K, V) = null.asInstanceOf[(K, V)]
      var rightNext: (K, W) = null.asInstanceOf[(K, W)]
      ...
      def next(): (K, (Iterator[V], Iterator[W])) = {
        val currentKey: K = leftNext._1
        val leftResultIter = new Iterator[V] {
        ...
        val rightResultIter = new Iterator[W] {
          ...
          def hasNext: Boolean = {
            ...
            if (flag) {
              ...
              while (comp > 0 && flag) {
                if (rightIter.hasNext) {
                  rightNext = rightIter.next()
                  comp = keyOrdering.compare(currentKey, rightNext._1)
                } else {
                  flag = false
                }
              }
        ...
        (currentKey, (leftResultIter, rightResultIter))
      }
    }
  }
