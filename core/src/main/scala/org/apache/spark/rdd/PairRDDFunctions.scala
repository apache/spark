...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  def combineByKeyWithClassTag[C](
    ...
    if (keyClass.isArray) {
      if (mapSideCombine) {
        throw ExecutionErrors.cannotUseMapSideCombiningWithArrayKeyError()
      }
      if (partitioner.isInstanceOf[HashPartitioner]) {
        throw ExecutionErrors.hashPartitionerCannotPartitionArrayKeyError()
      }
    }
  ...
  def reduceByKeyLocally(func: (V, V) => V): Map[K, V] = self.withScope {
    ...
    if (keyClass.isArray) {
      throw ExecutionErrors.reduceByKeyLocallyNotSupportArrayKeysError()
    }
