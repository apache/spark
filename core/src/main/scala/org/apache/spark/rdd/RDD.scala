...
abstract class RDD[T: ClassTag](
  ...
  private[spark] def getNarrowAncestors: Seq[RDD[_]] = {
    ...
    def visit(rdd: RDD[_]): Unit = {
      val narrowDependencies: Seq[Dependency[_]] = rdd.dependencies.filter(_.isInstanceOf[NarrowDependency[_]])
      val narrowParents: Seq[RDD[_]] = narrowDependencies.map(_.rdd)
      val narrowParentsNotVisited: Seq[RDD[_]] = narrowParents.filterNot(ancestors.contains)
  ...
  def flatMap[U: ClassTag](f: T => TraversableOnce[U]): RDD[U] = withScope {
    val cleanF: T => TraversableOnce[U] = sc.clean(f)
  ...
  def filter(f: T => Boolean): RDD[T] = withScope {
    val cleanF: T => Boolean = sc.clean(f)
  ...
  def distinct(numPartitions: Int)(implicit ord: Ordering[T] = null): RDD[T] = withScope {
    def removeDuplicatesInPartition(partition: Iterator[T]): Iterator[T] = {
      ...
      val map: ExternalAppendOnlyMap[T, Null, Null] = new ExternalAppendOnlyMap[T, Null, Null](
  ...
  private[spark] def randomSampleWithRange(lb: Double, ub: Double, seed: Long): RDD[T] = {
    this.mapPartitionsWithIndex({ (index, partition) =>
      val sampler: BernoulliCellSampler[T] = new BernoulliCellSampler[T](lb, ub)
    ...
    if (num == 0) {
      ...
      if (initialCount == 0) {
        ...
        if (!withReplacement && num >= initialCount) {
          ...
          val fraction: Double = SamplingUtils.computeFractionForSampleSize(num, initialCount,
            withReplacement)
  ...
  def intersection(other: RDD[T]): RDD[T] = withScope {
    this.map(v => (v, null)).cogroup(other.map(v => (v, Nothing)))
  ...
  def foreachPartition(f: Iterator[T] => Unit): Unit = withScope {
    val cleanF: Iterator[T] => Unit = sc.clean(f)
  ...
  def collect[U: ClassTag](f: PartialFunction[T, U]): RDD[U] = withScope {
    val cleanF: PartialFunction[T, U] = sc.clean(f)
