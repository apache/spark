...
class SparkContext(config: SparkConf) extends Logging {
  ...
  logInfo(
    s"OS info ${System.getProperty("os.name")}, ${System.getProperty("os.version")}, " +
...
object SparkContext extends Logging {
  ...
  /**
   * Executor id for the driver. In earlier versions of Spark, this was `&lt;driver&gt;`, but this
   * was changed to `driver` because the angle brackets caused escaping issues in URLs and XML
   * (see SPARK-6716 for more details).
   */
  ...
  private implicit def arrayToArrayWritable[T <: Writable: ClassTag](
      arr: Iterable[T]): ArrayWritable = {
    def anyToWritable[U <: Writable](u: U): Writable = u

    new ArrayWritable(
      classTag[T].runtimeClass.asInstanceOf[Class[Writable]],
