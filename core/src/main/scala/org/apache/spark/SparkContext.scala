...
class SparkContext(config: SparkConf) extends Logging {
  ...
  private[spark] def broadcastInternal[T: ClassTag](
    ...
    val bc = env.broadcastManager.newBroadcast(value, isLocal, serializedOnly, executionId)
  ...
  private[spark] def cleanBroadcastByExecution(executionId: String): Unit = {
    env.broadcastManager.unbroadcastByExecution(executionId, true, false)
  }
