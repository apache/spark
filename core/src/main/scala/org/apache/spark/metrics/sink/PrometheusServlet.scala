...
private[spark] class PrometheusServlet(
  ...
  def getMetricsSnapshot(): String = {
    ...
    registry.getHistograms.asScala.foreach { case (k, h) =>
      ...
      sb.append(s"${prefix}{quantile=\"0.5\"} ${snapshot.getMedian}\n")
      sb.append(s"${prefix}{quantile=\"0.75\"} ${snapshot.get75thPercentile}\n")
      sb.append(s"${prefix}{quantile=\"0.95\"} ${snapshot.get95thPercentile}\n")
      sb.append(s"${prefix}{quantile=\"0.98\"} ${snapshot.get98thPercentile}\n")
      sb.append(s"${prefix}{quantile=\"0.99\"} ${snapshot.get99thPercentile}\n")
      sb.append(s"${prefix}{quantile=\"0.999\"} ${snapshot.get999thPercentile}\n")
