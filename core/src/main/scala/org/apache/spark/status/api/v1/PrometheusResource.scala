...
private[v1] class PrometheusResource extends ApiRequestContext {
  ...
  def activeExecutorsAndStages(): String = {
    ...
    store.activeStages().foreach { stage =>
      val prefix = f"metrics_${appId}_${stage.attemptId}_executor_"
      ...
      sb.append(s"${prefix}runTime_Count ${stage.executorRunTime}\n")
      sb.append(s"${prefix}cpuTime_Count ${stage.executorCpuTime}\n")
