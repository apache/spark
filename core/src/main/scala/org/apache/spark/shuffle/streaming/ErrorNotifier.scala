 ...
 *
 * Background threads (Netty event loops, task discovery, client creation) record errors here
 * via [[markError]] rather than calling `TaskContext.markTaskFailed` directly. The owning task
 * thread polls [[throwErrorIfExists]] at safe points and re-throws on its own thread. This avoids
 * a race where invoking `markTaskFailed` from a background thread can cause the task thread's
 * subsequent `markTaskCompleted` to silently skip completion listeners (including cleanup),
