...
private[spark] abstract class ExternalBlockManager {
  ...
  protected var serializerManager : SerializerManager = _
  ...
  def putValues(blockId: BlockId, values: Iterator[_]): Unit = {
    val bytes = serializerManager.dataSerialize(blockId, values)
    putBytes(blockId, bytes.toByteBuffer)
  }

  def putFile(blockId: BlockId, file : File): Unit = {}
  ...
  def getValues(blockId: BlockId): Option[Iterator[_]] = {
    ...
    getBytes(blockId).map(buffer => serializerManager.dataDeserializeStream(blockId, new ByteBufferInputStream(buffer))(ct))
  }

  def createInputStream(path: Path): Option[InputStream]

  def getSize(path: Path): Long = 0L
  ...
  def getFile(blockId: BlockId): Path
}
