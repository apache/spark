...
class BlockManagerMasterEndpoint(
 ...
 private def updateShuffleBlockInfo(blockId: BlockId, blockManagerId: BlockManagerId)
  ...
  private def getLocationsAndStatus(
    ...
    val fromExternalShuffleService: Option[(BlockStatus, BlockManagerId)] =
      if (externalShuffleServiceRddFetchEnabled) {
        ...
        location.flatMap(blockStatusByShuffleService.get(_).flatMap(_.get(blockId))).zip(location)
      } else {
        None
      }

    val blockStatusWithBlockManagerId = fromExternalShuffleService.orElse {
        ...
        hostLocalLocations.iterator.flatMap { bmId =>
        ...
        val res = location.flatMap(blockManagerInfo.get(_)).flatMap(_.getStatus(blockId)).zip(location)
        res
      }
      logDebug(s"Identified block: $blockStatusWithBlockManagerId")
      blockStatusWithBlockManagerId.map { case (blockStatus: BlockStatus, bmId: BlockManagerId) =>
        if (bmId.host == requesterHost && blockStatus.storageLevel.useDisk) {
          BlockLocationsAndStatus(allLocations, blockStatus,
