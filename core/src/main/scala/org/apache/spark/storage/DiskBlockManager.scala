...
import org.apache.spark.util.{ShutdownHookManager, SparkFileUtils, Utils}
  ...
  def getFile(filename: String): File = {
    ...
    val subDir = subDirs(dirId).synchronized {
      ...
      if (old != null && old.exists()) {
        ...
        // or replacement. This dir could be accessed subsequently by others. Especially,
        // `filename` generated by `RDDBlockId` is unchanged between task reties, so it probably
        // attempts to access the same subDir repeatedly.
        // Therefore, it is necessary to check if the subDir exists. If it is stale and the hardware
        // has been recovered without data and directories, we will recreate the subDir to prevent
        // FileNotFoundException during writing.
        old
      } else {
        val newDir = new File(localDirs(dirId), "%02x".format(subDirId))
        if (!newDir.exists()) {
          // If the directory fails to be created, we do nothing more but simply log error here.
          // The reason is that the final file will be checked when we write to it.
          val created = SparkFileUtils.createDirectory(newDir)
          if (created && permissionChangingRequired) {
