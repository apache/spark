...
private[spark] class ExternalSorterAgg[K, V, C](
  ...
  if (SortShuffleWriter.shouldBypassMergeSort(conf, numPartitions, Some(aggregator), ordering)) {
  ...
  : Iterator[(Int, Iterator[Product2[K, C]])] = {
    ...
    (0 until numPartitions).iterator.map { p =>
      ...
      (p, mergeWithAggregation(
        iterators, aggregator.mergeCombiners, keyComparator, ordering.isDefined))
    }
  }
  ...
  def partitionedIterator: Iterator[(Int, Iterator[Product2[K, C]])] = {
    val collection: WritablePartitionedPairCollection[K, C] = map
    if (spills.isEmpty) {
