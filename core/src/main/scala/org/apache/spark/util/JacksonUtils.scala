...
object JacksonUtils {
  ...
  def treeToValue[T: JavaTypeable](node: JsonNode): T = mapper.treeToValue[T](node)

  def valueToTree(fromValue: Any): JsonNode = mapper.valueToTree(fromValue)

  def readTree(content: String): JsonNode = mapper.readTree(content)

  def readTree(reader: Reader): JsonNode = mapper.readTree(reader)
  ...
  def defaultNodeFactory: JsonNodeFactory = JsonNodeFactory.instance

  def createObjectNode: ObjectNode = defaultNodeFactory.objectNode()

  def createArrayNode(capacity: Int): ArrayNode =
    defaultNodeFactory.arrayNode(capacity)
}
