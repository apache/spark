...
class MapperRowCounter extends AccumulatorV2[jl.Long, java.util.List[java.util.List[jl.Long]]] {
  ...
  private def getOrCreate = {
    _agg = Option(_agg).getOrElse(new java.util.ArrayList[java.util.List[jl.Long]]())
    _agg
  }
  ...
  override def add(v: jl.Long): Unit = {
    this.synchronized {
      ...
      val p = getOrCreate.get(0).get(0)
      val n = getOrCreate.get(0).get(1) + 1
      val t = new java.util.ArrayList[jl.Long]()
      t.add(p)
      t.add(n)
      getOrCreate.set(0, t)
    }
  ...
  override def merge(
    other: AccumulatorV2[jl.Long, java.util.List[java.util.List[jl.Long]]]): Unit
  = other match {
  ...
  override def value: java.util.List[java.util.List[jl.Long]] = this.synchronized(getOrCreate)
}
