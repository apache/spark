...
private[spark] object ShutdownHookManager extends Logging {
  ...
  // Remove given path from shutdown hook registered list to deleted
  def removeShutdownDeleteDir(file: File): Boolean = {
    val absolutePath = file.getAbsolutePath()
    shutdownDeletePaths.synchronized {
  ...
  // Remove given path from shutdown hook registered list to deleted
  def removeShutdownDeleteTachyonDir(file: TachyonFile): Boolean = {
    val absolutePath = file.getPath()
    shutdownDeleteTachyonPaths.synchronized {
