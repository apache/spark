...
object ObjectWalker {
   ...
   * @return a new java.util.Set containing the 2-tuple of references from the BFS traversal of the 
   *         reference graph along with their parent references. (self, parent)
   */
  def buildRefGraph(rootObj: AnyRef): java.util.Set[(AnyRef, AnyRef)] = {
    val visited = Sets.newIdentityHashSet[(AnyRef, AnyRef)]()
