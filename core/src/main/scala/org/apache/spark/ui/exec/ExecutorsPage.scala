...
private[ui] class ExecutorsPage(
  ...
  private def execRow(info: ExecutorSummary, logsExist: Boolean): Seq[Node] = {
    ...
    <tr>
      ...
      <td>
        {info.totalCores}
      </td>{taskData(info.maxTasks, info.activeTasks, info.failedTasks, info.completedTasks,
      info.totalTasks, info.totalDuration, info.totalGCTime)}<td sorttable_customkey={info.totalInputBytes.toString}>
  ...
  private def execSummaryRow(execInfo: Seq[ExecutorSummary], rowName: String): Seq[Node] = {
    ...
    <tr>
      ...
      <td>
        {totalCores}
      </td>{taskData(execInfo.map(_.maxTasks).sum,
      ...
      execInfo.map(_.totalGCTime).sum)}<td sorttable_customkey={totalInputBytes.toString}>
  ...
  Seq[Node] = {
    ...
    <table class={UIUtils.TABLE_CLASS_STRIPED}>
      <thead>
        ...
        <th>RDD Blocks</th>
        <th>
          <span data-toggle="tooltip" title={ToolTips.STORAGE_MEMORY}>Storage Memory</span>
        </th>
        <th>Disk Used</th>
        <th>Cores</th>
        <th>Active Tasks</th>
        ...
        <th>Total Tasks</th>
        <th>
          <span data-toggle="tooltip" title={ToolTips.TASK_TIME}>Task Time (GC Time)</span>
        </th>
        <th>
          <span data-toggle="tooltip" title={ToolTips.INPUT}>Input</span>
        </th>
        <th>
          <span data-toggle="tooltip" title={ToolTips.SHUFFLE_READ}>Shuffle Read</span>
        </th>
        <th>
  ...
  private def taskData(
    ...
    val tableData =
      <td style={if (activeTasks > 0) {
        ...
        <td sorttable_customkey={totalDuration.toString} style={// Red if GC time over GCTimePercent of total time
