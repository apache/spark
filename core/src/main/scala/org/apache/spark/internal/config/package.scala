...
package object config {
  ...
  private[spark] val DRIVER_CLASS_PATH =
    buildConf(SparkLauncher.DRIVER_EXTRA_CLASSPATH).stringConf.createOptional

  private[spark] val DRIVER_JAVA_OPTIONS =
    buildConf(SparkLauncher.DRIVER_EXTRA_JAVA_OPTIONS).stringConf.createOptional

  private[spark] val DRIVER_LIBRARY_PATH =
    buildConf(SparkLauncher.DRIVER_EXTRA_LIBRARY_PATH).stringConf.createOptional

  private[spark] val DRIVER_USER_CLASS_PATH_FIRST =
    buildConf("spark.driver.userClassPathFirst").booleanConf.createWithDefault(false)

  private[spark] val DRIVER_MEMORY = buildConf("spark.driver.memory")
    .doc("Amount of memory to use for the driver process, in MiB unless otherwise specified.")
    .bytesConf(ByteUnit.MiB)
    .createWithDefaultString("1g")

  private[spark] val DRIVER_MEMORY_OVERHEAD = buildConf("spark.driver.memoryOverhead")
  ...
  private[spark] val EVENT_LOG_COMPRESS =
    buildConf("spark.eventLog.compress")
      .booleanConf
      .createWithDefault(false)

  private[spark] val EVENT_LOG_BLOCK_UPDATES =
    buildConf("spark.eventLog.logBlockUpdates.enabled")
      .booleanConf
      .createWithDefault(false)

  private[spark] val EVENT_LOG_ALLOW_EC =
    buildConf("spark.eventLog.allowErasureCoding")
      .booleanConf
      .createWithDefault(false)

  private[spark] val EVENT_LOG_TESTING =
    buildConf("spark.eventLog.testing")
      .internal()
      .booleanConf
      .createWithDefault(false)

  private[spark] val EVENT_LOG_OUTPUT_BUFFER_SIZE = buildConf("spark.eventLog.buffer.kb")
  ...
  private[spark] val EVENT_LOG_STAGE_EXECUTOR_METRICS =
    buildConf("spark.eventLog.logStageExecutorMetrics.enabled")
      .booleanConf
      .createWithDefault(false)

  private[spark] val EVENT_LOG_OVERWRITE =
    buildConf("spark.eventLog.overwrite").booleanConf.createWithDefault(false)

  private[spark] val EVENT_LOG_CALLSITE_LONG_FORM =
    buildConf("spark.eventLog.longForm.enabled").booleanConf.createWithDefault(false)

  private[spark] val EXECUTOR_CLASS_PATH =
    buildConf(SparkLauncher.EXECUTOR_EXTRA_CLASSPATH).stringConf.createOptional

  private[spark] val EXECUTOR_HEARTBEAT_DROP_ZERO_ACCUMULATOR_UPDATES =
    buildConf("spark.executor.heartbeat.dropZeroAccumulatorUpdates")
      .internal()
      .booleanConf
      .createWithDefault(true)

  private[spark] val EXECUTOR_HEARTBEAT_INTERVAL =
    buildConf("spark.executor.heartbeatInterval")
      .timeConf(TimeUnit.MILLISECONDS)
      .createWithDefaultString("10s")

  private[spark] val EXECUTOR_HEARTBEAT_MAX_FAILURES =
    buildConf("spark.executor.heartbeat.maxFailures").internal().intConf.createWithDefault(60)

  private[spark] val EXECUTOR_JAVA_OPTIONS =
    buildConf(SparkLauncher.EXECUTOR_EXTRA_JAVA_OPTIONS).stringConf.createOptional

  private[spark] val EXECUTOR_LIBRARY_PATH =
    buildConf(SparkLauncher.EXECUTOR_EXTRA_LIBRARY_PATH).stringConf.createOptional

  private[spark] val EXECUTOR_USER_CLASS_PATH_FIRST =
    buildConf("spark.executor.userClassPathFirst").booleanConf.createWithDefault(false)

  private[spark] val EXECUTOR_MEMORY = buildConf("spark.executor.memory")
    .doc("Amount of memory to use per executor process, in MiB unless otherwise specified.")
    .bytesConf(ByteUnit.MiB)
    .createWithDefaultString("1g")

  private[spark] val EXECUTOR_MEMORY_OVERHEAD = buildConf("spark.executor.memoryOverhead")
    .doc("The amount of off-heap memory to be allocated per executor in cluster mode, " +
      "in MiB unless otherwise specified.")
    .bytesConf(ByteUnit.MiB)
    .createOptional

  private[spark] val MEMORY_OFFHEAP_ENABLED = buildConf("spark.memory.offHeap.enabled")
  ...
  private[spark] val MEMORY_OFFHEAP_SIZE = buildConf("spark.memory.offHeap.size")
  ...
  private[spark] val PYSPARK_EXECUTOR_MEMORY = buildConf("spark.executor.pyspark.memory")
    .bytesConf(ByteUnit.MiB)
    .createOptional

  private[spark] val IS_PYTHON_APP = buildConf("spark.yarn.isPython").internal()
    .booleanConf.createWithDefault(false)

  private[spark] val CPUS_PER_TASK = buildConf("spark.task.cpus").intConf.createWithDefault(1)

  private[spark] val DYN_ALLOCATION_MIN_EXECUTORS =
    buildConf("spark.dynamicAllocation.minExecutors").intConf.createWithDefault(0)

  private[spark] val DYN_ALLOCATION_INITIAL_EXECUTORS =
    buildConf("spark.dynamicAllocation.initialExecutors")
      .fallbackConf(DYN_ALLOCATION_MIN_EXECUTORS)

  private[spark] val DYN_ALLOCATION_MAX_EXECUTORS =
    buildConf("spark.dynamicAllocation.maxExecutors").intConf.createWithDefault(Int.MaxValue)

  private[spark] val DYN_ALLOCATION_EXECUTOR_ALLOCATION_RATIO =
    buildConf("spark.dynamicAllocation.executorAllocationRatio")
      .doubleConf.createWithDefault(1.0)

  private[spark] val LOCALITY_WAIT = buildConf("spark.locality.wait")
    .timeConf(TimeUnit.MILLISECONDS)
    .createWithDefaultString("3s")

  private[spark] val SHUFFLE_SERVICE_ENABLED =
    buildConf("spark.shuffle.service.enabled").booleanConf.createWithDefault(false)

  private[spark] val SHUFFLE_SERVICE_PORT =
    buildConf("spark.shuffle.service.port").intConf.createWithDefault(7337)

  private[spark] val KEYTAB = buildConf("spark.kerberos.keytab")
    .doc("Location of user's keytab.")
    .stringConf.createOptional

  private[spark] val PRINCIPAL = buildConf("spark.kerberos.principal")
    .doc("Name of the Kerberos principal.")
    .stringConf.createOptional

  private[spark] val KERBEROS_RELOGIN_PERIOD = buildConf("spark.kerberos.relogin.period")
    .timeConf(TimeUnit.SECONDS)
    .createWithDefaultString("1m")

  private[spark] val EXECUTOR_INSTANCES = buildConf("spark.executor.instances")
    .intConf
    .createOptional

  private[spark] val PY_FILES = buildConf("spark.yarn.dist.pyFiles")
    .internal()
    .stringConf
    .toSequence
  ...
  private[spark] val MAX_TASK_FAILURES =
    buildConf("spark.task.maxFailures")
      .intConf
      .createWithDefault(4)

  // Blacklist confs
  private[spark] val BLACKLIST_ENABLED =
    buildConf("spark.blacklist.enabled")
      .booleanConf
      .createOptional

  private[spark] val MAX_TASK_ATTEMPTS_PER_EXECUTOR =
    buildConf("spark.blacklist.task.maxTaskAttemptsPerExecutor")
      .intConf
      .createWithDefault(1)

  private[spark] val MAX_TASK_ATTEMPTS_PER_NODE =
    buildConf("spark.blacklist.task.maxTaskAttemptsPerNode")
      .intConf
      .createWithDefault(2)

  private[spark] val MAX_FAILURES_PER_EXEC =
    buildConf("spark.blacklist.application.maxFailedTasksPerExecutor")
      .intConf
      .createWithDefault(2)

  private[spark] val MAX_FAILURES_PER_EXEC_STAGE =
    buildConf("spark.blacklist.stage.maxFailedTasksPerExecutor")
      .intConf
      .createWithDefault(2)

  private[spark] val MAX_FAILED_EXEC_PER_NODE =
    buildConf("spark.blacklist.application.maxFailedExecutorsPerNode")
      .intConf
      .createWithDefault(2)

  private[spark] val MAX_FAILED_EXEC_PER_NODE_STAGE =
    buildConf("spark.blacklist.stage.maxFailedExecutorsPerNode")
      .intConf
      .createWithDefault(2)

  private[spark] val BLACKLIST_TIMEOUT_CONF =
    buildConf("spark.blacklist.timeout")
      .timeConf(TimeUnit.MILLISECONDS)
      .createOptional

  private[spark] val BLACKLIST_KILL_ENABLED =
    buildConf("spark.blacklist.killBlacklistedExecutors")
      .booleanConf
      .createWithDefault(false)

  private[spark] val BLACKLIST_LEGACY_TIMEOUT_CONF =
    buildConf("spark.scheduler.executorTaskBlacklistTime")
      .internal()
      .timeConf(TimeUnit.MILLISECONDS)
      .createOptional

  private[spark] val BLACKLIST_FETCH_FAILURE_ENABLED =
    buildConf("spark.blacklist.application.fetchFailure.enabled")
      .booleanConf
      .createWithDefault(false)
  // End blacklist confs

  private[spark] val UNREGISTER_OUTPUT_ON_HOST_ON_FETCH_FAILURE =
    buildConf("spark.files.fetchFailure.unRegisterOutputOnHost")
  ...
  private[spark] val LISTENER_BUS_EVENT_QUEUE_CAPACITY =
    buildConf("spark.scheduler.listenerbus.eventqueue.capacity")
      .intConf
  ...
  private[spark] val LISTENER_BUS_METRICS_MAX_LISTENER_CLASSES_TIMED =
    buildConf("spark.scheduler.listenerbus.metrics.maxListenerClassesTimed")
      .internal()
      .intConf
      .createWithDefault(128)

  // This property sets the root namespace for metrics reporting
  private[spark] val METRICS_NAMESPACE = buildConf("spark.metrics.namespace")
    .stringConf
    .createOptional

  private[spark] val PYSPARK_DRIVER_PYTHON = buildConf("spark.pyspark.driver.python")
    .stringConf
    .createOptional

  private[spark] val PYSPARK_PYTHON = buildConf("spark.pyspark.python")
    .stringConf
    .createOptional

  // To limit how many applications are shown in the History Server summary ui
  private[spark] val HISTORY_UI_MAX_APPS =
    buildConf("spark.history.ui.maxApplications").intConf.createWithDefault(Integer.MAX_VALUE)

  private[spark] val UI_SHOW_CONSOLE_PROGRESS = buildConf("spark.ui.showConsoleProgress")
    .doc("When true, show the progress bar in the console.")
    .booleanConf
    .createWithDefault(false)

  private[spark] val IO_ENCRYPTION_ENABLED = buildConf("spark.io.encryption.enabled")
    .booleanConf
    .createWithDefault(false)

  private[spark] val IO_ENCRYPTION_KEYGEN_ALGORITHM =
    buildConf("spark.io.encryption.keygen.algorithm")
      .stringConf
      .createWithDefault("HmacSHA1")

  private[spark] val IO_ENCRYPTION_KEY_SIZE_BITS = buildConf("spark.io.encryption.keySizeBits")
    .intConf
    .checkValues(Set(128, 192, 256))
    .createWithDefault(128)

  private[spark] val IO_CRYPTO_CIPHER_TRANSFORMATION =
    buildConf("spark.io.crypto.cipher.transformation")
      .internal()
      .stringConf
      .createWithDefaultString("AES/CTR/NoPadding")

  private[spark] val DRIVER_HOST_ADDRESS = buildConf("spark.driver.host")
    .doc("Address of driver endpoints.")
    .stringConf
    .createWithDefault(Utils.localCanonicalHostName())

  private[spark] val DRIVER_BIND_ADDRESS = buildConf("spark.driver.bindAddress")
    .doc("Address where to bind network listen sockets on the driver.")
    .fallbackConf(DRIVER_HOST_ADDRESS)

  private[spark] val BLOCK_MANAGER_PORT = buildConf("spark.blockManager.port")
    .doc("Port to use for the block manager when a more specific setting is not provided.")
    .intConf
    .createWithDefault(0)

  private[spark] val DRIVER_BLOCK_MANAGER_PORT = buildConf("spark.driver.blockManager.port")
    .doc("Port to use for the block manager on the driver.")
    .fallbackConf(BLOCK_MANAGER_PORT)

  private[spark] val IGNORE_CORRUPT_FILES = buildConf("spark.files.ignoreCorruptFiles")
  ...
  private[spark] val IGNORE_MISSING_FILES = buildConf("spark.files.ignoreMissingFiles")
    .doc("Whether to ignore missing files. If true, the Spark jobs will continue to run when " +
        "encountering missing files and the contents that have been read will still be returned.")
    .booleanConf
    .createWithDefault(false)

  private[spark] val APP_CALLER_CONTEXT = buildConf("spark.log.callerContext")
    .stringConf
    .createOptional

  private[spark] val FILES_MAX_PARTITION_BYTES = buildConf("spark.files.maxPartitionBytes")
    .doc("The maximum number of bytes to pack into a single partition when reading files.")
    .longConf
    .createWithDefault(128 * 1024 * 1024)

  private[spark] val FILES_OPEN_COST_IN_BYTES = buildConf("spark.files.openCostInBytes")
  ...
  private[spark] val HADOOP_RDD_IGNORE_EMPTY_SPLITS =
    buildConf("spark.hadoopRDD.ignoreEmptySplits")
      .internal()
  ...
  private[spark] val SECRET_REDACTION_PATTERN =
    buildConf("spark.redaction.regex")
  ...
  private[spark] val STRING_REDACTION_PATTERN =
    buildConf("spark.redaction.string.regex")
  ...
  private[spark] val AUTH_SECRET_BIT_LENGTH =
    buildConf("spark.authenticate.secretBitLength")
      .intConf
      .createWithDefault(256)

  private[spark] val NETWORK_AUTH_ENABLED =
    buildConf("spark.authenticate")
      .booleanConf
      .createWithDefault(false)

  private[spark] val SASL_ENCRYPTION_ENABLED =
    buildConf("spark.authenticate.enableSaslEncryption")
      .booleanConf
      .createWithDefault(false)

  private[spark] val NETWORK_ENCRYPTION_ENABLED =
    buildConf("spark.network.crypto.enabled")
      .booleanConf
      .createWithDefault(false)

  private[spark] val BUFFER_WRITE_CHUNK_SIZE =
    buildConf("spark.buffer.write.chunkSize")
      .internal()
  ...
  private[spark] val CHECKPOINT_COMPRESS =
    buildConf("spark.checkpoint.compress")
  ...
  private[spark] val SHUFFLE_ACCURATE_BLOCK_THRESHOLD =
    buildConf("spark.shuffle.accurateBlockThreshold")
  ...
  private[spark] val SHUFFLE_REGISTRATION_TIMEOUT =
    buildConf("spark.shuffle.registration.timeout")
  ...
  private[spark] val SHUFFLE_REGISTRATION_MAX_ATTEMPTS =
    buildConf("spark.shuffle.registration.maxAttempts")
  ...
  private[spark] val REDUCER_MAX_BLOCKS_IN_FLIGHT_PER_ADDRESS =
    buildConf("spark.reducer.maxBlocksInFlightPerAddress")
  ...
  private[spark] val MAX_REMOTE_BLOCK_SIZE_FETCH_TO_MEM =
    buildConf("spark.maxRemoteBlockSizeFetchToMem")
  ...
  private[spark] val TASK_METRICS_TRACK_UPDATED_BLOCK_STATUSES =
    buildConf("spark.taskMetrics.trackUpdatedBlockStatuses")
  ...
  private[spark] val SHUFFLE_FILE_BUFFER_SIZE =
    buildConf("spark.shuffle.file.buffer")
  ...
  private[spark] val SHUFFLE_UNSAFE_FILE_OUTPUT_BUFFER_SIZE =
    buildConf("spark.shuffle.unsafe.file.output.buffer")
  ...
  private[spark] val SHUFFLE_DISK_WRITE_BUFFER_SIZE =
    buildConf("spark.shuffle.spill.diskWriteBufferSize")
  ...
  private[spark] val UNROLL_MEMORY_CHECK_PERIOD =
    buildConf("spark.storage.unrollMemoryCheckPeriod")
      .internal()
  ...
  private[spark] val UNROLL_MEMORY_GROWTH_FACTOR =
    buildConf("spark.storage.unrollMemoryGrowthFactor")
      .internal()
  ...
  private[spark] val FORCE_DOWNLOAD_SCHEMES =
    buildConf("spark.yarn.dist.forceDownloadSchemes")
  ...
  private[spark] val UI_X_XSS_PROTECTION =
    buildConf("spark.ui.xXssProtection")
  ...
  private[spark] val UI_X_CONTENT_TYPE_OPTIONS =
    buildConf("spark.ui.xContentTypeOptions.enabled")
  ...
  private[spark] val UI_STRICT_TRANSPORT_SECURITY =
    buildConf("spark.ui.strictTransportSecurity")
      .doc("Value for HTTP Strict Transport Security Response Header")
      .stringConf
      .createOptional

  private[spark] val EXTRA_LISTENERS = buildConf("spark.extraListeners")
  ...
  private[spark] val SHUFFLE_SPILL_NUM_ELEMENTS_FORCE_SPILL_THRESHOLD =
    buildConf("spark.shuffle.spill.numElementsForceSpillThreshold")
      .internal()
  ...
  private[spark] val SHUFFLE_MAP_OUTPUT_PARALLEL_AGGREGATION_THRESHOLD =
    buildConf("spark.shuffle.mapOutput.parallelAggregationThreshold")
      .internal()
  ...
  private[spark] val MAX_RESULT_SIZE = buildConf("spark.driver.maxResultSize")
    .doc("Size limit for results.")
    .bytesConf(ByteUnit.BYTE)
    .createWithDefaultString("1g")

  private[spark] val CREDENTIALS_RENEWAL_INTERVAL_RATIO =
    buildConf("spark.security.credentials.renewalRatio")
  ...
  private[spark] val CREDENTIALS_RENEWAL_RETRY_WAIT =
    buildConf("spark.security.credentials.retryWait")
  ...
  private[spark] val SHUFFLE_MIN_NUM_PARTS_TO_HIGHLY_COMPRESS =
    buildConf("spark.shuffle.minNumPartitionsToHighlyCompress")
      .internal()
  ...
  private[spark] val MEMORY_MAP_LIMIT_FOR_TESTS =
    buildConf("spark.storage.memoryMapLimitForTests")
      .internal()
  ...
  private[spark] val BARRIER_SYNC_TIMEOUT =
    buildConf("spark.barrier.sync.timeout")
  ...
  private[spark] val UNSCHEDULABLE_TASKSET_TIMEOUT =
    buildConf("spark.scheduler.blacklist.unschedulableTaskSetTimeout")
  ...
  private[spark] val BARRIER_MAX_CONCURRENT_TASKS_CHECK_INTERVAL =
    buildConf("spark.scheduler.barrier.maxConcurrentTasksCheck.interval")
  ...
  private[spark] val BARRIER_MAX_CONCURRENT_TASKS_CHECK_MAX_FAILURES =
    buildConf("spark.scheduler.barrier.maxConcurrentTasksCheck.maxFailures")
  ...
  private[spark] val EXECUTOR_PLUGINS =
    buildConf("spark.executor.plugins")
