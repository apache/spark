...
private[spark] object History {
  ...
  val EVENT_LOG_DIR = buildConf("spark.history.fs.logDirectory")
    .stringConf
    .createWithDefault(DEFAULT_LOG_DIR)

  val MAX_LOG_AGE_S = buildConf("spark.history.fs.cleaner.maxAge")
    .timeConf(TimeUnit.SECONDS)
    .createWithDefaultString("7d")

  val LOCAL_STORE_DIR = buildConf("spark.history.store.path")
  ...
  val MAX_LOCAL_DISK_USAGE = buildConf("spark.history.store.maxDiskUsage")
    .bytesConf(ByteUnit.BYTE)
    .createWithDefaultString("10g")

  val HISTORY_SERVER_UI_PORT = buildConf("spark.history.ui.port")
    .doc("Web UI port to bind Spark History Server")
    .intConf
    .createWithDefault(18080)

  val FAST_IN_PROGRESS_PARSING =
    buildConf("spark.history.fs.inProgressOptimization.enabled")
  ...
  val END_EVENT_REPARSE_CHUNK_SIZE =
    buildConf("spark.history.fs.endEventReparseChunkSize")
