...
private[spark] class CompressedMapStatus(
    ...
    private[this] var patitionSegments: scala.collection.Map[Int, Short])
  ...
  override def readExternal(in: ObjectInput): Unit = Utils.tryOrIOException {
    ...
    val patitionSegmentsImpl = mutable.Map.empty[Int, Short]
    (0 until count).foreach { _ =>
      val block = in.readInt()
      val segments = in.readShort()
      patitionSegmentsImpl(block) = segments
    }
    patitionSegments = patitionSegmentsImpl
  }
}
...
private[spark] class HighlyCompressedMapStatus private (
    ...
    private var hugeBlockSizes: scala.collection.Map[Int, Byte],
    private var partitionSegments: scala.collection.Map[Int, Short])
  extends MapStatus with Externalizable {
...
private[spark] object HighlyCompressedMapStatus {
  def apply(loc: BlockManagerId, uncompressedSizes: Array[Long]): HighlyCompressedMapStatus = {
    ...
    val partitionSegments = mutable.Map.empty[Int, Short]
    while (i < totalNumBlocks) {
      ...
      if (size > 0) {
        ...
        if (size > MapStatus.SHUFFLE_FETCH_THRESHOLD) {
          partitionSegments(i) =
            math.ceil(size.toDouble / MapStatus.SHUFFLE_FETCH_THRESHOLD).toShort
        }
    ...
    new HighlyCompressedMapStatus(loc, numNonEmptyBlocks, emptyBlocks, avgSize,
      hugeBlockSizes, partitionSegments)
  }
}
