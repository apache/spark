...
private[spark] class DAGScheduler(
  ...
  def createShuffleMapStage[K, V, C](
    ...
    // pipelinedCompletedPartitions, the reader locates writers through the streaming tracker, and
    // every MapOutputTracker path a pipelined shuffle could reach is either type-gated out or
    // tolerant of its absence). A regular shuffle registers with the MapOutputTracker as before.
