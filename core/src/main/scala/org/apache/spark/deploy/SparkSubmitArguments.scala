...
private[deploy] class SparkSubmitArguments(args: Seq[String], env: Map[String, String] = sys.env)
  ...
  private def loadEnvironmentArguments(): Unit = {
    ...
    keytab = Option(keytab).orElse(sparkProperties.get("spark.kerberos.keytab")).orNull
    principal = Option(principal).orElse(sparkProperties.get("spark.kerberos.principal")).orNull
