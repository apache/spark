...
private[history] class FsHistoryProvider(conf: SparkConf, clock: Clock)
  ...
  private def replay(
    ...
    try {
      ...
      val isImportedEventLog = logPath.toString.endsWith(IMPORTED_LOG_SUFFIX)
      ...
      if (appListener.appId.isDefined || !sparkVersionHasAppId(eventLog)) {
        Some(new FsApplicationAttemptInfo(
          ...
          appListener.appId.map {s => if (isImportedEventLog) s + IMPORTED_LOG_SUFFIX else s }
            .getOrElse(logPath.getName()),
