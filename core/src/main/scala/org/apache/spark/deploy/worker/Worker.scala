...
private[deploy] object Worker extends Logging {
  ...
  def main(argStrings: Array[String]) {
    ...
    // bounded, we may launch no more than one external shuffle service on each host.
    ...
    val sparkWorkerInstances = Option(System.getenv("SPARK_WORKER_INSTANCES")).getOrElse("1").toInt
    require(externalShuffleServiceEnabled == false || sparkWorkerInstances <= 1,
      "Start multiple worker on one host failed because we may launch no more than one external " +
        "shuffle service on each host, please set spark.shuffle.service.enabled to false or set " +
        "SPARK_WORKER_INSTANCES to 1 to resolve the conflict.")
