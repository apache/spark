 ...
 *
 */
private[spark] class StandaloneRestServerHandler(master: Master)
  extends StandaloneRestHandler(master) {
  ...
  override protected def handleKill(
    ...
    val response = AkkaUtils.askWithReply[KillDriverResponse](
      RequestKillDriver(driverId), master.self, askTimeout)
  ...
  private def buildDriverDescription(request: SubmitDriverRequestMessage): DriverDescription = {
    ...
    // Required fields
    //val _master = request.getFieldNotNull(MASTER)
    ...
    request.getFields.foreach { case (k, v) =>
      println(s"> Found this field: $k = $v")
      k match {
    ...
    conf.set("spark.master", master.masterUrl)
