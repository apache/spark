...
/**
 * Identifies where an update for a data property accumulator update came from.  This is important
 * to ensure that updates are not double-counted when rdds get recomputed.  When the executors send
 * back data property acucmualtor updates, they seperate out the updates per rdd or shuffle output
 * generated by the task. That gives the driver sufficient info to ensure that each update is
 * counted once. The shuffleId is important since two seperate shuffle actions could happen on the
 * same RDD and the RDD ID and partition ID with different accumulators.
 *
 * In addition to tracking where updates are coming from, the same TaskOutputId's are used to keep
 * track of if an update represents that of a complete partition. Partial evaluations (e.g. `take`)
 * should not be applied since they would prevent a later full evaluation of the partition from
 * being counted.
 *
...
abstract class TaskContext extends Serializable {
  ...
  /**
   * Set the information about the current task being processed for data property accumulator
   * tracking.
   */
  private[spark] def setTaskOutputInfo(taskOutputId: TaskOutputId): Unit

  /**
   * Returns the information about the current task being processed for data property accumulator
   * tracking.
   */
  private[spark] def getTaskOutputInfo(): TaskOutputId
}
