...
private[spark] object TestUtils {
  ...
  /** Creates a compiled class with the source file. Class file(s) will be placed in destDir.
    *
    * Note: This will return a Seq[File] containing the main class as well as any named or
    * anonymous inner classes.
    */
  def createCompiledClasses(
      className: String,
      destDir: File,
    ...
    // Calling this outputs class files in pwd. It's easier to just rename the files than
    ...
    // Assemble a pattern to match all classes generated by className. This includes
    // named and anonymous inner classes.
    val innerClassSuffix = "(\\$[0-1a-zA-Z]+)*"
    val resultRegex = "^" + className + innerClassSuffix + "\\.class$"

    val results = new File(".").listFiles.filter(x => x.isFile && x.getName.matches(resultRegex))
    results.map({ result =>
  ...
  def createCompiledClass(
    ...
    createCompiledClasses(className, destDir, sourceFile, classpathUrls).head
  }
}
