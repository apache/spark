...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  private long[] mergeSpillsWithFileStream(
    ...
    final int inputBufferSizeInBytes = (int) sparkConf.getSizeAsKb(
            "spark.shuffle.file.buffer", "32k") * 1024;
    ...
    try {
      ...
      for (int partition = 0; partition < numPartitions; partition++) {
        ...
        // Shield the underlying output stream from close() and flush() calls, so that we can
        // close the higher level streams to make sure all data is really flushed and internal
        // state is cleaned.
