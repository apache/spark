...
class TreeNodeSuite extends FunSuite {
  ...
  test("top node changed") {
    val after = Literal(1) transformDown { case Literal(1, _) => Literal(2) }
    assert(after === Literal(2))
  }

  test("one child changed") {
    val before = Add(Literal(1), Literal(2))
    val after = before transformDown { case Literal(2, _) => Literal(1) }
  ...
  test("no change") {
    ...
    val after = before transformDown { case Literal(5, _) => Literal(1)}
