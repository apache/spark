...
object KinesisUtils {
   ...
   * Credentials used to access Kinesis stream will be resolved using 
   ...
   *                                 the tip of the stream (InitialPositionInStream.LATEST).
   * @param storageLevel Storage level to use for storing the received objects
   * @param credentials Credentials to access Kinesis stream.
   *
   * @return ReceiverInputDStream[Array[Byte]]
   */
  @Experimental
  def createStream(
    ...
    ssc.receiverStream(new KinesisReceiver(ssc.sc.appName, streamName, endpointUrl,
        checkpointInterval, initialPositionInStream, storageLevel, 
        Some(credentials.getAWSAccessKeyId(), credentials.getAWSSecretKey())))
  }
