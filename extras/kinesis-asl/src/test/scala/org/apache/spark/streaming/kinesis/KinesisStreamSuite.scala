...
abstract class KinesisStreamTests(aggregateTestData: Boolean) extends KinesisFunSuite
  ...
  test("RDD generation") {
    ...
    nonEmptyRDD shouldBe a [KinesisBackedBlockRDD[Array[Byte] @unchecked]]
    val kinesisRDD = nonEmptyRDD.asInstanceOf[KinesisBackedBlockRDD[Array[Byte] @unchecked]]
  ...
  testIfEnabled("failure recovery") {
    ...
    times.foreach { time =>
      ...
      rdd shouldBe a [KinesisBackedBlockRDD[Array[Byte] @unchecked]]
