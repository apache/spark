...
private[client] class Shim_v0_13 extends Shim_v0_12 {
  ...
  object OperatorsInPartitionFilters {
  ...
  private def convertBasicFilters(table: Table, filters: Seq[Expression]): String = {
    ...
    filters.collect {
      case op @ OperatorsInPartitionFilters(a: Attribute, Literal(v, _: IntegralType)) =>
        s"${a.name} ${op.symbol} $v"
      case op @ OperatorsInPartitionFilters(Literal(v, _: IntegralType), a: Attribute) =>
        s"$v ${op.symbol} ${a.name}"
      case op @ OperatorsInPartitionFilters(a: Attribute, Literal(v, _: StringType))
        if !varcharKeys.contains(a.name) =>
        s"""${a.name} ${op.symbol} ${quoteStringLiteral(v.toString)}"""
      case op @ OperatorsInPartitionFilters(Literal(v, _: StringType), a: Attribute)
        if !varcharKeys.contains(a.name) =>
  ...
  private def convertComplexFilters(table: Table, filters: Seq[Expression]): String = {
    ...
    lazy val convert: PartialFunction[Expression, String] = {
      ...
      case InSet(a: Attribute, ExtractableValues(values))
          if !varcharKeys.contains(a.name) && values.nonEmpty =>
        convertInToOr(a, values)
      case op @ OperatorsInPartitionFilters(a: Attribute, ExtractableLiteral(value))
          if !varcharKeys.contains(a.name) =>
        s"${a.name} ${op.symbol} $value"
      case op @ OperatorsInPartitionFilters(ExtractableLiteral(value), a: Attribute)
          if !varcharKeys.contains(a.name) =>
