...
case class InsertIntoHiveTable(
  ...
  @transient private lazy val hiveContext =
    new Context(sc.sessionState.asInstanceOf[HiveSessionState].hiveconf)
  @transient private lazy val client =
    sc.sessionState.asInstanceOf[HiveSessionState].metadataHive
  ...
  protected[sql] lazy val sideEffectResult: Seq[InternalRow] = {
    ...
    val hiveconf = sc.sessionState.asInstanceOf[HiveSessionState].hiveconf
    ...
    if (numDynamicPartitions > 0) {
      ...
      if (numStaticPartitions == 0 &&
        hiveconf.getVar(HiveConf.ConfVars.DYNAMICPARTITIONINGMODE).equalsIgnoreCase("strict")) {
