...
import org.apache.spark.sql.catalyst.analysis.{FunctionRegistry, TableAlreadyExistsException, UnresolvedFunction}
...
private[spark] class HiveExternalCatalog(conf: SparkConf, hadoopConf: Configuration)
  ...
  private def tableMetaToTableProps(
    ...
    schema.foreach { field =>
      if (field.metadata.contains("defaultValueExpression")) {
        val expr = field.metadata.getExpression("defaultValueExpression")
