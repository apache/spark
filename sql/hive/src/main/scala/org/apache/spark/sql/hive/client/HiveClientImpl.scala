...
private[hive] class HiveClientImpl(
    ...
    Option(client.getTable(dbName, tableName, false)).map { h =>
      ...
      CatalogTable(
        ...
        storage = CatalogStorageFormat(
          locationUri = shim.getDataLocation(h).map(new Path(_).toUri),
  ...
  private def toHiveTable(table: CatalogTable): HiveTable = {
    ...
    table.storage.locationUri.foreach { loc => shim.setDataLocation(hiveTable, loc.getPath) }
  ...
  private def toHivePartition(
    ...
    p.storage.locationUri.foreach(u => storageDesc.setLocation(u.getPath))
  ...
  private def fromHivePartition(hp: HivePartition): CatalogTablePartition = {
    ...
    CatalogTablePartition(
      ...
      storage = CatalogStorageFormat(
        locationUri = Option(apiPartition.getSd.getLocation).map(new Path(_).toUri),
