...
class HiveScriptTransformationWriterThread(
    ...
    conf: Configuration
  ) extends ScriptTransformationWriterThread(
    iter,
    inputSchema,
    outputProjection,
    ioschema,
  ...
  var scriptInputWriter: Option[RecordWriter] = None

  override def init(): Unit = {
    super.init()
    dataOutputStream = new DataOutputStream(outputStream)
    scriptInputWriter = ioschema.recordWriter(dataOutputStream, conf)
  }

  override def processRow(row: InternalRow, numColumns: Int): Unit = {
    if (inputSerde == null) {
      super.processRow(row, numColumns)
    } else {
      val writable = inputSerde.serialize(
        row.asInstanceOf[GenericInternalRow].values, inputSoi)

      if (scriptInputWriter.isDefined) {
        scriptInputWriter.get.write(writable)
      } else {
...
case class HiveScriptIOSchema (
  ...
  extends ScriptTransformIOSchema(
    ...
    outputRowFormat,
    schemaLess
  ) with HiveInspectors with Serializable {
