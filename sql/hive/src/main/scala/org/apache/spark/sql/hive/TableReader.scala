...
class HadoopTableReader(
  ...
  private val _minSplitsPerRDD = if (sc.sparkContext.isLocal) {
    ...
    math.max(
      sc.sessionState.asInstanceOf[HiveSessionState].hiveconf.getInt("mapred.map.tasks", 1),
        ...
        partitionToDeserializer.filter {
          case (partition, partDeserializer) =>
            def updateExistPathSetByPathPattern(pathPatternStr: String) {
              ...
              val fs = pathPattern.getFileSystem(
                sc.sessionState.asInstanceOf[HiveSessionState].hiveconf)
  ...
  private def applyFilterIfNeeded(path: Path, filterOpt: Option[PathFilter]): String = {
    filterOpt match {
      case Some(filter) =>
        val fs = path.getFileSystem(sc.sessionState.asInstanceOf[HiveSessionState].hiveconf)
