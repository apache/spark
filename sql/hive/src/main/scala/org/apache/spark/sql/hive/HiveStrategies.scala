...
import org.apache.spark.sql.catalyst.plans.logical.{DeleteFromTable, Filter, InsertIntoDir, InsertIntoStatement, LogicalPlan, ReplaceData, ScriptTransformation, Statistics, SubqueryAlias}
...
class DetermineTableStats(session: SparkSession) extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    def resolveHiveTableWithStats(p: LogicalPlan): LogicalPlan = p resolveOperators {
      case relation: HiveTableRelation
        if DDLUtils.isHiveTable(relation.tableMeta) && relation.tableMeta.stats.isEmpty =>
        hiveTableWithStats(relation)

      // handles InsertIntoStatement specially as the table in InsertIntoStatement is not
      // added in its children, hence not matched directly by previous HiveTableRelation case.
      case i@InsertIntoStatement(relation: HiveTableRelation, _, _, _, _, _, _)
        if DDLUtils.isHiveTable(relation.tableMeta) && relation.tableMeta.stats.isEmpty =>
        i.copy(table = hiveTableWithStats(relation))

      // handles `ReplaceData.groupFilterCondition` generated by `RewriteMergeIntoTable`
      case r @ ReplaceData(_, _, _, _,
        Some(e @ Exists(f @ Filter(_, source: LogicalPlan), _, _, _, _)), _) =>
        val newSource = resolveHiveTableWithStats(source)
        r.copy(groupFilterCondition = Some(e.copy(plan = f.copy(child = newSource))))
    }

    resolveHiveTableWithStats(plan)
  }
}
