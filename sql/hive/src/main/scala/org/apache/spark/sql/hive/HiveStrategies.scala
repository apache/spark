...
object HiveAnalysis extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    case InsertIntoTable(r: CatalogRelation, partSpec, query, overwrite, ifStaticPartitionNotExists)
        if DDLUtils.isHiveTable(r.tableMeta) =>
      InsertIntoHiveTable(r.tableMeta, partSpec, query, overwrite, ifStaticPartitionNotExists)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    plan transformUp {
      ...
      case InsertIntoTable(
        r: CatalogRelation, partition, query, overwrite, ifStaticPartitionNotExists)
        // Inserting into partitioned table is not supported in Parquet/Orc data source (yet).
          if query.resolved && DDLUtils.isHiveTable(r.tableMeta) &&
            !r.isPartitioned && isConvertible(r) =>
        InsertIntoTable(convert(r), partition, query, overwrite, ifStaticPartitionNotExists)
