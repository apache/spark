...
import org.apache.spark.sql.execution.datasources.orc.{OrcTest, OrcUtils}
import org.apache.spark.sql.hive.test.TestHiveSingleton
import org.apache.spark.sql.internal.SQLConf
import org.apache.spark.sql.types.{LongType, StructField, StructType}

class HiveOrcFileFormatSuite extends OrcTest with TestHiveSingleton {

  test("read and merge orc schemas in parallel using hive orc lib") {
    def testMergeSchemasInParallelUsingHiveOrcLib(ignoreCorruptFiles: Boolean): Unit = {
      withSQLConf(
    ...
    testMergeSchemasInParallelUsingHiveOrcLib(true)
    val exception = intercept[SparkException] {
      testMergeSchemasInParallelUsingHiveOrcLib(false)
    }.getCause
