...
import org.apache.spark.sql.hive.execution.{HiveComparisonTest, HiveTableScanExec}
  ...
  test("simple inner join triggers DPP with mock-up tables") {
    withSQLConf(SQLConf.DYNAMIC_PARTITION_PRUNING_ENABLED.key -> "true",
      ...
      withTable("df1", "df2", "df1_hive", "df2_hive") {
        ...
        Seq(
          """
            |CREATE TABLE df1_hive(id int)
            |STORED AS PARQUET
            |PARTITIONED BY (k string)
          """.stripMargin,
          """
            |INSERT OVERWRITE TABLE df1_hive
            |SELECT * FROM df1
          """.stripMargin,
          """
            |CREATE TABLE df2_hive(id int)
            |STORED AS PARQUET
            |PARTITIONED BY (k string)
          """.stripMargin,
          """
            |INSERT OVERWRITE TABLE df2_hive
            |SELECT * FROM df2
          """.stripMargin).foreach(sql)

        val df = sql("SELECT df1_hive.id, df2_hive.k" +
          " FROM df1_hive JOIN df2_hive" +
          " ON df1_hive.k = df2_hive.k AND df2_hive.id < 2")
