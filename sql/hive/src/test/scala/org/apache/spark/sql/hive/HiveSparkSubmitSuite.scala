...
class HiveSparkSubmitSuite
  ...
  test("SPARK-8368: includes jars passed in through --jars") {
    ...
    val jar1 = TestUtils.createJarWithClasses(Seq("SparkSubmitClassA"))
    val jar2 = TestUtils.createJarWithClasses(Seq("SparkSubmitClassB"))
    val jar3 = TestHive.getHiveFile("hive-contrib-2.3.4.jar").getCanonicalPath
    val jar4 = TestHive.getHiveFile("hive-hcatalog-core-2.3.4.jar").getCanonicalPath
  ...
  test("SPARK-18989: DESC TABLE should not fail with format class not found") {
    ...
    val argsForCreateTable = Seq(
      ...
      "--conf", "spark.ui.enabled=false",
      "--conf", "spark.master.rest.enabled=false",
      "--jars", TestHive.getHiveFile("hive-contrib-2.3.4.jar").getCanonicalPath,
      unusedJar.toString)
...
object TemporaryHiveUDFTest extends Logging {
  def main(args: Array[String]) {
    ...
    val jar = hiveContext.getHiveFile("hive-contrib-2.3.4.jar").getCanonicalPath
    hiveContext.sql(
      s"""
