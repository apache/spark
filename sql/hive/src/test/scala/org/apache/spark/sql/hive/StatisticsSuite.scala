...
class StatisticsSuite
  ...
  test("analyze MetastoreRelations") {
    def queryTotalSize(tableName: String): BigInt =
      sqlContext.catalog.lookupRelation(Seq(tableName)).statistics.sizeInBytes
    ...
    assert(queryTotalSize("analyzeTable_part") === sqlContext.conf.defaultSizeInBytes)
    ...
    sqlContext.catalog.unregisterTable(Seq("tempTable"))
  }
  ...
  test("auto converts to broadcast hash join, by size estimate of a relation") {
    def mkTest(
      ...
      assert(sizes.size === 2 && sizes(0) <= sqlContext.conf.autoBroadcastJoinThreshold
        && sizes(1) <= sqlContext.conf.autoBroadcastJoinThreshold,
      ...
      sqlContext.conf.settings.synchronized {
        val tmp = sqlContext.conf.autoBroadcastJoinThreshold
  ...
  test("auto converts to broadcast left semi join, by size estimate of a relation") {
    ...
    assert(sizes.size === 2 && sizes(1) <= sqlContext.conf.autoBroadcastJoinThreshold
      && sizes(0) <= sqlContext.conf.autoBroadcastJoinThreshold,
      s"query should contain two relations, each of which has size smaller than autoConvertSize")
