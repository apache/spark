...
class MetastoreDataSourcesSuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("SPARK-6655 still support a schema stored in spark.sql.sources.schema") {
    ...
    withTable(tableName) {
      ...
      externalCatalog.createTable("default", hiveTable, ignoreIfExists = false)
  ...
  test("Saving partitionBy columns information") {
    ...
    withTable(tableName) {
      ...
      invalidateTable(tableName)
      val metastoreTable = externalCatalog.getTable("default", tableName)
  ...
  test("skip hive metadata on table creation") {
    withTempDir { tempPath =>
      ...
      assert(externalCatalog.getTable("default", "not_skip_hive_metadata").schema
      ...
      assert(externalCatalog.getTable("default", "skip_hive_metadata").schema.forall { c =>
