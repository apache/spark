...
class HiveCatalogedDDLSuite extends DDLSuite with TestHiveSingleton with BeforeAndAfterEach {
  ...
  test("SPARK-23890: Support CHANGE COLUMN to add nested fields to structs in arrays") {
    withTable("t1") {
      spark.sql("CREATE TABLE t1 (q ARRAY<STRUCT<col1:INT, col2:STRING>>, i1 INT)")
      spark.sql(
        "ALTER TABLE t1 CHANGE COLUMN q " +
        "q ARRAY<STRUCT<col1:INT, col2:STRING, col3:STRING>>"
      )
      assert(
        spark.sql("SELECT * FROM t1")
             .schema("q").dataType.asInstanceOf[ArrayType].elementType.asInstanceOf[StructType].fields.length == 3
        )
    }
  }

  test("SPARK-23890: negative Support CHANGE COLUMN to add nested fields to structs in arrays") {
    withTable("t1") {
      spark.sql("CREATE TABLE t1 (q ARRAY<STRUCT<col1:INT, col2:STRING>>, i1 INT)")
      val err = intercept[AnalysisException] {
        // Changing col2 to INT should throw exception
        spark.sql(
          "ALTER TABLE t1 CHANGE COLUMN q " +
          "q ARRAY<STRUCT<col1:INT, col2:INT, col3:STRING>>"
        )
      }.getMessage
      assert(err.contains("ALTER TABLE CHANGE COLUMN is not supported for changing column 'q'"))
    }
