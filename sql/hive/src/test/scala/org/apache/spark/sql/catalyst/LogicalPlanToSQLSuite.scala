 ...
 * built-in functionality to automatically generate these golden files. To do so, set the
 * saveQuery flag to true and run the test suite. After generating the golden files, make sure
