...
class LogicalPlanToSQLSuite extends SQLBuilderTest with SQLTestUtils {
  ...
  private def checkSQLStructure(originalSQL: String, convertedSQL: String, answerFile: String) = {
    val normalizedSQL = convertedSQL.replaceAll("#\\d+L?", "")
    if (answerFile != null) {
      val separator = "-" * 80
      if (regenerateGoldenFiles) {
        ...
        assert(normalizedSQL == expectedSQL)
      }
    }
  }
  ...
  if (!regenerateGoldenFiles) {
    ...
    test("Test should fail if the SQL query cannot be regenerated") {
      case class Unsupported() extends LeafNode with MultiInstanceRelation with NonSQLPlan {
  ...
  test("plans with non-SQL expressions") {
    ...
    intercept[UnsupportedOperationException](sql("SELECT foo(id) FROM t0").logicalPlan.sql)
  }
