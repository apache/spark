...
class HiveQuerySuite extends HiveComparisonTest with BeforeAndAfter {
  ...
  test("case sensitivity: registered table") {
    val testData =
      TestHive.sparkContext.parallelize(
        TestData(1, "str1") ::
        TestData(2, "str2") :: Nil)
    testData.toDataFrame.registerTempTable("REGisteredTABle")
  ...
  test("SPARK-2180: HAVING support in GROUP BY clauses (positive)") {
    ...
    TestHive.sparkContext.parallelize(fixture).toDataFrame.registerTempTable("having_test")
    val results =
  ...
  test("DESCRIBE commands") {
    ...
    val testData =
      TestHive.sparkContext.parallelize(
        TestData(1, "str1") ::
        TestData(1, "str2") :: Nil)
    testData.toDataFrame.registerTempTable("test_describe_commands2")
  ...
  test("SPARK-3414 regression: should store analyzed logical plan when registering a temp table") {
    sparkContext.makeRDD(Seq.empty[LogEntry]).toDataFrame.registerTempTable("rawLogs")
    sparkContext.makeRDD(Seq.empty[LogFile]).toDataFrame.registerTempTable("logFiles")
