...
class InsertIntoHiveTableSuite extends QueryTest with BeforeAndAfter with MyTestHiveContext {
  private val ctx = hiveContext
  import ctx.implicits._
  import ctx._

  private val _testData = sparkContext.parallelize(
  ...
  test("SPARK-4052: scala.collection.Map as value type of MapType") {
    ...
    val df = createDataFrame(rowRDD, schema)
  ...
  test("Insert ArrayType.containsNull == false") {
    ...
    val rowRDD = sparkContext.parallelize((1 to 100).map(i => Row(Seq(s"value$i"))))
    val df = createDataFrame(rowRDD, schema)
  ...
  test("SPARK-5498:partition schema does not match table schema") {
    ...
    val testDatawithNull = sparkContext.parallelize(
