...
class HiveDDLCommandSuite extends PlanTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("Test CTAS #1") {
    ...
    assert(desc.storage.locationUriString == Some("/user/external/page_view"))
  ...
  test("create table - external") {
    ...
    val (desc, _) = extractTableDesc(query)
    assert(desc.tableType == CatalogTableType.EXTERNAL)
    assert(desc.storage.locationUriString == Some("/path/to/nowhere"))
  }
  ...
  test("create table - everything!") {
    ...
    assert(desc.viewText.isEmpty)
    assert(desc.viewOriginalText.isEmpty)
    assert(desc.storage.locationUriString == Some("/path/to/mercury"))
  ...
  test("create hive serde table with new syntax - basic") {
    ...
    assert(table.storage.locationUriString == Some("/tmp/file"))
