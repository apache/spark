...
class BugTests extends QueryTest with SQLTestUtils {
  ...
  override protected def beforeAll(): Unit = {
    ...
    val sparkContext = new SparkContext(
      new SparkConf()
        ...
        .set(InsertEventListener.keyForResultFileName, resultFile)
  ...
  test("SPARK-43112 insert using dataframe api on partitioned table") {
    ...
    //  assert(tableSchema == dataFrameSchema)
  ...
  private def assertFromResultsFile(expectedPartitionCount: Int): Unit = {
    val reader = new BufferedReader(new InputStreamReader(new FileInputStream(
      new File(scratchDir, resultFile))))
...
class InsertEventListener(config: Configuration) extends MetaStoreEventListener(config) {
  ...
  lazy val resultFile = {
    val scratchDir = new File(config.get(ConfVars.SCRATCHDIR.varname).substring("file:".length))
    scratchDir.mkdir()
