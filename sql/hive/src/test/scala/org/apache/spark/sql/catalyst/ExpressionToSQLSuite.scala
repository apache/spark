...
class ExpressionToSQLSuite extends SQLBuilderTest with SQLTestUtils {
  ...
  test("aggregate functions") {
    ...
    checkSqlGeneration("SELECT percentile_approx(b, 0.5) FROM t2 GROUP BY a")
    checkSqlGeneration("SELECT percentile_approx(b, array(0.5)) FROM t2 GROUP BY a")
    checkSqlGeneration("SELECT percentile_approx(b, array(0.25, 0.75)) FROM t2 GROUP BY a")
    checkSqlGeneration("SELECT percentile_approx(b, 0.5, 100) FROM t2 GROUP BY a")
    checkSqlGeneration("SELECT percentile_approx(b, array(0.5), 100) FROM t2 GROUP BY a")
    checkSqlGeneration("SELECT percentile_approx(b, array(0.25, 0.75), 100) FROM t2 GROUP BY a")
