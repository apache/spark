...
import org.apache.spark.sql.hive.thriftserver.{cli, HiveThriftServer2}
    ...
    try {
      ...
      case e: Throwable =>
        ...
        if (currentState.isTerminal) {
          ...
          HiveThriftServer2.listener.onStatementError(
            statementId, e.getMessage, SparkUtils.exceptionString(e))
          if (e.isInstanceOf[SparkThriftServerSQLException]) {
            throw e.asInstanceOf[SparkThriftServerSQLException]
          } else {
            throw new SparkThriftServerSQLException("Error running query: " + e.toString, e)
          }
        }
    } finally {
      synchronized {
