...
private[hive] class SparkSQLSessionManager(hiveServer: HiveServer2, sparkSession: SparkSession)
  ...
  override def openSession(
    ...
    try {
      ...
      session.conf.set(SQLConf.DATETIME_JAVA8API_ENABLED, true)
      ...
      sparkSqlOperationManager.sessionToSparkSession.put(sessionHandle, session)
      sessionHandle
    } catch {
      case NonFatal(e) =>
        try {
          ...
          case NonFatal(inner) =>
            logWarning("Error closing hiveSession", inner)
        }
  ...
  override def closeSession(sessionHandle: SessionHandle): Unit = {
    ...
    val session =
      sparkSqlOperationManager.sessionToSparkSession.getOrDefault(sessionHandle, sparkSession)
  ...
  def setConfMap(session: SparkSession, confMap: java.util.Map[String, String]): Unit = {
    val iterator = confMap.entrySet().iterator()
    while (iterator.hasNext) {
      val kv = iterator.next()
      session.conf.set(kv.getKey, kv.getValue)
    }
  }
}
