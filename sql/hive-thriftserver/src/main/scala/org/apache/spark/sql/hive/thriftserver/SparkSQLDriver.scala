...
private[hive] class SparkSQLDriver(val session: SparkSession = SparkSQLEnv.sparkSession)
  extends Driver
  with Logging {
  ...
  override def run(command: String): CommandProcessorResponse = {
    try {
      val substitutorCommand = SQLConf.withExistingConf(session.sessionState.conf) {
        new VariableSubstitution().substitute(command)
      }
      session.sparkContext.setJobDescription(substitutorCommand)
      val execution = session.sessionState.executePlan(session.sql(command).logicalPlan)
