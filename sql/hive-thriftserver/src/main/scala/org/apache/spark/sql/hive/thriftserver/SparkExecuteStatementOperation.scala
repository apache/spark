...
import org.apache.spark.sql.internal.{SharedState, SQLConf, VariableSubstitution}
...
private[hive] class SparkExecuteStatementOperation(
  ...
  private val timeout = {
    val globalTimeout = sessionState.conf.getConf(SQLConf.THRIFTSERVER_QUERY_TIMEOUT)
  ...
  private val forceCancel = sessionState.conf.getConf(SQLConf.THRIFTSERVER_FORCE_CANCEL)

  private val redactedStatement = {
    val substitutorStatement = SQLConf.withExistingConf(sessionState.conf) {
      new VariableSubstitution().substitute(statement)
    }
    SparkUtils.redact(sessionState.conf.stringRedactionPattern, substitutorStatement)
  }
  ...
  private def execute(): Unit = withClassLoader {
    try {
      synchronized {
        ...
        if (currentState.isTerminal) {
          ...
          e match {
            ...
            case _ => throw HiveThriftServerErrors.runningQueryError(
              e, sessionState.conf.errorMessageFormat)
          }
        }
    } finally {
      synchronized {
