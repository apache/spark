...
class SparkExecuteStatementOperationSuite extends SparkFunSuite with SharedSparkSession {
  ...
  test("Test fetchFirst, fetchNext, fetchPrior with IteratorWithFetch") {
    val testData = 0 until 10

    def iteratorTest(iterWithFetch: IteratorWithFetch[Int]): Unit = {
      def assertNext(expected: Seq[Int]): Unit = {
        expected.foreach(n => assertResult(n)(iterWithFetch.next()))
      }

      iterWithFetch.fetchNext()
      assert(iterWithFetch.getPosition == 0)
      assertNext(0 until 3)

      iterWithFetch.fetchNext()
      assert(iterWithFetch.getPosition == 3)
      assertNext(3 until 6)

      iterWithFetch.fetchPrior(2)
      assert(iterWithFetch.getPosition == 1)
      assertNext(1 until 3)

      iterWithFetch.fetchNext()
      assert(iterWithFetch.getPosition == 3)
      assertNext(3 until 6)

      iterWithFetch.fetchPrior(10)
      assert(iterWithFetch.getPosition == 0)
      assertNext(0 until 3)

      iterWithFetch.fetchNext()
      assert(iterWithFetch.getPosition == 3)
      assertNext(3 until 10)

      iterWithFetch.fetchNext()
      assert(iterWithFetch.getPosition == 10)
      assertNext(Seq.empty[Int])

      iterWithFetch.fetchPrior(3)
      assert(iterWithFetch.getPosition == 7)
      assertNext(7 until 10)

      iterWithFetch.fetchFirst()
      assert(iterWithFetch.getPosition == 0)
      assertNext(0 until 3)
      ...
      iterWithFetch.fetchPrior(20)
      assert(iterWithFetch.getPosition == 0)
      assertNext(0 until 1)

      iterWithFetch.fetchPrior(-3)
      assert(iterWithFetch.getPosition == 3)
      assertNext(3 until 10)
    }
    iteratorTest(new ArrayIteratorWithFetch[Int](testData.toArray))
    iteratorTest(new IterableIteratorWithFetch[Int](testData))
  }
