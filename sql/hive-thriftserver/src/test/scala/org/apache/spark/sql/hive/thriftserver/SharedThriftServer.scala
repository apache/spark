...
trait SharedThriftServer extends SharedSparkSession {
  ...
  private def startThriftServer(attempt: Int): Unit = {
    ...
    session.conf.set(ConfVars.HIVE_SERVER2_THRIFT_PORT.varname, "0")
    session.conf.set(ConfVars.HIVE_SERVER2_THRIFT_HTTP_PORT.varname, "0")
    session.conf.set(ConfVars.HIVE_SERVER2_TRANSPORT_MODE.varname, mode.toString)
    session.conf.set(ConfVars.SCRATCHDIR.varname, tempScratchDir.getAbsolutePath)
    session.conf.set(ConfVars.HIVE_START_CLEANUP_SCRATCHDIR.varname, "true")

    try {
      hiveServer2 = HiveThriftServer2.startWithSparkSession(session)
