...
class LiteralExpressionProtoConverterSuite extends AnyFunSuite { // scalastyle:ignore funsuite
  ...
  test("data types of struct fields are not set for inferrable types") {
    val literalProto = LiteralValueProtoConverter.toLiteralProto(
      (1, 2, Array("1", "2", "3"), Map(1 -> 2, 3 -> 4), (1, 2)),
      StructType(
        Seq(
          StructField("a", IntegerType),
          StructField("b", BooleanType),
          StructField("c", ArrayType(StringType)),
          StructField("d", MapType(IntegerType, StringType)),
          StructField(
            "e",
            StructType(Seq(StructField("f", IntegerType), StructField("g", IntegerType)))))))
    assert(!literalProto.getStruct.getFieldsList.get(0).hasDataType)
    assert(!literalProto.getStruct.getFieldsList.get(1).hasDataType)
    assert(!literalProto.getStruct.getFieldsList.get(2).hasDataType)
    assert(!literalProto.getStruct.getFieldsList.get(3).hasDataType)
    assert(!literalProto.getStruct.getFieldsList.get(4).hasDataType)
  }

  test("data types of struct fields are set for non-inferrable types") {
    val literalProto = LiteralValueProtoConverter.toLiteralProto(
      ("string", Decimal(1)),
  ...
  test("nullable and metadata fields are set for struct literal proto") {
    ...
    val structFields = literalProto.getStruct.getFieldsList
