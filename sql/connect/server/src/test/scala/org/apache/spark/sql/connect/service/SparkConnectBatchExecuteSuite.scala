...
class SparkConnectBatchExecuteSuite extends SparkConnectServerTest {
  ...
  test("batch execute with multiple sequences") {
    withClient { client =>
  ...
  test("batch execute with custom sequence operation IDs") {
    withClient { client =>
  ...
  test("batch execute with custom query operation IDs") {
    withClient { client =>
  ...
  test("batch execute with invalid sequence operation ID format") {
    withClient { client =>
      val seq1 = Seq((createPlan("SELECT 1"), None))

      // Client-side validation should throw IllegalArgumentException
      val ex = intercept[IllegalArgumentException] {
        client.batchExecute(Seq(seq1), Seq(Some("invalid-uuid")))
      }
      assert(ex.getMessage.contains("Invalid sequence operation ID"))
    }
  }

  test("batch execute with duplicate sequence operation ID") {
    withClient { client =>
  ...
  test("batch execute with failing query stops sequence") {
    withClient { client =>
      val seq1 = Seq(
  ...
  test("batch execute with empty sequence list") {
    withClient { client =>
  ...
  test("batch execute with single query in sequence") {
    withClient { client =>
  ...
  test("batch execute with multiple queries in single sequence") {
    withClient { client =>
      val queries = (1 to 5).map(i => (createPlan(s"SELECT $i"), None))

      val response = client.batchExecute(Seq(queries))
  ...
  test("batch execute with mixed success and failure sequences") {
    withClient { client =>
      ...
      val response = client.batchExecute(Seq(seq1, seq2, seq3))
