      ...
      if (arg.hasParam) {
        ...
        literal.getLiteralTypeCase match {
          ...
          case proto.Expression.Literal.LiteralTypeCase.ARRAY =>
            val (catalystArray, arrayType) =
              LiteralValueProtoConverter.toCatalystArray(literal.getArray)
            arrayType.getElementType.getKindCase match {
              case proto.DataType.KindCase.DOUBLE =>
                (MLUtils.reconcileArray(classOf[Double], catalystArray), classOf[Array[Double]])
              case proto.DataType.KindCase.STRING =>
                (MLUtils.reconcileArray(classOf[String], catalystArray), classOf[Array[String]])
              case proto.DataType.KindCase.ARRAY =>
                arrayType.getElementType.getArray.getElementType.getKindCase match {
                  case proto.DataType.KindCase.STRING =>
                    (
                      MLUtils.reconcileArray(classOf[Array[String]], catalystArray),
