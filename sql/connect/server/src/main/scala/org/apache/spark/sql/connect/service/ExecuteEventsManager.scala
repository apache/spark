...
case class ExecuteEventsManager(executeHolder: ExecuteHolder, clock: Clock) {
   ...
   *
   * @param optimizedPlan
   *   The optimized plan generated by the Connect request plan. None when the request does not
   *   generate a plan.
   */
  def postReadyForExecution(optimizedPlan: Option[LogicalPlan] = None): Unit = {
    ...
    event.optimizedPlan = optimizedPlan
    listenerBus.post(event)
  }
  ...
  def createQueryPlanningTracker(): QueryPlanningTracker = {
    new QueryPlanningTracker(Some(new QueryPlanningTrackerCallback {
      ...
      def readyForExecution(tracker: QueryPlanningTracker, optimizePlan: LogicalPlan): Unit = {
        postReadyForExecution(Some(optimizePlan))
      }
    }))
  }
...
case class SparkListenerConnectOperationReadyForExecution(
  ...
  /**
   * Optimized Spark plan generated by the Connect request. None when the Connect request does not
   * generate a Spark plan.
   */
  @JsonIgnore var optimizedPlan: Option[LogicalPlan] = None
}
