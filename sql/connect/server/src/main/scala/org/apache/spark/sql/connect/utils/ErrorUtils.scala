    ...
    while (buffer.size < MAX_ERROR_CHAIN_LENGTH && currentError != null) {
      ...
      currentError match {
        case sparkThrowable: SparkThrowable =>
          ...
          if (sparkThrowable.getBreakingChangeInfo.isPresent) {
            val breakingChangeInfo = sparkThrowable.getBreakingChangeInfo.get()
            ...
            if (breakingChangeInfo.mitigationSparkConfig.isDefined) {
              ...
              breakingChangeInfoBuilder.setMitigationSparkConfig(mitigationConfigBuilder.build())
            }
