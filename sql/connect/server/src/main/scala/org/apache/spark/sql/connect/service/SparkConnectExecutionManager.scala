...
import org.apache.spark.sql.connect.config.Connect.{
  CONNECT_EXECUTE_MANAGER_ABANDONED_TOMBSTONES_SIZE,
  CONNECT_EXECUTE_MANAGER_DETACHED_TIMEOUT,
  CONNECT_EXECUTE_MANAGER_MAINTENANCE_INTERVAL,
  CONNECT_EXECUTE_MAX_CONCURRENT_QUERIES,
  CONNECT_EXECUTE_MAX_CONCURRENT_QUERIES_TIMEOUT}
...
private[connect] class SparkConnectExecutionManager() extends Logging {
  ...
  @volatile private var acceptingExecutions = true
   ...
   * FIFO order.
   */
  private val executionSemaphore: Option[Semaphore] = if (maxConcurrentQueries > 0) {
    Some(new Semaphore(maxConcurrentQueries, true))
  } else {
    None
  }
  ...
  private[connect] def getAvailableExecutionSlots: Int = {
    executionSemaphore.map(_.availablePermits()).getOrElse(Int.MaxValue)
  }
  ...
  private[connect] def getExecutionQueueLength: Int = {
    executionSemaphore.map(_.getQueueLength).getOrElse(0)
  }
   ...
   */
  private[connect] def acquireExecutionSlot(): Option[ExecutionPermit] = {
    if (!acceptingExecutions) {
      throw serviceUnavailableException()
    }

    executionSemaphore.map { semaphore =>
      val availableBefore = semaphore.availablePermits()
      if (availableBefore == 0) {
        val queueLength = semaphore.getQueueLength
        logInfo(
  ...
  private[connect] def removeExecuteHolder(key: ExecuteKey, abandoned: Boolean = false): Unit = {
    val removedHolder = new AtomicReference[ExecuteHolder]()
    executions.computeIfPresent(
      ...
      (_, executeHolder) => {
        ...
        removedHolder.set(executeHolder)
        null
      })

    val executeHolder = removedHolder.get()
    ...
    val executeHolder =
      try {
        synchronized {
          ...
          createExecuteHolder(executeKey, request, sessionHolder, executionPermit)
        }
      } catch {
        case t: Throwable =>
