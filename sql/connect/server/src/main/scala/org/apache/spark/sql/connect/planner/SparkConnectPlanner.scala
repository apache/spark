...
import org.apache.spark.sql.connect.common.{DataTypeProtoConverter, ForeachWriterPacket, FromProtoToScalaConverter, InvalidPlanInput, StorageLevelProtoConverter, StreamingListenerPacket, UdfPacket}
...
class SparkConnectPlanner(
  ...
  private def transformNAFill(rel: proto.NAFill): LogicalPlan = {
    ...
    if (values.length == 1) {
      val value = FromProtoToScalaConverter.convertToValue(values.head)
      ...
      cols.zip(values).foreach { case (col, value) =>
        valueMap.update(col, FromProtoToScalaConverter.convertToValue(value))
      }
  ...
  private def transformReplace(rel: proto.NAReplace): LogicalPlan = {
    ...
    rel.getReplacementsList.asScala.foreach { replace =>
      replacement.update(
        FromProtoToScalaConverter.convertToValue(replace.getOldValue),
        FromProtoToScalaConverter.convertToValue(replace.getNewValue))
    }
