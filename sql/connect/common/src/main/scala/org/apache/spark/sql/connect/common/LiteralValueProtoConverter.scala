    ...
    literal.getLiteralTypeCase match {
      ...
      case proto.Expression.Literal.LiteralTypeCase.ARRAY =>
        if (literal.getArray.hasDataTypeArray) {
          // The new way to define the type of the array.
      ...
      case proto.Expression.Literal.LiteralTypeCase.MAP =>
        if (literal.getMap.hasDataTypeMap) {
          // The new way to define the type of the map.
          builder.setMap(literal.getMap.getDataTypeMap)
        } else if (literal.getMap.hasKeyType && literal.getMap.hasValueType) {
          // For backward compatibility, we still support the old way to define the type of the map.
          builder.setMap(
            proto.DataType.Map
              .newBuilder()
