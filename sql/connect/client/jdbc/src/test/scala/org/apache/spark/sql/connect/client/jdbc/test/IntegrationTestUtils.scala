...
object IntegrationTestUtils {
  ...
  private val DEBUG_SC_JVM_CLIENT = "spark.debug.sc.jdbc"
  private val DEBUG_SC_JVM_CLIENT_ENV = "SPARK_DEBUG_SC_JDBC"
  ...
  private[sql] lazy val connectJdbcHomeDir = s"$sparkHome/sql/connect/client/jdbc"

  private[sql] lazy val connectJdbcTestClassDir = {
    s"$connectJdbcHomeDir/target/$scalaDir/test-classes"
  }

  private[sql] def debugConfigs: Seq[String] = {
    val log4j2 = s"$connectJdbcHomeDir/src/test/resources/log4j2.properties"
    if (isDebug) {
      Seq(
