...
import org.apache.spark.sql.connect.client.jdbc.test.IntegrationTestUtils._
 ...
 * `build/sbt package`. It is designed to start the server once but shared by all tests.
 *
 * Set system property `spark.test.home` or env variable `SPARK_HOME` if the test is not executed
 * from the Spark project top folder. Set system property `spark.debug.sc.jdbc=true` or
 * environment variable `SPARK_DEBUG_SC_JDBC=true` to print the server process output in the
...
object SparkConnectServerUtils {
  ...
  def syncTestDependencies(spark: SparkSession): Unit = {
    ...
    val testClassesPath = Paths.get(IntegrationTestUtils.connectJdbcTestClassDir)
