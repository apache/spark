...
abstract class BaseResultSet extends ResultSet {

  protected var currentRow: Row = _

  protected var _wasNull: Boolean = false
  ...
  override def findColumn(columnLabel: String): Int = {
    currentRow.schema.getFieldIndex(columnLabel) match {
      case Some(i) => i + 1
      case None => throw new SQLException(s"Invalid column label: $columnLabel")
    }
  }
  ...
  override def rowUpdated(): Boolean = throw new SQLFeatureNotSupportedException

  override def rowInserted(): Boolean = throw new SQLFeatureNotSupportedException

  override def rowDeleted(): Boolean = throw new SQLFeatureNotSupportedException

  override def updateNull(columnIndex: Int): Unit = throw new SQLFeatureNotSupportedException
