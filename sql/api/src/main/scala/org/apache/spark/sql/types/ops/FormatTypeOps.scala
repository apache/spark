 ...
 *
 * PURPOSE: Handles string formatting for display and SQL output. This includes formatting values
 * for CAST to STRING, EXPLAIN output, SHOW commands, and SQL literals.
 *
...
trait FormatTypeOps extends TypeApiOps {
   ...
   *
   * This method converts the internal Catalyst representation to a human-readable string suitable
   * for display or CAST to STRING operations.
   *
   * @param v
   ...
   * @example
   *   37800000000000L -> "10:30:00" (for TIME)
   */
   ...
   *
   * Convenience method that wraps format() for use in expressions that need UTF8String output
   * directly.
   *
   * @param v
   *   The internal value
   * @return
   *   UTF8String representation
   */
   ...
   *
   * This method produces a string that can be used in SQL statements, including the type prefix
   * if appropriate (e.g., "TIME '10:30:00'").
   *
   * @param v
   *   The internal value
   * @return
   ...
   * @example
   *   37800000000000L -> "TIME '10:30:00'" (for TIME)
   */
...
object FormatTypeOps {
   ...
   *
   * @param dt
   *   The DataType to get formatting operations for
   * @return
   *   FormatTypeOps instance
   * @throws SparkException
   *   if the type doesn't support FormatTypeOps
   */
   ...
   * @return
   *   true if the type supports FormatTypeOps
   */
