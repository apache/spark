...
trait SparkDateTimeUtils {
  ...
  def getZoneId(timeZoneId: String): ZoneId = {
    try {
      ZoneId.of(normalizeLegacyZoneOffset(timeZoneId), ZoneId.SHORT_IDS)
    } catch {
  ...
  private def normalizeLegacyZoneOffset(zoneId: String): String = {
    ...
    // Pad a single-digit trailing minute, e.g. "+07:3" -> "+07:03".
    val n = afterHour.length
    if (n >= 5) {
