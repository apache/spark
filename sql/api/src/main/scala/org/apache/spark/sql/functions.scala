...
// scalastyle:on
  ...
  /**
   * Aggregate function: returns the binary representation of a Datasketches ItemsSketch tracking
   * approximate frequency of items. The `maxMapSize` parameter controls sketch accuracy and must
   * be a power of 2.
   *
   * @group agg_funcs
   * @since 4.2.0
   */
  def items_sketch_agg(e: Column, maxMapSize: Column): Column =
    Column.fn("items_sketch_agg", e, maxMapSize)
   ...
   * @since 4.2.0
   */
  def items_sketch_agg(e: Column, maxMapSize: Int): Column =
    Column.fn("items_sketch_agg", e, lit(maxMapSize))
   ...
   * @since 4.2.0
   */
  def items_sketch_agg(columnName: String, maxMapSize: Int): Column =
    items_sketch_agg(Column(columnName), maxMapSize)

  /**
   * Aggregate function: returns the binary representation of a Datasketches ItemsSketch tracking
   * approximate frequency of items, using the default maxMapSize of 4096.
   *
   * @group agg_funcs
   * @since 4.2.0
   */
  def items_sketch_agg(e: Column): Column =
    Column.fn("items_sketch_agg", e)
   ...
   * @since 4.2.0
   */
  def items_sketch_agg(columnName: String): Column =
    items_sketch_agg(Column(columnName))

  /**
   * Aggregate function: merges ItemsSketch binary representations produced by `items_sketch_agg`
   * into a single ItemsSketch. Allows configuration of `maxMapSize` controlling the accuracy of
   * the merged sketch.
   *
   * @group agg_funcs
   * @since 4.2.0
   */
  def items_sketch_merge_agg(e: Column, maxMapSize: Column): Column =
    Column.fn("items_sketch_merge_agg", e, maxMapSize)

  /**
   * Aggregate function: merges ItemsSketch binary representations produced by `items_sketch_agg`
   * into a single ItemsSketch with the specified `maxMapSize`.
   *
   * @group agg_funcs
   * @since 4.2.0
   */
  def items_sketch_merge_agg(e: Column, maxMapSize: Int): Column =
    Column.fn("items_sketch_merge_agg", e, lit(maxMapSize))
   ...
   * @since 4.2.0
   */
  def items_sketch_merge_agg(columnName: String, maxMapSize: Int): Column =
    items_sketch_merge_agg(Column(columnName), maxMapSize)

  /**
   * Aggregate function: merges ItemsSketch binary representations produced by `items_sketch_agg`
   * into a single ItemsSketch using the default maxMapSize (4096).
   *
   * @group agg_funcs
   * @since 4.2.0
   */
  def items_sketch_merge_agg(e: Column): Column =
    Column.fn("items_sketch_merge_agg", e)
   ...
   * @since 4.2.0
   */
  def items_sketch_merge_agg(columnName: String): Column =
    items_sketch_merge_agg(Column(columnName))
  ...
  /**
   * Returns the frequent items from an ItemsSketch binary representation as an array of structs.
   * `errorType` must be 'NO_FALSE_POSITIVES' or 'NO_FALSE_NEGATIVES'.
   *
   * @group sketch_funcs
   * @since 4.2.0
   */
  def items_sketch_get_frequent_items(sketch: Column, errorType: Column): Column =
    Column.fn("items_sketch_get_frequent_items", sketch, errorType)
   ...
   * @since 4.2.0
   */
  def items_sketch_get_frequent_items(sketch: Column, errorType: String): Column =
    items_sketch_get_frequent_items(sketch, lit(errorType))
   ...
   * @since 4.2.0
   */
  def items_sketch_get_frequent_items(columnName: String, errorType: String): Column =
    items_sketch_get_frequent_items(Column(columnName), lit(errorType))

  /**
   * Returns the estimated frequency of a specific item from an ItemsSketch binary representation.
   *
   * @group sketch_funcs
   * @since 4.2.0
   */
  def items_sketch_get_estimate(sketch: Column, item: Column): Column =
    Column.fn("items_sketch_get_estimate", sketch, item)
   ...
   * @since 4.2.0
   */
  def items_sketch_get_estimate(sketch: Column, item: String): Column =
    items_sketch_get_estimate(sketch, lit(item))
   ...
   * @since 4.2.0
   */
  def items_sketch_merge(first: Column, second: Column): Column =
    Column.fn("items_sketch_merge", first, second)
   ...
   * @since 4.2.0
   */
  def items_sketch_to_string(sketch: Column): Column =
    Column.fn("items_sketch_to_string", sketch)
   ...
   * @since 4.2.0
   */
  def items_sketch_to_string(columnName: String): Column =
    items_sketch_to_string(Column(columnName))
