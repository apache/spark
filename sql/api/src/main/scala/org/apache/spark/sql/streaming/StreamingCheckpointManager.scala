...
private[spark] abstract class StreamingCheckpointManager {
   ...
   * `numPartitions` partitions. The streaming query MUST not be running.
   * If `numPartitions` is the same as the current number of partitions, this is a no-op,
   ...
   *
   * @note This operation should only be performed after the streaming query has been stopped.
   * @param checkpointLocation The checkpoint location of the streaming query, should be the
   *                           `checkpointLocation` option on the DataStreamWriter.
   * @param numPartitions the target number of state partitions.
