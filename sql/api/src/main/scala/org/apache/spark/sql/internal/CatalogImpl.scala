...
/**
 * Single implementation of [[Catalog]] for both Classic and Connect. Uses SQL DDL only (SHOW
 * TABLES, DESCRIBE TABLE, etc.) so behavior is identical everywhere.
 *
 * @param session
 *   Spark session that also provides CatalogSupport (parse/quote, current DB/catalog, refresh,
 *   etc.)
 */
private[sql] class CatalogImpl(session: SparkSession with CatalogSupport) extends Catalog {

  private def sql(str: String): DataFrame = session.sql(str)
  private def parseIdent(name: String): Seq[String] =
    session.parseMultipartIdentifier(name)
  private def quote(name: String): String = session.quoteIdentifier(name)
  ...
  override def currentDatabase: String = session.currentDatabase
  ...
  private def listDatabasesInternal(patternOpt: Option[String]): Dataset[Database] = {
    val catalogName = session.currentCatalog()
    ...
    val databases = rows.map { row =>
      ...
      new Database(name = name, catalog = catalogName, description = null, locationUri = null)
    }
    makeDataset(databases.toSeq)
  }

  override def listTables(): Dataset[Table] = listTables(session.currentDatabase)
  ...
  private def listTablesInternal(dbName: String, pattern: Option[String]): Dataset[Table] = {
    ...
    val catalogName =
      if (dbNameParts.length > 1) dbNameParts.head
      else session.currentCatalog()
    ...
    val currentNs = session.currentDatabase
    ...
    val tables = rows.map { row =>
      if (row.size < 3) {
        ...
        val nsInCatalog =
          if (isTempView) Array.empty[String]
          else {
  ...
  override def listFunctions(): Dataset[Function] = listFunctions(session.currentDatabase)
  ...
  private def listFunctionsInternal(
    ...
    val functions = rows.map { row =>
      ...
      val parts =
        try parseIdent(funcName)
        catch { case NonFatal(_) => Seq(funcName) }
      val name = if (parts.nonEmpty) parts.last else funcName
      val (namespace, isTemp, describeRows) =
        resolveFunctionNamespaceWithDescribe(qualifiedNs, name, nsArray)
  ...
  private def resolveFunctionNamespace(
    ...
    val (namespace, isTemp, _) =
      resolveFunctionNamespaceWithDescribe(qualifiedNs, funcName, nsArray)
    (namespace, isTemp)
  }
    ...
    val rows =
      try {
        ...
        case _: AssertionError | _: Exception =>
          ...
          val cols = simple
            .takeWhile(r => !r.getString(0).startsWith("#"))
  ...
  override def getDatabase(dbName: String): Database = {
    ...
    val identsToDescribe = if (idents.length == 1) {
      ...
      else (session.currentCatalog() +: idents).toSeq
    } else {
      idents
    }
  ...
  override def getTable(tableName: String): Table = {
    ...
    // Global temp views: database is the global temp DB (default "global_temp"); treat as temp so
    // catalog is null. Session temp views have "Is Temporary" = "true" in DESCRIBE output.
    val isGlobalTempDb =
      nameParts.length >= 2 && nameParts.head.equalsIgnoreCase("global_temp")
    val isTemp = info.get("Is Temporary").exists(_.equalsIgnoreCase("true")) || isGlobalTempDb
    val tableType = if (isTemp) "TEMPORARY" else info.getOrElse("Type", "TABLE")
    val ns =
      if (nameParts.length > 2) nameParts.drop(1).dropRight(1).toArray
      else if (nameParts.length == 1) {
        val defaultDb = session.currentDatabase
        val db = info.getOrElse("Database", defaultDb)
        Array(db)
      } else nameParts.dropRight(1).toArray
    val catalogName =
      if (isTemp) null
      else if (nameParts.length > 2) nameParts.head
      else session.currentCatalog()
    new Table(
      name = nameParts.last,
      catalog = catalogName,
  ...
  override def getFunction(functionName: String): Function = {
    ...
    try {
      ...
      val (namespace, isTemp) = if (nameParts.length > 1) {
        ...
        val qualified = s"${session.currentCatalog()}.${session.currentDatabase}" +
          s".${quote(nameParts.last)}"
        try {
          sql(s"DESCRIBE FUNCTION EXTENDED $qualified").collect()
          (Array(session.currentDatabase), false)
        } catch {
          case _: Exception => (null, true)
        }
      }
      val catalogName =
        if (nameParts.length > 2) nameParts.head
        else session.currentCatalog()
      ...
      case ae: AnalysisException if nameParts.length >= 3 =>
        ...
        session.getFunctionMetadata(catalogName, namespace, nameParts.last) match {
    ...
    rows.foreach { row =>
      if (row.size > 0) {
        ...
        if (line.startsWith("Class:")) {
          ...
          val value = line.stripPrefix("Usage:").trim
        ...
        } else if (line.startsWith("Extended Usage:") && description == "N/A.") {
          val value = line.stripPrefix("Extended Usage:").trim
          if (value.nonEmpty && !value.matches("No example/argument for .+")) {
            description = value
          }
        }
  ...
  override def tableExists(tableName: String): Boolean = {
    ...
    try {
      val quotedNs =
        if (nsParts.isEmpty) session.currentDatabase
  ...
  override def dropTempView(viewName: String): Boolean = {
    // Only drop if a temp view exists; do not resolve name to a metastore table.
    session.dropTempViewIfExists(session, viewName)
  }
  ...
  override def isCached(tableName: String): Boolean =
    session.isTableCached(session, tableName)
  ...
  override def cacheTable(tableName: String): Unit =
    sql(s"CACHE LAZY TABLE ${quoted(parseIdent(tableName))}")
  ...
  override def cacheTable(tableName: String, storageLevel: StorageLevel): Unit = {
    ...
    val sqlStr = if (levelStr.nonEmpty) {
      s"CACHE LAZY TABLE $quotedName OPTIONS (storageLevel '$levelStr')"
    } else {
      s"CACHE LAZY TABLE $quotedName"
    }
    sql(sqlStr)
  }
  ...
  override def refreshTable(tableName: String): Unit =
    session.refreshTable(session, tableName)

  override def refreshByPath(resourcePath: String): Unit =
    session.refreshByPath(session, resourcePath)

  override def currentCatalog(): String = session.currentCatalog()
