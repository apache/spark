 ...
 * current reader. Within a [[StructField]], schema JSON stores the collation in field metadata and
 * emits an uncollated `char(n)` type so preceding readers can still read the schema.
 *
 * @param length
