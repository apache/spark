 ...
 *
 * Instances of `ManagedMemoryRow` act as pointers to row data stored in this format.
 */
public final class ManagedMemoryRow implements MutableRow {
   ...
   * this ManagedMemoryRow, but is optional if callers will only use type-specific getTYPE() and
   * setTYPE() methods. This should be removed after the planned InternalRow / Row split; right now,
   * it's only needed by the generic get() method, which is only called internally by code that
   * accesses UTF8String-typed columns.
   */
  @Nullable
  ...
  /**
   * Field types that can be updated in place in ManagedMemoryRows (e.g. we support set() for these
   * types)
   */
  ...
  /**
   * Construct a new ManagedMemoryRow. The resulting row won't be usable until `pointTo()` has been
   * called, since the value returned by this constructor is equivalent to a null pointer.
   */
  public ManagedMemoryRow() { }

  /**
   * Update this ManagedMemoryRow to point to different backing data.
   *
  ...
  public Object get(int i) {
    ...
    // ManagedMemoryRow is only designed to be invoked by internal code, which only invokes this
    // generic get() method when trying to access UTF8String-typed columns. If we refactor the
    // codebase to separate the internal and external row interfaces, then internal code can fetch
    // strings via a new getUTF8String() method and we'll be able to remove this method.
    if (isNullAt(i)) {
      return null;
