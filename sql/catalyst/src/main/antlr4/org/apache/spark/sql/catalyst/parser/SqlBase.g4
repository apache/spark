...
nonReserved
    ...
    | BUCKET | BUCKETS | BY | CACHE | CASCADE | CASE | CAST | CHANGE | CLEAR | CLUSTER | CLUSTERED | CODEGEN
    | COLLECTION | COLUMN | COLUMNS | COMMENT | COMMIT | COMPACT | COMPACTIONS | COMPUTE | CONCATENATE | COST
    | CREATE | CUBE | CURRENT | DATA | DATABASE | DATABASES | DBPROPERTIES | DEFINED | DELETE | DELIMITED
    | DESC | DESCRIBE | DFS | DIRECTORIES | DIRECTORY | DISTINCT | DISTRIBUTE | DIV | DROP | ELSE | END
    | ESCAPED | EXCHANGE | EXISTS | EXPLAIN | EXPORT | EXTENDED | EXTERNAL | EXTRACT | FALSE | FIELDS
    | FILEFORMAT | FIRST | FOLLOWING | FOR | FORMAT | FORMATTED | FROM | FUNCTION | FUNCTIONS | GLOBAL
    | GRANT | GROUP | GROUPING | HAVING | IF | IGNORE | IMPORT | IN | INDEX | INDEXES | INPATH | INPUTFORMAT
    | INSERT | INTERVAL | INTO | IS |LIKE | ITEMS | KEYS | LAST | LATERAL | LAZY | LEADING | LIMIT | LINES
    | LIST | LOAD | LOCAL | LOCATION | LOCK | LOCKS | LOGICAL | MACRO | MAP | MSCK | NO | NOT | NULL | NULLS
    | OF | OPTION | OPTIONS | OR | ORDER | OUT | OUTER | OUTPUTFORMAT | OVER | OVERWRITE | PARTITION
    | PARTITIONED | PARTITIONS | PERCENTLIT | PIVOT | POSITION | PRECEDING | PRINCIPALS | PURGE | RANGE
    | RECORDREADER | RECORDWRITER | RECOVER | REDUCE | REFRESH | RENAME | REPAIR | REPLACE | RESET | RESTRICT
    | REVOKE | RLIKE | ROLE | ROLES | ROLLBACK | ROLLUP | ROW | ROWS | SELECT | SEPARATED | SERDE
