...
case class JoinEstimation(join: Join) extends Logging {
  ...
  private def computeByNdv(
    ...
    val leftKeyStat = leftStats.attributeStats(leftKey)
    val rightKeyStat = rightStats.attributeStats(rightKey)
    val newNdv = leftKeyStat.distinctCount.get.min(rightKeyStat.distinctCount.get)
    // Compute cardinality by the basic formula.
    val card = BigDecimal(leftStats.rowCount.get * rightStats.rowCount.get) / BigDecimal(newNdv)
    ...
    val newStats = ColumnStat(Some(newNdv), min, max, Some(0), newAvgLen, newMaxLen)
