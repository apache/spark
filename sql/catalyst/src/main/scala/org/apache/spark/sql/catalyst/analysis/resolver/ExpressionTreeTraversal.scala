 ...
 * @param sessionLocalTimeZone
 *   The session local time zone.
 * @param defaultCollation
 *   View's default collation if explicitly set.
 * @param invalidExpressionsInTheContextOfOperator
 *   The expressions that are invalid in the context of the current expression tree and its parent
 *   operator.
 * @param referencedAttributes
 *   All attributes that are referenced during the resolution of expression trees.
 * @param isFilterOnTopOfAggregate
 *   Whether the current expression tree is below a [[Filter]] on top of an [[Aggregate]]
 *   operator.
 * @param isSortOnTopOfAggregate
 *   Whether the current expression tree is below a [[Sort]] on top of an [[Aggregate]] operator
 *   (or on top of a [[Filter]] with an [[Aggregate]] as its child).
 */
 ...
 * [[ExpressionResolver.resolveExpressionTreeInOperatorImpl]], and will usually have size 1.
 * However, in case of subquery expressions we would call
 * [[ExpressionResolver.resolveExpressionTreeInOperatorImpl]] several times recursively for each
 * expression tree in the operator tree -> expression tree -> operator tree -> expression tree ->
 * ... chain. Consider this example:
 *
 * {{{
 * SELECT
 *   col1
