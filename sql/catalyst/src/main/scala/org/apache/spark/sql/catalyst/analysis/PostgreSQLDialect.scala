...
import org.apache.spark.sql.catalyst.expressions.postgreSQL.{PostgreCastToBoolean, PostgreCastToShort}
import org.apache.spark.sql.catalyst.plans.logical.LogicalPlan
import org.apache.spark.sql.catalyst.rules.Rule
import org.apache.spark.sql.internal.SQLConf
import org.apache.spark.sql.types.{BooleanType, ShortType, StringType}

object PostgreSQLDialect {
  val postgreSQLDialectRules: List[Rule[LogicalPlan]] =
    CastToBoolean :: CastToShort ::
      Nil
  ...
  object CastToShort extends Rule[LogicalPlan] with Logging {
    override def apply(plan: LogicalPlan): LogicalPlan = {
      // The SQL configuration `spark.sql.dialect` can be changed in runtime.
      // To make sure the configuration is effective, we have to check it during rule execution.
      val conf = SQLConf.get
      if (conf.usePostgreSQLDialect) {
        plan.transformExpressions {
          case Cast(child, dataType, timeZoneId)
            if child.dataType != ShortType && dataType == ShortType =>
            PostgreCastToShort(child, timeZoneId)
        }
      } else {
        plan
      }
