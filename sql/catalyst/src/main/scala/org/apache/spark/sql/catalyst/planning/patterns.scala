...
import org.apache.spark.sql.execution.datasources.v2.{DataSourceV2Relation, DataSourceV2ScanRelation, ExtractTable}
...
object GroupBasedRowLevelOperation {
  ...
  def unapply(plan: LogicalPlan): Option[ReturnType] = plan match {
    case rd @ ReplaceData(ExtractTable(table), cond, query, _, _, groupFilterCond, _) =>
