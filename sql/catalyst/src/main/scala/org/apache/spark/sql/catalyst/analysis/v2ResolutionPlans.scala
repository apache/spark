 ...
 */
case class UnresolvedNamespace(multipartIdentifier: Seq[String], fetchMetadata: Boolean = false)
...
case class UnresolvedTable(
    ...
    commandName: String,
    suggestAlternative: Boolean = false)
    extends UnresolvedLeafNode
...
/**
 * Holds the name of a table or view that has yet to be looked up in a catalog. It will be
 * resolved to [[ResolvedTable]], [[ResolvedPersistentView]] or [[ResolvedTempView]] during
 * analysis.
 */
...
sealed trait PartitionSpec extends LeafExpression with Unevaluable {
  override def dataType: DataType =
    throw SparkException.internalError("PartitionSpec.dataType should not be called.")
  override def nullable: Boolean =
    throw SparkException.internalError("PartitionSpec.nullable should not be called.")
}

case class UnresolvedPartitionSpec(spec: TablePartitionSpec, location: Option[String] = None)
    extends PartitionSpec {
...
sealed trait FieldName extends LeafExpression with Unevaluable {
  def name: Seq[String]
  override def dataType: DataType =
    throw SparkException.internalError("FieldName.dataType should not be called.")
  override def nullable: Boolean =
    throw SparkException.internalError("FieldName.nullable should not be called.")
}
...
sealed trait FieldPosition extends LeafExpression with Unevaluable {
  ...
  override def dataType: DataType =
    throw SparkException.internalError("FieldPosition.dataType should not be called.")
  override def nullable: Boolean =
    throw SparkException.internalError("FieldPosition.nullable should not be called.")
}
...
/**
 * Holds the name of a table/view/function identifier that we need to determine the catalog. It
 * will be resolved to [[ResolvedIdentifier]] during analysis.
 */
...
object ResolvedTable {
  def create(catalog: TableCatalog, identifier: Identifier, table: Table): ResolvedTable = {
...
case class ResolvedProcedure(catalog: ProcedureCatalog, ident: Identifier, procedure: Procedure)
    extends LeafNodeWithoutStats {
  override def output: Seq[Attribute] = Nil
}
