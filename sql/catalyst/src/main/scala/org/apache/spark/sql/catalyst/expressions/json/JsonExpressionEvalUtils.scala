...
import com.fasterxml.jackson.core.{JsonEncoding, JsonGenerator, JsonParser, JsonProcessingException, JsonToken}

import org.apache.spark.SparkException
import org.apache.spark.sql.catalyst.InternalRow
import org.apache.spark.sql.catalyst.expressions.{Expression, ExprUtils, GenericInternalRow, SharedFactory}
...
case class JsonTupleEvaluator(
  ...
  final def evaluate(input: InternalRow): IterableOnce[InternalRow] = {
    evaluate(jsonExpr.eval(input).asInstanceOf[UTF8String], fields(input))
  }

  final def evaluate(json: UTF8String, fields: Array[String]): IterableOnce[InternalRow] = {
  ...
  private def parseRow(parser: JsonParser, fields: Array[String]): Seq[InternalRow] = {
    // only objects are supported
    if (parser.nextToken() != JsonToken.START_OBJECT) return nullRow
    val row = Array.ofDim[Any](fields.length)
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      if (parser.getCurrentToken == JsonToken.FIELD_NAME) {
        ...
        var idx = fields.indexOf(jsonField)
        if (idx >= 0) {
          ...
          if (parser.nextToken() != JsonToken.VALUE_NULL) {
            ...
            do {
              row(idx) = jsonValue
              idx = fields.indexOf(jsonField, idx + 1)
            } while (idx >= 0)
          }
        }
      }
