...
class NameScope extends SQLConfHelper {
  ...
  private def update(attributes: Seq[Attribute], name: Option[String] = None): Unit = {
    validateAttributesType(attributes)
  ...
  private def validateAttributesType(attributes: Seq[Attribute]): Unit = {
    for (attribute <- attributes) {
      validateAttributeType(attribute)
    }
  }

  private def validateAttributeType(attribute: Attribute): Unit = {
    if (ExplicitlyUnsupportedResolverFeature.isUnsupportedType(attribute.dataType)) {
