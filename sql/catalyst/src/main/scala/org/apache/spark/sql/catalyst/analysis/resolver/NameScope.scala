...
class NameScope(
   ...
   * multipart attribute names, by metadata columns from hidden output. It's created from the
   * `hiddenOutput` when [[NameScope]] is updated.
   */
  private lazy val metadataAttributesForResolution: AttributeSeq =
    AttributeSeq.fromNormalOutput(hiddenOutput.filter(_.isMetadataCol))
...
class NameScopeStack extends SQLConfHelper {
   ...
   * `hiddenOutput` is not provided, preserve the previous `hiddenOutput`.
   *
  ...
  def overwriteCurrent(
    ...
    val newScope = stack.pop.overwriteOutput(output, hiddenOutput)

    stack.push(newScope)
  }

  /**
   * Overwrites output of the current [[NameScope]] entry and extends hidden output with the
   * provided output (only attributes that are not in the hidden output are added). This is done
   * because resolution of arguments can be done through certain operators by hidden output. This
   * use case is specific to Dataframe programs. Example:
   *
   * {{{
   ...
   * output we have to have both hidden output from the previous scope and the provided output. This
   * is done for [[Project]] and [[Aggregate]] operators.
   */
  def overwriteOutputAndExtendHiddenOutput(output: Seq[Attribute]): Unit = {
    val prevScope = stack.pop
    val hiddenOutput = prevScope.hiddenOutput ++ output.filter { attribute =>
