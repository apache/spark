  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformWithPruning(
    ...
    case j @ Join(left, right, joinType, Some(joinCondition), hint)
      ...
      if (alreadyProcessed || (leftExtraCondition.isEmpty && rightExtraCondition.isEmpty)) {
        ...
        lazy val newLeft =
          leftExtraCondition.reduceLeftOption(And.apply).map(Filter(_, left)).getOrElse(left)
        lazy val newRight =
          rightExtraCondition.reduceLeftOption(And.apply).map(Filter(_, right)).getOrElse(right)
