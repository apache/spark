...
abstract class TypeCoercionBase {
  ...
  object UnpivotCoercion extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
      case up: Unpivot
        if up.values.isDefined && up.values.get.forall(_.resolved) && !up.valuesTypeCoercioned =>
        val valueDataType = findWiderTypeWithoutStringPromotion(up.values.get.map(_.dataType))
        val values = valueDataType.map(valueType =>
          up.values.map(_.map(value => Alias(Cast(value, valueType), value.name)()))
        ).getOrElse(up.values)
