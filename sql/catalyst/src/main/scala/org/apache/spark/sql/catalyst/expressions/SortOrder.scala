...
case class SortPrefix(child: SortOrder) extends UnaryExpression {
  ...
  val nullValue = child.child.dataType match {
    case BooleanType | DateType | TimestampType | TimestampNTZType | TimeType |
  ...
  private lazy val calcPrefix: Any => Long = child.child.dataType match {
    ...
    case DateType | TimestampType | TimestampNTZType | TimeType |
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val prefixCode = child.child.dataType match {
      ...
      case _: IntegralType =>
        s"(long) $input"
      case DateType | TimestampType | TimestampNTZType | TimeType | _: AnsiIntervalType =>
        s"(long) $input"
