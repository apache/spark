...
import org.apache.spark.sql.catalyst.expressions.{AggregateWindowFunction, CurrentRow, FrameLessOffsetWindowFunction, RangeFrame, RankLike, RowFrame, SpecifiedWindowFrame, UnboundedFollowing, UnboundedPreceding, UnspecifiedFrame, WindowExpression, WindowFunction, WindowSpecDefinition}
import org.apache.spark.sql.catalyst.expressions.aggregate.{AggregateExpression, ListAgg, Median, PercentileCont, PercentileDisc}
...
object WindowResolution {
   ...
   *
   * It enforces that the frame in [[WindowExpression.windowFunction]] matches the frame in
   * [[WindowExpression.windowSpec]], alterantively it provides a default frame when it is
   * unspecified.
   */
  def resolveFrame(windowExpression: WindowExpression): WindowExpression =
   ...
   *
   * In case of [[RankLike]] window functions, it attaches the resolved order to the function to
   * finalize it.
   */
  def resolveOrder(windowExpression: WindowExpression): WindowExpression =
    windowExpression match {
