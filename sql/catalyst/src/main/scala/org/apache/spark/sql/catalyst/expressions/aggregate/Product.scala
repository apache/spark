...
/**
 * Product in Pandas' fashion. This expression is dedicated only for Pandas API on Spark. It has
 * three main differences from `Product`: 1, it compute the product of `Fractional` inputs in a
 * more numerical-stable way; 2, it compute the product of `Integral` inputs with LongType
 * variables internally; 3, it accepts NULLs when `ignoreNA` is False;
 */
case class PandasProduct(child: Expression, ignoreNA: Boolean)
    extends DeclarativeAggregate
    with ImplicitCastInputTypes
    with UnaryLike[Expression] {
  ...
  override lazy val updateExpressions: Seq[Expression] = child.dataType match {
    case _: IntegralType =>
      Seq(If(IsNull(child), product, product * child), containsNull || IsNull(child))
    case _ =>
  ...
  override lazy val mergeExpressions: Seq[Expression] = child.dataType match {
    case _: IntegralType =>
      Seq(product.left * product.right, containsNull.left || containsNull.right)
    case _ =>
      Seq(
  ...
  override lazy val evaluateExpression: Expression = child.dataType match {
    ...
    case _ =>
      ...
      if (ignoreNA) {
        ...
        If(containsNull, Literal(null, DoubleType), If(containsZero, Literal(0.0), product))
      }
  }
