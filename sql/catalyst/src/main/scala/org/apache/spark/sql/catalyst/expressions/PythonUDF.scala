...
case class PythonUDF(
    ...
    extends Expression
    with PythonFuncExpression
    with Unevaluable {

  lazy val resultAttribute: Attribute =
    AttributeReference(toPrettySQL(this), dataType, nullable)(exprId = resultId)
...
case class PythonUDAF(
  ...
  override protected def withNewChildrenInternal(
      newChildren: IndexedSeq[Expression]): PythonUDAF =
    copy(children = newChildren)
}
 ...
 * @param pickledAnalyzeResult
 *   if the UDTF defined an 'analyze' method, this contains the pickled 'AnalyzeResult' instance
 *   from that method, which contains all metadata returned including the result schema of the
 *   function call as well as optional other information
 * @param children
 *   input arguments to the UDTF call; for scalar arguments these are the expressions themeselves,
 *   and for TABLE arguments, these are instances of [[FunctionTableSubqueryArgumentExpression]]
 * @param evalType
 *   identifies whether this is a scalar or aggregate or table function, using an instance of the
 *   [[PythonEvalType]] enumeration
 * @param udfDeterministic
 *   true if this function is deterministic wherein it returns the same result rows for every call
 *   with the same input arguments
 * @param resultId
 *   unique expression ID for this function invocation
 * @param pythonUDTFPartitionColumnIndexes
 *   holds the zero-based indexes of the projected results of all PARTITION BY expressions within
 *   the TABLE argument of the Python UDTF call, if applicable
 * @param tableArguments
 *   holds whether an input argument is a table argument
 */
...
/**
 * Represents the result of invoking the polymorphic 'analyze' method on a Python user-defined
 * table function. This returns the table function's output schema in addition to other optional
 * metadata.
 *
 * @param schema
 *   result schema of this particular function call in response to the particular arguments
 *   provided, including the types of any provided scalar arguments (and their values, in the case
 *   of literals) as well as the names and types of columns of the provided TABLE argument (if
 *   any)
 * @param withSinglePartition
 *   true if the 'analyze' method explicitly indicated that the UDTF call should consume all rows
 *   of the input TABLE argument in a single instance of the UDTF class, in which case Catalyst
 *   will invoke a repartitioning to a separate stage with a single worker for this purpose
 * @param partitionByExpressions
 *   if non-empty, this contains the list of column names that the 'analyze' method explicitly
 *   indicated that the UDTF call should partition the input table by, wherein all rows
 *   corresponding to each unique combination of values of the partitioning columns are consumed
 *   by exactly one unique instance of the UDTF class
 * @param orderByExpressions
 *   if non-empty, this contains the list of ordering items that the 'analyze' method explicitly
 *   indicated that the UDTF call should consume the input table rows by
 * @param selectedInputExpressions
 *   If non-empty, this is a list of expressions that the UDTF is specifying for Catalyst to
 *   evaluate against the columns in the input TABLE argument. In this case, Catalyst will insert
 *   a projection to evaluate these expressions and return the result to the UDTF. The UDTF then
 *   receives one input column for each expression in the list, in the order they are listed.
 * @param pickledAnalyzeResult
 *   this is the pickled 'AnalyzeResult' instance from the UDTF, which contains all metadata
 *   returned by the Python UDTF 'analyze' method including the result schema of the function call
 *   as well as optional other information
 */
 ...
 *
 * @param expression
 *   the expression that the UDTF is specifying for Catalyst to evaluate against the columns in
 *   the input TABLE argument
 * @param alias
 *   If present, this is the alias for the column or expression as visible from the UDTF's 'eval'
 *   method. This is required if the expression is not a simple column reference.
 */
case class PythonUDTFSelectedExpression(expression: Expression, alias: Option[String])

/**
 * A place holder used when printing expressions without debugging information such as the result
 * id.
 */
case class PrettyPythonUDF(name: String, dataType: DataType, children: Seq[Expression])
    extends UnevaluableAggregateFunc
