...
// scalastyle:on
  ...
  def argString(maxFields: Option[Int]): String = stringArgs.flatMap {
    ...
    case set: Set[_] => Utils.truncatedString(set.toSeq, "{", ", ", "}", maxFields) :: Nil
    case array: Array[_] if array.isEmpty => Nil
    case array: Array[_] => Utils.truncatedString(array, "[", ", ", "]", maxFields) :: Nil
    case null => Nil
    case None => Nil
    case Some(null) => Nil
  ...
  private def parseToJson(obj: Any): JValue = obj match {
    ...
    case t: Seq[_] if t.length > 0 && t.head.isInstanceOf[String] =>
      JString(Utils.truncatedString(t, "[", ", ", "]", maxFields = None))
    case t: Seq[_] => JNull
