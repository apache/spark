...
class HigherOrderFunctionResolver(
  ...
  override def resolve(unresolvedFunction: UnresolvedFunction): Expression = {
    ...
    val partiallyResolvedFunction =
      functionResolution.resolveFunction(unresolvedFunctionWithExpandedStarArgs)

    val partiallyResolvedHigherOrderFunction =
      validateHigherOrderFunction(Some(partiallyResolvedFunction),
