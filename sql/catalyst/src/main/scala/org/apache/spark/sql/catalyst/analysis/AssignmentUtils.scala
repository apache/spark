...
object AssignmentUtils extends SQLConfHelper with CastSupport {
  ...
  /**
   * Aligns update assignments to match table columns. <p> This method processes and reorders
   * given assignments so that each target column gets an expression it should be set to. If a
   * column does not have a matching assignment, it will be set to its current value. For example,
   * if one passes table attributes c1, c2 and an assignment c2 = 1, this method will return c1 =
   * c1, c2 = 1. This allows Spark to construct an updated version of a row. <p> This method also
   * handles updates to nested columns. If there is an assignment to a particular nested field,
   * this method will construct a new struct with one field updated preserving other fields that
   * have not been modified. For example, if one passes table attributes c1, c2 where c2 is a
   * struct with fields n1 and n2 and an assignment c2.n2 = 1, this method will return c1 = c1, c2 =
   * struct(c2.n1, 1).
   *
   * @param attrs
   *   table attributes
   ...
   * @param fromStar
   *   whether the assignments were resolved from an UPDATE SET * clause. These updates may assign
   *   struct fields individually (preserving existing fields).
   * @param coerceNestedTypes
   *   whether to coerce nested types to match the target type for complex types
   * @return
  ...
  /**
   * Aligns insert assignments to match table columns. <p> This method processes and reorders
   * given assignments so that each target column gets an expression it should be set to. There
   * must be exactly one assignment for each top-level attribute and its value must be compatible.
   * <p> Insert assignments cannot refer to nested columns.
   *
   * @param attrs
   *   table attributes
   * @param assignments
      ...
      if (updateStar && SQLConf.get.coerceMergeNestedTypes) {
        ...
        col.dataType match {
          case _: StructType =>
            ...
            applyNestedFieldAssignments(col, colExpr, value, addError, colPath, coerceNestedTypes)
          case _ =>
  ...
  private def toNamedStruct(structType: StructType, fieldExprs: Seq[Expression]): Expression = {
    val namedStructExprs = structType.fields
      .zip(fieldExprs)
  ...
  /**
   * As UPDATE SET * assigns struct fields individually (preserving existing fields), this will
   * lead to indiscriminate null expansion, ie, a struct is created where all fields are null.
   * Wraps a struct assignment with a condition to return null if both conditions are true:
   *
   *   - source struct is null
   *   - target struct is null OR target struct is same as source struct
   *
   * If the condition is not true, we preserve the original structure. This includes cases where
   * the source was a struct of nulls, or there were any extra target fields (including null
   * ones), both cases retain the assignment to a struct of nulls.
   *
   * @param key
   *   the original assignment key (target struct) expression
   * @param value
   *   the original assignment value (source struct) expression
   * @param structType
   *   the target struct type
   * @param structExpression
   *   the result create struct expression result to wrap
   * @param colPath
   *   the column path for error reporting
   * @return
   *   the wrapped expression with null checks
   */
