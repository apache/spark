...
object CatalogColumnStat extends Logging {
  ...
  private def getTimestampFormatter(needVarLengthSecondFraction: Boolean): TimestampFormatter = {
    TimestampFormatter(
  ...
  def fromExternalString(s: String, name: String, dataType: DataType, version: Int): Any = {
    dataType match {
      ...
      case TimestampType => getTimestampFormatter(needVarLengthSecondFraction = true).parse(s)
  ...
  def toExternalString(v: Any, colName: String, dataType: DataType): String = {
    val externalValue = dataType match {
      ...
      case TimestampType =>
        getTimestampFormatter(needVarLengthSecondFraction = false).format(v.asInstanceOf[Long])
