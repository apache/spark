...
trait TracksResolvedNodes[TreeNodeType <: TreeNode[TreeNodeType]] extends SQLConfHelper {
  private val resolvedNodeSet = new java.util.HashSet[TreeNodeComparatorByReference[TreeNodeType]]()

  protected def withTrackResolvedNodes(node: TreeNodeType)(
      resolver: => TreeNodeType): TreeNodeType = {
    if (conf.getConf(SQLConf.ANALYZER_SINGLE_PASS_TRACK_RESOLVED_NODES_ENABLED)) {
      if (resolvedNodeSet.contains(new TreeNodeComparatorByReference(node))) {
        throw SparkException.internalError(
          s"Single-pass resolver attempted to resolve the same node more than once: $node"
        )
      }
      // compute the `resolver` lambda exactly once, store it to a node set and return the result
      val result = resolver
      resolvedNodeSet.add(new TreeNodeComparatorByReference(result))
      result
    } else {
      resolver
    }
