 ...
 * function results are further filtered by limit-like predicates. Example query:
 * {{{
 *   SELECT * FROM (
...
object InferWindowGroupLimit extends Rule[LogicalPlan] with PredicateHelper {
   ...
   */
  private def isExpandingWindow(windowExpression: NamedExpression): Boolean =
    windowExpression match {
      case Alias(
            ...
            _) if !windowFunction.isInstanceOf[SizeBasedWindowFunction] =>
        true
      case _ => false
    }
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    plan.transformWithPruning(_.containsAllPatterns(FILTER, WINDOW), ruleId) {
      case filter @ Filter(
          ...
          if !child.isInstanceOf[WindowGroupLimit] && windowExpressions.forall(
            isExpandingWindow) &&
            orderSpec.nonEmpty =>
        ...
        if (limits.isEmpty) {
          ...
          selectedLimits.minBy(_._1) match {
            case (limit, rankLikeFunction)
                if limit <= conf.windowGroupLimitThreshold &&
                  child.maxRows.forall(_ > limit) =>
              if (limit > 0) {
                val newFilterChild =
                  if (rankLikeFunction.isInstanceOf[RowNumber] &&
