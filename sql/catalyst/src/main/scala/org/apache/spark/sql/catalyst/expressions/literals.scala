...
object Literal {
  ...
  def apply(v: Any): Literal = v match {
    ...
    case lt: LocalTime => Literal(TimeUtils.localTimeToMicros(lt), TimeType)
    case t: Time => Literal(TimeUtils.sqlTimeToMicros(t), TimeType)
  ...
  private[this] def componentTypeToDataType(clz: Class[_]): DataType = clz match {
    ...
    case _ if clz == classOf[Time] => TimeType
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    if (value == null) {
      ...
      dataType match {
        ...
        case TimestampType | TimestampNTZType | TimeType | LongType | _: DayTimeIntervalType =>
