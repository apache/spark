 ...
 *      field value to the desired type from the schema. After that, it sets the value to an
 *      instance of `InternalRow` at the position according to the schema. Order of parsed JSON
 *      fields can be different from the order in the schema.
 ...
 *      field according to the schema. 3.1 `skipRow` finds a group of predicates that refers to
 *      this JSON field. 3.2 Per each predicate from the group, `skipRow` decrements its reference
 *      counter. 3.2.1 If predicate reference counter becomes 0, it means that all predicate
 *      attributes have been already set in the internal row, and the predicate can be applied to
 *      it. `skipRow` invokes the predicate for the row. 3.3 `skipRow` applies predicates until
 *      one of them returns `false`. In that case, the method returns `true` to the parser. 3.4 If
 *      all predicates with zero reference counter return `true`, the final result of the method
 *      is `false` which tells the parser to not skip the row.
