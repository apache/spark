 ...
 *    [[createMappingForLeafNode]] is called right after each [[LeafNode]] is resolved, and first
 *    remapped attributes come from that [[LeafNode]]. This is done in leaf operator output doesn't
 *    conflict with `globalExpressionIds`.
...
class ExpressionIdAssigner {
  ...
  /**
   * Create mapping with the given `newOutput` that rewrites the `oldOutput`. This is used by the
   * [[Resolver]] after a certain [[LeafNode]] is resolved to make sure that leaf node outputs
  ...
  def mapExpression[NamedExpressionType <: NamedExpression](
    ...
    if (mappingStack.peek().mapping.isEmpty) {
      throw SparkException.internalError(
        "Expression ID mapping doesn't exist. Please first call createMappingForLeafNode(...) " +
        "for leaf nodes or createMappingFromChildMappings(...) for multi-child nodes. " +
   ...
   */
  def shouldPreserveLeafOperatorIds(leafOperatorOutput: Seq[Attribute]): Boolean = {
    leafOperatorOutput.forall { attribute =>
