 ...
 *
 * PURPOSE:
 * Handles bidirectional conversion between user-facing types (e.g., java.time.LocalTime)
...
trait ExternalTypeOps extends TypeOps {
   ...
   * @return The internal Catalyst representation, or null if input was null/None
   */
   ...
   * @example LocalTime.of(10, 30, 0) -> 37800000000000L (nanoseconds since midnight)
   */
   ...
   * @return The external representation of the value at the given column
   */
   ...
   * @param row The InternalRow containing the value
   * @param column The column index
   * @return The external representation, or null if the column is null
   */
...
object ExternalTypeOps {
   ...
   *
   * @param dt The DataType to get external conversion operations for
   * @return ExternalTypeOps instance
   * @throws SparkException if the type doesn't support ExternalTypeOps
   */
   ...
   * @return true if the type supports ExternalTypeOps
   */
