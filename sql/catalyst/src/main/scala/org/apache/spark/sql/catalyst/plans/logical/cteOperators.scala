 ...
 * term, that contains an [[UnionLoopRef]] node in its recursive child. The node is very similar
 * to [[Union]] because the initial and "generated" children are union-ed and it is also similar
 * to a loop because the recursion continues until the last generated child is not empty. Union in
 ...
 * @param outputAttrIds
 *   The ids of UnionLoop's output attributes.
 * @param limit
 *   An optional limit that can be pushed down to the node to stop the loop earlier.
 * @param maxDepth
 *   Maximal number of iterations before we report an error.
 */
case class UnionLoop(
  ...
  override protected def withNewChildrenInternal(
      newChildren: IndexedSeq[LogicalPlan]): UnionLoop =
 ...
 *
 * @param loopId
 *   The id of the loop, inherited from [[CTERelationRef]] which got resolved into this
 *   UnionLoopRef.
 * @param output
 *   The output attributes of this recursive reference.
 * @param accumulated
 *   If false the reference stands for the result of the previous iteration. If it is true then it
 *   stands for the union of all previous iteration results.
 */
case class UnionLoopRef(loopId: Long, override val output: Seq[Attribute], accumulated: Boolean)
    extends LeafNode
 ...
 * @param maxDepth
 *   The maximal depth of a recursion in a recursive CTE.
 */
case class CTERelationDef(
  ...
  lazy val hasSelfReferenceAsCTERef: Boolean = child
    .collectFirstWithSubqueries { case CTERelationRef(this.id, _, _, _, _, true, _, _) =>
      true
    }
    .getOrElse(false)
  ...
  lazy val hasSelfReferenceAsUnionLoopRef: Boolean = child
    .collectFirstWithSubqueries { case UnionLoopRef(this.id, _, _) =>
      true
    }
    .getOrElse(false)
}
 ...
 * @param isUnlimitedRecursion
 *   If the node is a (non-recursive) reference to a recursive CTE that should be executed without
 *   a limit to the number of rows it returns.
 */
case class CTERelationRef(
    ...
    isUnlimitedRecursion: Boolean = false)
    extends LeafNode
    with MultiInstanceRelation {
