...
case class CTERelationDef(
  ...
  // are not set, so existing plan comparisons and golden files are unaffected by the new fields.
  // The materialization option is printed as its keyword, and omitted when unspecified.
  override def stringArgs: Iterator[Any] = {
    ...
    Iterator(child, id, originalPlanWithPredicates, underSubquery, maxDepth, option) ++
      ...
      (if (forcePartitioning.isDefined) Iterator(forcePartitioning) else Iterator.empty)
  }
