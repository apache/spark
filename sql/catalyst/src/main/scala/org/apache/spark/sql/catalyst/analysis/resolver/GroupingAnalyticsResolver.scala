...
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, Expand, Filter, LogicalPlan, Sort}
...
class GroupingAnalyticsResolver(resolver: Resolver, expressionResolver: ExpressionResolver)
  ...
  def handleSortOrderExpressionsWithGroupingAnalytics(
    ...
    val (groupingExpressions, resolvedGroupingId) = getGroupingExpressionsAndResolvedGid(sort)

    orderExpressions.map { orderExpression =>
      transformWithGroupingAnalytics(
        expression = orderExpression,
        groupingExpressions = groupingExpressions,
        resolvedGroupingId = resolvedGroupingId
      ).asInstanceOf[SortOrder]
    }
  }

  /**
   * Handles HAVING condition using `getGroupingExpressionsAndResolvedGid` and
   * `transformWithGroupingAnalytics` (see their scala docs for more details).
   */
  def handleHavingConditionWithGroupingAnalytics(
      havingCondition: Expression,
      partiallyResolvedHaving: Filter): Expression = {
    val (groupingExpressions, resolvedGroupingId) =
      getGroupingExpressionsAndResolvedGid(partiallyResolvedHaving)
  ...
  /**
   * Validates that there is a base [[Aggregate]] in the current scope:
   *  - If there is, collect grouping expressions from it using
   *    [[GroupingAnalyticsTransformer.collectGroupingExpressions]].
   *  - If there isn't, throw `groupingMustWithGroupingSetsOrCubeOrRollupError` exception.
   * After validation, creates a grouping ID attribute and resolve it using [[ExpressionResolver]].
   */
  private def getGroupingExpressionsAndResolvedGid(
      parentOperator: LogicalPlan): (Seq[Expression], AttributeReference) = {
    ...
    val resolvedGroupingId = expressionResolver
      .resolveExpressionTreeInOperator(groupingId, parentOperator)
      .asInstanceOf[AttributeReference]

    (groupingExpressions, resolvedGroupingId)
  }

  /**
   * Replace provided expression using [[GroupingAnalyticsTransformer.replaceGroupingFunction]]
   * (see its scala doc for more details).
   */
  private def transformWithGroupingAnalytics(
      expression: Expression,
      groupingExpressions: Seq[Expression],
      resolvedGroupingId: AttributeReference): Expression = {
