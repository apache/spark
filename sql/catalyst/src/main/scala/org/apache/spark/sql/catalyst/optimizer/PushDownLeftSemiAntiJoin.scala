  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformWithPruning(
    ...
    case join @ Join(agg: Aggregate, rightOp, LeftSemiOrAnti(_), joinCond, _)
      ...
      val makeJoinCondition = (predicates: Seq[Expression]) => {
        replaceAlias(predicates.reduce(And.apply), aliasMap)
      }
    ...
    case join @ Join(w: Window, rightOp, LeftSemiOrAnti(_), _, _)
      ...
      pushDownJoin(join, _.references.subsetOf(partitionAttrs), _.reduce(And.apply))
    ...
    case join @ Join(u: UnaryNode, rightOp, LeftSemiOrAnti(_), _, _)
      ...
      pushDownJoin(join, _.references.subsetOf(validAttrs), _.reduce(And.apply))
  }
      ...
      if (pushDown.isEmpty || referRightSideCols)  {
        ...
        if (stayUp.isEmpty) {
          ...
          join.joinType match {
            ...
            case LeftSemi => Filter(stayUp.reduce(And.apply), newPlan)
