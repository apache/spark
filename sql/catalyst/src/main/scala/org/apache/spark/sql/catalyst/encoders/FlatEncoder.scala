...
object FlatEncoder {
  ...
  def apply[T : TypeTag]: Encoder[T] = {
    ...
    val dt = schemaFor(tpe).dataType
    assert(!dt.isInstanceOf[StructType])

    val input = BoundReference(0, dataTypeFor(tpe), nullable = true)
    val extractExpression = CreateNamedStruct(
      Literal("value") :: ProductEncoder.extractorFor(input, tpe) :: Nil)
    val output = BoundReference(0, dt, nullable = true)
    val constructExpression = ProductEncoder.constructorFor(output, tpe)
