...
/**
 * An instance of the class compiles filters to predicates and sorts them in the order which
 * allows to apply the predicates to an internal row with partially initialized values, for
 * instance converted from parsed CSV fields.
 *
 * @param filters
 *   The filters pushed down to a datasource.
 * @param requiredSchema
 *   The schema with only fields requested by the upper layer.
 */
class OrderedFilters(filters: Seq[sources.Filter], requiredSchema: StructType)
    extends StructFilters(filters, requiredSchema) {

  /**
   * Converted filters to predicates and grouped by maximum field index in the read schema. For
   * example, if a filter refers to 2 attributes attrA with field index 5 and attrB with field
   * index 10 in the read schema: 0 === $"attrA" or $"attrB" < 100 the filter is compiled to a
   * predicate, and placed to the `predicates` array at the position 10. In this way, if there is
   * a row with initialized fields from the 0 to 10 index, the predicate can be applied to the row
   * to check that the row should be skipped or not. Multiple predicates with the same maximum
   * reference index are combined by the `And` expression.
   */
   ...
   * @param row
   *   The internal row to check.
   * @param index
   *   Maximum field index. The function assumes that all fields from 0 to `index` position are
   *   set.
   * @return
   *   `false` iff row fields at the position from 0 to `index` pass filters or there are no
   *   applicable filters otherwise `false` if at least one of the filters returns `false`.
   */
  def skipRow(row: InternalRow, index: Int): Boolean = {
    assert(
      0 <= index && index < requiredSchema.fields.length,
