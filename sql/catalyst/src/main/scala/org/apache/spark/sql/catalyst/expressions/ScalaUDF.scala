...
case class ScalaUDF(
  ...
  */
  ...
  def getInputConverters(): Seq[Any => Any] = inputConverters.getOrElse(Nil)
  ...
  private[this] def genCodeForConverter(ctx: CodegenContext, index: Int): String = {
    ...
    if (inputConverters.isDefined) {
      ctx.addMutableState(converterClassName, converterTerm,
        s"this.$converterTerm = ($converterClassName)" +
          s"((($scalaUDFClassName)references[$expressionIdx]).getInputConverters().apply($index));")
    } else {
