...
case class ScalaUDF(
  ...
  def exportedFunc(toScalaConversion: Boolean = true): AnyRef = {
    children.size match {
      case 1 =>
        ...
        (input: Any) => {
          if (toScalaConversion) {
            func(converter0(input))
          } else {
            func(input)
          }
        }
      ...
      case _ =>
        throw new RuntimeException("Can't export UDF function with 0 or more than 1 parameter.")
    }
  }
