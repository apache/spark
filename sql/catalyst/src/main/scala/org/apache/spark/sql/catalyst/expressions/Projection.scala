        ...
        if (setters == null) {
          ...
          for(i <- 0 until exprArray.length) {
            setters(i) = exprArray(i).dataType match {
              ...
              case _: DecimalType => (target: UnsafeRow, value: Any) =>
                val decNum = value.asInstanceOf[Decimal]
                target.setDecimal(i, decNum, decNum.precision)
              case DateType => (target: UnsafeRow, value: Any) =>
                target.setInt(i, value.asInstanceOf[Int])
              case TimestampType => (target: UnsafeRow, value: Any) =>
                target.setLong(i, value.asInstanceOf[Long])
            }
          }
        }
