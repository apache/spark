...
object DefaultCollationTypeCoercion {
  ...
  def apply(expression: Expression, collation: String): Expression = {
    ...
    expression match {
      ...
      // `JSON_OBJECT(... RETURNING STRING COLLATE UTF8_BINARY)`): the user chose it, so the
      // object/view default must not overwrite it. `isDefaultStringType` tells the two apart by
      // reference identity.
