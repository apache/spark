...
package org.apache.spark.sql.catalyst.expressions.ANSISQL
...
case class AnsiSqlCastToDecimal(child: Expression, timeZoneId: Option[String])
  ...
  override def toString: String = s"AnsiSqlCastToDecimal($child as ${dataType.simpleString})"
  ...
  override def checkInputDataTypes(): TypeCheckResult = child.dataType match {
    case StringType | LongType | IntegerType | NullType | FloatType | ShortType |
         DoubleType | ByteType | TimestampType =>
  ...
  override protected def ansiEnabled: Boolean = true
}
