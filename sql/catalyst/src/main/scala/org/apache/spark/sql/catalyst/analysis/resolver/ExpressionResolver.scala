...
import org.apache.spark.sql.catalyst.util.{toPrettySQL, CollationFactory}
...
class ExpressionResolver(
  ...
  /**
   * Get the operator resolution context stack.
   */
  ...
  def resolveProjectList(
    ...
    val resolvedProjectList = unresolvedProjectListWithStarsExpanded.flatMap { expression =>
      ...
      Seq(aliasOuterReferenceIfNeeded(resolvedElement).asInstanceOf[NamedExpression])
    }
    ...
    val resolvedAggregateExpressions =
      unresolvedAggregateExpressionsWithStarsExpanded.zipWithIndex.flatMap {
        case (expression, index) =>
          val (unaliasedResolvedElement, resolvedElementContext) =
            resolveExpressionTreeInOperatorImpl(
              expression,
              unresolvedAggregate,
              shouldPreserveAlias = true
            )
          val resolvedElement = aliasOuterReferenceIfNeeded(unaliasedResolvedElement)
   ...
   * strips the wrapper). Any other expression (including an [[Alias]] from struct star expansion or
   * an explicit alias) is left unchanged.
   *
   * The alias name follows the same rule as [[AliasResolution.resolve]]: when the reference was
   * replaced by an outer aggregate alias, [[AggregateExpressionResolver]] records the original
   * attribute's name in [[OuterReference.SINGLE_PASS_OUTER_AGGREGATE_ALIAS_NAME_OVERRIDE]], which
   * we honor here.
   */
  private def aliasOuterReferenceIfNeeded(resolvedElement: Expression): Expression =
    resolvedElement match {
      case outerReference: OuterReference =>
        ...
        autoGeneratedAliasProvider.newAlias(child = outerReference, name = Some(aliasName))
      case other => other
    }
   ...
   *
   * - If the candidate is a timezone-aware literal function ([[CurrentDate]] or [[CurrentTime]]),
   *   complete the resolution using the [[TimezoneAwareExpressionResolver]] so that the expression
   *   is fully resolved.
  ...
  private def processCandidateFromNameTarget(candidate: Expression): Expression = {
    candidate match {
      case timezoneAware: TimeZoneAwareExpression if isCurrentDateTimeFunction(timezoneAware) =>
        timezoneAwareExpressionResolver.resolve(timezoneAware)
  ...
  private def isCurrentDateTimeFunction(expression: Expression): Boolean = expression match {
    case _: CurrentDate | _: CurrentTime => true
    case _ => false
  }
  ...
  private def tryDrainLazySequences(list: Seq[Expression]): Seq[Expression] = {
    ...
    val result: Seq[Expression] = list match {
      case _: Stream[_] =>
        list.toList
      case other => other
    }
