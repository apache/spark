...
case class Cast(child: Expression, dataType: DataType, timeZoneId: Option[String] = None)
  ...
  private[this] def castCode(ctx: CodegenContext, input: ExprValue, inputIsNull: ExprValue,
    ...
    val javaType = inline"${CodeGenerator.javaType(resultType)}"
    code"""
  ...
  private def writeArrayToStringBuilder(
    ...
    val loopIndex = JavaCode.variable(ctx.freshName("loopIndex"), IntegerType)
    code"""
       |$buffer.append("[");
       |if ($array.numElements() > 0) {
       |  if (!$array.isNullAt(0)) {
  ...
  private def writeStructToStringBuilder(
    ...
    val structToStringCode = st.zipWithIndex.map { case (ft, i) =>
      ...
      val field = JavaCode.variable(ctx.freshName("field"), ft)
      val fieldStr = JavaCode.variable(ctx.freshName("fieldStr"), StringType)
      val javaType = inline"${CodeGenerator.javaType(ft)}"
      code"""
  ...
  private[this] def castToDateCode(
    ...
    case StringType =>
      val intOpt = JavaCode.variable(ctx.freshName("intOpt"), classOf[Option[Integer]])
  ...
  private[this] def castToDecimalCode(
    ...
    val tmp = JavaCode.variable(ctx.freshName("tmpDecimal"), classOf[Decimal])
    from match {
      case StringType =>
        (c, evPrim, evNull) =>
          code"""
  ...
  private[this] def castToByteCode(from: DataType, ctx: CodegenContext): CastFunction = from match {
    case StringType =>
      val wrapper = JavaCode.variable(ctx.freshName("intWrapper"), classOf[UTF8String.IntWrapper])
      (c, evPrim, evNull) =>
        code"""
  ...
  private[this] def castToLongCode(from: DataType, ctx: CodegenContext): CastFunction = from match {
    case StringType =>
      val wrapper = JavaCode.variable(ctx.freshName("longWrapper"), classOf[UTF8String.LongWrapper])
  ...
  private[this] def castArrayCode(
    ...
    val fromElementNull = JavaCode.isNullVariable(ctx.freshName("feNull"))
    val fromElementPrim = JavaCode.variable(ctx.freshName("fePrim"), fromType)
    val toElementNull = JavaCode.isNullVariable(ctx.freshName("teNull"))
    val toElementPrim = JavaCode.variable(ctx.freshName("tePrim"), toType)
    ...
    val javaType = inline"${CodeGenerator.javaType(fromType)}"
  ...
  private[this] def castMapCode(from: MapType, to: MapType, ctx: CodegenContext): CastFunction = {
    ...
    val keys = JavaCode.variable(ctx.freshName("keys"), ArrayType(from.keyType))
    val convertedKeys = JavaCode.variable(ctx.freshName("convertedKeys"), ArrayType(to.keyType))
    val convertedKeysNull = JavaCode.isNullVariable(ctx.freshName("convertedKeysNull"))

    val values = JavaCode.variable(ctx.freshName("values"), ArrayType(from.valueType))
    val convertedValues = JavaCode.variable(ctx.freshName("convertedValues"),
      ArrayType(to.valueType))
    val convertedValuesNull = JavaCode.isNullVariable(ctx.freshName("convertedValuesNull"))
  ...
  private[this] def castStructCode(
    ...
    val tmpResult = JavaCode.variable(ctx.freshName("tmpResult"), classOf[GenericInternalRow])
    ...
    val fieldsEvalCode = fieldsCasts.zipWithIndex.map { case (cast, i) =>
      val fromFieldPrim = JavaCode.variable(ctx.freshName("ffp"), from.fields(i).dataType)
      val fromFieldNull = JavaCode.isNullVariable(ctx.freshName("ffn"))
      val toFieldPrim = JavaCode.variable(ctx.freshName("tfp"), to.fields(i).dataType)
      val toFieldNull = JavaCode.isNullVariable(ctx.freshName("tfn"))
      val fromType = inline"${CodeGenerator.javaType(from.fields(i).dataType)}"
    ...
    val fieldsEvalCodes = ctx.splitExpressions(
      ...
      funcName = "castStruct",
      arguments = (tmpInput) :: (tmpResult) :: Nil)
