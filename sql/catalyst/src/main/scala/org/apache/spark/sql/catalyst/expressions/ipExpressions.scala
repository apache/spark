...
@ExpressionDescription(
  usage = """
    _FUNC_(str) - Converts an IPv4 address string to an unsigned 32-bit value represented as a
    64-bit Long.
...
@ExpressionDescription(
  usage = """
    _FUNC_(value) - Converts an unsigned 32-bit value represented as a 64-bit Long to an IPv4
    address string in dotted-decimal notation.
    Returns NULL if the value is out of valid IPv4 range (<0 or >4294967295) in non-ANSI mode.
  """,
  arguments = """
    Arguments:
      * value - a long expression representing an unsigned 32-bit IPv4 value
  """,
  examples = """
    Examples:
...
@ExpressionDescription(
  usage = """
    _FUNC_(value) - This is a special version of `inet_ntoa` that performs the same operation, but
    returns NULL instead of raising an error if the value is outside the valid IPv4 range.
  """,
  arguments = """
    Arguments:
      * value - a long expression representing a potential unsigned IPv4 value
  """,
  examples = """
    Examples:
      > SELECT _FUNC_(3232235777);
  ...
  group = "misc_funcs")
// scalastyle:on line.size.limit
case class TryInetNtoa(expr: Expression, replacement: Expression)
  extends RuntimeReplaceable with InheritAnalysisRules {

  def this(expr: Expression) = this(expr, InetNtoa(expr, false))
  ...
  override def prettyName: String = "try_inet_ntoa"
}
