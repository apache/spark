 ...
 *
 * The input is a sketch state that is generated by the ApproxTopKAccumulation function. The
 * output is an array of structs, each containing a frequent item and its estimated frequency. The
 * items are sorted by their estimated frequency in descending order.
 *
 * @param state
 *   The sketch state, which is a struct containing the serialized sketch data, the original data
 *   type and the max items tracked of the sketch.
 * @param k
 *   The number of top items to estimate.
 */
...
case class ApproxTopKEstimate(state: Expression, k: Expression)
  ...
  override protected def withNewChildrenInternal(
      newState: Expression,
      newK: Expression): Expression = copy(state = newState, k = newK)
