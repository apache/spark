...
trait NamedExpression extends Expression {
  ...
  def isGenerated: Option[Boolean] = None
...
case class Alias(child: Expression, name: String)(
  ...
  override def sql: String = {
    ...
    val aliasName = if (isGenerated.contains(true)) s"$name#${exprId.id}" else s"$name"
...
case class AttributeReference(
  ...
  override def sql: String = {
    val qualifiersString =
      if (qualifiers.isEmpty) "" else qualifiers.map("`" + _ + "`").mkString("", ".", ".")
    val attrRefName = if (isGenerated.contains(true)) s"$name#${exprId.id}" else s"$name"
