...
object GeneratedColumn {
  ...
  private def analyzeAndVerifyExpression(
    ...
    def unsupportedExpressionError(reason: String): AnalysisException = {
      new AnalysisException(
        ...
        messageParameters =
          Map("fieldName" -> fieldName, "expressionStr" -> expressionStr, "reason" -> reason))
    }
    ...
    val analyzed = plan.collectFirst { case Project(Seq(a: Alias), _: LocalRelation) =>
      a.child
    }.get
  ...
  /**
   * If `schema` contains any generated columns: 1) Check whether the table catalog supports
   * generated columns. Otherwise throw an error. 2) Parse, analyze and verify the generation
   * expressions for any generated columns.
   */
  def validateGeneratedColumns(
    ...
    if (hasGeneratedColumns(schema)) {
      if (!catalog
          .capabilities()
          .contains(TableCatalogCapability.SUPPORTS_CREATE_TABLE_WITH_GENERATED_COLUMNS)) {
 ...
 */
object GeneratedColumnAnalyzer
    extends Analyzer(
