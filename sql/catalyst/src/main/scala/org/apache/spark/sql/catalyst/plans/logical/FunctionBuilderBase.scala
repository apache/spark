...
object NamedParametersSupport {
  ...
  // TODO: align with handling of columns
  private def toInputParameter(param: ProcedureParameter): InputParameter = {
    val defaultValue = if (param.defaultValueExpression != null) {
      val parser = new CatalystSqlParser()
      Some(parser.parseExpression(param.defaultValueExpression))
    } else {
      None
    }
  ...
  private def defaultRearrange(
    ...
    if (parameters.size < args.length) {
      ...
      throw QueryCompilationErrors.wrongNumArgsError(routineName, validParameterSizes, args.length)
    }
