 ...
 * determined by the session's default string type, which is always UTF8_BINARY.
 * For DDL, the default string type is the default type of the object (table -> schema -> catalog).
 */
