...
import org.apache.spark.sql.catalyst.util.RowDeltaUtils.{OPERATION_COLUMN, WRITE_OPERATION, WRITE_WITHOUT_METADATA_OPERATION}
...
object RewriteMergeIntoTable extends RewriteRowLevelCommand with PredicateHelper {
  ...
  private def buildReplaceDataMergeRowsPlan(
    ...
    val carryoverRowsOutput = Literal(WRITE_OPERATION) +: targetTable.output
  ...
  private def toInstruction(action: MergeAction, metadataAttrs: Seq[Attribute]): Instruction = {
    action match {
      ...
      case InsertAction(cond, assignments) =>
        ...
        val output = Seq(Literal(WRITE_WITHOUT_METADATA_OPERATION)) ++ rowValues ++ metadataValues
