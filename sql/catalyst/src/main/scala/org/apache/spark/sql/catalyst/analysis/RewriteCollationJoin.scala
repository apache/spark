...
import org.apache.spark.sql.catalyst.expressions.{AttributeReference, EqualTo, ExpectsInputTypes, Expression, Lower, UnaryExpression}
    ...
    case j @ Join(_, _, _, Some(condition), _) =>
      val newCondition = condition transform {
        case EqualTo(l: AttributeReference, r: AttributeReference)
          if l.dataType.isInstanceOf[StringType] && r.dataType.isInstanceOf[StringType] =>
