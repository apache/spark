...
abstract class LogicalPlan
  ...
  /**
   * Metadata fields that can be projected from this node. Should be overridden if the plan does
   * not propagate its children's output.
   */
   ...
   * [[org.apache.spark.sql.catalyst.analysis.UnresolvedRelation UnresolvedRelation]] should
   * return `false`).
   */
  ...
  /**
   * Resolves a given schema to concrete [[Attribute]] references in this query plan. This
   * function should only be called on analyzed plans since it will throw [[AnalysisException]]
   * for unresolved [[Attribute]]s.
   */
   ...
   * nodes of this LogicalPlan. The attribute is expressed as string in the following form:
  ...
  /**
   * Given an attribute name, split it to name parts by dot, but don't split the name parts quoted
   * by backticks, for example, `ab.cd`.`efg` should be split into two parts "ab.cd" and "efg".
   */
...
trait OrderPreservingUnaryNode extends UnaryNode with AliasAwareQueryOutputOrdering[LogicalPlan] {
...
object LogicalPlanIntegrity {
   ...
   * this method checks if the same `ExprId` refers to attributes having the same data type in
   * plan output. Returns the error message if the check does not pass.
   */
  ...
  /**
   * This method checks if reference `ExprId`s are not reused when assigning a new `ExprId`. For
   * example, it returns false if plan transformers create an alias having the same `ExprId` with
   * one of reference attributes, e.g., `a#1 + 1 AS a#1`. Returns the error message if the check
   * does not pass.
   */
  ...
  /**
   * This method checks if the same `ExprId` refers to an unique attribute in a plan tree. Some
   * plan transformers (e.g., `RemoveNoopOperators`) rewrite logical plans based on this
   * assumption. Returns the error message if the check does not pass.
   */
  ...
  /**
   * This method validates there are no dangling attribute references. Returns an error message if
   * the check does not pass, or None if it does pass.
   */
    ...
    plan.collectFirst {
      ...
      case n if canGetOutputAttrs(n) =>
        if (n.missingInput.nonEmpty) {
          Some(
            s"Aliases ${n.missingInput.mkString(", ")} are dangling " +
  ...
  /**
   * Validate that the aggregation expressions in Aggregate plans are valid. Returns an error
   * message if the check fails, or None if it succeeds.
   */
  def validateAggregateExpressions(plan: LogicalPlan): Option[String] = {
    plan.collectFirst { case a: Aggregate =>
      try {
        ...
        case e: AnalysisException =>
          Some(
            s"Aggregate: ${a.toString} is not a valid aggregate expression: " +
              s"${e.getSimpleMessage}")
      }
    }.flatten
  }

  def validateSchemaOutput(
  ...
  /**
   * Validate the structural integrity of an optimized plan. For example, we can check after the
   * execution of each rule that each plan:
   *   - is still resolved
   *   - only host special expressions in supported operators
   *   - has globally-unique attribute IDs
   *   - has the same result schema as the previous plan
   *   - has no dangling attribute references If `lightweight` is true, we only run the first
   *     check above.
   */
