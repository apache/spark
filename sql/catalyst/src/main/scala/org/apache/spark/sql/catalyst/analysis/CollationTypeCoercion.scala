...
object CollationTypeCoercion {
  ...
  private def shouldFailWithIndeterminateCollation(expr: Expression): Boolean = {
    !canContainIndeterminateCollation(expr) && expr.children.exists(
      child => SchemaUtils.hasIndeterminateCollation(getDataType(child).getOrElse(NullType))
    )
  }
  ...
  private def canContainIndeterminateCollation(expr: Expression): Boolean = expr match {
    ...
    // is that they will fail at runtime once we try to fetch the collator/comparison fn.
    case _: BinaryComparison | _: StringPredicate | _: Upper | _: Lower | _: InitCap |
         ...
         _: StringSplitSQL => false
    case _ => true
  }

  private def getDataType(expression: Expression): Option[DataType] = {
    try {
      Some(expression.dataType)
    } catch {
