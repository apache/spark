...
import com.github.luben.zstd.{Zstd, ZstdInputStreamNoFinalizer, ZstdOutputStreamNoFinalizer}
...
@ExpressionDescription(
  usage = """
    _FUNC_(expr[, level[, streaming_mode]]) - Returns a compressed value of `expr` using Zstandard with the specified compression `level`.
      The default level is 3. Uses single-pass mode by default.
  """,
  arguments = """
    Arguments:
      * expr - The binary value to compress.
      * level - Optional integer argument that represents the compression level. The compression level controls the trade-off between compression speed and compression ratio.
                Valid values: between 1 and 22 inclusive, where 1 means fastest but lowest compression ratio, and 22 means slowest but highest compression ratio.
                The default level is 3 if not specified.
      * streaming_mode - Optional boolean argument that represents whether to use streaming mode. If true, the function will compress the input in streaming mode.
...
// scalastyle:off line.size.limit
/**
 * A function that decompress input using Zstandard. On decompression failure, it throws an exception.
 * If either argument is NULL, the return value is NULL.
 */
@ExpressionDescription(
  usage = """
  ...
  examples = """
    Examples:
      > SELECT string(_FUNC_(unbase64("KLUv/SCCpQAAaEFwYWNoZSBTcGFyayABABLS+QU=")));
...
extends UnaryExpression
  ...
  override protected def nullSafeEval(input: Any): Any = {
    ...
    if (result == null) {
      throw QueryExecutionErrors.zstdDecompressError()
    }
    result
  }
...
// scalastyle:off line.size.limit
/**
 * A function that decompress input using Zstandard. On decompression failure, it returns NULL.
 * If either argument is NULL, the return value is NULL.
 */
@ExpressionDescription(
  usage = """
...
case class TryZstdDecompress(input: Expression)
  ...
  override def prettyName: String = "try_zstd_decompress"
  ...
  override protected def nullSafeEval(input: Any): Any = {
    ZstdUtils.decompress(input.asInstanceOf[Array[Byte]])
  }

  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val clz = ZstdUtils.getClass.getName.stripSuffix("$")
    nullSafeCodeGen(ctx, ev, f = (input) => {
      s"${ev.value} = $clz.decompress($input);" +
...
/**
 * ZstdUtils provides utility functions for Zstandard compression and decompression.
 */
object ZstdUtils {
  /**
   * Compresses the input using Zstandard.
   */
  def compress(input: Array[Byte], level: Int, streamingMode: Boolean): Array[Byte] = {
    if (streamingMode) {
      compressInStreamingMode(input, level)
    } else {
      compressInSinglePassMode(input, level)
    }
  }
  ...
  /**
   * Compresses the input using Zstandard in single-pass mode.
   */
  private def compressInSinglePassMode(input: Array[Byte], level: Int): Array[Byte] = {
    Zstd.compress(input, level)
  }

  /**
   * Compresses the input using Zstandard in streaming mode.
   */
  private def compressInStreamingMode(input: Array[Byte], level: Int): Array[Byte] = {
    val outputStream = new ByteArrayOutputStream()
    val zstdStream = new ZstdOutputStreamNoFinalizer(outputStream, level)
    zstdStream.write(input)
    zstdStream.flush()
    // Close zstdStream before returning the compressed data, as closing it would add some ending
    // bytes to the result.
    zstdStream.close()
  ...
  /**
   * Decompresses the input using Zstandard in single-pass mode.
   */
  private def decompressInSinglePassMode(input: Array[Byte], destSize: Int): Array[Byte] = {
    val dest = new Array[Byte](destSize)
    val returnCode = Zstd.decompress(dest, input)
  ...
  /**
   * Decompresses the input using Zstandard in streaming mode.
   */
  private def decompressInStreamingMode(input: Array[Byte]): Array[Byte] = {
    val zstdStream = new ZstdInputStreamNoFinalizer(new ByteArrayInputStream(input))
    try {
      zstdStream.setContinuous(true)
      readAllBytes(zstdStream)
    } catch {
   ...
   * This method is an alternative to `InputStream.readAllBytes()` which is only available in
   * Java 9+.
   */
  @throws[IOException]
  private def readAllBytes(inputStream: InputStream): Array[Byte] = {
    ...
    try {
      val outputStream = new ByteArrayOutputStream()
      while ({ readLen = inputStream.read(buf, 0, bufLen); readLen != -1 }) {
