...
class HavingResolver(resolver: Resolver, expressionResolver: ExpressionResolver)
   ...
   *   2. After resolving the child, transform the operator from [[UnresolvedHaving]] to
   *      [[Filter]]. This is needed because result of the resolution is always a [[Filter]]
   *      (either on [[Aggregate]] or on [[Project]] depending on whether there are LCAs in the
   *      underlying [[Aggregate]]) and with this transformation we can avoid unnecessary checks
   *      for [[UnresolvedHaving]] in the rest of the code. Note that there is no `having` in
   *      Dataframes so unusual patterns are not expected.
  ...
  override def resolve(unresolvedHaving: UnresolvedHaving): LogicalPlan = {
    ...
    val (resolvedCondition, missingExpressions) = resolvedChild match {
      case _ @(_: Project | _: Aggregate) if scopes.current.baseAggregate.isDefined =>
        handleAggregateBelowHaving(scopes.current.baseAggregate.get, partiallyResolvedCondition)
      case other =>
    ...
    val resolvedHaving =
      partiallyResolvedHaving.copy(child = finalChild, condition = finalCondition)
  ...
  /**
   * Expression that are not resolved using the main output have to added to operators below (and
   * original output should be retained by placing a [[Project]] operator above the [[Filter]]
   * operator) for which we have three cases:
   *
   ...
   *      [[GroupingAndAggregateExpressionsExtractor]]. Example:
   *
   * {{{SELECT col1 FROM VALUES(1, 2) GROUP BY col1 HAVING max(col2) > 1;}}}
   *
   * Plan would be: Project [col1#1] +- Filter (max(col2)#2 > 1) +- Aggregate [col1#1], [col1#1,
   * max(col2#3) AS max(col2#3)#2] +- LocalRelation [col1#1, col2#3]
   *
   *   2. Expression is resolved using the [[Aggregate.groupingExpressions]]. These expression
   *      list is created while extracting grouping and aggregate expressions in the
   *      [[GroupingAndAggregateExpressionsExtractor]] by the
   *      [[UnresolvedHaving.havingCondition]]. Example:
   *
   * {{{SELECT col1 FROM VALUES(1, 2) GROUP BY col1, col2 + 1 order by col2 + 1;}}}
   *
   * Plan would be: Project [col1#1] +- Sort [(col2#2 + 1)#3 ASC NULLS FIRST], true +- Aggregate
   * [col1#1, (col2#2 + 1)], [col1#1, (col2#2 + 1) AS (col2#2 + 1)#3] +- LocalRelation [col1#1,
   * col2#2]
   *
   ...
   *
   * Plan would be: Project [col1#1] +- Filter 1 IN (list#3 [min(col1#1)#2]) : +- Aggregate
   * [outer(min(col1#1)#2) AS min(outer(col1))#4] : +- OneRowRelation +- Aggregate [col1#1],
   * [col1#1, min(col1#1) AS min(col1#1)#2] +- LocalRelation [col1#1]
   *
  ...
  private def extractReferencedGroupingAndAggregateExpressions(
    ...
    val resolvedCondition = condition.transformDown { case expression =>
      groupingAndAggregateExpressionsExtractor
