...
object ParseMode extends Logging {
   ...
   */
  def fromString(mode: String): ParseMode = Option(mode)
    .map { v =>
      v.toUpperCase(Locale.ROOT) match {
        case PermissiveMode.name => PermissiveMode
        case DropMalformedMode.name => DropMalformedMode
        case FailFastMode.name => FailFastMode
        case _ =>
          logWarning(
            log"${MDC(PARSE_MODE, v)} is not a valid parse mode. " +
    ...
    .getOrElse {
      logWarning(
        log"mode is null and not a valid parse mode. " +
          log"Using ${MDC(PARSE_MODE, PermissiveMode.name)}.")
      PermissiveMode
    }
}
