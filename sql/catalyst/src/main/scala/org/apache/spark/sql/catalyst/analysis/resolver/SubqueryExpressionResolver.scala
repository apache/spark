...
import org.apache.spark.sql.catalyst.expressions.{Exists, Expression, InSubquery, ListQuery, ScalarSubquery, SubExprUtils, SubqueryExpression}
...
class SubqueryExpressionResolver(expressionResolver: ExpressionResolver, resolver: Resolver)
  ...
  def resolveScalarSubquery(unresolvedScalarSubquery: ScalarSubquery): Expression = {
    ...
    val coercedScalarSubquery = coerceExpressionTypes(
      ...
      expressionTreeTraversal = traversals.current).asInstanceOf[ScalarSubquery]
  ...
  def resolveInSubquery(unresolvedInSubquery: InSubquery): Expression = {
    ...
    val coercedInSubquery = coerceExpressionTypes(
      ...
      expressionTreeTraversal = traversals.current).asInstanceOf[InSubquery]
  ...
  def resolveExists(unresolvedExists: Exists): Expression = {
    ...
    val coercedExists = coerceExpressionTypes(
      ...
      expressionTreeTraversal = traversals.current).asInstanceOf[Exists]
   ...
   *   - Fresh [[CteScope]] with the root flag set so that [[CTERelationDefs]] are merged under
   *     the subquery root.
  ...
  private def resolveSubqueryExpressionPlan(
    ...
    val resolvedSubqueryExpressionPlan =
      try {
  ...
  /**
   * Generically validates [[SubqueryExpression]]. Should not be done in the main pass, because
   * the logic is too sophisticated.
   */
