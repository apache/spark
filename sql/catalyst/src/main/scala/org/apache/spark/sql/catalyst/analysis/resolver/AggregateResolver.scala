...
import org.apache.spark.sql.catalyst.expressions.{Alias, AliasHelper, AttributeReference, Expression, ExprId, ExprUtils}
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, LogicalPlan}

/**
 * Resolves an [[Aggregate]] by resolving its child, aggregate expressions and grouping
 * expressions. Updates the [[NameScopeStack]] with its output and performs validation related to
...
class AggregateResolver(operatorResolver: Resolver, expressionResolver: ExpressionResolver)
   ...
   *   2. Clear [[NameScope.availableAliases]]. Those are only relevant for the immediate
   *      aggregate expressions for output prioritization to work correctly in
   *      [[NameScope.tryResolveMultipartNameByOutput]].
   *   3. Resolve aggregate expressions using [[ExpressionResolver.resolveAggregateExpressions]]
   *      and set [[NameScope.ordinalReplacementExpressions]] for grouping expressions resolution.
   *   4. If there's just one [[UnresolvedAttribute]] with a single-part name "ALL", expand it
   *      using aggregate expressions which don't contain aggregate functions. There should not
   *      exist a column with that name in the lower operator's output, otherwise it takes
   *      precedence.
   *   5. Resolve grouping expressions using [[ExpressionResolver.resolveGroupingExpressions]].
   *      This includes alias references to aggregate expressions, which is done in
   *      [[NameScope.resolveMultipartName]] and replacing [[UnresolvedOrdinals]] with
   *      corresponding expressions from aggregate list, done in [[OrdinalResolver]].
   ...
   *
   * Here we end up having inner [[Alias]]es in both the grouping and aggregate expressions lists
   * which are uncomparable because they have different expression IDs (thus we have to strip
   * them).
   *
   * If the resulting [[Aggregate]] contains lateral columns references, delegate the resolution
   * of these columns to [[LateralColumnAliasResolver.handleLcaInAggregate]]. Otherwise, validate
   * the [[Aggregate]] using the [[ExprUtils.assertValidAggregation]], update the `scopes` with
   * the output of [[Aggregate]] and return the result.
   */
      ...
      try {
        ...
        val resolvedGroupingExpressions =
          if (canGroupByAll(unresolvedAggregate.groupingExpressions)) {
            tryResolveGroupByAll(resolvedAggregateExpressions, unresolvedAggregate)
          } else {
        ...
        val resolvedGroupingExpressionsWithoutAliases =
          resolvedGroupingExpressions.map(trimAliases)
        ...
        if (resolvedAggregateExpressions.hasLateralColumnAlias) {
          val aggregateWithLcaResolutionResult =
            lcaResolver.handleLcaInAggregate(resolvedAggregate)
   ...
   * Example 4:
   *
   * {{{SELECT col1, col2 + 1, COUNT(col1 + 1) FROM VALUES(1, 2) GROUP BY ALL;}}} this one should
   * be grouped by keyword `ALL`. It means that the grouping expressions list is going to contain
   * all the aggregate expressions that don't have aggregate expressions in their subtrees. The
   * grouping expressions list will be [col1, col2 + 1], and COUNT(col1 + 1) won't be included,
   * being an [[AggregateExpression]].
   *
   * Example 5:
   *
   * {{{SELECT col1 AS b, sum(col2) + col1 FROM VALUES (1, 2) GROUP BY ALL;}}} this one should be
   * grouped by keyword `ALL`. It means that the grouping expressions list is going to contain all
   * the aggregate expressions that don't have aggregate expressions in their subtrees. The
   * grouping expressions list will be [col1 AS `col1`]. All the [[Alias]]es should be stripped in
   * order to pass logical plan comparison and to prevent unintentional exceptions from being
   * thrown by [[ExprUtils.assertValidAggregation]], so the final grouping expressions list will
   * be [col1].
   */
  ...
  private def canGroupByAll(expressions: Seq[Expression]): Boolean = {
    ...
    isOrderByAll && scopes.current
      .resolveMultipartName(Seq("ALL"), canReferenceAggregateExpressionAliases = true)
      .candidates
      .isEmpty
  }
