...
case class ArraysOverlap(left: Expression, right: Expression)
  ...
  @transient private lazy val doEvaluation = if (TypeUtils.typeSupportsEquals(elementType)) {
    fastEval _
  } else {
    bruteForceEval _
  }
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    nullSafeCodeGen(ctx, ev, (a1, a2) => {
      ...
      val comparisonCode = if (TypeUtils.typeSupportsEquals(elementType)) {
...
case class ArrayUnion(left: Expression, right: Expression) extends ArraySetLike
  ...
  @transient lazy val evalUnion: (ArrayData, ArrayData) => ArrayData = {
    if (TypeUtils.typeSupportsEquals(elementType)) {
      (array1, array2) =>
