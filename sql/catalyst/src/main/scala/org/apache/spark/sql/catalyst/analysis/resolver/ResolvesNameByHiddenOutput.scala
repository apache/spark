...
trait ResolvesNameByHiddenOutput {
   ...
   * original output list, but before any metadata columns that have been added as part of
   * resolution from hidden output.
   */
  ...
  private def expandOperatorsOutputList(
    ...
    val (metadataCols, nonMetadataCols) = operator match {
      case project: Project =>
        project.projectList.partition(_.toAttribute.isMetadataCol)
      case aggregate: Aggregate =>
        aggregate.aggregateExpressions.partition(_.toAttribute.isMetadataCol)
    }
    ...
    newOperator.copyTagsFrom(operator)
    newOperator
  }
   ...
   * all metadata columns from hidden output, because they may be needed in upper operators (if
   * not, they will be pruned away in [[PruneMetadataColumns]]).
   */
