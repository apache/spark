...
class AliasResolver(expressionResolver: ExpressionResolver)
  ...
  override def resolve(unresolvedAlias: UnresolvedAlias): NamedExpression =
    scopes.current.lcaRegistry.withNewLcaScope(isTopLevelAlias =
      expressionResolutionContextStack.peek().isTopOfProjectList) {
   ...
   * This is only the case with partially resolved DataFrame logical plans. We need to
   * deprioritize those aliases. See [[ExpressionIdAssigner.mapExpression]] doc for more details.
   */
  def handleResolvedAlias(alias: Alias): Alias = {
    val resolvedAlias = scopes.current.lcaRegistry.withNewLcaScope(isTopLevelAlias =
      expressionResolutionContextStack.peek().isTopOfProjectList) {
      val aliasWithResolvedChildren =
   ...
   * the plan is:
   *
   * Project[ Alias("alias_2")( Alias("alias_1")(id1) )(id2) ]( ... )
   *
   * and after the `collapseAlias` call (removing the bottom one) it would be:
   *
   * Project[ Alias("alias_2")(id2) ]( ... )
   */
