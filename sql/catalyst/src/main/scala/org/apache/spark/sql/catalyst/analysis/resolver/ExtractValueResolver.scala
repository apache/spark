...
class ExtractValueResolver(expressionResolver: ExpressionResolver)
   ...
   *
   * When the base is a `NullType` (e.g. from schema evolution with missing columns),
   * [[ExtractValue.applyOrNull]] propagates NULL by returning a bare `Literal(null, NullType)`
   * instead of an [[ExtractValue]]. That literal would otherwise be auto-aliased as `NULL`,
   * diverging from the fixed-point analyzer which aliases it with the extraction's text
   * (e.g. `col[0]`) in [[ColumnResolutionHelper]]. We mirror that here so both analyzers produce
   * the same output name; [[CleanupAliases]] trims the alias where it is not a top-level
   * projection output.
   */
  def resolve(unresolvedExtractValue: UnresolvedExtractValue): Expression = {
    ...
    val resolvedExtractValue = ExtractValue.applyOrNull(
      child = resolvedChild,
    ...
    resolvedExtractValue match {
      ...
      case nullExtraction if resolvedChild.dataType == NullType =>
        autoGeneratedAliasProvider.newAlias(
          child = nullExtraction,
          name = Some(
            toPrettySQL(
              unresolvedExtractValue.copy(
                child = resolvedChild,
                extraction = resolvedExtractionKey
              )
            )
