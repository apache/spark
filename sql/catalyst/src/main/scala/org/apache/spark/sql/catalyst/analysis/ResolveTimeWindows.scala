...
/**
 * Maps a time column to multiple time windows using the Expand operator. Since it's non-trivial
 * to figure out how many windows a time column can map to, we over-estimate the number of windows
 * and filter out the rows where the time column is not inside the time window.
 */
object TimeWindowing extends Rule[LogicalPlan] {
   ...
   * knowing what the timestamp value is, it's non-trivial to figure out deterministically how
   * many window ranges a timestamp will map to given all possible combinations of a window
   * duration, slide duration and start time (offset). Therefore, we express and over-estimate the
   * number of windows there may be, and filter the valid windows. We use last Project operator to
   * group the window columns into a struct so they can be accessed as `window.start` and
   * `window.end`.
   *
   * The windows are calculated as below: maxNumOverlapping <- ceil(windowDuration /
   * slideDuration) for (i <- 0 until maxNumOverlapping) remainder <- (timestamp - startTime) %
   * slideDuration lastStart <- timestamp - ((remainder < 0) ? remainder + slideDuration :
   * remainder) windowStart <- lastStart - i * slideDuration windowEnd <- windowStart +
   * windowDuration return windowStart, windowEnd
   *
   * This behaves as follows for the given parameters for the time: 12:05. The valid windows are
   * marked with a +, and invalid ones are marked with a x. The invalid ones are filtered using
   * the Filter operator. window: 12m, slide: 5m, start: 0m :: window: 12m, slide: 5m, start: 2m
   * 11:55 - 12:07 + 11:52 - 12:04 x 12:00 - 12:12 + 11:57 - 12:09 + 12:05 - 12:17 + 12:02 - 12:14
   * +
   *
   * @param plan
   *   The logical plan
   * @return
   *   the logical plan that will generate the time windows using the Expand operator, with the
   *   Filter operator for correctness and Project for usability.
   */
  def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsUpWithPruning(_.containsPattern(TIME_WINDOW), ruleId) {
      case p: LogicalPlan if p.children.size == 1 =>
        val child = p.children.head
        val windowExpressions =
          p.expressions.flatMap(_.collect { case t: TimeWindow => t }).toSet

        val numWindowExpr = p.expressions
          .flatMap(_.collect {
        ...
        if (numWindowExpr == 1 && windowExpressions.nonEmpty &&
          ...
          if (StructType.acceptsType(window.timeColumn.dataType)) {
            p.transformExpressions { case t: TimeWindow =>
              t.copy(timeColumn = WindowTime(window.timeColumn))
            }
          } else {
            ...
            def getWindow(i: Int, dataType: DataType): Expression = {
              ...
              val lastStart = timestamp - CaseWhen(
                Seq((LessThan(remainder, 0), remainder + window.slideDuration)),
                Some(remainder))
            ...
            val windowAttr =
              AttributeReference(WINDOW_COL_NAME, window.dataType, metadata = newMetadata)()
            ...
            if (window.windowDuration == window.slideDuration) {
              ...
              val replacedPlan = p transformExpressions { case t: TimeWindow =>
                windowAttr
              }
              ...
              replacedPlan.withNewChildren(
                Project(windowStruct +: child.output, Filter(filterExpr, child)) :: Nil)
            } else {
              val overlappingWindows =
                math.ceil(window.windowDuration * 1.0 / window.slideDuration).toInt
              val windows =
                Seq.tabulate(overlappingWindows)(i => getWindow(i, window.timeColumn.dataType))
              ...
              val substitutedPlan =
                Filter(filterExpr, Expand(projections, windowAttr +: child.output, child))

              val renamedPlan = p transformExpressions { case t: TimeWindow =>
                windowAttr
              }
...
object SessionWindowing extends Rule[LogicalPlan] {
  ...
  /**
   * Generates the logical plan for generating session window on a timestamp column. Each session
   * window is initially defined as [timestamp, timestamp + gap).
   *
   * This also adds a marker to the session column so that downstream can easily find the column
   * on session window.
   */
  def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsUpWithPruning(_.containsPattern(SESSION_WINDOW), ruleId) {
      case p: LogicalPlan if p.children.size == 1 =>
        val child = p.children.head
        ...
        if (numWindowExpr == 1 && sessionExpressions.nonEmpty &&
          ...
          if (StructType.acceptsType(session.timeColumn.dataType)) {
            p transformExpressions { case t: SessionWindow =>
              t.copy(timeColumn = WindowTime(session.timeColumn))
            }
          } else {
            ...
            val sessionAttr =
              AttributeReference(SESSION_COL_NAME, session.dataType, metadata = newMetadata)()
            ...
            val sessionEnd = PreciseTimestampConversion(
              session.timeColumn + gapDuration,
              session.timeColumn.dataType,
              LongType)
            ...
            val sessionStruct = Alias(literalSessionStruct, SESSION_COL_NAME)(
              exprId = sessionAttr.exprId,
              explicitMetadata = Some(newMetadata))

            val replacedPlan = p transformExpressions { case s: SessionWindow =>
              sessionAttr
            }
            ...
            replacedPlan.withNewChildren(
              Filter(filterExpr, Project(sessionStruct +: child.output, child)) :: Nil)
          }
        } else if (numWindowExpr > 1) {
          throw QueryCompilationErrors.multiTimeWindowExpressionsNotSupportedError(p)
        } else {
...
/**
 * Resolves the window_time expression which extracts the correct window time from the window
 * column generated as the output of the window aggregating operators. The window column is of
 * type struct { start: TimestampType, end: TimestampType }. The correct representative event time
 * of a window is ``window.end - 1``.
 */
object ResolveWindowTime extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsUpWithPruning(_.containsPattern(WINDOW_TIME), ruleId) {
      case p: LogicalPlan if p.children.size == 1 =>
        val child = p.children.head
        ...
        if (windowTimeExpressions.nonEmpty && allWindowTimeExprsResolved) {
          ...
          val replacedPlan = p transformExpressions { case w: WindowTime =>
            windowTimeToAttrAndNewColumn(w)._1
          }

          val newColumnsToAdd = windowTimeToAttrAndNewColumn.values.map(_._2)
          replacedPlan.withNewChildren(Project(newColumnsToAdd ++: child.output, child) :: Nil)
        } else {
          p // Return unchanged. Analyzer will throw exception later
        }
    }
