...
object GenerateUnsafeProjection extends CodeGenerator[Seq[Expression], UnsafeProjection] {
  ...
  private def writeExpressionsToBuffer(
    ...
    val rowWriterAccessor = ctx.addMutableState(rowWriterClass, rowWriter,
    ...
    val resetWriter = if (isTopLevel) {
      ...
      if (inputs.map(_.isNull).forall(_ == "false")) {
        ...
        s"$rowWriterAccessor.zeroOutNullBytes();"
      }
    } else {
      s"$rowWriterAccessor.reset();"
    }
      ...
      case ((input, dataType), index) =>
        ...
        val setNull = dt match {
          case t: DecimalType if t.precision > Decimal.MAX_LONG_DIGITS =>
            ...
            s"$rowWriterAccessor.write($index, (Decimal) null, ${t.precision}, ${t.scale});"
          case _ => s"$rowWriterAccessor.setNullAt($index);"
        }
        ...
        val writeField = dt match {
          case t: StructType =>
            s"""
              ...
              $rowWriterAccessor.setOffsetAndSize(
                $index, $tmpCursor, $bufferHolder.cursor - $tmpCursor);
            """
          ...
          case t: DecimalType =>
            s"$rowWriterAccessor.write($index, ${input.value}, ${t.precision}, ${t.scale});"

          case NullType => ""

          case _ => s"$rowWriterAccessor.write($index, ${input.value});"
        }
  ...
  private def writeArrayToBuffer(
    ...
    val arrayWriterAccessor = ctx.addMutableState(arrayWriterClass, arrayWriter,
    ...
    val writeElement = et match {
      case t: StructType =>
        s"""
          ...
          $arrayWriterAccessor.setOffsetAndSize(
            $index, $tmpCursor, $bufferHolder.cursor - $tmpCursor);
        """
      ...
      case t: DecimalType =>
        s"$arrayWriterAccessor.write($index, $element, ${t.precision}, ${t.scale});"

      case NullType => ""

      case _ => s"$arrayWriterAccessor.write($index, $element);"
    }
    ...
    s"""
      if ($input instanceof UnsafeArrayData) {
        ...
        $arrayWriterAccessor.initialize($bufferHolder, $numElements, $elementOrOffsetSize);

        for (int $index = 0; $index < $numElements; $index++) {
          if ($input.isNullAt($index)) {
            $arrayWriterAccessor.setNull$primitiveTypeName($index);
          } else {
  ...
  def createCode(
    ...
    val holderAccessor = ctx.addMutableState(holderClass, holder,
      s"$holder = new $holderClass($resultAccessor, ${numVarLenFields * 32});")

    val resetBufferHolder = if (numVarLenFields == 0) {
      ""
    } else {
      s"$holderAccessor.reset();"
    }
    val updateRowSize = if (numVarLenFields == 0) {
      ""
    } else {
      s"$resultAccessor.setTotalSize($holderAccessor.totalSize());"
    }
    ...
    val writeExpressions = writeExpressionsToBuffer(
      ctx, ctx.INPUT_ROW, exprEvals, exprTypes, holderAccessor, isTopLevel = true)
    ...
    val code =
      s"""
        ...
        $updateRowSize
      """
    ExprCode(code, "false", resultAccessor)
  }
