...
/**
 * This rule tries to merge multiple subplans that have one row result. This can be either the
 * plan tree of a [[ScalarSubquery]] expression or the plan tree starting at a non-grouping
 * [[Aggregate]] node.
 *
 * The process is the following:
 *   - While traversing through the plan each one row returning subplan is tried to merge into
 *     already seen one row returning subplans using `PlanMerger`s. During this first traversal
 *     each [[ScalarSubquery]] expression is replaced to a temporal [[ScalarSubqueryReference]]
 *     and each non-grouping [[Aggregate]] node is replaced to a temporal
 *     [[NonGroupingAggregateReference]] pointing to its possible merged version in `PlanMerger`s.
 *     `PlanMerger`s keep track of whether a plan is a result of merging 2 or more subplans, or is
 *     an original unmerged plan. [[ScalarSubqueryReference]]s and
 *     [[NonGroupingAggregateReference]]s contain all the required information to either restore
 *     the original subplan or create a reference to a merged CTE.
 ...
 *     original to a modified ones that reference a CTE with a merged plan. A modified
 *     [[ScalarSubquery]] is constructed like:
 ...
 * be rewritten as one aggregate without the need to introduce a CTE and keeping the join. But
 * there are more complex cases when this CTE based approach is the only viable option. Such cases
 * include when the aggregates reside at different parts of plan, maybe even in different subquery
 * expressions.
 */
object MergeSubplans extends Rule[LogicalPlan] {
        ...
        if (mergedPlan.merged) {
          CTERelationDef(
            Project(
              Seq(
                Alias(
                  CreateNamedStruct(planWithoutReferences.output.flatMap(a =>
  ...
  private def insertReferences(
    ...
    val (planWithReferences, level) = nodeSubqueriesWithReferences match {
      case a: Aggregate if !root && a.groupingExpressions.isEmpty =>
        ...
        val outputIndices =
          mergedOutput.map(a =>
            mergeResult.mergedPlan.plan.output.indexWhere(_.exprId == a.exprId))
  ...
  private def removeReferences(
    ...
    plan.transformUpWithPruning(
      ...
      case o =>
        o.transformExpressionsUpWithPruning(_.containsPattern(SCALAR_SUBQUERY_REFERENCE)) {
 ...
 *
 * @param level
 *   The level of the replaced subquery. It defines the `PlanMerger` instance into which the
 *   subquery is merged.
 * @param mergedPlanIndex
 *   The index of the merged plan in the `PlanMerger`.
 * @param outputIndex
 *   The index of the output attribute of the merged plan.
 * @param dataType
 *   The dataType of original scalar subquery.
 * @param exprId
 *   The expression id of the original scalar subquery.
 */
 ...
 *
 * @param level
 *   The level of the replaced aggregate. It defines the `PlanMerger` instance into which the
 *   aggregate is merged.
 * @param mergedPlanIndex
 *   The index of the merged plan in the `PlanMerger`.
 * @param outputIndices
 *   The indices of the output attributes of the merged plan.
 * @param output
 *   The output of original aggregate.
 */
