...
object CrossJoinArrayContainsToInnerJoin extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def buildPlan(
    ...
    val innerJoin = Join(newLeft, newRight, Inner, Some(joinCond), JoinHint.NONE)

    // Project to original output (exclude unnested column)
    val projected = Project(join.output.map(a => Alias(a, a.name)(a.exprId)), innerJoin)

    // Add remaining predicates if any
    val result = remaining.reduceLeftOption(And).map(Filter(_, projected)).getOrElse(projected)
    Some(result)
  }
}
