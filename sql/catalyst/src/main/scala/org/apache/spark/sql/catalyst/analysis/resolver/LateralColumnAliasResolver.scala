...
import org.apache.spark.sql.catalyst.expressions.{Alias, Expression, ExprId, ExprUtils, NamedExpression, ScalarSubquery}
...
class LateralColumnAliasResolver(expressionResolver: ExpressionResolver) extends QueryErrorsBase {
   ...
   *     eliminate lateral column references from the [[Aggregate]] (detailed description can be
   *     found in `extractLcaAndReplaceAggWithProject`).
   ...
   *     to collect all aliases from [[Aggregate]], as well as any aliases from artificially
   *     inserted [[Project]] nodes.
  ...
  /**
   * Builds a multi-level [[Project]] with all lateral column aliases and their dependencies.
   * First, from top scope, we acquire dependency levels of all aliases. Dependency level is
   * defined as a number of attributes that an attribute depends on in the lateral alias reference
   * chain. For example, in a query like:
   *
   ...
   *     in the project list based on `remappedAliases` (detailed description in
   *     `updateAliasTrees`). `remappedAliases` should be None if we are resolving LCAs in
   *     [[Project]].
   *   - `originalProjectList` is placed on top of the multi-level [[Project]]. Any aliases that
   *     have been laterally referenced need to be replaced with only their names. This is because
   *     their full definitions ( `attr` as `name` ) have already been defined on lower levels.
   *   - If an attribute is never referenced, it does not show up in multi-level project lists,
   *     but instead only in the top-most [[Project]].
   ...
   *
   * Project [a, b, 2 AS c, d, a AS e, d AS f, g, h, h AS i] +- Project [a, b, d, g, g AS h] +-
   * Project [a, b, b AS d, a AS g] +- Project [0 AS a, 1 AS b] +- OneRowRelation
   */
  def buildProjectWithResolvedLca(
    ...
    val (finalChildPlan, _) =
      aliasDependencyMap.asScala.foldLeft((resolvedChild, firstIterationProjectList)) {
   ...
   * following:
   *   - Extracts the aggregate and grouping expressions from the [[Aggregate]] and places them in
   *     a new [[Aggregate]] operator with aliases on top of them. If an expression is already a
   *     [[NamedExpression]] we don't need to alias it again.
   *   - Places a [[Project]] on top of the new [[Aggregate]] operator, where the project list
   *     will be created from [[Alias]] references to original aggregate expressions.
   ...
   *
   * !Project [1 AS a#4, (a#4 + 1) AS b#5, col1#3, max(col1)#11 AS max(col1)#10] +- Aggregate
   * [col1#3], [col1#3, max(col1#3) AS max(col1)#11] +- LocalRelation [col1#3]
   *
  ...
  private def extractLcaAndReplaceAggWithProject(
    ...
    val groupingExpressionSemanticComparator = new SemanticComparator(
    ...
    val extractedExpressions = aggregate.aggregateExpressions
      .map(aggregateExpression =>
        extractAggregateAndGroupingExpressions(
          ...
          newAggregateExpressions = newAggregateExpressions).asInstanceOf[NamedExpression])
   ...
   * to stay compatible with fixed-point when resolving LCAs in [[Aggregate]]. For example,
   ...
   *
   * Project [a#4, (a#4 + 1) AS b#5, max(col1)#11 AS max(col1)#10] +- Project [max(col1)#11, 1 AS
   * a#4] +- Aggregate [col1#3], [max(col1#3) AS max(col1)#11] +- LocalRelation [col1#3]
   *
   * An unresolved [[Aggregate]] will contain an aggregate expression
   * `max(col1#3) as max(col1)#10` with its implicit [[Alias]]. In order to preserve the [[Alias]]
   * with [[ExprId]] #2 we need to place a new [[Alias]] (`max(col1)#11`) on top of the aggregate
   * expression (this is done in `extractLcaAndReplaceAggWithProject`). Now we can place the
   * resulting [[Alias]] in the newly created [[Aggregate]] that contains only aggregate and
   * grouping expressions. However, in order to reference this new [[Alias]] from the [[Project]]
   * that we place on top of the new [[Aggregate]], we need to update the expression tree of
   * `max(col1)#10` [[Alias]] to point to the newly created alias `max(col1)#11` and we get
   * `max(col1)#11 as max(col1)#10` in the final project list.
   */
