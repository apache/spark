...
import org.apache.spark.sql.catalyst.trees.TreePattern.WINDOW_EXPRESSION
import org.apache.spark.sql.errors.{QueryCompilationErrors, QueryErrorsBase}

/**
 * Handles resolution of lateral column references in [[Project]] and [[Aggregate]] operators.
 */
class LateralColumnAliasResolver(expressionResolver: ExpressionResolver, operatorResolver: Resolver)
  ...
  private val aliasProvider = expressionResolver.getAutoGeneratedAliasProvider
   ...
   *  - Stores the original [[Aggregate]] in the current [[OperatorResolutionContext]] so it can be
   *    validated later.
  ...
  def handleLcaInAggregate(resolvedAggregate: Aggregate): AggregateWithLcaResolutionResult = {
    ...
    extractLcaAndReplaceAggWithProject(resolvedAggregate, aliasesToCollect) match {
      case _ @Project(projectList: Seq[_], aggregate: Aggregate) =>
        ...
        val finalOperator = buildOperatorWithResolvedLca(
        ...
        val finalProjectList = finalOperator match {
          case project: Project => project.projectList
          case window: Window => window.projectList
          case _ =>
            throw SparkException.internalError(
              s"Expected final operator to be Project or Window but found:" +
              s" ${finalOperator.treeString}"
            )
        }

        scopes.overwriteCurrent(output = Some(finalProjectList.map(_.toAttribute)))

        AggregateWithLcaResolutionResult(
          resolvedOperator = finalOperator,
          outputList = finalProjectList,
  ...
  /**
   * Builds a multi-level [[Project]]/[[Window]] with all lateral column aliases and their
   * dependencies. First, from top scope, we acquire dependency levels of all aliases. Dependency
   * level is defined as a number of attributes that an attribute depends on in the lateral alias
   * reference chain. For example, in a query like:
   *
   ...
   *  - Collect all aliases from newly created [[Project]] nodes and add them to
   *  `aliasesToCollect`.
   *  - Collect all new aliases and attributes from newly constructed [[Project]] nodes that can be
   *  used as window source expressions.
   *  - Try to build a [[Window]] from current [[Project]] if it contains any window expressions.
   *
   ...
   *          +- OneRowRelation
   */
  def buildOperatorWithResolvedLca(
      ...
      aliasesToCollect: ArrayBuffer[Alias] = ArrayBuffer.empty): LogicalPlan = {
    ...
    val (finalChildPlan, _) = aliasDependencyMap.asScala.foldLeft(
      ...
      case ((currentPlan, currentProjectList), availableAliases) =>
        ...
        if (referencedAliases.nonEmpty) {
          ...
          val newProject = Project(newProjectList, currentPlan)

          collectWindowSourceExpressionsFromLcaProject(newProject)

          tryBuildAndValidateWindow(newProject, resolvedChild)
        } else {
    ...
    val finalProject = Project(finalProjectList, finalChildPlan)

    val (withFinalWindow, _) = tryBuildAndValidateWindow(finalProject, resolvedChild)
  ...
  private def extractLcaAndReplaceAggWithProject(
    ...
    val extractedExpressions = aggregate.aggregateExpressions
      .map(
        aggregateExpression =>
          extractAggregateGroupingAndWindowExpressions(
  ...
  /**
   * Extracts aggregate, grouping and window expressions from an [[Aggregate]] that has lateral
   * column references in order to place them the newly created [[Aggregate]] without LCAs.
   */
  private def extractAggregateGroupingAndWindowExpressions(
    ...
    expression match {
      case windowExpression: WindowExpression =>
        val newWindowFunction = windowExpression.windowFunction.mapChildren(
          child =>
            extractAggregateGroupingAndWindowExpressions(
              expression = child,
        ...
        val newWindowSpec = extractAggregateGroupingAndWindowExpressions(
  ...
  /**
   * Collects window source expressions from a newly constructed [[Project]]. For detailed
   * description, see [[CollectsWindowSourceExpressions]] scaladoc.
   */
  private def collectWindowSourceExpressionsFromLcaProject(project: Project) = {
  ...
  /**
   * If the newly constructed [[Project]] contains any window expressions, delegate building of a
   * [[Window]] operator to [[WindowBuilder]]. Additionally, validate that constructed [[Window]]
   * is not below a HAVING node.
   */
  private def tryBuildAndValidateWindow(
      project: Project,
      originalChild: LogicalPlan): (LogicalPlan, Seq[NamedExpression]) = {
    if (project.projectList.exists(_.containsPattern(WINDOW_EXPRESSION))) {
      val (window, windowProjectList) = WindowBuilder.buildWindow(
        windowResolver = operatorResolver.getWindowResolver,
        originalOperator = project,
        resolvedProjectList = project.projectList,
        childOperator = project.children.head,
        hasCorrelatedScalarSubqueryExpressions = false,
        cteRegistry = cteRegistry
      )

      checkWindowWithLcasUnderHaving(originalChild)

      (window, windowProjectList.expressions)
    } else {
  ...
  /**
   * Throw an error if we are resolving an [[Aggregate]] that has both lateral column aliases and
   * window expressions and is under a HAVING clause.
   */
  private def checkWindowWithLcasUnderHaving(originalChild: LogicalPlan) = {
    originalChild match {
      case _: Aggregate if operatorResolutionContextStack.current.isResolvingTreeUnderHaving =>
        val lca = scopes.current.lcaRegistry.getAllLaterallyReferencedAttributes.head
        val lcaName = lca.qualifier :+ lca.name
