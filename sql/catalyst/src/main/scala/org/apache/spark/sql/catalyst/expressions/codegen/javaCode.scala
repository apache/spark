...
case class CodeBlock(codeParts: Seq[String], blockInputs: Seq[Any]) extends Block {
  override lazy val exprValues: Seq[ExprValue] = {
    blockInputs.flatMap {
...
case class Blocks(blocks: Seq[Block]) extends Block {
  override lazy val exprValues: Seq[ExprValue] = blocks.flatMap(_.exprValues).distinct
...
object EmptyBlock extends Block with Serializable {
  override def code: String = ""
  override val exprValues: Seq[ExprValue] = Seq.empty
