...
object JavaCode {
  ...
  /**
   * Create a default literal. This is null for reference types, false for boolean types and -1
...
trait Block extends TreeNode[Block] with JavaCode {
  ...
  /**
   * Apply a map function to each java expression codes present in this java code, and return a
   * new java code based on the mapped java expression codes.
   */
...
object Block {
  ...
  private def foldLiteralArgs(
      parts: Seq[String],
      args: Seq[Any]): (Seq[String], Seq[JavaCode]) = {
...
/**
 * A block of java code. Including a sequence of code parts and some inputs to this block. The
 * actual java code is generated by embedding the inputs into the code parts. Here we keep inputs
 * of `JavaCode` instead of simply folding them as a string of code, because we need to track
 * expressions (`ExprValue`) in this code block. We need to be able to manipulate the expressions
 * later without changing the behavior of this code block in some applications, e.g., method
 * splitting.
 */
