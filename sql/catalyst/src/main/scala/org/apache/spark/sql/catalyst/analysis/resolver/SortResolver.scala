...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, Expression, ExprId, NamedExpression, SortOrder}
...
class SortResolver(operatorResolver: Resolver, expressionResolver: ExpressionResolver)
   ...
   *      [[ResolveAsAllKeyword]], skip previous step and resolve it as an `ALL` keyword (by
   *      copying child's output and transforming it into attributes).
   *   4. In case there are attributes referenced in the order expressions are resolved using the
   *      hidden output (or in case we order by [[AggregateExpression]]s which are not present in
   *      [[Aggregate.aggregateExpressions]]) update the output of child operator and add a
   ...
   *
   * 'Sort ['col2 ASC NULLS FIRST], true +- 'Project ['col1] +- 'Filter ('col2 > 2) +-
   ...
   *
   * Project [col1] +- Sort [col2 ASC NULLS FIRST], true +- Project [col1, col2] +- Filter (col2 >
   * 2) +- LocalRelation [col1, col2]
   *
   ...
   * Has the following unresolved plan:
   *
   * 'Sort ['SUM('col1) ASC NULLS FIRST, ('col2 + 1) ASC NULLS FIRST], true +- 'Aggregate ['col1,
   * ('col2 + 1)], ['col1] +- LocalRelation [col1, col2]
   *
   ...
   * project list as a top node (`SUM(col2)` is extracted in the [[AggregateExpressionResolver]]
   * whereas `col2 + 1` is extracted using `extractReferencedGroupingAndAggregateExpressions`
   * helper method):
   *
   * Project [col1] +- Sort [sum(col1)#... ASC NULLS FIRST, (col2 + 1)#... ASC NULLS FIRST], true +-
   * Aggregate [col1, (col2 + 1)], [col1, sum(col1) AS sum(col1)#..., (col2 + 1) AS (col2 +
   * 1)#...] +- LocalRelation [col1, col2]
   */
  override def resolve(unresolvedSort: Sort): LogicalPlan = {
    ...
    if (canOrderByAll(unresolvedSort.order)) {
      ...
      val deduplicatedMissingExpressions = deduplicateMissingExpressions(
      ...
      val resolvedSort = unresolvedSort.copy(child = finalChild, order = finalOrderExpressions)
  ...
  /**
   * Extracts the referenced grouping and aggregate expressions from the order expressions. This
   * is used to update the output of the child operator and add a [[Project]] as a parent of
   * [[Sort]] later during the resolution (if needed). Consider the following example:
   * {{{
   ...
   *
   * 'Sort ['col2 ASC NULLS FIRST], true +- 'Aggregate ['col1, 'col2], ['col1] +- LocalRelation
   * [col1, col2]
   *
   * As it can be seen, `col2` (ordering expression) is not present in the [[Aggregate]] operator
   * , and thus we return it from this method. The plan will be altered later during the
   * resolution using the [[ResolvesNameByHiddenOutput]] (`col2` will be added to
   * [[Aggregate.aggregateExpressions]], [[Project]] will be added as a top node with original
   * [[Aggregate]] output) and it will look like:
   *
   * Project [col1] +- Sort [col2 ASC NULLS FIRST], true +- Aggregate [col1, col2], [col1, col2] +-
  ...
  private def extractReferencedGroupingAndAggregateExpressions(
    ...
    val transformedSortOrderEntries = sortOrderEntries.map { sortOrder =>
      sortOrder.copy(child = sortOrder.child.transformDown { case expression: Expression =>
   ...
   * don't exist in child [[Project]] but the aliases do. For more details see
   * [[tryReplaceSortOrderOrHavingConditionWithAlias]].
   */
  private def tryReplaceSortOrderWithAlias(
      orderExpressions: Seq[SortOrder],
      missingExpressions: Seq[NamedExpression]): (Seq[SortOrder], Seq[NamedExpression]) = {
