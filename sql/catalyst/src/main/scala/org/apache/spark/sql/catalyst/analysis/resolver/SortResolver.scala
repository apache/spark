...
class SortResolver(operatorResolver: Resolver, expressionResolver: ExpressionResolver)
  ...
  /**
   * Resolve order expressions of an unresolved [[Sort]], returns both attributes resolved using
   * hidden output and extracted [[AggregateExpression]]s and whether there are grouping analytics
   * expressions in the subtrees of [[SortOrder]] expressions. In case of [[UnresolvedAttribute]]
   * resolution, respect the following order.
   *
  ...
  private def resolveOrderExpressions(
    ...
    val resolvedSortOrder = partiallyResolvedSort.order.map { sortOrder =>
      val (resolvedSortOrder, hasGroupingAnalyticsExpressionInSortOrder) = expressionResolver
        .resolveSortOrderExpression(sortOrder, partiallyResolvedSort)

      hasGroupingAnalyticsExpression |= hasGroupingAnalyticsExpressionInSortOrder
