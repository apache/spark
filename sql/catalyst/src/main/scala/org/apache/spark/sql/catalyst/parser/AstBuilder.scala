...
class AstBuilder extends SqlBaseParserBaseVisitor[AnyRef] with SQLConfHelper with Logging {
  ...
  /**
   * Create a table based on an existing table, returning a [[CreateTableLike]] logical plan.
   *
   * For example:
   * {{{
  ...
  override def visitCreateTableLike(ctx: CreateTableLikeContext): LogicalPlan = withOrigin(ctx) {
    val targetTable = UnresolvedIdentifier(visitMultipartIdentifier(ctx.target))
    val sourceTable = createUnresolvedTable(ctx.source, "CREATE TABLE LIKE")
    ...
    val properties = Option(ctx.tableProps).map(visitPropertyKeyValues).getOrElse(Map.empty)
    val cleanedProperties = cleanTableProperties(ctx, properties)
    CreateTableLike(targetTable, sourceTable, provider, location, serdeInfo, cleanedProperties,
      ctx.EXISTS != null)
  }
