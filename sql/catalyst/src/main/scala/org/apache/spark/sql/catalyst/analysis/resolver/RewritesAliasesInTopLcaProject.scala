...
trait RewritesAliasesInTopLcaProject {
   ...
   *
   * Sort [max(tempresolvedcolumn(col3#5, col3, false)) ASC NULLS FIRST], true +- Aggregate
   * [col2#4], [avg(col1#3) AS a#6, lateralAliasReference(a) AS b#7] +- LocalRelation [col1#3,
   * col2#4, col3#5]
   *
   * After resolving [[TempResolvedColumn]]:
   *
   * Project [a#6, b#7] +- Sort [max(col3)#10 ASC NULLS FIRST], true +- Aggregate [col2#4],
   * [avg(col1#3) AS a#6, lca(a) AS b#7, max(col3#5) AS max(col3)#10] +- LocalRelation [col1#3,
   * col2#4, col3#5]
   *
   * In the above case fixed-point first resolves [[SortOrder]] to `max(col3)#10` and only then
   * resolves LCAs. However, while resolving LCAs in [[Aggregate]], fixed-point first constructs a
   * base [[Aggregate]] by pushing down all aggregate expressions with new aliases. It then places
   * a [[Project]] on top reinstating the original alias on top of a newly created one, in order
   * to still match the attribute reference from [[SortOrder]]:
   *
   * Project [a#6, b#7] +- Sort [max(col3)#10 ASC NULLS FIRST], true +- Project [avg(col1)#11 AS
   * a#6, lca(a) AS b#7, max(col3)#12 AS max(col3)#10] +- Aggregate [col2#4], [avg(col1#3) AS
   * avg(col1)#11, max(col3#5) AS max(col3)#12] +- LocalRelation [col1#3, col2#4, col3#5]
   *
   * In the example above, `max(col3#5)` gets pushed down and aliased as `max(col3)#12`, even
   * though `max(col3)#10` attribute reference already exists. Because of that `max(col3)#12`
   * needs to be remapped back to `max(col3)#10`.
   *
   ...
   *
   * Project [a#6, b#7] +- Sort [max(col3)#16 ASC NULLS FIRST], true +- Project [a#6, a#6 AS b#7,
   * max(col3)#16] +- Project [avg(col1)#14, avg(col1)#14 AS a#6, max(col3)#16] +- Aggregate
   * [col2#4], [avg(col1#3) AS avg(col1)#14, max(col3#5) AS max(col3)#16] +- LocalRelation
   ...
   * because there is no missing input in that case. For example, consider the following query:
   *
   * {{{SELECT col1 AS a, a FROM VALUES(1) GROUP BY col1 ORDER BY col1}}}
   *
   * After resolving lateral column references and partially resolving [[SortOrder]] expression,
   * we get the following plan:
   *
   * !Sort [col1#3 ASC NULLS FIRST], true +- Project [a#4, a#4] +- Project [col1#3, col1#3 AS a#4] +-
   * Aggregate [col1#3], [col1#3] +- LocalRelation [col1#3]
   *
   ...
   *
   * Sort [a#4 ASC NULLS FIRST], true +- Project [a#4, a#4] +- Project [col1#3, col1#3 AS a#4] +-
   * Aggregate [col1#3], [col1#3] +- LocalRelation [col1#3]
   *
   ...
   *
   * Sort [col1#4 ASC NULLS FIRST], true +- Aggregate [col1#4], [col1#4, col1#4 AS a#5] +-
   * LocalRelation [col1#4]
   *
   * In the above example, we do not replace `col1#4` with `a#5` because `col1#4` is present in
   * the output.
   */
  def tryReplaceSortOrderOrHavingConditionWithAlias(
    ...
    val filteredMissingExpressions = missingExpressions.filter(expression =>
      !replacedAttributeReferences.contains(expression.exprId))
  ...
  private def getCandidateExpressionsForRewrite(
      ...
      autoGeneratedAliasProvider: AutoGeneratedAliasProvider)
      : HashMap[ExprId, NamedExpression] = {
  ...
  private def updateAttributeReferencesInExpressions[ExpressionType <: Expression](
      ...
      candidateAliases: HashMap[ExprId, NamedExpression]): Seq[ExpressionType] = {
