...
import org.apache.spark.sql.catalyst.expressions.{Attribute, CurrentDate, CurrentTimestampLike, GroupingSets, MonotonicallyIncreasingID}
    ...
    plan.foreachUp { implicit subPlan =>
      ...
      subPlan.expressions.foreach { e =>
        if (e.collectLeaves().exists {
          case (_: CurrentTimestampLike | _: CurrentDate) => true
          case _ => false
        }) {
