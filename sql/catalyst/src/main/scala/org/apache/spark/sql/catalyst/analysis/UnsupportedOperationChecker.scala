...
import org.apache.spark.sql.catalyst.expressions.{Attribute, CurrentDate, CurrentTimestamp, GroupingSets, LocalTimestamp, MonotonicallyIncreasingID, Now}
    ...
    plan.foreachUp { implicit subPlan =>
      ...
      subPlan.expressions.foreach { e =>
        if (e.collectLeaves().exists {
          case (_: CurrentTimestamp | _: Now | _: CurrentDate | _: LocalTimestamp) => true
          case _ => false
        }) {
