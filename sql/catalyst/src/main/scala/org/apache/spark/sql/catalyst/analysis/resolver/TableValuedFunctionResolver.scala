...
class TableValuedFunctionResolver(
  ...
  def resolve(unresolvedTVF: UnresolvedTableValuedFunction): LogicalPlan = {
    ...
    val resolvedTVF = functionResolution
      .resolveTableFunction(
        unresolvedTVFWithResolvedArguments.name,
        unresolvedTVFWithResolvedArguments.functionArgs
      )
      .getOrElse {
        unresolvedTVFWithResolvedArguments.failAnalysis(
          errorClass = "UNRESOLVABLE_TABLE_VALUED_FUNCTION",
          messageParameters =
            Map("name" -> toSQLId(unresolvedTVFWithResolvedArguments.name))
        )
      }
