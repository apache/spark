  ...
  def microBatchUnsupportedByDataSourceError(
    ...
    new UnsupportedOperationException(
      s"""
         ...
         |
         |Either the data source is disabled at SQLConf.get.DISABLED_V2_STREAMING_MICROBATCH_READERS.key
         |(The disabled sources are [$disabledSources]) or the table $table does not have
         |MICRO_BATCH_READ capability. Meanwhile, the fallback, data source v1, is not available."
       """.stripMargin)
  }
