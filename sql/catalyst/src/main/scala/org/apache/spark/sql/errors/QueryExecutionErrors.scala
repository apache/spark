  ...
  def sparkUpgradeInReadError(
    ...
    new SparkUpgradeException("3.0",
      s"""
         ...
         |files can be ambiguous, as the files may be written by Spark 2.x or legacy versions of Hive,
         |which uses a legacy hybrid calendar that is different from Spark 3.0+'s Proleptic Gregorian
         |calendar. See more details in SPARK-31404. You can set the SQL config '$config' or the
         |datasource option '$option' to 'LEGACY' to rebase the datetime values w.r.t.
         |the calendar difference during reading. To read the datetime values as it is, set the SQL
         |config '$config' or the datasource option '$option' to 'CORRECTED'.
       """.stripMargin, null)
  }
  ...
  def sparkUpgradeInWriteError(format: String, config: String): SparkUpgradeException = {
    new SparkUpgradeException("3.0",
      s"""
         ...
         |if you are 100% sure that the written files will only be read by Spark 3.0+ or other systems
         |that use Proleptic Gregorian calendar.
       """.stripMargin, null)
  }
