...
case class PartialResultException(
    ...
    extends PartialValueException(cause)
 ...
 *   the actual exception about why the parser cannot return full result.
 */
case class PartialResultArrayException(partialResults: Array[InternalRow], cause: Throwable)
    extends Exception(cause)

/**
 * Exception thrown when the underlying parser met a bad record and can't parse it. The stacktrace
 * is not collected for better performance, and thus, this exception should not be used in a
 * user-facing context.
 * @param record
 *   a function to return the record that cause the parser to fail
 * @param partialResults
 *   a function that returns an row array, which is the partial results of parsing this bad
 *   record.
 * @param cause
 *   the actual exception about why the record is bad and can't be parsed. It's better to use
 *   `LazyBadRecordCauseWrapper` here to delay heavy cause construction until it's needed.
 */
...
/**
 * Exception to use as `BadRecordException` cause to delay heavy user-facing exception
 * construction. Does not contain stacktrace and used only for control flow
 */
 ...
 */
case class StringAsDataTypeException(fieldName: String, fieldValue: String, dataType: DataType)
    extends RuntimeException()

/**
 * No-stacktrace equivalent of `QueryExecutionErrors.cannotParseJSONFieldError`. Used for code
 * control flow in the parser without overhead of creating a full exception.
 */
...
/**
 * No-stacktrace equivalent of `QueryExecutionErrors.emptyJsonFieldValueError`. Used for code
 * control flow in the parser without overhead of creating a full exception.
 */
