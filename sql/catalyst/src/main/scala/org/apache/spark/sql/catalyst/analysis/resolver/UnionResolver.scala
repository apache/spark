...
class UnionResolver(resolver: Resolver, expressionResolver: ExpressionResolver)
    extends TreeNodeResolver[Union, LogicalPlan] {
   ...
   *  - Return the resolved [[Union]] with new children optionally wrapped in [[WithCTE]]. See
   *    [[CteScope]] scaladoc for more info.
   */
  override def resolve(unresolvedUnion: Union): LogicalPlan = {
    val (resolvedChildren, childOutputs) = resolveUnionChildren(unresolvedUnion)
    ...
    val output = Union.mergeChildOutputs(newChildOutputs)
    scopes.overwriteCurrent(output = Some(output), hiddenOutput = Some(output))

    cteRegistry.currentScope.tryPutWithCTE(
      unresolvedOperator = unresolvedUnion,
      resolvedOperator = unresolvedUnion.copy(children = newChildren)
    )
  }

  /**
   * Resolve `unresolvedUnion`'s children in the context of new [[NameScope]],
   ...
   * the only child whose expression IDs get propagated upwards through [[Union]]. This is an
   * optimization to avoid fast-growing expression ID mappings.
   */
  private def resolveUnionChildren(
      unresolvedUnion: Union): (Seq[LogicalPlan], Seq[Seq[Attribute]]) = {
