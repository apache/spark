...
case class JsonTuple(children: Seq[Expression])
  ...
  override def elementSchema: StructType = StructType(children.tail.zipWithIndex.map {
  ...
  @transient
  private lazy val evaluator: JsonTupleEvaluator = JsonTupleEvaluator(children)
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val jsonEval = children.head.genCode(ctx)
    val fieldsTerm = ctx.freshName("fields")
    val fieldsEval = children.tail.map(_.genCode(ctx))
    ...
    val jsonEvalCode =
      s"""
         ...
         |}
         |""".stripMargin
    val fieldsEvalCode = fieldsEval.zipWithIndex.map {
      case (fieldEval, idx) =>
        s"""
           |if (${fieldEval.isNull}) {
           |  $fieldsTerm[$idx] = null;
           |} else {
           |  $fieldsTerm[$idx] = ${fieldEval.value}.toString();
           |}
           |""".stripMargin
    }
    ev.copy(code =
      code"""
         ...
         |String[] $fieldsTerm = new String[${children.size - 1}];
         |${jsonEval.code}
         |${fieldsEval.map(_.code).mkString("\n")}
         |$jsonEvalCode
         |${fieldsEvalCode.mkString("\n")}
         |boolean ${ev.isNull} = false;
         |$wrapperClass<InternalRow> ${ev.value} = $refEvaluator.evaluate($jsonTerm, $fieldsTerm);
         """.stripMargin)
  }
