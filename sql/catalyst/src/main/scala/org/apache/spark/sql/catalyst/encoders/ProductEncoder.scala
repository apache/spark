...
import org.apache.spark.sql.catalyst.util.{DateTimeUtils, ArrayBasedMapData, ArrayData, GenericArrayData}
...
object ProductEncoder {
  ...
  def apply[T <: Product : TypeTag]: Encoder[T] = {
    ...
    val constructExpression = constructorFor(InputInternalRow(extractExpression.dataType), tpe)
  ...
  def constructorFor(
      ...
      tpe: `Type`): Expression = ScalaReflectionLock.synchronized {
    tpe match {
      case t if !dataTypeFor(t).isInstanceOf[ObjectType] => input

      case t if t <:< localTypeOf[Option[_]] =>
        val TypeRef(_, _, Seq(optType)) = t
        WrapOption(null, constructorFor(input, optType))

      case t if t <:< localTypeOf[java.lang.Integer] =>
        val boxedType = classOf[java.lang.Integer]
        val objectType = ObjectType(boxedType)
        NewInstance(boxedType, input :: Nil, propagateNull = true, objectType)
      ...
      case t if t <:< localTypeOf[Array[_]] =>
        ...
        primitiveMethod.map { method =>
          Invoke(input, method, arrayClassFor(elementType))
        }.getOrElse {
          Invoke(
            MapObjects(constructorFor(_, elementType), input, schemaFor(elementType).dataType),
            "array",
      ...
      case t if t <:< localTypeOf[Map[_, _]] =>
        ...
        val keyData =
          Invoke(
            MapObjects(
              constructorFor(_, keyType),
              Invoke(input, "keyArray", ArrayType(schemaFor(keyType).dataType)),
        ...
        val valueData =
          Invoke(
            MapObjects(
              constructorFor(_, valueType),
              Invoke(input, "valueArray", ArrayType(schemaFor(valueType).dataType)),
      ...
      case t if t <:< localTypeOf[Product] =>
        ...
        val arguments = params.head.zipWithIndex.map { case (p, i) =>
          ...
          constructorFor(GetInternalRowField(input, i, dataType), fieldType)
        }
