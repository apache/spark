...
import org.apache.spark.sql.catalyst.analysis.resolver.{AnalyzerBridgeState, HybridAnalyzer, ResolverExtension}
...
/**
 * A trivial [[Analyzer]] with a dummy [[SessionCatalog]] and [[EmptyTableFunctionRegistry]]. Used
 * for testing when all relations are already filled in and the analyzer needs only to resolve
 ...
 * star expressions in subqueries that reference the outer query plans. The state that is kept
 * here is per-query.
 *
 * Note this is thread local.
 *
 * @param catalogAndNamespace
 *   The catalog and namespace used in the view resolution. This overrides the current catalog and
 *   namespace when resolving relations inside views.
 ...
 * @param relationCache
 *   A mapping from qualified table names and time travel spec to resolved relations. This can
 *   ensure that the table is resolved only once if a table is used multiple times in a query.
 * @param referredTempViewNames
 *   All the temp view names referred by the current view we are resolving. It's used to make sure
 *   the relation resolution is consistent between view creation and view resolution. For example,
 *   if `t` was a permanent table when the current view was created, it should still be a
 *   permanent table when resolving the current view, even if a temp view `t` has been created.
 * @param outerPlan
 *   The query plan from the outer query that can be used to resolve star expressions in a
 *   subquery.
 */
case class AnalysisContext(
    ...
    /**
     * This is a bridge state between this fixed-point [[Analyzer]] and a single-pass
     * [[Resolver]]. It's managed ([[setSinglePassResolverBridgeState]] method) by the
     * [[HybridAnalyzer]] - the goal is to preserve it correctly between the fixed-point and
     * single-pass runs. [[AnalysisContext.reset]] simply propagates it to prevent it from being
     * reset in [[Analyzer.execute]]. Normally it's always [[None]], unless
...
object Analyzer {
   ...
   * version used to create the view is 4.0.0 or higher. We set it to `false` in case Spark
   * version is lower than 4.0.0 or if the Spark version wasn't stored (in that case we assume
...
class Analyzer(
      ...
      if (AnalysisContext.get.isDefault) {
        ...
        try {
          AnalysisHelper.markInAnalyzer {
            HybridAnalyzer
              .fromLegacyAnalyzer(legacyAnalyzer = this, tracker = tracker)
              .apply(plan)
          }
        } finally {
          AnalysisContext.reset()
  ...
  private def earlyBatches: Seq[Batch] = Seq(
    ...
    Batch(
      "Hints",
      fixedPoint,
      Seq(ResolveHints.ResolveJoinStrategyHints, ResolveHints.ResolveCoalesceHints) ++
  ...
  override def batches: Seq[Batch] = earlyBatches ++ Seq(
    ...
    Batch("UDF", Once, ResolveEncodersInUDF),
    ...
    Batch("Remove watermark for batch query", Once, EliminateEventTimeWatermark),
    Batch("ResolveUnresolvedHaving", Once, ResolveUnresolvedHaving))

  /**
   * For [[Add]]:
   *   1. if both side are interval, stays the same;
   *   2. else if one side is date and the other is interval, turns it to [[DateAddInterval]];
   ...
   * For [[Subtract]]:
   *   1. if both side are interval, stays the same;
   *   2. else if the left side is date and the right side is interval, turns it to
   *      [[DateAddInterval(l, -r)]];
  ...
  object WindowsSubstitution extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUpWithPruning(
      _.containsAnyPattern(WITH_WINDOW_DEFINITION, UNRESOLVED_WINDOW_EXPRESSION),
      ruleId) {
  ...
  object ResolveGroupingAnalytics extends Rule[LogicalPlan] {
    ...
    private def constructAggregate(
      ...
      GroupingAnalyticsTransformer(
        newAlias = (child, name, qualifier) => Alias(child, name.get)(qualifier = qualifier),
    ...
    private def tryResolveHavingCondition(
      ...
      if (extraAggExprs.isEmpty) {
        ...
        Project(newChild.output.dropRight(extraAggExprs.length), Filter(newCond, newChild))
      }
    }
      ...
      plan.resolveOperatorsDownWithPruning(_.containsPattern(GROUPING_ANALYTICS), ruleId) {
        case h @ UnresolvedHaving(
              ...
              agg @ Aggregate(
                ...
                aggExprs,
                _,
                _)) if agg.childrenResolved && aggExprs.forall(_.resolved) =>
        ...
        case a @ Aggregate(
              ...
              aggExprs,
              child,
              _) if aggExprs.forall(_.resolved) =>
          ...
          val newOrder = order.map { expression =>
            GroupingAnalyticsTransformer
              .replaceGroupingFunction(
                ...
                gid = gid,
                newAlias =
                  (child, name, qualifier) => Alias(child, name.get)(qualifier = qualifier))
      ...
      plan.resolveOperatorsWithPruning(_.containsPattern(PIVOT), ruleId) {
        ...
        case Pivot(groupByExprsOpt, pivotColumn, pivotValues, aggregates, child) =>
          ...
          PivotTransformer(
            ...
            childOutput = child.output,
            newAlias = (child, name) => Alias(child, name.get)())
      }
  ...
  /**
   * Adds metadata columns to output for child relations when nodes are missing resolved
   * attributes.
   *
   * References to metadata columns are resolved using columns from
   * [[LogicalPlan.metadataOutput]], but the relation's output does not include the metadata
   * columns until the relation is replaced. Unless this rule adds metadata to the relation's
   * output, the analyzer will detect that nothing produces the columns.
   *
   ...
   * By checking only resolved nodes, this ensures that * expansion is already done so that
   * metadata columns are not accidentally selected by *. This rule resolves operators downwards
   * to avoid projecting away metadata columns prematurely.
   */
          ...
          if (metaCols.isEmpty) {
            ...
            val newNode = node.mapChildren(
              addMetadataCol(
                ...
                onlyUniqueAndNecessaryMetadataColumns))
  ...
  object ResolveRelations extends Rule[LogicalPlan] {
    ...
    private def resolveViews(plan: LogicalPlan, options: CaseInsensitiveStringMap): LogicalPlan =
      plan match {
    ...
    private def resolveTableReferences(plan: LogicalPlan): LogicalPlan = {
      plan.resolveOperatorsUp { case r: V2TableReference =>
    ...
    /**
     * Resolves relations to `ResolvedTable` or `Resolved[Temp/Persistent]View`. This is for
     * resolving DDL and misc commands.
     */
            ...
            case CatalogAndIdentifier(catalog, ident) =>
              ...
              CatalogV2Util.loadTable(catalog, ident).map {
                case v1Table: V1Table
                  ...
                  ResolvedPersistentView(catalog, v2Ident, v1Table.catalogTable)
                case table =>
    ...
    def resolveRelation(
      ...
      relationResolution
        .resolveRelation(unresolvedRelation, timeTravelSpec)
        .map { relation =>
    ...
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsWithPruning(AlwaysProcess.fn, ruleId) {
        case i @ InsertIntoStatement(r: DataSourceV2Relation, _, _, _, _, _, _, _)
          ...
          if (!i.overwrite) {
            if (isByName) {
              AppendData.byName(r, query, writeOptions = schemaEvolutionWriteOption)
            } else {
              AppendData.byPosition(r, query, writeOptions = schemaEvolutionWriteOption)
            }
          ...
          val outputNameToStaticName = staticPartitions.keySet.map { staticName =>
            if (isByName) {
              ...
              if (query.output.exists(col => conf.resolver(col.name, staticName))) {
                throw QueryCompilationErrors.staticPartitionInUserSpecifiedColumnsError(
                  staticName)
              }
            }
  ...
  /**
   * Resolves column references in the query plan. Basically it transform the query plan tree
   * bottom up, and only try to resolve references for a plan node if all its children nodes are
   * resolved, and there is no conflicting attributes between the children nodes (see
   * `hasConflictingAttrs` for details).
   *
   * The general workflow to resolve references:
   *   1. Expands the star in Project/Aggregate/Generate.
   *   2. Resolves the columns to [[AttributeReference]] with the output of the children plans.
   *      This includes metadata columns as well.
   ...
   *   4. Resolves the columns to outer references with the outer plan if we are resolving
   *      subquery expressions.
   *   5. Resolves the columns to SQL variables.
   *
   * Some plan nodes have special column reference resolution logic, please read these sub-rules
   * for details:
   ...
   * previous options are permanently not applicable. If the current option can be applicable in
   * the next iteration (other rules update the plan), we should not try the next option.
   */
  class ResolveReferences(val catalogManager: CatalogManager)
     ...
     *
     * The children logical plans may output columns with conflicting attribute IDs. This may
     * happen in cases such as self-join. We should wait for the rule [[DeduplicateRelations]] to
     * eliminate conflicting attribute IDs, otherwise we can't resolve columns correctly due to
     * ambiguity.
     */
    ...
    def doApply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      ...
      case g @ Generate(generator, join, outer, qualifier, output, child) =>
        val newG =
          resolveExpressionByPlanOutput(generator, child, throws = true, includeLastResort = true)
        ...
        EliminateSubqueryAliases(targetTable) match {
          ...
          case _ =>
            ...
            val newNotMatchedActions = m.notMatchedActions.map {
              case InsertAction(insertCondition, assignments) =>
                ...
                val resolvedInsertCondition =
                  insertCondition.map(resolveExpressionByPlanOutput(_, m.sourceTable))
                InsertAction(
            ...
            val newNotMatchedBySourceActions = m.notMatchedBySourceActions.map {
              case DeleteAction(deleteCondition) =>
                val resolvedDeleteCondition =
                  deleteCondition.map(resolveExpressionByPlanOutput(_, targetTable))
                DeleteAction(resolvedDeleteCondition)
              case UpdateAction(updateCondition, assignments, fromStar) =>
                val resolvedUpdateCondition =
                  updateCondition.map(resolveExpressionByPlanOutput(_, targetTable))
                UpdateAction(
                  resolvedUpdateCondition,
    ...
    private def checkResolvedMergeExpr(e: Expression, p: LogicalPlan): Unit = {
      e.references.filter(!_.resolved).foreach { a =>
        ...
        a.failAnalysis(
          ...
          messageParameters = Map("objectName" -> toSQLId(a.name), "proposal" -> cols))
      }
    }
    ...
    private def isCountStarExpansionAllowed(arguments: Seq[Expression]): Boolean =
      arguments match {
    ...
    /**
     * Checks if the given function name parts match the expected builtin function name. This is
     * used for special syntax transformations (e.g., COUNT(*) -> COUNT(1)) that should only apply
     * to builtin functions, not to user-defined functions.
     *
     * In legacy mode (sessionOrder="first"), temp functions shadow builtins, so an unqualified
     * name that matches a temp function should NOT be treated as builtin.
     */
   ...
   *
   *   - When the sort references or group by expressions are not integer but foldable
   *     expressions, just ignore them.
   *   - When spark.sql.orderByOrdinal/spark.sql.groupByOrdinal is set to false, ignore the
   *     position numbers too.
   *
   * Before the release of Spark 2.0, the literals in order/sort by and group by clauses have no
  ...
  /**
   * Checks whether a function identifier referenced by an [[UnresolvedFunction]] is defined in
   * the function registry. Note that this rule doesn't try to resolve the [[UnresolvedFunction]].
   * It only performs simple existence check according to the function identifier to quickly
   * identify undefined functions without triggering relation resolution, which may incur
   * potentially expensive partition/schema discovery process in some cases.
   *
              ...
              functionType match {
                ...
                case FunctionType.NotFound =>
                  ...
                  val searchPath = SQLConf.get
                    .resolutionSearchPath(catalogPath.toSeq)
                    .map(_.quoted)
  ...
  /**
   * Replaces [[UnresolvedFunctionName]]s with concrete [[LogicalPlan]]s. Replaces
   * [[UnresolvedFunction]]s with concrete [[Expression]]s. Replaces [[UnresolvedGenerator]]s with
   * concrete [[Expression]]s. Replaces [[UnresolvedTableValuedFunction]]s with concrete
   * [[LogicalPlan]]s.
   */
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUpWithPruning(
      ...
      case u @ UnresolvedFunctionName(nameParts, _, _) =>
        functionResolution
          ...
          .orElse(functionResolution.lookupBuiltinOrTempTableFunction(nameParts))
          .map { info =>
      ...
      case u: UnresolvedTableValuedFunction if u.functionArgs.forall(_.resolved) =>
        withPosition(u) {
          val resolvedFunc = functionResolution
      ...
      case u: UnresolvedTVFAliases if u.child.resolved =>
        ...
        val aliases = outputAttrs.zip(u.outputNames).map { case (attr, name) =>
          Alias(attr, name)()
        }
        ...
        withPosition(p) {
          ...
          val tvfWithTableColumnIndexes = tvf match {
            case g @ Generate(pyudtf: PythonUDTF, _, _, _, _, _)
              ...
              g.copy(generator =
                pyudtf.copy(pythonUDTFPartitionColumnIndexes = Some(partitionColumnIndexes)))
            case _ => tvf
          }
      ...
      case q: LogicalPlan =>
        q.transformExpressionsUpWithPruning(
          ...
          case u @ UnresolvedFunction(nameParts, arguments, _, _, _, _, _)
              if functionResolution.hasLambdaAndResolvedArguments(arguments) =>
            withPosition(u) {
          ...
          case u: UnresolvedPolymorphicPythonUDTF =>
            withPosition(u) {
  ...
  object ResolveProcedures extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsWithPruning(_.containsPattern(UNRESOLVED_PROCEDURE), ruleId) {
  ...
  object ResolveSubquery extends Rule[LogicalPlan] {
    ...
    /**
     * Resolves the subquery. Apart of resolving the subquery and outer references (if any) in the
     * subquery plan, the children of subquery expression are updated to record the outer
     * references. This is needed to make sure (1) The column(s) referred from the outer query are
     * not pruned from the plan during optimization. (2) Any aggregate expression(s) that
     * reference outer attributes are pushed down to outer plan to get evaluated.
     */
    private def resolveSubQueries(plan: LogicalPlan, outer: LogicalPlan): LogicalPlan = {
      plan.transformAllExpressionsWithPruning(_.containsPattern(PLAN_EXPRESSION), ruleId) {
        ...
        case a: FunctionTableSubqueryArgumentExpression if !a.plan.resolved =>
          resolveSubQuery(a, outer)((plan, outerAttrs) =>
            a.copy(plan = plan, outerAttrs = outerAttrs))
      }
    }
  ...
  /**
   * This rule resolves SQL function expressions. It pulls out function inputs and place them in a
   * separate [[Project]] node below the operator and replace the SQL function with its actual
   * function body. SQL function expressions in [[Aggregate]] are handled in a special way.
   * Non-aggregated SQL functions in the aggregate expressions of an Aggregate need to be pulled
   * out into a Project above the Aggregate before replacing the SQL function expressions with
   * actual function bodies. For example:
   *
   * Before: Aggregate [c1] [foo(c1), foo(max(c2)), sum(foo(c2)) AS sum] +- Relation [c1, c2]
   *
   * After: Project [foo(c1), foo(max_c2), sum] +- Aggregate [c1] [c1, max(c2) AS max_c2,
   * sum(foo(c2)) AS sum] +- Relation [c1, c2]
   */
  object ResolveSQLFunctions extends Rule[LogicalPlan] {
    ...
    /**
     * Rewrite SQL function expressions into actual resolved function bodies and extract function
     * inputs into the given project list.
     */
    private def rewriteSQLFunctions[E <: Expression](
      ...
      val newExpr = expression match {
        case f: SQLFunctionExpression
            if !hasSQLFunctionExpression(f.inputs) &&
    ...
    /**
     * Check if the given expression contains expressions that should be extracted, i.e.
     * non-aggregated SQL functions with non-foldable inputs.
     */
    ...
    /**
     * Extract aggregate expressions from the given expression and replace them with attribute
     * references. Example: Before: foo(c1) + foo(max(c2)) + max(foo(c2)) After: foo(c1) +
     * foo(max_c2) + max_foo_c2 Extracted expressions: [c1, max(c2) AS max_c2, max(foo(c2)) AS
     * max_foo_c2]
     */
    ...
    /**
     * Replace all [[SQLFunctionExpression]]s in an expression with attribute references from the
     * aliasMap.
     */
    ...
    private def rewrite(plan: LogicalPlan): LogicalPlan = plan match {
      ...
      case a: Aggregate if a.resolved && hasSQLFunctionExpression(a.expressions) =>
        ...
        val newGrouping = a.groupingExpressions.map { expr =>
          expr.transformDown { case f: SQLFunctionExpression =>
        ...
        val aggregateExpressions =
          a.aggregateExpressions.map(replaceSQLFunctionWithAttr(_, aliasMap))
      ...
      case j: Join if j.resolved && hasSQLFunctionExpression(j.expressions) =>
        ...
        if (joinCond != j.condition) {
          ...
          val newJoinCond = joinCond.map(_.transform { case a: Attribute =>
  ...
  object ResolveSQLTableFunctions extends Rule[LogicalPlan] with AliasHelper {
    ...
    /**
     * Check if a subquery plan is subject to the COUNT bug that can cause wrong results. A
     * lateral correlation can only be removed if the lateral subquery is not subject to the COUNT
     * bug. Currently only lateral correlation can handle it correctly.
     */
    ...
    /**
     * Rewrite a resolved SQL table function plan by removing unnecessary lateral joins: Before:
     * LateralJoin lateral-subquery [a], Inner : +- Project [c1, c2] : +- Filter [outer(a) == c1]
     * : +- Relation [c1, c2] +- Project [1 AS a] +- OneRowRelation After: Project [c1, c2] +-
     * Filter [1 == c1] <---- Replaced outer(a) +- Relation [c1, c2]
     */
    private def rewrite(plan: LogicalPlan): LogicalPlan = {
      (plan transformUp {
        case j @ LateralJoin(
              Project(aliases, _: OneRowRelation),
              sub: LateralSubquery,
              Inner,
              None) if j.resolved && aliases.forall(_.deterministic) =>
          val attrMap =
            AttributeMap(aliases.collect { case a: Alias => a.toAttribute -> a.child })
          val newPlan =
            sub.plan.transformAllExpressionsWithPruning(_.containsPattern(OUTER_REFERENCE)) {
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(SQL_TABLE_FUNCTION)) {
   ...
   *
   * We need to make sure the expressions all fully resolved before looking for aggregate
   * functions and group by expressions from them.
   */
      ...
      plan.resolveOperatorsUpWithPruning(_.containsPattern(AGGREGATE), ruleId) {
        ...
        case s @ Sort(_, _, agg: Aggregate, _) if agg.resolved && s.order.forall(_.resolved) =>
          resolveOperatorWithAggregate(
            ...
            agg,
            (newExprs, newChild) => {
              val newSortOrder = s.order.zip(newExprs).map { case (sortOrder, expr) =>
                sortOrder.copy(child = expr)
              }
    ...
    private def buildAggExprList(
      ...
      val replacement: Option[NamedExpression] =
        agg.aggregateExpressions.foldLeft(Option.empty[NamedExpression]) {
          case (None, alias: Alias) if expr.semanticEquals(trimAliases(alias.child)) =>
            Some(alias)
    ...
    def resolveOperatorWithAggregate(
      ...
      if (extraAggExprs.isEmpty) {
        ...
        Project(
          ...
          buildOperator(
            resolvedExprs,
            agg.copy(aggregateExpressions = agg.aggregateExpressions ++ extraAggExprs)))
      }
    }
  }

  /**
   * Extracts [[Generator]] from the projectList of a [[Project]] operator and creates
   * [[Generate]] operator under [[Project]].
   *
   * This rule will throw [[AnalysisException]] for following cases:
   *   1. [[Generator]] is nested in expressions, e.g. `SELECT explode(list) + 1 FROM tbl`
   *   2. more than one [[Generator]] is found in projectList, e.g.
   *      `SELECT explode(list), explode(list) FROM tbl`
   *   3. [[Generator]] is found in other operators that are not [[Project]] or [[Generate]], e.g.
   *      `SELECT * FROM tbl SORT BY explode(list)`
   */
  object ExtractGenerator extends Rule[LogicalPlan] {
    ...
    private object AliasedGenerator {
      ...
      /**
       * Extracts a [[Generator]] expression, any names assigned by aliases to the outputs and the
       * outer flag. The outer flag is used when joining the generator output.
       * @param e
       *   the [[Expression]]
       * @return
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(GENERATOR), ruleId) {
        ...
        case Aggregate(groupList, aggList, child, _) if aggList.forall {
   ...
   *
   * Names for the output [[Attribute]]s are extracted from [[Alias]] or [[MultiAlias]]
   * expressions that wrap the [[Generator]].
   */
   ...
   * aggregateExpressions of an [[Aggregate]] operator and creates individual [[Window]] operators
   * for every distinct [[WindowSpecDefinition]].
   *
   * This rule handles three cases:
   *   - A [[Project]] having [[WindowExpression]]s in its projectList;
   *   - An [[Aggregate]] having [[WindowExpression]]s in its aggregateExpressions.
   *   - A [[Filter]]->[[Aggregate]] pattern representing GROUP BY with a HAVING clause and the
   *     [[Aggregate]] has [[WindowExpression]]s in its aggregateExpressions.
   * Note: If there is a GROUP BY clause in the query, aggregations and corresponding filters
   * (expressions in the HAVING clause) should be evaluated before any [[WindowExpression]]. If a
   * query has SELECT DISTINCT, the DISTINCT part should be evaluated after all
   * [[WindowExpression]]s.
   *
   ...
   *   1. For a list of [[Expression]]s (a projectList or an aggregateExpressions), partitions it
   *      two lists of [[Expression]]s, one for all [[WindowExpression]]s and another for all
   *      regular expressions.
   *   2. For all [[WindowExpression]]s, groups them based on their [[WindowSpecDefinition]]s and
   *      [[WindowFunctionType]]s.
  ...
  object ExtractWindowExpressions extends Rule[LogicalPlan] {
     ...
     * `col1, Sum(col2 + col3) OVER (PARTITION BY col4 ORDER BY col5)`, we will extract `col1`,
     * `col2 + col3`, `col4`, and `col5` out and replace their appearances in the window
     * expression as attribute references. So, the first returned value will be
     * `[Sum(_w0) OVER (PARTITION BY _w1 ORDER BY _w2)]` and the second returned value will be
     * [col1, col2 + col3 as _w0, col4 as _w1, col5 as _w2].
     *
     * @return
     *   (seq of expressions containing at least one window expression, seq of non-window
     *   expressions)
     */
    private def extract(
      ...
      def getOrExtract(key: Expression, value: Expression): Expression = {
        extractedExprMap
          .getOrElseUpdate(key.canonicalized, Alias(value, s"_w${extractedExprMap.size}")())
          .toAttribute
      }
      ...
      val newExpressionsWithWindowFunctions = expressionsWithWindowFunctions.map {
        _.transform {
          ...
          case agg: AggregateExpression if !seenWindowAggregates.contains(agg) =>
            extractedExprMap
              .getOrElseUpdate(agg.canonicalized, Alias(agg, s"_w${extractedExprMap.size}")())
              .toAttribute
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsDownWithPruning(_.containsPattern(WINDOW_EXPRESSION), ruleId) {
        ...
        case f @ UnresolvedHaving(
              condition,
              a @ Aggregate(groupingExprs, aggregateExprs, child, _))
            if child.resolved &&
              hasWindowFunction(aggregateExprs) &&
              a.expressions.forall(_.resolved) =>
          aggregateExprs.foreach(
            _.transformDownWithPruning(_.containsPattern(LATERAL_COLUMN_ALIAS_REFERENCE)) {
  ...
  object ResolveRandomSeed extends Rule[LogicalPlan] {
    ...
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(EXPRESSION_WITH_RANDOM_SEED), ruleId) {
   ...
   * primitive parameter is null or not, so here we assume the primitive input is
   * null-propagatable and we should return null if the input is null.
   */
          ...
          p.transformExpressionsUpWithPruning(_.containsPattern(SCALA_UDF)) {
            ...
            case udf: ScalaUDF if udf.inputPrimitives.contains(true) =>
              ...
              if (inputNullCheck.isDefined) {
                ...
                val newInputs = inputPrimitivesPair.map { case (isPrimitive, input) =>
  ...
  /**
   * Resolve the encoders for the UDF by explicitly given the attributes. We give the attributes
   * explicitly in order to handle the case where the data type of the input value is not the same
   * with the internal schema of the encoder, which could cause data loss. For example, the
   * encoder should not cast the input value to Decimal(38, 18) if the actual data type is
   * Decimal(30, 0).
   *
  ...
  object ResolveEncodersInUDF extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithSubqueriesAndPruning(_.containsPattern(SCALA_UDF), ruleId) {
        ...
        case p =>
          p.transformExpressionsUpWithPruning(_.containsPattern(SCALA_UDF), ruleId) {
  ...
  object ResolveNaturalAndUsingJoin extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(NATURAL_LIKE_JOIN), ruleId) {
        case j @ Join(left, right, UsingJoin(joinType, usingCols), _, hint)
            if left.resolved && right.resolved && j.duplicateResolved =>
          val project = commonNaturalJoinProcessing(left, right, joinType, usingCols, None, hint)
    ...
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsWithPruning(_.containsPattern(COMMAND), ruleId) {
        case v2Write: V2WriteCommand
          ...
          TableOutputResolver.suitableForByNameCheck(
            v2Write.isByName,
            expected = v2Write.table.output,
            queryOutput = v2Write.query.output)
  ...
  /**
   * Replaces [[UnresolvedDeserializer]] with the deserialization expression that has been
   * resolved to the given input attributes.
   */
  object ResolveDeserializer extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(UNRESOLVED_DESERIALIZER), ruleId) {
        case p if !p.childrenResolved => p
        case p if p.resolved => p

        case p =>
          p.transformExpressionsWithPruning(_.containsPattern(UNRESOLVED_DESERIALIZER), ruleId) {
            case UnresolvedDeserializer(deserializer, inputAttributes) =>
              ...
              val resolved =
                resolveExpressionByPlanOutput(deserializer, LocalRelation(inputs), throws = true)
              val result = resolved transformDown {
                case UnresolvedMapObjects(func, inputData, cls) if inputData.resolved =>
                  inputData.dataType match {
                    ...
                    case other =>
                      throw QueryCompilationErrors.dataTypeMismatchForDeserializerError(
                        other,
                        "array")
                  }
    ...
    /**
     * For each top-level Tuple field, we use [[GetColumnByOrdinal]] to get its corresponding
     * column by position. However, the actual number of columns may be different from the number
     * of Tuple fields. This method is used to check the number of columns and fields, and throw
    ...
    /**
     * For each nested Tuple field, we use [[GetStructField]] to get its corresponding struct
     * field by position. However, the actual number of struct fields may be different from the
     * number of nested Tuple fields. This method is used to check the number of struct fields and
     * nested Tuple fields, and throw an exception if they do not match.
     */
  ...
  object ResolveNewInstance extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(NEW_INSTANCE), ruleId) {
        case p if !p.childrenResolved => p
        case p if p.resolved => p

        case p =>
          p.transformExpressionsUpWithPruning(_.containsPattern(NEW_INSTANCE), ruleId) {
  ...
  object ResolveUpCast extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(UP_CAST), ruleId) {
        ...
        case p =>
          p.transformExpressionsWithPruning(_.containsPattern(UP_CAST), ruleId) {
  ...
  object ResolveFieldNameAndPosition extends Rule[LogicalPlan] {
    ...
    private def resolveFieldNamesOpt(
      ...
      table.schema
        .findNestedField(fieldName, includeCollections = true, conf.resolver, context.origin)
        .map { case (path, field) =>
          ResolvedFieldName(path, field)
        }
    }
 ...
 * expression in Project(project list) or Aggregate(aggregate expressions) or Window(window
 * expressions). Notice that if an expression has other expression parameters which are not in its
 * `children`, e.g. `RuntimeReplaceable`, the transformation for Aliases in this rule can't work
...
object CleanupAliases extends Rule[LogicalPlan] with AliasHelper {
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUpWithPruning(
    ...
    case Unpivot(ids, values, aliases, variableColumnName, valueColumnNames, child) =>
      ...
      Unpivot(cleanedIds, cleanedValues, aliases, variableColumnName, valueColumnNames, child)
 ...
 *
 * Multi-part names are allowed when they resolve to a top-level attribute via a table alias (e.g.
 * "alias.column"), but rejected when they resolve to a nested struct field extraction.
 */
...
/**
 * The aggregate expressions from subquery referencing outer query block are pushed down to the
 * outer query block for evaluation. This rule below updates such outer references as
 * AttributeReference referring attributes from the parent/outer query block.
 *
 * For example (SQL):
 * {{{
 *   SELECT l.a FROM l GROUP BY 1 HAVING EXISTS (SELECT 1 FROM r WHERE r.d < min(l.b))
 * }}}
 * Plan before the rule. Project [a#226] +- Filter exists#245 [min(b#227)#249] : +- Project [1 AS
 * 1#247] : +- Filter (d#238 < min(outer(b#227))) <----- : +- SubqueryAlias r : +- Project [_1#234
 * AS c#237, _2#235 AS d#238] : +- LocalRelation [_1#234, _2#235] +- Aggregate [a#226], [a#226,
 * min(b#227) AS min(b#227)#249] +- SubqueryAlias l +- Project [_1#223 AS a#226, _2#224 AS b#227] +-
 * LocalRelation [_1#223, _2#224] Plan after the rule. Project [a#226] +- Filter exists#245
 * [min(b#227)#249] : +- Project [1 AS 1#247] : +- Filter (d#238 < outer(min(b#227)#249)) <----- : +-
 * SubqueryAlias r : +- Project [_1#234 AS c#237, _2#235 AS d#238] : +- LocalRelation [_1#234,
 * _2#235] +- Aggregate [a#226], [a#226, min(b#227) AS min(b#227)#249] +- SubqueryAlias l +-
 * Project [_1#223 AS a#226, _2#224 AS b#227] +- LocalRelation [_1#223, _2#224]
 */
 ...
 *   - Replace [[TempResolvedColumn]] with [[AttributeReference]] if it's inside aggregate
 *     functions or grouping expressions.
 *   - Mark [[TempResolvedColumn]] as `hasTried` if not inside aggregate functions or grouping
 *     expressions, hoping other rules can re-resolve it. `ResolveReferences` will re-resolve
 *     [[TempResolvedColumn]] if `hasTried` is true, and keep it unchanged if the resolution
 *     fails. We should turn it back to [[UnresolvedAttribute]] so that the analyzer can report
...
/**
 * Rule that's used to handle `UnresolvedHaving` nodes with resolved `condition` and `child`. It's
 * placed outside the main batch to avoid conflicts with other rules that resolve
