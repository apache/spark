...
class Analyzer(override val catalogManager: CatalogManager)
  ...
  object ResolveAliases extends Rule[LogicalPlan] {
    private def assignAliases(exprs: Seq[NamedExpression]) = {
      ...
      def getAlias(e: Expression, derivedAlias: Option[String]): String = {
        derivedAlias.getOrElse(toPrettySQL(e))
      }

      exprs.map(_.transformUpWithPruning(_.containsPattern(UNRESOLVED_ALIAS)) {
          case u @ UnresolvedAlias(child, optGenAliasFunc, derivedAlias) =>
          child match {
            ...
            case e: ExtractValue =>
              if (extractOnly(e)) {
                Alias(e, getAlias(e, derivedAlias))()
              } else {
                Alias(e, getAlias(e, derivedAlias))(
            ...
            case l: Literal => Alias(l, getAlias(l, derivedAlias))()
            case e =>
              Alias(e, getAlias(e, derivedAlias))(
                explicitMetadata = Some(metaForAutoGeneratedAlias))
          }
  ...
  object ResolveReferences extends Rule[LogicalPlan] {
    ...
    private def buildExpandedProjectList(
      ...
      exprs.flatMap {
        ...
        case UnresolvedAlias(s: Star, _, _) => expand(s, child)
  ...
  object ExtractGenerator extends Rule[LogicalPlan] {
    ...
    private def hasNestedGenerator(expr: NamedExpression): Boolean = {
      ...
      trimNonTopLevelAliases(expr) match {
        case UnresolvedAlias(g: Generator, _, _) => hasInnerGenerator(g)
