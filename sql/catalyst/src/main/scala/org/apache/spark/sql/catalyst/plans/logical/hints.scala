 ...
 */
case class ResolvedHint(child: LogicalPlan, hints: HintInfo = HintInfo()) extends UnaryNode {
...
case class JoinHint(leftHint: Option[HintInfo], rightHint: Option[HintInfo]) {
  ...
  override def toString: String = {
    Seq(leftHint.map("leftHint=" + _), rightHint.map("rightHint=" + _))
      .filter(_.isDefined)
      .map(_.get)
      .mkString(", ")
  }
...
case class HintInfo(strategy: Option[JoinStrategyHint] = None) {
   ...
   * @param other
   *   the other [[HintInfo]]
   * @param hintErrorHandler
   *   the error handler to notify if any [[HintInfo]] has been overridden in this merge.
   *
   ...
   * `hintOverriddenCallback` will be called if this [[HintInfo]] and the other [[HintInfo]] both
   * have a strategy defined but the join strategies are different.
   */
 ...
 *
 * The hinted strategy will be used for the join with which it is associated if doable. In case of
 * contradicting strategy hints specified for each side of the join, hints are prioritized as
 * BROADCAST over SHUFFLE_MERGE over SHUFFLE_HASH over SHUFFLE_REPLICATE_NL.
 */
object JoinStrategyHint {

  val strategies: Set[JoinStrategyHint] =
    Set(BROADCAST, SHUFFLE_MERGE, SHUFFLE_HASH, SHUFFLE_REPLICATE_NL)
}

/**
 * The hint for broadcast hash join or broadcast nested loop join, depending on the availability
 * of equi-join keys.
 */
case object BROADCAST extends JoinStrategyHint {
  override def displayName: String = "broadcast"
  override def hintAliases: Set[String] = Set("BROADCAST", "BROADCASTJOIN", "MAPJOIN")
}
...
case object SHUFFLE_MERGE extends JoinStrategyHint {
  ...
  override def hintAliases: Set[String] = Set("SHUFFLE_MERGE", "MERGE", "MERGEJOIN")
}
...
case object SHUFFLE_HASH extends JoinStrategyHint {
  ...
  override def hintAliases: Set[String] = Set("SHUFFLE_HASH")
}
...
case object SHUFFLE_REPLICATE_NL extends JoinStrategyHint {
  ...
  override def hintAliases: Set[String] = Set("SHUFFLE_REPLICATE_NL")
}
...
trait HintErrorHandler {
  ...
  def hintRelationsNotFound(
      name: String,
      parameters: Seq[Any],
      invalidRelations: Set[Seq[String]]): Unit
  ...
  /**
   * Callback for a join hint specified on a join that doesn't support this build side or doesn't
   * have equi-join keys for equi-join.
   */
