...
class PlanLogger extends Logging {
  ...
  /**
   * Wraps `body`, which is a `resolve*` method of the [[Resolver]]. Calls `body` and logs the
   * `unresolvedPlan` and the resolved result from `body` execution.
   */
  def log(unresolvedPlan: LogicalPlan)(body: => LogicalPlan): LogicalPlan = {
    val resolvedPlan = body
  ...
  private def logBasedOnLevel(createMessage: () => MessageWithContext) = logLevel match {
