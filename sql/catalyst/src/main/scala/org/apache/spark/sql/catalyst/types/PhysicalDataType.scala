...
import org.apache.spark.sql.catalyst.util.{ArrayData, CollatorFactory, SQLOrderingUtil}
...
object PhysicalDataType {
  def apply(dt: DataType): PhysicalDataType = dt match {
    ...
    case st: StringType => PhysicalStringType(st.collationId)
...
case class PhysicalStringType(collationId: Int) extends PhysicalDataType {
  ...
  private[sql] val ordering = {
    CollatorFactory.getInfoForId(collationId).comparator.compare(_: UTF8String, _: UTF8String)
  }
  @transient private[sql] lazy val tag = typeTag[InternalType]
}
