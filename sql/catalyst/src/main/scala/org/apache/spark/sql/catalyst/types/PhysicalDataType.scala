...
import org.apache.spark.sql.catalyst.util.{ArrayData, CollationFactory, SQLOrderingUtil, StringTypeUtils}
...
object PhysicalDataType {
  def apply(dt: DataType): PhysicalDataType = dt match {
    ...
    case VarcharType(_) => PhysicalStringType(StringTypeUtils.getDefaultCollationId)
    case CharType(_) => PhysicalStringType(StringTypeUtils.getDefaultCollationId)
