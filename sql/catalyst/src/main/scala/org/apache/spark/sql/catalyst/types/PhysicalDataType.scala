...
object PhysicalDataType {
  def apply(dt: DataType): PhysicalDataType = dt match {
    ...
    case VarcharType(_) => PhysicalStringType(CollationFactory.UTF8_BINARY_COLLATION_ID)
    case CharType(_) => PhysicalStringType(CollationFactory.UTF8_BINARY_COLLATION_ID)
