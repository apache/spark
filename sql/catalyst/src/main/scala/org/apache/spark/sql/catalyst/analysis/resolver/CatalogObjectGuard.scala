...
import org.apache.spark.sql.catalyst.analysis.{FunctionResolution, RelationResolution}
 ...
 *
 * In OSS Spark, holistic catalog object validation is not possible since there is no Unity Catalog
 * to provide metadata for all nested dependencies up front. The guard always returns
