...
/**
 * Propagates source identifying names from NamedStreamingRelation wrappers to the underlying
 * StreamingRelationV2 nodes during analysis.
 *
 * This rule unwraps NamedStreamingRelation nodes and propagates their sourceIdentifyingName to
 * the underlying streaming relations. The names are used to:
 ...
 *
 * This rule also handles resolution of Streaming Tables and Materialized Views that are wrapped
 * in NamedStreamingRelation nodes, by calling resolveSTsAndMVs.
 *
 * Naming strategy:
 ...
 *
 * When `spark.sql.streaming.queryEvolution.enableSourceEvolution` is enabled, all sources must be
 * explicitly named (throws error if any Unassigned sources found).
 */
object NameStreamingSources extends Rule[LogicalPlan] {

  /**
   * Extracts a concise descriptor for a streaming source for use in error messages. Returns the
   * source/provider name for streaming relations, alias for SubqueryAlias, or the node class name
   * as a fallback.
   */
  ...
  /**
   * Validates that all streaming sources are explicitly named when enforcement is enabled. Throws
   * an AnalysisException if any sources are unnamed (Unassigned).
   *
  ...
  private def checkSourceNamingEnforcement(plan: LogicalPlan): Unit = {
    val namedRelations = plan.collect { case n: NamedStreamingRelation =>
      n
    }
    val unnamedSourcesInfo = namedRelations.zipWithIndex.flatMap { case (n, i) =>
  ...
  /**
   * Unwraps NamedStreamingRelation nodes and propagates their names to underlying streaming
   * relations.
   */
  private def propagateSourceNames(plan: LogicalPlan): LogicalPlan = {
    ...
    plan.resolveOperatorsWithPruning(_.containsPattern(NAMED_STREAMING_RELATION)) {
      ...
      case NamedStreamingRelation(h: HasStreamingSourceIdentifyingName, sourceIdentifyingName) =>
