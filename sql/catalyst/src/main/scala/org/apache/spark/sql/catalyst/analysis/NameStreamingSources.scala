...
object NameStreamingSources extends Rule[LogicalPlan] {
   ...
   * Throws an AnalysisException if any sources are unnamed (Unassigned) or have duplicate names.
   *
   * Collects NamedStreamingRelation wrappers (which carry user-provided names) and any
   * HasStreamingSourceIdentifyingName nodes that are NOT already wrapped. This avoids
  ...
  private def checkSourceNamingEnforcement(plan: LogicalPlan): Unit = {
    // First, collect all NamedStreamingRelation nodes to know which sources are wrapped
    val namedRelations = plan.collect {
      case n: NamedStreamingRelation => n
    }

    // Collect wrapped sources (get name from wrapper)
    val wrappedSources = namedRelations.map { n =>
      (n.sourceIdentifyingName, n.child)
    }

    // Collect unwrapped sources (only those not inside a NamedStreamingRelation)
    val wrappedChildren = namedRelations.map(_.child).toSet
    val unwrappedSources = plan.collect {
      case h: HasStreamingSourceIdentifyingName if !wrappedChildren.contains(h) =>
        (h.sourceIdentifyingName, h)
    }

    val allSources = wrappedSources ++ unwrappedSources

    val unnamedSourcesInfo = allSources.zipWithIndex.flatMap {
      case ((sourceId, node), i) =>
        sourceId match {
          case UserProvided(_) | FlowAssigned(_) => None  // Named, skip
          case Unassigned =>
            Some(s"Leaf position $i - ${getSourceDescriptor(node)}")  // Unnamed
        }
    }
    ...
    val sourceNames = allSources.flatMap {
      case (sourceId, _) =>
        sourceId match {
    ...
    val duplicates = sourceNames.groupBy(identity).filter(_._2.length > 1).keys
    if (duplicates.nonEmpty) {
      throw QueryCompilationErrors.duplicateStreamingSourceNamesError(duplicates.toSeq)
    }
  }
