...
/**
 * A rule that replaces `Literal(null, BooleanType)` with `FalseLiteral`, if possible, in the
 * search condition of the WHERE/HAVING/ON(JOIN) clauses, which contain an implicit Boolean
 * operator "(search condition) = TRUE". The replacement is only valid when
 * `Literal(null, BooleanType)` is semantically equivalent to `FalseLiteral` when evaluating the
 ...
 *
 * Another example containing branches is `Filter(If(cond, FalseLiteral, Literal(null, _)))`; this
 * can be optimized to `Filter(If(cond, FalseLiteral, FalseLiteral))`, and eventually
 * `Filter(FalseLiteral)`.
 *
...
object ReplaceNullWithFalseInPredicate extends Rule[LogicalPlan] {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformWithPruning(
    ...
    case p: LogicalPlan =>
      p.transformExpressionsWithPruning(
        _.containsAnyPattern(NULL_LITERAL, TRUE_OR_FALSE_LITERAL),
        ruleId) {
  ...
  /**
   * Recursively traverse the Boolean-type expression to replace `Literal(null, BooleanType)` with
   * `FalseLiteral`, if possible.
   *
    ...
    case e =>
      if (Utils.isTesting) {
        throw new SparkIllegalArgumentException(
          ...
          messageParameters = Map("dataType" -> e.dataType.catalogString, "expr" -> e.sql))
      } else {
  ...
  private def replaceNullWithFalse(mergeActions: Seq[MergeAction]): Seq[MergeAction] = {
    mergeActions.map {
      ...
      case u @ UpdateStarAction(Some(cond)) =>
        u.copy(condition = Some(replaceNullWithFalse(cond)))
      ...
      case i @ InsertStarAction(Some(cond)) =>
        i.copy(condition = Some(replaceNullWithFalse(cond)))
      case other => other
    }
  }
