...
object DeduplicateRelations extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    def containsMissingInput(p: LogicalPlan) = {
      p.find(_.missingInput.nonEmpty).nonEmpty
    }
    ...
    newPlan.resolveOperatorsUpWithPruning(
      ...
      case j @ Join(left, right, _, _, _) if !j.duplicateResolved && !containsMissingInput(right) =>
        j.copy(right = dedupRight(left, right))
      // Resolve duplicate output for LateralJoin.
      case j @ LateralJoin(left, right, _, _)
          if right.resolved && !j.duplicateResolved && !containsMissingInput(right.plan) =>
      ...
      case j @ AsOfJoin(left, right, _, _, _, _, _)
          if !j.duplicateResolved && !containsMissingInput(right) =>
        j.copy(right = dedupRight(left, right))
      ...
      case i @ Intersect(left, right, _) if !i.duplicateResolved && !containsMissingInput(right) =>
        i.copy(right = dedupRight(left, right))
      case e @ Except(left, right, _) if !e.duplicateResolved && !containsMissingInput(right) =>
      ...
      case merge: MergeIntoTable
          if !merge.duplicateResolved && !containsMissingInput(merge.sourceTable) =>
