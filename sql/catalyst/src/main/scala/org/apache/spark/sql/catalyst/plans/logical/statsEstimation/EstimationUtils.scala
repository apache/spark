...
object EstimationUtils {
  ...
  /**
   * Check if each attribute has column stat containing distinct and null counts in the
  ...
  def getOutputMap(
      inputMap: AttributeMap[ColumnStat],
      output: Seq[Attribute]): AttributeMap[ColumnStat] = {
   ...
   * represented as numbers, e.g. Boolean can be represented as 0 (false) or 1 (true). The two
   * methods below are the contract of conversion.
   */
  ...
  /**
   * Returns the index of the first bin into which the given value falls for a specified numeric
  ...
  /**
   * Returns the index of the last bin into which the given value falls for a specified numeric
   * equi-height histogram.
   */
  ...
  /**
   * Returns the number of histogram bins holding values within the given range [lowerBound,
   * upperBound].
   *
  ...
  /**
   * Returns overlapped ranges between two histograms, in the given value range [lowerBound,
   * upperBound].
   */
  ...
  /**
   * Given an original bin and a value range [lowerBound, upperBound], returns the trimmed part of
   * the bin in that range and its number of rows.
   * @param bin
  ...
  def trimBin(
      ...
      height: Double,
      lowerBound: Double,
      upperBound: Double): (HistogramBin, Double) = {
  ...
  /**
   * A join between two equi-height histograms may produce multiple overlapped ranges. Each
   * overlapped range is produced by a part of one bin in the left histogram and a part of one bin
