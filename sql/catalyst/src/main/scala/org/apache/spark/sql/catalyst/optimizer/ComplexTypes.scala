...
object SimplifyExtractValueOps extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.transformWithPruning(_.containsPattern(EXTRACT_VALUE), ruleId) { case p =>
      p.transformExpressionsUpWithPruning(_.containsPattern(EXTRACT_VALUE), ruleId) {
        // Remove redundant field extraction.
        case GetStructField(createNamedStruct: CreateNamedStruct, ordinal, _) =>
          createNamedStruct.valExprs(ordinal)
        case GetStructField(u: UpdateFields, ordinal, _)
        ...
        case GetArrayStructFields(
          ...
          CreateArray(
            elems.map(GetStructField(_, ordinal, Some(field.name))),
