...
object SimplifyExtractValueOps extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case p => p.transformExpressionsUp {
      ...
      case GetArrayStructFields(CreateArray(elems, emptyCollection), field, ordinal, _, _) =>
        ...
        CreateArray(elems.map(GetStructField(_, ordinal, Some(field.name))), emptyCollection)

      // Remove redundant map lookup.
      case ga @ GetArrayItem(CreateArray(elems, emptyCollection), IntegerLiteral(idx)) =>
      ...
      case GetMapValue(CreateMap(elems, emptyCollection), key) => CaseKeyWhen(key, elems)
    }
  }
}
