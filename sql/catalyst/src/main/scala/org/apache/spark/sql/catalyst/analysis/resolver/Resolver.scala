...
import com.databricks.sql.expressions.{GetWarmupTracing, ListSecrets}

import org.apache.spark.SparkException
import org.apache.spark.sql.AnalysisException
import org.apache.spark.sql.catalyst.{MetricKey, QueryPlanningTracker, SqlScriptingContextManager}
...
class Resolver(
  ...
  private val windowResolver = new WindowResolver(expressionResolver)
  private val aggregateResolver = new AggregateResolver(this, expressionResolver, windowResolver)
  ...
  /**
   * Get the [[WindowResolver]] bound to the used [[Resolver]].
   */
  def getWindowResolver: WindowResolver = windowResolver
      ...
      val resolvedPlan =
        unresolvedPlan match {
          ...
          case trustedPlan: TrustedPlan =>
            resolveTrustedPlan(trustedPlan)
          ...
          case explainResult: ExplainResult =>
            resolveExplainResult(explainResult)
          case listSecrets: ListSecrets =>
            resolveListSecrets(listSecrets)
          case getWarmupTracing: GetWarmupTracing =>
            handleLeafOperator(getWarmupTracing)
          // END-EDGE
          case signalStatement: SignalStatement =>
            resolveSignalStatement(signalStatement)
  ...
  /**
   * Resolve [[TrustedPlan]] operator. Its resolution doesn't require any specific logic (besides
   * child resolution).
   * It's used to avoid ACL checks e.g. using `CheckPermissions.trusted` and eventually removed by
   * the [[CleanUpACLNodes]] rule.
   */
  private def resolveTrustedPlan(trustedPlan: TrustedPlan): TrustedPlan = {
    trustedPlan.copy(resolve(trustedPlan.child))
  }
  // END-EDGE
  ...
  private def runExtensions(unresolvedOperator: LogicalPlan): Option[LogicalPlan] = {
    ...
    recordProfileAndLatency(
      "runExtensions",
      MetricKey.SINGLE_PASS_ANALYZER_RESOLVER_EXTENSIONS_LATENCY
    ) {
      // END-EDGE
  ...
  /**
   * `ExplainResult` is an `ExplainNode`. It doesn't require any specific resolution besides
   * resolving its child.
   */
  private def resolveExplainResult(explainResult: ExplainResult): LogicalPlan = {
    val resolvedLogicalPlan = resolve(explainResult.logicalPlan)
    explainResult.withNewPlan(resolvedLogicalPlan)
  }

  /**
   * Usually [[ListSecrets]] is fully resolved by [[TableValuedFunctionResolver]]. However
   * [[ListSecrets]] stores its input as expressions, so we need to resolve them here as well in
   * case of multiple resolutions.
   */
  private def resolveListSecrets(listSecrets: ListSecrets): LogicalPlan = {
    val resolvedInput = listSecrets.inputExpressions.map(expression =>
      expressionResolver.resolveExpressionTreeInOperator(expression, listSecrets)
    )
    handleLeafOperator(listSecrets.copy(inputExpressions = resolvedInput))
  }
  // END-EDGE

  /**
   * Resolve [[SignalStatement]]. The message text and arguments expressions are resolved, and
   * their types checked.
   * Message text should be a STRING, message arguments should be a MAP<STRING, STRING>.
   */
  private def resolveSignalStatement(signalStatement: SignalStatement): LogicalPlan = {
    val resolvedMessageExpr = expressionResolver.resolveExpressionTreeInOperator(
      signalStatement.messageExpr,
      signalStatement
    )
    val resolvedMessageArgumentsExpr = signalStatement.messageArgumentsExpr.map(
      expression => expressionResolver.resolveExpressionTreeInOperator(expression, signalStatement)
    )

    val resolvedSignalStatement = signalStatement.copy(
      messageExpr = resolvedMessageExpr,
      messageArgumentsExpr = resolvedMessageArgumentsExpr
    )

    if (SqlScriptingContextManager.get().isEmpty) {
      throw SparkException.internalError("SIGNAL statement outside of SQL script.")
    }
    resolvedSignalStatement.checkArgTypes() match {
      case mismatch: TypeCheckResult.DataTypeMismatch =>
        resolvedSignalStatement.dataTypeMismatch("SIGNAL", mismatch)
      case TypeCheckResult.TypeCheckSuccess => resolvedSignalStatement
    }
  }
  ...
  private def createCteRelationRef(name: String, cteRelationDef: CTERelationDef): LogicalPlan = {
    SubqueryAlias(
      ...
      child = CTERelationRef(
        ...
        _resolved = true,
        _isStreaming = cteRelationDef.isStreaming,
...
object Resolver {
  ...
  def createRelationResolution(
    ...
    // BEGIN-EDGE
    new RelationResolution(
      catalogManager,
      executeAsync[Option[LogicalPlan]],
      executeAsync[Option[Table]],
      sharedRelationCache
    )
    /* // END-EDGE
  ...
  private def executeAsync[T](
      executorService: ExecutorService,
      f: () => T): CompletableFuture[T] = {
    CompletableFuture.supplyAsync(() => f(), executorService)
  }
  // END-EDGE
}
