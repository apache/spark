...
class Resolver(
    ...
    metadataResolverExtensions: Seq[ResolverExtension] = Nil)
   ...
   * metadata. It's set once per query resolution by calling [[metadataResolver.resolve]] on an
   * unresolved logical plan tree, and is used in [[resolveRelation]] to replace the
   * [[UnresolvedRelation]].
   *
   * See [[MetadataResolver]] for more info.
   */
  private var relationsWithResolvedMetadata = new MetadataResolver.RelationsWithResolvedMetadata
  ...
  def lookupMetadataAndResolve(
    ...
    relationsWithResolvedMetadata = metadataResolver.resolve(unresolvedPlan, analyzerBridgeState)
   ...
   */
  private[resolver] def resolve(unresolvedPlan: LogicalPlan): LogicalPlan =
    planLogger.log(unresolvedPlan) {
      withTrackResolvedNodes(unresolvedPlan) {
  ...
  private def resolveRelation(unresolvedRelation: UnresolvedRelation): LogicalPlan = {
    val relation = relationsWithResolvedMetadata.get(
      metadataResolver.relationIdFromUnresolvedRelation(unresolvedRelation)
    )
    if (relation == null) {
    ...
    planLogger.log(unresolvedRelation)(relation)
