...
/**
 * The expression represents a named parameter that should be replaced by a literal or collection
 * constructor functions such as `map()`, `array()`, `struct()`.
 *
 * @param name
...
/**
 * The expression represents a positional parameter that should be replaced by a literal or by
 ...
 *   The parameterized logical plan.
 * @param args
 *   The literal values or collection constructor functions such as `map()`, `array()`, `struct()`
...
/**
 * The logical plan representing a parameterized query with general parameter support. This allows
 * the query to use either positional or named parameters based on the parameter markers found in
 * the query, with optional parameter names provided.
 *
 * @param child
 *   The parameterized logical plan.
 * @param args
 ...
 *   of parameters.
 * @param paramNames
 *   Optional parameter names corresponding to args. If provided for an argument, that argument
 *   can be used for named parameter binding. If not provided parameters are treated as
 *   positional.
 */
 ...
 *
 *   - Basic case: `PosParameterizedQuery(ExplainCommand(SomeQuery(...)))` =>
...
object MoveParameterizedQueriesDown extends Rule[LogicalPlan] {
  ...
  private def moveParameterizedQueryIntoSupervisingCommand(
      pq: ParameterizedQuery): LogicalPlan = {
...
/**
 * Binds all named parameters in `ParameterizedQuery` and substitutes them by literals or by
 * collection constructor functions such as `map()`, `array()`, `struct()` from the user-specified
 * arguments.
 */
object BindParameters extends Rule[LogicalPlan] with QueryErrorsBase {
  ...
  private def bind(p0: LogicalPlan)(f: PartialFunction[Expression, Expression]): LogicalPlan = {
    var stop = false
    p0.resolveOperatorsDownWithPruning(_.containsPattern(PARAMETER) && !stop) { case p1 =>
    ...
    plan.resolveOperatorsWithPruning(_.containsPattern(PARAMETERIZED_QUERY)) {
      ...
      case NameParameterizedQuery(child, argNames, argValues)
        ...
        if (argNames.length != argValues.length) {
          throw SparkException.internalError(
            s"The number of argument names ${argNames.length} " +
      ...
      case GeneralParameterizedQuery(child, args, paramNames)
        ...
        if (hasNamedParam && positionalParams.isEmpty) {
          if (paramNames.isEmpty) {
            ...
            val unnamedExpressions = paramNames.zipWithIndex.collect { case ("", index) =>
              index // empty strings are unnamed
            }
        ...
        if (hasNamedParam) {
          ...
          bind(child) { case NamedParameter(name) =>
            namedArgsMap.getOrElse(name, NamedParameter(name))
          }
        } else {
