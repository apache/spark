...
case class StringToMap(text: Expression, pairDelim: Expression, keyValueDelim: Expression)
  ...
  override def inputTypes: Seq[AbstractDataType] =
    Seq(StringTypeBinaryLcase, StringTypeWithCollation, StringTypeWithCollation)
  ...
  override def nullSafeEval(
    ...
    val keyValues = CollationAwareUTF8String.split(inputString.asInstanceOf[UTF8String],
    ...
    while (i < keyValues.length) {
      val keyValueArray = CollationAwareUTF8String.split(
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    nullSafeCodeGen(ctx, ev, (text, pd, kvd) =>
      s"""
         |UTF8String[] $keyValues = CollationAwareUTF8String.split($text, $pd, -1, $collationId);
         |for(UTF8String kvEntry: $keyValues) {
         |  UTF8String[] kv = CollationAwareUTF8String.split(kvEntry, $kvd, 2, $collationId);
