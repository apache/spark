...
import org.apache.spark.sql.catalyst.expressions.{Expression, ExprId, NamedExpression, OuterReference}
 ...
 * performs the cleanup of those columns that are unnecessary. Important thing to note here is
 * that by "unnecessary" columns we are not referring to the ones that are not needed in upper
 * operators for correct result, but to the columns that have been added by single-pass resolver
 * but are not present in the fixed-point plan.
 */
object PruneMetadataColumns extends Rule[LogicalPlan] {
   ...
   *
   * @param plan
   *   Operator for which we are pruning columns.
   * @param neededAttributes
   *   A set of [[ExprId]]s that is required by operators above [[plan]].
   *
  ...
  /**
   * Prune unnecessary columns for a [[Project]] node and recursively do it for its children.
   * While pruning we preserve all non-qualified-access-only columns as well as any columns that
   * are needed in the operators above, but without duplicating them in the project list. This
   * behavior is consistent with fixed-point's behavior when
   * [[SQLConf.ONLY_NECESSARY_AND_UNIQUE_METADATA_COLUMNS]] is true. We don't support legacy
   * behavior in single-pass.
   *
   * IMPORTANT NOTE: In this case we only prune away only the qualified access only columns
   * instead of all metadata columns. This is because we can have metadata columns from sources
   * other than hidden output and additionally, when a column is resolved from any source, its
   * qualified only flag is removed (see more in [[AttributeSeq]]). Therefore, qualified access
   * only columns that appear in the project list must have come from artificial appending, and we
   * should potentially prune them.
   */
  private def pruneMetadataColumnsInProject(
      project: Project,
    ...
    val newProjectList = if (!neededAttributes.isEmpty) {
      project.projectList.collect {
        case namedExpression: NamedExpression
            if !namedExpression.toAttribute.qualifiedAccessOnly =>
          existingExprIds.add(namedExpression.exprId)
          namedExpression
        case namedExpression: NamedExpression
            if namedExpression.toAttribute.qualifiedAccessOnly && neededAttributes.contains(
              namedExpression.exprId) && !existingExprIds.contains(namedExpression.exprId) =>
          existingExprIds.add(namedExpression.exprId)
          namedExpression
      }
    } else {
  ...
  private def withNewChildrenPrunedByNeededAttributes(
    ...
    plan.withNewChildren(
      plan.children.map(child => pruneMetadataColumns(child, neededAttributes)))
  }
}
