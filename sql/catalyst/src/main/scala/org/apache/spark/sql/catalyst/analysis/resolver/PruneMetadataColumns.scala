...
object PruneMetadataColumns extends Rule[LogicalPlan] {
  ...
  private def pruneMetadataColumns(
      plan: LogicalPlan,
      neededAttributes: HashSet[ExprId]): LogicalPlan = {
    val planWithVisitedSubqueries = pruneMetadataColumnsInSubqueryExpressions(plan)
    planWithVisitedSubqueries match {
  ...
  private def pruneMetadataColumnsInSubqueryExpressions(operator: LogicalPlan): LogicalPlan =
    operator.transformExpressionsWithPruning(_.containsPattern(PLAN_EXPRESSION)) {
      case subqueryExpression: SubqueryExpression =>
        subqueryExpression.withNewPlan(
          pruneMetadataColumns(
            plan = subqueryExpression.plan,
            neededAttributes = new HashSet[ExprId]
          )
        )
    }
   ...
   * pruning we preserve all non-metadata columns as well as any columns that are needed in the
   * operators above, but without duplicating them in the project list. This behavior is consistent
   * with fixed-point's behavior when [[SQLConf.ONLY_NECESSARY_AND_UNIQUE_METADATA_COLUMNS]] is
   * true. We don't support legacy behavior in single-pass.
   */
  private def pruneMetadataColumnsInProject(project: Project, neededAttributes: HashSet[ExprId]) = {
    ...
    val newProjectList = if (!neededAttributes.isEmpty) {
      project.projectList.collect {
        case namedExpression: NamedExpression if !namedExpression.toAttribute.isMetadataCol =>
          existingExprIds.add(namedExpression.exprId)
          namedExpression
        case namedExpression: NamedExpression
            if namedExpression.toAttribute.isMetadataCol && neededAttributes.contains(
    ...
    val newProject = projectWithNewChildren.copy(projectList = newProjectList)
