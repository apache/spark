...
object LikeSimplification extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def derivePrefixStartsWith(
    ...
    if (!binaryCollation || !CollapseProject.isCheap(input) ||
      ...
      if (prefix.isEmpty || prefix.length == pattern.length || prefix.contains(escapeChar)) {
        // No leading literal (pattern starts with a wildcard), no wildcard at all (the latter is
        // already turned into `EqualTo` by `simplifyLike`), or the escape char appears in the
        // prefix. An escape-free prefix consists solely of literal characters, so the terminating
        // wildcard is genuine and `StartsWith(prefix)` is a sound necessary condition; if the
        // escape char is present we bail rather than resolve escapes here (an escape only after
        // the first wildcard, e.g. 'ab%c\%d%', keeps the clean prefix 'ab').
        None
      } else {
