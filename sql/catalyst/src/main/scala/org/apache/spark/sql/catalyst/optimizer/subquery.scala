...
object RewritePredicateSubquery extends Rule[LogicalPlan] with PredicateHelper {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformWithPruning(
    ...
    case Filter(condition, child)
      ...
      val newFilter: LogicalPlan = withoutSubquery match {
        case Nil => child
        case conditions => Filter(conditions.reduce(And.apply), child)
      }
      ...
      withSubquery.foldLeft(newFilter) {
        ...
        case (p, Not(InSubquery(values, ListQuery(sub, _, _, _, conditions, subHint)))) =>
          ...
          val finalJoinCond = (nullAwareJoinConds ++ conditions).reduceLeft(And.apply)
  ...
  private def rewriteExistentialExprWithAttrs(
    ...
    val newExprs = exprs.map { e =>
      e.transformDownWithPruning(_.containsAnyPattern(EXISTS_SUBQUERY, IN_SUBQUERY)) {
        case Exists(sub, _, _, conditions, subHint) =>
          ...
          val newCondition = conditions.reduceLeftOption(And.apply)
          newPlan =
        ...
        case InSubquery(values, ListQuery(sub, _, _, _, conditions, subHint)) =>
          ...
          val newSub = dedupSubqueryOnSelfJoin(newPlan, sub, Some(values))
          val inConditions = values.zip(newSub.output).map(EqualTo.tupled)
          val newConditions = (inConditions ++ conditions).reduceLeftOption(And.apply)
          val joinHint = JoinHint(None, subHint)
    ...
    (newExprs.reduceOption(And.apply), newPlan, introducedAttrs.toSeq)
  }
}
      ...
      case f @ Filter(cond, child) =>
        ...
        correlated match {
          ...
          case xs if local.nonEmpty =>
            val newFilter = Filter(local.reduce(And.apply), child)
    ...
    val newChild = subqueries.foldLeft(child) {
      ...
      needSingleJoin)) =>
        ...
        lazy val planWithoutCountBug = Project(
          ...
          Join(currentChild, query, joinType, conditions.reduceOption(And.apply), joinHint))
            ...
            if (havingNode.isEmpty) {
              ...
              Project(
                ...
                Join(currentChild,
                  ...
                  joinType, conditions.reduceOption(And.apply), joinHint))
...
object RewriteLateralSubquery extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformUpWithPruning(
    ...
    case LateralJoin(left, LateralSubquery(sub, _, _, joinCond, subHint), joinType, condition) =>
      ...
      val newCond = (condition ++ joinCond).reduceOption(And.apply)
