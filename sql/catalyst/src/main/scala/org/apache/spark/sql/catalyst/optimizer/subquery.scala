 ...
 *      pulled out as join conditions, value = selected column will also be used as join
 *      condition.
 */
object RewritePredicateSubquery extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def rewriteDomainJoinsIfPresent(
    ...
    if (SQLConf.get.decorrelateInnerQueryEnabledForExistsIn) {
      DecorrelateInnerQuery.rewriteDomainJoins(
        outerPlan,
        sub,
        splitConjunctivePredicates(joinCond.get))
    } else {
      sub
    }
  }
  ...
  private def dedupSubqueryOnSelfJoin(
    ...
    val outerReferences = valuesOpt
      .map(values => AttributeSet.fromAttributeSets(values.map(_.references)))
  ...
  def apply(plan: LogicalPlan): LogicalPlan =
    plan.transformWithPruning(_.containsAnyPattern(EXISTS_SUBQUERY, LIST_SUBQUERY)) {
      case Filter(condition, child)
        ...
        withSubquery.foldLeft(newFilter) {
          ...
          case (p, InSubquery(values, ListQuery(sub, _, _, _, conditions, subHint))) =>
            ...
            val join = Join(
              outerPlan,
              rewriteDomainJoinsIfPresent(outerPlan, newSub, joinCond),
              LeftSemi,
              joinCond,
          ...
          case (p, Not(InSubquery(values, ListQuery(sub, _, _, _, conditions, subHint)))) =>
            ...
            Join(
              outerPlan,
              rewriteDomainJoinsIfPresent(outerPlan, newSub, Some(finalJoinCond)),
              LeftAnti,
      ...
      case j: Join
        ...
        if (relevantSubqueries.isEmpty) {
          ...
          if (subqueriesReferencingLeft.isEmpty && subqueriesReferencingRight.isEmpty) {
            ...
            val newLeft = subqueriesReferencingLeft.foldLeft(j.left) { case (p, e) =>
            ...
            val newRight = subqueriesReferencingRight.foldLeft(j.right) { case (p, e) =>
              val (newCond, newInputPlan) = rewriteExistentialExpr(Seq(e), p)
              // Update the join condition to rewrite the subquery expression
              newCondition = newCondition.transform {
                case expr if expr.fastEquals(e) => newCond.get
      ...
      case p @ PhysicalAggregation(
            ...
            resultExpressions,
            child) if exprsContainsAggregateInSubquery(p.expressions) =>
        val aggExprs =
          aggregateExpressions.map(ae => Alias(ae, "_aggregateexpression")(ae.resultId))
      ...
      case u: UnaryNode
          if u.expressions.exists(SubqueryExpression.hasInOrCorrelatedExistsSubquery) =>
        handleUnaryNode(u)
    }
  ...
  private def handleUnaryNode(u: UnaryNode): LogicalPlan = {
    ...
    updatedNode match {
      case a: Aggregate if conf.getConf(WRAP_EXISTS_IN_AGGREGATE_FUNCTION) =>
        ...
        val aggFunctionReferences =
          a.aggregateExpressions.flatMap(extractAggregateExpressions).flatMap(_.references).toSet
  ...
  /**
   * Given a predicate expression and an input plan, it rewrites any embedded existential
   * sub-query into an existential join. It returns the rewritten expression together with the
   * updated plan, as well as the newly introduced attributes. Currently, it does not support NOT
   * IN nested inside a NOT expression. This case is blocked in the Analyzer.
   */
    ...
    val newExprs = exprs.map { e =>
      e.transformDownWithPruning(_.containsAnyPattern(EXISTS_SUBQUERY, IN_SUBQUERY)) {
        case Exists(sub, _, _, conditions, subHint) =>
          ...
          newPlan = buildJoin(
            newPlan,
            rewriteDomainJoinsIfPresent(newPlan, sub, newCondition),
            existenceJoin,
            newCondition,
            subHint)
        ...
        case InSubquery(values, ListQuery(sub, _, _, _, conditions, subHint)) =>
          ...
          newPlan = Join(
            newPlan,
            rewriteDomainJoinsIfPresent(newPlan, newSub, newConditions),
            ExistenceJoin(exists),
            newConditions,
            joinHint)
          introducedAttrs += exists
 ...
 * correlated predicates from subquery [[Filter]]s and adds the references of these predicates to
 * all intermediate [[Project]] and [[Aggregate]] clauses (if they are missing) in order to be
 * able to evaluate the predicates at the top level.
 *
...
object PullupCorrelatedPredicates extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def pullOutCorrelatedPredicates(
    ...
    def missingReferences(p: LogicalPlan): AttributeSet = {
      val localPredicateReferences = p
        .collect(predicateMap)
        .flatten
        .map(_.references)
        .reduceOption(_ ++ _)
        .getOrElse(AttributeSet.empty)
  ...
  private def guaranteedToReturnOneRow(query: LogicalPlan): Boolean = {
    ...
    val aggregates = aggNode.get.expressions.flatMap(_.collect { case a: AggregateExpression =>
      a
    })
  ...
  private def rewriteSubQueries(plan: LogicalPlan): LogicalPlan = {
    ...
    /**
     * This function is used as a aid to enforce idempotency of pullUpCorrelatedPredicate rule. In
     * the first call to rewriteSubqueries, all the outer references from the subplan are pulled
     * up and join predicates are recorded as children of the enclosing subquery expression. The
     * subsequent call to rewriteSubqueries would simply re-records the `children` which would
    ...
    plan.transformExpressionsWithPruning(_.containsPattern(PLAN_EXPRESSION)) {
      case ScalarSubquery(
            ...
            hint,
            mayHaveCountBugOld,
            needSingleJoinOld) if children.nonEmpty =>
...
object RewriteCorrelatedScalarSubquery extends Rule[LogicalPlan] with AliasHelper {
  ...
  /**
   * Extract all correlated scalar subqueries from an expression. The subqueries are collected
   * using the given collector. The expression is rewritten and returned.
   */
  ...
  /**
   * Checks if given expression is foldable. Evaluates it and returns it as literal, if yes. If
   * not, returns the original expression without evaluation.
   */
  ...
  /**
   * Statically evaluate an expression containing zero or more placeholders, given a set of
   * bindings for placeholder values, if the expression is evaluable. If it is not, bind
   * statically evaluated expression results to an expression.
   */
  private def bindingExpr(expr: Expression, bindings: Map[ExprId, Expression]): Expression = {
    val rewrittenExpr = expr transform { case r: AttributeReference =>
  ...
  /**
   * Statically evaluate an [[Aggregate]] on an empty input and return a mapping between its
   * output attribute expression ID and evaluated result.
   */
   ...
   * [[org.apache.spark.sql.catalyst.analysis.CheckAnalysis]]. If the checks in CheckAnalysis
   * become less restrictive, this method will need to change.
   */
  ...
  /**
   * Split the plan for a scalar subquery into the parts above the innermost query block (first
   * part of returned value), the HAVING clause of the innermost query block (optional second
   * part) and the Aggregate below the HAVING CLAUSE (optional third part). When the third part is
   * empty, it means the subquery is a non-aggregated single-row subquery.
   */
  def splitSubquery(plan: LogicalPlan): (Seq[LogicalPlan], Option[Filter], Option[Aggregate]) = {
  ...
  /**
   * Construct a new child plan by left joining the given subqueries to a base plan. This method
   * returns the child plan and an attribute mapping for the updated `ExprId`s of subqueries. If
   * the non-empty mapping returned, this rule will rewrite subquery references in a parent plan
   * based on it.
   */
  private def constructLeftJoins(
    ...
    val newChild = subqueries.foldLeft(child) {
      case (
            currentChild,
            ScalarSubquery(sub, _, _, conditions, subHint, mayHaveCountBug, needSingleJoin)) =>
        ...
        if (!SQLConf.get.legacyDuplicateBetweenInput && currentChild.output.contains(
            origOutput)) {
          ...
          if (aggNode.isEmpty) {
            ...
            val alwaysTrueExpr =
              Alias(Literal.TrueLiteral, ALWAYS_TRUE_COLNAME)(exprId = alwaysTrueExprId)
            val alwaysTrueRef =
              AttributeReference(ALWAYS_TRUE_COLNAME, BooleanType)(exprId = alwaysTrueExprId)
            ...
            if (havingNode.isEmpty) {
              ...
              val subqueryResultExpr =
                Alias(
                  If(IsNull(alwaysTrueRef), resultWithZeroTups.get, aggValRef),
                  origOutput.name)()
  ...
  /**
   * Check if an [[Aggregate]] has no correlated subquery in aggregate expressions but still has
   * correlated scalar subqueries in its grouping expressions, which will not be rewritten.
   */
...
object OptimizeOneRowRelationSubquery extends Rule[LogicalPlan] {
  ...
  /**
   * Rewrite a subquery expression into one or more expressions. The rewrite can only be done if
   * there is no nested subqueries in the subquery plan.
   */
  private def rewrite(plan: LogicalPlan): LogicalPlan = plan.transformUpWithSubqueries {
    case LateralJoin(left, right @ LateralSubquery(OneRowSubquery(plan), _, _, _, _), _, None)
    ...
    case p: LogicalPlan =>
      p.transformExpressionsUpWithPruning(_.containsPattern(SCALAR_SUBQUERY)) {
        case s @ ScalarSubquery(
              OneRowSubquery(p @ Project(_, _: OneRowRelation)),
              _,
              _,
              _,
              _,
              _,
              _) if !hasCorrelatedSubquery(s.plan) && s.joinCond.isEmpty =>
