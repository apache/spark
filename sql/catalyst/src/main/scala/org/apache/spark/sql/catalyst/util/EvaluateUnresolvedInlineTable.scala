 ...
 * (whenever possible) with [[LocalRelation]]. Typically, [[UnresolvedInlineTable]] is a child of
 * [[InsertIntoStatement]]. Use the method: [[EvaluateUnresolvedInlineTable.evaluate]] as the
 * entry point for this transformation.
 */
object EvaluateUnresolvedInlineTable
  ...
  /**
   * This function attempts to early evaluate rows in inline table. If evaluation doesn't rely on
   * non-deterministic expressions (e.g. current_like) expressions will be evaluated and inlined
   * as [[LocalRelation]] This is package visible for unit testing.
   */
  ...
  /**
   * Validates that all inline table data are valid expressions that can be evaluated (in this
   * they must be foldable). Note that nondeterministic expressions are not supported since they
   * are not foldable. Exception are CURRENT_LIKE expressions, which are replaced by a literal in
   * later stages. This is package visible for unit testing.
   */
  ...
  def findCommonTypesAndCast(table: UnresolvedInlineTable): ResolvedInlineTable = {
    ...
    val (fields, columns) = table.rows.transpose
      .zip(table.names)
