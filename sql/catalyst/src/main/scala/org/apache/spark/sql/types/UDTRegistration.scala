    ...
    udtMap.get(userClass).map { udtClassName =>
      if (Utils.classIsLoadable(udtClassName)) {
        ...
        if (classOf[UserDefinedType[_]].isAssignableFrom(udtClass)) {
          ...
          throw SqlApiQueryExecutionErrors.notUserDefinedTypeError(udtClass.getName, userClass)
        }
      } else {
        throw SqlApiQueryExecutionErrors.cannotLoadUserDefinedTypeError(udtClassName, userClass)
      }
    }
  }
}
