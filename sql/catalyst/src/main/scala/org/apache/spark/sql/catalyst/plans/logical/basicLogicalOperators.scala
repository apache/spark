 ...
 */
case class View(desc: CatalogTable, output: Seq[Attribute], child: LogicalPlan)
    extends LogicalPlan
...
object Range {
  def apply(
      ...
      isStreaming: Boolean = false): Range = {
 ...
 * or be literals.
 */
case class CollectMetrics(name: String, metrics: Seq[NamedExpression], child: LogicalPlan)
    extends UnaryNode {
