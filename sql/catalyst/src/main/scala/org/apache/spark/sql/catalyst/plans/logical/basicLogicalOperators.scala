...
case class Intersect(left: LogicalPlan, right: LogicalPlan, isAll: Boolean)
  ...
  private def computeOutput(): Seq[Attribute] =
    Intersect.mergeChildOutputs(children.map(_.output))
}
...
/**
 * Helper base class for Union and UnionLoop logical nodes that contains their commonalities. It
 * reflects similarities of UNION ALL's two different use cases 1) for combining two or more
 * result sets, and 2) for combining anchor and recursive parts in the recursive CTE.
 *
 * Most of these methods historically existed in the Union node before the implementation of the
 * recursive CTE. One notable difference between two is that Union may have more than two children
 * while UnionLoop strictly has two (anchor and recursion).
 */
...
/**
 * Extractor and helper methods for Union and SequentialStreamingUnion. Does not match other
 * UnionBase subtypes like UnionLoop.
 */
object SequentialOrSimpleUnion {
  ...
  /**
   * Returns true if both unions are the same concrete type. Used during flattening to ensure
   * Union and SequentialStreamingUnion are not merged.
   */
  ...
  /**
   * Creates a new union of the same type with the specified children. This is needed when the
   * number of children may change (e.g., flattening) and we need to preserve the UnionBase return
   * type rather than getting back LogicalPlan.
   */
 ...
 *
 * NOTE: Child ordering is NOT semantically significant. Children are processed in parallel and
 * their order does not affect the result. This allows Union-specific optimizations to reorder
 * children (e.g., for performance), unlike SequentialStreamingUnion where order matters.
 *
 * @param byName
 ...
 * @param options
 *   The configuration used when reading data.
 */
case class View(
    desc: CatalogTable,
    isTempView: Boolean,
...
object Expand {
   ...
   *   3. The `groupingId` is added as the last output, using the bit mask as the concrete value
   *      for it.
   *   4. If `groupingSetsAttrs` has duplicate entries (e.g., `GROUPING SETS ((key), (key))`),
   *      adds one more virtual grouping attribute (`_gen_grouping_pos`) to avoid wrongly grouping
   *      rows with the same grouping ID.
   ...
   * @param child
   *   Child operator
   * @param childOutputOpt
   *   Optional child output. If not provided, then `child.output` is used
   */
  def apply(
   ...
   *
   * When a `groupByAlias` wraps a simple attribute (e.g., `Alias(c1#0, "c1")`), the Expand output
   * contains both the pass-through child attribute (`c1#0`) and a new grouping instance created
   * via `newInstance()` (e.g., `c1#5`). Both share the same name, which causes
   * `AMBIGUOUS_REFERENCE` errors during name-based resolution.
   *
   * By tagging the pass-through copy with `__is_duplicate`,
   * `AttributeSeq.getCandidatesForResolution` filters it out when multiple candidates match by
   * name, allowing the produced grouping attribute to be resolved instead. ExprId-based
   * resolution (used for already-resolved expressions like aggregate functions) is unaffected by
   * this metadata.
   *
   * Only child attributes whose ExprId matches a `groupByAlias` child that is a simple
   * `Attribute` are tagged. Complex grouping expressions (e.g., `c1 + 1`) produce aliases with
   * different names than any child column, so no name conflict arises.
   */
...
/**
 * Logical node that represents the LIMIT ALL operation. This operation is usually no-op and
 * exists to provide compatability with other databases. However, in case of recursive CTEs, Limit
 * nodes serve another purpose, to override the default row limit which is determined by a flag.
 * As a result, LIMIT ALL should also be used to completely negate the row limit, which is exactly
...
case class SubqueryAlias(identifier: AliasIdentifier, child: LogicalPlan)
  ...
  override def metadataOutput: Seq[Attribute] = {
    val canPropagate =
      if (conf.getConf(SQLConf.SUBQUERY_ALIAS_ALWAYS_PROPAGATE_METADATA_COLUMNS)) {
        true
      } else {
...
trait HasPartitionExpressions extends SQLConfHelper {
  ...
  protected def partitioning: Partitioning = if (partitionExpressions.isEmpty) {
    ...
    if (directShuffleExprs.nonEmpty) {
      assert(
        directShuffleExprs.length == 1 && partitionExpressions.length == 1,
      ...
      ShufflePartitionIdPassThrough(
        partitionExpressions.head.asInstanceOf[DirectShufflePartitionID],
        numPartitions)
    } else {
