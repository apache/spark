...
object SQLConf {
  ...
  val CODEGEN_COMPILE_MAX_CACHE_SIZE = buildConf("spark.sql.codegen.compile.maxCacheSize")
    .internal()
    .doc("The maximum number of compiled class cached in CodeGenerator.caches")
    .intConf
    .checkValue(maxCacheSize => maxCacheSize >= 0, "The maximum must be a positive interger " +
      "0 to disable cache")
    .createWithDefault(100)
...
class SQLConf extends Serializable with Logging {
  ...
  def codegenCompileMaxCacheSize: Int = getConf(CODEGEN_COMPILE_MAX_CACHE_SIZE)
