...
object SQLConf {
  ...
  val CODEGEN_HUGE_METHOD_LIMIT = buildConf("spark.sql.codegen.hugeMethodLimit")
    .internal()
    .doc("The maximum bytecode size of a single compiled Java function generated by codegen. " +
      "When the compiled function exceeds this threshold, the codegen is deactivated. " +
      s"The default value is ${CodeGenerator.DEFAULT_OPENJDK_JVM_HUGE_METHOD_LIMIT} and " +
      "this is a limit in the OpenJDK JVM implementation."
    )
    .intConf
    .createWithDefault(CodeGenerator.DEFAULT_OPENJDK_JVM_HUGE_METHOD_LIMIT)
...
class SQLConf extends Serializable with Logging {
  ...
  def maxLinesPerFunction: Int = getConf(WHOLESTAGE_MAX_LINES_PER_FUNCTION)
    .getOrElse(getConf(CODEGEN_HUGE_METHOD_LIMIT) / 2)

  def hugeMethodLimit: Int = getConf(CODEGEN_HUGE_METHOD_LIMIT)
