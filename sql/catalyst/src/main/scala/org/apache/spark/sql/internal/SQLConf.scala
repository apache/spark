...
object SQLConf {
  ...
  private def register(entry: ConfigEntry[_]): Unit = sqlConfEntriesUpdateLock.synchronized {
    require(
      !sqlConfEntries.containsKey(entry.key),
  ...
  /**
   * Default config. Only used when there is no active SparkSession for the thread. See [[get]]
  ...
  /**
   * Defines a getter that returns the SQLConf within scope. See [[get]] for more information.
   */
  ...
  /**
   * Returns the active config object within the current scope. If there is an active
   * SparkSession, the proper SQLConf associated with the thread's active session is used. If it's
   * called from tasks in the executor side, a SQLConf will be created from job local properties,
   * which are set and propagated from the driver side, unless a `SQLConf` has been set in the
   * scope by `withExistingConf` as done for propagating SQLConf for operations performed on RDDs
   * created from DataFrames.
   *
   * The way this works is a little bit convoluted, due to the fact that config was added
   * initially only for physical plans (and as a result not in sql/catalyst module).
   *
   * The first time a SparkSession is instantiated, we set the [[confGetter]] to return the active
   * SparkSession's config. If there is no active SparkSession, it returns using the thread local
   * [[fallbackConf]]. The reason [[fallbackConf]] is a thread local (rather than just a conf) is
   * to support setting different config options for different threads so we can potentially run
   * tests in parallel. At the time this feature was implemented, this was a no-op since we run
   * unit tests (that does not involve SparkSession) in serial order.
   */
  def get: SQLConf = {
  ...
  val UNION_IS_RESOLVED_WHEN_DUPLICATES_PER_CHILD_RESOLVED =
    buildConf("spark.sql.analyzer.unionIsResolvedWhenDuplicatesPerChildResolved")
      .internal()
      .version("4.1.0")
      .doc("When true, union should only be resolved once there are no duplicate attributes in " +
        "each branch.")
      .booleanConf
      .createWithDefault(true)
  ...
  val ANALYZER_SINGLE_PASS_RESOLVER_ENABLED_TENTATIVELY =
    buildConf("spark.sql.analyzer.singlePassResolver.enabledTentatively")
      .internal()
      .doc("When true, use the single-pass Resolver instead of the fixed-point Analyzer only if " +
  ...
  val ANALYZER_DUAL_RUN_SAMPLE_RATE =
    buildConf("spark.sql.analyzer.singlePassResolver.dualRunSampleRate")
      .internal()
      .doc("Represents the rate of queries that will be run in both fixed-point and single-pass " +
  ...
  val ANALYZER_SINGLE_PASS_RESOLVER_RUN_HEAVY_EXTENDED_RESOLUTION_CHECKS =
    buildConf("spark.sql.analyzer.singlePassResolver.runHeavyExtendedResolutionChecks")
      .internal()
      .doc("When true, run heavy `extendedResolutionChecks` after the main analysis. Otherwise skip " +
  ...
  val OPTIMIZER_INSET_SWITCH_THRESHOLD =
    buildConf("spark.sql.optimizer.inSetSwitchThreshold")
      ...
      .intConf
      .checkValue(
        threshold => threshold >= 0 && threshold <= 600,
        "The max set size " +
  ...
  val PLAN_CHANGE_VALIDATION = buildConf("spark.sql.planChangeValidation")
    .internal()
    .doc(
      "If true, Spark will validate all the plan changes made by analyzer/optimizer and other " +
  ...
  val EXPRESSION_TREE_CHANGE_LOG_LEVEL = buildConf("spark.sql.expressionTreeChangeLog.level")
    .internal()
    .doc(
      "Configures the log level for logging the change from the unresolved expression tree to " +
  ...
  val NAME_RESOLUTION_LOG_LEVEL = buildConf("spark.sql.nameResolutionLog.level")
    .internal()
    .doc(
      "Configures the log level for logging the name resolution in the single-pass bottom-up " +
  ...
  val LIGHTWEIGHT_PLAN_CHANGE_VALIDATION = buildConf("spark.sql.lightweightPlanChangeValidation")
    .internal()
    .doc(
      s"Similar to ${PLAN_CHANGE_VALIDATION.key}, this validates plan changes and runs after " +
  ...
  val TYPES_FRAMEWORK_ENABLED =
    buildConf("spark.sql.types.framework.enabled")
      .internal()
      .doc(
        "When true, use the Types Framework for supported types (currently TimeType). " +
  ...
  val EXTENDED_EXPLAIN_PROVIDERS = buildConf("spark.sql.extendedExplainProviders")
    .doc(
      "A comma-separated list of classes that implement the" +
  ...
  val DYNAMIC_PARTITION_PRUNING_REUSE_BROADCAST_ONLY =
    buildConf("spark.sql.optimizer.dynamicPartitionPruning.reuseBroadcastOnly")
      .internal()
      .doc(
        "When true, dynamic partition pruning will only apply when the broadcast exchange of " +
  ...
  val PLANNED_WRITE_ENABLED = buildConf("spark.sql.optimizer.plannedWrite.enabled")
    .internal()
    .doc(
      "When set to true, Spark optimizer will add logical sort operators to V1 write commands " +
  ...
  val EXPRESSION_PROJECTION_CANDIDATE_LIMIT =
    buildConf("spark.sql.optimizer.expressionProjectionCandidateLimit")
      .doc(
        "The maximum number of the candidate of output expressions whose alias are replaced." +
  ...
  val RADIX_SORT_ENABLED = buildConf("spark.sql.sort.enableRadixSort")
    .internal()
    .doc(
      "When true, enable use of radix sort when possible. Radix sort is much faster but " +
  ...
  val LIMIT_SCALE_UP_FACTOR = buildConf("spark.sql.limit.scaleUpFactor")
    .internal()
    .doc(
      "Minimal increase rate in number of partitions between attempts when executing a take " +
  ...
  val LEAF_NODE_DEFAULT_PARALLELISM = buildConf("spark.sql.leafNodeDefaultParallelism")
    .doc(
      "The default parallelism of Spark SQL leaf nodes that produce data, such as the file " +
  ...
  private[spark] val SHUFFLE_CHECKSUM_MISMATCH_QUERY_LEVEL_ROLLBACK_ENABLED =
    buildConf("spark.sql.shuffle.orderIndependentChecksum.enableQueryLevelRollbackOnMismatch")
      .internal()
      .doc(
        "Whether to rollback all the consumer stages from the same query executor " +
  ...
  val COALESCE_PARTITIONS_ENABLED =
    buildConf("spark.sql.adaptive.coalescePartitions.enabled")
      .doc(
        s"When true and '${ADAPTIVE_EXECUTION_ENABLED.key}' is true, Spark will coalesce " +
  ...
  val COALESCE_PARTITIONS_MIN_PARTITION_NUM =
    buildConf("spark.sql.adaptive.coalescePartitions.minPartitionNum")
      .internal()
      .doc(
        "(deprecated) The suggested (not guaranteed) minimum number of shuffle partitions " +
  ...
  val FETCH_SHUFFLE_BLOCKS_IN_BATCH =
    buildConf("spark.sql.adaptive.fetchShuffleBlocksInBatch")
      .internal()
      .doc(
        "Whether to fetch the contiguous shuffle blocks in batch. Instead of fetching blocks " +
  ...
  val LOCAL_SHUFFLE_READER_ENABLED =
    buildConf("spark.sql.adaptive.localShuffleReader.enabled")
      .doc(
        s"When true and '${ADAPTIVE_EXECUTION_ENABLED.key}' is true, Spark tries to use local " +
  ...
  val SKEW_JOIN_ENABLED =
    buildConf("spark.sql.adaptive.skewJoin.enabled")
      .doc(
        s"When true and '${ADAPTIVE_EXECUTION_ENABLED.key}' is true, Spark dynamically " +
  ...
  val SKEW_JOIN_SKEWED_PARTITION_FACTOR =
    buildConf("spark.sql.adaptive.skewJoin.skewedPartitionFactor")
      .doc(
        "A partition is considered as skewed if its size is larger than this factor " +
  ...
  val SKEW_JOIN_SKEWED_PARTITION_THRESHOLD =
    buildConf("spark.sql.adaptive.skewJoin.skewedPartitionThresholdInBytes")
      .doc(
        "A partition is considered as skewed if its size in bytes is larger than this " +
  ...
  val ADAPTIVE_MAX_SHUFFLE_HASH_JOIN_LOCAL_MAP_THRESHOLD =
    buildConf("spark.sql.adaptive.maxShuffledHashJoinLocalMapThreshold")
      .doc(
        "Configures the maximum size in bytes per partition that can be allowed to build " +
  ...
  val ADAPTIVE_CUSTOM_COST_EVALUATOR_CLASS =
    buildConf("spark.sql.adaptive.customCostEvaluatorClass")
      .doc(
        "The custom cost evaluator class to be used for adaptive execution. If not being set," +
  ...
  val SUBEXPRESSION_ELIMINATION_SKIP_FOR_SHORTCUT_EXPR =
    buildConf("spark.sql.subexpressionElimination.skipForShortcutExpr")
      .internal()
      .doc(
        "When true, shortcut eliminate subexpression with `AND`, `OR`. " +
  ...
  val PROPAGATE_DISTINCT_KEYS_ENABLED =
    buildConf("spark.sql.optimizer.propagateDistinctKeys.enabled")
      .internal()
      .doc(
        "When true, the query optimizer will propagate a set of distinct attributes from the " +
  ...
  val PARQUET_OUTPUT_COMMITTER_CLASS = buildConf("spark.sql.parquet.output.committer.class")
    ...
    .stringConf
    .checkValue(
      Utils.classIsLoadableAndAssignableFrom(_, classOf[OutputCommitter]),
  ...
  val ORC_COMPRESSION = buildConf("spark.sql.orc.compression.codec")
    .doc(
      "Sets the compression codec used when writing ORC files. If either `compression` or " +
  ...
  val ORC_IMPLEMENTATION = buildConf("spark.sql.orc.impl")
    .doc(
      "When native, use the native version of ORC support instead of the ORC library in Hive. " +
  ...
  val HIVE_METASTORE_PARTITION_PRUNING_FAST_FALLBACK =
    buildConf("spark.sql.hive.metastorePartitionPruningFastFallback")
      .doc(
        "When this config is enabled, if the predicates are not supported by Hive or Spark " +
  ...
  val HIVE_MANAGE_FILESOURCE_PARTITIONS =
    buildConf("spark.sql.hive.manageFilesourcePartitions")
      .doc(
        "When true, enable metastore partition management for file source tables as well. " +
  ...
  val HIVE_FILESOURCE_PARTITION_FILE_CACHE_SIZE =
    buildConf("spark.sql.hive.filesourcePartitionFileCacheSize")
      .doc(
        "When nonzero, enable caching of partition file metadata in memory. All tables share " +
  ...
  val HIVE_TABLE_PROPERTY_LENGTH_THRESHOLD =
    buildConf("spark.sql.hive.tablePropertyLengthThreshold")
      .internal()
      .doc(
        "The maximum length allowed in a single cell when storing Spark-specific information " +
  ...
  val GATHER_FASTSTAT = buildConf("spark.sql.hive.gatherFastStats")
    .internal()
    .doc(
      "When true, fast stats (number of files and total size of all files) will be gathered" +
  ...
  object BinaryOutputStyle extends Enumeration {
    ...
    val
    /**
     * Output as UTF-8 string. [83, 112, 97, 114, 107] -> "Spark"
     */
    UTF8: Value = Value("UTF-8")

    /**
     * Output as comma separated byte array string. [83, 112, 97, 114, 107] -> [83, 112, 97, 114,
     * 107]
     */
    val BASIC,
    /**
     * Output as base64 encoded string. [83, 112, 97, 114, 107] -> U3Bhcmsg
     */
    BASE64,
    /**
     * Output as hex string. [83, 112, 97, 114, 107] -> 537061726b
     */
    HEX,
    /**
     * Output as discrete hex string. [83, 112, 97, 114, 107] -> [53 70 61 72 6b]
     */
    HEX_DISCRETE = Value
  }
  ...
  val V2_BUCKETING_ENABLED = buildConf("spark.sql.sources.v2.bucketing.enabled")
    .doc(
      s"Similar to ${BUCKETING_ENABLED.key}, this config is used to enable bucketing for V2 " +
  ...
  val V2_BUCKETING_SHUFFLE_ENABLED =
    buildConf("spark.sql.sources.v2.bucketing.shuffle.enabled")
      .doc(
        "During a storage-partitioned join, whether to allow to shuffle only one side. " +
  ...
  val V2_BUCKETING_ALLOW_JOIN_KEYS_SUBSET_OF_PARTITION_KEYS =
    buildConf("spark.sql.sources.v2.bucketing.allowJoinKeysSubsetOfPartitionKeys.enabled")
      .doc(
        "Whether to allow storage-partition join in the case where join keys are " +
  ...
  val V2_BUCKETING_ALLOW_COMPATIBLE_TRANSFORMS =
    buildConf("spark.sql.sources.v2.bucketing.allowCompatibleTransforms.enabled")
      .doc(
        "Whether to allow storage-partition join in the case where the partition transforms " +
  ...
  val V2_BUCKETING_PARTITION_FILTER_ENABLED =
    buildConf("spark.sql.sources.v2.bucketing.partition.filter.enabled")
      .doc(
        s"Whether to filter partitions when running storage-partition join. " +
  ...
  val BUCKETING_MAX_BUCKETS = buildConf("spark.sql.sources.bucketing.maxBuckets")
    ...
    .checkValue(
      _ > 0,
      "the value of spark.sql.sources.bucketing.maxBuckets must be greater than 0")
  ...
  val CAN_CHANGE_CACHED_PLAN_OUTPUT_PARTITIONING =
    buildConf("spark.sql.optimizer.canChangeCachedPlanOutputPartitioning")
      .internal()
      .doc(
        "Whether to forcibly enable some optimization rules that can change the output " +
  ...
  val VIEW_SCHEMA_COMPENSATION = buildConf("spark.sql.legacy.viewSchemaCompensation")
    .internal()
    .doc(
      "Set to false to revert default view schema binding mode from WITH SCHEMA COMPENSATION " +
  ...
  val VIEW_SCHEMA_EVOLUTION_PRESERVE_USER_COMMENTS =
    buildConf("spark.sql.view.schemaEvolution.preserveUserComments")
      .internal()
      .doc(
        "When enabled, views with SCHEMA EVOLUTION mode will preserve user-set view comments " +
  ...
  val FILE_COMMIT_PROTOCOL_CLASS =
    buildConf("spark.sql.sources.commitProtocolClass")
      ...
      .stringConf
      .checkValue(
        Utils.classIsLoadableAndAssignableFrom(_, classOf[FileCommitProtocol]),
  ...
  val PARALLEL_PARTITION_DISCOVERY_THRESHOLD =
    buildConf("spark.sql.sources.parallelPartitionDiscovery.threshold")
      .doc(
        "The maximum number of paths allowed for listing files at driver side. If the number " +
      ...
      .checkValue(
        parallel => parallel >= 0,
        "The maximum number of paths allowed for listing " +
  ...
  val IGNORE_DATA_LOCALITY =
    buildConf("spark.sql.sources.ignoreDataLocality")
      .doc(
        "If true, Spark will not fetch the block locations for each file on " +
  ...
  val SESSION_FUNCTION_RESOLUTION_ORDER =
    buildConf("spark.sql.functionResolution.sessionOrder")
      ...
      .version("4.2.0")
      .doc(
        "Order of the session (temporary) function namespace when resolving unqualified " +
  ...
  val PRIORITIZE_SYSTEM_CATALOG =
    buildConf("spark.sql.legacy.prioritizeSystemCatalog")
      ...
      .doc("When true, partially qualified function names like 'builtin.func' or 'session.func' " +
        "will prioritize the system catalog (system.builtin.func or system.session.func) over " +
        "persistent schemas with matching names. When false, persistent schemas take precedence. " +
        "Default is true for security to prevent malicious schemas from shadowing system " +
        "functions.")
      .booleanConf
      .createWithDefault(true)
  ...
  val CODEGEN_FACTORY_MODE = buildConf("spark.sql.codegen.factoryMode")
    .internal()
    .doc(
      "This config determines the fallback behavior of several codegen generators " +
  ...
  val CODEGEN_FALLBACK = buildConf("spark.sql.codegen.fallback")
    .internal()
    .doc(
      "When true, (whole stage) codegen could be temporary disabled for the part of query that" +
  ...
  val IGNORE_INVALID_PARTITION_PATHS = buildConf("spark.sql.files.ignoreInvalidPartitionPaths")
    .doc(
      "Whether to ignore invalid partition paths that do not match <column>=<value>. When " +
  ...
  val NUM_STATE_STORE_MAINTENANCE_THREADS =
    buildConf("spark.sql.streaming.stateStore.numStateStoreMaintenanceThreads")
      .internal()
      .doc(
        "Number of threads in the thread pool that perform clean up and snapshotting tasks " +
  ...
  val STATE_STORE_MAINTENANCE_FORCE_SHUTDOWN_TIMEOUT =
    buildConf("spark.sql.streaming.stateStore.maintenanceForceShutdownTimeout")
      .internal()
      .doc(
        "Timeout in seconds to wait for tasks to respond to cancellation after " +
  ...
  val STATE_STORE_AUTO_SNAPSHOT_REPAIR_ENABLED =
    buildConf("spark.sql.streaming.stateStore.autoSnapshotRepair.enabled")
      .internal()
      .doc(
        "When true, enables automatic repair of state store snapshot, when a bad snapshot is " +
  ...
  val STATE_STORE_AUTO_SNAPSHOT_REPAIR_NUM_FAILURES_BEFORE_ACTIVATING =
    buildConf("spark.sql.streaming.stateStore.autoSnapshotRepair.numFailuresBeforeActivating")
      ...
      .checkValue(
        k => k > 0,
        "Must allow at least 1 failure before activating autoSnapshotRepair")
      .createWithDefault(1)
  ...
  val STATE_STORE_AUTO_SNAPSHOT_REPAIR_MAX_CHANGE_FILE_REPLAY =
    buildConf("spark.sql.streaming.stateStore.autoSnapshotRepair.maxChangeFileReplay")
      .internal()
      .doc("When autoSnapshotRepair is enabled, this specifies the maximum number of change " +
  ...
  val STATE_STORE_COORDINATOR_MULTIPLIER_FOR_MIN_VERSION_DIFF_TO_LOG =
    buildConf("spark.sql.streaming.stateStore.multiplierForMinVersionDiffToLog")
      .internal()
      .doc("Determines the version threshold for logging warnings when a state store falls behind. " +
  ...
  val STATE_STORE_COORDINATOR_SNAPSHOT_LAG_REPORT_INTERVAL =
    buildConf("spark.sql.streaming.stateStore.snapshotLagReportInterval")
      .internal()
      .doc("The minimum amount of time between the state store coordinator's reports on " +
  ...
  val STATE_STORE_ROW_CHECKSUM_ENABLED =
    buildConf("spark.sql.streaming.stateStore.rowChecksum.enabled")
      .internal()
      .doc(
        "When true, checksum would be generated and verified for each state store row. " +
  ...
  val RATIO_EXTRA_SPACE_ALLOWED_IN_CHECKPOINT =
    buildConf("spark.sql.streaming.ratioExtraSpaceAllowedInCheckpoint")
      .internal()
      .doc(
        "The ratio of extra space allowed for batch deletion of files when maintenance is" +
  ...
  val MAX_VERSIONS_TO_DELETE_PER_MAINTENANCE =
    buildConf("spark.sql.streaming.stateStore.maxVersionsToDeletePerMaintenance")
      .internal()
      .doc(
        "The maximum number of versions to delete per maintenance operation. By default, " +
  ...
  val MAX_BATCHES_TO_RETAIN_IN_MEMORY = buildConf(
    "spark.sql.streaming.maxBatchesToRetainInMemory")
    .internal()
    .doc(
      "The maximum number of batches which will be retained in memory to avoid " +
  ...
  val STREAMING_STATE_STORE_ENCODING_FORMAT =
    buildConf("spark.sql.streaming.stateStore.encodingFormat")
      ...
      .transform(_.toLowerCase(Locale.ROOT))
      .checkValue(
        v => Set("unsaferow", "avro").contains(v),
  ...
  val ENABLE_STREAMING_SOURCE_EVOLUTION =
    buildConf("spark.sql.streaming.queryEvolution.enableSourceEvolution")
      .internal()
      .doc(
        "When true, enforces that all streaming sources must be explicitly named via " +
  ...
  val STREAMING_CHECK_UNFINISHED_REPARTITION_ON_RESTART =
    buildConf("spark.sql.streaming.checkUnfinishedRepartitionOnRestart")
      .internal()
      .doc(
        "When true, checks if the latest batch is for an unfinished state repartitioning " +
  ...
  val STREAMING_CHECKPOINT_VERIFY_METADATA_EXISTS =
    buildConf("spark.sql.streaming.checkpoint.verifyMetadataExists.enabled")
      .internal()
      .doc(
        "When true, validates that the checkpoint metadata file exists when offset " +
  ...
  val STATE_STORE_UNLOAD_ON_COMMIT =
    buildConf("spark.sql.streaming.stateStore.unloadOnCommit")
      .internal()
      .doc(
        "When true, Spark will synchronously run maintenance and then close each StateStore " +
  ...
  val STATE_STORE_COMMIT_VALIDATION_ENABLED =
    buildConf("spark.sql.streaming.stateStore.commitValidation.enabled")
      .doc(
        "When true, Spark will validate that all StateStore instances have committed for " +
  ...
  val STATE_STORE_ROCKSDB_MERGE_OPERATOR_VERSION =
    buildConf("spark.sql.streaming.stateStore.rocksdb.mergeOperatorVersion")
      .internal()
      .doc(
        "Set the RocksDB merge operator version. This will be stored in the checkpoint when " +
  ...
  val STREAMING_AGGREGATION_STATE_FORMAT_VERSION =
    buildConf("spark.sql.streaming.aggregation.stateFormatVersion")
      .internal()
      .doc(
        "State format version used by streaming aggregation operations in a streaming query. " +
  ...
  val STREAMING_SESSION_WINDOW_MERGE_SESSIONS_IN_LOCAL_PARTITION =
    buildConf("spark.sql.streaming.sessionWindow.merge.sessions.in.local.partition")
      .doc(
        "When true, streaming session window sorts and merge sessions in local partition " +
   ...
   * Its possible for us to see nulls if state was written with an older version of Spark, the
   * state was corrupted on disk or if we had an issue with the state iterators.
   */
  val STATE_STORE_SKIP_NULLS_FOR_STREAM_STREAM_JOINS =
    buildConf("spark.sql.streaming.stateStore.skipNullsForStreamStreamJoins.enabled")
      .internal()
      .doc(
        "When true, this config will skip null values in hash based stream-stream joins. " +
  ...
  val STREAMING_TRIGGER_AVAILABLE_NOW_WRAPPER_ENABLED =
    buildConf("spark.sql.streaming.triggerAvailableNowWrapper.enabled")
      .internal()
      .doc(
        "Whether to use the wrapper implementation of Trigger.AvailableNow if the source " +
  ...
  val STREAMING_REAL_TIME_MODE_MIN_BATCH_DURATION =
    buildConf("spark.sql.streaming.realTimeMode.minBatchDuration")
      .doc("The minimum long-running batch duration in milliseconds for real-time mode.")
      .version("4.1.0")
      .timeConf(TimeUnit.MILLISECONDS)
      .createWithDefault(5000)

  val STREAMING_REAL_TIME_MODE_ALLOWLIST_CHECK =
    buildConf("spark.sql.streaming.realTimeMode.allowlistCheck")
  ...
  val ENABLE_TWOLEVEL_AGG_MAP =
    buildConf("spark.sql.codegen.aggregate.map.twolevel.enabled")
      .internal()
      .doc(
        "Enable two-level aggregate hash map. When enabled, records will first be " +
  ...
  val CODEGEN_SPLIT_AGGREGATE_FUNC =
    buildConf("spark.sql.codegen.aggregate.splitAggregateFunc.enabled")
      .internal()
      .doc(
        "When true, the code generator would split aggregate code into individual methods " +
  ...
  val MAX_NESTED_VIEW_DEPTH =
    buildConf("spark.sql.view.maxNestedViewDepth")
      ...
      .checkValue(
        depth => depth > 0,
        "The maximum depth of a view reference in a nested view " +
          "must be positive.")
      .createWithDefault(100)
  ...
  val ALLOW_TEMP_VIEW_CREATION_WITH_MULTIPLE_NAME_PARTS =
    buildConf("spark.sql.legacy.allowTempViewCreationWithMultipleNameparts")
      .internal()
      .doc(
        "When true, temp view creation Dataset APIs will allow the view creation even if " +
  ...
  val JSON_EXPRESSION_OPTIMIZATION =
    buildConf("spark.sql.optimizer.enableJsonExpressionOptimization")
      .doc(
        "Whether to optimize JSON expressions in SQL optimizer. It includes pruning " +
  ...
  val FILE_SOURCE_SCHEMA_FORCE_NULLABLE =
    buildConf("spark.sql.streaming.fileSource.schema.forceNullable")
      .internal()
      .doc(
        "When true, force the schema of streaming file source to be nullable (including all " +
  ...
  val STREAMING_NO_DATA_MICRO_BATCHES_ENABLED =
    buildConf("spark.sql.streaming.noDataMicroBatches.enabled")
      .doc("Whether streaming micro-batch engine will execute batches without data " +
  ...
  val STREAMING_CHECKPOINT_FILE_CHECKSUM_SKIP_CREATION_IF_FILE_MISSING_CHECKSUM =
    buildConf("spark.sql.streaming.checkpoint.fileChecksum.skipCreationIfFileMissingChecksum")
      .internal()
      .doc(
        "When true, if a microbatch is retried, if a file already exists but its checksum " +
  ...
  val PARALLEL_FILE_LISTING_IN_STATS_COMPUTATION =
    buildConf("spark.sql.statistics.parallelFileListingInStatsComputation.enabled")
      .internal()
      .doc(
        "When true, SQL commands use parallel file listing, " +
  ...
  val PERCENTILE_ACCURACY =
    buildConf("spark.sql.statistics.percentile.accuracy")
      .internal()
      .doc(
        "Accuracy of percentile approximation when generating equi-height histograms. " +
  ...
  val UPDATE_PART_STATS_IN_ANALYZE_TABLE_ENABLED =
    buildConf("spark.sql.statistics.updatePartitionStatsInAnalyzeTable.enabled")
      .doc(
        "When this config is enabled, Spark will also update partition statistics in analyze " +
  ...
  val JOIN_REORDER_CARD_WEIGHT =
    buildConf("spark.sql.cbo.joinReorder.card.weight")
      .internal()
      .doc(
        "The weight of the ratio of cardinalities (number of rows) " +
  ...
  val SESSION_WINDOW_BUFFER_IN_MEMORY_THRESHOLD =
    buildConf("spark.sql.sessionWindow.buffer.in.memory.threshold")
      .internal()
      .doc(
        "Threshold for number of windows guaranteed to be held in memory by the " +
  ...
  val CLASSIC_SHUFFLE_DEPENDENCY_FILE_CLEANUP_ENABLED =
    buildConf("spark.sql.classic.shuffleDependency.fileCleanup.enabled")
      .doc(
        "When enabled, shuffle files will be cleaned up at the end of classic " +
  ...
  val ARROW_PYSPARK_SELF_DESTRUCT_ENABLED =
    buildConf("spark.sql.execution.arrow.pyspark.selfDestruct.enabled")
      .doc(
        "(Experimental) When true, make use of Apache Arrow's self-destruct and split-blocks " +
  ...
  val PYSPARK_BINARY_AS_BYTES =
    buildConf("spark.sql.execution.pyspark.binaryAsBytes")
      .doc(
        "When true, BinaryType is consistently mapped to bytes in PySpark. " +
  ...
  val PYTHON_DATA_SOURCE_PROFILER =
    buildConf("spark.sql.pyspark.dataSource.profiler")
      .doc(
        "Configure the Python Data Source profiler by enabling or disabling it " +
  ...
  val PYTHON_UDF_PROFILER =
    buildConf("spark.sql.pyspark.udf.profiler")
      .doc(
        "Configure the Python/Pandas UDF profiler by enabling or disabling it " +
          "with the option to choose between \"perf\" and \"memory\" types, " +
          "or unsetting the config disables the profiler. This is disabled by default.")
      .version("4.0.0")
      .stringConf
  ...
  val PYTHON_UDF_WORKER_TRACEBACK_DUMP_INTERVAL_SECONDS =
    buildConf("spark.sql.execution.pyspark.udf.tracebackDumpIntervalSeconds")
      .doc(s"Same as ${Python.PYTHON_WORKER_TRACEBACK_DUMP_INTERVAL_SECONDS.key} " +
  ...
  val PYTHON_UDF_DAEMON_KILL_WORKER_ON_FLUSH_FAILURE =
    buildConf("spark.sql.execution.pyspark.udf.daemonKillWorkerOnFlushFailure")
      .doc(s"Same as ${Python.PYTHON_DAEMON_KILL_WORKER_ON_FLUSH_FAILURE.key} " +
        "for Python execution with DataFrame and SQL. It can change during runtime.")
      .version("4.1.0")
  ...
  val PYTHON_WORKER_LOGGING_ENABLED =
    buildConf("spark.sql.pyspark.worker.logging.enabled")
      .doc(
        "When set to true, this configuration enables comprehensive logging within " +
  ...
  val PYSPARK_PLOT_MAX_ROWS =
    buildConf("spark.sql.pyspark.plotting.max_rows")
      .doc(
        "The visual limit on plots. If set to 1000 for top-n-based plots (pie, bar, barh), " +
  ...
  val ARROW_EXECUTION_MAX_BYTES_PER_BATCH =
    buildConf("spark.sql.execution.arrow.maxBytesPerBatch")
      .internal()
      .doc(
        "When using Apache Arrow, limit the maximum bytes in each batch that can be written " +
  ...
  val ARROW_EXECUTION_ZSTD_COMPRESSION_LEVEL =
    buildConf("spark.sql.execution.arrow.compression.zstd.level")
      .doc(
        "Compression level for Zstandard (zstd) codec when compressing Arrow IPC data. " +
  ...
  val PYTHON_UDF_MAX_RECORDS_PER_BATCH =
    buildConf("spark.sql.execution.python.udf.maxRecordsPerBatch")
      ...
      .checkValue(
        _ > 0,
        "The value of spark.sql.execution.python.udf.maxRecordsPerBatch " +
          "must be positive.")
      .createWithDefault(100)
  ...
  val PYSPARK_HIDE_TRACEBACK =
    buildConf("spark.sql.execution.pyspark.udf.hideTraceback.enabled")
      .doc("When true, only show the message of the exception from Python UDFs, " +
  ...
  val PYTHON_UDF_PANDAS_INT_TO_DECIMAL_COERCION_ENABLED =
    buildConf("spark.sql.execution.pythonUDF.pandas.intToDecimalCoercionEnabled")
      .doc(
        "When true, convert int to Decimal python objects before converting " +
  ...
  val PYTHON_UDF_PANDAS_PREFER_INT_EXTENSION_DTYPE =
    buildConf("spark.sql.execution.pythonUDF.pandas.preferIntExtensionDtype")
      .doc(
        "When true, convert integers to Pandas ExtensionDtype (e.g. pandas.Int64Dtype) " +
  ...
  val PYTHON_TABLE_UDF_LEGACY_PANDAS_CONVERSION_ENABLED =
    buildConf("spark.sql.legacy.execution.pythonUDTF.pandas.conversion.enabled")
      .internal()
      .doc(
        s"When true and ${PYTHON_TABLE_UDF_ARROW_ENABLED.key} is enabled, extra pandas " +
  ...
  val PYTHON_UDF_LEGACY_PANDAS_CONVERSION_ENABLED =
    buildConf("spark.sql.legacy.execution.pythonUDF.pandas.conversion.enabled")
      .internal()
      .doc(
        s"When true and ${PYTHON_UDF_ARROW_ENABLED.key} is enabled, matches the " +
  ...
  val PYSPARK_WORKER_PYTHON_EXECUTABLE =
    buildConf("spark.sql.execution.pyspark.python")
      .internal()
      .doc(
        "Python binary executable to use for PySpark in executors when running Python " +
  ...
  val SQL_SCRIPTING_ENABLED =
    buildConf("spark.sql.scripting.enabled")
      .doc(
        "SQL Scripting feature is under development and its use should be done under this " +
  ...
  val DISABLE_MAP_KEY_NORMALIZATION =
    buildConf("spark.sql.legacy.disableMapKeyNormalization")
      .internal()
      .doc(
        "Disables key normalization when creating a map with `ArrayBasedMapBuilder`. When " +
  ...
  val FILE_SOURCE_INSERT_ENFORCE_NOT_NULL =
    buildConf("spark.sql.fileSource.insert.enforceNotNull")
      .doc(
        "When true, Spark enforces NOT NULL constraints when inserting data into " +
  ...
  val ENFORCE_RESERVED_KEYWORDS = buildConf("spark.sql.ansi.enforceReservedKeywords")
    .doc(
      s"When true and '${ANSI_ENABLED.key}' is true, the Spark SQL parser enforces the ANSI " +
  ...
  val LEGACY_CONSECUTIVE_STRING_LITERALS =
    buildConf("spark.sql.legacy.consecutiveStringLiterals.enabled")
      .internal()
      .doc(
        "When true, consecutive string literals separated by double quotes (e.g. 'a''b') will " +
  ...
  val CHUNK_BASE64_STRING_ENABLED = buildConf("spark.sql.chunkBase64String.enabled")
    .doc(
      "Whether to truncate string generated by the `Base64` function. When true, base64" +
  ...
  val ENABLE_DEFAULT_COLUMNS =
    buildConf("spark.sql.defaultColumn.enabled")
      .internal()
      .doc(
        "When true, allow CREATE TABLE, REPLACE TABLE, and ALTER COLUMN statements to set or " +
  ...
  val USE_NULLS_FOR_MISSING_DEFAULT_COLUMN_VALUES =
    buildConf("spark.sql.defaultColumn.useNullsForMissingDefaultValues")
      .internal()
      .doc(
        "When true, and DEFAULT columns are enabled, allow INSERT INTO commands with user-" +
  ...
  val SKIP_TYPE_VALIDATION_ON_ALTER_PARTITION =
    buildConf("spark.sql.legacy.skipTypeValidationOnAlterPartition")
      .internal()
      .doc(
        "When true, skip validation for partition spec in ALTER PARTITION. E.g., " +
  ...
  val SERIALIZER_NESTED_SCHEMA_PRUNING_ENABLED =
    buildConf("spark.sql.optimizer.serializer.nestedSchemaPruning.enabled")
      .internal()
      .doc(
        "Prune nested fields from object serialization operator which are unnecessary in " +
  ...
  val NESTED_PRUNING_ON_EXPRESSIONS =
    buildConf("spark.sql.optimizer.expression.nestedPruning.enabled")
      .internal()
      .doc(
        "Prune nested fields from expressions in an operator which are unnecessary in " +
  ...
  val DECORRELATE_UNION_OR_SET_OP_UNDER_LIMIT_ENABLED =
    buildConf("spark.sql.optimizer.decorrelateUnionOrSetOpUnderLimit.enabled")
      .internal()
      .doc(
        "Decorrelate UNION or SET operation under LIMIT operator. If not enabled," +
  ...
  val DECORRELATE_SUBQUERY_LEGACY_INCORRECT_COUNT_HANDLING_ENABLED =
    buildConf("spark.sql.optimizer.decorrelateSubqueryLegacyIncorrectCountHandling.enabled")
      .internal()
      .doc(
        "If enabled, revert to legacy incorrect behavior for certain subqueries with COUNT or " +
  ...
  val ALWAYS_INLINE_ONE_ROW_RELATION_SUBQUERY =
    buildConf("spark.sql.optimizer.optimizeOneRowRelationSubquery.alwaysInline")
      .internal()
      .doc(
        s"When true, the optimizer will always inline single row subqueries even if it " +
  ...
  val TOP_K_SORT_FALLBACK_THRESHOLD =
    buildConf("spark.sql.execution.topKSortFallbackThreshold")
      .doc(
        "In SQL queries with a SORT followed by a LIMIT like " +
  ...
  val CSV_INPUT_BUFFER_SIZE = buildConf("spark.sql.csv.parser.inputBufferSize")
    .internal()
    .doc(
      "If it is set, it configures the buffer size of CSV input during parsing. " +
  ...
  val REPL_EAGER_EVAL_MAX_NUM_ROWS = buildConf("spark.sql.repl.eagerEval.maxNumRows")
    .doc(
      "The max number of rows that are returned by eager evaluation. This only takes " +
  ...
  val FAST_HASH_AGGREGATE_MAX_ROWS_CAPACITY_BIT =
    buildConf("spark.sql.codegen.aggregate.fastHashMap.capacityBit")
      .internal()
      .doc(
        "Capacity for the max number of rows to be held in memory " +
  ...
  val AVRO_DEFLATE_LEVEL = buildConf("spark.sql.avro.deflate.level")
    .doc(
      "Compression level for the deflate codec used in writing of AVRO files. " +
  ...
  val AVRO_XZ_LEVEL = buildConf("spark.sql.avro.xz.level")
    .doc(
      "Compression level for the xz codec used in writing of AVRO files. " +
  ...
  val LEGACY_PARSE_NULL_PARTITION_SPEC_AS_STRING_LITERAL =
    buildConf("spark.sql.legacy.parseNullPartitionSpecAsStringLiteral")
      .internal()
      .doc(
        "If it is set to true, `PARTITION(col=null)` is parsed as a string literal of its " +
  ...
  val LEGACY_KEEP_PARTITION_SPEC_AS_STRING_LITERAL =
    buildConf("spark.sql.legacy.keepPartitionSpecAsStringLiteral")
      .internal()
      .doc(
        "If it is set to true, `PARTITION(col=05)` is parsed as a string literal of its " +
  ...
  val LEGACY_IDENTIFIER_CLAUSE_ONLY =
    buildConf("spark.sql.legacy.identifierClause")
      .internal()
      .doc(
        "When set to false, IDENTIFIER('literal') is resolved to an identifier at parse time " +
  ...
  val LEGACY_ALLOW_NEGATIVE_SCALE_OF_DECIMAL_ENABLED =
    buildConf("spark.sql.legacy.allowNegativeScaleOfDecimal")
      .internal()
      .doc(
        "When set to true, negative scale of Decimal type is allowed. For example, " +
  ...
  val LEGACY_BUCKETED_TABLE_SCAN_OUTPUT_ORDERING =
    buildConf("spark.sql.legacy.bucketedTableScan.outputOrdering")
      .internal()
      .doc(
        "When true, the bucketed table scan will list files during planning to figure out the " +
  ...
  val NAME_NON_STRUCT_GROUPING_KEY_AS_VALUE =
    buildConf("spark.sql.legacy.dataset.nameNonStructGroupingKeyAsValue")
      .internal()
      .doc(
        "When set to true, the key attribute resulted from running `Dataset.groupByKey` " +
  ...
  val MAX_TO_STRING_FIELDS = buildConf("spark.sql.debug.maxToStringFields")
    .doc(
      "Maximum number of fields of sequence-like entries can be converted to strings " +
  ...
  val MAX_PLAN_STRING_LENGTH = buildConf("spark.sql.maxPlanStringLength")
    ...
    .bytesConf(ByteUnit.BYTE)
    .checkValue(
      i => i >= 0 && i <= ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH,
      "Invalid " +
  ...
  val TIMESTAMP_TYPE =
    buildConf("spark.sql.timestampType")
      .doc(
        "Configures the default timestamp type of Spark SQL, including SQL DDL, Cast clause, " +
  ...
  val UI_EXPLAIN_MODE = buildConf("spark.sql.ui.explainMode")
    .doc(
      "Configures the query explain mode used in the Spark SQL UI. The value can be 'simple', " +
        "'extended', 'codegen', 'cost', or 'formatted'. The default value is 'formatted'.")
    .version("3.1.0")
    .stringConf
    .transform(_.toUpperCase(Locale.ROOT))
    .checkValue(
      mode => Set("SIMPLE", "EXTENDED", "CODEGEN", "COST", "FORMATTED").contains(mode),
  ...
  val LEGACY_CTE_PRECEDENCE_POLICY = buildConf("spark.sql.legacy.ctePrecedencePolicy")
    .internal()
    .doc(
      "When LEGACY, outer CTE definitions takes precedence over inner definitions. If set to " +
  ...
  val CTE_RECURSION_LEVEL_LIMIT = buildConf("spark.sql.cteRecursionLevelLimit")
    .internal()
    .doc(
      "Maximum level of recursion that is allowed while executing a recursive CTE definition." +
  ...
  val CTE_RECURSION_ROW_LIMIT = buildConf("spark.sql.cteRecursionRowLimit")
    .internal()
    .doc(
      "Maximum number of rows that can be returned when executing a recursive CTE definition." +
        "If a query does not get exhausted before reaching this limit it fails. Use -1 for " +
        "unlimited.")
    .version("4.1.0")
    .intConf
  ...
  val LEGACY_INLINE_CTE_IN_COMMANDS = buildConf("spark.sql.legacy.inlineCTEInCommands")
    .internal()
    .doc(
      "If true, always inline the CTE relations for the queries in commands. This is the " +
  ...
  val ADDITIONAL_REMOTE_REPOSITORIES =
    buildConf("spark.sql.maven.additionalRemoteRepositories")
      .doc(
        "A comma-delimited string config of the optional additional remote Maven mirror " +
  ...
  val LEGACY_FROM_DAYTIME_STRING =
    buildConf("spark.sql.legacy.fromDayTimeString.enabled")
      .internal()
      .doc(
        "When true, the `from` bound is not taken into account in conversion of " +
  ...
  val LEGACY_PROPERTY_NON_RESERVED =
    buildConf("spark.sql.legacy.notReserveProperties")
      .internal()
      .doc(
        "When true, all database and table properties are not reserved and available for " +
  ...
  val LEGACY_MYSQL_TIMESTAMPNTZ_MAPPING_ENABLED =
    buildConf("spark.sql.legacy.mysql.timestampNTZMapping.enabled")
      .internal()
      .doc(
        "When true, TimestampNTZType and MySQL TIMESTAMP can be converted bidirectionally. " +
  ...
  val LEGACY_POSTGRES_DATETIME_MAPPING_ENABLED =
    buildConf("spark.sql.legacy.postgres.datetimeMapping.enabled")
      .internal()
      .doc(
        "When true, TimestampType maps to TIMESTAMP WITHOUT TIME ZONE in PostgreSQL for " +
  ...
  val VARIANT_ALLOW_DUPLICATE_KEYS =
    buildConf("spark.sql.variant.allowDuplicateKeys")
      .internal()
      .doc(
        "When set to false, parsing variant from JSON will throw an error if there are " +
  ...
  val PARQUET_REBASE_MODE_IN_WRITE =
    buildConf("spark.sql.parquet.datetimeRebaseModeInWrite")
      .internal()
      .doc(
        "When LEGACY, Spark will rebase dates/timestamps from Proleptic Gregorian calendar " +
  ...
  val COALESCE_BUCKETS_IN_JOIN_ENABLED =
    buildConf("spark.sql.bucketing.coalesceBucketsInJoin.enabled")
      .doc(
        "When true, if two bucketed tables with the different number of buckets are joined, " +
  ...
  val COALESCE_BUCKETS_IN_JOIN_MAX_BUCKET_RATIO =
    buildConf("spark.sql.bucketing.coalesceBucketsInJoin.maxBucketRatio")
      .doc(
        "The ratio of the number of two buckets being coalesced should be less than or " +
  ...
  val BROADCAST_HASH_JOIN_OUTPUT_PARTITIONING_EXPAND_LIMIT =
    buildConf("spark.sql.execution.broadcastHashJoin.outputPartitioningExpandLimit")
      .internal()
      .doc(
        "The maximum number of partitionings that a HashPartitioning can be expanded to. " +
  ...
  val OPTIMIZE_NULL_AWARE_ANTI_JOIN =
    buildConf("spark.sql.optimizeNullAwareAntiJoin")
      .internal()
      .doc(
        "When true, NULL-aware anti join execution will be planed into " +
  ...
  val LEGACY_PATH_OPTION_BEHAVIOR =
    buildConf("spark.sql.legacy.pathOptionBehavior.enabled")
      .internal()
      .doc(
        "When true, \"path\" option is overwritten if one path parameter is passed to " +
  ...
  val LEGACY_EXTRA_OPTIONS_BEHAVIOR =
    buildConf("spark.sql.legacy.extraOptionsBehavior.enabled")
      .internal()
      .doc(
        "When true, the extra options will be ignored for DataFrameReader.table(). If set it " +
  ...
  val PRESERVE_CHAR_VARCHAR_TYPE_INFO = buildConf("spark.sql.preserveCharVarcharTypeInfo")
    .doc(
      "When true, Spark does not replace CHAR/VARCHAR types the STRING type, which is the " +
  ...
  val LEGACY_INTERVAL_ENABLED = buildConf("spark.sql.legacy.interval.enabled")
    .internal()
    .doc(
      "When set to true, Spark SQL uses the mixed legacy interval type `CalendarIntervalType` " +
  ...
  val INFER_PANDAS_DICT_AS_MAP = buildConf("spark.sql.execution.pandas.inferPandasDictAsMap")
    .doc(
      "When true, spark.createDataFrame will infer dict from Pandas DataFrame " +
  ...
  val LEGACY_INFER_ARRAY_TYPE_FROM_FIRST_ELEMENT =
    buildConf("spark.sql.pyspark.legacy.inferArrayTypeFromFirstElement.enabled")
      .internal()
      .doc(
        "PySpark's SparkSession.createDataFrame infers the element type of an array from all " +
  ...
  val LEGACY_INFER_MAP_STRUCT_TYPE_FROM_FIRST_ITEM =
    buildConf("spark.sql.pyspark.legacy.inferMapTypeFromFirstPair.enabled")
      .internal()
      .doc(
        "PySpark's SparkSession.createDataFrame infers the key/value types of a map from all " +
  ...
  val LEGACY_NULL_VALUE_WRITTEN_AS_QUOTED_EMPTY_STRING_CSV =
    buildConf("spark.sql.legacy.nullValueWrittenAsQuotedEmptyStringCsv")
      .internal()
      .doc(
        "When set to false, nulls are written as unquoted empty strings in CSV data source. " +
  ...
  val LEGACY_ALLOW_NULL_COMPARISON_RESULT_IN_ARRAY_SORT =
    buildConf("spark.sql.legacy.allowNullComparisonResultInArraySort")
      .internal()
      .doc(
        "When set to false, `array_sort` function throws an error " +
  ...
  val LEGACY_AVRO_ALLOW_INCOMPATIBLE_SCHEMA =
    buildConf("spark.sql.legacy.avro.allowIncompatibleSchema")
      .internal()
      .doc(
        "When set to false, if types in Avro are encoded in the same format, but " +
  ...
  val LEGACY_NON_IDENTIFIER_OUTPUT_CATALOG_NAME =
    buildConf("spark.sql.legacy.v1IdentifierNoCatalog")
      .internal()
      .doc(
        s"When set to false, the v1 identifier will include '$SESSION_CATALOG_NAME' as " +
  ...
  val LEGACY_IN_SUBQUERY_NULLABILITY =
    buildConf("spark.sql.legacy.inSubqueryNullability")
      .internal()
      .doc(
        s"When set to false, IN subquery nullability is correctly calculated based on " +
  ...
  val LEGACY_NULL_IN_EMPTY_LIST_BEHAVIOR =
    buildConf("spark.sql.legacy.nullInEmptyListBehavior")
      .internal()
      .doc(
        "When set to true, restores the legacy incorrect behavior of IN expressions for " +
  ...
  val ERROR_MESSAGE_FORMAT = buildConf("spark.sql.error.messageFormat")
    .doc(
      "When PRETTY, the error message consists of textual representation of error class, " +
  ...
  val LOCAL_RELATION_CHUNK_SIZE_ROWS =
    buildConf(SqlApiConfHelper.LOCAL_RELATION_CHUNK_SIZE_ROWS_KEY)
      .doc(
        "The chunk size in number of rows when splitting ChunkedCachedLocalRelation.data " +
  ...
  val LOCAL_RELATION_CHUNK_SIZE_BYTES =
    buildConf(SqlApiConfHelper.LOCAL_RELATION_CHUNK_SIZE_BYTES_KEY)
      .doc(
        "The chunk size in bytes when splitting ChunkedCachedLocalRelation.data " +
          "into batches. A new chunk is created when either " +
          "spark.sql.session.localRelationChunkSizeBytes " +
          "or spark.sql.session.localRelationChunkSizeRows is reached. " +
  ...
  val LOCAL_RELATION_BATCH_OF_CHUNKS_SIZE_BYTES =
    buildConf(SqlApiConfHelper.LOCAL_RELATION_BATCH_OF_CHUNKS_SIZE_BYTES_KEY)
      .internal()
      .doc(
        "Limit on how much memory the client can use when uploading a local relation to the " +
  ...
  val DECORRELATE_JOIN_PREDICATE_ENABLED =
    buildConf("spark.sql.optimizer.decorrelateJoinPredicate.enabled")
      .internal()
      .doc(
        "Decorrelate scalar and lateral subqueries with correlated references in join " +
  ...
  val DECORRELATE_PREDICATE_SUBQUERIES_IN_JOIN_CONDITION =
    buildConf("spark.sql.optimizer.decorrelatePredicateSubqueriesInJoinPredicate.enabled")
      .internal()
      .doc(
        "Decorrelate predicate (in and exists) subqueries with correlated references in join " +
          "predicates.")
      .version("4.0.0")
      .booleanConf
      .createWithDefault(true)
  ...
  val OPTIMIZE_UNCORRELATED_IN_SUBQUERIES_IN_JOIN_CONDITION =
    buildConf("spark.sql.optimizer.optimizeUncorrelatedInSubqueriesInJoinCondition.enabled")
      .internal()
      .doc(
        "When true, optimize uncorrelated IN subqueries in join predicates by rewriting them " +
  ...
  val LEGACY_NEGATIVE_INDEX_IN_ARRAY_INSERT =
    buildConf("spark.sql.legacy.negativeIndexInArrayInsert")
      .internal()
      .doc(
        "When set to true, restores the legacy behavior of `array_insert` for " +
  ...
  val LEGACY_RAISE_ERROR_WITHOUT_ERROR_CLASS =
    buildConf("spark.sql.legacy.raiseErrorWithoutErrorClass")
      .internal()
      .doc(
        "When set to true, restores the legacy behavior of `raise_error` and `assert_true` to " +
  ...
  val DATA_FRAME_QUERY_CONTEXT_ENABLED = buildConf("spark.sql.dataFrameQueryContext.enabled")
    .internal()
    .doc("Enable the DataFrame query context. This feature is enabled by default, but has a " +
  ...
  val LEGACY_CODING_ERROR_ACTION = buildConf("spark.sql.legacy.codingErrorAction")
    .internal()
    .doc(
      "When set to true, encode/decode functions replace unmappable characters with mojibake " +
  ...
  val LEGACY_EVAL_CURRENT_TIME = buildConf("spark.sql.legacy.earlyEvalCurrentTime")
    .internal()
    .doc(
      "When set to true, evaluation and constant folding will happen for now() and " +
  ...
  val LEGACY_BANG_EQUALS_NOT = buildConf("spark.sql.legacy.bangEqualsNot")
    .internal()
    .doc(
      "When set to true, '!' is a lexical equivalent for 'NOT'. That is '!' can be used " +
  ...
  val ORDERING_AWARE_LIMIT_OFFSET = buildConf("spark.sql.orderingAwareLimitOffset")
    .internal()
    .doc(
      "When set to true, a local sort will be inserted between GlobalLimitExec and " +
  ...
  val LAZY_SET_OPERATOR_OUTPUT = buildConf("spark.sql.lazySetOperatorOutput.enabled")
    .internal()
    .doc("When set to true, Except/Intersect/Union operator's output will be a lazy val. It " +
  ...
  val UNION_OUTPUT_PARTITIONING =
    buildConf("spark.sql.unionOutputPartitioning")
      .internal()
      .doc(
        "When set to true, the output partitioning of UnionExec will be the same as the " +
  ...
  val LEGACY_DF_WRITER_V2_IGNORE_PATH_OPTION =
    buildConf("spark.sql.legacy.dataFrameWriterV2IgnorePathOption")
      .internal()
      .doc(
        "When set to true, DataFrameWriterV2 ignores the 'path' option and always write data " +
  ...
  val VARIABLES_UNDER_IDENTIFIER_IN_VIEW =
    buildConf("spark.sql.legacy.allowSessionVariableInPersistedView")
      .internal()
      .doc("When set to true, variables can be found under identifiers in a view query. Throw " +
        "otherwise.")
      .version("4.1.0")
      .booleanConf
      .createWithDefault(false)
  ...
  val ENFORCE_TYPE_COERCION_BEFORE_UNION_DEDUPLICATION =
    buildConf("spark.sql.enforceTypeCoercionBeforeUnionDeduplication.enabled")
      .internal()
      .doc("When set to true, we enforce type coercion to run before deduplication of UNION " +
  ...
  val PRIORITIZE_ORDINAL_RESOLUTION_IN_SORT =
    buildConf("spark.sql.prioritizeOrdinalResolutionInSort.enabled")
      .internal()
      .doc("When set to true, we prioritize ordinal resolution in Sort over other expressions. " +
  ...
  val LEGACY_XML_PARSER_ENABLED = {
    buildConf("spark.sql.legacy.useLegacyXMLParser")
      .internal()
      .doc("When set to true, use the legacy XML parser for parsing XML files. " +
  ...
  val MERGE_INTO_NESTED_TYPE_COERCION_ENABLED =
    buildConf("spark.sql.mergeNestedTypeCoercion.enabled")
      .internal()
      .doc(
        "If enabled, allow MERGE INTO to coerce source nested types if they have less" +
  ...
  val LISTAGG_ALLOW_DISTINCT_CAST_WITH_ORDER =
    buildConf("spark.sql.listagg.allowDistinctCastWithOrder.enabled")
      .internal()
      .doc(
        "When true, LISTAGG(DISTINCT expr) WITHIN GROUP (ORDER BY expr) is allowed on " +
   ...
   * @param comment
   *   Additional info regarding to the removed config. For example, reasons of config
   *   deprecation, what users should use instead of it.
   */
   ...
   * @param defaultValue
   *   The default config value. It can be used to notice users that they set non-default value to
   *   an already removed config.
   * @param comment
  ...
  /**
   * The map contains info about removed SQL configs. Keys are SQL config names, map values
   * contain extra information like the version in which the config was removed, config's default
   * value and a comment.
   *
   * Please, add a removed SQL configuration property here only when it affects behaviours. For
   * example, `spark.sql.variable.substitute.depth` was not added as it virtually became no-op
   * later. By this, it makes migrations to new Spark versions painless.
   */
 ...
 *
 * In the presence of a SQLContext, these can be set and queried by passing SET commands into
 * Spark SQL's query functions (i.e. sql()). Otherwise, users of this class can modify the hints
 * by programmatically calling the setters and getters of this class.
 *
...
class SQLConf extends Serializable with Logging with SqlApiConf {
  ...
  def stateStoreMaintenanceShutdownTimeout: Long = getConf(
    STATE_STORE_MAINTENANCE_SHUTDOWN_TIMEOUT)
  ...
  def ratioExtraSpaceAllowedInCheckpoint: Double = getConf(
    RATIO_EXTRA_SPACE_ALLOWED_IN_CHECKPOINT)
  ...
  def inMemoryTableScanStatisticsEnabled: Boolean = getConf(
    IN_MEMORY_TABLE_SCAN_STATISTICS_ENABLED)
  ...
  def sessionWindowBufferInMemoryThreshold: Int = getConf(
    SESSION_WINDOW_BUFFER_IN_MEMORY_THRESHOLD)
  ...
  def allowAutoGeneratedAliasForView: Boolean = getConf(
    SQLConf.ALLOW_AUTO_GENERATED_ALIAS_FOR_VEW)
  ...
  def tvfAllowMultipleTableArguments: Boolean = getConf(
    TVF_ALLOW_MULTIPLE_TABLE_ARGUMENTS_ENABLED)
  ...
  def pythonUDFWorkerFaulthandlerEnabled: Boolean = getConf(
    PYTHON_UDF_WORKER_FAULTHANLDER_ENABLED)
  ...
  def continuousStreamingExecutorQueueSize: Int = getConf(
    CONTINUOUS_STREAMING_EXECUTOR_QUEUE_SIZE)
  ...
  override def enforceReservedKeywords: Boolean =
    ansiEnabled && getConf(ENFORCE_RESERVED_KEYWORDS)

  override def doubleQuotedIdentifiers: Boolean =
  ...
  /**
   * Returns the resolution search path for error messages and resolution order. This is the
   * single source of truth for the search path used for functions, tables, and views. Uses
   * [[sessionFunctionResolutionOrder]]: "first" (session first), "second" (session second),
  ...
  /**
   * Return the value of an optional Spark SQL configuration property for the given key. If the
   * key is not set yet, returns None.
   */
  ...
  def redactOptions[K, V](options: collection.Seq[(K, V)]): collection.Seq[(K, V)] = {
    val regexes =
      Seq(getConf(SQL_OPTIONS_REDACTION_PATTERN), SECRET_REDACTION_PATTERN.readFrom(reader))
  ...
  override def clone(): SQLConf = {
    ...
    getAllConfs.foreach { case (k, v) =>
      if (v ne null) result.setConfString(k, v)
    }
    result
  }
  ...
  def copy(entries: (ConfigEntry[_], Any)*): SQLConf = {
    val cloned = clone()
    entries.foreach { case (entry, value) =>
      cloned.setConfString(entry.key, value.toString)
    }
    cloned
  }
