...
object SQLConf {
  ...
  val ERROR_MESSAGE_FORMAT = buildConf("spark.sql.error.messageFormat")
    .doc("When PRETTY, the error message consists of textual representation of error class, " +
      "message and query context. Stack traces are only shown for internal errors (SQLSTATE XX***). " +
      "When DEBUG, the output is the same as PRETTY but stack traces are always included. " +
      "The MINIMAL and STANDARD formats are pretty JSON formats where " +
