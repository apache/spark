...
object SQLConf {
  ...
  val DEDUP_CASE_WHEN_BRANCHES_ENABLED =
    buildConf("spark.sql.optimizer.dedupCaseWhenBranches.enabled")
      .internal()
      .doc("When true, the query optimizer will remove duplicated expressions in CaseWhen " +
        "branches. Disable this rule by default due to check duplicated expressions has cost " +
        "and duplicated expressions are rare in handwritten query statements. A scenario " +
        "may be helpful to enable it is the query statements generated by visual SQL generator.")
      .version("3.2.0")
      .booleanConf
      .createWithDefault(false)
...
class SQLConf extends Serializable with Logging {
  ...
  def deduplicateCaseWhenBranchesEnabled: Boolean = getConf(DEDUP_CASE_WHEN_BRANCHES_ENABLED)
