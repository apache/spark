...
object UnsafeRowUtils {
   ...
   *   - UnsafeRow.calculateBitSetWidthInBytes(row.numFields) < row.getSizeInBytes should always
   *     be true if the expectedSchema contains at least one field.
   ...
   *         value, while the size is set to zero to signal that it's a null value. The offset may
   *         also be set to zero, in which case this variable-length Decimal no longer supports
   *         being mutable in the UnsafeRow.
   *     - otherwise the field is not null, then extract offset and size: 1) 0 <= size <
   *       row.getSizeInBytes should always be true. We can be even more precise than this, where
   *       the upper bound of size can only be as big as the variable length part of the row. 2)
   *       offset should be >= fixed sized part of the row. 3) offset + size should be within the
   *       row bounds.
   *   - For fixed-length fields that are narrower than 8 bytes (boolean/byte/short/int/float), if
   *     null bit says it's null then don't do anything, else: check if the unused bits in the
   *     field are all zeros. The UnsafeRowWriter's write() methods make this guarantee.
  ...
  private def validateStructuralIntegrityWithReasonImpl(
    ...
    if (expectedSchema.fields.length > 0 && bitSetWidthInBytes >= rowSizeInBytes) {
      return Some(
        s"rowSizeInBytes should not exceed bitSetWidthInBytes, " +
        ...
        field.dataType match {
          case dt: DecimalType if !UnsafeRow.isFixedLength(dt) =>
            ...
            if (size != 0 || offset != 0 &&
              ...
              return Some(
                s"Variable-length decimal field special case validation error: " +
                  s"field: $field, index: $index")
            }
          case _ =>
            if (row.getLong(index) != 0L) {
              return Some(
                s"Variable-length offset-size validation error: " +
                  s"field: $field, index: $index")
            }
        }
      case _ =>
  ...
  /**
   * Returns a Boolean indicating whether one should avoid calling UnsafeRow.setNullAt for a field
   * of the given data type. Fields of type DecimalType (with precision greater than
   * Decimal.MAX_LONG_DIGITS) and CalendarIntervalType use pointers into the variable length
   * region, and those pointers should never get zeroed out (setNullAt will zero out those
   * pointers) because UnsafeRow may do in-place update for these 2 types even though they are not
   * primitive.
   *
   * When avoidSetNullAt returns true, callers should not use UnsafeRow#setNullAt for fields of
   * that data type, but instead pass a null value to the appropriate set method, e.g.:
   *
   * row.setDecimal(ordinal, null, precision)
   *
   * Even though only UnsafeRow has this limitation, it's safe to extend this rule to all
   * subclasses of InternalRow, since you don't always know the concrete type of the row you are
   * dealing with, and all subclasses of InternalRow will handle a null value appropriately.
   */
  ...
  def getStructuralIntegrityStatus(row: UnsafeRow, expectedSchema: StructType): String = {
    ...
    val fieldStatusArr =
      expectedSchema.fields.take(minLength).zipWithIndex.map { case (field, index) =>
   ...
   * Unsafe row. i.e. binary(e1) = binary(e2) <=> e1 = e2. e.g. this is not true for non-binary
   * collations (any case/accent insensitive collation can lead to rows being semantically equal
   * even though their binary representations differ).
   */
