...
class AggregateExpressionResolver(
   ...
   *     [[OuterReference.SINGLE_PASS_OUTER_AGGREGATE_ALIAS_NAME_OVERRIDE]], both computed from
   *     the pretty-printed original [[AggregateExpression]] (before [[OuterReference]]
   *     stripping). The former overrides [[OuterReference.sql]] and the latter overrides the
   *     alias name assigned by [[AliasResolution.resolve]].
