 ...
 *
 * @param dataSchema
 *   The CSV data schema that is specified by the user, or inferred from underlying data files.
 * @param requiredSchema
 *   The schema of the data that should be output for each row. This should be a subset of the
 *   columns in dataSchema.
 * @param options
 *   Configuration options for a CSV parser.
 * @param filters
...
class UnivocityParser(
   ...
   *
   * For other nullable types, returns null if it is null or equals to the value specified in
   * `nullValue` option.
   */
  def makeConverter(name: String, dataType: DataType, nullable: Boolean = true): ValueConverter =
    dataType match {
      case _: ByteType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.toByte)

      case _: ShortType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.toShort)

      case _: IntegerType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.toInt)

      case _: LongType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.toLong)
      ...
      case _: BooleanType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.toBoolean)
      ...
      case _: StringType =>
        (d: String) => nullSafeDatum(d, name, nullable, options)(UTF8String.fromString)

      case _: BinaryType => (d: String) => nullSafeDatum(d, name, nullable, options)(_.getBytes)
  ...
  private def nullSafeDatum(datum: String, name: String, nullable: Boolean, options: CSVOptions)(
      converter: ValueConverter): Any = {
  ...
  /**
   * Parses a single CSV string and turns it into either one resulting row or no row (if the the
   * record is malformed).
   */
  ...
  private val getToken = if (columnPruning) { (tokens: Array[String], index: Int) =>
    tokens(index)
  } else { (tokens: Array[String], index: Int) =>
    tokens(tokenIndexArr(index))
  }

  /**
   * The entire line of CSV data is collected into a single variant object. When
   * `headerColumnNames` is defined, the field names will be extracted from it. Otherwise, the
   * field names will have a a format of "_c$i" to match the position of the values in the CSV
   * data.
   */
   ...
   * input as a specific scalar type, or fails after trying all the types and defaults to the
   * string type. The state is reset for every input file.
   *
