 ...
 *
 * @param isEmptyPlan At AQE side, we use the query stage stats to check the check.
...
case class PropagateEmptyRelationAdvanced(
    isEmptyPlan: Option[LogicalPlan => Boolean] = None,
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformUpWithPruning(
    ...
    case p @ Join(_, _, joinType, conditionOpt, _)
      ...
      if (isLeftEmpty || isRightEmpty || isFalseCondition) {
        ...
        isEmpty.isDefined && !isEmpty.get(p.right)) {
        p.left
