...
object ColumnPruning extends Rule[LogicalPlan] {
  ...
  private def removeProjectBeforeFilter(plan: LogicalPlan): LogicalPlan = plan transformUp {
    ...
    case p1 @ Project(_, j @ Join(p2 @ Project(_, child), right, LeftSemiOrAnti(_), _))
      if p2.outputSet.subsetOf(child.outputSet) &&
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case join @ Join(p @ Project(pList, grandChild), rightOp, LeftSemiOrAnti(joinType), joinCond)
      if pList.forall(_.deterministic) && !ScalarSubquery.hasScalarSubquery(pList) &&
        canPushThroughCondition(Seq(grandChild), joinCond, rightOp) =>
      if (joinCond.isEmpty) {
        // No join condition, just push down the Join below Project
        Project(pList, Join(grandChild, rightOp, joinType, joinCond))
      } else {
        // Create a map of Aliases to their values from the child projection.
        // e.g., 'SELECT a + b AS c, d ...' produces Map(c -> a + b).
        ...
        Project(pList, Join(grandChild, rightOp, joinType, newJoinCond))
      }
    ...
    case join @ Join(aggregate: Aggregate, rightOp, LeftSemiOrAnti(joinType), joinCond) =>
      if (joinCond.isEmpty) {
        ...
        splitConjunctivePredicates(joinCond.get).partition(_.deterministic)
...
object PushLeftSemiLeftAntiThroughJoin extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def pushTo(child: Join, subquery: LogicalPlan, joinCond: Option[Expression]) = {
    ...
    if (joinCond.nonEmpty) {
      ...
      val (candidates, containingNonDeterministic) = conditions.partition(_.deterministic)
      lazy val (leftConditions, rest) =
        candidates.partition(_.references.subsetOf(left.outputSet ++ subqueryOutput))
      lazy val (rightConditions, commonConditions) =
        rest.partition(_.references.subsetOf(right.outputSet ++ subqueryOutput))

      if (containingNonDeterministic.nonEmpty) {
        noPushdown
      } else {
