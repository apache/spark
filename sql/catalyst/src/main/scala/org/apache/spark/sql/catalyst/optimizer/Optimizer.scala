  ...
  object OptimizeSubqueries extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan transformAllExpressions {
      case s: SubqueryExpression =>
        val ReturnAnswer(newPlan) = Optimizer.this.execute(ReturnAnswer(s.plan))
...
object RemoveRedundantAliases extends Rule[LogicalPlan] {
  ...
  private def removeRedundantAliases(plan: LogicalPlan, blacklist: AttributeSet): LogicalPlan = {
    plan match {
      // A ReturnAnswer node means that we want to keep the same output attributes. This means we
      // cannot remove aliases that produce these attributes. A ReturnAnswer node is currently only
      // used during sub query optimization.
      case ReturnAnswer(child) =>
        ReturnAnswer(removeRedundantAliases(child, blacklist ++ child.outputSet))
