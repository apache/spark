 ...
 * {{{
 *   SELECT a FROM t WHERE 1=0 GROUP BY a HAVING COUNT(*)>1 ORDER BY a ==> empty result
 * }}}
 */
object CollapseEmptyPlan extends Rule[LogicalPlan] with PredicateHelper {
  def apply(plan: LogicalPlan): LogicalPlan = plan transformUp {
    ...
    case a: Aggregate => a
    case p: LogicalPlan if p.children.nonEmpty && p.children.forall(x =>
        x.isInstanceOf[LocalRelation] && x.asInstanceOf[LocalRelation].data.isEmpty) =>
