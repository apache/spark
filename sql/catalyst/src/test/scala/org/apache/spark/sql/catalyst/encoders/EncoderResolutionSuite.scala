...
class EncoderResolutionSuite extends PlanTest {
  ...
  test("real type doesn't match encoder schema but they are compatible: array") {
    ...
    val attrs =
      Seq(Symbol("arr").array(new StructType().add("a", "int").add("b", "int").add("c", "int")))
  ...
  test("SPARK-28497: complex type is not compatible with string encoder schema") {
    ...
    Seq(
      Symbol("a").struct(Symbol("x").long),
      Symbol("a").array(StringType),
      Symbol("a").map(StringType, StringType)).foreach { attr =>
      val attrs = Seq(attr)
      assert(intercept[AnalysisException](encoder.resolveAndBind(attrs)).message ==
        s"""
  ...
  test("throw exception if real type is not compatible with encoder schema") {
    ...
    val msg2 = intercept[AnalysisException] {
      ...
      ExpressionEncoder[ComplexClass].resolveAndBind(
        Seq(Symbol("a").long, Symbol("b").struct(structType)))
    }.message
    assert(msg2 ==
      s"""
