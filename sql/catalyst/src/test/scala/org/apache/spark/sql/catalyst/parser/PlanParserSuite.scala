...
class PlanParserSuite extends PlanTest {
  ...
  test("insert into") {
    ...
    def insert(
        ...
        ifStaticPartitionNotExists: Boolean = false): LogicalPlan =
      InsertIntoTable(table("s"), partition, plan, overwrite, ifStaticPartitionNotExists)

    // Single inserts
    assertEqual(s"insert overwrite table s $sql",
      insert(Map.empty, overwrite = true))
    assertEqual(s"insert overwrite table s partition (e = 1) if not exists $sql",
      insert(Map("e" -> Option("1")), overwrite = true, ifStaticPartitionNotExists = true))
    ...
    assertEqual("from t insert into s select * limit 1 insert into u select * where x > 5",
      InsertIntoTable(
        table("s"), Map.empty, plan.limit(1), false, ifStaticPartitionNotExists = false).union(
        InsertIntoTable(
          table("u"), Map.empty, plan2, false, ifStaticPartitionNotExists = false)))
  }
