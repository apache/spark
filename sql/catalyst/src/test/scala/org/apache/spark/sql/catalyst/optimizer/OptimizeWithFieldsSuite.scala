...
class OptimizeWithFieldsSuite extends PlanTest {
  ...
  private val testRelation = LocalRelation("a".attr.struct("a1".attr.int))
  private val testRelation2 = LocalRelation("a".attr.struct("a1".attr.int).notNull)
  ...
  test("combines two adjacent UpdateFields Expressions") {
    ...
    val correctAnswer = testRelation
      .select(Alias(
        UpdateFields("a".attr, WithField("b1", Literal(4)) :: WithField("c1", Literal(5)) ::
        Nil), "out")())
      .analyze
  ...
  test("SPARK-32941: optimize WithFields followed by GetStructField") {
    val originalQuery = testRelation2
      .select(Alias(
        GetStructField(UpdateFields("a".attr,
  ...
  test("SPARK-32941: optimize WithFields chain - case insensitive") {
    ...
    val correctAnswer = testRelation
      .select(
        Alias(UpdateFields("a".attr, WithField("b1", Literal(5)) :: Nil), "out1")(),
        Alias(UpdateFields("a".attr, WithField("B1", Literal(5)) :: Nil), "out2")())
      .analyze
