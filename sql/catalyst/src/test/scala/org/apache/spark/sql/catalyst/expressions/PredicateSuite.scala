...
class PredicateSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("basic IN predicate test") {
    ...
    checkEvaluation(In(Seq(NonFoldableLiteral.create(null, IntegerType)), Seq.empty),
      null)
    checkEvaluation(In(Seq(Literal(1)), Seq.empty), false)
    checkEvaluation(
      In(Seq(Literal(1)), Seq(NonFoldableLiteral.create(null, IntegerType))), null)
    checkEvaluation(In(Seq(Literal(1)),
      Seq(Literal(1), NonFoldableLiteral.create(null, IntegerType))), true)
    checkEvaluation(In(Seq(Literal(2)),
    ...
    checkEvaluation(And(In(Seq(Literal(1)), Seq(Literal(1), Literal(2))),
        In(Seq(Literal(2)), Seq(Literal(1), Literal(2)))), true)
  ...
  test("IN with different types") {
    def testWithRandomDataGeneration(dataType: DataType, nullable: Boolean): Unit = {
      ...
      checkEvaluation(In(Seq(input.head), input.slice(1, 10)), expected)
    }
