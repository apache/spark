...
class DecorrelateInnerQuerySuite extends PlanTest {
  ...
  private def getRowNumberFunc(input: LogicalPlan): Alias = {
    ...
    windowFunction.expressions
      .collect({ case w: Alias if w.child.isInstanceOf[WindowExpression] => w })
      .head
  }
  ...
  test("filter with correlated equality predicates only") {
    ...
    val innerPlan =
      Project(Seq(a, b), Filter(OuterReference(x) === a, testRelation))
  ...
  test("filter with local and correlated equality predicates") {
    val outerPlan = testRelation2
    val innerPlan =
      Project(Seq(a, b), Filter(And(OuterReference(x) === a, b === 3), testRelation))
    val correctAnswer =
      Project(Seq(a, b), Filter(b === 3, testRelation))
    check(innerPlan, outerPlan, correctAnswer, Seq(x === a))
  }
  ...
  test("duplicated output attributes") {
    ...
    val innerPlan =
      Project(Seq(a), Filter(OuterReference(a) === a, testRelation))
    val (outputPlan, joinCond) = DecorrelateInnerQuery(innerPlan, outerPlan.select())
    val a1 = outputPlan.output.head
    val correctAnswer =
      Project(Seq(Alias(a, a1.name)(a1.exprId)), Project(Seq(a), testRelation))
    comparePlans(outputPlan, correctAnswer)
  ...
  test("filter with equality predicates with correlated values on both sides") {
    val outerPlan = testRelation2
    val innerPlan =
      Project(Seq(a), Filter(OuterReference(x) === OuterReference(y) + b, testRelation))
    val correctAnswer = Project(Seq(a, b), testRelation)
  ...
  test("aggregate with correlated equality predicates that can be pulled up") {
    ...
    val innerPlan =
      Aggregate(Nil, Seq(minB), Filter(And(OuterReference(x) === a, b === 3), testRelation))
    val correctAnswer =
      Aggregate(Seq(a), Seq(minB, a), Filter(b === 3, testRelation))
    check(innerPlan, outerPlan, correctAnswer, Seq(x === a))
  }
  ...
  test("aggregate with correlated equality predicates that cannot be pulled up") {
    ...
    val minB = Alias(min(b), "min_b")()
    val innerPlan =
      Aggregate(Nil, Seq(minB), Filter(OuterReference(x) === OuterReference(y) + a, testRelation))
    val correctAnswer =
      Aggregate(
        Seq(x, y),
        Seq(minB, x, y),
        Filter(x === y + a, DomainJoin(Seq(x, y), testRelation)))
  ...
  test("aggregate with correlated equality predicates that has no attribute") {
    ...
    val minB = Alias(min(b), "min_b")()
    val innerPlan =
      Aggregate(Nil, Seq(minB), Filter(OuterReference(x) === OuterReference(y), testRelation))
    val correctAnswer =
      Aggregate(Nil, Seq(minB), testRelation)
  ...
  test("aggregate with correlated non-equality predicates") {
    ...
    val minB = Alias(min(b), "min_b")()
    val innerPlan =
      Aggregate(Nil, Seq(minB), Filter(OuterReference(x) > a, testRelation))
    val correctAnswer =
      Aggregate(Seq(x), Seq(minB, x), Filter(x > a, DomainJoin(Seq(x), testRelation)))
  ...
  test("join with correlated equality predicates") {
    ...
    val left =
      Project(Seq(b), Filter(OuterReference(x) === b, testRelation)).as("t1")
    val right =
      Project(Seq(b), Filter(OuterReference(x) === a, testRelation)).as("t2")
  ...
  test("correlated values inside join condition") {
    ...
    val innerPlan =
      Join(
        ...
        JoinHint.NONE)
    val correctAnswer =
      Join(testRelation.as("t1"), testRelation3, Inner, Some(a === a), JoinHint.NONE)
  ...
  test("correlated values in project") {
    ...
    val correctAnswer = Project(Seq(x.as("x1"), y.as("y1"), x, y), DomainJoin(Seq(x, y), t0))
    check(innerPlan, outerPlan, correctAnswer, Seq(x <=> x, y <=> y))
  }
  ...
  test("correlated values in project with alias") {
    ...
    val innerPlan =
      Project(
        Seq(OuterReference(x).as("x1"), $"y1", $"sum"),
        Project(
          Seq(
            OuterReference(x),
    ...
    val correctAnswer =
      Project(
        Seq(x.as("x1"), $"y1", $"sum", x, y),
        Project(
          Seq(x.as(x.name), y.as("y1"), (x + y).as("sum"), x, y),
    ...
    val innerPlan =
      Project(
        ...
        Filter(
          ...
          testRelation))
    val correctAnswer =
      Project(Seq(y.as("y1"), y, a, c), Filter(b === 1, DomainJoin(Seq(y), testRelation)))
  ...
  test("correlated values in project with aggregate") {
    ...
    val correctAnswer =
      Aggregate(
        Seq($"x1", y, a),
        Seq(min($"y1").as("min_y1"), y, a),
        Project(Seq(a, a.as("x1"), y.as("y1"), y), DomainJoin(Seq(y), testRelation))).analyze
  ...
  test("SPARK-36124: union in correlation path") {
    ...
    val innerPlan =
      Union(
        Filter(And(OuterReference(x) === a, c === 3), testRelation),
        Filter(And(OuterReference(y) === b, c === 6), testRelation))
    val correctAnswer =
      Union(
        Project(
          Seq(a, b, c, x, y),
          Filter(And(x === a, c === 3), DomainJoin(Seq(x, y), testRelation))),
        Project(
          Seq(a, b, c, x, y),
          Filter(And(y === b, c === 6), DomainJoin(Seq(x, y), testRelation))))
    check(innerPlan, outerPlan, correctAnswer, Seq(x <=> x, y <=> y))
  }
  ...
  test("SPARK-36124: another union in correlation path") {
    ...
    val innerPlan =
      Union(
        Seq(
          Filter(And(OuterReference(x) === a, a > 2), testRelation),
          Filter(And(OuterReference(y) === b, b > 3), testRelation),
          Filter(And(OuterReference(z) === c, c > 4), testRelation)))
    val correctAnswer =
      Union(
        Seq(
          Project(
            Seq(a, b, c, x, y, z),
            Filter(And(x === a, a > 2), DomainJoin(Seq(x, y, z), testRelation))),
          Project(
            Seq(a, b, c, x, y, z),
            Filter(And(y === b, b > 3), DomainJoin(Seq(x, y, z), testRelation))),
          Project(
            Seq(a, b, c, x, y, z),
            Filter(And(z === c, c > 4), DomainJoin(Seq(x, y, z), testRelation)))))
  ...
  test("SPARK-36124: INTERSECT ALL in correlation path") {
    ...
    val innerPlan =
      Intersect(
        Filter(And(OuterReference(x) === a, c === 3), testRelation),
        Filter(And(OuterReference(y) === b3, c3 === 6), testRelation3),
        isAll = true)
    ...
    val correctAnswer =
      Intersect(
        ...
        Project(
          Seq(a3, b3, c3, x2, y2),
          Filter(And(y2 === b3, c3 === 6), DomainJoin(Seq(x2, y2), testRelation3))),
        isAll = true)
    check(innerPlan, outerPlan, correctAnswer, Seq(x <=> x, y <=> y))
  }
  ...
  test("SPARK-38155: distinct with non-equality correlated predicates") {
    val outerPlan = testRelation2
    val innerPlan =
      Distinct(Project(Seq(b), Filter(OuterReference(x) > a, testRelation)))
    val correctAnswer =
      Distinct(Project(Seq(b, x), Filter(x > a, DomainJoin(Seq(x), testRelation))))
    check(innerPlan, outerPlan, correctAnswer, Seq(x <=> x))
  }
      ...
      Aggregate(
        ...
        Project(
          ...
          Join(
            ...
            testRelation3,
            Inner,
            Some(And(x === a3, y === OuterReference(a))),
            JoinHint.NONE)))

    val correctAnswer =
      Aggregate(
        Seq(y),
        Seq(Alias(count(Literal(1)), "a")(), y),
        Project(
          Seq(x, y, a3, b3),
  ...
  test(
    "SPARK-43780: aggregation in subquery with correlated left join, " +
      ...
      Aggregate(
        ...
        Project(
          ...
          Join(
            ...
            testRelation3,
            LeftOuter,
            Some(And(x === a3, b3 === OuterReference(b))),
            JoinHint.NONE)))

    val correctAnswer =
      Aggregate(
        Seq(b),
        Seq(Alias(count(Literal(1)), "a")(), b),
        Project(
          Seq(x, y, a3, b3, b),
          Join(
            DomainJoin(Seq(b), testRelation2),
            testRelation3,
            LeftOuter,
            Some(And(b === b3, x === a3)),
            JoinHint.NONE)))
          ...
          Project(
            ...
            Project(
              ...
              Join(
                ...
                Some(And(x === a3, y === a)),
                JoinHint.NONE))),
          Project(Seq(a4, b4, a), DomainJoin(Seq(a), Project(Seq(a4, b4), testRelation4)))))
    check(innerPlan, outerPlan, correctAnswer, Seq(a <=> a))
  }
  ...
  test("window function with correlated equality predicate") {
    ...
    val innerPlan =
      Window(
        ...
        partitionSpec = Seq(c),
        orderSpec = b.asc :: Nil,
        Filter(And(OuterReference(x) === a, b === 3), testRelation))
  ...
  test("window function with correlated non-equality predicate") {
    ...
    val correctAnswer =
      Window(
        ...
        orderSpec = b.asc :: Nil,
        Filter(And(b === 3, x > a), DomainJoin(Seq(x), testRelation)))
    check(innerPlan, outerPlan, correctAnswer, Seq(x <=> x))
  }
  ...
  test("window function with correlated columns inside") {
    ...
    val innerPlan =
      Window(
        Seq(b, c),
        partitionSpec = Seq(c, OuterReference(x)),
        orderSpec = b.asc :: Nil,
        Filter(b === 3, testRelation))
  ...
  test("SPARK-36191: limit in the correlated subquery") {
    val outerPlan = testRelation
    val innerPlan =
      Project(Seq(x), Limit(1, Filter(OuterReference(a) === x, testRelation2)))
    val (outputPlan, joinCond) = DecorrelateInnerQuery(innerPlan, outerPlan.select())
    ...
    val correctAnswer = Project(
      ...
      Project(
        Seq(x, y, z),
        Filter(
          GreaterThanOrEqual(1, alias.toAttribute),
  ...
  test("SPARK-36191: limit and order by in the correlated subquery") {
    ...
    val innerPlan =
      Project(
        ...
        Limit(
          5,
          Sort(Seq(SortOrder(x, Ascending)), true, Filter(OuterReference(a) > x, testRelation2))))
    ...
    val correctAnswer = Project(
      ...
      Project(
        Seq(a, x, y, z),
        Filter(
          LessThanOrEqual(rowNumberAlias.toAttribute, 5),
          Window(
            Seq(rowNumberAlias),
            Seq(a),
            Seq(SortOrder(x, Ascending)),
            Filter(GreaterThan(a, x), DomainJoin(Seq(a), testRelation2))))))
      ...
      Project(
        ...
        Limit(
          ...
          Sort(
            ...
            true,
            Aggregate(Seq(x), Seq(minY, x), Filter(OuterReference(a) > x, testRelation2)))))
    ...
    val innerPlan =
      Project(
        ...
        Limit(
          5,
          Sort(
            Seq(SortOrder(OuterReference(a), Ascending)),
            true,
            Filter(OuterReference(a) > x, testRelation2))))
    val (outputPlan, joinCond) = DecorrelateInnerQuery(innerPlan, outerPlan.select())
