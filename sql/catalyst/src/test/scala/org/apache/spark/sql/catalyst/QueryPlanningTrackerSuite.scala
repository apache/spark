...
class QueryPlanningTrackerSuite extends SparkFunSuite {
  ...
  test("test ready for execution callback") {
    ...
    t.setReadyForExecution(mockPlan3)
    verify(mockCallback, times(1)).readyForExecution(t, mockPlan3)
    t.setReadyForExecution(mockPlan4)
    verify(mockCallback, times(0)).readyForExecution(t, mockPlan4)
  }
}
