...
class MergeSubplansSuite extends PlanTest {
  ...
  val testRelationWithNonBinaryCollation =
    LocalRelation($"utf8_binary".string("UTF8_BINARY"), $"utf8_lcase".string("UTF8_LCASE"))
  ...
  test("Merging subqueries with projects") {
    ...
    val originalQuery = testRelation
      .select(subquery1, subquery2, subquery3, subquery4, subquery5, subquery6)

    val mergedSubquery = testRelation
      .select(($"a" + 1).as("a_plus1"), ($"a" + 2).as("a_plus2"), $"b")
      .select(
        CreateNamedStruct(
          Seq(Literal("a_plus1"), $"a_plus1", Literal("a_plus2"), $"a_plus2", Literal("b"), $"b"))
          .as("mergedValue"))
  ...
  test("Merging subqueries with aggregates") {
    ...
    val mergedSubquery = testRelation
      .groupBy($"b")(max($"a").as("max_a"), sum($"a").as("sum_a"), $"b")
      .select(
        CreateNamedStruct(
          Seq(Literal("max_a"), $"max_a", Literal("sum_a"), $"sum_a", Literal("b"), $"b"))
          .as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
  ...
  test("Merging subqueries with aggregates with complex grouping expressions") {
    val subquery1 =
      ScalarSubquery(testRelation.groupBy($"b" > 1 && $"a" === 2)(max($"a").as("max_a")))
    ...
    val mergedSubquery = testRelation
      .select($"a", $"b", $"c")
      .groupBy($"b" > 1 && $"a" === 2)(max($"a").as("max_a"), sum($"a").as("sum_a"))
      .select(CreateNamedStruct(Seq(Literal("max_a"), $"max_a", Literal("sum_a"), $"sum_a"))
        .as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
  ...
  test("Merging subqueries with aggregates with multiple grouping expressions") {
    ...
    val hashAggregates = testRelation
      .groupBy($"b", $"c")(max($"a").as("max_a"), min($"a").as("min_a"))
      .select(CreateNamedStruct(Seq(Literal("max_a"), $"max_a", Literal("min_a"), $"min_a"))
        .as("mergedValue"))
  ...
  test("Avoid merge when mixing sort and hash aggs") {
    ...
    Optimize.execute(originalQuery.analyze).collect { case WithCTE(_, _) =>
      fail("Should not have merged")
    }
  }
  ...
  test("Merging subqueries with filters") {
    ...
    val subquery2 =
      ScalarSubquery(testRelation.where($"a" > 1).select($"b".as("b_1")).select($"b_1"))
    ...
    val originalQuery = testRelation
      .select(subquery1, subquery2, subquery3, subquery4)
    ...
    val mergedSubquery = testRelation
      ...
      .select($"a", $"b", $"c")
      .select($"a", $"b")
      .select(CreateNamedStruct(Seq(Literal("a"), $"a", Literal("b"), $"b")).as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
        .select(
  ...
  test("Merging subqueries with complex filter conditions") {
    ...
    val mergedSubquery = testRelation
      ...
      .select($"a", $"b".as("b_2"))
      .select(
        CreateNamedStruct(Seq(Literal("a"), $"a", Literal("b_2"), $"b_2")).as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
        .select(
  ...
  test("Merging subqueries with aggregate filters") {
    val subquery1 =
      ScalarSubquery(testRelation.having($"b")(max($"a").as("max_a"))(max($"a") > 1))
    val subquery2 =
      ScalarSubquery(testRelation.having($"b")(sum($"a").as("sum_a"))(max($"a") > 1))
    val originalQuery = testRelation.select(subquery1, subquery2)

    val mergedSubquery = testRelation
      .having($"b")(max($"a").as("max_a"), sum($"a").as("sum_a"))($"max_a" > 1)
      .select($"max_a", $"sum_a")
      .select(CreateNamedStruct(Seq(Literal("max_a"), $"max_a", Literal("sum_a"), $"sum_a"))
        .as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
  ...
  test("Merging subqueries with joins") {
    ...
    val mergedSubquery = testRelation
      ...
      .join(testRelation.as("t2"), Inner, Some($"t1.b" === $"t2.b"))
      .select($"t1.a", $"t2.c")
      .select(CreateNamedStruct(Seq(Literal("a"), $"a", Literal("c"), $"c")).as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
        .select(
  ...
  test("Do not merge different aggregate implementations") {
    ...
    val objectHashAggregates = testRelation
      .groupBy($"b")(
        CollectList($"a").toAggregateExpression(isDistinct = false).as("collectlist_a"),
        CollectSet($"a").toAggregateExpression(isDistinct = false).as("collectset_a"))
      .select(CreateNamedStruct(
        Seq(Literal("collectlist_a"), $"collectlist_a", Literal("collectset_a"), $"collectset_a"))
        .as("mergedValue"))
    ...
    val sortAggregates = testRelation
      ...
      .select(CreateNamedStruct(Seq(Literal("max_c"), $"max_c", Literal("min_c"), $"min_c"))
        .as("mergedValue"))
  ...
  test("Merge aggregates") {
    ...
    val mergedSubquery = testRelation
      .groupBy()(min($"a").as("min_a"), max($"a").as("max_a"))
      .select(CreateNamedStruct(Seq(Literal("min_a"), $"min_a", Literal("max_a"), $"max_a"))
        .as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      OneRowRelation()
        .select(extractorExpression(0, analyzedMergedSubquery.output, 0, "min_a"))
        .join(OneRowRelation()
  ...
  test("Merge identical subqueries and aggregates") {
    ...
    val mergedSubquery = testRelation
      ...
      .select(CreateNamedStruct(Seq(Literal("min_a"), $"min_a")).as("mergedValue"))
    val analyzedMergedSubquery = mergedSubquery.analyze
    val correctAnswer = WithCTE(
      testRelation
        .select(
