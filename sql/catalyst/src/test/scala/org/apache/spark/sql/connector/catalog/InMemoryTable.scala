...
import org.apache.spark.sql.connector.read.partitioning.{HashPartitioning, Partitioning, UnknownPartitioning}
  ...
  case class InMemoryBatchScan(
    ...
    override def outputPartitioning(): Partitioning = {
      InMemoryTable.this.distribution match {
        case cd: ClusteredDistribution => new HashPartitioning(cd.clustering(), data.size)
