...
abstract class ExternalCatalogSuite extends SparkFunSuite with BeforeAndAfterEach {
  ...
  test("list partitions by filter") {
    ...
    checkAnswer(tbl2, Seq("a".attr.int <= 1), Set(part1))
    checkAnswer(tbl2, Seq("a".attr.int === 2), Set.empty)
    checkAnswer(tbl2, Seq(In("a".attr.int * 10, Seq(30))), Set(part2))
    checkAnswer(tbl2, Seq(Not(In("a".attr.int, Seq(4)))), Set(part1, part2))
    checkAnswer(tbl2, Seq("a".attr.int === 1, "b".attr.string === "2"), Set(part1))
    checkAnswer(tbl2, Seq("a".attr.int === 1 && "b".attr.string === "2"), Set(part1))
    checkAnswer(tbl2, Seq("a".attr.int === 1, "b".attr.string === "x"), Set.empty)
    checkAnswer(tbl2, Seq("a".attr.int === 1 || "b".attr.string === "x"), Set(part1))

    intercept[AnalysisException] {
      try {
        checkAnswer(tbl2, Seq("a".attr.int > 0 && "col1".attr.int > 0), Set.empty)
      } catch {
