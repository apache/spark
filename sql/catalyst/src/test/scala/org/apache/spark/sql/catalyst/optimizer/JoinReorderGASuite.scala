...
import org.apache.spark.sql.internal.SQLConf.{CBO_ENABLED, JOIN_REORDER_DP_THRESHOLD, JOIN_REORDER_ENABLED, JOIN_REORDER_GA_ENABLED}


/**
 * This test suite tests the same cases with [[JoinReorderSuite]] (without the bushy tree case).
 * GA cannot guarantee the best plan because it is a random algorithm. But for simple cases,
 * like for 3 or 4 tables, the best plan can be found with a high probability, as long as the
 * number of iterations is large enough.
 */
class JoinReorderGASuite extends PlanTest with StatsEstimationTestBase {
  ...
  var originalThreadHold = 12
  var originalGAEnabled = false
  var originalGARelaxFactor = 3.5

  override def beforeAll(): Unit = {
    super.beforeAll()
    originalConfCBOEnabled = conf.cboEnabled
    originalConfJoinReorderEnabled = conf.joinReorderEnabled
    originalThreadHold = conf.joinReorderDPThreshold
    originalGAEnabled = conf.joinReorderGAEnabled
    originalGARelaxFactor = conf.joinReorderGARelaxFactor
    conf.setConf(CBO_ENABLED, true)
    conf.setConf(JOIN_REORDER_ENABLED, true)
    conf.setConf(SQLConf.JOIN_REORDER_DP_THRESHOLD, 1)
    conf.setConf(SQLConf.JOIN_REORDER_GA_ENABLED, true)
