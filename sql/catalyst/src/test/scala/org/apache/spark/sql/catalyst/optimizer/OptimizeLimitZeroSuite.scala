...
class OptimizeLimitZeroSuite extends PlanTest {
  ...
  Seq(
    (Inner, LocalRelation("a".attr.int, "b".attr.int)),
    (LeftOuter, Project(Seq("a".attr, Literal(null).cast(IntegerType).as("b")),
      testRelation1).analyze),
    (RightOuter, LocalRelation("a".attr.int, "b".attr.int)),
    (FullOuter, Project(Seq("a".attr, Literal(null).cast(IntegerType).as("b")),
      testRelation1).analyze)
  ...
  test("Limit 0: 3-way join") {
    val testRelation3 = LocalRelation.fromExternalRows(Seq("c".attr.int), data = Seq(Row(1)))
    ...
    val optimized = Optimize.execute(query.analyze)
    val correctAnswer = LocalRelation("a".attr.int, "b".attr.int, "c".attr.int)
  ...
  test("Limit 0: intersect") {
    ...
    val optimized = Optimize.execute(query.analyze)
    val correctAnswer = Distinct(LocalRelation("a".attr.int))
