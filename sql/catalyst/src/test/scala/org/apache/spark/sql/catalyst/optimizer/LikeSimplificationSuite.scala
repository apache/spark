...
class LikeSimplificationSuite extends PlanTest {
  ...
  test("no StartsWith prefix guard when the escape char is in the prefix") {
    // 'a\%b%' escapes the first '%' into a literal, so the leading literal is 'a%b'. Rather than
    // resolve escapes to recover it, the derivation bails whenever the escape char falls in the
    // prefix (here 'a\'), keeping the leading literal free of escapes.
    val originalQuery = testRelation.where($"a" like "a\\%b%").analyze
    comparePlans(Optimize.execute(originalQuery), originalQuery)
  }

  test("derive StartsWith prefix guard when the escape char is only after the first wildcard") {
    // 'ab%c\%d%' has an escape-free leading literal 'ab' (the escaped '%' sits past the first
    // wildcard), so StartsWith('ab') is a sound necessary condition and the Like re-checks exactly.
    val originalQuery = testRelation.where($"a" like "ab%c\\%d%")
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation
      .where(StartsWith($"a", "ab") && ($"a" like "ab%c\\%d%"))
      .analyze
    comparePlans(optimized, correctAnswer)
  }
