...
class BooleanSimplificationSuite extends PlanTest with ExpressionEvalHelper {
  ...
  test("SPARK-48360: simplify IsNull(x) && x, IsNotNull(x) && x, IsNull(x) || x, ...") {
    checkCondition(($"b" > 2) && IsNull($"b" > 2), ($"b" > 2) && IsNull($"b" > 2))
    checkCondition(IsNull($"b" > 2) && ($"b" > 2), IsNull($"b" > 2) && ($"b" > 2))
    ...
    checkCondition(($"b" > 2) || IsNotNull($"b" > 2), ($"b" > 2) || IsNotNull($"b" > 2))
    checkCondition(IsNotNull($"b" > 2) || ($"b" > 2), IsNotNull($"b" > 2) || ($"b" > 2))
