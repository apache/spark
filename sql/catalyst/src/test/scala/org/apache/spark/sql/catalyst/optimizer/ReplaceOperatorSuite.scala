...
class ReplaceOperatorSuite extends PlanTest {
  ...
  test("replace Intersect with Left-semi Join") {
    val table1 = LocalRelation("a".attr.int, "b".attr.int)
    val table2 = LocalRelation("c".attr.int, "d".attr.int)
    ...
    val correctAnswer =
      Aggregate(table1.output, table1.output,
        Join(table1, table2, LeftSemi,
          Option("a".attr <=> "c".attr && "b".attr <=> "d".attr), JoinHint.NONE)).analyze

    comparePlans(optimized, correctAnswer)
  }

  test("replace Except with Filter while both the nodes are of type Filter") {
    val attributeA = "a".attr.int
    val attributeB = "b".attr.int
  ...
  test("replace Except with Filter when only right filter can be applied to the left") {
    val table = LocalRelation(Seq("a".attr.int, "b".attr.int))
    val left = table.where("b".attr < 1).select("a".attr).as("left")
    val right = table.where("b".attr < 3).select("a".attr).as("right")
  ...
  test("don't replace streaming Deduplicate") {
    val input = LocalRelation(Seq("a".attr.int, "b".attr.int), isStreaming = true)
    val attrA = input.output(0)
    val query = Deduplicate(Seq(attrA), input) // dropDuplicates("a")
    val optimized = Optimize.execute(query.analyze)
  ...
  test("SPARK-26366: ReplaceExceptWithFilter should handle properly NULL") {
    val basePlan = LocalRelation(Seq("a".attr.int, "b".attr.int))
    val otherPlan = basePlan.where("a".attr.in(1, 2) || "b".attr.in())
    ...
    val correctAnswer = Aggregate(basePlan.output, basePlan.output,
      Filter(!Coalesce(Seq("a".attr.in(1, 2) || If("b".attr.isNotNull,
        Literal.FalseLiteral, Literal(null, BooleanType)),
  ...
  test("SPARK-26366: ReplaceExceptWithFilter should not transform non-deterministic") {
    val basePlan = LocalRelation(Seq("a".attr.int, "b".attr.int))
    val otherPlan = basePlan.where("a".attr > rand(1L))
    val except = Except(basePlan, otherPlan, false)
