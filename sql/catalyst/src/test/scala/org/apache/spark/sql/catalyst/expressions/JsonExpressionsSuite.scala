...
class JsonExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("to_json - sortKeys false by default") {
    val schema = StructType(
      StructField("b", IntegerType) :: StructField("a", IntegerType) :: Nil)
    val struct = Literal.create(create_row(2, 1), schema)
    checkEvaluation(
      StructsToJson(Map("sortKeys" -> "false"), struct, UTC_OPT),
      """{"b":2,"a":1}""")
  }

  test("to_json - sortKeys with pretty") {
    val schema = StructType(
      StructField("c", IntegerType) ::
      StructField("a", IntegerType) ::
      StructField("b", IntegerType) :: Nil)
    val struct = Literal.create(create_row(3, 1, 2), schema)
    checkEvaluation(
      StructsToJson(Map("sortKeys" -> "true", "pretty" -> "true"), struct, UTC_OPT),
      "{\n  \"a\" : 1,\n  \"b\" : 2,\n  \"c\" : 3\n}")
  }
