...
import org.apache.spark.sql.catalyst.util.{DateTimeTestUtils, DateTimeUtils, GenericArrayData, ParseMode}
...
class JsonExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("from_json - invalid data") {
    ...
    checkEvaluation(
      JsonToStructs(schema, Map("mode" -> ParseMode.Permissive.toString), Literal(jsonData), gmtId),
      null
    )
  }
