...
class OptimizedConstraintPropagationSuite extends ConstraintPropagationSuite with PredicateHelper {

  val trivialConstraintAbsenceChecker = (constraints: ExpressionSet) =>
    assert(
      !constraints.exists {
  ...
  test("checking number of base constraints in project node") {
    ...
    val y = tr
      .where($"c" > 10)
      .select($"a".as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .analyze
    assert(y.resolved)
  ...
  test("checking number of base constraints with " +
    ...
    val y = tr
      .where($"c" + $"a" > 10)
      .select($"a", $"a".as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .analyze
    assert(y.resolved)
    val constraints = y.constraints
    trivialConstraintAbsenceChecker(constraints)
    ...
    verifyConstraints(
      ...
      ExpressionSet(
        Seq(
          resolveColumn(y.analyze, "c") + resolveColumn(y.analyze, "a") > 10,
          IsNotNull(resolveColumn(y.analyze, "c")),
  ...
  test("checking filter pruning") {
    ...
    val allFilters = collectFilters(optimized)
    assert(1 === allFilters.size)
    val conditionalExps = collectConditionalExprs(Seq(allFilters.head))
    assert(1 === conditionalExps.size)
    val correctAnswer = tr
      .where($"c" + $"a" > 10 && IsNotNull($"a") && IsNotNull($"c"))
      .select($"a", $"a".as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .analyze
  ...
  test("Alias in different projects have same exprID..!") {
    ...
    val y = tr1
      ...
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"),
        Literal(1).as("one"), Literal(1).as("one_"))
      .where($"b1" > 10).select( Literal(1).as("one"), Literal(1).as("one_"), $"b1")
      .where($"one" > 1)
      .analyze
    ...
    val bugify = y.transformUp {
      ...
      case f: Filter if exprId1.isDefined =>
    ...
    val optimized = GetOptimizer(OptimizerTypes.NO_PUSH_DOWN_ONLY_PRUNING).execute(bugify)
  ...
  test("trivial conditions should not be part of constraints") {
    ...
    val y = tr1
      .where($"c" + $"a" > 10)
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(tr2.select($"x", $"x".as("x1"), $"y", $"z"), Inner, Some("a" === "x"))
      .where($"y" > 10)
      .analyze
    assert(y.resolved)
    val optimized =
      GetOptimizer(OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_PRUNING).execute(y)
    val topConstraint = optimized.constraints
    // there should not be any trivial constraint present
    trivialConstraintAbsenceChecker(topConstraint)
  }
  ...
  test("filter pruning on Join Node") {
    ...
    val allFilters = collectFilters(optimized)
    val conditionalExps = collectConditionalExprs(allFilters)
    assert(1 === conditionalExps.size)
    val correctAnswer = tr1
      .where($"c" + $"a" > 10 && IsNotNull($"a") && IsNotNull($"c"))
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(tr2.where(IsNotNull($"x")), Inner, Some($"a2" === $"x"))
      .analyze
  ...
  test("new filter pushed down on Join Node") {
    ...
    val correctAnswer = tr1
      ...
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(
        tr2.where(IsNotNull($"x") && $"x" > -15), Inner, Some($"a2" === $"x")
      ).analyze
  ...
  test("new filter pushed down on Join Node with multiple join conditions") {
    ...
    assert(joinNode.left.collect {
      case f: Filter => f
    }.exists(_.condition.collectFirst {
      case a: Add => a
    }.isDefined))
    ...
    val correctAnswer = tr1
      ...
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(
        tr2.where(IsNotNull($"x") && IsNotNull($"z") && $"x" > -15 && $"z" + $"x" > 10),
  ...
  test("filter pruning when original attributes are lost") {
    ...
    val y = tr
      .where($"c" + $"a" > 10)
      .select($"a", $"a".as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .select($"x".as("x1"), $"y".as("y1"), $"c1".as("c2"))
      .where($"x1" + $"c2" > 10)
      .analyze
    assert(y.resolved)
    val optimized = GetOptimizer(OptimizerTypes.NO_PUSH_DOWN_ONLY_PRUNING).execute(y)
  ...
  test("filter pruning when partial attributes are lost") {
    ...
    val y = tr
      .where($"c" + $"a" > 10)
      .select($"a", $"a".as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .select($"c", $"x".as("x1"), $"y".as("y1"), $"c1".as("c2"))
      .where($"x1" + $"c" > 10)
      .analyze
    assert(y.resolved)
    val optimized = GetOptimizer(OptimizerTypes.NO_PUSH_DOWN_ONLY_PRUNING).execute(y)
  ...
  test("filter pruning with expressions in alias") {
    ...
    val y = tr
      .where($"c" + $"a" > 10)
      .select($"a", ($"a" + $"c").as("x"), $"b".as("y"), $"c", $"c".as("c1"))
      .select($"c", $"x".as("x1"), $"y".as("y1"), $"c1".as("c2"))
      .where($"x1" > 10)
      .analyze
    assert(y.resolved)
  ...
  test("filter pruning with subexpressions in alias") {
    ...
    assert(1 === conditionalExps.size)
    val correctAnswer = tr
      .where($"c" + $"a" + $"b" > 10 && IsNotNull($"a") && IsNotNull($"c") && IsNotNull($"b"))
      .select(($"a" + $"c").as("x"), $"b".as("y"))
      .select($"x".as("x1"), $"y".as("y1"))
      .analyze
  ...
  test("filter pruning using expression equivalence list - #2") {
    ...
    val correctAnswer = tr
      .where($"c" + $"a" + $"b" > 10 && IsNotNull($"a") && IsNotNull($"c") && IsNotNull($"b"))
      .select($"c", ($"a" + $"c").as("x"), ($"a" + $"c").as("z"), $"b", $"b".as("y"))
      .select($"z", $"y")
      .analyze
  ...
  test("check redundant constraints are not added") {
    ...
    val aliasedAnalyzed = trAnalyzed
      .where($"c" + $"a" + $"b" > 10 && $"d" > 8)
      .select($"a", $"d", $"d".as("z"), $"d".as("z1"), ($"a" + $"c").as("x1"),
        ($"a" + $"c").as("x"), $"b", $"b".as("y"), $"c")
      .analyze
    ...
    6) isnotnull(d)
    */
    val expectedConstraints = ExpressionSet(
      Seq(
  ...
  test("new filter pushed down on Join Node with filter on each variable" +
    ...
    val y = tr1_
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(tr2_.select($"x".as("x1")), Inner, Some($"a2" === $"x1"))
      .where($"a1" + $"c1" > 10)
      .analyze
    assert(y.resolved)
    val optimized = GetOptimizer(
      OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_PRUNING).execute(y)
    trivialConstraintAbsenceChecker(optimized.constraints)
    ...
    val allFilterExpressions = collectFilters(optimized).flatMap(_.expressions)
    assert(5 === allFilterExpressions.flatMap(_.collect {
      case _: GreaterThan => true
    }).size)
    val correctAnswer = tr1
      .where($"c" + $"a" > 10 && $"a" > -11 && $"a" > -12 && IsNotNull($"a") && IsNotNull($"c"))
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(
        tr2.where($"x" > -12 && IsNotNull($"x") && $"x" > -11).select($"x".as("x1")), Inner,
        Some($"a2" === $"x1")
      ).analyze
  ...
  test("compound filter push down for left outer join") {
    ...
    val y = tr1
      .where($"a" + $"b" > 10)
      .join(tr2.where($"x" > 100), LeftOuter, Some($"a" === $"x" && $"b" === $"y"))
      .analyze
    assert(y.resolved)
    ...
    val correctAnswer = tr1
      .where($"a" + $"b" > 10 && IsNotNull($"a") && IsNotNull($"b"))
      .join(
        tr2.where($"x" > 100 && IsNotNull($"x") && IsNotNull($"y") && $"x" + $"y" > 10),
        LeftOuter, Some($"a" === $"x" && $"b" === $"y")
      ).analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("compound filter push down for left Anti join") {
    ...
    val y = tr1
      .where($"a" + $"b" > 10)
      .join(tr2.where($"x" > 100), LeftAnti, Some($"a" === $"x" && $"b" === $"y"))
      .analyze
    assert(y.resolved)
    val optimized = GetOptimizer(OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_PRUNING).
      execute(y)
  ...
  test("compound filter push down for left Semi join") {
    ...
    val y = tr1
      .where($"a" + $"b" > 10)
      .join(tr2.where($"x" > 100), LeftSemi, Some($"a" === $"x" && $"b" === $"y"))
      .analyze
    assert(y.resolved)
    val optimized = GetOptimizer(OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_PRUNING).
      execute(y)
    ...
    val correctAnswer = tr1
      .where($"a" + $"b" > 10 && IsNotNull($"a") && IsNotNull($"b") && $"a" > 100)
      .join(
        tr2.where($"x" > 100 && IsNotNull($"x") && IsNotNull($"y") && $"x" + $"y" > 10),
        LeftSemi, Some($"a" === $"x" && $"b" === $"y"))
      .analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("compound filter push down for right outer join") {
   val tr1 = LocalRelation($"a".int, $"b".int, $"c".int).subquery("tr1")
    val tr2 = LocalRelation($"x".int, $"y".int, $"z".int).subquery("tr2")
    val y = tr1.join(
      tr2.where($"x" > 100 && $"x" + $"y" > 10), RightOuter, Some($"a" === $"x" && $"b" === $"y")
    ).analyze
    assert(y.resolved)
    val optimized = GetOptimizer(OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_PRUNING).
      execute(y)
  ...
  test("filter pruning due to new filter pushed down on Join Node ") {
    ...
    val query = tr1_
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(
        tr2_.select($"x".as("x1")), Inner, Some($"a2" === $"x1"))
      .where($"x1" + $"c1" > 10)
    ...
    val expected = tr1
      ...
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .join(
        tr2.where(IsNotNull($"x") && $"x" > -12 && $"x" > -11).select($"x".as("x1")),
        Inner, Some($"a2" === $"x1" && $"x1" + $"c1" > 10))
      .analyze
  ...
  test("top filter should not be pruned for union with lower filter only on one table") {
    ...
    val correctAnswer = new Union(
      Seq(
        tr1.where($"a" > 10 && IsNotNull($"a")),
        tr2.where($"d" > 10 && IsNotNull($"d")),
        tr3.where($"g" > 10 && IsNotNull($"g"))
      )
    ).analyze
  ...
  test("Union Node. missing attribute from equiv list behaviour - 1") {
    ...
    val u1 = tr1
      .where($"a" > 10)
      .select($"a", $"a".as("a1_1"), $"a".as("a1_2"), $"a".as("a1_3"))
    val u2 = tr2
      .where($"b" > 10)
      .select($"b", $"b1", $"b1".as("b1_2"), $"b1".as("b1_3"))
    val u3 = tr3
      .where($"c" > 10)
      .select($"c", $"c1", $"c2", $"c2".as("c1_3"))
    ...
    val correctAnswer1 = new Union(
      Seq(
        tr1
          .where($"a" > 10 && IsNotNull($"a"))
          .select($"a", $"a".as("a1_1"), $"a".as("a1_2"), $"a".as("a1_3"))
          .union(
            tr2
              .where($"b" > 10 && IsNotNull($"b"))
              .select($"b", $"b1", $"b1".as("b1_2"), $"b1".as("b1_3"))
          ),
        tr3
          .where($"c" > 10 && IsNotNull($"c"))
          .select($"c", $"c1", $"c2", $"c2".as("c1_3"))
      )
    ).analyze
    ...
    val correctAnswer2 = tr1
      ...
      .union(
        tr3
          .where($"c" > 10 && IsNotNull($"c"))
          .select($"c", $"c1", $"c2", $"c2".as("c1_3")))
      .select($"a1_1", $"a1_2", $"a1_3").
      where($"a1_1" > 10 && IsNotNull($"a1_1"))
      .analyze
  ...
  test("Union Node. missing attribute from equiv list behaviour - 2") {
    ...
    val tr2 = LocalRelation($"b".int, $"b1".int, $"b2".int, $"b3".int, $"b4".int)
    val tr3 = LocalRelation($"c".int, $"c1".int, $"c2".int, $"c3".int, $"c4".int)
    val u1 = tr1.select($"a", $"a".as("a1_1"), $"a".as("a1_2"), $"a".as("a1_3"))
    val u1_f1 = u1.where($"a1_2" > 10)
    val u2 = tr2.select($"b", $"b1", $"b1".as("b1_2"), $"b1".as("b1_3"))
    ...
    val correctAnswer1 = u1
      .where($"a1_2" > 10 && IsNotNull($"a"))
      .union(u2.where($"b1_2" > 10 && IsNotNull($"b1")))
      .union(u3.where($"c2" > 10 && IsNotNull($"c2"))).analyze
  ...
  test("Union Node.  missing attribute from equiv list behaviour - 2." +
    ...
    val correctAnswer1 = u3
      .where($"c2" > 10 && IsNotNull($"c2"))
      .union(u1.where($"a1_2" > 10 && IsNotNull($"a")))
      .union(u2.where($"b1_2" > 10 && IsNotNull($"b1")))
      .analyze
  ...
  test("Union Node. missing attribute from equiv list behaviour." +
    ...
    val correctAnswer1 = u1
      ...
      .union(
        u2.where($"a_3" + $"b_2" + $"c_2" > 10 && IsNotNull($"a_3") && IsNotNull($"b_1") &&
          IsNotNull($"c_2")))
      .union(u3.where($"a_3" + $"b_2" + $"c_2" > 10 && IsNotNull($"a_3") && IsNotNull($"b_2")
  ...
  test("Union Node.  missing attribute from equiv list behaviour." +
    ...
    val tr2 = LocalRelation($"a1".int, $"b1".int, $"c1".int, $"d1".int, $"e1".int)
    val tr3 = LocalRelation($"a2".int, $"b2".int, $"c2".int, $"d2".int, $"e2".int)
    val u1 = tr1.select($"a", $"a".as("a_1"), $"a".as("a_2"), $"a".as("a_3"), $"b", $"b".as("b_1"),
      $"b".as("b_2"), $"c", $"c".as("c_1"), $"c".as("c_2"))
    val u1_f1 = u1.where($"a" + $"b" + $"c" > 10).analyze
    assert(u1_f1.resolved)
    ...
    val correctAnswer1 = u2
      .where($"a_3" + $"b_2" + $"c_2" > 10 && IsNotNull($"a_3")&& IsNotNull($"b_1") &&
        IsNotNull($"c_2"))
      .union(
        u1.where($"a" + $"b" + $"c" > 10 && IsNotNull($"a") && IsNotNull($"b") && IsNotNull($"c")))
      .union(
        u3.where($"a_3" + $"b_2" + $"c_2" > 10 && IsNotNull($"a_3") && IsNotNull($"b_2")
          && IsNotNull($"c_2"))).analyze
  ...
  test("Union Node identifying 'not common' constraints resulting in OR based constraints -1") {
    ...
    val u1 = tr1.select($"a", $"a".as("a1"), $"a".as("a2"), $"a".as("a3"), $"b", $"b".as("a5"),
      $"b".as("a6"), $"b".as("a7"))
    val u1_f1 = u1.where($"a" > 10 && $"b" > 11)

    val u2 = tr2.select($"a", $"a".as("a1"), $"a".as("a2"), $"a".as("a3"), $"a".as("X"),
      $"a".as("a5"), $"a".as("a6"), $"a".as("a7"))
    ...
    val attribEquivList = constraints.asInstanceOf[ConstraintSet].getAttribEquivalenceList
    val output1 = optimized1.output
    val expectedExprIdsInEquivList =
      Array[Seq[Attribute]](
        Seq(
          output1(0),
          output1(1),
    ...
    assert(expectedExprIdsInEquivList.forall(buff =>
      attribEquivList.exists(eqiv =>
        eqiv.map(_.canonicalized).toSet.diff(
          buff.map(_.canonicalized).toSet).isEmpty && eqiv.size == buff.size
      ))
    )
  ...
  test("Union Node. identifying 'not common' constraints resulting in OR based constraints -2") {
    ...
    val u2 = tr2.select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("a3"), $"b".as("a4"),
      $"b".as("a5"), $"e".as("b"), $"e".as("b1"), $"e".as("b2"), $"e".as("b3"), $"e".as("b4"))
  ...
  test("Union Node. identifying 'not common' constraints resulting in OR based constraints -3") {
    ...
    val tr1 = LocalRelation($"a".int, $"b".int, $"c".int, $"d".int, $"e".int)
    val tr2 = LocalRelation($"a".int, $"b".int, $"c".int, $"d".int, $"e".int)
   val u1 = tr1.select($"a", $"a".as("a1"), $"a".as("a2"), ($"a" + $"b").as("a3"),
     ($"a" + $"b").as("a4"), ($"a" + $"b").as("a5"), $"c".as("a6"))

    val u1_f1 = u1.where($"a" > 7 && $"a6" > 9 && $"a3" > -13)

    val u2 = tr2.select($"a", $"a".as("a1"), $"a".as("a2"), $"a".as("a3"), $"a".as("a4"),
      $"a".as("a5"), $"a".as("a6"))
    ...
    assert(expectedExprIdsInEquivList.forall(buff =>
      attribEquivList.exists(eqiv =>
        eqiv.map(_.canonicalized).toSet.diff(buff.map(_.canonicalized).toSet).isEmpty
          && eqiv.size == buff.size)))
  ...
  test("Union Node. identifying 'not common' constraints resulting in OR based constraints -4") {
    ...
    val tr2 = LocalRelation($"a".int, $"b".int, $"c".int, $"d".int, $"e".int)
    val u1 = tr1.select($"a", $"a".as("a1"), $"a".as("a2"), ($"a" + $"b").as("a3"),
      ($"a" + $"c").as("a4"), ($"a" + $"d").as("a5"), $"c".as("a6"))
    val u1_f1 = u1.where($"a" > 7 && $"a3" > -13 && $"a4" > 9 && $"a5" > 11)
    val u2 = tr2.select($"a", $"a".as("a1"), $"a".as("a2"), $"a".as("a3"),
      $"a".as("a4"), ($"a" *$"e").as("a5"), $"d".as("a6"))
  ...
  test("Union Node. identifying 'not common' constraints resulting in OR based constraints -5") {
    ...
    val tr1 = LocalRelation($"a".int, $"b".int, $"c".int, $"d".int, $"e".int)
    val tr2 = LocalRelation($"a".int, $"b".int, $"c".int, $"d".int, $"e".int)
    val u1 = tr1.select($"a", $"a".as("a1"), $"a".as("a2"), ($"a").as("a3"), ($"a").as("a4"),
      $"b", $"b".as("b1"), $"b".as("b2"), $"b".as("b3"), $"b".as("b4"))

    val u1_f1 = u1.where($"a" + $"b" > 5)

    val u2 = tr2.select($"a", $"a".as("a1"), $"c".as("a2"), ($"c" + $"d").as("a3"),
      ($"c" + $"d").as("a4"), $"b", $"b".as("b1"), $"d".as("b2"), ($"d" * $"e").as("b3"),
      ($"d" * $"e").as("b4"))
  ...
  test("templatization of constraints") {
    ...
    var filter1 = tr1.where($"a" + $"e" * ($"e" + $"a") > 7 ).analyze.asInstanceOf[Filter]
    val expr1 = filter1.condition
    var templatized1 = ConstraintSet.templatizedConstraints(templateGenerator, Seq(expr1))
    var filter2 = tr2.where($"a1" + $"e1" * ($"a1" + $"e1") > 7).analyze.asInstanceOf[Filter]
    ...
    // negative test
    filter1 = tr1.where($"a" + $"b"  > 3).analyze.asInstanceOf[Filter]
    val expr5 = filter1.condition
    templatized1 = ConstraintSet.templatizedConstraints(templateGenerator, Seq(expr5))
    filter2 = tr2.where($"c1" + $"b1" > 3).analyze.asInstanceOf[Filter]
  ...
  test("top filter should be pruned for Intersection with lower filter on one" +
    ...
    val y = tr1
      .where($"a" > 10)
      .intersect(
        tr2.where($"e" > 5), isAll = true)
      .intersect(tr3.where($"i" > -5), isAll = true)
    val y1 = y.select($"a".as("a1"), $"b".as("b1"), $"c".as("c1")).analyze
    assert(y1.resolved)
  ...
  test("top filter should be pruned for aggregate with lower filter") {
    ...
    trivialConstraintAbsenceChecker(optimized.constraints)
    val correctAnswer = tr
      .where($"c" > 10 && $"a" < 5 && IsNotNull($"a") && IsNotNull($"c"))
      .groupBy($"a", $"c", $"b")($"a", $"c".as("c1"), count($"a").as("a3"))
      .where($"a3" > Literal(20).cast(LongType))
      .select($"c1", $"a", $"a3")
      .analyze
  ...
  test("duplicate removed attributes with different metadata" +
    ...
    val tr = LocalRelation($"a".int, $"b".string, $"c".int, $"d".int)
    val aliasedRelation = tr
      .select($"a", $"a".as("a1"), $"a", $"a".as("a2"), $"a".as("a3"), $"c", $"c".as("c1"))
      .select($"c1", $"a3").where($"a3" > 5)
      .analyze
  ...
  test("filter push down on join with aggregate") {
    ...
    val query = tr1
      ...
      .select($"c2", $"a3")
      .join(tr2.where($"x" > 9), Inner, Some($"c2" === $"x"))
    ...
    val expected = tr1
      .where(IsNotNull($"c") && IsNotNull($"a") && $"c" + $"a" > 10 && $"a" > -15 && $"c" > 9)
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
      .groupBy($"b1", $"c1")($"b1", $"c1".as("c2"), count($"a").as("a3"))
      .select($"c2", $"a3")
      .join(tr2.where($"x" > 9 && IsNotNull($"x")), Inner, Some($"c2" === $"x"))
      .analyze
  ...
  test("test pruning using constraints with filters after project - 1") {
    ...
    trivialConstraintAbsenceChecker(constraints)
    val filters = collectFilters(plan)
    ...
    val expectedFilters = Seq(
      IsNotNull(proj.output.find(_.name == "a").get),
  ...
  test("aliased expression contains embedded alias in projection") {
    ...
    val correctAnswer = tr1
      .where($"c" + $"a" + $"b" * $"a" > 10 && IsNotNull($"a") && IsNotNull($"c") &&
        IsNotNull($"b"))
      .select($"a", $"b", $"c", ($"a" + $"c").as("summ"))
      .select($"a", $"b", $"c", $"summ", ($"b" * $"a").as("mult"))
      .select($"a", $"b", $"c", ($"summ" + $"mult").as("z"))
  ...
  test("Bug caused by swapping of operands due to" +
    ...
    val templatizedConstraintsMap = ConstraintSet.templatizedConstraints(
      templateAttributeGenerator, constraints.toSeq)
  ...
  ignore("plan equivalence with case statements and performance comparison with benefit" +
    ...
    val expected = tr
      ...
      .select($"a".as("a1"), $"b".as("b1"), $"c".as("c1"), $"d".as("d1"), $"e".as("e1"),
        $"f".as("f1"), $"g".as("g1"), $"h".as("h1"), $"i".as("i1"), $"j".as("j1"), $"k".as("k1"),
        $"l".as("l1"), $"m".as("m1"), $"n".as("n1"), $"JoinKey1".as("cf1"), $"JoinKey1")
      .select($"a1", $"b1", $"c1", $"d1", $"e1", $"f1", $"g1", $"h1", $"i1", $"j1", $"k1",
        $"l1", $"m1", $"n1", $"cf1", $"JoinKey1")
      .join(tr, condition = Option($"a" <=> $"JoinKey1"))
  ...
  private def collectConditionalExprs(allFilters: Seq[Filter]): Seq[BinaryComparison] =
    allFilters.flatMap(_.expressions).flatMap(_.collect {
      case x: GreaterThan => x
      case y: LessThan => y
    })

  private def collectFilters(plan: LogicalPlan): Seq[Filter] = plan.collect[Filter] {
    case x: Filter => x
  }
