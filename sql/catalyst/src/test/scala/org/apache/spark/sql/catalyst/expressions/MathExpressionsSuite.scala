...
class MathExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("SPARK-55557: asinh should be numerically stable for large values") {
    // Hardcoded reference values cross-verified against C libm (glibc/musl fdlibm)
    ...
    // Small value: |x| < 2^-28 (identity branch)
    checkEvaluation(Asinh(Literal(1.0e-10)), 1.0e-10, EmptyRow)
    // Large value: |x| > 2^28 branch
    val expected = Math.log(Double.MaxValue) + StrictMath.log(2.0)
