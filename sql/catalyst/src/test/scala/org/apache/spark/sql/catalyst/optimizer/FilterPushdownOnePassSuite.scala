...
class FilterPushdownOnePassSuite extends PlanTest {
  ...
  val testRelation1 = LocalRelation(Symbol("a").int, Symbol("b").int, Symbol("c").int)
  val testRelation2 = LocalRelation(Symbol("a").int, Symbol("d").int, Symbol("e").int)
  ...
  test("push down through join and project") {
    ...
    val originalQuery =
      x.where(Symbol("a") > 0).select(Symbol("a"), Symbol("b"))
        .join(y.where(Symbol("d") < 100).select(Symbol("e")))
        .where("x.a".attr < 100)

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      x.where(Symbol("a") > 0 && Symbol("a") < 100).select(Symbol("a"), Symbol("b"))
        .join(y.where(Symbol("d") < 100).select(Symbol("e"))).analyze
  ...
  test("push down through deep projects") {
    ...
    val originalQuery =
      x.select((Symbol("a") + 1) as Symbol("a1"), Symbol("b"))
        .select((Symbol("a1") + 1) as Symbol("a2"), Symbol("b"))
        .select((Symbol("a2") + 1) as Symbol("a3"), Symbol("b"))
        .select((Symbol("a3") + 1) as Symbol("a4"), Symbol("b"))
        .select(Symbol("b"))
        .where(Symbol("b") > 0)
    ...
    val correctAnswer =
      x.where(Symbol("b") > 0)
        .select((Symbol("a") + 1) as Symbol("a1"), Symbol("b"))
        .select((Symbol("a1") + 1) as Symbol("a2"), Symbol("b"))
        .select((Symbol("a2") + 1) as Symbol("a3"), Symbol("b"))
        .select((Symbol("a3") + 1) as Symbol("a4"), Symbol("b"))
  ...
  test("push down through aggregate and join") {
    ...
    val right = y
      .where(Symbol("d") < 0)
      .groupBy(Symbol("a"))(Symbol("a"), count(Symbol("d")))
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      x.where(Symbol("c") > 0 && Symbol("a") < 100)
        .groupBy(Symbol("a"))(Symbol("a"), count(Symbol("b")))
        .join(y.where(Symbol("d") < 0 && Symbol("a") < 100)
          .groupBy(Symbol("a"))(Symbol("a"), count(Symbol("d"))))
        .analyze
