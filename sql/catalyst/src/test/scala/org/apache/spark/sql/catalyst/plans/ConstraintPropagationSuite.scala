...
class ConstraintPropagationSuite extends SparkFunSuite with PlanTest {
  ...
  test("propagating constraints in filters") {
    ...
    verifyConstraints(
      tr
        ...
        .analyze
        .constraints,
      ExpressionSet(Seq(resolveColumn(tr, "a") > 10, IsNotNull(resolveColumn(tr, "a")))))
  ...
  test("propagating constraints in expand") {
    ...
    val expand = Expand(
      ...
      Seq($"c", $"a", $"gid".int),
      Project(Seq($"a", $"c"), notNullRelation))
    verifyConstraints(expand.analyze.constraints, ExpressionSet(Seq.empty[Expression]))
  }
  ...
  test("propagating constraints in aliases") {
    ...
    assert(
      tr.where($"c".attr > 10).select($"a".as("x"), $"b".as("y")).analyze.constraints.isEmpty)
      ...
      ExpressionSet(
        Seq(
          ...
          resolveColumn(aliasedRelation.analyze, "b") <=> resolveColumn(
            ...
            "y"),
          resolveColumn(aliasedRelation.analyze, "z") <=> resolveColumn(
            aliasedRelation.analyze,
            "x"),
    ...
    verifyConstraints(
      ...
      ExpressionSet(
        Seq(
          ...
          resolveColumn(multiAlias.analyze, "x") === resolveColumn(
            multiAlias.analyze,
            "y") + 10)))
  }
  ...
  test("propagating constraints in union") {
    ...
    verifyConstraints(
      tr1
        ...
        .analyze
        .constraints,
      ExpressionSet(Seq(resolveColumn(tr1, "a") > 10, IsNotNull(resolveColumn(tr1, "a")))))
    ...
    verifyConstraints(
      ...
      ExpressionSet(
        Seq(
          ...
          IsNotNull(
            castWithTimeZone(castWithTimeZone(resolveColumn(tr, "e"), LongType), LongType)))))
  }
  ...
  test("infer isnotnull constraints from compound expressions") {
    ...
    verifyConstraints(
      tr.where(
        $"a".attr === 1 && IsNotNull(resolveColumn(tr, "b")) &&
          IsNotNull(resolveColumn(tr, "c")))
        .analyze
        .constraints,
      ExpressionSet(
