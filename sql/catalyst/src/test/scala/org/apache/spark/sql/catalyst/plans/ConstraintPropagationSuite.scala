...
class ConstraintPropagationSuite extends SparkFunSuite with PlanTest {
  ...
  test("propagating constraints in filters") {
    ...
    assert(tr.where(Symbol("a").attr > 10).select(Symbol("c").attr,
      Symbol("b").attr).analyze.constraints.isEmpty)
  ...
  test("propagating constraints in aggregate") {
    ...
    val aliasedRelation = tr.where(Symbol("c").attr > 10 && Symbol("a").attr < 5)
      .groupBy(Symbol("a"), Symbol("c"), Symbol("b"))(
        Symbol("a"), Symbol("c").as("c1"), count(Symbol("a")).as("a3"))
      .select(Symbol("c1"), Symbol("a"), Symbol("a3")).analyze
  ...
  test("propagating constraints in expand") {
    ...
    val notNullRelation =
    tr.where(Symbol("c").attr > 10 && Symbol("a").attr < 5 && Symbol("b").attr > 2)
  ...
  test("propagating constraints in aliases") {
    ...
    assert(tr.where(Symbol("c").attr > 10).select(Symbol("a").as(Symbol("x")),
      Symbol("b").as(Symbol("y"))).analyze.constraints.isEmpty)

    val aliasedRelation = tr.where(Symbol("a").attr > 10).select(Symbol("a").as(Symbol("x")),
      Symbol("b"), Symbol("b").as(Symbol("y")), Symbol("a").as(Symbol("z")))
    ...
    val multiAlias = tr.where(Symbol("a") === Symbol("c") + 10).select(Symbol("a").as(Symbol("x")),
      Symbol("c").as(Symbol("y")))
  ...
  test("infer constraints on cast") {
    val tr = LocalRelation(
      Symbol("a").int, Symbol("b").long, Symbol("c").int, Symbol("d").long, Symbol("e").int)
    verifyConstraints(
      ...
      ExpressionSet(Seq(
        ...
        IsNotNull(castWithTimeZone(castWithTimeZone(
          resolveColumn(tr, "e"), LongType), LongType)))))
  }
  ...
  test("infer isnotnull constraints from compound expressions") {
    ...
    verifyConstraints(
      tr.where(Symbol("a").attr + Symbol("b").attr === Symbol("c").attr &&
        IsNotNull(
          Cast(
            Cast(
              Cast(resolveColumn(tr, "e"), LongType), LongType), LongType))).analyze.constraints,
      ExpressionSet(Seq(
        castWithTimeZone(resolveColumn(tr, "a"), LongType) + resolveColumn(tr, "b") ===
    ...
    verifyConstraints(
      tr.where(Symbol("a").attr + Symbol("b").attr - Symbol("c").attr * Symbol("d").attr >
        Symbol("e").attr * 1000).analyze.constraints,
      ExpressionSet(Seq(
        (castWithTimeZone(resolveColumn(tr, "a"), LongType) + resolveColumn(tr, "b")) -
  ...
  test("enable/disable constraint propagation") {
    ...
    val aliasedRelation = tr.where(Symbol("c").attr > 10 && Symbol("a").attr < 5)
      .groupBy(Symbol("a"), Symbol("c"), Symbol("b"))(
        Symbol("a"), Symbol("c").as("c1"), count(Symbol("a")).as("a3"))
      .select(Symbol("c1"), Symbol("a"), Symbol("a3"))
