...
class CollapseWindowSuite extends PlanTest {
  ...
  test("SPARK-24066: Exchanged two adjacent windows with order field") {
    val query = testRelation
  ...
  test("SPARK-24066: Exchanged three adjacent windows with order field") {
    val query = testRelation
      .window(Seq(min(b).as('min_b)), partitionSpec1, Seq())
      .window(Seq(max(b).as('max_b)), partitionSpec1, orderSpecA)
      .window(Seq(sum(a).as('sum_a)), partitionSpec1, orderSpecAB)
