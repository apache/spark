...
class LimitPushdownSuite extends PlanTest {
  ...
  test("Union: no limit to both sides if children having smaller limit values") {
    val unionQuery = Union(testRelation.limit(1),
      testRelation2.select("d".attr, "e".attr, "f".attr).limit(1)).limit(2)
    val unionOptimized = Optimize.execute(unionQuery.analyze)
    val unionCorrectAnswer = Union(testRelation.limit(1),
      testRelation2.select("d".attr, "e".attr, "f".attr).limit(1)).analyze
    comparePlans(unionOptimized, unionCorrectAnswer)
  }

  test("Union: limit to each sides if children having larger limit values") {
    val unionQuery = Union(testRelation.limit(3),
      testRelation2.select("d".attr, "e".attr, "f".attr).limit(4)).limit(2)
    val unionOptimized = Optimize.execute(unionQuery.analyze)
    val unionCorrectAnswer =
      Limit(2, Union(LocalLimit(2, testRelation),
        LocalLimit(2, testRelation2.select("d".attr, "e".attr, "f".attr)))).analyze
    comparePlans(unionOptimized, unionCorrectAnswer)
  }
  ...
  test("SPARK-33433: Change Aggregate max rows to 1 if grouping is empty") {
    ...
    val analyzed2 = Limit(1, Union(
      x.groupBy("a".attr)(count(1)),
      y.groupBy("b".attr)(count(1)))).analyze
    val optimized2 = Optimize.execute(analyzed2)
    val expected2 = Limit(1, Union(
      LocalLimit(1, x.groupBy("a".attr)(count(1))),
      LocalLimit(1, y.groupBy("b".attr)(count(1))))).analyze
