  ...
  protected def checkResult(result: Any, expected: Any, dataType: DataType): Boolean = {
    (result, expected) match {
      ...
      case (result: InternalRow, expected: InternalRow) =>
        result.numFields == expected.numFields && {
          val types = dataType.asInstanceOf[StructType].map(_.dataType)
          var isSame = true
          var i = 0
          while (isSame && i < result.numFields) {
            val dt = types(i)
            isSame = checkResult(result.get(i, dt), expected.get(i, dt), dt)
            i += 1
          }
          isSame
        }
