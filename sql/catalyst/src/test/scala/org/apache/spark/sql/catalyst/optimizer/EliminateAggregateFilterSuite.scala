...
class EliminateAggregateFilterSuite extends PlanTest {
  ...
  test("Eliminate Filter always is true") {
    val query = testRelation
      .select(sumDistinct("a".attr, Some(Literal.TrueLiteral)).as("result"))
      .analyze
    val answer = testRelation
      .select(sumDistinct("a".attr).as("result"))
      .analyze
  ...
  test("Eliminate Filter is foldable and always is true") {
    val query = testRelation
      .select(countDistinctWithFilter(GreaterThan(Literal(2), Literal(1)), "a".attr).as("result"))
      .analyze
    val answer = testRelation
      .select(countDistinct("a".attr).as("result"))
      .analyze
    comparePlans(Optimize.execute(query), answer)
  }
  ...
  test("Eliminate Filter always is false") {
    val query = testRelation
      .select(sumDistinct("a".attr, Some(Literal.FalseLiteral)).as("result"))
      .analyze
    val answer = testRelation
  ...
  test("Eliminate Filter is foldable and always is false") {
    val query = testRelation
      .select(countDistinctWithFilter(GreaterThan(Literal(1), Literal(2)), "a".attr).as("result"))
      .analyze
    val answer = testRelation
