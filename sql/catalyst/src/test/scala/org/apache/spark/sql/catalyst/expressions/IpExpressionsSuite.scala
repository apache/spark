...
class IpExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("Ipv4Utils.ipv4ToLong - invalid inputs") {
    ...
    assert(Ipv4Utils.ipv4ToLong("127.0.0.1" + 233.toChar).isEmpty)
    // Empty segments
  ...
  test("InetAton and InetNtoa - valid input works in ANSI mode") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkEvaluation(InetAton(Literal("127.0.0.1")), 2130706433L)
      checkEvaluation(
        InetNtoa(Literal(2130706433L)),
  ...
  test("TryInetNtoa - valid input works like InetNtoa") {
    checkEvaluation(
      new TryInetNtoa(Literal(0L)),
      UTF8String.fromString("0.0.0.0"))
    checkEvaluation(
      new TryInetNtoa(Literal(4294967295L)),
      UTF8String.fromString("255.255.255.255"))
    checkEvaluation(
      new TryInetNtoa(Literal(3232235777L)),
      UTF8String.fromString("192.168.1.1"))
  }

  test("TryInetNtoa - invalid input returns null regardless of ANSI mode") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      checkEvaluation(new TryInetNtoa(Literal(-1L)), null)
      checkEvaluation(new TryInetNtoa(Literal(4294967296L)), null)
    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkEvaluation(new TryInetNtoa(Literal(-1L)), null)
      checkEvaluation(new TryInetNtoa(Literal(4294967296L)), null)
  ...
  test("TryInetNtoa - null input returns null") {
    checkEvaluation(new TryInetNtoa(Literal.create(null, LongType)), null)
  }
  ...
  test("InetAton - not foldable (prevents ConstantFolding crash)") {
    ...
    assert(!InetAton(Literal("192.168.1.1")).contextIndependentFoldable)
  ...
  test("InetNtoa - not foldable (prevents ConstantFolding crash)") {
    ...
    assert(!InetNtoa(Literal(3232235777L)).contextIndependentFoldable)
    assert(!InetNtoa(Literal(-1L), false).foldable)
  }

  test("TryInetAton - not foldable (wraps InetAton which is not foldable)") {
    // RuntimeReplaceable uses Expression's default foldability rather than replacement.foldable.
    assert(!new TryInetAton(Literal("invalid")).foldable)
    assert(!new TryInetAton(Literal("invalid")).contextIndependentFoldable)
  }

  test("TryInetNtoa - not foldable") {
    assert(!new TryInetNtoa(Literal(-1L)).foldable)
    assert(!new TryInetNtoa(Literal(-1L)).contextIndependentFoldable)
  }
