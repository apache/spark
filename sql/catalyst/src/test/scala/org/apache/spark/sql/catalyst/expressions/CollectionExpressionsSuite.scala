...
class CollectionExpressionsSuite
  ...
  test("Sort Map") {
    ...
    checkEvaluation(new SortMap(intKey), Map(1 -> 1, 2 -> 2, 3 -> 3))
    checkEvaluation(SortMap(intKey, Literal.create(false, BooleanType)),
      Map(3 -> 3, 2 -> 2, 1 -> 1))

    checkEvaluation(new SortMap(boolKey), Map(false -> 1, true -> 2))
    checkEvaluation(SortMap(boolKey, Literal.create(false, BooleanType)),
      Map(true -> 2, false -> 1))

    checkEvaluation(new SortMap(stringKey), Map("1" -> 1, "2" -> 2, "3" -> 3))
    checkEvaluation(SortMap(stringKey, Literal.create(false, BooleanType)),
      Map("3" -> 3, "2" -> 2, "1" -> 1))

    checkEvaluation(new SortMap(arrayKey), Map(Seq(1) -> 1, Seq(2) -> 2, Seq(3) -> 3))
    checkEvaluation(SortMap(arrayKey, Literal.create(false, BooleanType)),
      Map(Seq(3) -> 3, Seq(2) -> 2, Seq(1) -> 1))

    checkEvaluation(new SortMap(nestedArrayKey),
      Map(Seq(Seq(1)) -> 1, Seq(Seq(2)) -> 2, Seq(Seq(3)) -> 3))
    checkEvaluation(SortMap(nestedArrayKey, Literal.create(false, BooleanType)),
      Map(Seq(Seq(3)) -> 3, Seq(Seq(2)) -> 2, Seq(Seq(1)) -> 1))

    checkEvaluation(new SortMap(structKey),
      Map(create_row(1) -> 1, create_row(2) -> 2, create_row(3) -> 3))
    checkEvaluation(SortMap(structKey, Literal.create(false, BooleanType)),
