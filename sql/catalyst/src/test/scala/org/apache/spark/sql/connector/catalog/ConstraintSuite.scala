...
import org.apache.spark.sql.connector.catalog.constraints.{Constraint, ConstraintState}
...
class ConstraintSuite extends SparkFunSuite {
  ...
  test("CHECK constraint toDDL") {
    val con1 = Constraint.check(
      "con1",
      "id > 10",
      ConstraintState.builder().enforced(true).validated(true).rely(true).build())
    assert(con1.toDDL == "CONSTRAINT con1 CHECK id > 10 ENFORCED VALID RELY")

    val con2 = Constraint.check(
      "con2",
      new Predicate(
        "=",
        Array[Expression](
      ...
      ConstraintState.builder().enforced(false).validated(true).rely(true).build())
    assert(con2.toDDL == "CONSTRAINT con2 CHECK a.`b.c`.d = 1 NOT ENFORCED VALID RELY")

    val con3 = Constraint.check(
      "con3",
      "a.b.c <=> 1",
      new Predicate(
        "<=>",
        Array[Expression](
          ...
          LiteralValue(1, IntegerType))),
      ConstraintState.builder().enforced(false).validated(false).rely(false).build())
    assert(con3.toDDL == "CONSTRAINT con3 CHECK a.b.c <=> 1 NOT ENFORCED NOT VALID NORELY")
  }
  ...
  test("UNIQUE constraint toDDL") {
    val con1 = Constraint.unique(
      ...
      ConstraintState.builder().enforced(false).validated(false).rely(true).build())
    assert(con1.toDDL == "CONSTRAINT con1 UNIQUE (a.b.c, d) NOT ENFORCED NOT VALID RELY")
  ...
  test("PRIMARY KEY constraint toDDL") {
    ...
    val pk2 = Constraint.primaryKey(
      ...
      ConstraintState.builder().enforced(false).validated(false).rely(false).build())
    assert(pk2.toDDL == "CONSTRAINT pk2 PRIMARY KEY (`x.y`.z, id) NOT ENFORCED NOT VALID NORELY")
  }
