  ...
  test("extract watermark from time condition") {
    ...
    def watermarkFrom(
      ...
      val conditionExpr = Some(conditionStr).map { str =>
        val plan =
          Filter(CatalystSqlParser.parseExpression(str), DummyLeafNode())
    ...
    assert(
      watermarkFrom("rightTime - interval 1 second < leftTime - interval 3 second")
        === Some(12000))

    assert(
      watermarkFrom("leftTime > rightTime + interval '0 00:00:01' day to second")
        === Some(11000))
    assert(
      watermarkFrom("leftTime + interval '00:00:02' hour to second > rightTime ")
        === Some(8000))
    assert(
      watermarkFrom("leftTime > rightTime - interval '00:03' minute to second")
        === Some(7000))
    assert(
      watermarkFrom("rightTime < leftTime - interval '1 20:30:40' day to second")
        === Some(160250000))
    assert(
      watermarkFrom(
    ...
    assert(
      watermarkFrom("CAST(leftTime AS LONG) > CAST((rightTime + interval 1 second) AS LONG)")
        === Some(11000))
    assert(watermarkFrom("CAST(leftTime AS LONG) > 2 + CAST(rightTime AS LONG)") === Some(12000))
    assert(
      watermarkFrom("CAST(leftTime AS LONG) > -0.5 + CAST(rightTime AS LONG)") === Some(9500))
    assert(watermarkFrom("CAST(leftTime AS LONG) - CAST(rightTime AS LONG) > 2") === Some(12000))
    assert(
      watermarkFrom("-CAST(rightTime AS DOUBLE) + CAST(leftTime AS LONG) > 0.1")
        === Some(10100))
    assert(
      watermarkFrom("0 > CAST(rightTime AS LONG) - CAST(leftTime AS LONG) + 0.2")
        === Some(10200))
    ...
    assert(
      watermarkFrom("CAST(leftTime AS LONG) - CAST(rightTime AS LONG) - 2 > 0")
        === Some(12000))
    assert(
      watermarkFrom("-CAST(rightTime AS LONG) + CAST(leftTime AS LONG) - 0.1 > 0")
        === Some(10100))
    // -- Constants on both sides, mixed types
    assert(
      watermarkFrom("CAST(leftTime AS LONG) - 2.0 > CAST(rightTime AS LONG) + 1")
        === Some(13000))
    ...
    assert(
      watermarkFrom("cast(rightTime AS DOUBLE) < rightOther") === None
    ) // non-time attributes
    ...
    assert(
      watermarkFrom("CAST(leftTime AS LONG) > CAST(rightTime AS LONG) - 100") === Some(-90000))
  }
}
