...
class NullFilteringSuite extends PlanTest {
  ...
  object Optimize extends RuleExecutor[LogicalPlan] {
    val batches = Batch("NullFiltering", FixedPoint(50), NullFiltering) :: Nil
  }
  ...
  test("filter: filter out nulls") {
    ...
    val correctAnswer =
      testRelation
        .where(IsNotNull('a) && IsNotNull('b) && 'a === 1 && 'b > 2)
        .analyze
  ...
  test("inner join: filter out nulls on either side") {
    ...
    val left = testRelation.where(IsNotNull('b))
    val right = testRelation.where(IsNotNull('b))
    val correctAnswer = left.join(right, joinType = Inner,
      condition = Some("x.b".attr === "y.b".attr)).analyze
