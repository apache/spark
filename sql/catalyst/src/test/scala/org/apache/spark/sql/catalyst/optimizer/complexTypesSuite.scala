...
class ComplexTypesSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val testRelation = LocalRelation(
    Symbol("a").int, Symbol("b").int, Symbol("c").int, Symbol("d").double, Symbol("e").int)
    ...
    val query = rel
      .select(
        GetArrayStructFields(
          Symbol("arr"), StructField("att1", LongType, false), 0, 1, false) as "a1",
  ...
  test("SPARK-23500: Ensure that aggregation expressions are not simplified") {
    ...
    val arrayAggRel = relation.groupBy(
      CreateArray(
        Seq(Symbol("nullable_id"))))(GetArrayItem(CreateArray(Seq(Symbol("nullable_id"))), 0))
  ...
  private val structAttr =
    Symbol("struct1").struct(Symbol("a").int, Symbol("b").int).withNullability(false)
  ...
  test("simplify GetStructField on basic UpdateFields") {
    def check(fieldOps: Seq[StructFieldsOperation], ordinal: Int, expected: Expression): Unit = {
      def query(relation: LocalRelation): LogicalPlan =
        relation.select(
          GetStructField(UpdateFields(Symbol("struct1"), fieldOps), ordinal).as("res"))
  ...
  test("simplify add multiple nested fields to nullable struct") {
    ...
    val expected = {
      val repeatedExpr =
        UpdateFields(GetStructField(Symbol("a1"), 0), WithField("b3", Literal(2)) :: Nil)
  ...
  test("simplify drop multiple nested fields in non-nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      Symbol("a1").struct(
        Symbol("a2").struct(Symbol("a3").int.notNull,
          Symbol("b3").int.notNull, Symbol("c3").int.notNull).notNull
      ).notNull)
    ...
    val expected = structLevel2.select(
      UpdateFields(Symbol("a1"), Seq(
        WithField("a2", UpdateFields(GetStructField(Symbol("a1"), 0),
  ...
  test("simplify drop multiple nested fields in nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      Symbol("a1").struct(
        Symbol("a2").struct(Symbol("a3").int.notNull,
          Symbol("b3").int.notNull, Symbol("c3").int.notNull)
      ))
