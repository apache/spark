...
class ComplexTypesSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val idAtt = "id".attr.long.notNull
  private val nullableIdAtt = "nullable_id".attr.long

  private val relation = LocalRelation(idAtt, nullableIdAtt)
  private val testRelation =
    LocalRelation("a".attr.int, "b".attr.int, "c".attr.int, "d".attr.double, "e".attr.int)
  ...
  test("explicit get from namedStruct") {
    val query = relation
      .select(
        GetStructField(
          CreateNamedStruct(Seq("att", "id".attr)),
          0,
          None) as "outerAtt")
    val expected = relation.select("id".attr as "outerAtt")
  ...
  test("explicit get from named_struct- expression maintains original deduced alias") {
    val query = relation
      .select(GetStructField(CreateNamedStruct(Seq("att", "id".attr)), 0, None))

    val expected = relation
      .select("id".attr as "named_struct(att, id).att")
  ...
  test("collapsed getStructField ontop of namedStruct") {
    val query = relation
      .select(CreateNamedStruct(Seq("att", "id".attr)) as "struct1")
      .select(GetStructField("struct1".attr, 0, None) as "struct1Att")
    val expected = relation.select("id".attr as "struct1Att")
    checkRule(query, expected)
  }
    ...
    val query = relation
      .select(
        CreateNamedStruct(Seq(
          "att1", "id".attr,
          "att2", "id".attr * "id".attr)) as "struct1")
      .select(
        GetStructField("struct1".attr, 0, None) as "struct1Att1",
        GetStructField("struct1".attr, 1, None) as "struct1Att2")

    val expected =
      relation.
        select(
          "id".attr as "struct1Att1",
          ("id".attr * "id".attr) as "struct1Att2")
    ...
    val query = relation
      .select(
        CreateNamedStruct(Seq(
          ...
          "att2", "id".attr * "id".attr)) as "struct1")
      .select(
        GetStructField("struct1".attr, 0, None),
        GetStructField("struct1".attr, 1, None))

    val expected =
      relation.
        select(
          "id".attr as "struct1.att1",
          ("id".attr * "id".attr) as "struct1.att2")
  ...
  test("simplified array ops") {
    val rel = relation.select(
      CreateArray(Seq(
        CreateNamedStruct(Seq(
          "att1", "id".attr,
          "att2", "id".attr * "id".attr)),
        CreateNamedStruct(Seq(
          "att1", "id".attr + 1,
          "att2", ("id".attr + 1) * ("id".attr + 1))
       ))
      ) as "arr"
    )
    val query = rel
      .select(
        GetArrayStructFields("arr".attr, StructField("att1", LongType, false), 0, 1, false) as "a1",
        GetArrayItem("arr".attr, 1) as "a2",
        GetStructField(GetArrayItem("arr".attr, 1), 0, None) as "a3",
        GetArrayItem(
          GetArrayStructFields("arr".attr,
    ...
    val expected = relation
      .select(
        CreateArray(Seq("id".attr, "id".attr + 1L)) as "a1",
        CreateNamedStruct(Seq(
          "att1", ("id".attr + 1L),
          "att2", (("id".attr + 1L) * ("id".attr + 1L)))) as "a2",
        ("id".attr + 1L) as "a3",
        ("id".attr + 1L) as "a4")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops") {
    val rel = relation
      .select(
        CreateMap(Seq(
          "r1", CreateNamedStruct(Seq("att1", "id".attr)),
          "r2", CreateNamedStruct(Seq("att1", ("id".attr + 1L))))) as "m")
    val query = rel
      .select(
        GetMapValue("m".attr, "r1") as "a1",
        GetStructField(GetMapValue("m".attr, "r1"), 0, None) as "a2",
        GetMapValue("m".attr, "r32") as "a3",
        GetStructField(GetMapValue("m".attr, "r32"), 0, None) as "a4")

    val expected =
      relation.select(
        CreateNamedStruct(Seq("att1", "id".attr)) as "a1",
        "id".attr as "a2",
        Literal.create(
          null,
          StructType(
  ...
  test("simplify map ops, constant lookup, dynamic keys") {
    val query = relation.select(
      GetMapValue(
        CreateMap(Seq(
          "id".attr, ("id".attr + 1L),
          ("id".attr + 1L), ("id".attr + 2L),
          ("id".attr + 2L), ("id".attr + 3L),
          Literal(13L), "id".attr,
          ("id".attr + 3L), ("id".attr + 4L),
          ("id".attr + 4L), ("id".attr + 5L))),
        13L) as "a")

    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo(13L, "id".attr), ("id".attr + 1L)),
          (EqualTo(13L, ("id".attr + 1L)), ("id".attr + 2L)),
          (EqualTo(13L, ("id".attr + 2L)), ("id".attr + 3L)),
          (Literal(true), "id".attr))) as "a")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops, dynamic lookup, dynamic keys, lookup is equivalent to one of the keys") {
    ...
    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo("id".attr + 3L, "id".attr), ("id".attr + 1L)),
          (EqualTo("id".attr + 3L, ("id".attr + 1L)), ("id".attr + 2L)),
          (EqualTo("id".attr + 3L, ("id".attr + 2L)), ("id".attr + 3L)),
          (Literal(true), ("id".attr + 4L)))) as "a")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops, no positive match") {
    ...
    val expected = relation.select(
      CaseWhen(Seq(
        (EqualTo("id".attr + 30L, "id".attr), ("id".attr + 1L)),
        (EqualTo("id".attr + 30L, ("id".attr + 1L)), ("id".attr + 2L)),
        (EqualTo("id".attr + 30L, ("id".attr + 2L)), ("id".attr + 3L)),
        (EqualTo("id".attr + 30L, ("id".attr + 3L)), ("id".attr + 4L)),
        (EqualTo("id".attr + 30L, ("id".attr + 4L)), ("id".attr + 5L)))) as "a")
  ...
  test("simplify map ops, constant lookup, mixed keys, eliminated constants") {
    ...
    val expected = relation
      .select(
        CaseKeyWhen(13L,
          Seq("id".attr, ("id".attr + 1L),
            ("id".attr + 1L), ("id".attr + 2L),
            ("id".attr + 2L), ("id".attr + 3L),
            ("id".attr + 3L), ("id".attr + 4L),
            ("id".attr + 4L), ("id".attr + 5L))) as "a")
  ...
  test("simplify map ops, potential dynamic match with null value + an absolute constant match") {
      ...
      .select(
        GetMapValue(
          CreateMap(Seq(
            "id".attr, ("id".attr + 1L),
            ("id".attr + 1L), ("id".attr + 2L),
            ("id".attr + 2L), Literal.create(null, LongType),
            Literal(2L), "id".attr,
            ("id".attr + 3L), ("id".attr + 4L),
            ("id".attr + 4L), ("id".attr + 5L))),
          2L ) as "a")
    ...
    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo(2L, "id".attr), ("id".attr + 1L)),
          // these two are possible matches, we can't tell until runtime
          (EqualTo(2L, ("id".attr + 1L)), ("id".attr + 2L)),
          (EqualTo(2L, "id".attr + 2L), Literal.create(null, LongType)),
          // this is a definite match (two constants),
          // but it cannot override a potential match with ("id".attr + 2L),
          // which is exactly what [[Coalesce]] would do in this case.
          (Literal.TrueLiteral, "id".attr))) as "a")
    checkRule(rel, expected)
  }

  test("SPARK-23500: Simplify array ops that are not at the top node") {
    val query = LocalRelation("id".attr.long)
      .select(
        CreateArray(Seq(
          CreateNamedStruct(Seq(
            "att1", "id".attr,
      ...
      .select(
        GetStructField(GetArrayItem("arr".attr, 1), 0, None) as "a1",
        GetArrayItem(
          GetArrayStructFields("arr".attr,
    ...
    val expected = LocalRelation("id".attr.long)
      .select(
        ("id".attr + 1L) as "a1",
        ("id".attr + 1L) as "a2")
      .orderBy("id".attr.asc)
  ...
  test("SPARK-23500: Simplify map ops that are not top nodes") {
    val query =
      LocalRelation("id".attr.long)
        ...
        .select(
          GetMapValue("m".attr, "r1") as "a1",
          GetMapValue("m".attr, "r32") as "a2")
        .orderBy("id".attr.asc)
        .select("a1".attr, "a2".attr)

    val expected =
      LocalRelation("id".attr.long).select(
        "id".attr as "a1",
  ...
  test("SPARK-23500: Simplify complex ops that aren't at the plan root") {
    val structRel = relation
      .select(GetStructField(CreateNamedStruct(Seq("att1", "nullable_id".attr)), 0, None) as "foo")
      .groupBy($"foo")("1")
    val structExpected = relation
      .select("nullable_id".attr as "foo")
      .groupBy($"foo")("1")
    checkRule(structRel, structExpected)

    val arrayRel = relation
      .select(GetArrayItem(
        CreateArray(Seq("nullable_id".attr, "nullable_id".attr + 1L)), 0) as "a1")
      .groupBy($"a1")("1")
    val arrayExpected = relation.select("nullable_id".attr as "a1").groupBy($"a1")("1")
    checkRule(arrayRel, arrayExpected)

    val mapRel = relation
      .select(GetMapValue(CreateMap(Seq("id", "nullable_id".attr)), "id") as "m1")
      .groupBy($"m1")("1")
    val mapExpected = relation
      .select("nullable_id".attr as "m1")
      .groupBy($"m1")("1")
  ...
  test("SPARK-23500: Ensure that aggregation expressions are not simplified") {
    ...
    val structAggRel = relation.groupBy(
      CreateNamedStruct(Seq("att1", "nullable_id".attr)))(
      GetStructField(CreateNamedStruct(Seq("att1", "nullable_id".attr)), 0, None))
    checkRule(structAggRel, structAggRel)

    val arrayAggRel = relation.groupBy(
      CreateArray(Seq("nullable_id".attr)))(GetArrayItem(CreateArray(Seq("nullable_id".attr)), 0))
    ...
    val originalQuery = relation
      .groupBy("id".attr)(
        GetMapValue(CreateMap(Seq("id".attr, "id".attr + 1L)), 0L) as "a"
      )
  ...
  test("SPARK-23500: namedStruct and getField in the same Project #1") {
    val originalQuery =
      testRelation
        .select(
          namedStruct("col1", "b".attr, "col2", "c".attr).as("s1"), "a".attr, "b".attr)
        .select("s1".attr getField "col2" as "s1Col2",
          namedStruct("col1", "a".attr, "col2", "b".attr).as("s2"))
        .select("s1Col2".attr, "s2".attr getField "col2" as "s2Col2")
    val correctAnswer =
      testRelation
        .select("c".attr as "s1Col2", "b".attr as "s2Col2")
  ...
  test("SPARK-23500: namedStruct and getField in the same Project #2") {
    val originalQuery =
      testRelation
        .select(
          namedStruct("col1", "b".attr, "col2", "c".attr) getField "col2" as "sCol2",
          namedStruct("col1", "a".attr, "col2", "c".attr) getField "col1" as "sCol1")
    val correctAnswer =
      testRelation
        .select("c".attr as "sCol2", "a".attr as "sCol1")
    checkRule(originalQuery, correctAnswer)
  }
  ...
  private val structAttr = "struct1".attr.struct("a".attr.int, "b".attr.int).withNullability(false)
  private val testStructRelation = LocalRelation(structAttr)

  private val nullableStructAttr = "struct1".attr.struct("a".attr.int, "b".attr.int)
  ...
  test("simplify GetStructField on basic UpdateFields") {
    def check(fieldOps: Seq[StructFieldsOperation], ordinal: Int, expected: Expression): Unit = {
      def query(relation: LocalRelation): LogicalPlan =
        relation.select(GetStructField(UpdateFields("struct1".attr, fieldOps), ordinal).as("res"))
      ...
      checkRule(
        ...
        testNullableStructRelation.select((expected match {
          ...
          case expr => If(IsNull("struct1".attr), Literal(null, expr.dataType), expr)
        }).as("res")))
    }
    ...
    check(WithField("c", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(WithField("c", Literal(3)) :: Nil, 1, GetStructField("struct1".attr, 1))
    ...
    check(WithField("a", Literal(1)) :: Nil, 1, GetStructField("struct1".attr, 1))
    check(WithField("b", Literal(2)) :: Nil, 0, GetStructField("struct1".attr, 0))
    ...
    check(WithField("c", Literal(3)) :: WithField("c", Literal(4)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(WithField("c", Literal(3)) :: WithField("d", Literal(4)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(WithField("c", Literal(3)) :: WithField("c", Literal(4)) :: Nil, 1, GetStructField("struct1".attr, 1))
    check(WithField("c", Literal(3)) :: WithField("d", Literal(4)) :: Nil, 1, GetStructField("struct1".attr, 1))
    ...
    check(DropField("b") :: Nil, 0, GetStructField("struct1".attr, 0))
    check(DropField("a") :: Nil, 0, GetStructField("struct1".attr, 1))

    // drop attribute, add attribute, extract an attribute from the original struct
    check(DropField("b") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(DropField("a") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 1))
    ...
    check(WithField("c", Literal(3)) :: DropField("a") :: Nil, 0, GetStructField("struct1".attr, 1))
    check(WithField("c", Literal(3)) :: DropField("b") :: Nil, 0, GetStructField("struct1".attr, 0))
    ...
    check(WithField("b", Literal(3)) :: DropField("b") :: Nil, 0, GetStructField("struct1".attr, 0))
    check(WithField("a", Literal(3)) :: DropField("a") :: Nil, 0, GetStructField("struct1".attr, 1))

    // add attribute, drop same attribute, extract an attribute from the original struct
    check(WithField("c", Literal(3)) :: DropField("c") :: Nil, 0, GetStructField("struct1".attr, 0))
    check(WithField("c", Literal(3)) :: DropField("c") :: Nil, 1, GetStructField("struct1".attr, 1))
    ...
    check(DropField("b") :: WithField("b", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(DropField("a") :: WithField("a", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 1))
    ...
    check(DropField("c") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField("struct1".attr, 0))
    check(DropField("c") :: WithField("c", Literal(3)) :: Nil, 1, GetStructField("struct1".attr, 1))
  ...
  test("simplify GetStructField that is extracting a field nested inside a struct") {
    val struct2 = "struct2".attr.struct("b".attr.int)
    ...
    def addFieldFromSameStructAndThenExtractIt(relation: LocalRelation): LogicalPlan =
      relation.select(GetStructField(
        UpdateFields("struct1".attr,
          WithField("b", GetStructField("struct1".attr, 0)) :: Nil), 1).as("res"))

    checkRule(
      addFieldFromSameStructAndThenExtractIt(testStructRelation),
      testStructRelation.select(GetStructField("struct1".attr, 0).as("res")))

    checkRule(
      addFieldFromSameStructAndThenExtractIt(testNullableStructRelation),
      testNullableStructRelation.select(GetStructField("struct1".attr, 0).as("res")))
    ...
    def addFieldFromAnotherStructAndThenExtractIt(relation: LocalRelation): LogicalPlan =
      relation.select(GetStructField(
        UpdateFields("struct1".attr,
          WithField("b", GetStructField("struct2".attr, 0)) :: Nil), 1).as("res"))

    checkRule(
      addFieldFromAnotherStructAndThenExtractIt(testStructRelation),
      testStructRelation.select(GetStructField("struct2".attr, 0).as("res")))

    checkRule(
      addFieldFromAnotherStructAndThenExtractIt(testNullableStructRelation),
      testNullableStructRelation.select(
        If(IsNull("struct1".attr),
          Literal(null, IntegerType), GetStructField("struct2".attr, 0)).as("res")))
  }
  ...
  test("simplify GetStructField on nested UpdateFields") {
    ...
    checkRule(
      ...
      testNullableStructRelation.select(
        If(IsNull("struct1".attr), Literal(null, IntegerType), Literal(4)) as "res"))
  ...
  test("simplify multiple GetStructField on the same UpdateFields") {
    def query(relation: LocalRelation): LogicalPlan = relation
      .select(UpdateFields("struct1".attr, WithField("b", Literal(2)) :: Nil) as "struct2")
      .select(
        GetStructField("struct2".attr, 0, Some("a")) as "struct1A",
        GetStructField("struct2".attr, 1, Some("b")) as "struct1B")

    checkRule(
      query(testStructRelation),
      testStructRelation.select(
        GetStructField("struct1".attr, 0) as "struct1A",
    ...
    checkRule(
      ...
      testNullableStructRelation.select(
        GetStructField("struct1".attr, 0) as "struct1A",
        If(IsNull("struct1".attr), Literal(null, IntegerType), Literal(2)) as "struct1B"))
  }

  test("simplify multiple GetStructField on different UpdateFields") {
    def query(relation: LocalRelation): LogicalPlan = relation
      .select(
        UpdateFields("struct1".attr, WithField("b", Literal(2)) :: Nil) as "struct2",
        UpdateFields("struct1".attr, WithField("b", Literal(3)) :: Nil) as "struct3")
      .select(
        GetStructField("struct2".attr, 0, Some("a")) as "struct2A",
        GetStructField("struct2".attr, 1, Some("b")) as "struct2B",
        GetStructField("struct3".attr, 0, Some("a")) as "struct3A",
        GetStructField("struct3".attr, 1, Some("b")) as "struct3B")

    checkRule(
      query(testStructRelation),
      testStructRelation
        .select(
          GetStructField("struct1".attr, 0) as "struct2A",
          Literal(2) as "struct2B",
          GetStructField("struct1".attr, 0) as "struct3A",
    ...
    checkRule(
      ...
      testNullableStructRelation
        .select(
          GetStructField("struct1".attr, 0) as "struct2A",
          If(IsNull("struct1".attr), Literal(null, IntegerType), Literal(2)) as "struct2B",
          GetStructField("struct1".attr, 0) as "struct3A",
          If(IsNull("struct1".attr), Literal(null, IntegerType), Literal(3)) as "struct3B"))
  }
  ...
  test("simplify add multiple nested fields to non-nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      "a1".attr.struct(
        "a2".attr.struct("a3".attr.int.notNull)).notNull)

    val query = {
      val addB3toA1A2 = UpdateFields("a1".attr, Seq(WithField("a2",
        UpdateFields(GetStructField("a1".attr, 0), Seq(WithField("b3", Literal(2)))))))
    ...
    val expected = structLevel2.select(
      UpdateFields("a1".attr, Seq(
        // scalastyle:off line.size.limit
        WithField("a2", UpdateFields(GetStructField("a1".attr, 0), WithField("b3", 2) :: WithField("c3", 3) :: Nil))
        // scalastyle:on line.size.limit
      )).as("a1"))
  ...
  test("simplify add multiple nested fields to nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      "a1".attr.struct(
        "a2".attr.struct("a3".attr.int.notNull)))
    ...
    val expected = {
      val repeatedExpr =
        UpdateFields(GetStructField("a1".attr, 0), WithField("b3", Literal(2)) :: Nil)
      ...
      structLevel2.select(
        UpdateFields("a1".attr, Seq(
          WithField("a2", UpdateFields(
            If(IsNull("a1".attr), Literal(null, repeatedExprDataType), repeatedExpr),
  ...
  test("simplify drop multiple nested fields in non-nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      "a1".attr.struct(
        "a2".attr.struct(
          "a3".attr.int.notNull, "b3".attr.int.notNull, "c3".attr.int.notNull).notNull
      ).notNull)

    val query = {
      val dropA1A2B = UpdateFields("a1".attr, Seq(WithField("a2", UpdateFields(
        GetStructField("a1".attr, 0), Seq(DropField("b3"))))))
    ...
    val expected = structLevel2.select(
      UpdateFields("a1".attr, Seq(
        WithField("a2",
          UpdateFields(GetStructField("a1".attr, 0), Seq(DropField("b3"), DropField("c3"))))
      )).as("a1"))
  ...
  test("simplify drop multiple nested fields in nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      "a1".attr.struct(
        "a2".attr.struct("a3".attr.int.notNull, "b3".attr.int.notNull, "c3".attr.int.notNull)
      ))
    ...
    val expected = {
      val repeatedExpr = UpdateFields(GetStructField("a1".attr, 0), DropField("b3") :: Nil)
      val repeatedExprDataType = StructType(Seq(
        StructField("a3", IntegerType, nullable = false),
