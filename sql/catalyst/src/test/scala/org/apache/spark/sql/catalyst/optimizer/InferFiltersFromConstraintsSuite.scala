...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test("filter: filter out constraints in condition") {
    val originalQuery = testRelation.where("a".attr === 1 && "a".attr === "b".attr).analyze
    val correctAnswer = testRelation
      .where(IsNotNull("a".attr) && IsNotNull("b".attr) && "a".attr === "b".attr &&
        "a".attr === 1 && "b".attr === 1)
      .analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  ...
  test("single inner join: filter out values on either side on equi-join keys") {
    ...
    val originalQuery = x.join(y,
      ...
      .analyze
    val left = x.where(IsNotNull("a".attr) && "x.a".attr === 1)
    val right =
      y.where(IsNotNull("a".attr) && IsNotNull("c".attr) && "y.c".attr > 5 && "y.a".attr === 1)
  ...
  test("single inner join: filter out nulls on either side on non equal keys") {
    ...
    val originalQuery = x.join(y,
      ...
      .analyze
    val left = x.where(IsNotNull("a".attr) && IsNotNull("b".attr) && "x.b".attr === 1)
    val right = y.where(IsNotNull("a".attr) && IsNotNull("c".attr) && "y.c".attr > 5)
    val correctAnswer = left.join(right, condition = Some("x.a".attr =!= "y.a".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("single inner join with pre-existing filters: filter out values on either side") {
    val x = testRelation.subquery("x")
    val y = testRelation.subquery("y")
    val originalQuery = x.where("b".attr > 5).join(y.where("a".attr === 10),
      condition = Some("x.a".attr === "y.a".attr && "x.b".attr === "y.b".attr)).analyze
    val left =
      x.where(IsNotNull("a".attr) && "a".attr === 10 && IsNotNull("b".attr) && "b".attr > 5)
    val right =
      y.where(IsNotNull("a".attr) && IsNotNull("b".attr) && "a".attr === 10 && "b".attr > 5)
  ...
  test("multiple inner joins: filter out values on all sides on equi-join keys") {
    ...
    val originalQuery = t1.where("b".attr > 5)
    ...
    val correctAnswer = t1.where(IsNotNull("b".attr) && "b".attr > 5)
      .join(t2.where(IsNotNull("b".attr) && "b".attr > 5),
        condition = Some("t1.b".attr === "t2.b".attr))
      .join(t3.where(IsNotNull("b".attr) && "b".attr > 5),
        condition = Some("t2.b".attr === "t3.b".attr))
      .join(t4.where(IsNotNull("b".attr) && "b".attr > 5),
  ...
  test("inner join with filter: filter out values on all sides on equi-join keys") {
    ...
    val correctAnswer = x.where(IsNotNull("a".attr) && "a".attr > 5)
      .join(y.where(IsNotNull("a".attr) && "a".attr > 5), Inner,
  ...
  test("inner join with alias: alias contains multiple attributes") {
    ...
    val originalQuery = t1.select("a".attr, Coalesce(Seq("a".attr, "b".attr)).as("int_col")).as("t")
      .join(t2, Inner, Some("t.a".attr === "t2.a".attr && "t.int_col".attr === "t2.a".attr))
      .analyze
    val correctAnswer = t1
      .where(IsNotNull("a".attr) && IsNotNull(Coalesce(Seq("a".attr, "b".attr))) &&
        "a".attr === Coalesce(Seq("a".attr, "b".attr)))
      .select("a".attr, Coalesce(Seq("a".attr, "b".attr)).as("int_col")).as("t")
      .join(t2.where(IsNotNull("a".attr)), Inner,
  ...
  test("inner join with alias: alias contains single attributes") {
    ...
    val originalQuery = t1.select("a".attr, "b".attr.as("d")).as("t")
      .join(t2, Inner, Some("t.a".attr === "t2.a".attr && "t.d".attr === "t2.a".attr))
      .analyze
    val correctAnswer = t1
      .where(IsNotNull("a".attr) && IsNotNull("b".attr) &&"a".attr === "b".attr)
      .select("a".attr, "b".attr.as("d")).as("t")
      .join(t2.where(IsNotNull("a".attr)), Inner,
  ...
  test("generate correct filters for alias that don't produce recursive constraints") {
    ...
    val originalQuery = t1
      .select("a".attr.as("x"), "b".attr.as("y"))
      .where("x".attr === 1 && "x".attr === "y".attr).analyze
    val correctAnswer = t1
      .where("a".attr === 1 && "b".attr === 1 && "a".attr === "b".attr &&
        IsNotNull("a".attr) && IsNotNull("b".attr))
      .select("a".attr.as("x"), "b".attr.as("y")).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("constraints should be inferred from aliased literals") {
    ...
    val optimizedLeft =
      testRelation.subquery("left").where(IsNotNull("a".attr) && "a".attr <=> 2).as("left")
  ...
  test("SPARK-23405: left-semi equal-join should filter out null join keys on both sides") {
    val x = testRelation.subquery("x")
    val y = testRelation.subquery("y")
    testConstraintsAfterJoin(x, y, x.where(IsNotNull("a".attr)),
      y.where(IsNotNull("a".attr)), LeftSemi)
  }
  ...
  test("SPARK-21479: Outer join after-join filters push down to null-supplying side") {
    ...
    val left = x.where(IsNotNull("a".attr) && "a".attr === 2)
    val right = y.where(IsNotNull("a".attr) && "a".attr === 2)
  ...
  test("SPARK-21479: Outer join pre-existing filters push down to null-supplying side") {
    ...
    val left = x.where(IsNotNull("a".attr) && "a".attr > 5)
    val right = y.where(IsNotNull("a".attr) && "a".attr > 5)
  ...
  test("SPARK-21479: Outer join no filter push down to preserved side") {
    ...
    testConstraintsAfterJoin(
      x, y.where("a".attr === 1),
      x, y.where(IsNotNull("a".attr) && "a".attr === 1),
      LeftOuter)
  }
  ...
  test("SPARK-23564: left anti join should filter out null join keys on right side") {
    val x = testRelation.subquery("x")
    val y = testRelation.subquery("y")
    testConstraintsAfterJoin(x, y, x, y.where(IsNotNull("a".attr)), LeftAnti)
  }

  test("SPARK-23564: left outer join should filter out null join keys on right side") {
    val x = testRelation.subquery("x")
    val y = testRelation.subquery("y")
    testConstraintsAfterJoin(x, y, x, y.where(IsNotNull("a".attr)), LeftOuter)
  }

  test("SPARK-23564: right outer join should filter out null join keys on left side") {
    val x = testRelation.subquery("x")
    val y = testRelation.subquery("y")
    testConstraintsAfterJoin(x, y, x.where(IsNotNull("a".attr)), y, RightOuter)
  }

  test("Constraints should be inferred from cast equality constraint(filter higher data type)") {
    val testRelation1 = LocalRelation("a".attr.int)
    val testRelation2 = LocalRelation("b".attr.long)
    val originalLeft = testRelation1.subquery("left")
    val originalRight = testRelation2.where("b".attr === 1L).subquery("right")

    val left =
      testRelation1.where(IsNotNull("a".attr) && "a".attr.cast(LongType) === 1L).subquery("left")
    val right = testRelation2.where(IsNotNull("b".attr) && "b".attr === 1L).subquery("right")
    ...
    Seq(Some("left.a".attr === "right.b".attr.cast(IntegerType)),
      ...
      testConstraintsAfterJoin(
        originalLeft,
        originalRight,
        testRelation1.where(IsNotNull("a".attr)).subquery("left"),
        right,
        Inner,
        condition)
    }
  ...
  test("Constraints shouldn't be inferred from cast equality constraint(filter lower data type)") {
    val testRelation1 = LocalRelation("a".attr.int)
    val testRelation2 = LocalRelation("b".attr.long)
    val originalLeft = testRelation1.where("a".attr === 1).subquery("left")
    val originalRight = testRelation2.subquery("right")

    val left = testRelation1.where(IsNotNull("a".attr) && "a".attr === 1).subquery("left")
    val right = testRelation2.where(IsNotNull("b".attr)).subquery("right")
    ...
    Seq(Some("left.a".attr === "right.b".attr.cast(IntegerType)),
      ...
      testConstraintsAfterJoin(
        ...
        left,
        testRelation2
          .where(IsNotNull("b".attr) && "b".attr.cast(IntegerType) === 1)
          .subquery("right"),
        Inner,
        condition)
    }
