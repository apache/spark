...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test("filter: filter out constraints in condition") {
    val originalQuery =
      testRelation.where(Symbol("a") === 1 && Symbol("a") === Symbol("b")).analyze
  ...
  test("single inner join: filter out values on either side on equi-join keys") {
    ...
    val right = y.where(IsNotNull(Symbol("a")) &&
      IsNotNull(Symbol("c")) && "y.c".attr > 5 && "y.a".attr === 1)
  ...
  test("single inner join with pre-existing filters: filter out values on either side") {
    ...
    val left = x.where(IsNotNull(Symbol("a")) &&
      Symbol("a") === 10 && IsNotNull(Symbol("b")) && Symbol("b") > 5)
    val right = y.where(IsNotNull(Symbol("a")) &&
      IsNotNull(Symbol("b")) && Symbol("a") === 10 && Symbol("b") > 5)
  ...
  test("inner join with filter: filter out values on all sides on equi-join keys") {
    ...
    val correctAnswer = x.where(IsNotNull(Symbol("a")) && Symbol("a").attr > 5)
      .join(y.where(IsNotNull(Symbol("a")) && Symbol("a").attr > 5),
        Inner, Some("x.a".attr === "y.a".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("inner join with alias: alias contains multiple attributes") {
    ...
    val originalQuery = t1.select(Symbol("a"), Coalesce(Seq(Symbol("a"),
      Symbol("b"))).as(Symbol("int_col"))).as("t")
  ...
  test("generate correct filters for alias that don't produce recursive constraints") {
    ...
    val originalQuery = t1.select(Symbol("a").as(Symbol("x")),
      Symbol("b").as(Symbol("y"))).where(Symbol("x") === 1 && Symbol("x") === Symbol("y")).analyze
    val correctAnswer =
  ...
  test("constraints should be inferred from aliased literals") {
    ...
    val optimizedLeft = testRelation.subquery(
      Symbol("left")).where(IsNotNull(Symbol("a")) &&Symbol("a") <=> 2).as("left")

    val right = Project(
      Seq(Literal(2).as("two")), testRelation.subquery(Symbol("right"))).as("right")
  ...
  test("SPARK-23405: left-semi equal-join should filter out null join keys on both sides") {
    ...
    testConstraintsAfterJoin(
      x, y, x.where(IsNotNull(Symbol("a"))), y.where(IsNotNull(Symbol("a"))), LeftSemi)
  }
  ...
  test("Constraints should be inferred from cast equality constraint(filter higher data type)") {
    ...
    val left = testRelation1.where(IsNotNull(Symbol("a")) &&
      Symbol("a").cast(LongType) === 1L).subquery(Symbol("left"))
    val right = testRelation2.where(IsNotNull(Symbol("b")) &&
      Symbol("b") === 1L).subquery(Symbol("right"))
  ...
  test("Constraints shouldn't be inferred from cast equality constraint(filter lower data type)") {
    ...
    val left = testRelation1.where(IsNotNull(Symbol("a")) &&
      Symbol("a") === 1).subquery(Symbol("left"))
    ...
    Seq(Some("left.a".attr === "right.b".attr.cast(IntegerType)),
      ...
      testConstraintsAfterJoin(
        ...
        originalRight,
        left,
        testRelation2.where(IsNotNull(Symbol("b")) &&
          Symbol("b").attr.cast(IntegerType) === 1).subquery(Symbol("right")),
        Inner,
        condition)
    }
  }
