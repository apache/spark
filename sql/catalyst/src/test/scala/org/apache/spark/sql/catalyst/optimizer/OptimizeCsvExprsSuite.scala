...
class OptimizeCsvExprsSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val csvAttr = "csv".attr.string
  ...
  test("SPARK-32968: prune unnecessary columns from GetStructField + from_csv") {
    ...
    val query1 = testRelation
      .select(GetStructField(CsvToStructs(schema, options, "csv".attr), 0))
    ...
    val expected1 = testRelation
      .select(GetStructField(
        CsvToStructs(schema, options, "csv".attr, None, Some(prunedSchema1)), 0))
      .analyze
    comparePlans(optimized1, expected1)

    val query2 = testRelation
      .select(GetStructField(CsvToStructs(schema, options, "csv".attr), 1))
    val optimized2 = Optimizer.execute(query2.analyze)

    val prunedSchema2 = StructType.fromDDL("b int")
    val expected2 = testRelation
      .select(GetStructField(
        CsvToStructs(schema, options, "csv".attr, None, Some(prunedSchema2)), 0))
      .analyze
