...
class CountMinSketchAggSuite extends SparkFunSuite {
   ...
   * the two groups back (to emulate partial aggregation), and then compares the result with that
   * generated by [[CountMinSketch]] directly. This assumes insertion order does not impact the
   * result in count-min sketch.
   */
  private def testDataType[T](dataType: DataType, items: Seq[T]): Unit = {
    test("test data type " + dataType) {
      val agg = new CountMinSketchAgg(
        BoundReference(0, dataType, nullable = true),
        Literal(epsOfTotalCount),
        Literal(confidence),
        Literal(seed))
      assert(!agg.nullable)
  ...
  testDataType[UTF8String](
    StringType,
    Seq.fill(100) { UTF8String.fromString(rand.nextString(1)) })
  ...
  test("fails analysis if parameters are invalid") {
    ...
    assertResult(DataTypeMismatch(errorSubClass = "UNEXPECTED_NULL", Map("exprName" -> "eps")))(
      wrongEps.checkInputDataTypes())
    assertResult(
      DataTypeMismatch(errorSubClass = "UNEXPECTED_NULL", Map("exprName" -> "confidence")))(
      wrongConfidence.checkInputDataTypes())
    assertResult(DataTypeMismatch(errorSubClass = "UNEXPECTED_NULL", Map("exprName" -> "seed")))(
      wrongSeed.checkInputDataTypes())
