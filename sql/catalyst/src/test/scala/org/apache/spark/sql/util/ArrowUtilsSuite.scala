...
class ArrowUtilsSuite extends SparkFunSuite {
    ...
    roundtrip(
      new StructType()
        ...
        .add(
          ...
          false,
          new MetadataBuilder()
            .putLongArray("list", Array(1, 2, 3))
            .putString("is_geo?", "true")
            .build())
        .add(
          "i",
          GeographyType("ANY"),
    ...
    roundtrip(
      new StructType()
        .add(
          ...
          false,
          new MetadataBuilder()
            .putBoolean("is_array?", true)
            .putString("old_name", "old_arr")
            .build())
        .add(
          "map",
          MapType(LongType, StringType),
          false,
          new MetadataBuilder()
            .putBoolean("is_array?", false)
            .putString("old_name", "old_map")
            .build())
        .add(
          "struct",
          new StructType()
              ...
              ArrayType(
                new StructType()
                  ...
                  .add(
                    ...
                    new MetadataBuilder()
                      .putLong("v", 1)
                      .putString("city", "singapore")
                      .build())))),
          true,
          new MetadataBuilder()
            .putBoolean("is_nested_array?", true)
            .putString("dims", "x-y-z")
            .build()))
  }
  ...
  test("struct with duplicated field names") {
    ...
    def check(dt: DataType, expected: DataType): Unit = {
      ...
      assert(
        ArrowUtils.fromArrowSchema(ArrowUtils.toArrowSchema(schema, null, false, false))
