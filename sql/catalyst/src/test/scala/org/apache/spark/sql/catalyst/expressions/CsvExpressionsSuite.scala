...
class CsvExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper with PlanTestBase {
  ...
  test("from_csv") {
    ...
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal(csvData), utcId),
      InternalRow(1)
    )
  }
  ...
  test("from_csv - invalid data") {
    ...
    checkEvaluation(
      CsvToStructs(schema, Map("mode" -> PermissiveMode.name), Literal(csvData), utcId),
      InternalRow(null))

    // Default mode is Permissive
    checkEvaluation(CsvToStructs(schema, Map.empty, Literal(csvData), utcId), InternalRow(null))
  }

  test("from_csv null input column") {
    val schema = StructType(StructField("a", IntegerType) :: Nil)
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal.create(null, StringType), utcId),
      null
    )
  }
  ...
  test("from_csv bad UTF-8") {
    val schema = StructType(StructField("a", IntegerType) :: Nil)
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal(badCsv), utcId),
      InternalRow(null))
  }
  ...
  test("from_csv with timestamp") {
    ...
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal(csvData1), utcId),
  ...
  test("from_csv empty input column") {
    val schema = StructType(StructField("a", IntegerType) :: Nil)
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal.create(" ", StringType), utcId),
      InternalRow(null)
    )
  }
  ...
  test("forcing schema nullability") {
    ...
    val expr = CsvToStructs(csvSchema, Map.empty, Literal.create(input, StringType), utcId)
  ...
  test("from_csv missing columns") {
    val schema = new StructType()
      ...
      .add("b", IntegerType)
    checkEvaluation(
      CsvToStructs(schema, Map.empty, Literal.create("1"), utcId),
      InternalRow(1, null)
    )
  }
  ...
  test("unsupported mode") {
    ...
    val exception = intercept[TestFailedException] {
      checkEvaluation(
        CsvToStructs(schema, Map("mode" -> DropMalformedMode.name), Literal(csvData), utcId),
        InternalRow(null))
    }.getCause
  ...
  test("to_csv null input column") {
    ...
    checkEvaluation(
      StructsToCsv(Map.empty, struct, utcId),
      null
    )
  }
  ...
  test("to_csv with timestamp") {
    ...
    checkEvaluation(StructsToCsv(Map.empty, struct, utcId), "2016-01-01T00:00:00.000Z")
    checkEvaluation(
  ...
  test("parse date with locale") {
    Seq("en-US", "ru-RU").foreach { langTag =>
      ...
      checkEvaluation(
        CsvToStructs(schema, options, Literal.create(dateStr), utcId),
