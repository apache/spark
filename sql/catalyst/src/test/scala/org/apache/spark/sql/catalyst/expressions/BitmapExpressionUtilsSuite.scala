...
class BitmapExpressionUtilsSuite extends SparkFunSuite {
  ...
  test("bitmap_bucket_number with positive inputs") {
    Seq(
      ...
      (845894934L, 25815L),
      (2147483647L, 65536L),
      (Long.MaxValue, 281474976710656L),
      (32768L, 1L),
      (32769L, 2L),
      (32770L, 2L)).foreach { case (input, expected) =>
  ...
  test("bitmap_bucket_number with negative inputs") {
    Seq(
      ...
      (-2147483647L, -65535L),
      (-2147483648L, -65536L),
      (Long.MinValue, -281474976710656L),
      (Long.MinValue + 1, -281474976710655L),
      (-32767L, 0L),
      (-32768L, -1L),
      (-32769L, -1L)).foreach { case (input, expected) =>
      assert(BitmapExpressionUtils.bitmapBucketNumber(input) == expected)
    }
  }
  ...
  test("bitmap_bit_position with positive inputs") {
    Seq(
      ...
      (845894934L, 21781L),
      (2147483647L, 32766L),
      (Long.MaxValue, 32766L),
      (32768L, 32767L),
      (32769L, 0L),
      (32770L, 1L)).foreach { case (input, expected) =>
