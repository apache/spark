...
class StarJoinReorderSuite extends JoinReorderPlanTestBase with StatsEstimationTestBase {
  ...
  private val d3_ns =
    LocalRelation("d3_fk1".attr.int, "d3_c2".attr.int, "d3_pk1".attr.int, "d3_c4".attr.int)
  ...
  private val subq = d3.select(sum("d3_fk1".attr).as("col"))
  ...
  test("Test 6: Join with complex plans") {
    ...
    val expected =
      d3.select("d3_fk1".attr).select(sum("d3_fk1".attr).as("col"))
