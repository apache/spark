...
class InferFiltersFromGenerateSuite extends PlanTest {
  ...
  Seq(Explode(_), PosExplode(_)).foreach { f =>
     val createArrayExplode = f(CreateArray(Seq("c1".attr)))
     ...
     val createMapExplode = f(CreateMap(Seq("c1".attr, "c2".attr)))
   ...
   Seq(Inline(_)).foreach { f =>
     val createArrayStructExplode = f(CreateArray(Seq(CreateStruct(Seq("c1".attr)))))
