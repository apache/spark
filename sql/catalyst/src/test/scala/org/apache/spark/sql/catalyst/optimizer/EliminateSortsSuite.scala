...
class EliminateSortsSuite extends AnalysisTest {
  ...
  test("SPARK-33183: remove redundant sort by") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val unnecessaryReordered = orderedPlan.limit(2).select(Symbol("a"))
      .sortBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
  ...
  test("SPARK-33183: remove all redundant local sorts") {
    val orderedPlan = testRelation.sortBy(Symbol("a").asc)
      .orderBy(Symbol("a").asc).sortBy(Symbol("a").asc)
  ...
  test("SPARK-33183: should not remove global sort") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val reordered = orderedPlan.limit(2).select(Symbol("a"))
      .orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
  ...
  test("do not remove sort if the order is different") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val reorderedDifferently = orderedPlan.limit(2).select(Symbol("a"))
      .orderBy(Symbol("a").asc, Symbol("b").desc)
  ...
  test("SPARK-33183: remove top level local sort with filter operators") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered = orderedPlan.where(Symbol("a") > Literal(10))
  ...
  test("SPARK-33183: limits should not affect order for local sort") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered = orderedPlan.limit(Literal(10))
      .sortBy(Symbol("a").asc, Symbol("b").desc)
    val optimized = Optimize.execute(filteredAndReordered.analyze)
  ...
  test("different sorts are not simplified if limit is in between") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("b").desc).limit(Literal(10)).orderBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
  ...
  test("remove sorts separated by Filter/Project operators") {
    ...
    val orderedTwiceWithFilter = testRelation.orderBy(Symbol("a").asc)
    ...
    val orderedTwiceWithBoth = testRelation.orderBy(Symbol("a").asc).select(Symbol("b"))
      .where(Symbol("b") > Literal(0)).orderBy(Symbol("b").desc)
    val optimizedWithBoth = Optimize.execute(orderedTwiceWithBoth.analyze)
    val correctAnswerWithBoth = testRelation.select(Symbol("b")).where(Symbol("b") > Literal(0))
      .orderBy(Symbol("b").desc).analyze
    comparePlans(optimizedWithBoth, correctAnswerWithBoth)

    val orderedThrice =
      orderedTwiceWithBoth.select((Symbol("b") + 1).as(Symbol("c"))).orderBy(Symbol("c").asc)
  ...
  test("SPARK-33183: remove consecutive local sorts with the same ordering") {
    val orderedPlan =
      testRelation.sortBy(Symbol("a").asc).sortBy(Symbol("a").asc).sortBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
  ...
  test("SPARK-33183: remove consecutive local sorts with different ordering") {
    val orderedPlan =
      testRelation.sortBy(Symbol("b").asc).sortBy(Symbol("a").desc).sortBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
    val correctAnswer = testRelation.sortBy(Symbol("a").asc).analyze
    comparePlans(optimized, correctAnswer)
  }
