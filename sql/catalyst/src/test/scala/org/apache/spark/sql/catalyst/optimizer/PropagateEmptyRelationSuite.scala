...
class PropagateEmptyRelationSuite extends PlanTest {
  ...
  val testRelation3 = LocalRelation.fromExternalRows(
    Seq(Symbol("c").int.notNull.withMetadata(metadata)), data = Seq(Row(1)))
  ...
  test("SPARK-32241: remove empty relation children from Union") {
    ...
    val correctAnswer4 =
      testRelation2.union(testRelation3).select(Symbol("b").as(Symbol("a"))).analyze
  ...
  test("propagate empty relation through Join") {
    ...
    val testcases = Seq(
      ...
      (true, false, LeftOuter, Some(Project(Seq(Symbol("a"),
      ...
      (true, false, FullOuter, Some(Project(Seq(Symbol("a"),
        Literal(null).cast(IntegerType).as(Symbol("b"))), testRelation1).analyze)),
      ...
      (false, true, RightOuter, Some(Project(Seq(Literal(null).cast(IntegerType).as(Symbol("a")),
        Symbol("b")), testRelation2).analyze)),
      (false, true, FullOuter, Some(Project(Seq(Literal(null).cast(IntegerType).as(Symbol("a")),
        Symbol("b")), testRelation2).analyze)),
