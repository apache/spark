...
class SubstituteUnresolvedOrdinalsSuite extends AnalysisTest {
  ...
  test("group by ordinal") {
    ...
    val plan2 = testRelation2.groupBy(Literal(1), Literal(2))("a".attr, "b".attr)
    comparePlans(
      SubstituteUnresolvedOrdinals.apply(plan2),
      testRelation2.groupBy(UnresolvedOrdinal(1), UnresolvedOrdinal(2))("a".attr, "b".attr))
    ...
    withSQLConf(SQLConf.GROUP_BY_ORDINAL.key -> "false") {
      comparePlans(
        SubstituteUnresolvedOrdinals.apply(plan2),
        testRelation2.groupBy(Literal(1), Literal(2))("a".attr, "b".attr))
    }
  }
}
