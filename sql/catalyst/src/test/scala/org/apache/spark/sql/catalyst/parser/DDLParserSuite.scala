...
import org.apache.spark.sql.catalyst.analysis.{AnalysisTest, GlobalTempView, LocalTempView, PersistedView, UnresolvedAttribute, UnresolvedNamespace, UnresolvedRelation, UnresolvedStar, UnresolvedTable, UnresolvedTableOrPermanentView, UnresolvedTableOrView}
...
class DDLParserSuite extends AnalysisTest {
  ...
  test("SHOW TBLPROPERTIES table") {
    comparePlans(
      ...
      ShowTableProperties(UnresolvedTableOrPermanentView(Seq("a", "b", "c")), None))

    comparePlans(
      parsePlan("SHOW TBLPROPERTIES a.b.c('propKey1')"),
      ShowTableProperties(UnresolvedTableOrPermanentView(Seq("a", "b", "c")), Some("propKey1")))
  }
