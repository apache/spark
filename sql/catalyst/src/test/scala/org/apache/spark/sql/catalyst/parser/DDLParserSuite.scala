...
class DDLParserSuite extends AnalysisTest {
  ...
  test("describe database") {
    ...
    comparePlans(parsePlan(sql1), DescribeDatabaseStatement(Seq("a", "b"), extended = true))
    comparePlans(parsePlan(sql2), DescribeDatabaseStatement(Seq("a", "b"), extended = false))
  }
