...
class SqlStatementSplitterSuite extends SparkFunSuite {
  ...
  test("parse_sql batch boundary leaves an unclosed compound as partial") {
    ...
    val malformedPartial = "BEGIN SELECT 1; SELEC 2;"
    val malformed = SqlStatementSplitter
      .splitForParseSql(s"SELECT 0; $malformedPartial")
      .withoutPositions
    assert(malformed.completeStatements == Seq(statement("SELECT 0")))
    assert(malformed.partialStatement == malformedPartial)
  }
  ...
  test("parse_sql batch boundary tolerates malformed control headers") {
    val blocks = Seq(
      "BEGIN IF THEN SELECT 1; END IF; END",
  ...
  test("parse_sql malformed BEGIN boundaries preserve following statements") {
    val malformedBegin = SqlStatementSplitter
      .splitForParseSql("BEGIN; SELECT 1;")
      .withoutPositions
    assert(malformedBegin.completeStatements == Seq(
      statement("BEGIN"),
      statement("SELECT 1")))
    assert(malformedBegin.partialStatement.isEmpty)

    val strayEnd = SqlStatementSplitter
      .splitForParseSql("BEGIN SELECT 1; END; END; SELECT 2")
      .withoutPositions
    assert(strayEnd.completeStatements == Seq(
      statement("BEGIN SELECT 1; END"),
      statement("END")))
    assert(strayEnd.partialStatement == "SELECT 2")

    val malformedNestedBegin = SqlStatementSplitter
      .splitForParseSql("BEGIN BEGIN; END; END; SELECT 3")
      .withoutPositions
    assert(malformedNestedBegin.completeStatements == Seq(
      statement("BEGIN BEGIN; END; END")))
    assert(malformedNestedBegin.partialStatement == "SELECT 3")

    val malformedIf = "BEGIN IF TRUE THEN BEGIN SELECT 1; END IF; END"
    val nestedIf = SqlStatementSplitter
      .splitForParseSql(s"SELECT 0; $malformedIf; SELECT 9")
      .withoutPositions
    assert(nestedIf.completeStatements == Seq(
      statement("SELECT 0"),
      statement(malformedIf)))
    assert(nestedIf.partialStatement == "SELECT 9")

    val malformedHandler =
      "BEGIN DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN; END; END"
    val handler = SqlStatementSplitter
      .splitForParseSql(s"SELECT 0; $malformedHandler; SELECT 9")
      .withoutPositions
    assert(handler.completeStatements == Seq(
      statement("SELECT 0"),
      statement(malformedHandler)))
    assert(handler.partialStatement == "SELECT 9")

    val malformedBlocks = Seq(
      "BEGIN lbl: BEGIN; END lbl; END",
      "BEGIN BEGIN; END lbl; END",
      "BEGIN IF TRUE THEN lbl: BEGIN; END lbl; END IF; END",
      "BEGIN IF TRUE THEN SELECT 1; ELSEIF THEN lbl: BEGIN; END lbl; END IF; END",
      "BEGIN CASE WHEN TRUE THEN SELECT 1; ELSE BEGIN; END; END CASE; END",
      "BEGIN CASE x WHEN 1 THEN SELECT 1; ELSE BEGIN; END; END CASE; END",
      "BEGIN CASE WHEN THEN BEGIN; END; END CASE; END",
      "BEGIN IF TRUE THEN SELECT 1; END; END IF; END",
      "BEGIN CASE WHEN TRUE THEN SELECT 1; END; END CASE; END",
      "BEGIN WHILE TRUE DO SELECT 1; END; END WHILE; END",
      "BEGIN LOOP SELECT 1; END; END LOOP; END",
      "BEGIN FOR x AS SELECT 1 DO SELECT 1; END; END FOR; END",
      "BEGIN REPEAT SELECT 1; END; UNTIL TRUE END REPEAT; END",
      "BEGIN DECLARE CONTINUE HANDLER FOR SQLEXCEPTION lbl: BEGIN; END lbl; END")
    malformedBlocks.foreach { block =>
      val result = try {
        SqlStatementSplitter.splitForParseSql(s"$block; SELECT 9").withoutPositions
      } catch {
        case e: Exception => fail(block, e)
      }
      assert(result.completeStatements == Seq(statement(block)), block)
      assert(result.partialStatement == "SELECT 9", block)
    }
