...
class ANSIDataTypeWriteCompatibilitySuite extends DataTypeWriteCompatibilityBaseSuite {
  ...
  private val stringPoint2 = StructType(
    Seq(
...
abstract class DataTypeWriteCompatibilityBaseSuite extends SparkFunSuite {
  ...
  protected val point2 = StructType(
    Seq(
      StructField("x", FloatType, nullable = false),
      StructField("y", FloatType, nullable = false)))

  protected val widerPoint2 = StructType(
    Seq(
      StructField("x", DoubleType, nullable = false),
      StructField("y", DoubleType, nullable = false)))

  protected val point3 = StructType(
    Seq(
      StructField("x", FloatType, nullable = false),
      StructField("y", FloatType, nullable = false),
  ...
  private val nestedContainerTypes = Seq(
    ArrayType(point2, containsNull = false),
    MapType(StringType, point3, valueContainsNull = false))

  protected val allNonNullTypes = Seq(
    atomicTypes,
    simpleContainerTypes,
    nestedContainerTypes,
    Seq(CalendarIntervalType)).flatten
  ...
  test("Check struct types: missing middle field, matched by position") {
    val missingMiddleField = StructType(
      Seq(
        StructField("x", FloatType, nullable = false),
  ...
  test("Check struct types: required field is optional") {
    val requiredFieldIsOptional =
      StructType(Seq(StructField("x", FloatType), StructField("y", FloatType, nullable = false)))
  ...
  test("Check struct type: ignore field name mismatch with byPosition mode") {
    val nameMismatchFields = StructType(
      Seq(
    ...
    assertAllowed(
      ...
      point2,
      "t",
      "Should allow field name mismatch with byPosition mode",
      false)
  }
  ...
  ignore("Check struct types: missing optional field is allowed") {
    ...
    assertAllowed(point2, point3, "t", "Should allow writing point (x,y) to point(x,y,z=null)")
  }
    ...
    checkError(
      exception = intercept[AnalysisException](
        DataTypeUtils.canWrite(
          ...
          errMsg => errs += errMsg)),
      condition = "INCOMPATIBLE_DATA_FOR_TABLE.NULLABLE_COLUMN",
      parameters = Map("tableName" -> "``", "colName" -> "`t`.`col2`"))
  }
  ...
  test("Check string types: cast allowed regardless of collation") {
    assertAllowed(
      ...
      "date time types",
      "Should allow writing string to collated string")
  }
  ...
  def assertAllowed(
    ...
    assert(
      DataTypeUtils.canWrite(
        ...
        name,
        storeAssignmentPolicy,
        errMsg => fail(s"Should not produce errors but was called with: $errMsg")),
      desc)
  }

  def assertSingleError(writeType: DataType, readType: DataType, name: String, desc: String)(
  ...
  def assertNumErrors(
      ...
      desc: String,
      numErrs: Int,
      byName: Boolean = true)(checkErrors: Seq[String] => Unit): Unit = {
    val errs = new mutable.ArrayBuffer[String]()
    assert(
      DataTypeUtils.canWrite(
        "",
