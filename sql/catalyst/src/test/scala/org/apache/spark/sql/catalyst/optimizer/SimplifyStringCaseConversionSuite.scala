...
class SimplifyStringCaseConversionSuite extends PlanTest {
  ...
  test("simplify UPPER(UPPER(str))") {
    val originalQuery =
      testRelation
        .select(Upper(Upper(Symbol("a"))) as Symbol("u"))

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      testRelation
        .select(Upper(Symbol("a")) as Symbol("u"))
        .analyze
  ...
  test("simplify UPPER(LOWER(str))") {
    val originalQuery =
      testRelation
        .select(Upper(Lower(Symbol("a"))) as Symbol("u"))
  ...
  test("simplify LOWER(UPPER(str))") {
    val originalQuery =
      testRelation
        .select(Lower(Upper(Symbol("a"))) as Symbol("l"))

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation
      .select(Lower(Symbol("a")) as Symbol("l"))
      .analyze
  ...
  test("simplify LOWER(LOWER(str))") {
    val originalQuery =
      testRelation
        .select(Lower(Lower(Symbol("a"))) as Symbol("l"))
