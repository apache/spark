...
private class BufferWriter extends DataWriter[InternalRow] {
  ...
  private final val UNKNOWN_OPERATION = UTF8String.fromString(Unknown.toString)
  ...
  override def write(metadata: InternalRow, row: InternalRow): Unit = {
    ...
    val logEntry = new GenericInternalRow(Array[Any](
      UNKNOWN_OPERATION, null, metadata.copy(), row.copy()))
  ...
  override def write(row: InternalRow): Unit = {
    buffer.rows.append(row.copy())
    val logEntry = new GenericInternalRow(Array[Any](
      UNKNOWN_OPERATION, null, null, row.copy()))
    buffer.log.append(logEntry)
  }
