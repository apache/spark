...
private[sql] class SetRoleTestParser extends AbstractSparkSQLParser{
  protected val SET = Keyword("SET")
  protected val ROLE = Keyword("ROLE")

  override protected lazy val start: Parser[LogicalPlan] = setR | set

  private lazy val setR: Parser[LogicalPlan] =
    SET ~ ROLE ~ ident ^^ {
      case set ~ role ~ roleName => TestCommand(List(set, role, roleName).mkString(" "))
    }

  private lazy val set: Parser[LogicalPlan] =
    SET ~> restInput ^^ {
      case input => TestCommand(input)
    }
}
...
class SqlParserSuite extends PlanTest {
  ...
  test("test set role command") {
    val parser = new SetRoleTestParser
    assert(TestCommand(" A.B.C = ADMIN") === parser.parse("SET A.B.C = ADMIN"))
    assert(TestCommand(" A.B.C") === parser.parse("SET A.B.C"))
    assert(TestCommand(" ROLE.A.B = ADMIN") === parser.parse("SET ROLE.A.B = ADMIN"))
    assert(TestCommand(" ROLEA.A.B = ADMIN") === parser.parse("SET ROLEA.A.B = ADMIN"))
    assert(TestCommand(" role.A.B = ADMIN") === parser.parse("SET role.A.B = ADMIN"))
    assert(TestCommand("set role ADMIN") === parser.parse("SET ROLE ADMIN"))
    assert(TestCommand("set role ADMIN") === parser.parse("SET role ADMIN"))
  }
