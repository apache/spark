...
import org.apache.spark.sql.catalyst.expressions.{In, InValues, ListQuery, OuterReference}
...
class ResolveSubquerySuite extends AnalysisTest {
  ...
  test("SPARK-17251 Improve `OuterReference` to be `NamedExpression`") {
    val expr = Filter(
      In(InValues(Seq(a)), Seq(ListQuery(Project(Seq(UnresolvedAttribute("a")), t2)))), t1)
