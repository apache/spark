...
class CompareNewAndOldConstraintsSuite extends SparkFunSuite with PlanTest with PredicateHelper {
  ...
  test("new filter pushed down on Join Node with multiple join conditions") {
    ...
    val query = tr1
      .where($"c" + $"a" > 10 && $"a" > -15)
      .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
    ...
    withSQLConf() {
      ...
      val correctAnswer = tr1
        .where($"c" + $"a" > 10 && $"a" > -15 && IsNotNull($"a") && IsNotNull($"c"))
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
        .join(tr2.where(IsNotNull($"x") && IsNotNull($"z") && $"x" > -15 && $"z" + $"x" > 10),
          Inner, Some($"a2" === $"x" && $"c1" === $"z"))
        .analyze
  ...
  test("test pruning using constraints with filters after project - 3") {
    ...
    withSQLConf() {
      ...
      val correctAnswer = LocalRelation($"a".int, $"b".string, $"c".int)
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"))
        .where($"c1" + $"a1" > 10 && $"a2" > -15 && IsNotNull($"a") && IsNotNull($"c"))
        .analyze
      comparePlans(correctAnswer, optimizedPlan)
    }
  }
  ...
  test("test new filter inference with decanonicalization for expression not" +
    ...
    def getTestPlan: LogicalPlan = {
      ...
      tr1.select(
        $"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"),
        CaseWhen(
          Seq(
            ($"a" + $"b" + $"c" > Literal(1), Literal(1)),
            ($"a" + $"b" + $"c" > Literal(2), Literal(2))
          ), Option(Literal(null))
        ).as("z")
      ).where($"z" > 10 && $"a2" > -15)
       .where(
    ...
    withSQLConf() {
      ...
      val correctAnswer = LocalRelation($"a".int, $"b".int, $"c".int)
        ...
        .where($"z" > 10 && $"a2" > -15 && IsNotNull($"a") && IsNotNull($"z"))
        .select($"a", $"a1", $"a2", $"b1", $"c", $"c1", $"z")
        .analyze
      comparePlans(correctAnswer, optimizedPlan)
    }
  }
  ...
  test("test new filter inference with decanonicalization for expression" +
    ...
    withSQLConf() {
      ...
      val correctAnswer = LocalRelation($"a".int, $"b".int, $"c".int)
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b".as("b1"), $"c", $"c".as("c1"),
          ($"a" + $"b" + $"c").as("z"))
        .where($"z" > 10 && $"a2" > -15 && IsNotNull($"a") && IsNotNull($"b1") && IsNotNull($"c"))
        .analyze
      comparePlans(correctAnswer, optimizedPlan)
    }
  }
  ...
  test("test pruning using constraints with filters after project with expression in" +
    ...
    def getTestPlan: LogicalPlan = {
      val tr1 = LocalRelation($"a".int, $"b".int, $"c".int)
      tr1
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b", $"b".as("b1"), $"c", $"c".as("c1"),
          ($"a" + $"b").as("z"))
        .where($"c1" + $"z" > 10 && $"a2" > -15)
        .where($"c" + $"a" + $"b" > 10 && $"a" > -15)
    }
    ...
    withSQLConf() {
      ...
      val correctAnswer = LocalRelation($"a".int, $"b".int, $"c".int)
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b", $"b".as("b1"), $"c", $"c".as("c1"),
          ($"a" + $"b").as("z"))
        .where($"c1" + $"z" > 10 && $"a2" > -15 && IsNotNull($"b") && IsNotNull($"a") &&
          IsNotNull($"c"))
        .analyze
      comparePlans(correctAnswer, optimizedPlan)
    }
  ...
  ignore("Disabled due to spark's canonicalization bug." +
    ...
    withSQLConf() {
      ...
      val correctAnswer = LocalRelation($"a".int, $"b".string, $"c".int)
        .select($"a", $"a".as("a1"), $"a".as("a2"), $"b", $"b".as("b1"), $"c",
          $"c".as("c1"), ($"a" + $"b").as("z"))
        .where($"c1" + $"z" > 10 && $"a2" > -15 && IsNotNull($"a") && IsNotNull($"c") &&
          IsNotNull($"b"))
        .analyze
      comparePlans(correctAnswer, optimizedPlan)
    }
        ...
        CaseWhen(
          Seq(
            ($"a" + $"b" + $"c" + $"d" + $"e" + $"f" + $"g" + $"h" + $"i" + $"j" + $"k" + $"l"
              + $"m" + $"n" > Literal(1), Literal(1)),
            ($"a" + $"b" + $"c" + $"d" + $"e" + $"f" + $"g" + $"h" + $"i" + $"j" + $"k" + $"l" +
              $"m" + $"n" > Literal(2), Literal(2))
          ), Option(Literal(0))
        ).as("JoinKey1"))
      .select($"a".as("a1"), $"b".as("b1"), $"c".as("c1"), $"d".as("d1"), $"e".as("e1"),
        $"f".as("f1"), $"g".as("g1"), $"h".as("h1"), $"i".as("i1"), $"j".as("j1"), $"k".as("k1"),
        $"l".as("l1"), $"m".as("m1"), $"n".as("n1"), $"JoinKey1".as("cf1"), $"JoinKey1")
      .select($"a1", $"b1", $"c1", $"d1", $"e1", $"f1", $"g1", $"h1", $"i1", $"j1", $"k1", $"l1",
        $"m1", $"n1", $"cf1", $"JoinKey1")
  ...
  def executePlan(plan: LogicalPlan, optimizerType: OptimizerTypes.Value): LogicalPlan = {
    object SimpleAnalyzer extends Analyzer(
      new CatalogManager(FakeV2SessionCatalog,
        new SessionCatalog(new InMemoryCatalog, EmptyFunctionRegistry, SQLConf.get)))
      ...
      optimizerType match {
        ...
        case OptimizerTypes.NO_PUSH_DOWN_ONLY_PRUNING =>
          new Optimizer(
            ...
            override def defaultBatches: Seq[Batch] =
              ...
              Batch("Filter Pruning", Once, InferFiltersFromConstraints, PruneFilters) ::
              Nil
        ...
        case OptimizerTypes.WITH_FILTER_PUSHDOWN_THRU_JOIN_AND_UNIONS_PRUNING =>
          new Optimizer(
            ...
            override def defaultBatches: Seq[Batch] =
              Batch("Subqueries", Once, EliminateSubqueryAliases) ::
              Batch("Union Pushdown", FixedPoint(100), CombineUnions, PushProjectionThroughUnion,
                PushDownPredicates, InferFiltersFromConstraints, CombineFilters, PruneFilters) ::
              Nil
