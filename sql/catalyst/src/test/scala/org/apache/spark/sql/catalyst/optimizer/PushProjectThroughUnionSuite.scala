...
class PushProjectThroughUnionSuite extends PlanTest {
  ...
  test("SPARK-25450 PushProjectThroughUnion rule uses the same exprId for project expressions " +
    ...
    val testRelation1 = LocalRelation(Symbol("a").string, Symbol("b").int, Symbol("c").string)
    val testRelation2 = LocalRelation(Symbol("d").string, Symbol("e").int, Symbol("f").string)
    val query = testRelation1
      .union(testRelation2.select("bar".as("d"), Symbol("e"), Symbol("f")))
      .select(Symbol("a").as("n"))
      .select(Symbol("n"), "dummy").analyze
    val optimized = Optimize.execute(query)

    val expected = testRelation1
      .select(Symbol("a").as("n"))
      .select(Symbol("n"), "dummy")
      .union(testRelation2
        .select("bar".as("d"), Symbol("e"), Symbol("f"))
        .select("bar".as("n"))
