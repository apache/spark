...
class ApproximatePercentileSuite extends SparkFunSuite {
  ...
  test("class ApproximatePercentile, sql string") {
    ...
    assertEqual(
      ...
      new ApproximatePercentile(
        "a".attr,
        percentageExpression = CreateArray(Seq(0.25d, 0.5d, 0.75d).map(Literal(_)))).sql: String)
  ...
  test(
    "class ApproximatePercentile, fails analysis if percentage or accuracy is not a constant") {
  ...
  test("class ApproximatePercentile, fails analysis if parameters are invalid") {
    ...
    val wrongPercentageExpressions =
      Seq(Literal(1.1d), Literal(-0.5d), CreateArray(Seq(0d, 0.5d, 1.1d).map(Literal(_))))
  ...
  test("class ApproximatePercentile, automatically add type casting for parameters") {
    ...
    val accuracyExpressions =
      Seq(Literal(1.toByte), Literal(100.toShort), Literal(100), Literal(1000L))
