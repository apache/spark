...
class UnsafeRowConverterSuite extends SparkFunSuite with Matchers {
  ...
  test("basic conversion with array type") {
    ...
    assert(unsafeArray2.getSizeInBytes == 4 + 8 + 4 + nestedArray.getSizeInBytes)
  ...
  test("basic conversion with map type") {
...

    {
      ...
      assert(valueArray.getSizeInBytes == 4 + 8 + 4 + nestedMap.getSizeInBytes)
    }
  ...
  test("basic conversion with struct and array") {
    ...
    assert(field2.getSizeInBytes == 4 + 8 + 4 + innerStruct.getSizeInBytes)
  ...
  test("basic conversion with struct and map") {
...

    {
      ...
      assert(valueArray.getSizeInBytes == 4 + 8 + 4 + innerStruct.getSizeInBytes)
    }
  ...
  test("basic conversion with array and map") {
    ...
    assert(field1.getSizeInBytes == 4 + 8 + 4 + innerMap.getSizeInBytes)
...

    {
      ...
      assert(valueArray.getSizeInBytes == 4 + 8 + 4 + innerArray.getSizeInBytes)
    }
