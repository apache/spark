...
class ResolveHintsSuite extends AnalysisTest {
  ...
  test("case-sensitive or insensitive parameters") {
    checkAnalysis(
      ...
      ResolvedHint(testRelation, HintInfo(forceBroadcast = true)),
      caseSensitive = false)
  ...
  test("multiple broadcast hint aliases") {
    checkAnalysis(
      ...
      Join(ResolvedHint(testRelation, HintInfo(forceBroadcast = true)),
        ResolvedHint(testRelation2, HintInfo(forceBroadcast = true)), Inner, None),
      caseSensitive = false)
  }
  ...
  test("do not traverse past existing broadcast hints") {
    checkAnalysis(
      UnresolvedHint("MAPJOIN", Seq("table"),
        ResolvedHint(table("table").where('a > 1), HintInfo(forceBroadcast = true))),
      ResolvedHint(testRelation.where('a > 1), HintInfo(forceBroadcast = true)).analyze,
      caseSensitive = false)
  }
  ...
  test("should work for CTE") {
    checkAnalysis(
      ...
      ResolvedHint(testRelation.where('a > 1).select('a), HintInfo(forceBroadcast = true))
        .select('a).analyze,
      caseSensitive = false)
  }
