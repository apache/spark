...
class OptimizeJsonExprsSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val structAtt = "struct".attr.struct(schema).notNull
  private val jsonAttr = "json".attr.string
  ...
  test("SPARK-32948: optimize from_json + to_json") {
    ...
    val query1 = testRelation
      .select(JsonToStructs(schema, options, StructsToJson(options, "struct".attr)).as("struct"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val expected = testRelation.select("struct".attr.as("struct")).analyze
      ...
      .select(
        JsonToStructs(schema, options,
          StructsToJson(options,
            JsonToStructs(schema, options,
              StructsToJson(options, "struct".attr)))).as("struct"))
  ...
  test("SPARK-32948: not optimize from_json + to_json if schema is different") {
    ...
    val expected = testRelation.select(
      JsonToStructs(schema, options, StructsToJson(options, "struct".attr)).as("struct")).analyze
  ...
  test("SPARK-32948: not optimize from_json + to_json if nullability is different") {
    ...
    val structAtt = "struct".attr.struct(nonNullSchema).notNull
  ...
  test("SPARK-32948: not optimize from_json + to_json if timezone is different") {
    ...
    val query1 = testRelation
      .select(JsonToStructs(schema, options,
        StructsToJson(options, "struct".attr, Option(PST.getId)), UTC_OPT).as("struct"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val expected1 = testRelation.select(
      JsonToStructs(schema, options,
        StructsToJson(options, "struct".attr, Option(PST.getId)), UTC_OPT).as("struct")).analyze
    comparePlans(optimized1, expected1)

    val query2 = testRelation
      .select(JsonToStructs(schema, options,
        StructsToJson(options, "struct".attr, UTC_OPT), UTC_OPT).as("struct"))
    val optimized2 = Optimizer.execute(query2.analyze)
    val expected2 = testRelation.select("struct".attr.as("struct")).analyze
  ...
  test("SPARK-32958: prune unnecessary columns from GetStructField + from_json") {
    ...
    val query1 = testRelation2
      .select(GetStructField(JsonToStructs(schema, options, "json".attr), 0))
    val optimized1 = Optimizer.execute(query1.analyze)

    val prunedSchema1 = StructType.fromDDL("a int")
    val expected1 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema1, options, "json".attr), 0)).analyze
    comparePlans(optimized1, expected1)

    val query2 = testRelation2
      .select(GetStructField(JsonToStructs(schema, options, "json".attr), 1))
    val optimized2 = Optimizer.execute(query2.analyze)

    val prunedSchema2 = StructType.fromDDL("b int")
    val expected2 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema2, options, "json".attr), 0)).analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-32958: prune unnecessary columns from GetArrayStructFields + from_json") {
    ...
    val query1 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(schema1, options, "json".attr), field1, 0, 2, true).as("a"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val prunedSchema1 = ArrayType(StructType.fromDDL("a int"), containsNull = true)
    val expected1 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(prunedSchema1, options, "json".attr), field1, 0, 1, true).as("a")).analyze
    comparePlans(optimized1, expected1)
    ...
    val query2 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(schema2, options, "json".attr), field2, 1, 2, false).as("b"))
    val optimized2 = Optimizer.execute(query2.analyze)
    ...
    val expected2 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(prunedSchema2, options, "json".attr), field2, 0, 1, false).as("b")).analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-33007: simplify named_struct + from_json") {
    ...
    val UTC_OPT = Option("UTC")
    val json: BoundReference = "json".attr.string.canBeNull.at(0)
