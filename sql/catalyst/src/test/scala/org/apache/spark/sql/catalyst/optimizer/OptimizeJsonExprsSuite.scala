...
class OptimizeJsonExprsSuite extends PlanTest with ExpressionEvalHelper {
    ...
    val query2 = testRelation
      .select(
        JsonToStructs(
          ...
          StructsToJson(
            options,
            JsonToStructs(schema, options, StructsToJson(options, $"struct")))).as("struct"))
  ...
  test("SPARK-32948: not optimize from_json + to_json if nullability is different") {
    ...
    val expected = testRelationWithNonNullAttr
      .select(JsonToStructs(schema, options, StructsToJson(options, $"struct")).as("struct"))
      .analyze
    comparePlans(optimized, expected)
  }
  ...
  test("SPARK-32948: not optimize from_json + to_json if timezone is different") {
    ...
    val query1 = testRelation
      .select(
        JsonToStructs(
          schema,
          options,
          StructsToJson(options, $"struct", Option(PST.getId)),
          UTC_OPT).as("struct"))
    val optimized1 = Optimizer.execute(query1.analyze)
    ...
    val query2 = testRelation
      .select(
        JsonToStructs(schema, options, StructsToJson(options, $"struct", UTC_OPT), UTC_OPT)
          .as("struct"))
    val optimized2 = Optimizer.execute(query2.analyze)
  ...
  test("SPARK-32958: prune unnecessary columns from GetStructField + from_json") {
    ...
    val expected1 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema1, options, $"json"), 0))
      .analyze
    comparePlans(optimized1, expected1)
    ...
    val expected2 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema2, options, $"json"), 0))
      .analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-32958: prune unnecessary columns from GetArrayStructFields + from_json") {
    ...
    val query1 = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(schema1, options, $"json"), field1, 0, 2, true)
          .as("a"))
    val optimized1 = Optimizer.execute(query1.analyze)
    ...
    val expected1 = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(prunedSchema1, options, $"json"), field1, 0, 1, true)
          .as("a"))
      .analyze
    comparePlans(optimized1, expected1)
    ...
    val query2 = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(schema2, options, $"json"), field2, 1, 2, false)
          .as("b"))
    val optimized2 = Optimizer.execute(query2.analyze)

    val prunedSchema2 =
      ArrayType(StructType(StructField("b", IntegerType, false) :: Nil), containsNull = false)
    val expected2 = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(prunedSchema2, options, $"json"), field2, 0, 1, false)
          .as("b"))
      .analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-33007: simplify named_struct + from_json") {
    ...
    assertEquivalent(
      ...
      namedStruct(
        "a",
        GetStructField(JsonToStructs(schema, options, json, UTC_OPT), 0),
        "b",
        GetStructField(JsonToStructs(schema, options, json, UTC_OPT), 1)).as("struct"),
      If(
        IsNull(json),
        nullStruct,
    ...
    assertEquivalent(
      ...
      namedStruct(
        "a1",
        GetStructField(JsonToStructs(field1, options, json, UTC_OPT), 0),
        "b",
        GetStructField(JsonToStructs(field2, options, json, UTC_OPT), 0)).as("struct"))
    ...
    assertEquivalent(
      ...
      namedStruct(
        ...
        GetStructField(JsonToStructs(schema, options, json, UTC_OPT), 0),
        "a",
        GetStructField(JsonToStructs(schema, options, json, UTC_OPT), 0)).as("struct"),
      namedStruct(
        "a",
        GetStructField(JsonToStructs(field1, options, json, UTC_OPT), 0),
        "a",
        GetStructField(JsonToStructs(field1, options, json, UTC_OPT), 0)).as("struct"))
    ...
    assertEquivalent(
      ...
      namedStruct(
        ...
        GetStructField(JsonToStructs(schema, options, json, UTC_OPT), 0),
        "b",
        GetStructField(JsonToStructs(schema, options, json, Option(PST.getId)), 1))
        .as("struct"),
      namedStruct(
        "a",
        GetStructField(JsonToStructs(field1, options, json, UTC_OPT), 0),
        "b",
        GetStructField(JsonToStructs(field2, options, json, Option(PST.getId)), 0))
        .as("struct"))
  }
  ...
  test(
    "SPARK-49743: prune unnecessary columns from GetArrayStructFields does not change schema") {
    val options = Map.empty[String, String]
    ...
    val query = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(schema, options, $"json"), field, 0, 2, true).as("a"))
    val optimized = Optimizer.execute(query.analyze)

    val prunedSchema = ArrayType(StructType.fromDDL("a int"), containsNull = true)
    val expected = testRelation2
      .select(
        GetArrayStructFields(JsonToStructs(prunedSchema, options, $"json"), field, 0, 1, true).as(
          "a"))
      .analyze
    comparePlans(optimized, expected)
  }
