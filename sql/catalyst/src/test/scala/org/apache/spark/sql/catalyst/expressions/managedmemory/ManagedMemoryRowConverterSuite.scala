...
class ManagedMemoryRowConverterSuite extends FunSuite with Matchers {

  test("basic conversion with only primitive types") {
    val fieldTypes: Array[DataType] = Array(LongType, LongType, IntegerType)
    val converter = new ManagedMemoryRowConverter(fieldTypes)
    ...
    val managedMemoryRow = new ManagedMemoryRow()
    managedMemoryRow.pointTo(buffer, PlatformDependent.LONG_ARRAY_OFFSET, fieldTypes.length, null)
    managedMemoryRow.getLong(0) should be (0)
    managedMemoryRow.getLong(1) should be (1)
    managedMemoryRow.getInt(2) should be (2)
  }
  ...
  test("basic conversion with primitive and string types") {
    ...
    managedMemoryRow.pointTo(buffer, PlatformDependent.LONG_ARRAY_OFFSET, fieldTypes.length, null)
    managedMemoryRow.getLong(0) should be (0)
    managedMemoryRow.getString(1) should be ("Hello")
    managedMemoryRow.getString(2) should be ("World")
  }
  ...
  test("null handling") {
    ...
    val createdFromNull = new ManagedMemoryRow()
    ...
    // If we have an managedMemoryRow with columns that are initially non-null and we null out those
    ...
    val setToNullAfterCreation = new ManagedMemoryRow()
