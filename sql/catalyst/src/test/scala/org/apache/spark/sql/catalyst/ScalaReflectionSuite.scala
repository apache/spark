...
class ScalaReflectionSuite extends SparkFunSuite {
  ...
  test("nullable data") {
    ...
    assert(schema === Schema(
      StructType(Seq(
        ...
        StructField("decimalField", DecimalType.Maximum, nullable = true),
  ...
  test("get data type of a value") {
    ...
    def typeOfObject1: PartialFunction[Any, DataType] = typeOfObject orElse {
      case value: java.math.BigInteger => DecimalType.Maximum
      case value: java.math.BigDecimal => DecimalType.Maximum
      case _ => StringType
    }

    assert(DecimalType.Maximum === typeOfObject1(
