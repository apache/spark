...
case class NestedGeneric[T](generic: GenericData[T])

case class SeqNestedGeneric[T](generic: Seq[T])
...
class ScalaReflectionSuite extends SparkFunSuite {
  ...
  test("SQLUserDefinedType annotation on Scala structure") {
    ...
    assert(schema === Schema(new TestingUDT.NestedStructUDT, nullable = true))
  }
  ...
  test("SPARK-38681: List nested generic") {
    ...
    assert(
      schema === Schema(
        StructType(Seq(StructField("generic", ArrayType(IntegerType, false), nullable = true))),
        nullable = true))
  }
  ...
  test("convert Option[Product] to catalyst") {
    ...
    val data = OptionalData(
      ...
      Some(true),
      Some(primitiveData),
      Some(new CalendarInterval(1, 2, 3)))
  ...
  test("serialize and deserialize arbitrary sequence types") {
    ...
    assert(
      arrayBufferSerializer.dataType ==
        ArrayType(IntegerType, containsNull = false))
  ...
  test("serialize and deserialize arbitrary map types") {
    ...
    assert(
      linkedHashMapSerializer.dataType ==
  ...
  test("SPARK-22442: Generate correct field names for special characters") {
    ...
    val argumentsFields = newInstance.arguments.flatMap {
      _.collect { case UpCast(u: UnresolvedExtractValue, _, _) =>
        u.extraction.toString
      }
    }
  ...
  test("SPARK-8288: schemaFor works for a class with only a companion object constructor") {
    ...
    assert(
      schema === Schema(
        StructType(Seq(StructField("x", IntegerType, nullable = false))),
        nullable = true))
  }
  ...
  test("SPARK-27625: annotated data types") {
    assert(
      serializerFor[FooWithAnnotation].dataType == StructType(
        Seq(StructField("f1", StringType), StructField("f2", StringType))))
  ...
  test("SPARK-32585: Support scala enumeration in ScalaReflection") {
    assert(
      serializerFor[FooClassWithEnum].dataType == StructType(
        Seq(StructField("i", IntegerType, false), StructField("e", StringType, true))))
  ...
  test("encoder for row") {
    ...
    assert(
      encoderForWithRowEncoderSupport[Map[Row, Row]] ===
        MapEncoder(
          ...
          true))
    assert(
      encoderForWithRowEncoderSupport[MyClass] ===
        ProductEncoder(
