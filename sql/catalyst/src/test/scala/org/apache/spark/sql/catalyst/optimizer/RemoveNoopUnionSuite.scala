  ...
  object Optimize extends RuleExecutor[LogicalPlan] {
    val batches =
      Batch("CollapseProject", Once, CollapseProject) ::
        Batch("RemoveNoopUnion", Once, RemoveNoopUnion) :: Nil
  }

  val testRelation = LocalRelation($"a".int, $"b".int)
  val testRelation2 =
    LocalRelation(output = Seq($"a".int, $"b".int), data = Seq(InternalRow(1, 2)))
  ...
  test("SPARK-34474: Do not remove necessary Project 1") {
    val child1 = Project(
      Seq(
        ...
        testRelation.output(1),
        (testRelation.output(0) + 1).as("expr")),
      testRelation)
    val child2 = Project(
      Seq(
        testRelation.output(0),
        testRelation.output(1),
        (testRelation.output(0) + 2).as("expr")),
      testRelation)
