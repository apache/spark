...
class FilterPushdownSuite extends PlanTest {
  ...
  test("generate: predicate referenced no generated column") {
    val originalQuery = {
      testRelationWithArrayType
        .generate(Explode('c_arr), unrequiredChildOutput = Nil, false, Some("arr"))
        .where(('b >= 5) && ('a > 6))
    }
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = {
      testRelationWithArrayType
        .where(('b >= 5) && ('a > 6))
        .generate(Explode('c_arr), unrequiredChildOutput = Nil, false, Some("arr")).analyze
    }
  ...
  test("generate: part of conjuncts referenced generated column") {
    ...
    val originalQuery = {
      testRelationWithArrayType
        .generate(generator, unrequiredChildOutput = Nil, false, Some("arr"))
        .where(('b >= 5) && ('c > 6))
    }
    val optimized = Optimize.execute(originalQuery.analyze)
    val referenceResult = {
