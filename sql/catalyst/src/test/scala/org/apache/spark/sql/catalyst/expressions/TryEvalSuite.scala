...
class TryEvalSuite extends SparkFunSuite with ExpressionEvalHelper {
  test("try_add") {
    Seq((1, 1, 2), (Int.MaxValue, 1, null), (Int.MinValue, -1, null)).foreach {
      case (a, b, expected) =>
        val left = Literal(a)
        val right = Literal(b)
  ...
  test("try_divide") {
    Seq((3.0, 2.0, 1.5), (1.0, 0.0, null), (-1.0, 0.0, null)).foreach { case (a, b, expected) =>
      val left = Literal(a)
      val right = Literal(b)
  ...
  test("try_mod") {
    Seq((3.0, 2.0, 1.0), (1.0, 0.0, null), (-1.0, 0.0, null)).foreach { case (a, b, expected) =>
      val left = Literal(a)
      val right = Literal(b)
  ...
  test("try_subtract") {
    Seq((1, 1, 0), (Int.MaxValue, -1, null), (Int.MinValue, 1, null)).foreach {
      case (a, b, expected) =>
        val left = Literal(a)
        val right = Literal(b)
  ...
  test("try_multiply") {
    Seq((2, 3, 6), (Int.MaxValue, -10, null), (Int.MinValue, 10, null)).foreach {
      case (a, b, expected) =>
        val left = Literal(a)
        val right = Literal(b)
  ...
  test("Throw exceptions from children") {
    ...
    Seq(
      ...
      Divide(Literal(1.0), failingChild, EvalMode.TRY)).foreach { expr =>
