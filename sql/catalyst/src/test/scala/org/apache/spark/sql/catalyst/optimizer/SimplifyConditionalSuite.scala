...
class SimplifyConditionalSuite extends PlanTest with ExpressionEvalHelper with PredicateHelper {
  ...
  private val relation = LocalRelation(attr("a").int, attr("b").int, attr("c").boolean)
  ...
  test("simplify CaseWhen if all the outputs are semantic equivalence") {
    ...
    assertEquivalent(
      CaseWhen((attr("a").isNotNull, Subtract(Literal(3), Literal(2))) ::
        (attr("b").isNull, Literal(1)) ::
        (!attr("c"), Add(Literal(6), Literal(-5))) ::
        Nil,
  ...
  test("simplify if when one clause is null and another is boolean") {
    ...
    assertEquivalent(If(p, nullLiteral, TrueLiteral), Or(IsNotNull(attr("a")), nullLiteral))
    assertEquivalent(If(p, FalseLiteral, nullLiteral), And(IsNotNull(attr("a")), nullLiteral))
    ...
    Seq(TrueLiteral, FalseLiteral).foreach { b =>
      assertEquivalent(If(GreaterThan(attr("a"), 42), nullLiteral, b),
        If(GreaterThan(attr("a"), 42), nullLiteral, b))
      assertEquivalent(If(GreaterThan(attr("a"), 42), b, nullLiteral),
        If(GreaterThan(attr("a"), 42), b, nullLiteral))
    }
  ...
  test("SPARK-33845: remove unnecessary if when the outputs are boolean type") {
    ...
    val fields = Seq(
      attr("cond").boolean.notNull,
      attr("cond_nullable").boolean,
      attr("a").boolean,
      attr("b").boolean
    )
  ...
  test("SPARK-33847: Remove the CaseWhen if elseValue is empty and other outputs are null") {
    assertEquivalent(
      CaseWhen((GreaterThan(attr("a"), 1), Literal.create(null, IntegerType)) :: Nil, None),
      Literal.create(null, IntegerType))
