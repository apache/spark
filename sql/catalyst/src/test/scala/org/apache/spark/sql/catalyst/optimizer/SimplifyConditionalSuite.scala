...
class SimplifyConditionalSuite extends PlanTest with ExpressionEvalHelper with PredicateHelper {
  ...
  private val relation = LocalRelation("a".attr.int, "b".attr.int, "c".attr.boolean)
  ...
  test("simplify CaseWhen if all the outputs are semantic equivalence") {
    ...
    assertEquivalent(
      CaseWhen(("a".attr.isNotNull, Subtract(Literal(3), Literal(2))) ::
        ("b".attr.isNull, Literal(1)) ::
        (!"c".attr, Add(Literal(6), Literal(-5))) ::
        Nil,
  ...
  test("simplify if when one clause is null and another is boolean") {
    ...
    assertEquivalent(If(p, nullLiteral, TrueLiteral), Or(IsNotNull("a".attr), nullLiteral))
    assertEquivalent(If(p, FalseLiteral, nullLiteral), And(IsNotNull("a".attr), nullLiteral))
    ...
    Seq(TrueLiteral, FalseLiteral).foreach { b =>
      assertEquivalent(If(GreaterThan("a".attr, 42), nullLiteral, b),
        If(GreaterThan("a".attr, 42), nullLiteral, b))
      assertEquivalent(If(GreaterThan("a".attr, 42), b, nullLiteral),
  ...
  test("SPARK-33845: remove unnecessary if when the outputs are boolean type") {
    ...
    val fields = Seq(
      "cond".attr.boolean.notNull,
      "cond_nullable".attr.boolean,
      "a".attr.boolean,
      "b".attr.boolean
    )
  ...
  test("SPARK-33847: Remove the CaseWhen if elseValue is empty and other outputs are null") {
    assertEquivalent(
      CaseWhen((GreaterThan("a".attr, 1), Literal.create(null, IntegerType)) :: Nil, None),
      Literal.create(null, IntegerType))
