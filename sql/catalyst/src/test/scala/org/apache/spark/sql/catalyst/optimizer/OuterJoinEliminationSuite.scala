...
class OuterJoinEliminationSuite extends PlanTest {
  ...
  test("joins: left to inner with complicated filter predicates #1") {
    ...
    val left = testRelation
    val right =
      testRelation1.where(!Symbol("e").isNull || (Symbol("d").isNotNull && Symbol("f").isNull))
    val correctAnswer =
      left.join(right, Inner, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: left to inner with complicated filter predicates #3") {
    ...
    val originalQuery =
      x.join(y, LeftOuter, Option("x.a".attr === "y.d".attr))
        .where((!Symbol("e").isNull || (Symbol("d").isNotNull &&
          Symbol("f").isNull)) && Symbol("e").isNull)

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation
    val right = testRelation1.where(
      (!Symbol("e").isNull || (Symbol("d").isNotNull && Symbol("f").isNull)) && Symbol("e").isNull)
    val correctAnswer =
      left.join(right, Inner, Option("a".attr === "d".attr)).analyze
