...
private class DeltaBufferWriter extends BufferWriter with DeltaWriter[InternalRow] {
  ...
  private final val DELETE_OPERATION = UTF8String.fromString(Delete.toString)
  private final val UPDATE_OPERATION = UTF8String.fromString(Update.toString)
  private final val INSERT_OPERATION = UTF8String.fromString(Insert.toString)

  override def delete(meta: InternalRow, id: InternalRow): Unit = {
    val pk = id.getInt(0)
    buffer.deletes += pk
    val logEntry = new GenericInternalRow(Array[Any](DELETE_OPERATION, pk, meta.copy(), null))
    buffer.log += logEntry
  }
  ...
  override def update(meta: InternalRow, id: InternalRow, row: InternalRow): Unit = {
    ...
    val logEntry = new GenericInternalRow(Array[Any](UPDATE_OPERATION, pk, meta.copy(), row.copy()))
    buffer.log += logEntry
  }

  override def insert(meta: InternalRow, row: InternalRow): Unit = {
    buffer.rows.append(row.copy())
    val logEntry = new GenericInternalRow(Array[Any](
      INSERT_OPERATION, null, meta.copy(), row.copy()))
    buffer.log += logEntry
  }

  override def insert(row: InternalRow): Unit = {
    buffer.rows.append(row.copy())
    val logEntry = new GenericInternalRow(Array[Any](INSERT_OPERATION, null, null, row.copy()))
    buffer.log += logEntry
  }
