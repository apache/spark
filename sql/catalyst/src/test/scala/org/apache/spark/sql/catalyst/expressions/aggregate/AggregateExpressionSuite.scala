...
class AggregateExpressionSuite extends SparkFunSuite {
  ...
  test("test regr_avg input types") {
    val checkResult1 = RegrAvgX(Literal("a"), Literal(1)).checkInputDataTypes()
    ...
    val checkResult2 = RegrAvgX(Literal(3.0D), Literal('b')).checkInputDataTypes()
    ...
    val checkResult3 = RegrAvgX(Literal(3.0D), Literal(Array(0))).checkInputDataTypes()
    ...
    assert(RegrAvgX(Literal(3.0D), Literal(1)).checkInputDataTypes() ===
      TypeCheckResult.TypeCheckSuccess)
    assert(RegrAvgX(Literal(3.0D), Literal(Decimal(1))).checkInputDataTypes() ===
      TypeCheckResult.TypeCheckSuccess)
  }
