...
public abstract class Operation {
    protected final ThriftServerSession parentSession;
    ...
    protected volatile SparkThriftServerSQLException operationException;
    ...
    protected Operation(ThriftServerSession parentSession, OperationType opType) {
        this(parentSession, null, opType);
    }

    protected Operation(ThriftServerSession parentSession, Map<String, String> confOverlay,
                        OperationType opType) {
        this(parentSession, confOverlay, opType, false);
    }

    protected Operation(ThriftServerSession parentSession,
    ...
    public ThriftServerSession getParentSession() {
    ...
    protected final OperationState setState(OperationState newState) throws SparkThriftServerSQLException {
    ...
    protected void setOperationException(SparkThriftServerSQLException operationException) {
        this.operationException = operationException;
    }

    protected final void assertState(OperationState state) throws SparkThriftServerSQLException {
        if (this.state != state) {
            throw new SparkThriftServerSQLException("Expected state " + state + ", but found " + this.state);
        }
        this.lastAccessTime = System.currentTimeMillis();
    }
     ...
     */
    protected abstract void runInternal() throws SparkThriftServerSQLException;

    public void run() throws SparkThriftServerSQLException {
        beforeRun();
        try {
            runInternal();
        } finally {
    ...
    public void cancel() throws SparkThriftServerSQLException {
        setState(OperationState.CANCELED);
        throw new UnsupportedOperationException("SQLOperation.cancel()");
    }

    public abstract void close() throws SparkThriftServerSQLException;

    public abstract TableSchema getResultSetSchema() throws SparkThriftServerSQLException;

    public abstract RowSet getNextRowSet(FetchOrientation orientation, long maxRows) throws SparkThriftServerSQLException;

    public RowSet getNextRowSet() throws SparkThriftServerSQLException {
    ...
    protected void validateFetchOrientation(FetchOrientation orientation,
                                            EnumSet<FetchOrientation> supportedOrientations) throws SparkThriftServerSQLException {
        if (!supportedOrientations.contains(orientation)) {
            throw new SparkThriftServerSQLException("The fetch type " + orientation.toString() +
                    " is not supported for this resultset", "HY106");
        }
    }

    protected SparkThriftServerSQLException toSQLException(String prefix, CommandProcessorResponse response) {
        SparkThriftServerSQLException ex = new SparkThriftServerSQLException(prefix + ": " + response.getErrorMessage(),
