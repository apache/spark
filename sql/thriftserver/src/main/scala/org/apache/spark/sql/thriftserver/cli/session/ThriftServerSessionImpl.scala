...
import org.apache.spark.sql.thriftserver.cli.operation.{Operation, OperationManager}
import org.apache.spark.sql.thriftserver.cli.thrift.TProtocolVersion
import org.apache.spark.sql.thriftserver.cli.utils.VariableSubstitution
import org.apache.spark.sql.thriftserver.server.ThreadWithGarbageCleanup
...
private[thriftserver] class ThriftServerSessionImpl(
  ...
  protected def release(userAccess: Boolean): Unit = {
    ...
    if (Thread.currentThread.isInstanceOf[ThreadWithGarbageCleanup]) {
      val currentThread = Thread.currentThread.asInstanceOf[ThreadWithGarbageCleanup]
