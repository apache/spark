...
import org.apache.spark.sql.connector.catalog.{CatalogV2Util, Column, Delete, Identifier, InMemoryRowLevelOperationTable, InMemoryRowLevelOperationTableCatalog, Insert, MetadataColumn, Operation, Unknown, Update}
...
abstract class RowLevelOperationSuiteBase
  ...
  protected final val PARTITION_FIELD = StructField(
    ...
    metadata = new MetadataBuilder()
      ...
      .putBoolean(MetadataColumn.PRESERVE_ON_INSERT_AS_UPDATE, value = true)
      .build())
  ...
  protected def writeLogEntry(metadata: Row, data: Row): WriteLogEntry = {
    WriteLogEntry(operation = Unknown, metadata = Some(metadata), data = Some(data))
  }
  ...
  protected def insertWriteLogEntry(metadata: Row, data: Row): WriteLogEntry = {
    WriteLogEntry(operation = Insert, metadata = Some(metadata), data = Some(data))
  }
