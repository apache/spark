...
class FiltersSuite extends SparkFunSuite {
  ...
  private def withFieldNames(f: (String, Array[String]) => Unit): Unit = {
    Seq(
      ...
      ("a.b", Array("a", "b")),
      ("`a.b`.c", Array("a.b", "c")),
      ("`a.b`.`c.d`.`e.f`", Array("a.b", "c.d", "e.f"))).foreach { case (name, fieldNames) =>
      f(name, fieldNames)
    }
  }

  test("EqualTo references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        EqualTo(name, EqualTo("b", "2")).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        EqualTo("b", EqualTo(name, "2")).v2references.toSeq.map(_.toSeq)
  ...
  test("EqualNullSafe references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        EqualNullSafe(name, EqualTo("b", "2")).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        EqualNullSafe("b", EqualTo(name, "2")).v2references.toSeq.map(_.toSeq)
          == Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("GreaterThan references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        GreaterThan(name, EqualTo("b", "2")).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        GreaterThan("b", EqualTo(name, "2")).v2references.toSeq.map(_.toSeq)
          == Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("GreaterThanOrEqual references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        GreaterThanOrEqual(name, "1").v2references.toSeq.map(_.toSeq) == Seq(fieldNames.toSeq))
      ...
      assert(
        GreaterThanOrEqual(name, EqualTo("b", "2")).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        GreaterThanOrEqual("b", EqualTo(name, "2")).v2references.toSeq.map(_.toSeq)
          == Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("LessThanOrEqual references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        LessThanOrEqual(name, EqualTo("b", "2")).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        LessThanOrEqual("b", EqualTo(name, "2")).v2references.toSeq.map(_.toSeq)
          == Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("In references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        In(name, Array("1", EqualTo("b", "2"))).v2references.toSeq.map(_.toSeq)
          == Seq(fieldNames.toSeq, Seq("b")))
      assert(
        In("b", Array("1", EqualTo(name, "2"))).v2references.toSeq.map(_.toSeq)
          == Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("And references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        And(EqualTo(name, "1"), EqualTo("b", "1")).v2references.toSeq.map(_.toSeq) ==
          Seq(fieldNames.toSeq, Seq("b")))
      assert(
        And(EqualTo("b", "1"), EqualTo(name, "1")).v2references.toSeq.map(_.toSeq) ==
          Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("Or references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        Or(EqualTo(name, "1"), EqualTo("b", "1")).v2references.toSeq.map(_.toSeq) ==
          Seq(fieldNames.toSeq, Seq("b")))
      assert(
        Or(EqualTo("b", "1"), EqualTo(name, "1")).v2references.toSeq.map(_.toSeq) ==
          Seq(Seq("b"), fieldNames.toSeq))
    }
  }
  ...
  test("StringStartsWith references") {
    withFieldNames { (name, fieldNames) =>
      ...
      assert(
        StringStartsWith(name, "str").v2references.toSeq.map(_.toSeq) == Seq(fieldNames.toSeq))
    }
  }
