...
class DateFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("function current_date") {
    ...
    val d2 =
      DateTimeUtils.fromJavaDate(sql("""SELECT CURRENT_DATE()""").collect().head.getDate(0))
    val d3 = DateTimeUtils.fromJavaDate(sql("""SELECT CURDATE()""").collect().head.getDate(0))
  ...
  test("date comparison with date strings") {
    val df = Seq((1, Date.valueOf("2015-01-01")), (2, Date.valueOf("2014-01-01"))).toDF("i", "t")

    checkAnswer(
      df.select("t").filter($"t" <= "2014-06-01"),
      Row(Date.valueOf("2014-01-01")) :: Nil)

    checkAnswer(df.select("t").filter($"t" >= "2015"), Row(Date.valueOf("2015-01-01")) :: Nil)
  }
  ...
  test("year") {
    ...
    checkAnswer(df.select(year($"a"), year($"b"), year($"c")), Row(2015, 2015, 2013))

    checkAnswer(df.selectExpr("year(a)", "year(b)", "year(c)"), Row(2015, 2015, 2013))
  }
  ...
  test("quarter") {
    ...
    checkAnswer(df.selectExpr("quarter(a)", "quarter(b)", "quarter(c)"), Row(2, 2, 4))
  }
  ...
  test("dayofmonth & day") {
    ...
    checkAnswer(df.select(dayofmonth($"a"), dayofmonth($"b"), dayofmonth($"c")), Row(8, 8, 8))

    checkAnswer(df.select(day($"a"), day($"b"), day($"c")), Row(8, 8, 8))
  ...
  test("dayofyear") {
    ...
    checkAnswer(df.select(dayofyear($"a"), dayofyear($"b"), dayofyear($"c")), Row(98, 98, 98))

    checkAnswer(df.selectExpr("dayofyear(a)", "dayofyear(b)", "dayofyear(c)"), Row(98, 98, 98))
  }
  ...
  test("minute") {
    ...
    checkAnswer(df.select(minute($"a"), minute($"b"), minute($"c")), Row(0, 10, 10))

    checkAnswer(df.selectExpr("minute(a)", "minute(b)", "minute(c)"), Row(0, 10, 10))
  }

  test("second") {
    val df = Seq((d, sdf.format(d), ts)).toDF("a", "b", "c")

    checkAnswer(df.select(second($"a"), second($"b"), second($"c")), Row(0, 15, 15))

    checkAnswer(df.selectExpr("second(a)", "second(b)", "second(c)"), Row(0, 15, 15))
  }

  test("weekofyear") {
    val df = Seq((d, sdfDate.format(d), ts)).toDF("a", "b", "c")

    checkAnswer(df.select(weekofyear($"a"), weekofyear($"b"), weekofyear($"c")), Row(15, 15, 15))

    checkAnswer(df.selectExpr("weekofyear(a)", "weekofyear(b)", "weekofyear(c)"), Row(15, 15, 15))
  }

  test("weekday") {
    val df = Seq((d, sdfDate.format(d), ts)).toDF("a", "b", "c")

    checkAnswer(df.select(weekday($"a"), weekday($"b"), weekday($"c")), Row(2, 2, 0))

    checkAnswer(df.selectExpr("weekday(a)", "weekday(b)", "weekday(c)"), Row(2, 2, 0))
  }
  ...
  test("dayname") {
    ...
    checkAnswer(df.select(dayname($"a"), dayname($"b"), dayname($"c")), Row("Wed", "Wed", "Mon"))

    checkAnswer(df.selectExpr("dayname(a)", "dayname(b)", "dayname(c)"), Row("Wed", "Wed", "Mon"))
  }
  ...
  test("time_add") {
    ...
    checkAnswer(
      ...
      Seq(Row(Date.valueOf("2016-09-30")), Row(Date.valueOf("2017-02-28"))))
    checkAnswer(
      df.selectExpr(s"t + INTERVAL'$ym'"),
      Seq(
        Row(Timestamp.valueOf("2016-09-30 23:59:59")),
        Row(Timestamp.valueOf("2017-02-28 00:00:00"))))
    checkAnswer(
      df.selectExpr(s"d + INTERVAL'$dt'"),
      Seq(
        Row(Timestamp.valueOf("2015-08-01 02:03:04.005006")),
        Row(Timestamp.valueOf("2016-01-01 02:03:04.005006"))))
    checkAnswer(
      df.selectExpr(s"d + INTERVAL '$day'"),
      Seq(Row(Date.valueOf("2015-08-01")), Row(Date.valueOf("2016-01-01"))))
    checkAnswer(
      df.selectExpr(s"t + INTERVAL'$dt'"),
      Seq(
        Row(Timestamp.valueOf("2015-08-02 02:03:03.005006")),
        Row(Timestamp.valueOf("2016-01-01 02:03:04.005006"))))
  ...
  test("time_sub") {
    ...
    checkAnswer(
      df.selectExpr(s"d - INTERVAL'$ym'"),
      Seq(Row(Date.valueOf("2014-07-30")), Row(Date.valueOf("2014-12-29"))))
    checkAnswer(
      df.selectExpr(s"t - INTERVAL'$ym'"),
      Seq(
        Row(Timestamp.valueOf("2014-08-01 00:00:01")),
        Row(Timestamp.valueOf("2014-12-29 00:00:02"))))
    checkAnswer(
      df.selectExpr(s"d - INTERVAL'$dt'"),
      Seq(
        Row(Timestamp.valueOf("2015-09-28 21:56:55.994994")),
        Row(Timestamp.valueOf("2016-02-27 21:56:55.994994"))))
    checkAnswer(
      df.selectExpr(s"d - INTERVAL '$day'"),
      Seq(Row(Date.valueOf("2015-09-29")), Row(Date.valueOf("2016-02-28"))))
    checkAnswer(
      df.selectExpr(s"t - INTERVAL'$dt'"),
      Seq(
        Row(Timestamp.valueOf("2015-09-29 21:56:56.994994")),
  ...
  test("function to_date") {
    ...
    checkAnswer(
      ...
      Row(Date.valueOf("2016-02-29")) :: Row(null) :: Nil)
  }
  ...
  test("function date_trunc") {
    ...
    checkAnswer(
      ...
      Seq(
        Row(Timestamp.valueOf("2015-07-22 10:01:40.123")),
    ...
    Seq("corrected", "legacy").foreach { legacyParserPolicy =>
      withSQLConf(SQLConf.LEGACY_TIME_PARSER_POLICY.key -> legacyParserPolicy) {
        ...
        checkAnswer(
          ...
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
        checkAnswer(
          df.select(to_unix_timestamp(col("ts"))),
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
        ...
        checkAnswer(
          ...
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
        checkAnswer(
          df.select(to_unix_timestamp(col("ss"))),
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
        ...
        checkAnswer(
          ...
          Seq(Row(secs(date1.getTime)), Row(secs(date2.getTime))))
        checkAnswer(
          df.select(to_unix_timestamp(col("d"), lit("$fmt"))),
          Seq(Row(secs(date1.getTime)), Row(secs(date2.getTime))))
        ...
        checkAnswer(
          ...
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
        checkAnswer(
          df.select(to_unix_timestamp(col("s"), lit(fmt))),
          Seq(Row(secs(ts1.getTime)), Row(secs(ts2.getTime))))
  ...
  test("datediff & date_diff") {
    val df = Seq(
      ...
      (
        ...
        "2015-07-24",
        "2015-07-24 04:00:00")).toDF("a", "b", "c", "d")
  ...
  test("from_utc_timestamp with literal zone") {
    val df = Seq(
      ...
      (Timestamp.valueOf("2015-07-25 00:00:00"), "2015-07-25 00:00:00")).toDF("a", "b")
    checkAnswer(
  ...
  test("timestamp_millis") {
    ...
    checkAnswer(
      ...
      Row(
        Timestamp.valueOf("1970-01-02 02:17:36.789"),
  ...
  test("timestamp_micros") {
    ...
    checkAnswer(
      ...
      Row(
        Timestamp.valueOf("1969-12-31 16:02:03.456789"),
  ...
  test("SPARK-30668: use legacy timestamp parser in to_timestamp") {
    ...
    withSQLConf(confKey -> "legacy") {
      ...
      checkAnswer(df.select(to_timestamp(col("ts"), "yyyy-MM-dd'T'HH:mm:ss.SSSz")), Row(expected))
    }
    withSQLConf(confKey -> "corrected") {
  ...
  test("make_interval") {
    val df = Seq((100, 11, 1, 1, 12, 30, 01.001001)).toDF(
      "year",
      "month",
      "week",
      "day",
    ...
    val result4 = df.select(
      make_interval(col("year"), col("month"), col("week"), col("day"), col("hour"), col("min")))
    checkAnswer(result3, result4)

    val result5 = df.selectExpr(s"make_interval(year, month, week, day, hour)")
    val result6 =
      df.select(make_interval(col("year"), col("month"), col("week"), col("day"), col("hour")))
    checkAnswer(result5, result6)

    val result7 = df.selectExpr(s"make_interval(year, month, week, day)")
    val result8 = df.select(make_interval(col("year"), col("month"), col("week"), col("day")))
    checkAnswer(result7, result8)
  ...
  test("make_timestamp") {
    val df = Seq((100, 11, 1, 12, 30, 01.001001, "UTC")).toDF(
      "year",
      "month",
      "day",
      "hour",
    ...
    val result4 = df.select(
      make_timestamp(col("year"), col("month"), col("day"), col("hour"), col("min"), col("sec")))
    checkAnswer(result3, result4)
  }
  ...
  test("make_timestamp_ntz") {
    val df =
      Seq((100, 11, 1, 12, 30, 01.001001)).toDF("year", "month", "day", "hour", "min", "sec")
  ...
  test("try_make_timestamp_ntz") {
    ...
    val result1 = df.selectExpr("try_make_timestamp_ntz(year, month, day, hour, min, sec)")
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      val result2 = df.select(
        make_timestamp_ntz(
          col("year"),
    ...
    val result3 = df.selectExpr("try_make_timestamp_ntz(year, month, day, hour, min, sec)")
    val result4 = df.select(
  ...
  test("try_make_timestamp_ltz") {
    ...
    val result1 =
      df.selectExpr("try_make_timestamp_ltz(year, month, day, hour, min, sec, timezone)")
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      val result2 = df.select(
        make_timestamp_ltz(
          col("year"),
    ...
    val result3 = df.selectExpr("try_make_timestamp_ltz(year, month, day, hour, min, sec)")
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      val result4 = df.select(
        make_timestamp_ltz(
          col("year"),
    ...
    val result5 = df.selectExpr("try_make_timestamp_ltz(year, month, day, hour, min, sec)")
    val result6 = df.select(
