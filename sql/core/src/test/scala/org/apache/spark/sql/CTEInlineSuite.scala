...
abstract class CTEInlineSuiteBase
    ...
    withTempView("t") {
      ...
      assert(
        df.queryExecution.analyzed
          .collect { case WithCTE(_, cteDefs) =>
            cteDefs
          }
          .head
          .length == 2,
      ...
      assert(
        df.queryExecution.optimizedPlan.collect { case r: RepartitionOperation =>
          r
        }.length == 6,
  ...
  test("SPARK-36447: nested non-deterministic CTEs referenced only once are inlined") {
    withTempView("t") {
      ...
      assert(
        ...
        "With-CTE should contain 2 CTE defs after analysis.")
      assert(
        df.queryExecution.optimizedPlan.collectFirst { case r: RepartitionOperation =>
          r
        }.isEmpty,
  ...
  test("SPARK-36447: With in subquery of main query") {
    withSQLConf(
      ...
      withTempView("t") {
        ...
        checkAnswer(df, Nil)
        assert(
          collectWithSubqueries(df.queryExecution.executedPlan) { case r: ReusedExchangeExec =>
            r
          }.length == 3,
  ...
  test("SPARK-36447: invalid nested CTEs") {
    withTempView("t") {
      Seq((0, 1), (1, 2)).toDF("c1", "c2").createOrReplaceTempView("t")
      val ex = intercept[AnalysisException](sql(s"""with
           |v2 as (
           |  select * from v1 where c3 in (select c3 from v1)
           |),
           |v1 as (
           ...
           |)
         """.stripMargin))
      checkErrorTableNotFound(ex, "`v1`", ExpectedContext("v1", 29, 30))
    }
  }
  ...
  test("CTE Predicate push-down and column pruning") {
    withTempView("t") {
      ...
      assert(cteRepartitions.length == 6, "CTE should not be inlined after optimization.")
      ...
      assert(
        ...
        "CTE columns should be pruned.")
      assert(
        distinctCteRepartitions(1).collectFirst {
          case f: Filter if f.condition.semanticEquals(GreaterThan(f.output(1), Literal(0))) => f
        }.isDefined,
        "Predicate 'c2 > 0' should be pushed down to the CTE def 'v'.")
      assert(
        distinctCteRepartitions(0).collectFirst {
          case f: Filter if f.condition.find(_.semanticEquals(f.output(0))).isDefined => f
        }.isDefined,
        "CTE 'vv' definition contains predicate 'c1 > 0'.")
      assert(
        distinctCteRepartitions(1).collectFirst {
          case f: Filter if f.condition.find(_.semanticEquals(f.output(0))).isDefined => f
        }.isEmpty,
        "Predicate 'c1 > 0' should be not pushed down to the CTE def 'v'.")
  ...
  test("CTE Predicate push-down and column pruning - combined predicate") {
    withTempView("t") {
      ...
      assert(
        distinctCteRepartitions(1).collectFirst {
          case f: Filter
              if f.condition.semanticEquals(And(
                GreaterThan(f.output(1), Literal(0)),
                Or(GreaterThan(f.output(0), Literal(0)), LessThan(f.output(2), Literal(5))))) =>
            f
        }.isDefined,
  ...
  test("CTE definitions out of original order when not inlined") {
    withTempView("issue_current") {
      Seq((1, 2, 10, 100), (2, 3, 20, 200))
        .toDF("workspace_id", "issue_id", "shard_id", "field_id")
  ...
  test("Make sure CTESubstitution places WithCTE back in the plan correctly.") {
    withView("t") {
      ...
      df3.queryExecution.analyzed match {
        ...
        case other =>
          fail(s"Expect pattern Aggregate(Join(_, (WithCTE(Union(_, _))))) but got $other")
      }
  ...
  test("SPARK-44934: CTE column pruning handles duplicate exprIds in CTE") {
    withTempView("t") {
      ...
      assert(cteRepartitions.length == 2, "CTE should not be inlined after optimization.")
      assert(
        cteRepartitions.head.collectFirst {
          case p: Project if p.projectList.length == 4 => p
        }.isDefined,
        "CTE columns should not be pruned.")
    }
  }

  test("SPARK-45752: Unreferenced CTE should all be checked by CheckAnalysis0") {
    val e = intercept[AnalysisException](sql(s"""
        |with
  ...
  test("SPARK-51625: command in CTE relations should trigger inline") {
    val plan = UnresolvedWith(
      ...
      cteRelations =
        Seq(("t", SubqueryAlias("t", ShowTables(CurrentNamespace, pattern = None)), None)))
