  ...
  private def checkEstimation(
    ...
    val mapOutputStatistics = bytesByPartitionIdArray.zipWithIndex.map {
      case (bytesByPartitionId, index) =>
        new MapOutputStatistics(index, bytesByPartitionId, Array[Int](1))
    }
  ...
  test("test estimatePartitionStartIndices - 2 Exchanges") {
...

    {
      ...
      val mapOutputStatistics =
        Array(
          new MapOutputStatistics(0, bytesByPartitionId1, Array[Int](0)),
          new MapOutputStatistics(1, bytesByPartitionId2, Array[Int](0)))
