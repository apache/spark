...
trait AlsoTestWithVirtualColumnFamilyJoins extends SQLTestUtils {
  ...
  override protected def test(testName: String, testTags: Tag*)(testBody: => Any)(implicit
    ...
    Seq("false", "true").foreach { enabled =>
      testWithVirtualColumnFamilyJoins(
        testName + s" with (with changelog checkpointing = $enabled)",
        testTags: _*) {
        withSQLConf(
    ...
    providers.foreach { provider =>
      testWithoutVirtualColumnFamilyJoins(testName + s" (with $provider)", testTags: _*) {
        withSQLConf(SQLConf.STATE_STORE_PROVIDER_CLASS.key -> provider) {
          testBody
        }
      }
    }
  ...
  def testWithVirtualColumnFamilyJoins(
      testName: String,
      testTags: Tag*)(testBody: => Any): Unit = {
...
abstract class StreamingJoinSuite
  ...
  protected def setupWindowedJoin(
      joinType: String): (MemoryStream[Int], MemoryStream[Int], DataFrame) = {
  ...
  protected def setupWindowedJoinWithLeftCondition(
      joinType: String): (MemoryStream[Int], MemoryStream[Int], DataFrame) = {
    ...
    val right =
      df2.select($"key", window($"rightTime", "10 second"), $"rightValue".cast("string"))
    ...
    val select = if (joinType == "left_semi") {
      ...
      joined.select(
        left("key"),
        left("window.end").cast("long"),
        $"leftValue",
        right("key"),
        right("window.end").cast("long"),
        $"rightValue")
    }
  ...
  protected def setupWindowedJoinWithRightCondition(
      joinType: String): (MemoryStream[Int], MemoryStream[Int], DataFrame) = {
  ...
  protected def setupSelfJoin(joinType: String): (MemoryStream[(Int, Long)], DataFrame) = {
    ...
    val joined = leftStream
      ...
      .join(
        ...
        expr(
          "leftId = rightId AND rightTime >= leftTime AND " +
  ...
  protected def assertStateStoreRows(opId: Long, joinSide: String, expectedRows: Seq[Row])(
      projFn: DataFrame => DataFrame): AssertOnQuery = Execute { q =>
...
class StreamingInnerJoinSuite extends StreamingJoinSuite {
  ...
  test("stream stream inner join on windows - without watermark") {
    ...
    val df1 = input1
      ...
      .select(
        $"value" as "key",
        timestamp_seconds($"value") as "timestamp",
  ...
  test("stream stream inner join with time range - with watermark - one side condition") {
    ...
    val joined =
      df1
        .join(df2, expr("leftKey = rightKey AND leftTime < rightTime - interval 5 seconds"))
  ...
  test("stream stream self join") {
    ...
    val df = input.toDF()
    val join =
      df.select($"value" % 5 as "key", $"value")
        .join(df.select($"value" % 5 as "key", $"value"), "key")
  ...
  test("join between three streams") {
    ...
    val joined =
      df1.join(df2, expr("leftKey = middleKey")).join(df3, expr("rightKey = middleKey"))
        ...
        assert(query.lastExecution.executedPlan.collect {
          case j @ StreamingSymmetricHashJoinExec(
              ...
              if partitionExpressionsColumns(opA.expressions) === Seq("a", "b")
                ...
                && opA.numPartitions == numPartitions && opB.numPartitions == numPartitions =>
            j
        }.size == 1)
      })
  }
  ...
  test("SPARK-26187 restore the stream-stream inner join query from Spark 2.4") {
    ...
    val resourceUri = this.getClass
      .getResource("/structured-streaming/checkpoint-version-2.4.0-streaming-join/")
      .toURI
  ...
  test(
    ...
    val df = inputStream
      ...
      .select(
        col("_1").as("value"),
        timestamp_seconds($"_2").as("timestamp"),
        col("_3").as("name"))

    val leftStream = df.select(
      col("value").as("leftId"),
      col("timestamp").as("leftTime"),
      col("name").as("leftName"))

    val rightStream = df
      // Introduce misses for ease of debugging
      .where(col("value") % 2 === 0)
      .select(
        col("value").as("rightId"),
        col("timestamp").as("rightTime"),
        col("name").as("rightName"))
    ...
    val query = leftStream
      ...
      .join(
        rightStream.withWatermark("rightTime", "5 seconds"),
        expr(
          "rightId = leftId AND leftName = rightName AND rightTime >= leftTime AND " +
            "rightTime <= leftTime + interval 5 seconds"),
        joinType = "inner")
      .select(
        col("leftId"),
    ...
    val resourceUri = this.getClass
      .getResource("/structured-streaming/checkpoint-version-3.5.1-streaming-join/")
      .toURI
    val checkpointDir = Utils.createTempDir().getCanonicalFile
    // Copy the checkpoint to a temp dir to prevent changes to the original.
    // Not doing this will lead to the test passing on the first run, but fail subsequent runs.
    ...
    checkError(
      ...
      parameters = Map("storedKeySchema" -> ".*", "newKeySchema" -> ".*"),
      matchPVals = true)
  }
  ...
  test(
    ...
    checkError(
      ...
      parameters = Map("storedValueSchema" -> ".*", "newValueSchema" -> ".*"),
      matchPVals = true)
  }
  ...
  testWithVirtualColumnFamilyJoins(
    "SPARK-51779 Join query using virtual column families with HDFS should fail") {
    withTempDir { checkpointDir =>
      withSQLConf(
        ...
        testStream(joined)(
          StartStream(checkpointLocation = checkpointDir.getCanonicalPath),
          AddData(input1, 1, 2),
          ExpectFailure[StateStoreMultipleColumnFamiliesNotSupportedException] { (t: Throwable) =>
            {
              checkError(
                exception = t.asInstanceOf[StateStoreMultipleColumnFamiliesNotSupportedException],
                condition = "UNSUPPORTED_FEATURE.STATE_STORE_MULTIPLE_COLUMN_FAMILIES",
                sqlState = Some("0A000"),
                parameters = Map("stateStoreProvider" -> "HDFSBackedStateStoreProvider"))
            }
          })
      }
    }
  ...
  testWithVirtualColumnFamilyJoins(
    "SPARK-51779 Restart the join query and changing " +
  ...
  private def testForJoinKeyNullability(left: DataFrame, right: DataFrame): DataFrame = {
    ...
    df1
      .join(df2, expr("leftKey <=> rightKey"))
...
class StreamingOuterJoinSuite extends StreamingJoinSuite {
  ...
  Seq(("left_outer", Row(3, null, 5, null)), ("right_outer", Row(null, 2, null, 5))).foreach {
    case (joinType: String, outerResult) =>
      test(s"${joinType.replace("_", " ")} with watermark range condition") {
        ...
        Seq(
          ("10 minutes", "interval 3 minutes 30 seconds"),
          ("10 minutes", "interval '3:30' minute to second")).foreach { case (watermark, bound) =>
          val (leftInput2, rightInput2, joined2) =
            setupJoinWithRangeCondition(joinType, watermark, bound, bound)
  ...
  test(
    "SPARK-26187 self left outer join should not return outer nulls for already matched rows") {
  ...
  test(
    "SPARK-26187 self right outer join should not return outer nulls for already matched rows") {
    val inputStream = MemoryStream[(Int, Long)]
    ...
    val query = leftStream
      ...
      .join(
        rightStream.withWatermark("rightTime", "5 seconds"),
        expr(
          "leftId = rightId AND leftTime >= rightTime AND " +
  ...
  test("SPARK-29438: ensure UNION doesn't lead stream-stream join to use shifted partition IDs") {
    def constructUnionDf(desiredPartitionsForInput1: Int)
      ...
      val df1 = input1
        .toDF()
        .select($"value" as "key", $"value" as "leftValue", $"value" as "rightValue")
      val (input2, df2) = setupStream("left", 2)
      val (input3, df3) = setupStream("right", 3)

      val joined = df2
        .join(df3, df2("key") === df3("key") && df2("leftTime") === df3("rightTime"), "inner")
  ...
  test("SPARK-32148 stream-stream join regression on Spark 3.0.0") {
    ...
    val expectedOutput = Seq(
      ...
      (
        ...
        "abc",
        "joined with A",
        Timestamp.valueOf("2020-01-02 00:00:10"),
        "abc",
        "A"),
      (
        Timestamp.valueOf("2020-01-02 01:00:00"),
        "abc",
        "joined with B",
        Timestamp.valueOf("2020-01-02 00:59:59"),
        "abc",
        "B"))
  ...
  test("left-outer: joining non-nullable left join key with nullable right join key") {
    ...
    testStream(joined)(
      ...
      CheckNewAnswer(Row(1, null, 1, null, 10, 2, null), Row(1, null, 2, null, 10, 4, null)))
  }
  ...
  test("left-outer: joining nullable left join key with non-nullable right join key") {
    ...
    testStream(joined)(
      ...
      CheckNewAnswer(Row(1, null, 1, null, 10, 2, null), Row(null, null, 2, null, 10, 4, null)))
  }
  ...
  private def testForLeftOuterJoinKeyNullability(left: DataFrame, right: DataFrame): DataFrame = {
    val df1 = left
      .selectExpr(
        "_1 as leftKey1",
        "_2 as leftKey2",
        "timestamp_seconds(_2) as leftTime",
        "_2 * 2 as leftValue")
      .withWatermark("leftTime", "10 seconds")
    val df2 = right
      .selectExpr(
        "_1 as rightKey1",
        "_2 as rightKey2",
        "timestamp_seconds(_2) as rightTime",
        "_2 * 3 as rightValue")
      .withWatermark("rightTime", "10 seconds")

    val windowed1 = df1.select(
      $"leftKey1",
      $"leftKey2",
      window($"leftTime", "10 second").as(Symbol("leftWindow")),
      $"leftValue")
    val windowed2 = df2.select(
      $"rightKey1",
      $"rightKey2",
      window($"rightTime", "10 second").as(Symbol("rightWindow")),
      $"rightValue")
    windowed1
      .join(
        windowed2,
  ...
  test(
    "SPARK-38684: outer join works correctly even if processing input rows and " +
    ...
    withSQLConf(
      ...
      testStream(joined)(
        ...
        CheckNewAnswer(
          (Timestamp.valueOf("2020-01-02 01:00:00"), "abc", "left in batch 2", null, null, null),
          (
            Timestamp.valueOf("2020-01-02 01:30:00"),
            "abc",
            "left in batch 3",
            Timestamp.valueOf("2020-01-02 01:30:10"),
            "abc",
            "match with left in batch 3")))
    }
  }
...
class StreamingFullOuterJoinSuite extends StreamingJoinSuite {
  ...
  test("full outer early state exclusion on left") {
    ...
    testStream(joined)(
      ...
      // not get added to the state.
      CheckNewAnswer(
        Row(1, 10, 2, null, null, null),
        Row(2, 10, 4, null, null, null),
        Row(3, 10, 6, 3, 10, "9")),
      // states
      // left: 3
      // right: 3, 4, 5
  ...
  test("full outer early state exclusion on right") {
    ...
    testStream(joined)(
      ...
      CheckNewAnswer(
        Row(null, null, null, 1, 10, "3"),
        Row(null, null, null, 2, 10, "6"),
        Row(3, 10, 6, 3, 10, "9")),
      // states
      // left: 3, 4, 5
      // right: 3
  ...
  testWithoutVirtualColumnFamilyJoins(
    ...
    withSQLConf(SQLConf.STREAMING_NO_DATA_MICRO_BATCHES_ENABLED.key -> "false") {
      ...
      testStream(join)(
        ...
        MultiAddData((memoryStream1, Seq((21L, 2))), (memoryStream2, Seq((21L, 2)))),
        CheckNewAnswer(),
        ...
        MultiAddData((memoryStream1, Seq((22L, 3))), (memoryStream3, Seq((22L, 3)))),
        CheckNewAnswer(),
