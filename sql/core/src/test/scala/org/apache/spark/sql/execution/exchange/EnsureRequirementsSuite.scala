...
class EnsureRequirementsSuite extends SharedSparkSession {
  ...
  test("SPARK-41471: shuffle right side when" +
    ...
    withSQLConf(SQLConf.V2_BUCKETING_SHUFFLE_ENABLED.key -> "true") {
      ...
      EnsureRequirements.apply(smjExec) match {
        ...
        ShuffleExchangeExec(KeyGroupedPartitioning(attrs, 4, pv, _, _, _),
  ...
  // Backported from SPARK-53322 together with the test below, which needs a keyed plan whose
  // subtree holds a `BatchScanExec` for the join key positions to be pushed into.
