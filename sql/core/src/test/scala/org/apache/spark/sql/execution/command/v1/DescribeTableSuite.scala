...
/**
 * This base suite contains unified tests for the `DESCRIBE TABLE` command that checks V1 table
 ...
 *   - V1 Hive External catalog: `org.apache.spark.sql.hive.execution.command.DescribeTableSuite`
 */
trait DescribeTableSuiteBase
    extends command.DescribeTableSuiteBase
  ...
  test("Describing of a non-existent partition") {
    withNamespaceAndTable("ns", "table") { tbl =>
      ...
      checkError(
        ...
        parameters =
          Map("partitionList" -> "PARTITION (`id` = 1)", "tableName" -> "`ns`.`table`"))
    }
  }
  ...
  test("describe a non-existent column") {
    withNamespaceAndTable("ns", "tbl") { tbl =>
      ...
      checkError(
        ...
        parameters =
          Map("colName" -> "`key1`", "caseSensitiveConfig" -> "\"spark.sql.caseSensitive\""))
    }
  }
    ...
    withSQLConf(SQLConf.CASE_SENSITIVE.key -> "true") {
      withNamespaceAndTable("ns", "tbl") { tbl =>
        ...
        checkError(
          ...
          condition = "COLUMN_NOT_FOUND",
          parameters =
            Map("colName" -> "`KEY`", "caseSensitiveConfig" -> "\"spark.sql.caseSensitive\""))
      }
    }
  }
  ...
  test("DESCRIBE AS JSON partitions, clusters, buckets") {
    withNamespaceAndTable("ns", "table") { t =>
      ...
      val expectedOutput = DescribeTableJson(
        ...
        table_properties = Some(Map("version" -> "1.0")),
        serde_library = if (getProvider() == "hive") {
          Some("org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe")
        } else {
          None
        },
        storage_properties = Some(Map("compression" -> "snappy", "max_records" -> "1000")),
  ...
  test("DESCRIBE AS JSON partition spec and statistics") {
    withNamespaceAndTable("ns", "table") { t =>
      ...
      val expectedOutput = DescribeTableJson(
        ...
        table_properties = Some(Map("t" -> "test")),
        serde_library = if (getProvider() == "hive") {
          Some("org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe")
        } else {
          None
        ...
        statistics = Some(Map("size_in_bytes" -> 0, "num_rows" -> 0)))

      assert(parsedOutput.location.isDefined)
      assert(iso8601Regex.matches(parsedOutput.created_time.get))
      assert(
        expectedOutput == parsedOutput
          .copy(location = None, created_time = None, storage_properties = None))
    }
  }
  ...
  test("DESCRIBE AS JSON cluster spec") {
    withNamespaceAndTable("ns", "table") { t =>
      ...
      val expectedOutput = DescribeTableJson(
        ...
        table_properties =
          Some(Map("t" -> "test", "clusteringColumns" -> "[[\"id\"],[\"name\"]]")),
        serde_library = if (getProvider() == "hive") {
          Some("org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe")
        } else {
          None
      ...
      val expectedOutput = DescribeTableJson(
        ...
        columns = Some(
          List(
            ...
            TableColumn(
              "name",
              Type("string", collation = Some("UTF8_BINARY")),
      ...
      Seq(true, false).foreach { isTemp =>
        withNamespaceAndTable("ns", "table") { t =>
          withView("view") {
            ...
            assert(iso8601Regex.matches(parsedOutput.created_time.get))
            assert(
              expectedOutput == parsedOutput.copy(
                created_time = None,
                table_properties = None,
                storage_properties = None,
                serde_library = None,
            ...
            if (!isTemp) {
              assert(
                parsedOutput.view_creation_spark_configuration
  ...
  test("DESCRIBE AS JSON complex types") {
    withNamespaceAndTable("ns", "table") { t =>
      ...
      val expectedOutput = DescribeTableJson(
        ...
        columns = Some(
          List(
            TableColumn(name = "logs", `type` = Type("variant"), default = None),
            TableColumn(
              name = "nested_struct",
              `type` =
                Type(
                                  ...
                                  List(
                                    ...
                                    Field(
                                      ...
                                      `type` = Type(
                                        name = "array",
                                        element_type =
                                          Some(Type("string", collation = Some("UTF8_BINARY"))),
        ...
        storage_properties = Some(Map("option1" -> "value1", "option2" -> "value2")),
        last_access = Some("UNKNOWN"),
        created_by = Some(s"Spark $SPARK_VERSION"),
        `type` = Some("MANAGED"),
        collation = Some("UTF8_BINARY"),
        ...
        table_properties =
          Some(Map("password" -> "*********(redacted)", "property1" -> "value1")),
        partition_provider = Some("Catalog"),
...
/**
 * The class contains tests for the `DESCRIBE TABLE` command to check V1 In-Memory table catalog.
 */
class DescribeTableSuite extends DescribeTableSuiteBase with CommandSuiteBase {
  ...
  test("DESCRIBE TABLE EXTENDED of a partitioned table") {
    withNamespaceAndTable("ns", "table") { tbl =>
      ...
      val createdTimeValue = descriptionDf
