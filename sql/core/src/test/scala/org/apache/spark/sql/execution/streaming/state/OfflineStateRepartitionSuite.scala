...
/**
 * Test for offline state repartitioning. This tests that repartition behaves as expected for
 * different scenarios.
 */
class OfflineStateRepartitionSuite extends StreamTest with AlsoTestWithRocksDBFeatures {
  ...
  test("Fail if empty checkpoint directory") {
    withTempDir { dir =>
      ...
      checkError(
        ...
        parameters = Map("checkpointLocation" -> dir.getAbsolutePath))
    }
  }
...
object OfflineStateRepartitionTestUtils {
  ...
  def verifyRepartitionBatch(
    ...
    val baseOperatorsMetadata =
      getOperatorMetadata(checkpointLocation, serializableConf, previousBatchId)
    val repartitionOperatorsMetadata =
      getOperatorMetadata(checkpointLocation, serializableConf, batchId)
    verifyOperatorMetadata(
      baseOperatorsMetadata,
      repartitionOperatorsMetadata,
      expectedShufflePartitions)
  ...
  private def verifyOffsetAndCommitLog(
    ...
    assert(
      lastBatchId == repartitionBatchId,
    ...
    assert(
      lastCommitId == repartitionBatchId,
    ...
    assert(
      lastBatch.offsets == previousBatch.offsets,
    ...
    (lastBatch.metadataOpt, previousBatch.metadataOpt) match {
      case (Some(lastMetadata), Some(previousMetadata)) =>
        ...
        assert(
          lastMetadata.batchWatermarkMs == previousMetadata.batchWatermarkMs,
          "Batch watermark should be the same")
        assert(
          lastMetadata.batchTimestampMs == previousMetadata.batchTimestampMs,
        ...
        assert(
          lastConfWithoutShufflePartitions == previousConfWithoutShufflePartitions,
  ...
  private def verifyPartitionDirs(
    ...
    for (op <- 0 until numOperators) {
      ...
      assert(
        expectedShufflePartitions <= numPartitions,
  ...
  private def getOperatorMetadata(
      ...
      batchId: Long): Array[OperatorStateMetadata] = {
    val metadataPartitionReader =
      new StateMetadataPartitionReader(checkpointLocation, serializableConf, batchId)
  ...
  private def verifyOperatorMetadata(
    ...
    assert(
      repartitionOperatorsMetadata.nonEmpty,
      "Repartition batch should have operator metadata")
    assert(
      baseOperatorsMetadata.length == repartitionOperatorsMetadata.length,
    ...
    baseOperatorsMetadata.zip(repartitionOperatorsMetadata).foreach {
      case (baseOp, repartitionOp) =>
        ...
        assert(
          baseOp.getClass == repartitionOp.getClass,
        ...
        (baseOp, repartitionOp) match {
          case (baseV2: OperatorStateMetadataV2, repartitionV2: OperatorStateMetadataV2) =>
            ...
            assert(
              baseV2.operatorInfo == repartitionV2.operatorInfo,
            ...
            assert(
              baseV2.operatorPropertiesJson == repartitionV2.operatorPropertiesJson,
              "Operator properties JSON should match")

            // Verify state store info (except numPartitions)
            assert(
              baseV2.stateStoreInfo.length == repartitionV2.stateStoreInfo.length,
          ...
          case (baseV1: OperatorStateMetadataV1, repartitionV1: OperatorStateMetadataV1) =>
            ...
            assert(
              baseV1.operatorInfo == repartitionV1.operatorInfo,
              "V1 operator info should be the same")
            assert(
              baseV1.stateStoreInfo.sameElements(repartitionV1.stateStoreInfo),
  ...
  private def verifyCheckpointIds(
    ...
    assert(
      commitMetadata.stateUniqueIds.isDefined,
    ...
    assert(operatorIdToCkptInfos.nonEmpty, "operatorIdToCkptInfos should not be empty")
    ...
    operatorIdToCkptInfos.foreach { case (operatorId, partitionToCkptIds) =>
      ...
      assert(
        partitionToCkptIds.length == expectedShufflePartitions,
      ...
      partitionToCkptIds.zipWithIndex.foreach { case (ckptIds, partitionId) =>
        assert(
          ckptIds.length == expectedStoreCnts(operatorId),
