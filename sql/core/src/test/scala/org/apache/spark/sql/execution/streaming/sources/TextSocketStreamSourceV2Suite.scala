...
class TextSocketStreamSourceV2Suite extends StreamTest
  ...
  test("basic usage") {
    ...
    val provider = new TextSocketSourceProviderV2
    ...
    failAfter(streamingTimeout) {
      ...
      while (batchReader.getEndOffset.asInstanceOf[TextSocketStreamOffset].offset == -1L) {
        batchReader.setOffsetRange(Optional.empty(), Optional.empty())
        Thread.sleep(10)
      }
  ...
  test("non-boolean includeTimestamp") {
    ...
    intercept[AnalysisException] {
      provider.createMicroBatchReader(Optional.empty(), "",
        new DataSourceV2Options(
          Map("host" -> "localhost", "port" -> "1234", "includeTimestamp" -> "fasle").asJava))
    }
  }
  ...
  test("user-specified schema given") {
    ...
    val exception = intercept[AnalysisException] {
      provider.createMicroBatchReader(
        Optional.of(userSpecifiedSchema),
        "",
        new DataSourceV2Options(Map("host" -> "localhost", "port" -> "1234").asJava))
    }
  ...
  test("no server up") {
    val provider = new TextSocketSourceProviderV2
    val parameters = new DataSourceV2Options(Map("host" -> "localhost", "port" -> "0").asJava)
    intercept[IOException] {
      batchReader = provider.createMicroBatchReader(Optional.empty(), "", parameters)
      batchReader.setOffsetRange(Optional.empty(), Optional.empty())
    }
  }
