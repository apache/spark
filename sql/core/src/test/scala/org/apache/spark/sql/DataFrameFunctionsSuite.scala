...
class DataFrameFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("array_prepend -> Unit Test cases for the function ") {
    val df1 = Seq((Array[Int](3, 2, 5, 1, 2), 3)).toDF("a", "b")
    checkAnswer(df1.select(array_prepend(col("a"), col("b"))), Seq(Row(Seq(3, 3, 2, 5, 1, 2))))
    val df2 = Seq((Array[String]("a", "b", "c"), "d")).toDF("a", "b")
    checkAnswer(df2.select(array_prepend(col("a"), col("b"))), Seq(Row(Seq("d", "a", "b", "c"))))
    val df3 = Seq((Array[String]("a", "b", "c"), 3)).toDF("a", "b")
    checkError(
      exception = intercept[AnalysisException] {
        df3.select(array_prepend(col("a"), col("b")))
      },
      errorClass = "DATATYPE_MISMATCH.ARRAY_FUNCTION_DIFF_TYPES",
      parameters = Map(
        ...
        "leftType" -> "\"ARRAY<STRING>\"",
        "rightType" -> "\"INT\"",
        "sqlExpr" -> "\"array_prepend(a, b)\"")
    )

    checkAnswer(df1.selectExpr("array_prepend(a, 3)"), Seq(Row(Seq(3, 3, 2, 5, 1, 2))))

    checkAnswer(df2.selectExpr("array_prepend(a, b)"), Seq(Row(Seq("d", "a", "b", "c"))))

    checkError(
      exception = intercept[AnalysisException] {
        df3.selectExpr("array_prepend(a, b)")
      },
      errorClass = "DATATYPE_MISMATCH.ARRAY_FUNCTION_DIFF_TYPES",
      parameters = Map(
        "functionName" -> "`array_prepend`",
    ...
    checkAnswer(df4.selectExpr("array_prepend(a, b)"),
      Seq(Row(Seq("d", "a", "b", "c")), Row(null), Row(null), Row(null)))

    val df5 = Seq((Array[Double](3d, 2d, 5d, 1d, 2d), 3)).toDF("a", "b")
    checkAnswer(df5.selectExpr("array_prepend(a, b)"),
      Seq(Row(Seq(3d, 3d, 2d, 5d, 1d, 2d))))

    val df6 = Seq(("x", "y")).toDF("a", "b")
    checkError(
      exception = intercept[AnalysisException] {
        df6.selectExpr("array_prepend(a, b)")
      },
      errorClass = "DATATYPE_MISMATCH.UNEXPECTED_INPUT_TYPE",
      parameters = Map(
        "sqlExpr" -> "\"array_prepend(a, b)\"",
    ...
    val df7 = Seq((Array[Int](3, 2, 5, 1, 2), 3d)).toDF("a", "b")
    checkAnswer(df7.select(array_prepend(col("a"), col("b"))),
      Seq(Row(Seq(3d, 3d, 2d, 5d, 1d, 2d))))

    val df8 = Seq((Array[Double](3d, 2d, 5d, 1d, 2d), 3)).toDF("a", "b")
    checkAnswer(df8.select(array_prepend(col("a"), col("b"))),
      Seq(Row(Seq(3d, 3d, 2d, 5d, 1d, 2d))))

    val df9 = spark.sql("SELECT array(1, 2, null) as a, CAST(null AS INT) as b")
    checkAnswer(df9.selectExpr("array_prepend(a, b)"),
      Seq(Row(null))
    )
    ...
    checkAnswer(df10.selectExpr("array_prepend(a, b)"),
      Seq(Row(null))
    )
  }
}
