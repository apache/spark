...
class DataFrameFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("DataFrame function and SQL function parity") {
    ...
    val excludedDataFrameFunctions = Set(
      ...
      "expr",
      "lit",
      "negate", // first class functionality in SQL
      "countDistinct",
      "count_distinct", // equivalent to count(distinct foo)
      "sum_distinct", // equivalent to sum(distinct foo)
      "typedLit",
      "typedlit", // Scala only
      "udaf",
      "udf", // create function statement in sql
    ...
    val expectedOnlyDataFrameFunctions = Set(
      ...
      "hours",
      "months",
      "years", // Datasource v2 partition transformations
    ...
    assert(
      onlyDataFrameFunctions === expectedOnlyDataFrameFunctions,
      "symmetric difference is: "
        + onlyDataFrameFunctions
          .union(expectedOnlyDataFrameFunctions)
  ...
  test("struct with column name") {
    ...
    val expectedType = StructType(
      Seq(StructField("a", IntegerType, nullable = false), StructField("b", StringType)))
    assert(row.schema(0).dataType === expectedType)
  ...
  test("struct with column expression") {
    ...
    val expectedType = StructType(
      Seq(StructField("c", IntegerType, nullable = false), StructField("b", StringType)))
    assert(row.schema(0).dataType === expectedType)
  ...
  test("struct with column expression to be automatically named") {
    ...
    val expectedType = StructType(
      Seq(StructField("col1", IntegerType, nullable = false), StructField("b", StringType)))
      ...
      withSQLConf(SQLConf.ALWAYS_INLINE_COMMON_EXPR.key -> alwaysInlineCommonExpr.toString) {
        Seq(
          ...
          "SELECT NULLIF(id, 1), COUNT(*)" +
            ...
            "HAVING COUNT(*) > 1" -> Seq.empty[Row]).foreach { case (sqlText, expected) =>
          checkAnswer(sql(sqlText), expected)
        }
        ...
        checkError(
          ...
          parameters = Map("expression" -> "\"id\"", "expressionAnyValue" -> "\"any_value(id)\""))
      }
    }
  }
  ...
  test("randstr function") {
    withTable("t") {
      ...
      checkAnswer(df.select(randstr(lit(5)).alias("x")).select(length(col("x"))), Seq(Row(5)))
    }
  ...
  test("misc aes function") {
    ...
    Seq((key32, encryptedEcb, "ECB"), (key32, encryptedGcm, "GCM"), (key32, encryptedCbc, "CBC"))
      .foreach { case (key, encryptedText, mode) =>
        checkAnswer(
          df1.selectExpr(
  ...
  test("aes IV test function") {
    ...
    val df1 = Seq("Spark").toDF()
    Seq((key32, encryptedGcm, "GCM", gcmIv), (key32, encryptedCbc, "CBC", cbcIv)).foreach {
      case (key, ciphertext, mode, iv) =>
        checkAnswer(
          df1.selectExpr(
            s"cast(aes_decrypt(unbase64('$ciphertext'), " +
  ...
  test("aes IV and AAD test function") {
    ...
    val df1 = Seq("Spark").toDF()
    Seq((key32, encryptedGcm, "GCM", gcmIv, aad)).foreach {
  ...
  test("misc aes ECB function") {
    ...
    Seq(("value16", key16), ("value24", key24), ("value32", key32)).foreach {
      case (colName, key) =>
        checkAnswer(
        ...
        checkAnswer(
          df2.selectExpr(
            s"cast(aes_decrypt(unbase64($colName), binary('$key'), 'ECB') as string)"),
          Seq(Row("Spark"), Row("")))
    }
  ...
  test("conditional function: least") {
    checkAnswer(testData2.select(least(lit(-1), lit(0), col("a"), col("b"))).limit(1), Row(-1))
    checkAnswer(
  ...
  test("conditional function: greatest") {
    checkAnswer(testData2.select(greatest(lit(2), lit(3), col("a"), col("b"))).limit(1), Row(3))
    checkAnswer(
  ...
  test("array_sort with lambda functions") {
    ...
    val df4 =
      Seq((Array[Array[Int]](Array(2, 3, 1), Array(4, 2, 1, 4), Array(1, 2)), "x")).toDF("a", "b")
    checkAnswer(
      df4.select(array_sort(col("a"), (x, y) => call_udf("fAsc", size(x), size(y)))),
      Seq(Row(Seq[Seq[Int]](Seq(1, 2), Seq(2, 3, 1), Seq(4, 2, 1, 4)))))
  ...
  test("SPARK-38130: array_sort with lambda of non-orderable items") {
    val df6 = Seq((Array[Map[String, Int]](Map("a" -> 1), Map("b" -> 2, "c" -> 3), Map()), "x"))
      .toDF("a", "b")
    checkAnswer(
      df6.select(array_sort(col("a"), (x, y) => size(x) - size(y))),
      Seq(Row(Seq[Map[String, Int]](Map(), Map("a" -> 1), Map("b" -> 2, "c" -> 3)))))
  ...
  test("sort_array/array_sort functions") {
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"STRING\"",
        "requiredType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "array_sort(a)", start = 0, stop = 12))
    // scalastyle:on line.size.limit
  }

  def testSizeOfArray(sizeOfNull: Any): Unit = {
    val df =
      Seq((Seq[Int](1, 2), "x"), (Seq[Int](), "y"), (Seq[Int](1, 2, 3), "z"), (null, "empty"))
        .toDF("a", "b")
  ...
  test("dataframe arrays_zip function") {
    ...
    val df5 =
      Seq((Seq(-1), Seq(null), Seq(), Seq(null, null))).toDF("val1", "val2", "val3", "val4")
    ...
    val expectedValue6 =
      Row(Seq(Row(192.toByte, 1.1, null, null), Row(256.toByte, null, null, null)))
    ...
    val expectedValue7 = Row(Seq(Row(Seq(1, 2, 3), 1.1), Row(Seq(4, 5), 2.2)))
    checkAnswer(df7.select(arrays_zip($"v1", $"v2")), expectedValue7)
    checkAnswer(df7.selectExpr("arrays_zip(v1, v2)"), expectedValue7)

    val expectedValue8 = Row(Seq(Row(Array[Byte](1.toByte, 5.toByte), null)))
  ...
  testWithWholeStageCodegenOnAndOff(
    "SPARK-24633: arrays_zip splits input " +
      "processing correctly") { _ =>
    val df = spark.range(1)
    val exprs = (0 to 5).map(x => array($"id" + lit(x)))
    checkAnswer(df.select(arrays_zip(exprs: _*)), Row(Seq(Row(0, 1, 2, 3, 4, 5))))
  }
  ...
  test("SPARK-40292: arrays_zip should retain field names in nested structs") {
    ...
    val fieldNames = res.schema.head.dataType
  ...
  test("map_keys/map_values function") {
    val df = Seq(
      ...
      (Map[Int, Int](), "y"),
      (Map[Int, Int](1 -> 100, 2 -> 200, 3 -> 300), "z")).toDF("a", "b")
    checkAnswer(
  ...
  test("map_entries") {
    ...
    val idf = Seq(Map[Int, Int](1 -> 100, 2 -> 200, 3 -> 300), Map[Int, Int](), null).toDF("m")
    val iExpected =
      Seq(Row(Seq(Row(1, 100), Row(2, 200), Row(3, 300))), Row(Seq.empty), Row(null))
    ...
    def testPrimitiveType(): Unit = {
      ...
      checkAnswer(
        idf.selectExpr("map_entries(map(1, null, 2, null))"),
  ...
  test("map_contains_key function") {
    ...
    checkError(
      ...
      parameters =
        Map("sqlExpr" -> "\"map_contains_key(a, NULL)\"", "functionName" -> "`map_contains_key`"),
      context = ExpectedContext(fragment = "map_contains_key(a, null)", start = 0, stop = 24))
  }
  ...
  test("map_concat function") {
    val df1 = Seq(
      ...
      (null, Map[Int, Int](3 -> 300, 4 -> 400))).toDF("map1", "map2")
    ...
    val expected1b = Seq(Row(Map(1 -> 100, 2 -> 200)), Row(Map(1 -> 100, 2 -> 200)), Row(null))
    ...
    val df2 = Seq(
      (
        ...
        Map[String, Int]("3" -> 300, "4" -> 400))).toDF("map1", "map2")
  ...
  test("map_from_entries function") {
    ...
    val idf =
      Seq(Seq((1, 10), (2, 20), (3, 10)), Seq((1, 10), null, (2, 20)), Seq.empty, null).toDF("a")
    val iExpected = Seq(Row(Map(1 -> 10, 2 -> 20, 3 -> 10)), Row(null), Row(Map.empty), Row(null))

    def testPrimitiveType(): Unit = {
      checkAnswer(idf.select(map_from_entries($"a")), iExpected)
      checkAnswer(idf.selectExpr("map_from_entries(a)"), iExpected)
      checkAnswer(
        idf.selectExpr("map_from_entries(array(struct(1, null), struct(2, null)))"),
  ...
  test("array contains function") {
    val df = Seq((Seq[Int](1, 2), "x", 1), (Seq[Int](), "x", 1)).toDF("a", "b", "c")

    // Simple test cases
    checkAnswer(df.select(array_contains(df("a"), 1)), Seq(Row(true), Row(false)))
    checkAnswer(df.selectExpr("array_contains(a, 1)"), Seq(Row(true), Row(false)))
    checkAnswer(df.select(array_contains(df("a"), df("c"))), Seq(Row(true), Row(false)))
    checkAnswer(df.selectExpr("array_contains(a, c)"), Seq(Row(true), Row(false)))
    ...
    checkError(
      ...
      condition = "DATATYPE_MISMATCH.NULL_TYPE",
      parameters =
        Map("sqlExpr" -> "\"array_contains(a, NULL)\"", "functionName" -> "`array_contains`"),
      context = ExpectedContext(
    ...
    checkError(
      ...
      condition = "DATATYPE_MISMATCH.NULL_TYPE",
      parameters =
        Map("sqlExpr" -> "\"array_contains(NULL, 1)\"", "functionName" -> "`array_contains`"),
    ...
    checkAnswer(OneRowRelation().selectExpr("array_contains(array(1), 1.23D)"), Seq(Row(false)))

    checkAnswer(OneRowRelation().selectExpr("array_contains(array(1), 1.0D)"), Seq(Row(true)))

    checkAnswer(OneRowRelation().selectExpr("array_contains(array(1.0D), 1)"), Seq(Row(true)))

    checkAnswer(OneRowRelation().selectExpr("array_contains(array(1.23D), 1)"), Seq(Row(false)))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "leftType" -> "\"ARRAY<INT>\"",
        "rightType" -> "\"STRING\""),
      queryContext = Array(ExpectedContext("", "", 0, 30, "array_contains(array(1), 'foo')")))
  ...
  test("SPARK-29600: ArrayContains function may return incorrect result for DecimalType") {
    checkAnswer(sql("select array_contains(array(1.10), 1.1)"), Seq(Row(true)))

    checkAnswer(sql("SELECT array_contains(array(1.1), 1.10)"), Seq(Row(true)))

    checkAnswer(sql("SELECT array_contains(array(1.11), 1.1)"), Seq(Row(false)))
  }
  ...
  test("arrays_overlap function") {
    val df = Seq(
      ...
      (Seq[Option[Int]](Some(3), Some(2)), Seq[Option[Int]](Some(1), Some(2)))).toDF("a", "b")
    ...
    checkError(
      ...
      queryContext = Array(
        ExpectedContext("", "", 7, 58, "arrays_overlap(array(1, 2, 3), array('a', 'b', 'c'))")))
    ...
    checkError(
      ...
      queryContext =
        Array(ExpectedContext("", "", 7, 42, "arrays_overlap(map(1, 2), map(3, 4))")))
  }
  ...
  test("array_join function") {
    val df = Seq(
      (Seq[String]("a", "b"), ","),
      (Seq[String]("a", null, "b"), ","),
      (Seq.empty[String], ",")).toDF("x", "delimiter")

    checkAnswer(df.select(array_join(df("x"), ";")), Seq(Row("a;b"), Row("a;b"), Row("")))
    checkAnswer(
      df.select(array_join(df("x"), ";", "NULL")),
      Seq(Row("a;b"), Row("a;NULL;b"), Row("")))
    checkAnswer(df.selectExpr("array_join(x, delimiter)"), Seq(Row("a,b"), Row("a,b"), Row("")))
    checkAnswer(
    ...
    checkAnswer(idf.select(array_join(idf("x"), ", ")), Seq(Row("1, 2, 3")))
    checkAnswer(idf.selectExpr("array_join(x, ', ')"), Seq(Row("1, 2, 3")))
    checkError(
      exception = intercept[AnalysisException] {
  ...
  test("array_min function") {
    val df = Seq(
      ...
      Seq[Option[Int]](None),
      Seq[Option[Int]](None, Some(1), Some(-100))).toDF("a")
  ...
  test("array_size function") {
    val df = Seq(
      ...
      Seq.empty[Option[Int]],
      null,
      Seq[Option[Int]](None, Some(1))).toDF("a")
  ...
  test("sequence") {
    checkAnswer(
      Seq((-2, 2)).toDF().select(sequence($"_1", $"_2")),
      Seq(Row(Array(-2, -1, 0, 1, 2))))
    checkAnswer(
      Seq((7, 2, -2)).toDF().select(sequence($"_1", $"_2", $"_3")),
      Seq(Row(Array(7, 5, 3))))
  ...
  test("reverse function - array for primitive type not containing null") {
    val idfNotContainsNull = Seq(Seq(1, 9, 8, 7), Seq(5, 8, 9, 7, 2), Seq.empty, null).toDF("i")
  ...
  test("reverse function - array for primitive type containing null") {
    val idfContainsNull =
      Seq[Seq[Integer]](Seq(1, 9, 8, null, 7), Seq(null, 5, 8, 9, 7, 2), Seq.empty, null)
        .toDF("i")
  ...
  test("reverse function - array for non-primitive type") {
    val sdf = Seq(Seq("c", "a", "b"), Seq("b", null, "c", null), Seq.empty, null).toDF("s")
  ...
  test("array position function") {
    ...
    checkAnswer(df.select(array_position(df("a"), 1)), Seq(Row(1L), Row(0L)))
    checkAnswer(df.selectExpr("array_position(a, 1)"), Seq(Row(1L), Row(0L)))
    checkAnswer(df.selectExpr("array_position(a, c)"), Seq(Row(1L), Row(0L)))
    checkAnswer(df.select(array_position(df("a"), df("c"))), Seq(Row(1L), Row(0L)))
    checkAnswer(df.select(array_position(df("a"), null)), Seq(Row(null), Row(null)))
    checkAnswer(df.selectExpr("array_position(a, null)"), Seq(Row(null), Row(null)))

    checkAnswer(OneRowRelation().selectExpr("array_position(array(1), 1.23D)"), Seq(Row(0L)))

    checkAnswer(OneRowRelation().selectExpr("array_position(array(1), 1.0D)"), Seq(Row(1L)))

    checkAnswer(OneRowRelation().selectExpr("array_position(array(1.D), 1)"), Seq(Row(1L)))

    checkAnswer(OneRowRelation().selectExpr("array_position(array(1.23D), 1)"), Seq(Row(0L)))
    ...
    checkError(
      ...
      condition = "DATATYPE_MISMATCH.NULL_TYPE",
      parameters =
        Map("sqlExpr" -> "\"array_position(_1, _2)\"", "functionName" -> "`array_position`"),
  ...
  test("element_at function") {
    val df = Seq((Seq[String]("1", "2", "3"), 1), (Seq[String](null, ""), -1), (Seq[String](), 2))
      .toDF("a", "b")

    intercept[Exception] {
      checkAnswer(df.select(element_at(df("a"), 0)), Seq(Row(null), Row(null), Row(null)))
    }.getMessage.contains("SQL array indices start at 1")
    intercept[Exception] {
      checkAnswer(df.select(element_at(df("a"), 1.1)), Seq(Row(null), Row(null), Row(null)))
    }
    if (!conf.ansiEnabled) {
      checkAnswer(df.select(element_at(df("a"), 4)), Seq(Row(null), Row(null), Row(null)))
      checkAnswer(df.select(element_at(df("a"), df("b"))), Seq(Row("1"), Row(""), Row(null)))
      checkAnswer(df.selectExpr("element_at(a, b)"), Seq(Row("1"), Row(""), Row(null)))

      checkAnswer(df.select(element_at(df("a"), 1)), Seq(Row("1"), Row(null), Row(null)))
      checkAnswer(df.select(element_at(df("a"), -1)), Seq(Row("3"), Row(""), Row(null)))

      checkAnswer(df.selectExpr("element_at(a, 4)"), Seq(Row(null), Row(null), Row(null)))

      checkAnswer(df.selectExpr("element_at(a, 1)"), Seq(Row("1"), Row(null), Row(null)))
      checkAnswer(df.selectExpr("element_at(a, -1)"), Seq(Row("3"), Row(""), Row(null)))
    }
    ...
    checkAnswer(OneRowRelation().selectExpr("element_at(array(2, 1), 2S)"), Seq(Row(1)))

    checkAnswer(OneRowRelation().selectExpr("element_at(array('a', 'b'), 1Y)"), Seq(Row("a")))

    checkAnswer(OneRowRelation().selectExpr("element_at(array(1, 2, 3), 3)"), Seq(Row(3)))
    ...
    checkAnswer(OneRowRelation().selectExpr("element_at(map(1, 'a', 2, 'b'), 2Y)"), Seq(Row("b")))

    checkAnswer(OneRowRelation().selectExpr("element_at(map(1, 'a', 2, 'b'), 1S)"), Seq(Row("a")))

    checkAnswer(OneRowRelation().selectExpr("element_at(map(1, 'a', 2, 'b'), 2)"), Seq(Row("b")))

    checkAnswer(OneRowRelation().selectExpr("element_at(map(1, 'a', 2, 'b'), 2L)"), Seq(Row("b")))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "rightType" -> "\"STRING\""),
      queryContext =
        Array(ExpectedContext("", "", 0, 35, "element_at(map(1, 'a', 2, 'b'), '1')")))
  }
  ...
  test("SPARK-40214: get function") {
    ...
    checkAnswer(df.select(get(df("a"), lit(-1))), Seq(Row(null), Row(null), Row(null), Row(null)))
    checkAnswer(df.select(get(df("a"), lit(0))), Seq(Row("1"), Row(null), Row(null), Row(null)))
    checkAnswer(df.select(get(df("a"), lit(1))), Seq(Row("2"), Row(""), Row(null), Row(null)))
    checkAnswer(df.select(get(df("a"), lit(2))), Seq(Row("3"), Row(null), Row(null), Row(null)))
    checkAnswer(df.select(get(df("a"), lit(3))), Seq(Row(null), Row(null), Row(null), Row(null)))
    checkAnswer(df.select(get(df("a"), df("b"))), Seq(Row("3"), Row(""), Row(null), Row(null)))
    checkAnswer(
  ...
  test("array_union functions") {
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "leftType" -> "\"VOID\"",
        "rightType" -> "\"ARRAY<STRING>\""),
      context = ExpectedContext(fragment = "array_union(a, b)", start = 0, stop = 16))
  ...
  test("concat function - arrays") {
    ...
    // Null test cases
    def nullTest(): Unit = {
      checkAnswer(df.select(concat($"i1", $"in")), Seq(Row(null), Row(null)))
      checkAnswer(df.select(concat($"in", $"i1")), Seq(Row(null), Row(null)))
      checkAnswer(df.select(concat($"s1", $"sn")), Seq(Row(null), Row(null)))
      checkAnswer(df.select(concat($"sn", $"s1")), Seq(Row(null), Row(null)))
    }
  ...
  test("flatten function") {
    ...
    checkError(
      ...
      context =
        ExpectedContext(fragment = "flatten", callSitePattern = getCurrentClassCallSitePattern))
    checkError(
      exception = intercept[AnalysisException] {
  ...
  test("array_repeat function") {
    val strDF = Seq(("hi", 2), (null, 2)).toDF("a", "b")

    val strDFTwiceResult = Seq(Row(Seq("hi", "hi")), Row(Seq(null, null)))
    ...
    val intDF = {
      ...
      val data = Seq(Row(3, 2), Row(null, 2))
      spark.createDataFrame(spark.sparkContext.parallelize(data), schema)
    }

    val intDFTwiceResult = Seq(Row(Seq(3, 3)), Row(Seq(null, null)))
    ...
    val nullCountDF = {
      ...
      val data = Seq(Row("hi", null), Row(null, null))
      spark.createDataFrame(spark.sparkContext.parallelize(data), schema)
    }

    def testNull(): Unit = {
      checkAnswer(nullCountDF.select(array_repeat($"a", $"b")), Seq(Row(null), Row(null)))
    }
  ...
  test("SPARK-41233: array prepend") {
    ...
    val df2 = Seq(
      ...
      (null, null)).toDF("a", "b")
    checkAnswer(
      df2.selectExpr("array_prepend(a, b)"),
      Seq(Row(Seq("d", "a", "b", "c")), Row(null), Row(Seq(null, "x", "y", "z")), Row(null)))
    val dataA =
      Seq[Array[Byte]](Array[Byte](5, 6), Array[Byte](1, 2), Array[Byte](1, 2), Array[Byte](5, 6))
  ...
  test("array remove") {
    ...
    checkAnswer(
      ...
      Seq(Row(Seq(1, 3)), Row(Seq.empty[Int]), Row(null)))
    ...
    checkAnswer(
      df.selectExpr("array_remove(a, 2)", "array_remove(b, \"a\")", "array_remove(c, \"\")"),
      Seq(
        Row(Seq(1, 3), Seq("b", "c"), Seq.empty[String]),
        Row(Seq.empty[Int], Seq.empty[String], Seq.empty[String]),
        Row(null, null, null)))
  ...
  test("array_except functions") {
    ...
    val df10 = Seq(
      ...
      (Array[Integer](1, null), Array[Integer](2, null))).toDF("a", "b")
  ...
  test("array_insert functions") {
    ...
    checkError(
      ...
      parameters = Map.empty,
      context = ExpectedContext(fragment = "array_insert(a, b, c)", start = 0, stop = 20))
  ...
  test("transform function - array for primitive type not containing null") {
    val df = Seq(Seq(1, 9, 8, 7), Seq(5, 8, 9, 7, 2), Seq.empty, null).toDF("i")

    def testArrayOfPrimitiveTypeNotContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("transform(i, x -> x + 1)"),
        Seq(Row(Seq(2, 10, 9, 8)), Row(Seq(6, 9, 10, 8, 3)), Row(Seq.empty), Row(null)))
      checkAnswer(
        df.selectExpr("transform(i, (x, i) -> x + i)"),
        Seq(Row(Seq(1, 10, 10, 10)), Row(Seq(5, 9, 11, 10, 6)), Row(Seq.empty), Row(null)))
      checkAnswer(
  ...
  test("transform function - array for primitive type containing null") {
    val df = Seq[Seq[Integer]](Seq(1, 9, 8, null, 7), Seq(5, null, 8, 9, 7, 2), Seq.empty, null)
      .toDF("i")
  ...
  test("transform function - array for non-primitive type") {
    val df = Seq(Seq("c", "a", "b"), Seq("b", null, "c", null), Seq.empty, null).toDF("s")

    def testNonPrimitiveType(): Unit = {
      checkAnswer(
        df.selectExpr("transform(s, x -> concat(x, x))"),
        Seq(
          Row(Seq("cc", "aa", "bb")),
          ...
          Row(null)))
      checkAnswer(
        df.selectExpr("transform(s, (x, i) -> concat(x, i))"),
        Seq(
          Row(Seq("c0", "a1", "b2")),
          ...
          Row(null)))
      checkAnswer(
        df.select(transform(col("s"), x => concat(x, x))),
        Seq(
          Row(Seq("cc", "aa", "bb")),
          Row(Seq("bb", null, "cc", null)),
          Row(Seq.empty),
          Row(null)))
      checkAnswer(
        df.select(transform(col("s"), (x, i) => concat(x, i))),
        Seq(
          Row(Seq("c0", "a1", "b2")),
          Row(Seq("b0", null, "c2", null)),
          Row(Seq.empty),
  ...
  test("transform function - special cases") {
    val df = Seq(Seq("c", "a", "b"), Seq("b", null, "c", null), Seq.empty, null).toDF("arg")

    def testSpecialCases(): Unit = {
      checkAnswer(
        df.selectExpr("transform(arg, arg -> arg)"),
        Seq(Row(Seq("c", "a", "b")), Row(Seq("b", null, "c", null)), Row(Seq.empty), Row(null)))
      checkAnswer(
        df.selectExpr("transform(arg, arg)"),
        Seq(
          ...
          Row(null)))
      checkAnswer(
        df.selectExpr("transform(arg, x -> concat(arg, array(x)))"),
        Seq(
          ...
          Row(null)))
      checkAnswer(
        df.select(transform(col("arg"), arg => arg)),
        Seq(Row(Seq("c", "a", "b")), Row(Seq("b", null, "c", null)), Row(Seq.empty), Row(null)))
      checkAnswer(
        df.select(transform(col("arg"), _ => col("arg"))),
        Seq(
          Row(Seq(Seq("c", "a", "b"), Seq("c", "a", "b"), Seq("c", "a", "b"))),
          Row(
            Seq(
          ...
          Row(null)))
      checkAnswer(
        df.select(transform(col("arg"), x => concat(col("arg"), array(x)))),
        Seq(
          Row(Seq(Seq("c", "a", "b", "c"), Seq("c", "a", "b", "a"), Seq("c", "a", "b", "b"))),
          Row(
            Seq(
  ...
  test("transform function - invalid") {
    val df =
      Seq((Seq("c", "a", "b"), 1), (Seq("b", null, "c", null), 2), (Seq.empty, 3), (null, 4))
        .toDF("s", "i")
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "(x, y, z) -> x + y + z", start = 13, stop = 34))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"INT\"",
        "requiredType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "transform(i, x -> x)", start = 0, stop = 19))

    checkError(
      exception = intercept[AnalysisException](df.selectExpr("transform(a, x -> x)")),
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`a`", "proposal" -> "`i`, `s`"),
      context = ExpectedContext(fragment = "a", start = 10, stop = 10))
  }
  ...
  test("map_filter") {
    ...
    checkAnswer(
      dfInts.selectExpr("map_filter(m, (k, v) -> k * 10 = v)", "map_filter(m, (k, v) -> k = -v)"),
      Seq(
    ...
    checkAnswer(
      dfComplex.selectExpr(
        "map_filter(m, (k, v) -> k = v[0])",
        "map_filter(m, (k, v) -> k = size(v))"),
      Seq(
    ...
    // Invalid use cases
    val df = Seq((Map(1 -> "a"), 1), (Map.empty[Int, String], 2), (null, 3)).toDF("s", "i")
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "(x, y, z) -> x + y + z", start = 14, stop = 35))
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "x -> x", start = 14, stop = 19))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "requiredType" -> "\"MAP\""),
      context =
        ExpectedContext(fragment = "map_filter(i, (k, v) -> k > v)", start = 0, stop = 29))
    ...
    checkError(
      exception = intercept[AnalysisException](df.selectExpr("map_filter(a, (k, v) -> k > v)")),
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`a`", "proposal" -> "`i`, `s`"),
      context = ExpectedContext(fragment = "a", start = 11, stop = 11))
  }
  ...
  test("filter function - array for primitive type not containing null") {
    ...
    def testArrayOfPrimitiveTypeNotContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("filter(i, x -> x % 2 == 0)"),
        Seq(Row(Seq(8)), Row(Seq(8, 2)), Row(Seq.empty), Row(null)))
      checkAnswer(
        df.select(filter(col("i"), _ % 2 === 0)),
        Seq(Row(Seq(8)), Row(Seq(8, 2)), Row(Seq.empty), Row(null)))
    }
  ...
  test("filter function - array for non-primitive type") {
    ...
    def testNonPrimitiveType(): Unit = {
      checkAnswer(
        df.selectExpr("filter(s, x -> x is not null)"),
        Seq(Row(Seq("c", "a", "b")), Row(Seq("b", "c")), Row(Seq.empty), Row(null)))
      checkAnswer(
        df.select(filter(col("s"), x => x.isNotNull)),
        Seq(Row(Seq("c", "a", "b")), Row(Seq("b", "c")), Row(Seq.empty), Row(null)))
    }
  ...
  test("filter function - index argument") {
    ...
    def testIndexArgument(): Unit = {
      checkAnswer(
        df.selectExpr("filter(s, (x, i) -> i % 2 == 0)"),
        Seq(Row(Seq("c", "b")), Row(Seq("b", "c")), Row(Seq.empty), Row(null)))
      checkAnswer(
  ...
  test("filter function - invalid") {
    ...
    checkError(
      ...
      condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
      parameters = Map("expectedNumArgs" -> "3", "actualNumArgs" -> "1"),
      context = ExpectedContext(fragment = "(x, y, z) -> x + y", start = 10, stop = 27))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"INT\"",
        "requiredType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "filter(i, x -> x)", start = 0, stop = 16))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"STRING\"",
        "requiredType" -> "\"BOOLEAN\""),
      ExpectedContext(fragment = "filter(s, x -> x)", start = 0, stop = 16))
    ...
    checkError(
      exception = intercept[AnalysisException](df.selectExpr("filter(a, x -> x)")),
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`a`", "proposal" -> "`i`, `s`"),
      context = ExpectedContext(fragment = "a", start = 7, stop = 7))
  }

  test("exists function - array for primitive type not containing null") {
    val df = Seq(Seq(1, 9, 8, 7), Seq(5, 9, 7), Seq.empty, null).toDF("i")

    def testArrayOfPrimitiveTypeNotContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("exists(i, x -> x % 2 == 0)"),
        Seq(Row(true), Row(false), Row(false), Row(null)))
      checkAnswer(
  ...
  test("exists function - array for primitive type containing null") {
    ...
    def testArrayOfPrimitiveTypeContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("exists(i, x -> x % 2 == 0)"),
        Seq(Row(true), Row(false), Row(null), Row(false), Row(null)))
      checkAnswer(
        df.select(exists(col("i"), _ % 2 === 0)),
        Seq(Row(true), Row(false), Row(null), Row(false), Row(null)))
    }
  ...
  test("exists function - array for non-primitive type") {
    ...
    def testNonPrimitiveType(): Unit = {
      checkAnswer(
        df.selectExpr("exists(s, x -> x is null)"),
        Seq(Row(false), Row(true), Row(false), Row(null)))
      checkAnswer(
        df.select(exists(col("s"), x => x.isNull)),
        Seq(Row(false), Row(true), Row(false), Row(null)))
    }
  ...
  test("exists function - invalid") {
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "(x, y) -> x + y", start = 10, stop = 24))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"INT\"",
        "requiredType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "exists(i, x -> x)", start = 0, stop = 16))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"STRING\"",
        "requiredType" -> "\"BOOLEAN\""),
      context = ExpectedContext(fragment = "exists(s, x -> x)", start = 0, stop = 16))
  ...
  test("forall function - array for primitive type not containing null") {
    val df = Seq(Seq(1, 9, 8, 7), Seq(2, 4, 6), Seq.empty, null).toDF("i")

    def testArrayOfPrimitiveTypeNotContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("forall(i, x -> x % 2 == 0)"),
        Seq(Row(false), Row(true), Row(true), Row(null)))
      checkAnswer(
  ...
  test("forall function - array for primitive type containing null") {
    ...
    def testArrayOfPrimitiveTypeContainsNull(): Unit = {
      checkAnswer(
        df.selectExpr("forall(i, x -> x % 2 == 0 or x is null)"),
        Seq(Row(false), Row(true), Row(true), Row(true), Row(null)))
      checkAnswer(
        df.select(forall(col("i"), x => (x % 2 === 0) || x.isNull)),
        Seq(Row(false), Row(true), Row(true), Row(true), Row(null)))
      checkAnswer(
        df.selectExpr("forall(i, x -> x % 2 == 0)"),
        Seq(Row(false), Row(null), Row(true), Row(true), Row(null)))
      checkAnswer(
        df.select(forall(col("i"), x => x % 2 === 0)),
        Seq(Row(false), Row(null), Row(true), Row(true), Row(null)))
    }
  ...
  test("forall function - array for non-primitive type") {
    val df =
      Seq(Seq("c", "a", "b"), Seq[String](null, null, null, null), Seq.empty, null).toDF("s")

    def testNonPrimitiveType(): Unit = {
      checkAnswer(
        df.selectExpr("forall(s, x -> x is null)"),
        Seq(Row(false), Row(true), Row(true), Row(null)))
      checkAnswer(
        df.select(forall(col("s"), _.isNull)),
        Seq(Row(false), Row(true), Row(true), Row(null)))
    }
  ...
  test("forall function - invalid") {
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"INT\"",
        "requiredType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "forall(i, x -> x)", start = 0, stop = 16))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "inputType" -> "\"STRING\"",
        "requiredType" -> "\"BOOLEAN\""),
      context = ExpectedContext(fragment = "forall(s, x -> x)", start = 0, stop = 16))
    ...
    def testArrayOfPrimitiveTypeNotContainsNull(): Unit = {
      Seq("aggregate", "reduce").foreach { agg =>
        checkAnswer(
          ...
          Seq(Row(25), Row(31), Row(0), Row(null)))
        checkAnswer(
          df.selectExpr(s"$agg(i, 0, (acc, x) -> acc + x, acc -> acc * 10)"),
          Seq(Row(250), Row(310), Row(0), Row(null)))
      }
      checkAnswer(
  ...
  test("aggregate function - array for primitive type containing null") {
    val df =
      Seq[Seq[Integer]](Seq(1, 9, 8, 7), Seq(5, null, 8, 9, 7, 2), Seq.empty, null).toDF("i")

    def testArrayOfPrimitiveTypeContainsNull(): Unit = {
      Seq("aggregate", "reduce").foreach { agg =>
        checkAnswer(
          df.selectExpr(s"$agg(i, 0, (acc, x) -> acc + x)"),
          Seq(Row(25), Row(null), Row(0), Row(null)))
        checkAnswer(
          df.selectExpr(s"$agg(i, 0, (acc, x) -> acc + x, acc -> coalesce(acc, 0) * 10)"),
          Seq(Row(250), Row(0), Row(0), Row(null)))
      }
      checkAnswer(
        df.select(aggregate(col("i"), lit(0), (acc, x) => acc + x)),
        Seq(Row(25), Row(null), Row(0), Row(null)))
    ...
    def testNonPrimitiveType(): Unit = {
      Seq("aggregate", "reduce").foreach { agg =>
        checkAnswer(
          ...
          Seq(Row("acab"), Row(null), Row("c"), Row(null)))
        checkAnswer(
          df.selectExpr(s"$agg(ss, s, (acc, x) -> concat(acc, x), acc -> coalesce(acc , ''))"),
          Seq(Row("acab"), Row(""), Row("c"), Row(null)))
      }
      checkAnswer(
        df.select(aggregate(col("ss"), col("s"), (acc, x) => concat(acc, x))),
        Seq(Row("acab"), Row(null), Row("c"), Row(null)))
      checkAnswer(
        df.select(
          aggregate(
  ...
  test("aggregate function - invalid") {
    ...
    Seq(("aggregate", 17, 32), ("reduce", 14, 29)).foreach {
      case (agg, startIndex1, startIndex2) =>
        checkError(
          ...
          parameters = Map("expectedNumArgs" -> "1", "actualNumArgs" -> "2"),
          context =
            ExpectedContext(fragment = "x -> x", start = startIndex1, stop = startIndex1 + 5))
    ...
    Seq("aggregate", "reduce").foreach { agg =>
      checkError(
        exception = intercept[AnalysisException](df.selectExpr(s"$agg(a, 0, (acc, x) -> x)")),
        condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
        sqlState = None,
        parameters = Map("objectName" -> "`a`", "proposal" -> "`i`, `s`"),
        context = ExpectedContext(fragment = "a", start = agg.length + 1, stop = agg.length + 1))
    }
  }
  ...
  test("map_zip_with function - map of non-primitive types") {
    val df = Seq(
      ...
      (Map("a" -> "d"), null)).toDF("m1", "m2")

    checkAnswer(
      df.selectExpr("map_zip_with(m1, m2, (k, v1, v2) -> (v1, v2))"),
      Seq(
    ...
    checkAnswer(
      df.select(map_zip_with(col("m1"), col("m2"), (k, v1, v2) => struct(v1, v2))),
      Seq(
        Row(Map("z" -> Row("a", "c"), "y" -> Row("b", null), "x" -> Row("c", "a"))),
        Row(Map("b" -> Row("a", null), "c" -> Row("d", "a"), "d" -> Row(null, "k"))),
        Row(Map("a" -> Row("d", null))),
  ...
  test("map_zip_with function - invalid") {
    val df = Seq((Map(1 -> 2), Map(1 -> "a"), Map("a" -> "b"), Map(Map(1 -> 2) -> 2), 1))
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "(x, y) -> x + y", start = 23, stop = 37))
  ...
  test("transform keys function - primitive data types") {
    val dfExample1 = Seq(Map[Int, Int](1 -> 1, 9 -> 9, 8 -> 8, 7 -> 7)).toDF("i")

    val dfExample2 = Seq(Map[Int, Double](1 -> 1.0, 2 -> 1.40, 3 -> 1.70)).toDF("j")

    val dfExample3 = Seq(Map[Int, Boolean](25 -> true, 26 -> false)).toDF("x")

    val dfExample4 = Seq(Map[Array[Int], Boolean](Array(1, 2) -> false)).toDF("y")
  ...
  test("transform keys function - Invalid lambda functions and exceptions") {
    val dfExample1 = Seq(Map[String, String]("a" -> null)).toDF("i")

    val dfExample2 = Seq(Seq(1, 2, 3, 4)).toDF("j")
    ...
    checkError(
      ...
      condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
      parameters = Map("expectedNumArgs" -> "1", "actualNumArgs" -> "2"),
      context = ExpectedContext(fragment = "k -> k", start = 18, stop = 23))
    ...
    checkError(
      ...
      condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
      parameters = Map("expectedNumArgs" -> "3", "actualNumArgs" -> "2"),
      context = ExpectedContext(fragment = "(k, v, x) -> k + 1", start = 18, stop = 35))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "requiredType" -> "\"MAP\""),
      context =
        ExpectedContext(fragment = "transform_keys(j, (k, v) -> k + 1)", start = 0, stop = 33))
  }

  test("transform values function - test primitive data types") {
    val dfExample1 = Seq(Map[Int, Int](1 -> 1, 9 -> 9, 8 -> 8, 7 -> 7)).toDF("i")

    val dfExample2 = Seq(Map[Boolean, String](false -> "abc", true -> "def")).toDF("x")

    val dfExample3 = Seq(Map[String, Int]("a" -> 1, "b" -> 2, "c" -> 3)).toDF("y")

    val dfExample4 = Seq(Map[Int, Double](1 -> 1.0, 2 -> 1.40, 3 -> 1.70)).toDF("z")

    val dfExample5 = Seq(Map[Int, Array[Int]](1 -> Array(1, 2))).toDF("c")

    def testMapOfPrimitiveTypesCombination(): Unit = {
      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> k + v)"),
        Seq(Row(Map(1 -> 2, 9 -> 18, 8 -> 16, 7 -> 14))))

      checkAnswer(
        dfExample2.selectExpr("transform_values(x, (k, v) -> if(k, v, CAST(k AS String)))"),
        Seq(Row(Map(false -> "false", true -> "def"))))

      checkAnswer(
        dfExample2.selectExpr("transform_values(x, (k, v) -> NOT k AND v = 'abc')"),
        Seq(Row(Map(false -> true, true -> false))))

      checkAnswer(
        dfExample3.selectExpr("transform_values(y, (k, v) -> v * v)"),
        Seq(Row(Map("a" -> 1, "b" -> 4, "c" -> 9))))

      checkAnswer(
        dfExample3.selectExpr("transform_values(y, (k, v) -> k || ':' || CAST(v as String))"),
      ...
      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => k + v)),
        Seq(Row(Map(1 -> 2, 9 -> 18, 8 -> 16, 7 -> 14))))
      ...
      checkAnswer(
        dfExample2.select(transform_values(col("x"), (k, v) => (!k) && v === "abc")),
        Seq(Row(Map(false -> true, true -> false))))

      checkAnswer(
        dfExample3.select(transform_values(col("y"), (k, v) => v * v)),
        Seq(Row(Map("a" -> 1, "b" -> 4, "c" -> 9))))
  ...
  test("transform values function - test empty") {
    val dfExample1 = Seq(Map.empty[Integer, Integer]).toDF("i")

    val dfExample2 = Seq(Map.empty[BigInt, String]).toDF("j")

    def testEmpty(): Unit = {
      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> NULL)"),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> k)"),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> v)"),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> 0)"),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> 'value')"),
        Seq(Row(Map.empty[Integer, String])))

      checkAnswer(
        dfExample1.selectExpr("transform_values(i, (k, v) -> true)"),
        Seq(Row(Map.empty[Integer, Boolean])))

      checkAnswer(
        dfExample2.selectExpr("transform_values(j, (k, v) -> k + cast(v as BIGINT))"),
        Seq(Row(Map.empty[BigInt, BigInt])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => lit(null).cast("int"))),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => k)),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => v)),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => lit(0))),
        Seq(Row(Map.empty[Integer, Integer])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => lit("value"))),
        Seq(Row(Map.empty[Integer, String])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => lit(true))),
        Seq(Row(Map.empty[Integer, Boolean])))

      checkAnswer(
        dfExample1.select(transform_values(col("i"), (k, v) => v.cast("bigint"))),
        Seq(Row(Map.empty[BigInt, BigInt])))
    }
  ...
  test("transform values function - test null values") {
    val dfExample1 = Seq(Map[Int, Integer](1 -> 1, 2 -> 2, 3 -> 3, 4 -> 4)).toDF("a")

    val dfExample2 = Seq(Map[Int, String](1 -> "a", 2 -> "b", 3 -> null)).toDF("b")

    def testNullValue(): Unit = {
      checkAnswer(
        dfExample1.selectExpr("transform_values(a, (k, v) -> null)"),
        Seq(Row(Map[Int, Integer](1 -> null, 2 -> null, 3 -> null, 4 -> null))))

      checkAnswer(
        dfExample2.selectExpr("transform_values(b, (k, v) -> IF(v IS NULL, k + 1, k + 2))"),
        Seq(Row(Map(1 -> 3, 2 -> 4, 3 -> 4))))

      checkAnswer(
        dfExample1.select(transform_values(col("a"), (k, v) => lit(null).cast("int"))),
        Seq(Row(Map[Int, Integer](1 -> null, 2 -> null, 3 -> null, 4 -> null))))
  ...
  test("transform values function - test invalid functions") {
    ...
    val dfExample2 = Seq(Map[String, String]("a" -> "b")).toDF("j")

    val dfExample3 = Seq(Seq(1, 2, 3, 4)).toDF("x")
    ...
    def testInvalidLambdaFunctions(): Unit = {
      ...
      checkError(
        ...
        condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
        parameters = Map("expectedNumArgs" -> "1", "actualNumArgs" -> "2"),
        context = ExpectedContext(fragment = "k -> k", start = 20, stop = 25))
      ...
      checkError(
        ...
        condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
        parameters = Map("expectedNumArgs" -> "3", "actualNumArgs" -> "2"),
        context = ExpectedContext(fragment = "(k, v, x) -> k + 1", start = 20, stop = 37))
  ...
  test("arrays zip_with function - for primitive types") {
    val df1 = Seq[(Seq[Integer], Seq[Integer])](
      ...
      (Seq(1, 2), Seq(3, 4)),
      (Seq.empty, Seq.empty),
      (null, null)).toDF("val1", "val2")
    val df2 =
      Seq[(Seq[Integer], Seq[Long])]((Seq(1, null, 3), Seq(1L, 2L)), (Seq(1, 2, 3), Seq(4L, 11L)))
        .toDF("val1", "val2")
    val expectedValue1 =
      Seq(Row(Seq(9005, 9007, 9009)), Row(Seq(4, 6)), Row(Seq.empty), Row(null))
  ...
  test("arrays zip_with function - invalid") {
    ...
    checkError(
      ...
      condition = "INVALID_LAMBDA_FUNCTION_CALL.NUM_ARGS_MISMATCH",
      parameters = Map("expectedNumArgs" -> "1", "actualNumArgs" -> "2"),
      context = ExpectedContext(fragment = "x -> x", start = 17, stop = 22))
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "(x, x) -> x", start = 17, stop = 27))
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "requiredType" -> "\"ARRAY\""),
      context =
        ExpectedContext(fragment = "zip_with(i, a2, (acc, x) -> x)", start = 0, stop = 29))
    ...
    checkError(
      exception = intercept[AnalysisException](df.selectExpr("zip_with(a1, a, (acc, x) -> x)")),
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`a`", "proposal" -> "`a1`, `a2`, `i`"),
      context = ExpectedContext(fragment = "a", start = 13, stop = 13))
  }
  ...
  private def assertValuesDoNotChangeAfterCoalesceOrUnion(v: Column): Unit = {
    ...
    for ((codegenFallback, wholeStage) <- Seq((true, false), (false, false), (false, true))) {
      ...
      withSQLConf(
        ...
        assert(
          rows === rowsAfterCoalesce,
          "Values changed after coalesce when " +
        ...
        assert(
          rowsAfterUnion === rows1 ++ rows2,
          "Values changed after union when " +
            s"codegenFallback=$codegenFallback and wholeStage=$wholeStage.")
      }
    }
  }
  ...
  test(
    "SPARK-14393: values generated by non-deterministic functions shouldn't change after " +
      "coalesce or union") {
    Seq(
      monotonically_increasing_id(),
      spark_partition_id(),
      rand(Random.nextLong()),
      randn(Random.nextLong())).foreach(assertValuesDoNotChangeAfterCoalesceOrUnion(_))
  }
  ...
  test("SPARK-26370: Fix resolution of higher-order function for the same identifier") {
    val df = Seq((Seq(1, 9, 8, 7), 1, 2), (Seq(5, 9, 7), 2, 2), (Seq.empty, 3, 2), (null, 4, 2))
      .toDF("i", "x", "d")

    checkAnswer(
      df.selectExpr("x", "exists(i, x -> x % d == 0)"),
      Seq(Row(1, true), Row(2, false), Row(3, false), Row(4, null)))
    checkAnswer(df.filter("exists(i, x -> x % d == 0)"), Seq(Row(Seq(1, 9, 8, 7), 1, 2)))
    checkAnswer(df.select("x").filter("exists(i, x -> x % d == 0)"), Seq(Row(1)))
  }
  ...
  test("SPARK-29462: Empty array of NullType for array function with no arguments") {
    Seq((true, StringType), (false, NullType)).foreach {
      case (arrayDefaultToString, expectedType) =>
        withSQLConf(
          SQLConf.LEGACY_CREATE_EMPTY_COLLECTION_USING_STRING_TYPE.key ->
  ...
  test(
    "SPARK-30790: Empty map with NullType as key/value type for map function with no argument") {
    Seq((true, StringType), (false, NullType)).foreach {
  ...
  test("SPARK-26071: convert map to array and use as map key") {
    ...
    checkError(
      ...
      parameters = Map("sqlExpr" -> "\"map(m, 1)\"", "keyType" -> "\"MAP<INT, STRING>\""),
      context =
        ExpectedContext(fragment = "map", callSitePattern = getCurrentClassCallSitePattern))
    checkAnswer(df.select(map(map_entries($"m"), lit(1))), Row(Map(Seq(Row(1, "a")) -> 1)))
  }
      ...
      df1.select(
        flatten(
          transform(
            $"numbers",
            (number: Column) =>
              transform($"letters", (letter: Column) => struct(number, letter))))),
    ...
    checkAnswer(
      df1.select(
        flatten(
          transform(
            $"numbers",
            (number: Column, i: Column) =>
              transform(
                $"letters",
                (letter: Column, j: Column) => struct(number + j, concat(letter, i)))))),
    ...
    checkAnswer(
      df2.select(
        map_zip_with(
          $"m1",
          $"m2",
          (k1: Column, ov1: Column, ov2: Column) =>
            map_zip_with(
              $"m1",
              $"m2",
              (k2: Column, iv1: Column, iv2: Column) => ov1 + iv1 + ov2 + iv2))),
  ...
  test("from_json - invalid schema string") {
    checkError(
      ...
      context = ExpectedContext(fragment = "from_json('{\"a\":1}', 1)", start = 7, stop = 29))
  }

  test("mask function") {
    val df = Seq("AbCD123-@$#", "abcd-EFGH-8765-4321").toDF("a")

    checkAnswer(df.selectExpr("mask(a)"), Seq(Row("XxXXnnn-@$#"), Row("xxxx-XXXX-nnnn-nnnn")))
    checkAnswer(df.select(mask($"a")), Seq(Row("XxXXnnn-@$#"), Row("xxxx-XXXX-nnnn-nnnn")))

    checkAnswer(
      df.selectExpr("mask(a, 'Y')"),
      Seq(Row("YxYYnnn-@$#"), Row("xxxx-YYYY-nnnn-nnnn")))
    checkAnswer(
      df.select(mask($"a", lit('Y'))),
      Seq(Row("YxYYnnn-@$#"), Row("xxxx-YYYY-nnnn-nnnn")))

    checkAnswer(
      df.selectExpr("mask(a, 'Y', 'y')"),
      Seq(Row("YyYYnnn-@$#"), Row("yyyy-YYYY-nnnn-nnnn")))
    checkAnswer(
      df.select(mask($"a", lit('Y'), lit('y'))),
      Seq(Row("YyYYnnn-@$#"), Row("yyyy-YYYY-nnnn-nnnn")))

    checkAnswer(
      df.selectExpr("mask(a, 'Y', 'y', 'd')"),
      Seq(Row("YyYYddd-@$#"), Row("yyyy-YYYY-dddd-dddd")))
    checkAnswer(
      df.select(mask($"a", lit('Y'), lit('y'), lit('d'))),
      Seq(Row("YyYYddd-@$#"), Row("yyyy-YYYY-dddd-dddd")))

    checkAnswer(
      df.selectExpr("mask(a, 'X', 'x', 'n', null)"),
      Seq(Row("XxXXnnn-@$#"), Row("xxxx-XXXX-nnnn-nnnn")))
    checkAnswer(
      df.select(mask($"a", lit('X'), lit('x'), lit('n'), lit(null))),
      Seq(Row("XxXXnnn-@$#"), Row("xxxx-XXXX-nnnn-nnnn")))

    checkAnswer(
      df.selectExpr("mask(a, null, null, null, '*')"),
      Seq(Row("AbCD123****"), Row("abcd*EFGH*8765*4321")))
    checkAnswer(
      df.select(mask($"a", lit(null), lit(null), lit(null), lit('*'))),
      Seq(Row("AbCD123****"), Row("abcd*EFGH*8765*4321")))
  }
  ...
  test("test array_compact") {
    val df = Seq(
      (
        ...
        Array("a", null, "b", null, "c", "d"),
        Array("", "")),
      (Array.empty[Integer], Array("1.0", "2.2", "3.0"), Array.empty[String]),
      (Array[Integer](null, null, null), null, null)).toDF("a", "b", "c")
    ...
    // complex data type
    checkAnswer(
      OneRowRelation().selectExpr(
        "array_compact(array(array(1, null,3), null, array(null, 2, 3)))"),
  ...
  test("array_append -> Unit Test cases for the function ") {
    ...
    checkError(
      ...
      parameters = Map(
        ...
        "sqlExpr" -> "\"array_append(a, b)\"",
        "dataType" -> "\"ARRAY\""),
      context = ExpectedContext(fragment = "array_append(a, b)", start = 0, stop = 17))
    ...
    checkAnswer(
      df7.select(array_append(col("a"), col("b"))),
      Seq(Row(Seq(3d, 2d, 5d, 1d, 2d, 3d))))

    val df8 = Seq((Array[Double](3d, 2d, 5d, 1d, 2d), 3)).toDF("a", "b")
    checkAnswer(
      df8.select(array_append(col("a"), col("b"))),
      Seq(Row(Seq(3d, 2d, 5d, 1d, 2d, 3d))))

    val df9 = spark.sql("SELECT array(1, 2, null) as a, CAST(null AS INT) as b")
    checkAnswer(df9.selectExpr("array_append(a, b)"), Seq(Row(Seq(1, 2, null, null))))

    val df10 = spark.createDataFrame(
      spark.sparkContext.parallelize(Seq(Row(Seq[Integer](1, 2, 3, null), null))),
      StructType(
        List(
          StructField("a", ArrayType.apply(IntegerType), true),
          StructField("b", IntegerType, true))))

    checkAnswer(df10.selectExpr("array_append(a, b)"), Seq(Row(Seq(1, 2, 3, null, null))))
  }
  ...
  test(
    "CANNOT_INVOKE_IN_TRANSFORMATIONS - Dataset transformations and actions " +
