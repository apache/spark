...
import org.apache.spark.sql.execution.streaming.{FileStreamSink, FileStreamSinkWriter, MemoryStream, MetadataLogFileCatalog}
...
class FileStreamSinkSuite extends StreamTest {
  ...
  private def testUnpartitionedData(
    ...
    checkAnswer(
      ...
      (0 until 10).map { id => Row(id.toString, "100") } )
    ...
    checkAnswer(
      spark.read.format(fileFormat.shortName).load(path.getCanonicalPath),
      (0 until 20).map { id => Row(id.toString, "100") } )
  }
  ...
  private def testPartitionedData(
    ...
    def writeRange(start: Int, end: Int, numPartitions: Int): Seq[String] = {
      val df = spark
        ...
        .select($"id", lit("100").as("value1"), lit("1000").as("value2"))
    ...
    try {
      query =
        ds.map(i => (i, (i * 1000).toString))
          ...
          .option("checkpointLocation", checkpointDir)
          .option("compression", codec)
          .option("header", "true") // for `cvs` format to save column names into files
          .format(fileFormat.shortName)
          .start(outputDir)
      ...
      val outputDf =
        spark
          .read
          .option("header", "true") // for `cvs` format to load column names from files
          .format(fileFormat.shortName)
          .load(outputDir)
  ...
  test("getSinkFileStatusUsingGlob()") {
    ...
    // We would expect only one file matched, so it should fail if more than one file match
