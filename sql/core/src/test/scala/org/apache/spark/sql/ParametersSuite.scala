...
class IdentifierWithParametersSuite extends QueryTest with SharedSparkSession {
  import testImplicits._

  test("IDENTIFIER with parameter - table reference") {
    // Test IDENTIFIER with parameters that get substituted before parse
    // This tests: parameter substitution -> identifier-lite parse-time resolution
    withTable("test_table") {
      spark.range(5).write.saveAsTable("test_table")

      checkAnswer(
        spark.sql("SELECT * FROM IDENTIFIER(:table_name) ORDER BY id",
          Map("table_name" -> "test_table")),
        Seq(Row(0), Row(1), Row(2), Row(3), Row(4))
      )
    }
  }

  test("IDENTIFIER with parameter - column reference in SELECT list") {
    // Test IDENTIFIER with parameters for column names in SELECT list
    val df = Seq((1, "a"), (2, "b"), (3, "c")).toDF("col1", "col2")
    df.createOrReplaceTempView("test_view")

    checkAnswer(
      spark.sql("SELECT IDENTIFIER(:col_name) FROM test_view",
        Map("col_name" -> "col1")),
      Seq(Row(1), Row(2), Row(3))
    )

    checkAnswer(
      spark.sql("SELECT IDENTIFIER(:c1), IDENTIFIER(:c2) FROM test_view",
        Map("c1" -> "col1", "c2" -> "col2")),
      Seq(Row(1, "a"), Row(2, "b"), Row(3, "c"))
    )
  }

  test("IDENTIFIER with parameter - qualified table name") {
    // Test IDENTIFIER with parameters for qualified identifiers
    withTable("test_qualified") {
      spark.sql("CREATE TABLE test_qualified (c1 INT) USING parquet")
      spark.sql("INSERT INTO test_qualified VALUES (42)")

      checkAnswer(
        spark.sql("SELECT * FROM IDENTIFIER(:qual_table)",
          Map("qual_table" -> "default.test_qualified")),
        Seq(Row(42))
      )
    }
  }

  test("IDENTIFIER with parameter - backticked identifier with spaces") {
    // Test IDENTIFIER with parameters for backticked identifiers
    val df = Seq((1, 2)).toDF("col 1", "col 2")
    df.createOrReplaceTempView("test_view2")

    checkAnswer(
      spark.sql("SELECT IDENTIFIER(:col_name) FROM test_view2",
        Map("col_name" -> "`col 1`")),
      Seq(Row(1))
    )
  }

  test("IDENTIFIER with parameter - DDL statements") {
    // Test IDENTIFIER with parameters in CREATE/DROP TABLE statements
    val tableName = "param_table"
    withTable(tableName) {
      spark.sql("CREATE TABLE IDENTIFIER(:tbl) (c1 INT) USING parquet",
        Map("tbl" -> tableName))
      spark.sql("INSERT INTO IDENTIFIER(:tbl) VALUES (100)",
        Map("tbl" -> tableName))

      checkAnswer(
        spark.sql("SELECT * FROM IDENTIFIER(:tbl)", Map("tbl" -> tableName)),
        Seq(Row(100))
      )

      spark.sql("DROP TABLE IDENTIFIER(:tbl)", Map("tbl" -> tableName))
    }
  }

  test("IDENTIFIER with parameter - function names") {
    // Test IDENTIFIER with parameters for function references
    checkAnswer(
      spark.sql("SELECT IDENTIFIER(:func_name)(-5)", Map("func_name" -> "abs")),
      Seq(Row(5))
    )

    checkAnswer(
      spark.sql("SELECT IDENTIFIER(:func)('hello')", Map("func" -> "upper")),
      Seq(Row("HELLO"))
    )
  }

  test("IDENTIFIER with parameter - column reference in WHERE clause") {
    // Test IDENTIFIER with parameters in WHERE clause
    val df = Seq((1, "a"), (2, "b"), (3, "c")).toDF("col1", "col2")
    df.createOrReplaceTempView("test_view3")

    checkAnswer(
      spark.sql("SELECT * FROM test_view3 WHERE IDENTIFIER(:col) > 1",
        Map("col" -> "col1")),
      Seq(Row(2, "b"), Row(3, "c"))
    )
  }

  test("IDENTIFIER with parameter - column reference in GROUP BY") {
    // Test IDENTIFIER with parameters in GROUP BY clause
    val df = Seq((1, "a"), (1, "b"), (2, "c")).toDF("col1", "col2")
    df.createOrReplaceTempView("test_view4")

    checkAnswer(
      spark.sql(
        "SELECT IDENTIFIER(:col), COUNT(*) FROM test_view4 GROUP BY IDENTIFIER(:col)",
        Map("col" -> "col1")),
      Seq(Row(1, 2), Row(2, 1))
    )
  }

  test("IDENTIFIER with parameter - column reference in ORDER BY") {
    // Test IDENTIFIER with parameters in ORDER BY clause
    val df = Seq((3, "a"), (1, "b"), (2, "c")).toDF("col1", "col2")
    df.createOrReplaceTempView("test_view5")

    checkAnswer(
      spark.sql("SELECT * FROM test_view5 ORDER BY IDENTIFIER(:col)",
        Map("col" -> "col1")),
      Seq(Row(1, "b"), Row(2, "c"), Row(3, "a"))
    )
  }
}
