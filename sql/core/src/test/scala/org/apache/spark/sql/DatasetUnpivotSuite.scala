 ...
 */
class DatasetUnpivotSuite extends QueryTest with SharedSparkSession {
  ...
  lazy val wideDataDs: Dataset[WideData] = Seq(
    ...
    WideData(4, null, null, None, None)).toDS()
  ...
  val longSchema: StructType = StructType(
    Seq(
  ...
  lazy val wideStructDataDs: DataFrame = wideDataDs.select(
    struct($"id").as("an"),
    struct($"str1".as("one"), $"str2".as("two")).as("str"))
  ...
  test("unpivot with two ids") {
    ...
    val unpivoted = wideDataDs
      .unpivot(
        ...
        valueColumnName = "val")
    assert(
      unpivoted.schema === StructType(Seq(
        StructField("id", IntegerType, nullable = false),
  ...
  test("unpivot without values") {
    val unpivoted = wideDataDs
      .select($"id", $"str1", $"str2")
      .unpivot(Array($"id"), variableColumnName = "var", valueColumnName = "val")
    assert(unpivoted.schema === longSchema)
    checkAnswer(unpivoted, longDataRows)
    ...
    val e = intercept[AnalysisException] {
      wideDataDs
        .select($"id", $"str1", $"str2")
        .unpivot(Array($"id"), Array.empty, variableColumnName = "var", valueColumnName = "val")
    }
    checkError(exception = e, condition = "UNPIVOT_REQUIRES_VALUE_COLUMNS", parameters = Map())

    // ids expressions are not allowed when no values are given
    val e2 = intercept[AnalysisException] {
      wideDataDs
        .select($"id", $"int1", $"long1")
        .unpivot(Array($"id" * 2), variableColumnName = "var", valueColumnName = "val")
    }
    checkError(
      exception = e2,
      condition = "UNPIVOT_REQUIRES_ATTRIBUTES",
      parameters =
        Map("given" -> "id", "empty" -> "value", "expressions" -> "\"(id * 2) AS `(id * 2)`\""))

    val e3 = intercept[AnalysisException] {
      wideDataDs
        .select($"id", $"int1", $"long1")
        .unpivot(Array($"id".as("uid")), variableColumnName = "var", valueColumnName = "val")
    }
    checkError(
      exception = e3,
      condition = "UNPIVOT_REQUIRES_ATTRIBUTES",
      parameters = Map("given" -> "id", "empty" -> "value", "expressions" -> "\"id AS uid\""))
  }

  test("unpivot without ids or values") {
    val unpivoted = wideDataDs
      .select($"str1", $"str2")
      .unpivot(Array.empty, variableColumnName = "var", valueColumnName = "val")
    assert(
      unpivoted.schema === StructType(
        Seq(
          StructField("var", StringType, nullable = false),
    ...
    val e = intercept[AnalysisException] {
      wideDataDs
        .select($"str1", $"str2")
        .unpivot(Array.empty, Array.empty, variableColumnName = "var", valueColumnName = "val")
    }
    checkError(exception = e, condition = "UNPIVOT_REQUIRES_VALUE_COLUMNS", parameters = Map())
  }
  ...
  test("unpivot with star values") {
    val unpivoted = wideDataDs
      .select($"str1", $"str2")
      .unpivot(Array.empty, Array($"*"), variableColumnName = "var", valueColumnName = "val")
    assert(
      unpivoted.schema === StructType(
        Seq(
          StructField("var", StringType, nullable = false),
  ...
  test("unpivot with id and star values") {
    val unpivoted = wideDataDs
      .select($"id", $"int1", $"long1")
      .unpivot(Array($"id"), Array($"*"), variableColumnName = "var", valueColumnName = "val")
  ...
  test("unpivot with invalid arguments") {
    ...
    checkError(
      ...
      parameters =
        Map("objectName" -> "`1`", "proposal" -> "`id`, `int1`, `str1`, `long1`, `str2`"),
    ...
    checkError(
      ...
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      parameters =
        Map("objectName" -> "`does`", "proposal" -> "`id`, `int1`, `long1`, `str1`, `str2`"),
      context = ExpectedContext(fragment = "$", callSitePattern = getCurrentClassCallSitePattern))

    // unpivoting without values where potential value columns are of incompatible types
    val e3 = intercept[AnalysisException] {
      wideDataDs.unpivot(Array.empty, variableColumnName = "var", valueColumnName = "val")
    }
    checkError(
      exception = e3,
      condition = "UNPIVOT_VALUE_DATA_TYPE_MISMATCH",
      parameters =
        Map("types" -> """"BIGINT" (`long1`), "INT" (`id`, `int1`), "STRING" (`str1`, `str2`)"""))

    // unpivoting with star id columns so that no value columns are left
    val e4 = intercept[AnalysisException] {
      wideDataDs.unpivot(Array($"*"), variableColumnName = "var", valueColumnName = "val")
    }
    checkError(exception = e4, condition = "UNPIVOT_REQUIRES_VALUE_COLUMNS", parameters = Map())
  ...
  test("unpivot with struct fields") {
    checkAnswer(
      wideStructDataDs.unpivot(Array($"an.id"), Array($"str.one", $"str.two"), "var", "val"),
      longStructDataRows)
  }

  test("unpivot with struct ids star") {
    checkAnswer(
      wideStructDataDs.unpivot(Array($"an.*"), Array($"str.one", $"str.two"), "var", "val"),
      longStructDataRows)
  }

  test("unpivot with struct values star") {
    checkAnswer(
      wideStructDataDs.unpivot(Array($"an.id"), Array($"str.*"), "var", "val"),
      longStructDataRows)
  }
  ...
  test("unpivot sql with struct fields") {
    ...
    checkAnswer(
      spark.sql("""SELECT * FROM (
                  ...
                  |);
                  |""".stripMargin),
      Seq(("dotNET", "2012", 15000), ("dotNET", "2013", 48000), ("dotNET", "2014", 22500))
    ...
    checkAnswer(
      ...
      Seq(
        ...
        ("dotNET", "2014", 22500, Some(1))).toDF("course", "year", "earnings", "sales"))
    ...
    checkError(
      ...
      parameters = Map(
        "given" -> "value",
        "empty" -> "id",
        "expressions" -> ("\"the.earnings.2012 AS `2012`\", " +
