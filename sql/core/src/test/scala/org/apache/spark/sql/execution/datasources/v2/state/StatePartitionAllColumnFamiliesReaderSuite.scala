...
class StatePartitionAllColumnFamiliesReaderSuite extends StateDataSourceTestBase {
  ...
  private def getNormalReadDf(checkpointDir: String): DataFrame = {
    spark.read
      .format("statestore")
  ...
  Seq(true, false).foreach { changelogCheckpointingEnabled =>
    ...
    testWithChangelogConfig("all-column-families: simple aggregation state ver 1") {
    ...
    testWithChangelogConfig("all-column-families: simple aggregation state ver 2") {
    ...
    testWithChangelogConfig("all-column-families: composite key aggregation state ver 1") {
      withSQLConf(SQLConf.STREAMING_AGGREGATION_STATE_FORMAT_VERSION.key -> "1") {
      withTempDir { tempDir =>
    ...
    testWithChangelogConfig("all-column-families: composite key aggregation state ver 2") {
      withSQLConf(SQLConf.STREAMING_AGGREGATION_STATE_FORMAT_VERSION.key -> "2") {
      withTempDir { tempDir =>
        runCompositeKeyStreamingAggregationQuery(tempDir.getAbsolutePath)
    ...
    testWithChangelogConfig("all-column-families: flatMapGroupsWithState, state ver 1") {
    ...
    testWithChangelogConfig("all-column-families: flatMapGroupsWithState, state ver 2") {
