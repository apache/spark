...
class ExplicitlyUnsupportedResolverFeatureSuite extends QueryTest with SharedSparkSession {
  ...
  test("Unsupported recursive data types") {
    withTable("with_array") {
      spark.sql("CREATE TABLE with_array (col1 ARRAY<INT>) USING JSON;").collect()
      checkResolution("SELECT col1 FROM with_array;")
    }
    withTable("with_map") {
      spark.sql("CREATE TABLE with_map(col1 MAP<STRING, INT>) USING JSON;").collect()
      checkResolution("SELECT col1 FROM with_map;")
    }
  }
