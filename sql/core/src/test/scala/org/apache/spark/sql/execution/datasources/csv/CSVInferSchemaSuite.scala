...
class CSVInferSchemaSuite extends SparkFunSuite {
  ...
  test("Timestamp field types are inferred correctly via custom data format") {
    ...
    assert(
      CSVInferSchema.inferField(TimestampType, "2015-08", dateFormat = dateFormat) == TimestampType)
    dateFormat.applyPattern("yyyy")
    assert(
      CSVInferSchema.inferField(TimestampType, "2015", dateFormat = dateFormat) == TimestampType)
  }
