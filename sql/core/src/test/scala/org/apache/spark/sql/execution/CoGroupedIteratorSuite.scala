...
class CoGroupedIteratorSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("basic") {
    ...
    val leftGrouped =
      GroupedIterator(leftInput, Seq("i".attr.int.at(0)), Seq("i".attr.int, "s".attr.string))
    val rightGrouped =
      GroupedIterator(rightInput, Seq("i".attr.int.at(0)), Seq("i".attr.int, "l".attr.long))
    val cogrouped = new CoGroupedIterator(leftGrouped, rightGrouped, Seq("i".attr.int))
