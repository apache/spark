...
class TwsTester2Suite extends SharedSparkSession {
  ...
  test("TwsTester2 should correctly test RunningCountProcessor") {
    ...
    assert(tester.peekValueState[Long]("count", "key2").get == 3L)
  ...
  test("TwsTester2 should allow direct access to ValueState") {
    ...
    // Initialize column families by processing empty batch
    tester.test(List())
    // Now set state - it will be visible in next batch
    tester.setValueState[Long]("count", "foo", 5)
    tester.setValueState[Long]("count", "bar", 10)
    val result = tester.test(List(("foo", "a"),("bar", "a")))
    assert(result.sorted == List(("bar", 11L), ("foo", 6L)))
    tester.stop()
    assert(tester.peekValueState[Long]("count", "foo").get == 6L)
    assert(tester.peekValueState[Long]("count", "bar").get == 11L)
  }
  ...
  test("TwsTester2 should allow direct access to ListState") {
    val tester = new TwsTester2(new TopKProcessor(2))
    // TODO: enforce in tester that state must be initialized (by batch or init state) before set/get.
    tester.test(List()) 
