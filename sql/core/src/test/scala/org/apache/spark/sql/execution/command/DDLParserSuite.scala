  ...
  test("create table like") {
    ...
    val (target, source, fileFormat, provider, properties, exists) =
      parser.parsePlan(v1).collect {
        case CreateTableLikeCommand(t, s, None, f, p, pr, e) => (t, s, f, p, pr, e)
      }.head
    assert(exists == false)
    assert(target.database.isEmpty)
    assert(target.table == "table1")
    assert(source.get.database.isEmpty)
    assert(source.get.table == "table2")
    ...
    assert(source2.get.database.isEmpty)
    assert(source2.get.table == "table2")
    ...
    assert(source3.get.database.isEmpty)
    assert(source3.get.table == "table2")
    ...
    assert(source4.get.database.isEmpty)
    assert(source4.get.table == "table2")
    ...
    assert(source5.get.database.isEmpty)
    assert(source5.get.table == "table2")
    ...
    assert(source6.get.database.isEmpty)
    assert(source6.get.table == "table2")
    assert(fileFormat6.locationUri.isEmpty)
    assert(provider6 == Some("ORC"))

    val v7 = "CREATE TABLE table1 LIKE FILE PARQUET 'examples/src/main/resources/users.parquet'"
    val (target7, likeFile, fileFormat7, provider7, properties7, exists7) =
      parser.parsePlan(v7).collect {
        case CreateTableLikeCommand(t, None, l, f, p, pr, e) => (t, l, f, p, pr, e)
      }.head
    assert(!exists7)
    assert(target7.database.isEmpty)
    assert(target7.table == "table1")
    assert(likeFile.get.format == "PARQUET")
    assert(likeFile.get.location == "examples/src/main/resources/users.parquet")
