...
class FileFormatWriterSuite
  ...
  test("SPARK-22252: FileFormatWriter should respect the input query schema") {
    withTable("t1", "t2", "t3", "t4") {
      spark.range(1).select(Symbol("id") as Symbol("col1"), Symbol("id") as Symbol("col2")).write.saveAsTable("t1")
      ...
      spark.range(1).select(Symbol("id"), Symbol("id") as Symbol("col1"), Symbol("id") as Symbol("col2")).write.saveAsTable("t3")
