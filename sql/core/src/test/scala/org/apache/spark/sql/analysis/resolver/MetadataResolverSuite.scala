...
class MetadataResolverSuite extends QueryTest with SharedSparkSession with SQLTestUtils {
  ...
  private def checkResolve(
    ...
    val relationsWithResolvedMetadata =
      metadataResolver.resolve(unresolvedPlan, analyzerBridgeState)
    val actualTableData = new mutable.HashMap[RelationId, TestTableData]
    for ((relationId, relationsWithResolvedMetadata) <- relationsWithResolvedMetadata.asScala) {
      val relation = relationsWithResolvedMetadata match {
        case SubqueryAlias(_, relation) => relation
        case relation => relation
      }
      actualTableData(relationId) = TestTableData(getTableName(relation), getTableSchema(relation))
    }
