...
class AlterTableDropColumnSuite
  ...
  test("drop column") {
    withNamespaceAndTable("ns", "tbl") { t =>
      sql(s"CREATE TABLE $t (id int, data string) USING PARQUET")
  ...
  test("drop nested column") {
    withNamespaceAndTable("ns", "tbl") { t =>
      sql(s"CREATE TABLE $t (id int, point struct<x: double, y: double, z: double>) USING PARQUET")
  ...
  test("drop nested column in array element") {
    withNamespaceAndTable("ns", "tbl") { t =>
      sql(s"CREATE TABLE $t (id int, points array<struct<x: double, y: double>>) USING PARQUET")
