...
class DataSourceV2Suite extends QueryTest with SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  test("simple writable data source") {
    ...
    Seq(classOf[SimpleWritableDataSource]).foreach { cls =>
      withTempPath { file =>
        ...
        spark.range(10).select(Symbol("id") as Symbol("i"), -Symbol("id") as Symbol("j"))
          .write.format(cls.getName).option("path", path).mode("append").save()
        checkAnswer(
        ...
        spark.range(10).select(Symbol("id") as Symbol("i"), -Symbol("id") as Symbol("j"))
          .write.mode("append").format(cls.getName).option("path", path).save()
        checkAnswer(
          spark.read.format(cls.getName).option("path", path).load(),
        ...
        spark.range(5).select(Symbol("id") as Symbol("i"), -Symbol("id") as Symbol("j"))
          .write.format(cls.getName).option("path", path).mode("overwrite").save()
        checkAnswer(
          spark.read.format(cls.getName).option("path", path).load(),
        ...
        val e = intercept[AnalysisException] {
          spark.range(5).select(Symbol("id") as Symbol("i"), -Symbol("id") as Symbol("j"))
            .write.format(cls.getName).option("path", path).mode("ignore").save()
        }
        assert(e.message.contains("please use Append or Overwrite modes instead"))

        val e2 = intercept[AnalysisException] {
          spark.range(5).select(Symbol("id") as Symbol("i"), -Symbol("id") as Symbol("j"))
            .write.format(cls.getName).option("path", path).mode("error").save()
        }
    ...
    Seq(classOf[SimpleWritableDataSource]).foreach { cls =>
      withTempPath { file =>
        ...
        spark.range(0, 10, 1, numPartition)
          ...
          .write.format(cls.getName).mode("append").option("path", path).save()
        checkAnswer(
          spark.read.format(cls.getName).option("path", path).load(),
          spark.range(10).select(Symbol("id"), -Symbol("id")))
  ...
  test("SPARK-23301: column pruning with arbitrary expressions") {
    ...
    // Symbol("j") === 1 can't be pushed down, but we should still be able do column pruning
