  ...
  test("merge handles metadata columns correctly") {
    withTempView("source") {
      ...
      checkLastWriteInfo(
        ...
        expectedMetadataSchema = Some(StructType(Array(
          PARTITION_FIELD_NULLABLE,
          INDEX_FIELD_NULLABLE))))

      checkLastWriteLog(
        writeLogEntry(metadata = Row("software", 0), data = Row(2, 200, "software")),
        writeLogEntry(metadata = Row("hr", null), data = Row(3, 301, "hr")),
        writeLogEntry(metadata = Row("hr", null), data = Row(4, 401, "hr")),
        writeLogEntry(metadata = Row("hr", null), data = Row(5, 501, "hr")),
        writeLogEntry(metadata = Row(null, null), data = Row(6, 0, "new")),
        writeLogEntry(metadata = Row("hr", 4), data = Row(7, 700, "hr")))
    }
  }
