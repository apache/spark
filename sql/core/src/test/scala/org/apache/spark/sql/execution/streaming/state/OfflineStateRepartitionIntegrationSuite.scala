...
abstract class OfflineStateRepartitionIntegrationSuiteBase extends StateDataSourceTestBase {
   ...
   * @param storeToColumnFamilyToStateSourceOptions:
   *   Map[store -> Map [colFamily -> sourceOptions]] Returns Map[store -> Map[columnFamily ->
   *   state rows]]
   */
  private def readStateDataByStoreName(
    ...
    storeToColumnFamilyToStateSourceOptions.map { case (storeName, columnFamilyToOptions) =>
      val columnFamilyData = columnFamilyToOptions.map { case (cfName, options) =>
        ...
        val stateData =
          readStateData(checkpointDir, batchId, storeName, options, selectExprs).collect()
        cfName -> stateData
      }
   ...
   *
   * @param newPartitions
   *   The target number of partitions after repartition
   * @param setupInitialState
   *   Callback to set up initial state (receives inputStream, checkpointDir, clock)
   * @param verifyResumedQuery
   *   Callback to verify resumed query (receives inputStream, checkpointDir, clock)
   * @param useManualClock
   *   Whether this test requires a manual clock (for processing time)
   * @param storeToColumnFamilyToStateSourceOptions
   *   Map of store name -> column family name -> options
   * @param storeToColumnFamilyToSelectExprs
   *   Map of store name -> column family name -> select expressions
   * @tparam T
   *   The type of data in the input stream (requires implicit Encoder)
   */
  def testRepartitionWorkflow[T: Encoder](
    ...
    withTempDir { checkpointDir =>
      ...
      val checkpointMetadata =
        new StreamingQueryCheckpointMetadata(spark, checkpointDir.getAbsolutePath)
      ...
      spark.streamingCheckpointManager.repartition(checkpointDir.getAbsolutePath, newPartitions)
  ...
  /**
   * Helper function to test with both increase and decrease repartition operations. This reduces
   * code duplication across different operator tests.
   *
   * @param testNamePrefix
   *   The prefix for the test name (e.g., "aggregation state v2")
   * @param testFun
   *   The test function that takes newPartitions as parameter
   */
  def testWithAllRepartitionOperations(testNamePrefix: String)(testFun: Int => Unit): Unit = {
          ...
          testRepartitionWorkflow[Int](
            ...
            verifyResumedQuery = (inputData, checkpointDir, _) => {
              ...
              testStream(aggregated, OutputMode.Update)(
                ...
                CheckLastBatch((0, "Apple", 3, 6, 6, 0), (1, "Banana", 3, 9, 7, 1)))
            })
        }
    }
  }
      ...
      verifyResumedQuery = (inputData, checkpointDir, _) => {
        ...
        testStream(resumed, OutputMode.Complete())(
          ...
          CheckNewAnswer(
            ...
            ("structured", 41, 51, 10, 1),
            ("new", 100, 110, 10, 1),
            ("session", 100, 110, 10, 1)))
      })
  }

  /**
   * Helper function to test FlatMapGroupsWithState operator with different state versions and
   * both increase/decrease repartition operations.
   * @param testNamePrefix
   *   The prefix for the test name (e.g., "flatMapGroupsWithState")
   * @param testFun
   *   The test function that takes in the number of partitions after repartition
   */
  def testFMGWOnAllPartitionOperation(testNamePrefix: String)(testFun: Int => Unit): Unit = {
    ...
    testRepartitionWorkflow[(String, Long)](
      ...
      verifyResumedQuery = (inputData, checkpointDir, clockOpt) => {
        ...
        testStream(resumed, OutputMode.Update)(
          ...
          AdvanceManualClock(1 * 1000),
          CheckNewAnswer(("hello", 4, 9000, false), ("new", 1, 0, false)))
      },
      useManualClock = true)
  }
