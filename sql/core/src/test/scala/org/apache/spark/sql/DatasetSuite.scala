...
class DatasetSuite extends QueryTest with SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  test("cogroup with complex key types") {
    ...
    val ds1 = Seq(
      (ClassData("x", 1), "left1"),
      (ClassData("x", 1), "left2"),
      (ClassData("y", 2), "left3")).toDS()

    val ds2 = Seq((ClassData("x", 1), 100), (ClassData("z", 3), 200)).toDS()

    val cogrouped =
      ds1.groupByKey(_._1).cogroup(ds2.groupByKey(_._1)) { case (key, left, right) =>
  ...
  test("cogroup with null keys") {
    ...
    val ds1 = Seq((Some(1), "a"), (Some(1), "b"), (None, "c"), (None, "d"), (Some(2), "e")).toDS()
    val ds2 = Seq((Some(1), 10), (None, 20), (Some(3), 30)).toDS()
  ...
  test("cogroup with empty datasets") {
    ...
    val emptyLeftResult = emptyDs
      .groupByKey(_._1)
      .cogroup(ds2.groupByKey(_._1)) { case (key, left, right) =>
        Iterator((key, countElements(left, right)))
      }
      .collect()
      .sortBy(_._1)
    ...
    val emptyRightResult = ds1
      .groupByKey(_._1)
      .cogroup(emptyDs.groupByKey(_._1)) { case (key, left, right) =>
        Iterator((key, countElements(left, right)))
      }
      .collect()
      .sortBy(_._1)
    ...
    val bothEmptyResult = emptyDs
      .groupByKey(_._1)
      .cogroup(emptyDs2.groupByKey(_._1)) { case (key, left, right) =>
        Iterator((key, countElements(left, right)))
      }
      .collect()
  ...
  test("SPARK-49960: joinWith custom encoder") {
     ...
     */
    val ds1 = Seq(1, 1, 2).toDS()
    val ds2 = spark.createDataset[ClassData](Seq(ClassData("a", 1), ClassData("b", 2)))(
      CustomPathEncoder.custClassDataEnc)
  ...
  test("SPARK-54620: Observation should not blocking forever") {
    ...
    df = df
      ...
      .join(Seq((1, 2), (3, 4)).toDF("v1", "v2").repartition($"v2"), Seq("v1"), "inner")
    df.collect()
...
object CustomPathEncoder {
  ...
  val custClassDataEnc: ProductEncoder[ClassData] = realClassDataEnc.copy(fields =
    Seq(realClassDataEnc.fields.head.copy(enc = custStringEnc), realClassDataEnc.fields.last))
}
