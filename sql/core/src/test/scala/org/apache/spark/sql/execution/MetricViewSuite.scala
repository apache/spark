...
abstract class MetricViewSuite extends QueryTest with SQLTestUtils {
  ...
  protected def withMetricView(viewName: String, metricViewDefinition: MetricView)(
      body: => Unit): Unit = {
  ...
  test("test source type") {
    val sources = Seq(AssetSource(testTableName), SQLSource("SELECT * FROM test_table"))
    sources.foreach { source =>
      ...
      withMetricView(testMetricViewName, metricView) {
        ...
        checkAnswer(
          sql(
            "SELECT measure(count_sum), measure(price_avg) " +
      ...
      checkAnswer(
        sql(
          "SELECT measure(count_sum), measure(price_avg) " +
            "FROM test_metric_view WHERE region_upper = 'REGION_1'"),
        sql(
          "SELECT sum(count), avg(price) FROM test_table WHERE " +
  ...
  test("test dimensions and measures") {
    ...
    withMetricView(testMetricViewName, metricView) {
      ...
      checkAnswer(
        sql(
          "SELECT region, product, measure(count_sum), measure(price_avg) " +
            "FROM test_metric_view GROUP BY region, product"),
        sql(
          "SELECT region, product, sum(count), avg(price) " +
  ...
  test("column from source cannot be used when query metric view") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkError(
        ...
        queryContext = Array(ExpectedContext(fragment = "count", start = 11, stop = 15)))
    }
  }
  ...
  test("test ORDER BY and LIMIT clauses") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT region, measure(count_sum) " +
      ...
      checkAnswer(
        sql(
          "SELECT product, measure(price_avg) " +
  ...
  test("test complex WHERE conditions with dimensions") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT region, product, measure(count_sum) " +
            "FROM test_metric_view WHERE region_upper = 'REGION_1' " +
            "AND product IN ('product_1', 'product_2') " +
            "GROUP BY region, product"),
        sql(
          "SELECT region, product, sum(count) " +
  ...
  test("test metric view with where clause and additional query filters") {
    val metricView = MetricView(
      "0.1",
      AssetSource(testTableName),
      Some("product IN ('product_1', 'product_2')"),
      testMetricViewColumns)
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
  ...
  test("test multiple measures with different aggregations") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT measure(count_sum), measure(count_avg), measure(count_max), " +
      ...
      checkAnswer(
        sql(
          "SELECT region, measure(count_sum), measure(count_max), measure(price_sum) " +
  ...
  test("test dimension expressions with case statements") {
    val columns = Seq(
      ...
      Column(
        "region_category",
        DimensionExpression("CASE WHEN region = 'region_1' THEN 'Group A' ELSE 'Group B' END"),
        1),
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT region_category, measure(count_sum) " +
            "FROM test_metric_view GROUP BY region_category"),
        sql(
          "SELECT CASE WHEN region = 'region_1' THEN 'Group A' ELSE 'Group B' END, " +
  ...
  test("test dimensions with aggregate functions in GROUP BY") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        ...
        sql(
          "SELECT upper(region), product, sum(count) " +
  ...
  test("test SQLSource with complex query") {
    val sqlSource =
      SQLSource("SELECT region, product, count, price FROM test_table WHERE count > 20")
  ...
  test("test combining multiple dimension expressions in WHERE") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT product, measure(count_sum) " +
  ...
  test("test measure with COUNT DISTINCT") {
    ...
    withMetricView(testMetricViewName, metricView) {
      checkAnswer(
        sql(
          "SELECT region, measure(product_count), measure(count_sum) " +
            "FROM test_metric_view GROUP BY region"),
        sql(
          "SELECT region, count(distinct product), sum(count) " +
  ...
  test("test union of same aggregated metric view dataframe") {
    ...
    withMetricView(testMetricViewName, metricView) {
      ...
      val df =
        sql(s"""SELECT region, measure(count_sum) as total_count, measure(price_avg) as avg_price
      ...
      assert(unionDf.count() == df.count() * 2, "Union should double the row count")
