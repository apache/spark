...
class StateStoreCoordinatorSuite extends SparkFunSuite with SharedSparkContext {
  ...
  /**
   * Verifies snapshot upload RPC messages from state stores are registered and verifies the
   * coordinator detected the correct lagging partitions.
   */
  ...
  faultyStateStoreProviders.foreach { case (providerName, providerClassName, badPartitions) =>
    test(s"SPARK-54063: Force trigger snapshot upload for $providerName when lagging") {
      withCoordinatorAndSQLConf(
        sc,
        ...
        case (coordRef, spark) =>
          ...
          def verifyShouldForceSnapshotOnBadPartitions(
            ...
            badPartitions.foreach { partitionId =>
              val storeId =
                StateStoreId(checkpointDir, 0, partitionId, StateStoreId.DEFAULT_STORE_NAME)
  ...
  test(s"SPARK-54063: force snapshot propagate when upgrade read store to write store") {
    withCoordinatorAndSQLConf(
      sc,
      (StateStoreCoordinatorSuite.spark54063CommonConfigs :+
      ...
      case (coordRef, spark) =>
        ...
        // Verify only the partitions in badPartitions doesn't have a snapshot
        val laggingStores =
          coordRef.getLaggingStoresForTesting(query.runId, snapshotVersionOnLagDetected + 1)
        ...
        val stateCheckpointDir = streamingQuery.lastExecution.checkpointLocation
        val storeId =
          StateStoreId(stateCheckpointDir, 0, badPartitions.head, StateStoreId.DEFAULT_STORE_NAME)
        val providerId = StateStoreProviderId(storeId, query.runId)
        val latestSnapshotVersion = coordRef.getLatestSnapshotVersionForTesting(providerId)
...
class StateStoreCoordinatorStreamingSuite extends StreamTest {
  ...
  Seq(
    ...
    ("HDFS", classOf[HDFSBackedSkipMaintenanceOnCertainPartitionsProvider].getName)).foreach {
  ...
  Seq(
    ...
    ("HDFS", classOf[HDFSBackedStateStoreProvider].getName)).foreach {
    case (providerName, providerClassName) =>
      test(
