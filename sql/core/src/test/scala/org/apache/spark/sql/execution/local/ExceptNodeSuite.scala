...
class ExceptNodeSuite extends LocalNodeTest {
  ...
  test("basic") {
    ...
    val exceptNode = new ExceptNode(conf, leftNode, rightNode)
    val expectedOutput = (1 to n).filter {
      i => i % 2 == 0 && i % 3 != 0 }.map { i => (i, i) }.toArray
    val actualOutput = exceptNode.collect().map { case row =>
