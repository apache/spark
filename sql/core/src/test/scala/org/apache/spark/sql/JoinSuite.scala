...
//  }
  ...
  test("SPARK-44514: Rewrite the join to filter") {
    def verifyResult(sqlString: String, expectedAnswer: Row*): Unit = {
      checkAnswer(sql(sqlString), expectedAnswer)
    }

    // scalastyle:off
    withTempView("t1", "t2", "one_row", "empty_row", "null_row") {
      val leftSideAllRows = Seq(Row(1), Row(null), Row(3))
      sql("CREATE TEMPORARY VIEW t1 AS SELECT * FROM values(1), (null), (3) AS t1(id)")
      sql("CREATE TEMPORARY VIEW t2 AS SELECT * FROM values(1), (2), (null) AS t2(id)")

      sql("CREATE TEMPORARY VIEW one_row AS SELECT min(id) as id FROM t2")
      sql("CREATE TEMPORARY VIEW empty_row AS SELECT id FROM t2 WHERE id < 0")
      sql("CREATE TEMPORARY VIEW null_row AS SELECT id FROM t2 WHERE id is null LIMIT 1")

      verifyResult("select t1.* from t1 join one_row on t1.id = one_row.id", Row(1))
      verifyResult("select t1.* from t1 join empty_row on t1.id = empty_row.id")
      verifyResult("select t1.* from t1 join null_row on t1.id = null_row.id")

      verifyResult("select t1.* from t1 join one_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 join empty_row")
      verifyResult("select t1.* from t1 join null_row", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 cross join one_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 cross join empty_row")
      verifyResult("select t1.* from t1 cross join null_row", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 left join one_row on t1.id = one_row.id", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left join empty_row on t1.id = empty_row.id", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left join null_row on t1.id = null_row.id", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 left join one_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left join empty_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left join null_row", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 full join one_row on t1.id = one_row.id", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 full join empty_row on t1.id = empty_row.id", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 full join null_row on t1.id = null_row.id", (leftSideAllRows :+ Row(null)): _*)

      verifyResult("select t1.* from t1 full join one_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 full join empty_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 full join null_row", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 left semi join one_row on t1.id = one_row.id", Row(1))
      verifyResult("select t1.* from t1 left semi join empty_row on t1.id = empty_row.id")
      verifyResult("select t1.* from t1 left semi join null_row on t1.id = null_row.id")

      verifyResult("select t1.* from t1 left semi join one_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left semi join empty_row")
      verifyResult("select t1.* from t1 left semi join null_row", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 left anti join one_row on t1.id = one_row.id", Row(3), Row(null))
      verifyResult("select t1.* from t1 left anti join empty_row on t1.id = empty_row.id", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left anti join null_row on t1.id = null_row.id", leftSideAllRows: _*)

      verifyResult("select t1.* from t1 left semi join one_row")
      verifyResult("select t1.* from t1 left semi join empty_row", leftSideAllRows: _*)
      verifyResult("select t1.* from t1 left semi join null_row")
    }
  }
}
