...
class ColumnExpressionSuite extends QueryTest with SharedSQLContext {
  ...
  private lazy val booleanData = {
    sqlContext.createDataFrame(sqlContext.sparkContext.parallelize(
      Row(false, false) ::
      Row(false, true) ::
      Row(true, false) ::
      Row(true, true) :: Nil),
  ...
  test("isNaN") {
    val testData = sqlContext.createDataFrame(sqlContext.sparkContext.parallelize(
  ...
  test("!==") {
    val nullData = sqlContext.createDataFrame(sqlContext.sparkContext.parallelize(
      Row(1, 1) ::
      Row(1, 2) ::
      Row(1, null) ::
      Row(null, null) :: Nil),
  ...
  test("monotonicallyIncreasingId") {
    ...
    val df = sqlContext.sparkContext.parallelize(Seq[Int](), 2).mapPartitions { _ =>
