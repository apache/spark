...
class XmlSuite
  ...
  test("DSL test for permissive mode for corrupt records") {
    ...
    if (legacyParserEnabled) {
      ...
      val malformedRowTwo =
        malformedRows.select("_malformed_records").take(2).last.get(0).toString
  ...
  Seq(
    ...
    "basket_invalid_at_the_end.xml").foreach { file =>
  ...
  test("schema_of_xml with DROPMALFORMED parse error test") {
    checkError(
      ...
      parameters = Map("funcName" -> "`schema_of_xml`", "mode" -> "DROPMALFORMED"))
  }
  ...
  test("SPARK-45488: root-level value tag for not attributes-only object") {
    ...
    val schema =
      buildSchema(field(VALUETAG_NAME), field(ATTRIBUTE_NAME), field(TAG_NAME, LongType))
    val expectedAns = Seq(
  ...
  test("ignore commented and CDATA row tags") {
    ...
    val expectedResults4 =
      Seq(Row("<a>1</a>"), Row("2"), Row("<ROW>3</ROW>"), Row("4"), Row("<ROW>5</ROW>"))
  ...
  def bigIntegerRecords: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize(s"""<ROW><a>1${"0".repeat(
        38)}</a><b>92233720368547758070</b></ROW>""" :: Nil))(Encoders.STRING)
  ...
  def floatingValueRecords: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize(s"""<ROW><a>0.${"0".repeat(
        38)}1</a><b>.01</b></ROW>""" :: Nil))(Encoders.STRING)
  ...
  test("SPARK-46355: Check Number of open files") {
    withSQLConf(
      "fs.file.impl" -> classOf[XmlSuiteDebugFileSystem].getName,
  ...
  test("from_xml only allow StructType or VariantType") {
    ...
    Seq("array<string>", "map<string, string>").foreach { schema =>
      val exception = intercept[AnalysisException](df
        .withColumn("decoded", from_xml(df.col("payload"), schema, Map[String, String]().asJava)))
      assert(exception.getCondition.contains("INVALID_XML_SCHEMA"))
    }

    Seq("struct<a:string>", "variant").foreach { schema =>
      df.withColumn("decoded", from_xml(df.col("payload"), schema, Map[String, String]().asJava))
    }
  }
  ...
  test("Test all supported codec and extension including zst, zstd and gzip") {
    for (fileFormatWriter <- Seq(true, false)) {
      logInfo(s"Testing with fileFormatWriter=$fileFormatWriter")
      withTempDir { dir =>
        val options = Map("rowTag" -> "ROW", "recursiveFileLookup" -> "true")
  ...
  test("TIME type roundtrip with XML datasource - representative precisions") {
    ...
    def testTimesForPrecision(p: Int): Seq[LocalTime] = {
      p match {
        case 0 => Seq(LocalTime.of(0, 0, 0), LocalTime.of(14, 30, 45), LocalTime.of(23, 59, 59))
        case 3 =>
          Seq(
