...
class PlannerSuite extends SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  test("count distinct is partially aggregated") {
    val query =
      testData.groupBy(Symbol("value")).agg(count_distinct(Symbol("key"))).queryExecution.analyzed
    testPartialAggregationPlan(query)
  }

  test("mixed aggregates are partially aggregated") {
    val query = testData.groupBy(Symbol("value")).agg(count(Symbol("value")),
      count_distinct(Symbol("key"))).queryExecution.analyzed
    testPartialAggregationPlan(query)
  }
  ...
  test("terminal limit -> project -> sort should use TakeOrderedAndProject") {
    val query = testData.select(Symbol("key"),
      Symbol("value")).sort(Symbol("key")).select(Symbol("value"), Symbol("key")).limit(2)
    val planned = query.queryExecution.executedPlan
    assert(planned.isInstanceOf[execution.TakeOrderedAndProjectExec])
  ...
  test("TakeOrderedAndProject can appear in the middle of plans") {
    val query = testData.select(Symbol("key"),
      Symbol("value")).sort(Symbol("key")).limit(2).filter(Symbol("key") === 3)
    val planned = query.queryExecution.executedPlan
