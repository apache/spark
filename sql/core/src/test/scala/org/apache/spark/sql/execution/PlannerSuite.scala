...
class PlannerSuite extends SharedSQLContext {
  ...
  test("non-partial aggregation for aggregates") {
    withTempView("testNonPartialAggregation") {
      ...
      // aggregation values, no partial aggregation should exist in query plans.
      ...
      val aggOps2 = planned2.collect { case n if n.nodeName contains "Aggregate" => n }
      assert(aggOps2.size == 2, s"The plan $planned2 has partial aggregations.")
    }
  }
