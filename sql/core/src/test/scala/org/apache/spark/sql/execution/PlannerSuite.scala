...
class PlannerSuite extends SharedSQLContext {
  ...
  test("EnsureRequirements with incompatible child partitionings which satisfy distribution") {
    ...
    val outputPlan = EnsureRequirements(sqlContext.sessionState.sqlConf).apply(inputPlan)
  ...
  test("Reuse exchanges") {
    ...
    val outputPlan = ReuseExchange(sqlContext.sessionState.sqlConf).apply(inputPlan)
    ...
    val outputPlan2 = ReuseExchange(sqlContext.sessionState.sqlConf).apply(inputPlan2)
