...
class ProductAggSuite extends QueryTest with SharedSparkSession {
  ...
  test("bare factorial") {
    checkAnswer(data16.agg(product(col("x"))), Row((1L to 16L).reduce { _ * _ }.toDouble))

    checkAnswer(data16.agg(product(col("x"))), Row(factorials(16)))
  }
  ...
  test("type flexibility") {
    ...
    val prods = variants
      .foldLeft(bytes16) { case (df, (id, typ)) =>
        df.withColumn(id, df.col("x").cast(typ))
      }
      .agg(lit(1) as "dummy", variants.keys.toSeq.map { id => product(col(id)) as id }: _*)

    variants.keys.foreach { typ =>
      checkAnswer(prods.select(typ), Row(factorials(16)))
    }
  }
  ...
  test("windowed factorials") {
    ...
    checkAnswer(
      ...
      Seq(Row(1, 1.0), Row(2, 2.0), Row(3, 6.0), Row(4, 24.0), Row(5, 120.0)))
  ...
  test("grouped factorials") {
    ...
    val expectedBase =
      Seq((3 * 6 * 9 * 12 * 15), (1 * 4 * 7 * 10 * 13 * 16), (2 * 5 * 8 * 11 * 14))

    checkAnswer(grouped.select("product"), expectedBase.map { n => Row(n.toDouble) })
