...
class DataSourceStrategySuite extends PlanTest with SharedSparkSession {
  val attrInts = Seq(
    "cint".attr.int,
    "c.int".attr.int,
    GetStructField("a".attr.struct(StructType(
    ...
    GetStructField("a.b".attr.struct(StructType(
    ...
    GetStructField(GetStructField("a".attr.struct(StructType(
      StructField("cstr1", StringType, nullable = true) ::
  ...
  test("SPARK-31027 test `PushableColumn.unapply` that finds the column name of " +
    ...
    assert(PushableColumnAndNestedColumn.unapply(Abs("col".attr.int)) === None)
  }
