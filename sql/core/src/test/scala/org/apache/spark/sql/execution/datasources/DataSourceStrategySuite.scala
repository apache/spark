...
class DataSourceStrategySuite extends SharedSparkSession {
  val attrInts = Seq(
    ...
    GetStructField(
      $"a".struct(
        StructType(StructField("cstr", StringType, nullable = true) ::
          StructField("cint", IntegerType, nullable = true) :: Nil)),
      1,
      None),
    GetStructField(
      $"a".struct(
        StructType(StructField("c.int", IntegerType, nullable = true) ::
    ...
    GetStructField(
      ...
      None),
    GetStructField(
      $"`a.b`".struct(StructType(StructField("c.int", IntegerType, nullable = true) :: Nil)),
      0,
      None),
    GetStructField(
      GetStructField(
        $"a".struct(
          StructType(
            StructField("cstr1", StringType, nullable = true) ::
              StructField(
                "b",
                StructType(StructField("cint", IntegerType, nullable = true) ::
  ...
  val attrStrs = Seq(
    ...
    GetStructField(
      $"a".struct(
        StructType(StructField("c.str", StringType, nullable = true) ::
          StructField("cint", IntegerType, nullable = true) :: Nil)),
      0,
      None),
    GetStructField(
      ...
      None),
    GetStructField(
      $"`a.b`".struct(StructType(StructField("c.str", StringType, nullable = true) :: Nil)),
      0,
      None),
    GetStructField(
      GetStructField(
          ...
          StructType(
            StructField("cint1", IntegerType, nullable = true) ::
              StructField(
                "b",
                StructType(StructField("cstr", StringType, nullable = true) ::
                  StructField("cint2", IntegerType, nullable = true) :: Nil)) :: Nil)),
        1,
        None),
      0,
      None)).zip(
  ...
  test("translate simple expression") {
    attrInts
      ...
      .foreach { case ((attrInt, intColName), (attrStr, strColName)) =>
        ...
        testTranslateFilter(
          EqualNullSafe(attrStr, Literal(null)),
          Some(sources.EqualNullSafe(strColName, null)))
        testTranslateFilter(
          EqualNullSafe(Literal(null), attrStr),
          Some(sources.EqualNullSafe(strColName, null)))
        ...
        testTranslateFilter(
          GreaterThanOrEqual(attrInt, 1),
          Some(sources.GreaterThanOrEqual(intColName, 1)))
        testTranslateFilter(
          GreaterThanOrEqual(1, attrInt),
        ...
        testTranslateFilter(
          ...
          Some(sources.LessThanOrEqual(intColName, 1)))
        testTranslateFilter(
          LessThanOrEqual(1, attrInt),
          Some(sources.GreaterThanOrEqual(intColName, 1)))

        testTranslateFilter(
          InSet(attrInt, Set(1, 2, 3)),
          Some(sources.In(intColName, Array(1, 2, 3))))

        testTranslateFilter(
          In(attrInt, Seq(1, 2, 3)),
          Some(sources.In(intColName, Array(1, 2, 3))))
        ...
        // cint > 1 AND cint < 10
        testTranslateFilter(
          And(GreaterThan(attrInt, 1), LessThan(attrInt, 10)),
          Some(sources.And(sources.GreaterThan(intColName, 1), sources.LessThan(intColName, 10))))

        // cint >= 8 OR cint <= 2
        testTranslateFilter(
          Or(GreaterThanOrEqual(attrInt, 8), LessThanOrEqual(attrInt, 2)),
          Some(
            sources.Or(
  ...
  test("translate complex expression") {
    attrInts.foreach { case (attrInt, intColName) =>
      ...
      testTranslateFilter(
        Or(
          And(GreaterThan(attrInt, 1), LessThan(attrInt, 10)),
          And(GreaterThan(attrInt, 50), LessThan(attrInt, 100))),
        Some(
          sources.Or(
            sources.And(sources.GreaterThan(intColName, 1), sources.LessThan(intColName, 10)),
            sources.And(sources.GreaterThan(intColName, 50), sources.LessThan(intColName, 100)))))
      ...
      testTranslateFilter(
        Not(
          And(
            ...
            Or(LessThanOrEqual(attrInt, 50), GreaterThanOrEqual(attrInt, 100)))),
        None)

      // (cint = 1 OR cint = 10) OR (cint > 0 OR cint < -10)
      testTranslateFilter(
        Or(
          Or(EqualTo(attrInt, 1), EqualTo(attrInt, 10)),
          Or(GreaterThan(attrInt, 0), LessThan(attrInt, -10))),
        Some(
          sources.Or(
            sources.Or(sources.EqualTo(intColName, 1), sources.EqualTo(intColName, 10)),
            sources.Or(sources.GreaterThan(intColName, 0), sources.LessThan(intColName, -10)))))
      ...
      testTranslateFilter(
        And(
          And(GreaterThan(attrInt, 1), LessThan(attrInt, 10)),
          And(EqualTo(attrInt, 6), IsNotNull(attrInt))),
        Some(
          sources.And(
            sources.And(sources.GreaterThan(intColName, 1), sources.LessThan(intColName, 10)),
            sources.And(sources.EqualTo(intColName, 6), sources.IsNotNull(intColName)))))
      ...
      testTranslateFilter(
        And(
          Or(GreaterThan(attrInt, 1), LessThan(attrInt, 10)),
          Or(EqualTo(attrInt, 6), IsNotNull(attrInt))),
        Some(
          sources.And(
            sources.Or(sources.GreaterThan(intColName, 1), sources.LessThan(intColName, 10)),
            sources.Or(sources.EqualTo(intColName, 6), sources.IsNotNull(intColName)))))
  ...
  test(
    "SPARK-31027 test `PushableColumn.unapply` that finds the column name of " +
  ...
  /**
   * Translate the given Catalyst [[Expression]] into data source [[sources.Filter]] then verify
   * against the given [[sources.Filter]].
   */
  ...
  test("SPARK-41636: selectFilters returns predicates in deterministic order") {
    ...
    val predicates = Seq(
      EqualTo(idColAttribute, 1),
      EqualTo(idColAttribute, 2),
      EqualTo(idColAttribute, 3),
      EqualTo(idColAttribute, 4),
  ...
  test("SPARK-48431: Push filters on columns with UTF8_BINARY collation") {
    ...
    testTranslateFilter(
      Not(EqualTo(colAttr, Literal("value"))),
      Some(sources.Not(sources.EqualTo("col", "value"))))
    testTranslateFilter(
      LessThan(colAttr, Literal("value")),
      Some(sources.LessThan("col", "value")))
    testTranslateFilter(
      LessThan(colAttr, Literal("value")),
      Some(sources.LessThan("col", "value")))
    testTranslateFilter(
      LessThanOrEqual(colAttr, Literal("value")),
      Some(sources.LessThanOrEqual("col", "value")))
    testTranslateFilter(
      GreaterThan(colAttr, Literal("value")),
      Some(sources.GreaterThan("col", "value")))
    testTranslateFilter(
      GreaterThanOrEqual(colAttr, Literal("value")),
