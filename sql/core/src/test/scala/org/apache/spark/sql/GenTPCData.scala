...
class GenTPCDataConfig(args: Array[String]) {
  ...
  private def printUsageAndExit(exitCode: Int): Unit = {
    ...
    System.err.println("""
      ...
      |  --tableFilter                   comma-separated list of table names to generate (e.g., store_sales,lineitem),
      |                                  all the tables are generated by default
      |  --numPartitions                 how many dsdgen/dbgen partitions to run - number of input tasks
      """.stripMargin)
    // scalastyle:on
  ...
  private def checkRequiredArguments(): Unit = {
    if (dsdgenDir == null && dbgenDir == null) {
      // scalastyle:off println
      System.err.println("Must specify a dsdgen or dbgen path")
      // scalastyle:on println
      printUsageAndExit(-1)
    }
    if (location == null) {
