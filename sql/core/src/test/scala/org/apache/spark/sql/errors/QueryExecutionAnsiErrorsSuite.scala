  ...
  test("CAST_INVALID_INPUT: cast string to double") {
    checkErrorClass(
      ...
      msg = """The value '111111111111xe23' of the type "STRING" cannot be cast to "DOUBLE" """ +
        "because it is malformed. Correct the syntax for the value before casting it, " +
        "or change the type to one appropriate for the value. To return NULL instead, " +
        "use `try_cast`. If necessary set " +
