...
import org.apache.spark.sql.catalyst.plans.physical.{ClusteredDistribution, HashClusteredDistribution, HashPartitioning}
import org.apache.spark.sql.execution.SparkPlan

trait HashClusteredDistributionTestHelper extends SparkFunSuite {
  ...
  protected def requireHashClusteredDistribution(
      plan: SparkPlan,
      desiredClusterColumns: Seq[Seq[String]],
      desiredNumPartitions: Int): Boolean = {
    assert(plan.requiredChildDistribution.length === desiredClusterColumns.length)
    plan.requiredChildDistribution.zip(desiredClusterColumns).forall {
      case (d: HashClusteredDistribution, clusterColumns: Seq[String])
        if partitionExpressionsColumns(d.expressions) == clusterColumns &&
          d.requiredNumPartitions == Some(desiredNumPartitions) => true
