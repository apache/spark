...
class DataFrameNaFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("fill with map") {
    withSQLConf(SQLConf.SUPPORT_QUOTED_REGEX_COLUMN_NAME.key -> "false") {
      ...
      assert(
        df.na.fill(fillMap).schema.fields.map(_.dataType) === df.schema.fields.map(_.dataType))
    }
  }
  ...
  def createDFsWithSameFieldsName(): (classic.DataFrame, classic.DataFrame) = {
    val df1 = Seq(
      ...
      ("f1-2", null, null),
      ("f1-3", "f2", "f3-1"),
      ("f1-4", "f2", "f3-1")).toDF("f1", "f2", "f3")
    val df2 = Seq(("f1-1", null, null), ("f1-2", "f2", null), ("f1-3", "f2", "f4-1")).toDF(
      "f1",
      "f2",
      "f4")
    (df1, df2)
  ...
  test("fill ambiguous field for join operation") {
    ...
    checkError(
      ...
      parameters = Map("name" -> "`f2`", "referenceNames" -> "[`f2`, `f2`]"))
  }
  ...
  test("replace with null") {
    val input = Seq[(String, java.lang.Double, java.lang.Boolean)](
      ("Bob", 176.5, true),
      ("Alice", 164.3, false),
      ("David", null, true)).toDF("name", "height", "married")

    // Replace String with String and null
    checkAnswer(
      input.na.replace("name", Map("Bob" -> "Bravo", "Alice" -> null)),
    ...
    checkAnswer(
      input.na.replace("height", Map[Any, Any](164.3 -> null)),
      Row("Bob", 176.5, true) ::
  ...
  test("SPARK-29890: duplicate names are allowed for fill() if column names are not specified.") {
    ...
    checkError(
      ...
      condition = "AMBIGUOUS_REFERENCE",
      parameters = Map("name" -> "`col2`", "referenceNames" -> "[`col2`, `col2`]"))

    // If column names are not specified, fill() is applied to all the eligible columns.
    checkAnswer(df.na.fill("hello"), Row("1", "hello", "2") :: Row("3", "4", "hello") :: Nil)
  }
  ...
  test("SPARK-30065: duplicate names are allowed for drop() if column names are not specified.") {
    ...
    checkAnswer(df.na.drop("any"), Row("5", "6", "6") :: Nil)
  }
  ...
  test("SPARK-34417: test fillMap() for column with a dot in the name") {
    val na = "n/a"
    checkAnswer(
      Seq(("abc", 23L), ("def", 44L), (null, 0L))
        .toDF("ColWith.Dot", "Col")
        .na
        .fill(Map("`ColWith.Dot`" -> na)),
  ...
  test("SPARK-34417: test fillMap() for qualified-column with a dot in the name") {
    ...
    checkAnswer(
      Seq(("abc", 23L), ("def", 44L), (null, 0L))
        ...
        .as("testDF")
        .na
        .fill(Map("testDF.`ColWith.Dot`" -> na)),
      Row("abc", 23) :: Row("def", 44L) :: Row(na, 0L) :: Nil)
  }

  test(
    "SPARK-34417: test fillMap() for column without a dot in the name" +
      " and dataframe with another column having a dot in the name") {
    val na = "n/a"
    checkAnswer(
      Seq(("abc", 23L), ("def", 44L), (null, 0L))
        .toDF("Col", "ColWith.Dot")
        .na
        .fill(Map("Col" -> na)),
      Row("abc", 23) :: Row("def", 44L) :: Row(na, 0L) :: Nil)
  }
  ...
  test("SPARK-34649: replace value of a column with dot in the name") {
    checkAnswer(
      Seq(("abc", 23), ("def", 44), ("n/a", 0))
        .toDF("Col.1", "Col.2")
        .na
        .replace("`Col.1`", Map("n/a" -> "unknown")),
  ...
  test("SPARK-34649: replace value of a qualified-column with dot in the name") {
    checkAnswer(
      Seq(("abc", 23), ("def", 44), ("n/a", 0))
        ...
        .as("testDf")
        .na
        .replace("testDf.`Col.1`", Map("n/a" -> "unknown")),
      Row("abc", 23) :: Row("def", 44L) :: Row("unknown", 0L) :: Nil)
  }
