...
import org.apache.spark.sql.catalyst.{ExtendedAnalysisException, QueryPlanningTracker, TableIdentifier}
import org.apache.spark.sql.catalyst.analysis.{AnalysisContext, Analyzer, UnresolvedAttribute, UnresolvedStar}
...
class HybridAnalyzerSuite extends QueryTest with SharedSparkSession {
  ...
  private val malformedUnresolvedPlan: LogicalPlan =
    Project(Seq(UnresolvedAttribute("nonexistent_col")), LocalRelation(col1Integer))
  private val resolvedPlan =
    Project(Seq(col1Integer), LocalRelation(Seq(col1Integer)))
  private val malformedResolvedPlan: LogicalPlan =
    Project(Seq(col1Integer), Project(Seq(col1Integer), LocalRelation(Seq(col1Integer))))
  ...
  test("Fixed-point analyzer passes, single-pass analyzer fails with Stack Overflow") {
    intercept[StackOverflowError](
      new HybridAnalyzer(
        new ValidatingAnalyzer(bridgeRelations = true),
        new ResolverGuard(spark.sessionState.catalogManager),
        new BrokenResolver(new StackOverflowError("Stack Overflow"), bridgeRelations = true),
        new QueryPlanningTracker).apply(unresolvedPlan))
  }
    ...
    checkError(
      exception = intercept[AnalysisException](
        new HybridAnalyzer(
          ...
          new QueryPlanningTracker).apply(malformedUnresolvedPlan)),
  ...
  test("Both fixed-point and single-pass analyzers fail") {
    checkError(
      ...
      parameters = Map("objectName" -> "`nonexistent_col`", "proposal" -> "`col1`"))
  }
  ...
  test("Missing metadata in output schema") {
    val plan: LogicalPlan =
      Project(Seq(UnresolvedAttribute("col2")), LocalRelation(col2IntegerWithMetadata))
    val resolvedPlan =
      Project(Seq(col2Integer), LocalRelation(Seq(col2Integer)))
    checkError(
      exception = intercept[AnalysisException](
        new HybridAnalyzer(
          new ValidatingAnalyzer(bridgeRelations = true),
          new ResolverGuard(spark.sessionState.catalogManager),
          new HardCodedResolver(resolvedPlan, bridgeRelations = true),
          new QueryPlanningTracker).apply(plan)),
  ...
  test("Broken plan normalization") {
    intercept[Exception] {
      new HybridAnalyzerWithBrokenPlanNormalization(
        new ValidatingAnalyzer(bridgeRelations = true),
        new ResolverGuard(spark.sessionState.catalogManager),
        new HardCodedResolver(resolvedPlan, bridgeRelations = true)).apply(unresolvedPlan)
    }
  }
  ...
  test("Fixed-point only run") {
    val plan = Project(Seq(UnresolvedStar(None)), LocalRelation(col1Integer))
    val resolvedPlan = Project(Seq(col1Integer), LocalRelation(Seq(col1Integer)))
    assertPlansEqual(
  ...
  test("Nested invocations") {
    ...
    assertPlansEqual(
      new HybridAnalyzer(
        new CustomAnalyzer(customCode = () => { nestedAnalysis() }, bridgeRelations = true),
        new ResolverGuard(spark.sessionState.catalogManager),
        new ValidatingResolver(bridgeRelations = true),
        new QueryPlanningTracker).apply(plan),
      resolvedPlan)
  ...
  test("Extended resolution checks are enabled/disabled") {
    ...
    intercept[Exception] {
      new HybridAnalyzer(
        ...
        tracker = new QueryPlanningTracker,
        extendedResolutionChecks = Seq(new BrokenCheckRule)).apply(plan)
    }
