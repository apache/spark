...
abstract class DeleteFromTableSuiteBase extends RowLevelOperationSuiteBase {
  ...
  test("delete from table containing added column with default value") {
    ...
    checkAnswer(
      ...
      Seq(
        ...
        Row(4, "hr", "initial-text"),
        Row(6, "hr", "new-text")))
    checkDeleteMetrics(numDeletedRows = 2, numCopiedRows = 3, groupFilterTimeMissing = deltaDelete)
  }
  ...
  test("delete from table with table constraints") {
    ...
    checkDeleteMetrics(numDeletedRows = 1, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)

    sql(s"DELETE FROM $tableNameAsString WHERE pk >=3")
    checkAnswer(
      sql(s"SELECT * FROM $tableNameAsString"),
      Seq(Row(2, 4, "eng")))
    checkDeleteMetrics(numDeletedRows = 1, numCopiedRows = 1, groupFilterTimeMissing = deltaDelete)
  }
  ...
  test("delete from empty tables") {
    ...
    checkDeleteMetrics(numDeletedRows = 0, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)
  }
  ...
  test("delete with aliases") {
    ...
    checkDeleteMetrics(numDeletedRows = 2, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)
  }
  ...
  test("delete with IN subqueries") {
    withTempView("deleted_id", "deleted_dep") {
      ...
      checkDeleteMetrics(
        numDeletedRows = 1, numCopiedRows = 1, groupFilterTimeMissing = deltaDelete)
      ...
      checkDeleteMetrics(
        numDeletedRows = 1, numCopiedRows = 3, groupFilterTimeMissing = deltaDelete)
    }
  }
  ...
  test("delete with NOT IN subqueries") {
    withTempView("deleted_id", "deleted_dep") {
      ...
      checkDeleteMetrics(
        numDeletedRows = 0, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)
      ...
      checkDeleteMetrics(
        numDeletedRows = 2, numCopiedRows = 1, groupFilterTimeMissing = deltaDelete)
      ...
      checkDeleteMetrics(
        numDeletedRows = 3, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)
  ...
  test("delete with EXISTS subquery") {
    withTempView("deleted_id", "deleted_dep") {
      ...
      checkDeleteMetrics(
        numDeletedRows = 1, numCopiedRows = 0, groupFilterTimeMissing = deltaDelete)
  ...
  test("delete with NOT IN over empty subquery") {
    withTempView("empty_subq") {
      ...
      // scan node.
      checkDeleteMetrics(
        numDeletedRows = if (deltaDelete) 3 else -1,
        numCopiedRows = 0,
        groupFilterTimeMissing = true)
    }
  }
