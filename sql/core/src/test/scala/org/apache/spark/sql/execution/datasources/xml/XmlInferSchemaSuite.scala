...
import org.apache.spark.sql.types.{ArrayType, BooleanType, DecimalType, DoubleType, IntegerType, LongType, StringType, StructField, StructType}
...
class XmlInferSchemaSuite
  ...
  override protected def customSQLConf: Map[String, String] = Map.empty
    ...
    val expectedSchema = StructType(
      StructField(
        ...
        StructField(
          "arrayOfArray2",
          ArrayType(
            StructType(StructField("item", ArrayType(DecimalType(21, 1), true)) :: Nil),
            true)) ::
  ...
  test("complex arrays") {
    ...
    checkAnswer(
      ...
      Row(Seq(Row(Seq(Row(Seq(5)))), Row(Seq(Row(Seq(6, 7)), Row(Seq(8)))))) :: Nil)
            ...
            ArrayType(new StructType().add(
              ...
              ArrayType(
                new StructType()
                  ...
                  .add(
                    "inner3",
                    ArrayType(new StructType().add("inner4", ArrayType(LongType))))))))))
    ...
    checkAnswer(
      ...
      Row(
        Seq(
          ...
          Row(Seq(Row(Seq(Row(null, null, Seq(Row(Seq(2, 3)), Row(null))))))))) :: Nil)
  }
    ...
    val expectedSchema = StructType(
      StructField(
        ...
        StructField(
          "field2",
          ArrayType(new StructType()
            .add("array1", ArrayType(StructType(StructField("Test", LongType) :: Nil))))) :: Nil)
    val expectedAns = Seq(
  ...
  test("turn non-nullable schema into a nullable schema") {
    ...
    val missingFieldInputDS =
      spark.createDataset(spark.sparkContext.parallelize(missingFieldInput :: Nil))(
        Encoders.STRING)
    // XML filed is null.
    ...
    Seq(missingFieldInputDS, nullValueInputDS).foreach { xmlStringDS =>
      ...
      withSQLConf(SQLConf.LEGACY_RESPECT_NULLABILITY_IN_TEXT_DATASET_CONVERSION.key -> "true") {
        checkAnswer(
          spark.read
            .schema(StructType(StructField("c1", LongType, nullable = false) ::
  ...
  test("XML with partitions") {
    def makePartition(rows: Seq[String], parent: File, partName: String, partValue: Any): File = {
      ...
      (Seq("<ROWS>") ++ rows ++ Seq("</ROWS>"))
        .toDF("data")
        .coalesce(1)
        .write
        .text(p.getAbsolutePath)
      p
    }
  ...
  test("value tags - multiple lines") {
    ...
    val expectedAns = Seq(Row(Seq("value1", "value2"), 1), Row(Seq("value3\n    value4"), 1))
    assert(xmlDF.schema == expectedSchema)
    checkAnswer(xmlDF, expectedAns)
  }
  ...
  test("value tags - around structs") {
    ...
    assert(xmlDF.schema == expectedSchema)
    val expectedAns = Seq(
      Row(Seq("value1", "value5"), Row(Seq("value2", "value4"), Row(3, 1))),
      Row(Seq("value6"), Row(Seq("value4", "value5"), Row(null, null))),
      Row(Seq("value1", "value5"), Row(Seq("value2", "value4"), Row(3, null))),
      Row(Seq("value1"), Row(Seq("value2", "value4"), Row(3, null))))
    checkAnswer(xmlDF, expectedAns)
  }
  ...
  test("value tags - around arrays") {
    ...
    val expectedAns = Seq(
      ...
      Row(null, Seq(Row(Seq("value1"), null))),
      Row(Seq("value1"), Seq(Row(null, Seq(Row(Seq("1"), null))))))
    checkAnswer(xmlDF, expectedAns)
  }
  ...
  test("value tag - comments") {
    ...
    val expectedAns = Seq(Row(2, Seq(Row(null), Row(1))))
    assert(xmlDF.schema === expectedSchema)
    checkAnswer(xmlDF, expectedAns)
  }
...
trait XmlSchemaInferenceCaseSensitivityTests extends QueryTest {
  ...
  private val valueTagCaseSensitivityTestcase: XmlSchemaInferenceCaseSensitiveTestCase = {
    ...
    XmlSchemaInferenceCaseSensitiveTestCase(
      ...
      expectedCaseSensitiveAns = Seq(Row(null, Row(1, 2)), Row(3, null)),
      expectedCaseInsensitiveSchema = new StructType()
        .add("a", new StructType().add("_VALUE", LongType).add("b", LongType)),
      expectedCaseInsensitiveAns = Seq(Row(Row(1, 2)), Row(Row(3, null))))
  }
      ...
      expectedCaseSensitiveSchema = new StructType()
        ...
        .add(
          ...
          new StructType()
            ...
            .add("b", LongType)
            .add("c", LongType)),
      expectedCaseSensitiveAns = Seq(Row(4, Row(1, 2, 3)), Row(5, null)),
      expectedCaseInsensitiveSchema = new StructType()
        .add(
          "a",
          ArrayType(
            new StructType()
              ...
              .add("b", LongType)
              .add("c", LongType))),
      expectedCaseInsensitiveAns =
        Seq(Row(List(Row(1, 2, 3), Row(4, null, null))), Row(List(Row(5, null, null)))))
  }
  ...
  private val arraySimpleCaseSensitivityTestcase: XmlSchemaInferenceCaseSensitiveTestCase = {
    ...
    XmlSchemaInferenceCaseSensitiveTestCase(
      ...
      expectedCaseSensitiveSchema = new StructType()
        ...
        .add("a", new StructType().add("b", LongType).add("c", LongType)),
      expectedCaseSensitiveAns = Seq(Row(Row(3, 4), Row(1, 2))),
      expectedCaseInsensitiveSchema = new StructType()
        .add("a", ArrayType(new StructType().add("b", LongType).add("c", LongType))),
      expectedCaseInsensitiveAns = Seq(Row(List(Row(1, 2), Row(3, 4)))))
  }
  ...
  private val primitiveTypeCaseSensitivityTestcase: XmlSchemaInferenceCaseSensitiveTestCase = {
    ...
    XmlSchemaInferenceCaseSensitiveTestCase(
      ...
      expectedCaseSensitiveSchema = new StructType()
        ...
        .add("b", LongType)
        .add("c", LongType),
      expectedCaseSensitiveAns = Seq(Row(null, 1, 2, 3), Row(5, 4, null, 6)),
      expectedCaseInsensitiveSchema = new StructType()
        .add("a", LongType)
        .add("b", LongType)
        .add("c", LongType),
      expectedCaseInsensitiveAns = Seq(Row(1, 2, 3), Row(4, 5, 6)))
  }
  ...
  private val attributesCaseSensitivityTestcase: XmlSchemaInferenceCaseSensitiveTestCase = {
    ...
    XmlSchemaInferenceCaseSensitiveTestCase(
      ...
      expectedCaseSensitiveSchema = new StructType()
        ...
        .add("b", LongType)
        .add("c", LongType),
      expectedCaseSensitiveAns = Seq(Row(null, 1, 1, 2, 3), Row(2, null, 4, 5, 6)),
      expectedCaseInsensitiveSchema = new StructType()
        .add("_attr", LongType)
        .add("a", LongType)
        .add("b", LongType)
        .add("c", LongType),
      expectedCaseInsensitiveAns = Seq(Row(1, 1, 2, 3), Row(2, 4, 5, 6)))
  }
      ...
      expectedCaseSensitiveSchema = new StructType()
        ...
        .add(
          ...
          new StructType()
            .add("A", LongType)
            .add("c", LongType)),
      expectedCaseSensitiveAns = Seq(Row(Row(1, 3), null), Row(null, Row(5, 7))),
      expectedCaseInsensitiveSchema = new StructType()
        .add(
          "A",
          new StructType()
            .add("a", LongType)
            .add("c", LongType)),
      expectedCaseInsensitiveAns = Seq(Row(Row(1, 3)), Row(Row(5, 7))))
  }
