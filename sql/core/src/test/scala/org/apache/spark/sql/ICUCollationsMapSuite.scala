...
/**
 * Guard against breaking changes in ICU locale names and codes supported by Collator class and
 * provider by CollationFactory. Map is in form of rows of pairs (locale name, locale id); locale
 * name consists of three parts:
...
class ICUCollationsMapSuite extends SparkFunSuite {
  ...
  private val collationsMapFile = {
    getWorkspaceFilePath(
      ...
      "resources",
      "collations",
      "ICU-collations-map.md").toFile
  }

  if (regenerateGoldenFiles) {
    val map = CollationFactory.getICULocaleNames
    val mapOutput = map.zipWithIndex
      .map { case (localeName, idx) =>
        s"| $idx | $localeName |"
      }
      .mkString("\n")
    val goldenOutput = {
  ...
  test("ICU locales map breaking change") {
    ...
    val input =
      goldenRelevantLines.map(s => (s.split('|')(2).strip(), s.split('|')(1).strip().toInt))
