...
class TPCDSQueryTestSuite extends QueryTest with TPCDSBase with SQLQueryTestHelper {
  ...
  private def runQuery(query: String, goldenFile: File): Unit = {
    ...
    val queryOutput = QueryOutput(query.trim, schema, output.mkString("\n").replaceAll("\\s+$", ""))
    ...
    val expectedOutput = {
      ...
      assert(segments.size == 4,
        s"Expected 4 blocks in result file but got ${segments.size}. " +
          s"Try regenerate the result files.")
      QueryOutput(
        sql = segments(1).trim,
        schema = segments(2).trim,
        output = segments(3).trim
      )
    }

    assertResult(expectedOutput.sql,
      s"SQL query did not match\n${expectedOutput.sql}") {
      queryOutput.sql
    }
    assertResult(expectedOutput.schema,
      s"Schema did not match\n${expectedOutput.sql}: $output") {
      queryOutput.schema
    }
