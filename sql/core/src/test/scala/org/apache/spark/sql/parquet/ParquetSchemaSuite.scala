...
class ParquetSchemaSuite extends SparkFunSuite with ParquetTest {
  ...
  private def testSchema[T <: Product: ClassTag: TypeTag](
    ...
    test(s"sql <= parquet: $testName") {
      val converter = new CatalystSchemaConverter(binaryAsString, int96AsTimestamp)
      val actual = converter.convert(MessageTypeParser.parseMessageType(messageType))
      val expected = StructType.fromAttributes(ScalaReflection.attributesFor[T])
      assert(
        actual === expected,
        s"""Schema mismatch.
