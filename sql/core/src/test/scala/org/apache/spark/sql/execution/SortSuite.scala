...
class SortSuite extends SparkPlanTest with SharedSparkSession {
  ...
  test("basic sorting using ExternalSort") {
    ...
    checkAnswer(
      input.toDF("a", "b", "c"),
      (child: SparkPlan) => SortExec("a".attr.asc :: "b".attr.asc :: Nil,
    ...
    checkAnswer(
      input.toDF("a", "b", "c"),
      (child: SparkPlan) => SortExec("b".attr.asc :: "a".attr.asc :: Nil,
        global = true, child = child),
  ...
  test("sorting all nulls") {
    checkThatPlansAgree(
      ...
      (child: SparkPlan) =>
        GlobalLimitExec(10, SortExec("a".attr.asc :: Nil, global = true, child = child)),
      (child: SparkPlan) =>
        GlobalLimitExec(10, ReferenceSort("a".attr.asc :: Nil, global = true, child)),
      sortAnswers = false
    )
  }
  ...
  test("sorting updates peak execution memory") {
    AccumulatorSuite.verifyPeakExecutionMemorySet(sparkContext, "unsafe external sort") {
      checkThatPlansAgree(
        (1 to 100).map(v => Tuple1(v)).toDF("a"),
        (child: SparkPlan) => SortExec("a".attr.asc :: Nil, global = true, child = child),
        (child: SparkPlan) => ReferenceSort("a".attr.asc :: Nil, global = true, child),
        sortAnswers = false)
    }
  }
  ...
  test("SPARK-33260: sort order is a Stream") {
    ...
    checkAnswer(
      input.toDF("a", "b", "c"),
      (child: SparkPlan) => SortExec(Stream("a".attr.asc, "b".attr.asc, "c".attr.asc),
        global = true, child = child),
  ...
  for (
    ...
    sortOrder <-
      Seq("a".attr.asc :: Nil, "a".attr.asc_nullsLast :: Nil, "a".attr.desc :: Nil,
        "a".attr.desc_nullsFirst :: Nil);
