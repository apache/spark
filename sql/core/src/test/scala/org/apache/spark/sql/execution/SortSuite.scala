...
class SortSuite extends SparkPlanTest with SharedSQLContext {
  ...
  test("sort followed by limit") {
    checkThatPlansAgree(
      ...
      (child: SparkPlan) => GlobalLimitExec(10, SortExec('a.asc :: Nil, global = true, child = child)),
      (child: SparkPlan) => GlobalLimitExec(10, ReferenceSort('a.asc :: Nil, global = true, child)),
      sortAnswers = false
    )
  }
