...
class DynamicPartitionPruningSuite
  ...
  test("Subquery reuse across all subquery levels") {
    Seq(false, true).foreach { dppReuseBroadcast =>
      withSQLConf(SQLConf.DYNAMIC_PARTITION_PRUNING_ENABLED.key -> "true",
        SQLConf.DYNAMIC_PARTITION_PRUNING_REUSE_BROADCAST.key -> dppReuseBroadcast.toString) {
        withTable("df1", "df2") {
          spark.range(1000)
            .select(col("id"), col("id").as("k"))
            .write
          ...
          checkPartitionPruningPredicate(df, !dppReuseBroadcast, dppReuseBroadcast)
          ...
          assert(countSubqueries == (if (dppReuseBroadcast) 0 else 1) + 1)
