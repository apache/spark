...
@SlowSQLTest
class StreamingDeduplicationSuite
  ...
  test("deduplicate with aggregate - append mode") {
    val inputData = MemoryStream[Int]
    val windowedaggregate = inputData
      .toDS()
      .withColumn("eventTime", timestamp_seconds($"value"))
      .withWatermark("eventTime", "10 seconds")
      .dropDuplicates()
    ...
    testStream(windowedaggregate)(
      ...
      CheckLastBatch(
        (10 -> 5)
      ), // 5 items (10 to 14) after deduplicate, emitted with no-data-batch
  ...
  test("SPARK-21546: dropDuplicates should ignore watermark when it's not a key") {
    ...
    testStream(df)(AddData(input, 1 -> 1, 1 -> 2, 2 -> 2), CheckAnswer(1 -> 1, 2 -> 2))
  }
  ...
  test("test no-data flag") {
    ...
    def testWithFlag(flag: Boolean): Unit = withClue(s"with $flagKey = $flag") {
      ...
      testStream(result, Append)(
        ...
        assertNumStateRows(total = 6, updated = 6),
        AddData(inputData, 25), // Advance watermark to 15 seconds
        CheckNewAnswer(25), { // State should have been cleaned if flag is set, otherwise should not have been cleaned
  ...
  test(
    "SPARK-29438: ensure UNION doesn't lead streaming deduplication to use" +
  ...
  test("SPARK-35880: custom metric numDroppedDuplicateRows in state operator progress") {
    ...
    val dedupeWithWatermark = dedupeWithWMInputData
      .toDS()
      .withColumn("eventTime", timestamp_seconds($"value"))
      .withWatermark("eventTime", "10 seconds")
      .dropDuplicates()
  ...
  Seq("3.3.0", "3.5.1").foreach { sparkVersion =>
    test(
      "SPARK-39650: recovery from checkpoint having all columns as value schema " +
  ...
  Seq("3.3.0", "3.5.1").foreach { sparkVersion =>
    test(
      "SPARK-39650: recovery from checkpoint with changes on key schema " +
  ...
  test("collation aware deduplication") {
    ...
    val result = inputData
      ...
      .select(
        col("_1")
          .try_cast(StringType("UTF8_BINARY"))
          .as("str"),
        col("_2").as("int"))
      .dropDuplicates("str")
  ...
  test("non-binary collation aware deduplication not supported") {
    ...
    val result = inputData
      ...
      .select(
        col("value")
          .try_cast(StringType("UTF8_LCASE"))
          .as("str"))
      .dropDuplicates("str")

    val ex = intercept[StreamingQueryException] {
      testStream(result, Append)(AddData(inputData, "a"), CheckLastBatch("a"))
    }

    checkError(
      ex.getCause.asInstanceOf[SparkUnsupportedOperationException],
      condition = "STATE_STORE_UNSUPPORTED_OPERATION_BINARY_INEQUALITY",
      parameters = Map("schema" -> ".+\"str\":\"spark.UTF8_LCASE\".+"),
      matchPVals = true)
  }
  ...
  test("test that avro encoding is not supported") {
    ...
    val ex = intercept[Exception] {
      withSQLConf(SQLConf.STREAMING_STATE_STORE_ENCODING_FORMAT.key -> "avro") {
        testStream(result, Append)(AddData(inputData, "a"), ProcessAllAvailable())
      }
    }
  ...
  test("Partition key extraction - Dedupe") {
    val df = (input: Dataset[(String, Int)]) => input.dropDuplicates("_1").toDF()
  ...
  protected def testPartitionKeyExtraction(
    ...
    withTempDir { checkpointDir =>
      ...
      def createExpectedKeyRow(inputKey: String) = {
        val row = new GenericInternalRow(Array[Any](UTF8String.fromString(inputKey)))
      ...
      try {
        val extractor = StatePartitionKeyExtractorFactory.create(operatorName, keySchema)
        ...
        // Verify each partition key equals its corresponding state key (for dedup)
