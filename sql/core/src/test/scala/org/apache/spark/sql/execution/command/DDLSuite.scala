...
import org.apache.spark.sql.catalyst.analysis.{FunctionRegistry, NoSuchDatabaseException, NoSuchPartitionException, NoSuchTableException, TempTableAlreadyExistsException, TempViewAlreadyExistsException}
...
abstract class DDLSuite extends QueryTest with SQLTestUtils {
  ...
  test("block creating duplicate temp table") {
    withTempView("t_temp") {
      sql("CREATE TEMPORARY TABLE t_temp USING parquet AS SELECT 1, 2")
      val e = intercept[TempTableAlreadyExistsException] {
        sql("CREATE TEMPORARY TABLE t_temp (c3 int, c4 string) USING JSON")
      }.getMessage
      assert(e.contains("Temporary table 'default.t_temp' already exists"))
    }
  }
