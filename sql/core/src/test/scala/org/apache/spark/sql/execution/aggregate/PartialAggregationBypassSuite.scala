...
abstract class PartialAggregationBypassSuiteBase
    extends QueryTest
  ...
  test("results are identical with and without partial aggregation - SUM with FILTER") {
    val data = spark.range(1000).selectExpr("id % 7 as k", "id as v")
    withTempView("filter_test_data") {
      data.createOrReplaceTempView("filter_test_data")
      val expected = sql(
        "SELECT k, SUM(v) FILTER (WHERE v > 3) FROM filter_test_data GROUP BY k ORDER BY k")
        .collect()
      // Sanity-check: the filter must actually change the result, otherwise the test cannot
      // detect a bug where the filter is silently dropped in Complete mode.
      val unfiltered = sql(
        "SELECT k, SUM(v) FROM filter_test_data GROUP BY k ORDER BY k")
        .collect()
      assert(expected.zip(unfiltered).exists { case (f, u) => f.get(1) != u.get(1) },
        "filtered and unfiltered sums must differ for the test to be meaningful")
      withSQLConf(SQLConf.BYPASS_PARTIAL_AGGREGATION.key -> "true") {
        val actual = sql(
          "SELECT k, SUM(v) FILTER (WHERE v > 3) FROM filter_test_data GROUP BY k ORDER BY k")
        checkAnswer(actual, expected)
...
class PartialAggregationBypassSuite extends PartialAggregationBypassSuiteBase
  with DisableAdaptiveExecutionSuite

class PartialAggregationBypassSuiteAE extends PartialAggregationBypassSuiteBase
