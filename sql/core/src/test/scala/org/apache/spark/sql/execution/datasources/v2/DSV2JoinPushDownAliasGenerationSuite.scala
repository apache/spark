...
class DSV2JoinPushDownAliasGenerationSuite extends SparkFunSuite {
  ...
  private def assertAliases(
      ...
      expectedRight: Array[ColumnWithAlias]): Unit = {
    ...
    assert(
      (actualLeft ++ actualRight).map(uniqName).distinct.length
  ...
  test("Basic case with no duplicate column names") {
    assertAliases(
      ...
      expectedLeft = Array(new ColumnWithAlias("id", null), new ColumnWithAlias("name", null)),
      expectedRight =
        Array(new ColumnWithAlias("email", null), new ColumnWithAlias("phone", null)))
  }
  ...
  test("Extreme duplication scenarios") {
    assertAliases(
      ...
      expectedRight = Array(new ColumnWithAlias("id", "id_3"), new ColumnWithAlias("id", "id_4")))
  }
  ...
  test("Exact duplicate column names") {
    assertAliases(
      ...
      expectedLeft = Array(new ColumnWithAlias("id", null), new ColumnWithAlias("name", null)),
      expectedRight =
        Array(new ColumnWithAlias("id", "id_1"), new ColumnWithAlias("name", "name_1")))
  }
