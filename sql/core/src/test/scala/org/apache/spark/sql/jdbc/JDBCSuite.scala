...
class JDBCSuite extends FunSuite with BeforeAndAfter {
  ...
  before {
    ...
    conn.prepareStatement("insert into test.people values ('''joe'' \"armstrong\"', 3)").executeUpdate()
    conn.commit()
  ...
  test("SELECT * WHERE (quoted strings)") {
    ...
    assert(sql("SELECT * FROM foobar WHERE NAME > 'fred'").collect().size == 2)
    assert(sql("SELECT * FROM foobar WHERE NAME != 'fred'").collect().size == 2)
    assert(sql("SELECT * FROM foobar WHERE NAME = '''joe'' \"armstrong\"'").collect().size == 1)
  }
  ...
  test("SELECT first field") {
    ...
    assert(names(1).equals("'joe' \"armstrong\""))
