...
case class MapInputEvent(key: String, userKey: String, action: String, value: Int)
...
class MapStateTTLProcessor(ttlConfig: TTLConfig)
  ...
  private def processRow(
    ...
    if (row.action == "get") {
      if (mapState.containsKey(userKey)) {
        results = MapOutputEvent(
          key,
          userKey,
          mapState.getValue(userKey),
          isTTLValue = false,
          -1) :: results
      }
    } else if (row.action == "get_without_enforcing_ttl") {
      ...
      ttlValues.foreach { elem =>
        results =
          MapOutputEvent(key, elem._1, -1, isTTLValue = true, ttlValue = elem._2) :: results
      }
  ...
  test("verify iterator doesn't return expired keys") {
    withSQLConf(
      ...
      testStream(result)(
        ...
        AdvanceManualClock(1 * 1000),
        CheckNewAnswer(MapOutputEvent("k1", "key3", 3, isTTLValue = true, 126000)),
        assertNumStateRows(total = 3, updated = 0),
        StopStream)
    }
  }
