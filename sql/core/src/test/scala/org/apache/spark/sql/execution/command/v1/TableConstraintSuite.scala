 ...
 * FOREIGN KEY) that check V1 table catalogs. V1 tables do not support table constraints. The
 * tests that cannot run for all V1 catalogs are located in more specific test suites:
 *
 *   - V1 In-Memory catalog: `org.apache.spark.sql.execution.command.v1.TableConstraintSuite`
...
trait TableConstraintSuiteBase extends DDLCommandTestUtils {
  ...
  private val constraintTypes =
    Seq("CHECK (id > 0)", "PRIMARY KEY (id)", "UNIQUE (id)", "FOREIGN KEY (id) REFERENCES t2(id)")
    ...
    constraint =>
      withNamespaceAndTable("ns", "table_1") { t =>
        ...
        checkError(
          ...
          parameters =
            Map("tableName" -> s"`$catalog`.`ns`.`table_1`", "operation" -> "CONSTRAINT"))
      }
  }

  gridTest("SPARK-54761: alter table add constraint - should fail")(constraintTypes) {
    constraint =>
      withNamespaceAndTable("ns", "table_1") { t =>
        ...
        checkError(
          ...
          condition = "UNSUPPORTED_FEATURE.TABLE_OPERATION",
          parameters =
            Map("tableName" -> s"`$catalog`.`ns`.`table_1`", "operation" -> "ADD CONSTRAINT"))
      }
  }
  ...
  test("SPARK-54761: alter table drop constraint - should fail") {
    withNamespaceAndTable("ns", "table_1") { t =>
      ...
      checkError(
        ...
        condition = "UNSUPPORTED_FEATURE.TABLE_OPERATION",
        parameters =
          Map("tableName" -> s"`$catalog`.`ns`.`table_1`", "operation" -> "DROP CONSTRAINT"))
    }
  }
  ...
  gridTest("SPARK-54761: replace table with constraint - should fail")(constraintTypes) {
    constraint =>
      withNamespaceAndTable("ns", "table_1") { t =>
        val replaceTableSql =
          s"REPLACE TABLE $t (id INT, CONSTRAINT c1 $constraint) $defaultUsing"
        val error = intercept[AnalysisException] {
          sql(replaceTableSql)
        }
        checkError(
          ...
          condition = "UNSUPPORTED_FEATURE.TABLE_OPERATION",
          parameters =
            Map("tableName" -> s"`$catalog`.`ns`.`table_1`", "operation" -> "REPLACE TABLE"))
      }
  }
}
