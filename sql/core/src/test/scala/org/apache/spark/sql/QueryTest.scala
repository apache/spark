...
trait QueryTestBase extends PlanTestBase
    ...
    with test.SQLTestData
    with classic.ClassicConversions { self: Suite =>

  protected def sparkContext = spark.sparkContext

  // Shorthand for running a query using our SparkSession
  protected lazy val sql: String => classic.DataFrame =
    (s: String) => spark.asInstanceOf[classic.SparkSession].sql(s)
