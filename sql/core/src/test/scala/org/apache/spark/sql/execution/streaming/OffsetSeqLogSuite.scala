...
class OffsetSeqLogSuite extends SharedSparkSession {
  ...
  test("OffsetSeqMetadata - deserialization") {
    ...
    // One set
    assert(
      new OffsetSeqMetadata(1, 0, Map.empty) ===
        OffsetSeqMetadata("""{"batchWatermarkMs":1}"""))
    assert(
      new OffsetSeqMetadata(0, 2, Map.empty) ===
        OffsetSeqMetadata("""{"batchTimestampMs":2}"""))
    assert(
      OffsetSeqMetadata(0, 0, getConfWith(shufflePartitions = 2)) ===
        OffsetSeqMetadata(s"""{"conf": {"$key":2}}"""))

    // Two set
    assert(
      new OffsetSeqMetadata(1, 2, Map.empty) ===
        OffsetSeqMetadata("""{"batchWatermarkMs":1,"batchTimestampMs":2}"""))
    assert(
      OffsetSeqMetadata(1, 0, getConfWith(shufflePartitions = 3)) ===
        OffsetSeqMetadata(s"""{"batchWatermarkMs":1,"conf": {"$key":3}}"""))
    assert(
      OffsetSeqMetadata(0, 2, getConfWith(shufflePartitions = 3)) ===
        OffsetSeqMetadata(s"""{"batchTimestampMs":2,"conf": {"$key":3}}"""))

    // All set
    assert(
      OffsetSeqMetadata(1, 2, getConfWith(shufflePartitions = 3)) ===
  ...
  test("offset log records defaults to unsafeRow for store encoding format") {
    val offsetSeqMetadata =
      OffsetSeqMetadata.apply(batchWatermarkMs = 0, batchTimestampMs = 0, spark.conf)
    assert(
      offsetSeqMetadata.conf.get(SQLConf.STREAMING_STATE_STORE_ENCODING_FORMAT.key) ===
        Some("unsaferow"))
  }

  test("offset log uses the store encoding format set in the conf") {
    val offsetSeqMetadata = OffsetSeqMetadata.apply(
      batchWatermarkMs = 0,
      batchTimestampMs = 0,
  ...
  private def verifyOffsetLogEntry(
    ...
    if (entryExists) {
      val encodingFormatOpt =
        offsetSeqMetadata.conf.get(SQLConf.STREAMING_STATE_STORE_ENCODING_FORMAT.key)
      assert(
        encodingFormatOpt.isDefined,
        "No store encoding format found in the offset log entry")
    ...
    test(s"verify format values from checkpoint loc - $storeEncodingFormat") {
      withTempDir { checkpointDir =>
        val resourceUri = this.getClass
          .getResource(
            "/structured-streaming/checkpoint-version-4.0.0-tws-" + storeEncodingFormat + "/")
          .toURI
        Utils.copyDirectory(new File(resourceUri), checkpointDir.getCanonicalFile)
        verifyOffsetLogEntry(
  ...
  test("verify format values from old checkpoint with Spark version 3.5.1") {
    withTempDir { checkpointDir =>
      val resourceUri = this.getClass
        .getResource("/structured-streaming/checkpoint-version-3.5.1-streaming-deduplication/")
        .toURI
      Utils.copyDirectory(new File(resourceUri), checkpointDir.getCanonicalFile)
      verifyOffsetLogEntry(checkpointDir.getAbsolutePath, entryExists = false, "unsaferow")
    }
  }
  ...
  test("Row checksum disabled by default") {
    val offsetSeqMetadata =
      OffsetSeqMetadata.apply(batchWatermarkMs = 0, batchTimestampMs = 0, spark.conf)
    assert(
      offsetSeqMetadata.conf.get(SQLConf.STATE_STORE_ROW_CHECKSUM_ENABLED.key) ===
        Some(false.toString))
  }
    ...
    withTempDir { checkpointDir =>
      withSQLConf(SQLConf.STREAMING_OFFSET_LOG_FORMAT_VERSION.key -> "2") {
        ...
        try {
          ...
          assert(
            offsetSeq.isInstanceOf[OffsetMap],
  ...
  test("STREAMING_OFFSET_LOG_FORMAT_VERSION config - default VERSION_1") {
    withTempDir { checkpointDir =>
      ...
      try {
        ...
        val (batchId, offsetSeq) = latestBatch.get
        assert(
          offsetSeq.isInstanceOf[OffsetSeq],
  ...
  Seq((1, 2, classOf[OffsetSeq]), (2, 1, classOf[OffsetMap])).foreach {
    case (startingVersion, restartVersion, expectedClass) =>
  ...
  test("SPARK-55131: offset log records defaults to merge operator version 2") {
    val offsetSeqMetadata =
      OffsetSeqMetadata.apply(batchWatermarkMs = 0, batchTimestampMs = 0, spark.conf)
    assert(
      offsetSeqMetadata.conf.get(SQLConf.STATE_STORE_ROCKSDB_MERGE_OPERATOR_VERSION.key) ===
        Some("2"))
  }
  ...
  test("SPARK-55131: Backward compatibility test with merge operator version") {
    ...
    withTempDir { checkpointDir =>
      val resourceUri = this.getClass
        .getResource("/structured-streaming/checkpoint-version-4.0.0-tws-unsaferow/")
        .toURI
      Utils.copyDirectory(new File(resourceUri), checkpointDir.getCanonicalFile)
  ...
  def testWithOffsetV2(testName: String, testTags: Tag*)(testBody: => Any): Unit = {
