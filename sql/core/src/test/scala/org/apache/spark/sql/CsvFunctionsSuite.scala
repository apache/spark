...
class CsvFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("from_csv with non struct schema") {
    checkError(
      exception = intercept[AnalysisException] {
        Seq("1")
          .toDS()
          .select(from_csv($"value", lit("ARRAY<int>"), Map[String, String]().asJava))
      },
      condition = "INVALID_SCHEMA.NON_STRUCT_TYPE",
      parameters = Map("inputSchema" -> "\"ARRAY<int>\"", "dataType" -> "\"ARRAY<INT>\""),
    ...
    checkError(
      ...
      condition = "INVALID_SCHEMA.NON_STRUCT_TYPE",
      parameters = Map("inputSchema" -> "\"ARRAY<int>\"", "dataType" -> "\"ARRAY<INT>\""),
      context = ExpectedContext(fragment = "from_csv(csv, 'ARRAY<int>')", start = 0, stop = 26))
  }
    ...
    val df2 = df
      .select(
        from_csv(
          ...
          Map("mode" -> "Permissive", "columnNameOfCorruptRecord" -> columnNameOfCorruptRecord)))
  ...
  test("from_csv with option (escape)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row("\"")))
  }
  ...
  test("from_csv with option (comment)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row(null)))
  }
  ...
  test("from_csv with option (ignoreLeadingWhiteSpace)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row("a   ")))
  }
  ...
  test("from_csv with option (nanValue)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row(Float.NaN)))
  }
  ...
  test("from_csv with option (positiveInf)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row(Double.PositiveInfinity)))
  }
  ...
  test("from_csv with option (negativeInf)") {
    ...
    checkAnswer(df.select(from_csv($"value", schema, options)), Row(Row(Double.NegativeInfinity)))
  }
  ...
  test("from_csv with option (maxCharsPerColumn)") {
    ...
    assert(
      exception.contains("Length of parsed input (3) exceeds the maximum number of " +
  ...
  test("support foldable schema by from_csv") {
    ...
    checkError(
      exception = intercept[AnalysisException] {
        Seq(("1", "i int"))
          .toDF("csv", "schema")
          .select(from_csv($"csv", $"schema", options))
          .collect()
      },
  ...
  test("SPARK-35998: Make from_csv/to_csv to handle year-month intervals properly") {
    ...
    Seq(
      ...
      (YearMonthIntervalType(MONTH), "INTERVAL '14' MONTH", Period.of(1, 2, 0))).foreach {
      case (toCsvDtype, toCsvExpected, fromCsvExpected) =>
  ...
  test("SPARK-35999: Make from_csv/to_csv to handle day-time intervals properly") {
    ...
    Seq(
      ...
      (DayTimeIntervalType(DAY), "INTERVAL '1' DAY", Duration.ofDays(1)),
      (
        DayTimeIntervalType(HOUR, SECOND),
        "INTERVAL '26:03:04' HOUR TO SECOND",
      ...
      (DayTimeIntervalType(HOUR), "INTERVAL '26' HOUR", Duration.ofHours(26)),
      (
        DayTimeIntervalType(MINUTE, SECOND),
        "INTERVAL '1563:04' MINUTE TO SECOND",
        Duration.ofMinutes(1563).plusSeconds(4)),
      (DayTimeIntervalType(MINUTE), "INTERVAL '1563' MINUTE", Duration.ofMinutes(1563)),
      (DayTimeIntervalType(SECOND), "INTERVAL '93784' SECOND", Duration.ofSeconds(93784)))
  ...
  test("SPARK-37326: Handle incorrectly formatted timestamp_ntz values in from_csv") {
    val fromCsvDF = Seq("2021-08-12T15:16:23.000+11:00")
      .toDF("csv")
      .select(
        from_csv(
          $"csv",
  ...
  test("SPARK-38955: disable lineSep option in from_csv and schema_of_csv") {
    ...
    val actual =
      df.select(from_csv($"csv", schema_of_csv("1,2\n2"), Map.empty[String, String].asJava))
  ...
  test("SPARK-47497: to_csv support the data of MapType as pretty strings") {
    val rows = new java.util.ArrayList[Row]()
    rows.add(Row(1L, Row(2L, "Alice", Map("math" -> 100L, "english" -> 200L, "science" -> null))))
        ...
        checkAnswer(
          df.select(
            from_csv(
              ...
              Map("header" -> header, "singleVariantColumn" -> "v")).cast("string")),
          Seq(
            Row("""{{"_c0":100,"_c1":1.1}}"""),
      ...
      Seq(largeInput)
        ...
        .select(
          from_csv(
            $"value",
            StructType.fromDDL("v variant, _corrupt_record string"),
            Map("singleVariantColumn" -> "v")).cast("string")),
  ...
  test("from_csv with variant: extreme negative scale decimal does not hang") {
    ...
    checkAnswer(
      df.select(
        from_csv($"value", StructType.fromDDL("a variant"), Map.empty[String, String])
          .cast("string")),
      Seq(Row("""{"1E99999"}""")))
  }
  ...
  test("from_csv/to_csv with TIME type - all precisions") {
    ...
    val customParse = Seq("14-30-45.123456")
      .toDF("csv")
    ...
    val customCsv = customDF
      .select(to_csv(struct($"time"), customOptions.asJava))
      .collect()
      .head
      .getString(0)
  ...
  test("TIME type with nulls") {
    ...
    val parseNull = Seq(null.asInstanceOf[String])
      .toDF("csv")
      .select(from_csv($"csv", schema, Map.empty[String, String]))
      .collect()
      .head
