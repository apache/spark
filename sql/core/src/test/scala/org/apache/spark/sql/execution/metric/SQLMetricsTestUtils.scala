...
trait SQLMetricsTestUtils extends SQLTestUtils {
  ...
  // Pattern of size SQLMetric value, e.g. "96.2 MiB;32.1 MiB;32.1 MiB;32.1 MiB (0.0:4)
  // " OR "96.2 MiB;32.1 MiB;32.1 MiB;32.1 MiB"
  protected val sizeMetricPattern = {
    val bytes = "([0-9]+(\\.[0-9]+)?) (EiB|PiB|TiB|GiB|MiB|KiB|B)"
    val maxMetrics = "\\(([0-9]+)\\.([0-9]+)\\:([0-9]+)\\)"
    s"$bytes;$bytes;$bytes;$bytes ($maxMetrics)?"
  }

  // Pattern of timing SQLMetric value, e.g. "2.0 ms;1.0 ms;1.0 ms;1.0 ms (3.0:217)"
  // OR "2.0 ms;1.0 ms;1.0 ms;1.0 ms)"
  protected val timingMetricPattern = {
    val duration = "([0-9]+(\\.[0-9]+)?) (ms|s|m|h)"
    val maxMetrics = "\\(([0-9]+)\\.([0-9]+)\\:([0-9]+)\\)"
    s"$duration;$duration;$duration;$duration ($maxMetrics)?"
  }

  // Pattern of size SQLMetric value for Aggregate tests.
  // e.g "1;1;0.9 (1.0:8) OR "1;1;0.9"
  protected val aggregateMetricsPattern = {
    val iters = "([0-9]+(\\.[0-9]+)?)"
    val maxMetrics = "\\(([0-9]+)\\.([0-9]+)\\:([0-9]+)\\)"
    s"$iters;$iters;$iters ($maxMetrics)?"
  }
