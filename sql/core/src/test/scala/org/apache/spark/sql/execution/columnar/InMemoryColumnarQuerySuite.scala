...
import org.apache.spark.sql.catalyst.expressions.{Attribute, AttributeReference, AttributeSet, DynamicPruningExpression, In, NamedExpression}
...
class InMemoryColumnarQuerySuite extends SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  test("SPARK-56479: empty DPP build side skips all cached batches") {
    withSQLConf(
        SQLConf.IN_MEMORY_TABLE_SCAN_STATISTICS_ENABLED.key -> "true",
        SQLConf.IN_MEMORY_PARTITION_PRUNING.key -> "true") {
      val df = spark.range(0, 100, 1, 5).toDF("id")
      df.cache()
      try {
        df.count() // materialize the cache
        val mem = df.queryExecution.withCachedData.collect { case CachedRelation(m) => m }.head
        val idAttr = mem.output.head
        // A DPP filter whose build side produced no rows: IN () can never match, so the
        // scan must skip every cached batch via the constant-false batch filter.
        val emptyBuildSide = SubqueryExec(
          "dpp", LocalTableScanExec(Seq(idAttr.newInstance()), Nil, None))
        val inSubquery = InSubqueryExec(idAttr, emptyBuildSide, NamedExpression.newExprId)
        inSubquery.updateResult()
        val scan = InMemoryTableScanExec(mem.output, Nil, mem,
          runtimeFilters = Seq(DynamicPruningExpression(inSubquery)))
        assert(scan.executeCollect().isEmpty)
        assert(scan.readBatches.value === 0, "expected the empty DPP filter to skip all batches")
      } finally {
        df.unpersist()
      }
    }
