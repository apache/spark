 ...
 * mixed-type test at the bottom pins the current behavior of comparisons that need type
...
class TimestampNanosComparisonE2ESuite extends QueryTest with SharedSparkSession {
  ...
  test("literal comparisons: sub-microsecond digits decide") {
    // NTZ: 123456789 vs 123456790 differ only below the microsecond.
  ...
  test("aggregates and filters over a nanos column") {
    checkAnswer(
  ...
  test("joins on nanos equality: values differing only in nanosWithinMicro must not match") {
    val joinQuery =
      s"""
         ...
         |ON a.ts = b.ts
         |""".stripMargin
    // Broadcast join (default for these tiny inputs).
  ...
  test("mixed-type comparisons currently fail as INTERNAL_ERROR (coercion not implemented)") {
    ...
    // SparkException[INTERNAL_ERROR] instead of a proper analysis error. This pins the current
    // behavior and should be rewritten once nanos coercion lands.
    val mixedQueries = Seq(
      "nanos(9) vs nanos(7)" -> s"SELECT ${ntz("123456789")} < ${ntz("1234567")}",
      "nanos(9) vs micros(6)" -> s"SELECT ${ntz("123456789")} < ${ntz("123456")}",
      "nanos NTZ(9) vs nanos LTZ(9)" -> s"SELECT ${ntz("123456789")} < ${ltz("123456789")}",
      "nanos(9) vs DATE" -> s"SELECT DATE'2026-01-01' < ${ntz("123456789")}",
      "IN over mixed nanos precisions" ->
        s"SELECT ${ntz("123456789")} IN (${ntz("1234567")}, ${ntz("123456789")})")
    for ((name, query) <- mixedQueries) {
      val e = intercept[SparkException](sql(query).collect())
      assert(e.getCondition == "INTERNAL_ERROR", s"$name: got condition ${e.getCondition}")
      assert(e.getCause.isInstanceOf[MatchError],
        s"$name: expected MatchError from non-exhaustive findWiderDateTimeType, " +
          s"got: ${e.getCause}")
    }
  }
}
