...
trait CreatePipelineDatasetAsSelectParserSuiteBase extends CommandSuiteBase {
       ...
       """.stripMargin,
        Seq(
          ...
          ColumnDefinition(
            ...
            identityColumnSpec = None,
            metadata = new MetadataBuilder().build()))),
      (s"CREATE $datasetSqlSyntax table1 AS SELECT * FROM input", Seq.empty)).foreach {
      case (query, columnDefs) =>
  ...
  test("Location is unsupported") {
    val ex = intercept[ParseException] {
      parser.parsePlan(s"""CREATE $datasetSqlSyntax table1 LOCATION 'dbfs:/path/to/table'
  ...
  test("Comment is correctly parsed") {
    Seq(
      ...
      (s"CREATE $datasetSqlSyntax table1 AS SELECT * FROM input", None)).foreach {
      case (query, commentOpt) =>
        val plan = parser.parsePlan(query)
        val cmd = plan.asInstanceOf[CreatePipelineDatasetAsSelect]
  ...
  test("STORED AS syntax is unsupported") {
    val ex = intercept[ParseException] {
      parser.parsePlan(
        s"CREATE $datasetSqlSyntax table1 STORED AS TEXTFILE AS SELECT * FROM input")
    }
    checkError(
      exception = ex,
      condition = "_LEGACY_ERROR_TEMP_0035",
      parameters = Map(
        "message" -> ("The STORED AS syntax is not supported for CREATE " +
  ...
  test("SerDe information is unsupported") {
    ...
    checkError(
      ...
      condition = "_LEGACY_ERROR_TEMP_0035",
      parameters = Map(
        "message" -> ("Hive SerDe format options are not supported for CREATE " +
  ...
  test("IF NOT EXISTS parses correctly") {
    Seq(
      ...
      (s"CREATE $datasetSqlSyntax table1 AS SELECT * FROM input", false)).foreach {
      case (query, ifNotExists) =>
        val plan = parser.parsePlan(query)
        val cmd = plan.asInstanceOf[CreatePipelineDatasetAsSelect]
    ...
    Seq(
      (
        s"CREATE $datasetSqlSyntax table1 TBLPROPERTIES(a=FALSE, b='long comment', c=100) AS " +
          "SELECT 1",
        Map("a" -> "false", "b" -> "long comment", "c" -> "100")),
      (s"CREATE $datasetSqlSyntax tabel1 AS SELECT 1", Map.empty)).foreach {
      case (query, tblProperties) =>
        val plan = parser.parsePlan(query)
        val cmd = plan.asInstanceOf[CreatePipelineDatasetAsSelect]
  ...
  test("Bucket spec is unsupported") {
    val ex = intercept[ParseException] {
      parser.parsePlan(s"""CREATE $datasetSqlSyntax table1 CLUSTERED BY (a) INTO 5 BUCKETS
           |AS SELECT * FROM input""".stripMargin)
    }
    checkError(
      exception = ex,
      condition = "_LEGACY_ERROR_TEMP_0035",
      parameters = Map(
        "message" -> (s"Bucketing is not supported for CREATE $datasetSqlSyntax " +
  ...
  test("Options is unsupported") {
    val ex = intercept[ParseException] {
      parser.parsePlan(s"""CREATE $datasetSqlSyntax table1 OPTIONS ('key' = 'value')
           |AS SELECT * FROM input""".stripMargin)
    }
    checkError(
      exception = ex,
      condition = "_LEGACY_ERROR_TEMP_0035",
      parameters = Map(
        "message" -> (s"Options are not supported for CREATE $datasetSqlSyntax " +
