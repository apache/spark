...
object Base64Benchmark extends SqlBasedBenchmark {
  ...
  private def doDecode(len: Int, f: Array[Byte] => Array[Byte]): Unit = {
    spark.range(N).map(_ => "Spark" * len).map { s =>
      ...
      java.util.Base64.getMimeEncoder(-1, new byte[0]).encode(s.getBytes)
    }.foreach { s =>
      f(s)
      ()
    }
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    Seq(1, 3, 5, 7).map { len =>
      ...
      benchmark.addCase("java", 3) { _ =>
        doEncode(len, x => java.util.Base64.getMimeEncoder(-1, new byte[0]).encode(x))
      }
    ...
    Seq(1, 3, 5, 7).map { len =>
      ...
      benchmark.addCase("java", 3) { _ =>
        doDecode(len, x => java.util.Base64.getDecoder.decode(x))
      }
      benchmark.addCase(s"apache", 3) { _ =>
