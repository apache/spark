...
class AdaptiveQueryExecSuite
  ...
  private def findTopLevelBroadcastHashJoin(plan: SparkPlan): Seq[BroadcastHashJoinExec] = {
    collect(plan) { case j: BroadcastHashJoinExec =>
      j
    }
  }

  def findTopLevelBroadcastNestedLoopJoin(plan: SparkPlan): Seq[BaseJoinExec] = {
    collect(plan) { case j: BroadcastNestedLoopJoinExec =>
      j
    }
  }
  ...
  private def findReusedSubquery(plan: SparkPlan): Seq[ReusedSubqueryExec] = {
    collectWithSubqueries(plan) { case e: ReusedSubqueryExec =>
      e
    }
  }
  ...
  private def checkNumLocalShuffleReads(
    ...
    val numShuffles = collect(plan) { case s: ShuffleQueryStageExec =>
      s
    }.length
  ...
  test(
    "SPARK-37753: Allow changing outer join to broadcast join even if too many empty" +
      ...
      withSQLConf(
        ...
        withTempView("skewData1", "skewData2") {
          ...
          def checkSkewJoin(
            ...
            assert(
              joins.head.left
                .collect { case r: AQEShuffleReadExec =>
                  r
                }
                .head
                .partitionSpecs
                .collect { case p: PartialReducerPartitionSpec =>
                  p.reducerIndex
                }
                .distinct
                .length == leftSkewNum)
    ...
    withUserDefinedFunction("slow_udf" -> true) {
      ...
      try {
        ...
        withSQLConf(
          ...
          assert(
            errMsgList.exists(_.contains("coalesce test error")),
            s"""
          ...
          val shuffleQueryStageExecs = collect(adaptivePlan) { case sqse: ShuffleQueryStageExec =>
            sqse
          }
          assert(
            shuffleQueryStageExecs.length == 3,
            s"Physical Plan should include " +
          ...
          assert(
            shuffleQueryStageExecs(1).shuffle.futureAction.get.isDefined,
  ...
  test("aqe in stateless streaming query") {
    withSQLConf(
      ...
      withTempView("test") {
        ...
        assert(ret.nonEmpty, s"expected AQE to take effect but can't find AQE node, plan: $plan")
  ...
  test("no aqe in stateless streaming query if stateless streaming config is disabled") {
    withSQLConf(
      ...
      withTempView("test") {
        ...
        assert(ret.isEmpty, s"expected AQE to not in effect but AQE node exists, plan: $plan")
      }
    }
  }
  ...
  test("no aqe in stateful streaming query - transformWithState") {
    class RunningCountStatefulProcessorInt extends StatefulProcessor[Int, Int, (Int, Long)] {
  ...
  private def testNoAqeInStatefulStreamingQuery(outputMode: OutputMode)(
      fn: MemoryStream[Int] => DataFrame): Unit = {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.ADAPTIVE_EXECUTION_FORCE_APPLY.key -> "true",
  ...
  test("SPARK-34682: AQEShuffleReadExec operating on canonicalized plan") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      ...
      val reads = collect(adaptivePlan) { case r: AQEShuffleReadExec =>
        r
      }
    ...
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      ...
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "80") {
        val (_, adaptivePlan) = runAdaptiveAndVerifyResult(
          "SELECT * FROM testData join testData2 ON key = a where value = '1'")
        val join = collect(adaptivePlan) { case j: BroadcastHashJoinExec =>
          j
        }.head
        assert(join.buildSide == BuildLeft)

        val reads = collect(join.right) { case r: AQEShuffleReadExec =>
          r
        }
        assert(reads.length == 1)
        val read = reads.head
  ...
  test("control a plan explain mode in listeners via SQLConf") {
    ...
    def checkPlanDescription(mode: String, expected: Seq[String]): Unit = {
      ...
      spark.sparkContext.addSparkListener(listener)
      withSQLConf(
        SQLConf.UI_EXPLAIN_MODE.key -> mode,
        SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
        SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "80") {
              ...
              event match {
                case SparkListenerSQLAdaptiveExecutionUpdate(_, _, planInfo) =>
                  if (enabled) {
                    ...
                    assert(
                      planInfo.children.head.children.head.nodeName ==
    ...
    withSQLConf(
      ...
      withTable("t1") {
        ...
        try {
          ...
          assert(
            commands(1).children.head.nodeName == "Execute InsertIntoHadoopFsRelationCommand")
    ...
    withSQLConf(
      ...
      Seq(
        ...
        (
          "SELECT * FROM (SELECT * FROM testData WHERE key = 0)t1 FULL JOIN " +
            "testData2 t2 ON t1.key = t2.a",
          true)).foreach { case (query, isEliminated) =>
        val (plan, adaptivePlan) = runAdaptiveAndVerifyResult(query)
        assert(findTopLevelBaseJoin(plan).size == 1)
    ...
    withTable("t1left", "t1right", "t1empty") {
      ...
      spark
        .sql("""
                  ...
                  |from
                  |  leftT
                  |  join t1right on leftT.a = t1right.a""".stripMargin)
        .collect()
    }
  }
  ...
  test("SPARK-35585: Support propagate empty relation through project/filter") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
      val (plan1, adaptivePlan1) =
        runAdaptiveAndVerifyResult("SELECT key FROM testData WHERE key = 0 ORDER BY key, value")
  ...
  test("SPARK-35442: Support propagate empty relation through aggregate") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      ...
      val (plan3, adaptivePlan3) =
        runAdaptiveAndVerifyResult("SELECT count(*) FROM testData WHERE value = 'no_match'")
  ...
  test("SPARK-35442: Support propagate empty relation through union") {
    def checkNumUnion(plan: SparkPlan, numUnion: Int): Unit = {
      assert(collect(plan) { case u: UnionExec =>
        u
      }.size == numUnion)
    }
  ...
  test("SPARK-32753: Only copy tags to node with no tags") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      withTempView("v1") {
        ...
        assert(collect(adaptivePlan) { case s: ShuffleExchangeExec =>
          s
        }.length == 1)
      }
    }
  ...
  test("SPARK-33551: Do not use AQE shuffle read for repartition") {
    ...
    def checkSMJ(
      ...
      val aqeReads = collect(smj.head) { case c: AQEShuffleReadExec =>
        c
      }
  ...
  test("SPARK-34899: Use origin plan if we can not coalesce shuffle partition") {
    def checkNoCoalescePartitions(ds: Dataset[Row], origin: ShuffleOrigin): Unit = {
      ...
      assert(collect(plan) { case c: AQEShuffleReadExec =>
        c
      }.isEmpty)
      assert(collect(plan) {
        case s: ShuffleExchangeExec if s.shuffleOrigin == origin && s.numPartitions == 2 => s
  ...
  test("SPARK-34980: Support coalesce partition through union") {
    def checkResultPartition(
      ...
      df.collect()
      assert(collect(df.queryExecution.executedPlan) { case u: UnionExec =>
        u
      }.size == numUnion)
      assert(collect(df.queryExecution.executedPlan) { case r: AQEShuffleReadExec =>
        r
    ...
    Seq(true, false).foreach { combineUnionEnabled =>
      ...
      withSQLConf(
        ...
        withTempView("t1", "t2") {
          ...
          spark.sparkContext
            .parallelize((1 to 10).map(i => TestData(i, i.toString)), 4)
            .toDF()
            .createOrReplaceTempView("t2")
    ...
    withSQLConf(SQLConf.COALESCE_PARTITIONS_MIN_PARTITION_NUM.key -> "1") {
      Seq("REPARTITION", "REBALANCE(key)")
        .foreach { repartition =>
          ...
          val (_, adaptivePlan) = runAdaptiveAndVerifyResult(query)
          collect(adaptivePlan) { case r: AQEShuffleReadExec =>
            r
          } match {
  ...
  test("SPARK-35888: join with a 0-partition table") {
    withSQLConf(
      ...
      withTempView("t2") {
        ...
        val (_, adaptive) =
          runAdaptiveAndVerifyResult("SELECT * FROM testData2 t1 left semi join t2 ON t1.a=t2.b")
        val aqeReads = collect(adaptive) { case c: AQEShuffleReadExec =>
          c
        }
        assert(aqeReads.length == 2)
        aqeReads.foreach { c =>
      ...
      withTempView("skewData1", "skewData2") {
        ...
        Seq(true, false).foreach { hasRequiredDistribution =>
          Seq(true, false).foreach { hasPartitionNumber =>
            ...
            val (_, adaptive1) =
              runAdaptiveAndVerifyResult(
                s"SELECT $repartition key1 FROM skewData1 " +
                  s"JOIN skewData2 ON key1 = key2 GROUP BY key1")
            val shuffles1 = collect(adaptive1) { case s: ShuffleExchangeExec =>
              s
            }
            ...
            val shuffles2 = collect(adaptive2) { case s: ShuffleExchangeExec =>
              s
            }
  ...
  test("SPARK-36424: Support eliminate limits in AQE Optimizer") {
    withTempView("v") {
      spark.sparkContext
        .parallelize((1 to 10).map(i => TestData(i, if (i > 2) "2" else i.toString)), 2)
        .toDF("c1", "c2")
        .createOrReplaceTempView("v")
  ...
  test(
    ...
    withTable("t3") {
      ...
      withSQLConf(
        SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
        SQLConf.SHUFFLE_PARTITIONS.key -> (SortShuffleManager.MAX_SHUFFLE_OUTPUT_PARTITIONS_FOR_SERIALIZED_MODE + 1).toString,
  ...
  test("SPARK-37063: OptimizeSkewInRebalancePartitions support optimize non-root node") {
    withTempView("v") {
      withSQLConf(
        ...
        SQLConf.COALESCE_PARTITIONS_MIN_PARTITION_NUM.key -> "1") {
        spark.sparkContext
          .parallelize((1 to 10).map(i => TestData(if (i > 2) 2 else i, i.toString)), 2)
          .toDF("c1", "c2")
          .createOrReplaceTempView("v")

        def checkRebalance(query: String, numShufflePartitions: Int): Unit = {
          val (_, adaptive) = runAdaptiveAndVerifyResult(query)
          assert(adaptive.collect { case sort: SortExec =>
            sort
          }.size == 1)
          val read = collect(adaptive) { case read: AQEShuffleReadExec =>
            read
  ...
  test("SPARK-37357: Add small partition factor for rebalance partitions") {
    withTempView("v") {
      withSQLConf(
        ...
        SQLConf.SHUFFLE_PARTITIONS.key -> "1") {
        spark.sparkContext
          .parallelize((1 to 8).map(i => TestData(if (i > 2) 2 else i, i.toString)), 3)
          .toDF("c1", "c2")
          .createOrReplaceTempView("v")

        def checkAQEShuffleReadExists(query: String, exists: Boolean): Unit = {
          val (_, adaptive) = runAdaptiveAndVerifyResult(query)
          assert(collect(adaptive) { case read: AQEShuffleReadExec =>
            read
          }.nonEmpty == exists)
        }
  ...
  test("SPARK-39915: Dataset.repartition(N) may not create N partitions") {
    withSQLConf(SQLConf.SHUFFLE_PARTITIONS.key -> "6") {
      ...
      assert(
        spark
          .range(0)
          .selectExpr("id % 3 as c1", "id % 7 as c2")
          .repartition(5, $"c1")
          .select($"c2")
          .rdd
          .getNumPartitions == 5)
  ...
  test("SPARK-42778: QueryStageExec should respect supportsRowBased") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_FORCE_APPLY.key -> "true") {
      withTempView("t") {
        ...
        df.collect()
        assert(collect(df.queryExecution.executedPlan) { case c: ColumnarToRowExec =>
          c
        }.isEmpty)
      }
    }
  ...
  test("SPARK-42101: Make AQE support InMemoryTableScanExec") {
    withSQLConf(
      ...
      def checkShuffleAndSort(firstAccess: Boolean): Unit = {
        ...
        assert(collect(initialExecutedPlan) { case s: ShuffleExchangeLike =>
          s
        }.size == (if (firstAccess) 1 else 0))
        assert(collect(initialExecutedPlan) { case s: SortExec =>
          s
        }.size == (if (firstAccess) 2 else 0))
        assert(collect(initialExecutedPlan) { case i: InMemoryTableScanLike =>
          i
        }.head.isMaterialized != firstAccess)

        df.collect()
        val finalExecutedPlan = df.queryExecution.executedPlan
        assert(collect(finalExecutedPlan) { case s: ShuffleExchangeLike =>
          s
        }.isEmpty)
        assert(collect(finalExecutedPlan) { case s: SortExec =>
          s
        }.isEmpty)
        assert(collect(initialExecutedPlan) { case i: InMemoryTableScanLike =>
          i
  ...
  test("SPARK-42101: Do not coalesce shuffle partition if other side is TableCacheQueryStage") {
    withSQLConf(
      ...
      withTempView("v1", "v2") {
        ...
        assert(collect(finalPlan) { case q: ShuffleQueryStageExec =>
          q
        }.size == 1)
        assert(collect(finalPlan) { case r: AQEShuffleReadExec =>
          r
        }.isEmpty)
      }
    }
  ...
  test("SPARK-42101: Coalesce shuffle partition with union even if exists TableCacheQueryStage") {
    withSQLConf(
      ...
      assert(collect(df.queryExecution.executedPlan) { case c: TableCacheQueryStageExec =>
        c
      }.size == 1)
    }
  }
  ...
  test("SPARK-43026: Apply AQE with non-exchange table cache") {
    Seq(true, false).foreach { canChangeOP =>
      withSQLConf(
        SQLConf.CAN_CHANGE_CACHED_PLAN_OUTPUT_PARTITIONING.key -> canChangeOP.toString) {
    ...
    withSQLConf(SQLConf.CAN_CHANGE_CACHED_PLAN_OUTPUT_PARTITIONING.key -> "true") {
      withTable("t1", "t2") {
        withCache("t1") {
          ...
          val (_, adaptive) =
            runAdaptiveAndVerifyResult("SELECT * FROM t1 WHERE c1 < (SELECT c2 FROM t2)")
  ...
  test("Collect twice on the same dataframe with no AQE plan changes") {
    ...
    assert(
      plan1
        .asInstanceOf[ResultQueryStageExec]
        .plan
  ...
  test("SPARK-42322: STAGE_MATERIALIZATION_MULTIPLE_FAILURES error class validation") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true") {
      ...
      withTempView("test_table1", "test_table2") {
        ...
        spark
          .range(100)
          .selectExpr("id", "id % 10 as group_col")
          .createOrReplaceTempView("test_table1")
        spark
          .range(100)
          .selectExpr("id", "id % 5 as group_col")
        ...
        assert(
          cause.getCondition == "STAGE_MATERIALIZATION_MULTIPLE_FAILURES",
          s"Expected STAGE_MATERIALIZATION_MULTIPLE_FAILURES, " +
            s"got: ${cause.getCondition}")
        val errorMessage = cause.getMessage
        assert(
          errorMessage.contains("Multiple failures (2) in stage materialization:"),
          s"Error message should contain failure count, got: $errorMessage")
        assert(
          errorMessage.contains("1. RuntimeException: Stage 1 materialization failed"),
          s"Error message should contain first error details, got: $errorMessage")
        assert(
          errorMessage.contains("2. RuntimeException: Stage 2 materialization failed"),
...
private object TestProblematicCoalesceStrategy extends Strategy {
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = {
    plan match {
      case org.apache.spark.sql.catalyst.plans.logical.Repartition(numPartitions, false, child) =>
