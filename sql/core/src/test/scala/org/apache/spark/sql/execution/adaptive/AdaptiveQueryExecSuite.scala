...
class AdaptiveQueryExecSuite
  ...
  test("SPARK-39921: Support optimize skew broadcast join even if introduce extra shuffle") {
    withSQLConf(
      SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
      SQLConf.ADAPTIVE_OPTIMIZE_SKEWS_IN_REBALANCE_PARTITIONS_ENABLED.key -> "false",
      SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1",
      SQLConf.ADAPTIVE_AUTO_BROADCASTJOIN_THRESHOLD.key -> "10M",
      SQLConf.SKEW_JOIN_SKEWED_PARTITION_THRESHOLD.key -> "100",
      SQLConf.ADVISORY_PARTITION_SIZE_IN_BYTES.key -> "100",
      SQLConf.COALESCE_PARTITIONS_MIN_PARTITION_NUM.key -> "1",
      SQLConf.SHUFFLE_PARTITIONS.key -> "10",
      ...
      withTempView("skewData1", "skewData2") {
        ...
        Seq(true, false).foreach { hasRequiredDistribution =>
          Seq(true, false).foreach { hasPartitionNumber =>
            ...
            // shuffles1.head is the top-level shuffle under the Aggregate operator
            assert(shuffles1.head.shuffleOrigin == ENSURE_REQUIREMENTS)
            val bhj1 = findTopLevelBroadcastHashJoin(adaptive1)
            assert(bhj1.size == 1 && bhj1.head.isSkewJoin)
            ...
            val bhj2 = findTopLevelBroadcastHashJoin(adaptive2)
            assert(bhj2.size == 1 && bhj2.head.isSkewJoin)
          }
        }
      }
    }
