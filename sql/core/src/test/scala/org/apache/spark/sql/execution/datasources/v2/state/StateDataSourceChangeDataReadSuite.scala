...
class HDFSBackedStateDataSourceChangeDataReaderSuite
    extends StateDataSourceChangeDataReaderSuite {
...
class RocksDBWithCheckpointV2StateDataSourceChangeDataReaderSuite
    extends RocksDBWithChangelogCheckpointStateDataSourceChangeDataReaderSuite {
  override def beforeAll(): Unit = {
    super.beforeAll()
...
abstract class StateDataSourceChangeDataReaderSuite
  ...
  test("ERROR: mixed checkpoint format versions not supported") {
    withTempDir { tempDir =>
      ...
      val endMetadata =
        CommitMetadata(0, Some(Map[Long, Array[Array[String]]](0L -> Array(Array("uid")))))
  ...
  test("getChangeDataReader of state store provider") {
    ...
    def withNewStateStore(provider: StateStoreProvider, version: Int)(
        f: StateStore => Unit): Unit = {
    ...
    withTempDir { tempDir =>
      ...
      assert(
        reader.next() === (RecordType.PUT_RECORD, dataToKeyRow("a", 1), dataToValueRow(1), 0L))
      assert(
        reader.next() === (RecordType.PUT_RECORD, dataToKeyRow("b", 2), dataToValueRow(2), 1L))
      assert(
        reader.next() ===
  ...
  test("read stream-stream join state change feed") {
    withTempDir { tempDir =>
      ...
      val keyWithIndexToValueDf = spark.read
        .format("statestore")
      ...
      val keyToNumValuesDf = spark.read
        .format("statestore")
