...
object DataSourceReadBenchmark extends SqlBasedBenchmark {
  ...
  private def prepareTable(dir: File, df: DataFrame, partition: Option[String] = None,
      csv: Boolean = true): Unit = {
    ...
    if (csv) saveAsCsvTable(testDf, dir.getCanonicalPath + "/csv")
  ...
  def structDFGenerator[T](delta: Int, func: Long => T): Long => (T, T, T, T) = { x =>
  ...
  def nestedStructBenchmark(values: Int): Unit = {
    ...
    withTempPath { dir =>
      withTempTable("jsonTable", "parquetTable", "orcTable") {
        import spark.implicits._
        ...
        prepareTable(dir, spark.range(values).map { _ => structGen.apply(Random.nextLong) }
            .toDF("col1", "col2", "col3", "col4"), csv = false)

        // CSV is missing because the this data structure is not supported

        benchmark.addCase("SQL Json") { _ =>
          spark.sql(s"SELECT sum(${writeString}) FROM jsonTable").noop()
        }

        benchmark.addCase("SQL Parquet Vectorized") { _ =>
          spark.sql(s"SELECT sum(${writeString}) FROM parquetTable").noop()
        }
        ...
        benchmark.addCase("SQL ORC Vectorized") { _ =>
          spark.sql(s"SELECT sum(${writeString}) FROM orcTable").noop()
        }
  ...
  def arrayBenchmark(values: Int): Unit = {
    ...
    withTempPath { dir =>
      withTempTable("jsonTable", "parquetTable", "orcTable") {
        ...
        // CSV is missing because this data structure is not supported

        benchmark.addCase("SQL Json") { _ =>
          spark.sql("SELECT sum(array_max(col1)) FROM jsonTable").noop()
        }

        benchmark.addCase("SQL Parquet Vectorized") { _ =>
          spark.sql("SELECT sum(array_max(col1)) FROM parquetTable").noop()
        }
        ...
        benchmark.addCase("SQL ORC Vectorized") { _ =>
          spark.sql("SELECT sum(array_max(col1)) FROM orcTable").noop()
        }
