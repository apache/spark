...
class DataFrameSuite extends QueryTest
  ...
  test("global sorting") {
    ...
    checkAnswer(
      ...
      arrayData.as[ArrayData].collect().sortBy(_.data(0)).toSeq)

    checkAnswer(
      arrayData.orderBy($"data".getItem(0).desc),
      arrayData.as[ArrayData].collect().sortBy(_.data(0)).reverse.toSeq)

    checkAnswer(
      arrayData.orderBy($"data".getItem(1).asc),
      arrayData.as[ArrayData].collect().sortBy(_.data(1)).toSeq)

    checkAnswer(
      arrayData.orderBy($"data".getItem(1).desc),
      arrayData.as[ArrayData].collect().sortBy(_.data(1)).reverse.toSeq)
  }
