...
class DataFrameSuite extends QueryTest with SharedSQLContext {
  ...
  test("showString: truncate = [0, 20], extendedMode = true") {
    val longString = Array.fill(21)("1").mkString
    val df = sparkContext.parallelize(Seq("1", longString)).toDF()
    ...
    assert(df.showString(10, truncate = 0, extendedMode = true) === expectedAnswerForFalse)
    val expectedAnswerForTrue = "-RECORD 0---------------------\n" +
                                " value | 1                    \n" +
                                "-RECORD 1---------------------\n" +
                                " value | 11111111111111111... \n"
    assert(df.showString(10, truncate = 20, extendedMode = true) === expectedAnswerForTrue)
  }
  ...
  test("showString: truncate = [3, 17], extendedMode = true") {
    val longString = Array.fill(21)("1").mkString
    val df = sparkContext.parallelize(Seq("1", longString)).toDF()
    val expectedAnswerForFalse = "-RECORD 0----\n" +
                                 " value | 1   \n" +
                                 "-RECORD 1----\n" +
                                 " value | 111 \n"
    assert(df.showString(10, truncate = 3, extendedMode = true) === expectedAnswerForFalse)
    val expectedAnswerForTrue = "-RECORD 0------------------\n" +
                                " value | 1                 \n" +
                                "-RECORD 1------------------\n" +
                                " value | 11111111111111... \n"
    assert(df.showString(10, truncate = 17, extendedMode = true) === expectedAnswerForTrue)
  }
  ...
  test("showString(negative), extendedMode = true") {
    val expectedAnswer = "only showing top 0 rows\n"
    assert(testData.select($"*").showString(-1, extendedMode = true) === expectedAnswer)
  }
  ...
  test("showString(0), extendedMode = true") {
    val expectedAnswer = "only showing top 0 rows\n"
    assert(testData.select($"*").showString(0, extendedMode = true) === expectedAnswer)
  }
  ...
  test("showString: array, extendedMode = true") {
    val df = Seq(
      (Array(1, 2, 3), Array(1, 2, 3)),
      (Array(2, 3, 4), Array(2, 3, 4))
    ).toDF()
    val expectedAnswer = "-RECORD 0--------\n" +
                         ...
                         " _1  | [2, 3, 4] \n" +
                         " _2  | [2, 3, 4] \n"
    assert(df.showString(10, extendedMode = true) === expectedAnswer)
  }
  ...
  test("showString: binary, extendedMode = true") {
    val df = Seq(
      ("12".getBytes(StandardCharsets.UTF_8), "ABC.".getBytes(StandardCharsets.UTF_8)),
      ("34".getBytes(StandardCharsets.UTF_8), "12346".getBytes(StandardCharsets.UTF_8))
    ).toDF()
  ...
  test("showString: minimum column width, extendedMode = true") {
    val df = Seq(
      (1, 1),
      (2, 2)
    ).toDF()
  ...
  test("SPARK-7319 showString, extendedMode = true") {
    val expectedAnswer = "-RECORD 0----\n" +
                         " key   | 1   \n" +
                         " value | 1   \n" +
                         "only showing top 1 row\n"
  ...
  test("SPARK-7327 show with empty dataFrame, extendedMode = true") {
    assert(testData.select($"*").filter($"key" < 0).showString(1, extendedMode = true) === "")
  }
  ...
  test("SPARK-18350 show with session local timezone, extendedMode = true") {
    val d = Date.valueOf("2016-12-01")
    val ts = Timestamp.valueOf("2016-12-01 00:00:00")
    val df = Seq((d, ts)).toDF("d", "ts")
    val expectedAnswer = "-RECORD 0------------------\n" +
    ...
    assert(df.showString(1, truncate = 0, extendedMode = true) === expectedAnswer)
