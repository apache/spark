...
class CreateSQLFunctionParserSuite extends AnalysisTest {
  ...
  test("create temporary SQL functions") {
    ...
    checkError(
      ...
      queryContext =
        Array(ExpectedContext("CREATE TEMPORARY FUNCTION a.b() RETURNS INT RETURN 1", 0, 51)))
    ...
    checkError(
      ...
      condition = "INVALID_TEMP_OBJ_QUALIFIER",
      parameters =
        Map("objectType" -> "FUNCTION", "objectName" -> "`c`", "qualifier" -> "`a`.`b`"),
      queryContext =
        Array(ExpectedContext("CREATE TEMPORARY FUNCTION a.b.c() RETURNS INT RETURN 1", 0, 53)))
  ...
  test("create persistent SQL functions") {
    ...
    comparePlans(
      parser.parsePlan("CREATE FUNCTION a.b.c(x INT) RETURNS TABLE (a INT) RETURN SELECT x"),
      createSQLFunction(
        Seq("a", "b", "c"),
        Some("x INT"),
        returnTypeText = "a INT",
    ...
    comparePlans(
      parser.parsePlan("CREATE FUNCTION IF NOT EXISTS a() RETURNS INT RETURN 1"),
      createSQLFunction(Seq("a"), exprText = Some("1"), ignoreIfExists = true))

    comparePlans(
      parser.parsePlan("CREATE OR REPLACE FUNCTION a() RETURNS INT RETURN 1"),
    ...
    comparePlans(
      ...
      createSQLFunction(
        Seq("a"),
        inputParamText = Some("x INT COMMENT 'x'"),
        exprText = Some("x"),
        isDeterministic = Some(true),
        containsSQL = Some(true),
        comment = Some("function")))

    intercept(
      "CREATE OR REPLACE FUNCTION IF NOT EXISTS a() RETURNS INT RETURN 1",
  ...
  test("create SQL functions with unsupported routine characteristics") {
    intercept(
      "CREATE FUNCTION foo() RETURNS INT LANGUAGE blah RETURN 1",
      "Operation not allowed: Unsupported language for user defined functions: blah")

    intercept(
      "CREATE FUNCTION foo() RETURNS INT SPECIFIC foo1 RETURN 1",
      "Operation not allowed: SQL function with SPECIFIC name is not supported")

    intercept(
      "CREATE FUNCTION foo() RETURNS INT NO SQL RETURN 1",
      "Operation not allowed: SQL function with NO SQL is not supported")

    intercept(
      "CREATE FUNCTION foo() RETURNS INT NO SQL CONTAINS SQL RETURN 1",
      "Found duplicate clauses: SQL DATA ACCESS")

    intercept(
      "CREATE FUNCTION foo() RETURNS INT RETURNS NULL ON NULL INPUT RETURN 1",
      "Operation not allowed: SQL function with RETURNS NULL ON NULL INPUT is not supported")

    intercept(
      "CREATE FUNCTION foo() RETURNS INT SQL SECURITY INVOKER RETURN 1",
