...
private[json] trait TestJsonData {
  ...
  def primitiveFieldAndType: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize("""{"string":"this is a simple string.",
          "integer":10,
          "long":21474836470,
  ...
  def complexFieldAndType1: Dataset[String] =
    spark.createDataset(
      spark.sparkContext.parallelize("""{"struct":{"field1": true, "field2": 92233720368547758070},
  ...
  def timestampAsLong: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize("""{"ts":1451732645}""" :: Nil))(
      Encoders.STRING)

  def arrayAndStructRecords: Dataset[String] =
    spark.createDataset(
      spark.sparkContext.parallelize("""{"a": {"b": 1}}""" ::
        """{"a": []}""" :: Nil))(Encoders.STRING)

  def floatingValueRecords: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize(s"""{"a": 0.${"0".repeat(
        38)}1, "b": 0.01}""" :: Nil))(Encoders.STRING)

  def bigIntegerRecords: Dataset[String] =
    spark.createDataset(spark.sparkContext.parallelize(s"""{"a": 1${"0".repeat(
        38)}, "b": 92233720368547758070}""" :: Nil))(Encoders.STRING)
  ...
  def oldDatesRecord: Dataset[String] =
    spark.createDataset(
      spark.sparkContext.parallelize("""{"date": "01/01/1800 18:00Z"}""" ::
