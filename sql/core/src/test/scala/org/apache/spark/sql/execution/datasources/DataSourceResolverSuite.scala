...
class DataSourceResolverSuite extends QueryTest with SharedSparkSession {
  ...
  private def checkResolveOperator(
    ...
    val relationsWithResolvedMetadata = metadataResolver.resolve(unresolvedPlan)
    assert(relationsWithResolvedMetadata.size == 1)

    val relation =
      relationsWithResolvedMetadata.values.iterator.next.asInstanceOf[SubqueryAlias].child

    var result = dataSourceResolver.resolveOperator(relation)
