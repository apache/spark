...
class ColumnTypeSuite extends SparkFunSuite {
  ...
  test("defaultSize") {
    val checks = Map(
      ...
      STRING(0) -> 8, BINARY -> 16, STRUCT_TYPE -> 20, ARRAY_TYPE -> 28, MAP_TYPE -> 68,
  ...
  test("actualSize") {
    ...
    checkActualSize(STRING(0), "hello", 4 + "hello".getBytes(StandardCharsets.UTF_8).length)
