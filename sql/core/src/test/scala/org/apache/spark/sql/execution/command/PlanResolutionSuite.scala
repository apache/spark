...
class PlanResolutionSuite extends SharedSparkSession with AnalysisTest {
  ...
  test("relation resolution - plan ID cloning on cache hit") {
    ...
    AnalysisContext.withAnalysisContext(ctx) {
      ...
      val resolved1 = resolve(unresolved1, planId = Some(planId1))
      ...
      val resolved2 = resolve(unresolved2, planId = Some(planId2))
      ...
      // verify the underlying data (table, catalog, identifier) are the same
