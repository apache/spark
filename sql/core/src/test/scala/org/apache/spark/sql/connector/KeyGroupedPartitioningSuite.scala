...
class KeyGroupedPartitioningSuite
  ...
  test("SPARK-59045: canonicalization normalizes the reduced expressions") {
    ...
    def groupPartitions(
      ...
      val reduced = TransformExpression(BucketFunction, Seq(Literal(2), otherAttr))
    ...
    assert(groupPartitions(a1, o1, physical.IdentityReducer(
        TransformExpression(BucketFunction, Seq(Literal(2), a1)))).canonicalized ==
      groupPartitions(a2, o2, physical.IdentityReducer(
        TransformExpression(BucketFunction, Seq(Literal(2), a2)))).canonicalized)
  ...
  test("SPARK-50593: generalized identity is strictly more discriminating than " +
    ...
    def truncate(e: Expression, width: Int): TransformExpression =
      TransformExpression(TruncateFunction, Seq(e, Literal(width)))
    def bucket(n: Int, e: Expression): TransformExpression =
      TransformExpression(BucketFunction, Seq(Literal(n), e))
    ...
    assert(!TransformExpression(DaysFunction, Seq(a)).isSameFunction(
      TransformExpression(YearsFunction, Seq(a))), "different function name")
  }
  ...
  test("SPARK-50593: a column whose type differs from the declared input type is not reducible " +
    ...
    assert(idSpec.reducersBothWays(trSpec) == ((None, None)),
  ...
  test("SPARK-50593: an arity-flexible transform (children > declared inputTypes) is not " +
    ...
    assert(idSpec.reducersBothWays(afSpec) == ((None, None)),
  ...
  test("SPARK-50593: identity-vs-transform compatibility gate agrees with reducers on a " +
    ...
    withSQLConf(
      ...
      SQLConf.V2_BUCKETING_PARTIALLY_CLUSTERED_DISTRIBUTION_ENABLED.key -> "false",
      SQLConf.V2_BUCKETING_ALLOW_COMPATIBLE_TRANSFORMS.key -> "true") {
      assert(!idSpec.areKeysCompatible(trSpec, allowReduce = true),
        "gate must reject a mismatched-type literal so it agrees with reducers (no mis-join)")
      assert(!trSpec.areKeysCompatible(idSpec, allowReduce = true), "symmetric")
    }
  }
