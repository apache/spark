...
class KeyGroupedPartitioningSuite extends DistributionAndOrderingSuiteBase with ExplainSuiteHelper {
  ...
  test("SPARK-59057: a collapse is reported when the splits are distributed, not replicated") {
    ...
    withSQLConf(
        ...
        SQLConf.V2_BUCKETING_PARTIALLY_CLUSTERED_DISTRIBUTION_ENABLED.key -> "true",
        SQLConf.V2_BUCKETING_ALLOW_KEYS_SUBSET_OF_PARTITION_KEYS.key -> "true",
        // `items` is partitioned by more keys than the join uses, and on this branch the
        // co-partition gate still asks the partition attributes to match the clustering keys
        // one for one, so it refuses the shape this test needs. SPARK-58558 relaxed that gate to
        // "every clustering key is covered" on master, and it is not on this branch.
        SQLConf.REQUIRE_ALL_CLUSTER_KEYS_FOR_CO_PARTITION.key -> "false") {
