...
import org.apache.spark.unsafe.types.IntervalConstants
...
class StreamingAggregationSuite extends StateStoreMetricsTest with Assertions {
  ...
  testWithAllStateVersions("prune results by current_date, complete mode") {
    ...
    val aggregated =
      inputData.toDF()
        .select(($"value" * IntervalConstants.SECONDS_PER_DAY).cast("timestamp").as("value"))
        .groupBy($"value")
        .agg(count("*"))
        .where($"value".cast("date") >= date_sub(current_timestamp().cast("date"), 10))
        .select(
          ($"value".cast("long") / IntervalConstants.SECONDS_PER_DAY).cast("long"), $"count(1)")
    testStream(aggregated, Complete)(
      StartStream(Trigger.ProcessingTime("10 day"), triggerClock = clock),
      // advance clock to 10 days, should retain all keys
      AddData(inputData, 0L, 5L, 5L, 10L),
      AdvanceManualClock(IntervalConstants.MILLIS_PER_DAY * 10),
      CheckLastBatch((0L, 1), (5L, 2), (10L, 1)),
      ...
      AssertOnQuery { q => // clear the sink
        ...
        clock.advance(IntervalConstants.MILLIS_PER_DAY * 60)
        true
      },
      StartStream(Trigger.ProcessingTime("10 day"), triggerClock = clock),
