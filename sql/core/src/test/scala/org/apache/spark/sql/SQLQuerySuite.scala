    ...
    Seq("orc", "parquet").foreach { format =>
      withSQLConf(SQLConf.USE_V1_SOURCE_LIST.key -> "") {
        withTempPath { dir =>
          ...
          checkPushedFilters(
            format,
            df.where(('id < 2 and Symbol("s").contains("foo")) or ('id > 10 and Symbol("s").contains("bar"))),
            Array(sources.Or(sources.LessThan("id", 2), sources.GreaterThan("id", 10))))
          checkPushedFilters(
            format,
            df.where(Symbol("s").contains("foo") or ('id > 10 and Symbol("s").contains("bar"))),
            Array.empty)
          checkPushedFilters(
            format,
            df.where('id < 2 and not('id > 10 and Symbol("s").contains("bar"))),
