...
class SQLQuerySuite extends QueryTest with SharedSQLContext {
  ...
  test("correlated subqueries") {
    ...
    withTempTable("l", "r") {
      ...
      val query = "select * from l where"
      checkAnswer(
        sql(s"$query exists(select * from r where l.a = r.c)"),
        Row(2, 1.0) :: Row(2, 1.0) :: Row(3, 3.0) :: Row(6, null) :: Nil)

      checkAnswer(
        sql(s"$query exists(select * from r where l.a = r.c) and l.a <= 2"),
        Row(2, 1.0) :: Row(2, 1.0) :: Nil)

      checkAnswer(
        sql(s"$query not exists(select * from r where l.a = r.c)"),
        Row(1, 2.0) :: Row(1, 2.0) :: Row(null, null) :: Row(null, 5.0) :: Nil)

      checkAnswer(
        sql(s"$query not exists(select * from r where l.a = r.c and l.b < r.d)"),
        Row(1, 2.0) :: Row(1, 2.0) ::
          Row(3, 3.0) :: Row(null, null) ::
          Row(null, 5.0) :: Row(6, null) :: Nil)

      checkAnswer(
        sql(s"$query l.a in (select c from r)"),
        Row(2, 1.0) :: Row(2, 1.0) :: Row(3, 3.0) :: Row(6, null) :: Nil)

      checkAnswer(
        sql(s"$query l.a in (select c from r) and l.a > 2 and l.b is not null"),
        Row(3, 3.0) :: Nil)

      checkAnswer(
        sql(s"$query a not in(select c from r)"),
        Row(1, 2.0) :: Row(1, 2.0) :: Row(null, null) :: Row(null, 5.0) :: Nil)

      checkAnswer(
        sql(s"$query a not in(select c from r where d is not null)"),
        Row(1, 2.0) :: Row(1, 2.0) ::
          Row(null, null) :: Row(null, 5.0) ::
          Row(6, null) :: Nil)

      checkAnswer(
        sql(s"$query (a, b) not in(select c, d from r)"),
