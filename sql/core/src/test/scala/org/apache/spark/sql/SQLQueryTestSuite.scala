 ...
 *      --QUERY-DELIMITER-START and --QUERY-DELIMITER-END is still separated by semicolons.
 *   2. Lines starting with -- are treated as comments and ignored.
 *   3. Lines starting with --SET are used to specify the configs when running this testing file.
 *      You can set multiple configs in one --SET, using comma to separate them. Or you can use
 *      multiple --SET statements.
 *   4. Lines starting with --IMPORT are used to load queries from another test file.
 *   5. Lines starting with --CONFIG_DIM are used to specify config dimensions of this testing
 *      file. The dimension name is decided by the string after --CONFIG_DIM. For example,
 ...
 *   6. A line with --DEBUG (on its own line before a query) marks that query for debug mode. When
 *      any --DEBUG marker is present, the test enters debug mode:
 ...
 *      - The test always fails at the end with a reminder to remove --DEBUG markers. To inspect
 *        the DataFrame interactively, set a breakpoint in `runDebugQueries` at the line where
 *        `localSparkSession.sql(sql)` is called, then evaluate the DataFrame in the debugger
 *        (e.g., `df.queryExecution.analyzed`, `df.explain(true)`).
 *
 * For example:
 * {{{
 *   -- this is a comment
...
class SQLQueryTestSuite
  ...
  protected def runQueriesWithSparkConfigDimensions(
    ...
    sparkConfigDims.foreach { configDim =>
      try {
        ...
        case e: Throwable =>
          val configs = configDim.map { case (k, v) =>
            s"$k=$v"
          }
  ...
  /**
   * Creates and configures a local SparkSession for running test queries. Handles UDF/UDTF
   * registration, SQL config, and TPCDS table setup.
   */
  ...
  /**
   * Runs queries in debug mode. Only commands and --DEBUG-marked queries are executed. Failed
   * debug queries print the full error stacktrace. Results are compared against the golden file.
   * The test always fails at the end to prevent accidental commits.
   *
   * To inspect the DataFrame interactively, set a breakpoint at the line where
   * `localSparkSession.sql(sql)` is called inside this method, then evaluate the DataFrame in the
