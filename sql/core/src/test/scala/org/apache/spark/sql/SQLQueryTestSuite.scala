...
import org.apache.spark.sql.execution.command.{DescribeColumnCommand, DescribeTableOrQueryCommand}
...
class SQLQueryTestSuite extends QueryTest with SharedSQLContext {
  ...
  private def getNormalizedResult(session: SparkSession, sql: String): (StructType, Seq[String]) = {
    ...
    def isSorted(plan: LogicalPlan): Boolean = plan match {
      ...
      case _: DescribeTableOrQueryCommand | _: DescribeColumnCommand => true
