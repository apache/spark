...
class DataFrameAggregateSuite extends QueryTest
  ...
  test("SPARK-47430 Support GROUP BY MapType") {
    def genMapData(dataType: String): String = {
      s"""
        |case when id % 3 == 0 then map()
        |when id % 3 == 1 then map(cast(0 as $dataType), cast(0 as $dataType))
        |else map(cast(0 as $dataType), cast(0 as $dataType),
        |  cast(1 as $dataType), cast(0 as $dataType))
        |end
        |""".stripMargin
    }
    Seq("int", "long", "float", "double", "decimal(10, 2)", "string", "varchar(6)").foreach { dt =>
      withTempView("v") {
        spark.range(20)
          .selectExpr(
            s"cast(id % 6 as $dt) as c1",
            s"${genMapData(dt)} as c2",
            "map(c1, null) as c3",
            s"cast(null as map<$dt, $dt>) as c4")
          .selectExpr("c1", "c2", "c3", "c4", "map(array(c1), c2) as c5")
        ...
        assertAggregateOnDataframe(
          ...
          6)
        assertAggregateOnDataframe(
          spark.sql("SELECT c3 FROM v GROUP BY c3"),
          6)
        assertAggregateOnDataframe(
          spark.sql("SELECT c4 FROM v GROUP BY c4"),
          1)
        assertAggregateOnDataframe(
          spark.sql("SELECT c5 FROM v GROUP BY c5"),
          6)
        assertAggregateOnDataframe(
          spark.sql("SELECT c1, c2, c3, c4, c5, count(*) FROM v GROUP BY c1, c2, c3, c4, c5"),
          6)
      }
    }
  }
