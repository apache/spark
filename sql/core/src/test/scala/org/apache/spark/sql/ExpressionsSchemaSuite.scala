...
class ExpressionsSchemaSuite extends QueryTest with SharedSparkSession {
  ...
  test("Check schemas for expression examples") {
    ...
    outputBuffer += s"-- Automatically generated by ${getClass.getSimpleName}\n\n"

    classFunsMap.foreach { kv =>
      val className = kv._1
      if (!ignoreSet.contains(className)) {
        outputBuffer += s"-- Class name: ${kv._1}\n"
        kv._2.foreach { funInfo =>
          outputBuffer += s"-- Function name: ${funInfo.getName}"
    ...
    if (regenerateGoldenFiles) {
      val goldenOutput = outputBuffer.mkString("\n")
