...
/**
 * SQLMetrics visibility coverage for [[SegmentTreeWindowFunctionFrame]]: segtree path bumps
 * `numSegmentTreeFrames`; fallback path bumps `numSegmentTreeFallbackFrames`; feature-flag off
 * leaves both at 0.
 */
class SegmentTreeWindowMetricsSuite extends SharedSparkSession with SQLMetricsTestUtils {
   ...
   * like `spillSize` are UI-pretty-printed strings and are skipped.)
   */
  ...
  test("segment-tree path increments numSegmentTreeFrames (one per frame per partition)") {
    withSQLConf(
      ...
      val df =
        baseDF.select($"id", min($"v").over(winSpec).as("mn"), max($"v").over(winSpec).as("mx"))
      val m = windowMetricValues(df)
      assert(
        m("number of segment-tree frames prepared") === 3L,
        s"expected 3 segtree frames (one per partition), got metrics = $m")
      assert(
        m("number of segment-tree fallback frames prepared") === 0L,
  ...
  test("fallback path increments numSegmentTreeFallbackFrames") {
    withSQLConf(
      ...
      val m = windowMetricValues(df)
      assert(
        m("number of segment-tree fallback frames prepared") === 3L,
        s"expected 3 fallback frames (one per partition under threshold), got $m")
      assert(
        m("number of segment-tree frames prepared") === 0L,
  ...
  test("T1 (G1) numPartitions > numTasks, identical length: every partition counted") {
    withSQLConf(
      ...
      val df = spark
        .range(0, 300)
        .selectExpr("id", "(id % 3) AS pk", "CAST(id AS INT) AS v")
        .select(
          $"id",
          min($"v").over(Window.partitionBy($"pk").orderBy($"id").rowsBetween(-3, 3)).as("mn"))
      val m = windowMetricValues(df)
      assert(
        m("number of segment-tree frames prepared") === 3L,
  ...
  test("T2 (G2) identical-length partitions across keys") {
    withSQLConf(
      ...
      val df = spark
        ...
        .selectExpr(
          "id",
          "CASE WHEN id < 200 THEN 'a' ELSE 'b' END AS pk",
          "CAST(id AS INT) AS v")
        .select(
          $"id",
          min($"v").over(Window.partitionBy($"pk").orderBy($"id").rowsBetween(-3, 3)).as("mn"))
      val m = windowMetricValues(df)
      assert(
        m("number of segment-tree frames prepared") === 2L,
  ...
  test("T3 (G3) all-length-1 unique keys, fallback path: every partition counted") {
    withSQLConf(
      ...
      val df = spark
        .range(0, 100)
        .selectExpr("id AS pk", "CAST(id AS INT) AS v")
        .select(
          $"pk",
          min($"v").over(Window.partitionBy($"pk").orderBy($"pk").rowsBetween(-3, 3)).as("mn"))
      val m = windowMetricValues(df)
      assert(
        m("number of segment-tree fallback frames prepared") === 100L,
  ...
  test("T4 (G4) mixed segtree + fallback, non-aliasing order") {
    withSQLConf(
      ...
      assert(
        m("number of segment-tree fallback frames prepared") === 2L,
