...
class CrossDbmsSQLQueryTestRunner:
    ...
    def __init__(self, cross_dbms_to_generate_golden_files: str):
        ...
        for testcase in testcases:
            self._run_sql_test_case(testcase, testcases)
    ...
    def _translate_queries(self, query: str) -> str:
        for x in sqlglot.parse_one(query, read=SPARK).find_all(sqlglot.exp.Order):
            print(x)
        return sqlglot.transpile(
            query, read=SPARK, write=self.cross_dbms_to_generate_golden_files)[0]

    def _run_sql_test_case(self, test_case: TestCase, list_test_cases):
        ...
        print(f'Generating golden files for testcase\t{test_case.name}')
    ...
    def _run_queries_and_generate_golden_files(self, queries: List[str], test_case: TestCase):
        ...
        for query in queries:
            translated_query = self._translate_queries(query)
            try:
        ...
        outputs = sorted([str(s) for s in results])
