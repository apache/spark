...
class MathFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  private def testOneToOneNonNegativeMathFunction(
    ...
    if (f(-1) === StrictMath.log1p(-1)) {
      checkAnswer(nnDoubleData.select(c($"b")), (1 to 9).map(n => Row(f(n * -0.1))) :+ Row(null))
    }

    checkAnswer(nnDoubleData.select(c(lit(null))), (1 to 10).map(_ => Row(null)))
  }
  ...
  test("asinh") {
    testOneToOneMathFunction(asinh, (x: Double) => math.log(x + math.sqrt(x * x + 1)))
  }
  ...
  test("acosh") {
    testOneToOneMathFunction(acosh, (x: Double) => math.log(x + math.sqrt(x * x - 1)))
  }
  ...
  test("atanh") {
    testOneToOneMathFunction(atanh, (x: Double) => (0.5 * (math.log1p(x) - math.log1p(-x))))
  }
  ...
  test("SPARK-33428 conv function shouldn't raise error if input string is too big") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> false.toString) {
      val df =
        Seq(("aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0aaaaaaa0")).toDF(
          "num")
  ...
  test("SPARK-44973: conv must allocate enough space for all digits plus negative sign") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> false.toString) {
      val df =
        Seq(((BigInt(Long.MaxValue) + 1).toString(16)), (BigInt(Long.MinValue).toString(16)))
          .toDF("num")
      checkAnswer(
  ...
  test("round/bround/ceil/floor with table columns") {
    withTable("t") {
      ...
      checkAnswer(sql("select i, round(i) from t"), Seq(Row(BigDecimal("5.9"), BigDecimal("6"))))
      checkAnswer(sql("select i, bround(i) from t"), Seq(Row(BigDecimal("5.9"), BigDecimal("6"))))
      checkAnswer(sql("select i, ceil(i) from t"), Seq(Row(BigDecimal("5.9"), BigDecimal("6"))))
      checkAnswer(
      ...
      checkAnswer(sql("select i, floor(i) from t"), Seq(Row(BigDecimal("5.9"), BigDecimal("5"))))
      checkAnswer(
  ...
  test("signum / sign") {
    ...
    checkAnswer(Seq((1, 2)).toDF().select(signum(lit(10)), signum(lit(-11))), Row(1, -1))
  }
  ...
  test("SPARK-35926: Support YearMonthIntervalType in width-bucket function") {
    Seq(
      ...
      (
        ...
        10) -> 5).foreach { case ((value, start, end, num), expected) =>
  ...
  test("width_bucket with numbers") {
    val df1 = Seq(
      ...
      (-2.1, 1.3, 3.4, 3),
      (8.1, 0.0, 5.7, 4),
      (-0.9, 5.2, 0.5, 2)).toDF("v", "min", "max", "n")
  ...
  test("try_add") {
    ...
    checkAnswer(df1.selectExpr("try_add(d, i)"), df1.select(try_add(col("d"), col("i"))))
    checkAnswer(
      df1.selectExpr(s"try_add(d, make_interval(i))"),
      df1.select(try_add(column("d"), make_interval(col("i")))))
    checkAnswer(
      df1.selectExpr(s"try_add(d, make_interval(0, 0, 0, i))"),
      df1.select(try_add(column("d"), make_interval(lit(0), lit(0), lit(0), col("i")))))
    checkAnswer(
      df1.selectExpr("try_add(make_interval(i), make_interval(i))"),
  ...
  test("try_divide") {
    ...
    checkAnswer(
      df1.selectExpr(s"try_divide(make_interval(year, month), 2)"),
      df1.select(try_divide(make_interval(col("year"), col("month")), lit(2))))
    checkAnswer(
      df1.selectExpr(s"try_divide(make_interval(year, month), 0)"),
  ...
  test("try_multiply") {
    ...
    checkAnswer(
      df.selectExpr("try_multiply(make_interval(a), b)"),
  ...
  test("try_subtract") {
    ...
    checkAnswer(
      df1.selectExpr("try_subtract(d, i)"),
      df1.select(try_subtract(col("d"), col("i"))))
    checkAnswer(
      df1.selectExpr(s"try_subtract(d, make_interval(i))"),
      df1.select(try_subtract(col("d"), make_interval(col("i")))))
    checkAnswer(
      df1.selectExpr(s"try_subtract(d, make_interval(0, 0, 0, i))"),
      df1.select(try_subtract(col("d"), make_interval(lit(0), lit(0), lit(0), col("i")))))
    checkAnswer(
      df1.selectExpr("try_subtract(make_interval(i), make_interval(i))"),
