...
trait MergeIntoSchemaEvolutionTypeWideningAndExtraFieldTests
  ...
  testNestedStructsEvolution(
    "source has extra struct fields -" +
    ...
    targetSchema = StructType(
      Seq(
        ...
        StructField(
          "info",
          StructType(Seq(StructField("salary", IntegerType), StructField("status", StringType)))),
  ...
  testNestedStructsEvolution(
    ...
    expectErrorWithoutEvolutionContains =
      "Cannot write extra fields `bonus` to the struct `info`")
          ...
          StructType(
            Seq(
              ...
              StructField(
                "details",
                StructType(
                  Seq(StructField("salary", IntegerType), StructField("status", StringType))))))),
        StructField("dep", StringType))),
    sourceSchema = StructType(
      Seq(
        StructField("pk", IntegerType, nullable = false),
  ...
  testEvolution(
    ...
    targetData = Seq((1, 100, "hr"), (2, 200, "software")).toDF("pk", "salary", "dep"),
    sourceData = Seq((2, 150, "dummy", "blah"), (3, 250, "dummy", "blah"))
      .toDF("pk", "salary", "dep", "extra"),
    clauses = Seq(update(set = "extra=s.dep")),
    ...
    sourceSchema = StructType(
      Seq(
        ...
        StructField(
          "a",
          ArrayType(
            StructType(Seq(StructField("c1", IntegerType), StructField("c2", StringType)))
          )
        ), // missing c3 field
        StructField("dep", StringType))),
    clauses = Seq(updateAll(), insertAll()),
  ...
  testNestedStructsEvolution("source missing fields in top-level struct")(
    target = Seq("""{ "pk": 0, "s": { "c1": 1, "c2": "a", "c3": true }, "dep": "sales" }"""),
    source = Seq(
      // Source is missing c3 field
    ...
    targetData = {
      val schema = StructType(
        Seq(
          ...
          StructField("dep", StringType)))
      val data = Seq(Row(0, 100, "sales"), Row(1, 200, "hr"))
      spark.createDataFrame(spark.sparkContext.parallelize(data), schema)
    },
    sourceData = Seq((1, "engineering"), (2, "finance")).toDF("pk", "dep"),
    clauses = Seq(updateAll(), insertAll()),
  ...
  testNestedStructsEvolution(
    "source has missing struct field - source has struct of nulls" +
            ...
            StructType(
              Seq(
                ...
                StructField(
                  "c2",
                  StructType(Seq(StructField("a", IntegerType), StructField("b", StringType)))))))
            .withCurrentDefaultValue(defaultExpr)
            .withExistenceDefaultValue(defaultExpr),
          StructField("dep", StringType)))
      val data = Seq(Row(0, Row(1, Row(10, "x")), "sales"), Row(1, Row(2, Row(20, "y")), "hr"))
      spark.createDataFrame(spark.sparkContext.parallelize(data), schema)
    },
    sourceData = Seq((1, "engineering"), (2, "finance")).toDF("pk", "dep"),
    clauses = Seq(update("dep = s.dep"), insert("(pk, dep) VALUES (s.pk, s.dep)")),
    // When inserting without specifying the struct column, default should be used
    expected = Seq(
      (0, (1, (10, "x")), "sales"),
      (1, (2, (20, "y")), "engineering"),
      (2, (999, (999, "default")), "finance")).toDF("pk", "s", "dep"),
        ...
        StructField(
          ...
          StructType(
            Seq(
              ...
              StructField(
                "c2",
                StructType(Seq(StructField("a", IntegerType), StructField("b", BooleanType))))))),
        StructField("dep", StringType))),
    sourceSchema = StructType(
      Seq(
        StructField("pk", IntegerType, nullable = false),
  ...
  testNestedStructsEvolution("source has missing non-nullable struct field")(
    target = Seq(
  ...
  testNestedStructsEvolution(
    "schemas match - source has null struct " +
  ...
  testNestedStructsEvolution(
    "source has missing struct field - source has null struct" +
      " and target has struct of nulls")(
    target = Seq(
        ...
        Seq(
          ...
          StructField(
            ...
            StructType(
              Seq(
                StructField("c1", IntegerType),
                StructField("c2", StructType(Seq(StructField("a", IntegerType))))))),
          StructField("dep", StringType)))
      val data = Seq(Row(1, null, "engineering"), Row(2, null, "finance"))
  ...
  testNestedStructsEvolution(
    "source has missing struct field -" +
  ...
  testNestedStructsEvolution(
    "source has missing nested struct field - " +
              ...
              StructType(
                Seq(
                  ...
                  StructField(
                    "b",
                    StructType(
                      Seq(StructField("x", IntegerType), StructField("y", StringType)))))))))),
        StructField("dep", StringType))),
    sourceSchema = StructType(
      Seq(
        StructField("pk", IntegerType, nullable = false),
  ...
  testNestedStructsEvolution(
    "source has missing field in struct inside array -" +
