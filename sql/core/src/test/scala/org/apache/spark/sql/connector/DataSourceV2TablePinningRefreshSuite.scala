...
class DataSourceV2TablePinningRefreshSuite
  ...
  test("[1.5] temp view after session drop/re-add column same type") {
    withTable(T) {
      ...
      // Temp view resolves by name (like SQL views). InMemoryTable does not
      // track column identity at the data level, so old rows retain their
      // salary values even after drop and re-add.
      checkAnswer(sql("SELECT * FROM tmp"), Seq(Row(1, 100)))
    }
  }
  ...
  test("[1.5-ext] temp view after external drop/re-add column same type") {
    withTable(T) {
      ...
      // Temp view resolves by name. InMemoryTable does not track column
      // identity at the data level, so old rows retain their values.
      checkAnswer(sql("SELECT * FROM tmp"), Seq(Row(1, 100)))
    }
  }
  ...
  test("[Writer.2] writeTo().append() succeeds after session DROP COLUMN") {
    withTable(T) {
      setupTable()
      val source = spark.table(T)
      ...
      // The writeTo path re-analyzes the plan, so the source DF gets
      // refreshed to the latest table state. The append succeeds.
      source.writeTo(T).append()
    }
  }

  test("[Writer.2-ext] writeTo().append() succeeds after external DROP COLUMN") {
    withTable(T) {
      setupTable()
      val source = spark.table(T)
  ...
  test("[Writer.3] writeTo().overwrite() succeeds after session DROP COLUMN") {
    withTable(T) {
      setupTable()
      val source = spark.table(T)
      ...
      // The writeTo path re-analyzes the plan, so the source DF gets
      // refreshed to the latest table state. The overwrite succeeds.
      import org.apache.spark.sql.functions.lit
      source.writeTo(T).overwrite(lit(true))
    }
  }

  test("[Writer.4] writeTo().append() after session type widening succeeds") {
    withTable(T) {
      setupTable()
      val source = spark.table(T)
  ...
  test("[Writer.5] writeTo().append() after session column rename succeeds") {
    withTable(T) {
      setupTable()
      val source = spark.table(T)
  ...
  test("[Writer.6] writeTo().createOrReplace() succeeds after DROP COLUMN") {
    withTable(T, T2) {
      setupTable()
      ...
      // The writeTo path re-analyzes the plan, so the source DF gets
      // refreshed to the latest table state. The createOrReplace succeeds.
