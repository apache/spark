 ...
 /** RocksDB memory management tests for bounded memory usage */
  ...
  testWithChangelogCheckpointingEnabled("reloading the same version") {
    ...
    withDB(remoteDir, conf = conf) { db =>
      ...
      // local -> cloud file map and the information is used to skip some file uploading, which is
      // wrong because these files aren't a part of the RocksDB checkpoint.
      // This bug was accidentally fixed by SPARK-48931 (https://github.com/apache/spark/pull/47393)
      db.load(0)
      db.put("foo", "bar")
      db.commit()
