...
class SQLMetricsSuite extends SharedSparkSession with SQLMetricsTestUtils
      ...
      rightDf, rows, numMatched, enableWholeStage) =>
        ...
        testSparkPlanMetrics(df, 1, Map(
          nodeId -> (("ShuffledHashJoin", Map(
            "number of output rows" -> rows, "number of matched pairs" -> numMatched)))),
          enableWholeStage
        )
      }
  }
  ...
  test("BroadcastHashJoin(outer) metrics") {
    ...
    Seq((df1, df2, false, "left_outer", 0L, 6L, 5L, false),
      ...
      testSparkPlanMetrics(df, 2, Map(
        nodeId -> (("BroadcastHashJoin", Map(
          "number of output rows" -> numRows, "number of matched pairs" -> numMatched)))),
        enableWholeStage
      )
    }
  }
        ...
        Seq((leftQuery, 12L, false), (rightQuery, 12L, false), (leftQuery, 12L, true),
          ...
          testSparkPlanMetrics(df, 2, Map(
            0L -> (("BroadcastNestedLoopJoin", Map(
              "number of output rows" -> rows, "number of matched pairs" -> 12L)))),
            enableWholeStage
          )
        }
      }
  ...
  test("BroadcastLeftSemiJoinHash metrics") {
    ...
    Seq((df1, df2, false, 1L, 2L, 0L, false),
      ...
      testSparkPlanMetrics(df, 2, Map(
        nodeId -> (("BroadcastHashJoin", Map(
          "number of output rows" -> rows, "number of matched pairs" -> matched)))),
        enableWholeStage
      )
    }
  }
    ...
    withSQLConf(SQLConf.CROSS_JOINS_ENABLED.key -> "true") {
      ...
      withTempView("testDataForJoin") {
        ...
        Seq((12L, query, true), (12L, query, false), (6L, boundedQuery, true),
          ...
          testSparkPlanMetrics(df, 1, Map(
            0L -> (("CartesianProduct",
              Map("number of output rows" -> rows, "number of matched pairs" -> 12L)))),
            enableWholeStage
          )
        }
      }
  ...
  test("SortMergeJoin(left-anti) metrics") {
    ...
    withTempView("antiData") {
      ...
      Seq(false, true).foreach { enableWholeStage =>
        ...
        testSparkPlanMetrics(df, 1, Map(
          0L -> (("SortMergeJoin",
            Map("number of output rows" -> 4L, "number of matched pairs" -> 2L)))),
          enableWholeStage
        )
      }
    }
