...
class SQLMetricsSuite
  ...
  test("SPARK-43214: LocalTableScanExec metrics") {
    ...
    testSparkPlanMetrics(df1.toDF(), 0, Map(0L -> (("LocalTableScan", expected))))
  }
  ...
  test("SPARK-43214: CommandResultExec metrics") {
    withTable("t", "t2") {
      ...
      val command = df.queryExecution.executedPlan.collect { case cmd: CommandResultExec =>
        cmd
      }
      sparkContext.listenerBus.waitUntilEmpty()
      assert(command.size == 1)
      val expected = Map("number of output rows" -> 2L)
      testSparkPlanMetrics(df, 0, Map(0L -> (("CommandResult", expected))))
    }
  }
  ...
  test("SPARK-54749: OneRowRelation metrics") {
    Seq((1L, "false"), (2L, "true")).foreach { case (nodeId, enableWholeStage) =>
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> enableWholeStage) {
        ...
        testSparkPlanMetrics(df.toDF(), 1, Map(nodeId -> (("Scan OneRowRelation", expected))))
      }
    }
  }
  ...
  test("Recursive CTEs metrics") {
    withSQLConf(SQLConf.OPTIMIZER_EXCLUDED_RULES.key -> "") {
      ...
      val unionLoopExec = df.queryExecution.executedPlan.collect { case ule: UnionLoopExec =>
        ule
      }
      sparkContext.listenerBus.waitUntilEmpty()
      assert(unionLoopExec.size == 1)
      val expected = Map(
        "number of output rows" -> 39L,
        "number of recursive iterations" -> 20L,
        "number of anchor output rows" -> 2L)
      testSparkPlanMetrics(df, 22, Map(2L -> (("UnionLoop", expected))))
    }
  }
  ...
  test("Filter metrics") {
    ...
    Seq((0L, false), (1L, true)).foreach { case (nodeId, enableWholeStage) =>
      ...
      testSparkPlanMetrics(
        df,
        1,
        Map(nodeId -> (("Filter", Map("number of output rows" -> 1L)))),
        enableWholeStage)
    }
  }
    ...
    Seq(true, false).foreach { enableWholeStage =>
      ...
      nodeIds.foreach { nodeId =>
        ...
        if (!probes.contains("\n")) {
          ...
          mainValue.slice(0, index).split(", ").foreach { probe =>
  ...
  test("ObjectHashAggregate metrics") {
    ...
    testSparkPlanMetrics(
      ...
      Map(
        2L -> ((
          "ObjectHashAggregate",
          Map("number of output rows" -> 4L, "number of sort fallback tasks" -> 0L))),
        1L -> ((
          "Exchange",
          Map(
            "shuffle records written" -> 4L,
        ...
        0L -> ((
          "ObjectHashAggregate",
          Map("number of output rows" -> 3L, "number of sort fallback tasks" -> 0L)))))
    ...
    withSQLConf(SQLConf.OBJECT_AGG_SORT_BASED_FALLBACK_THRESHOLD.key -> "1") {
      ...
      testSparkPlanMetrics(
        ...
        Map(
          2L -> ((
            "ObjectHashAggregate",
            Map("number of output rows" -> 4L, "number of sort fallback tasks" -> 2L))),
          0L -> ((
            "ObjectHashAggregate",
            Map("number of output rows" -> 3L, "number of sort fallback tasks" -> 1L)))))
      testSparkPlanMetricsWithPredicates(
        df3,
        1,
        Map(
  ...
  test("SortMergeJoin(outer) metrics") {
    ...
    withTempView("testDataForJoin") {
      ...
      Seq(
        (leftJoinQuery, 0L, false),
        (leftJoinQuery, 1L, true),
        (rightJoinQuery, 0L, false),
  ...
  test("BroadcastHashJoin metrics") {
    ...
    Seq((1L, false), (2L, true)).foreach { case (nodeId, enableWholeStage) =>
      ...
      testSparkPlanMetrics(
        df,
        2,
        Map(nodeId -> (("BroadcastHashJoin", Map("number of output rows" -> 2L)))),
        enableWholeStage)
    }
  }
      ...
      Seq((1L, 2L, 5L, false), (2L, 3L, 7L, true)).foreach {
        case (nodeId1, nodeId2, nodeId3, enableWholeStage) =>
          ...
          testSparkPlanMetrics(
            ...
            1,
            Map(
              nodeId1 -> (("ShuffledHashJoin", Map("number of output rows" -> 2L))),
              nodeId2 -> ((
                "Exchange",
                Map("shuffle records written" -> 2L, "records read" -> 2L))),
              nodeId3 -> ((
                "Exchange",
                Map("shuffle records written" -> 10L, "records read" -> 10L)))),
            enableWholeStage)
      }
    }
  }
  ...
  test("ShuffledHashJoin(left, outer) metrics") {
    ...
    Seq(
      (0L, "right_outer", leftDf, rightDf, 10L, false),
      ...
      .foreach { case (nodeId, joinType, leftDf, rightDf, rows, enableWholeStage) =>
        val df = leftDf
          .hint("shuffle_hash")
          .join(rightDf.hint("shuffle_hash"), $"key" === $"key2", joinType)
        testSparkPlanMetrics(
          df,
          1,
          Map(nodeId -> (("ShuffledHashJoin", Map("number of output rows" -> rows)))),
          enableWholeStage)
      }
  }
  ...
  test("SPARK-32629: ShuffledHashJoin(full outer) metrics") {
    ...
    Seq(
      ...
      (nonUniqueLeftDf, rightDf, 12, 11)).foreach { case (leftDf, rightDf, fojRows, rojRows) =>
      val fojDf = leftDf.hint("shuffle_hash").join(rightDf, $"key" === $"key2", "full_outer")
      fojDf.collect()
      ...
      val rojDf = leftDf.hint("shuffle_hash").join(rightDf, $"key" === $"key2", "right_outer")
      rojDf.collect()
      ...
      assert(
        fojBuildSize > rojBuildSize && rojBuildSize > 0,
  ...
  test("BroadcastHashJoin(outer) metrics") {
    ...
    Seq(
      ("left_outer", 0L, 5L, false),
      ("right_outer", 0L, 6L, false),
      ...
      case (joinType, nodeId, numRows, enableWholeStage) =>
        ...
        testSparkPlanMetrics(
          df,
          2,
          Map(nodeId -> (("BroadcastHashJoin", Map("number of output rows" -> numRows)))),
          enableWholeStage)
    }
  }
      ...
      withTempView("testDataForJoin") {
        ...
        Seq(
          ...
          testSparkPlanMetrics(
            df,
            2,
            Map(nodeId -> (("BroadcastNestedLoopJoin", Map("number of output rows" -> 12L)))),
            enableWholeStage)
        }
      }
    }
      ...
      withTempView("testDataForJoin") {
        ...
        Seq(true, false).foreach { enableWholeStage =>
          ...
          testSparkPlanMetrics(
            df,
            1,
            Map(0L -> (("CartesianProduct", Map("number of output rows" -> 12L)))),
            enableWholeStage)
        }
      }
    }
    ...
    withTempView("antiData") {
      ...
      Seq((0L, false), (1L, true)).foreach { case (nodeId, enableWholeStage) =>
        ...
        testSparkPlanMetrics(
          df,
          1,
          Map(nodeId -> (("SortMergeJoin", Map("number of output rows" -> 4L)))),
          enableWholeStage)
      }
    }
  }
  ...
  test("metrics can be loaded by history server") {
    ...
    val metricInfoJson =
      JsonProtocol.toJsonString(JsonProtocol.accumulableInfoToJson(metricInfo, _))
  ...
  test("range metrics") {
    val res1 = InputOutputMetricsHelper.run(spark.range(30).filter(x => x % 3 == 0).toDF())
    assert(res1 === (30L, 0L, 30L) :: Nil)

    val res2 =
      InputOutputMetricsHelper.run(spark.range(150).repartition(4).filter(x => x < 10).toDF())
    ...
    withSQLConf(SQLConf.USE_V1_SOURCE_LIST.key -> "parquet") {
      withTempDir { tempDir =>
        withTempView("pqS") {
          ...
          assert(
            res3 === (30L, 0L, 30L) :: (0L, 30L, 0L) :: (10L, 0L, 300L) :: (0L, 300L, 0L) ::
              Nil)
        }
      }
    }
  ...
  private def collectNodeWithinWholeStage[T <: SparkPlan: ClassTag](plan: SparkPlan): Seq[T] = {
    val stages = plan.collect { case w: WholeStageCodegenExec =>
      w
    }
  ...
  test("SPARK-26327: FileSourceScanExec metrics") {
    withTable("testDataForScan") {
      spark
        ...
        .write
        .partitionBy("p")
        .saveAsTable("testDataForScan")
      // The execution plan only has 1 FileScan node.
      val df = spark.sql("SELECT * FROM testDataForScan WHERE p = 1")
      testSparkPlanMetrics(
        df,
        1,
        Map(
          0L -> ((
            s"Scan parquet $SESSION_CATALOG_NAME.default.testdataforscan",
            Map("number of output rows" -> 3L, "number of files read" -> 2L)))))
    }
  }
  ...
  test("SPARK-28332: SQLMetric merge should handle -1 properly") {
    ...
    val exchanges = plan.collect { case s: ShuffleExchangeExec =>
      s
    }

    assert(exchanges.size == 2, "The query plan should have two shuffle exchanges")

    testMetricsInSparkPlanOperator(
      exchanges.head,
      Map("dataSize" -> 3200, "shuffleRecordsWritten" -> 100))
    testMetricsInSparkPlanOperator(
      exchanges(1),
      Map("dataSize" -> 0, "shuffleRecordsWritten" -> 0))
  }
  ...
  test("Add numRows to metric of BroadcastExchangeExec") {
    withSQLConf(SQLConf.AUTO_SIZE_UPDATE_ENABLED.key -> "true") {
      withTable("t1", "t2") {
        ...
        val exchanges = plan.collect { case s: BroadcastExchangeExec =>
          s
        }
      ...
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> enableWholeStage.toString) {
        withSQLConf(SQLConf.OPTIMIZE_NULL_AWARE_ANTI_JOIN.key -> "true") {
          withTable("t1", "t2") {
            ...
            val joins1 = plan1.collect { case s: BroadcastHashJoinExec =>
              s
            }
            ...
            val joins2 = plan2.collect { case s: BroadcastHashJoinExec =>
              s
            }
  ...
  test("SPARK-37099: Add numOutputRows metric for WindowGroupLimitExec") {
    ...
    val windowGroupLimit =
      df.queryExecution.executedPlan.find(_.isInstanceOf[WindowGroupLimitExec])
  ...
  test("Creating metrics with initial values") {
    ...
    assert(
      SQLMetrics.createNanoTimingMetric(sparkContext, name = "m", initValue = -1).value === 0)
