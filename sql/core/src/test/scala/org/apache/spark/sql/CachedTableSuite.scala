  ...
  test("A cached table preserves the partitioning and ordering of its cached SparkPlan") {
   ...
   // verifyNumExchanges(sql("SELECT key, count(*) FROM orderedTable GROUP BY key"), 0)
    ...
    withTempView("t1", "t2") {
      ...
      if (spark.conf.get(SQLConf.ADAPTIVE_EXECUTION_ENABLED) == false) {
        assert(query.queryExecution.executedPlan.outputPartitioning.numPartitions === 6)
      } else {
        assert(query.queryExecution.executedPlan.asInstanceOf[AdaptiveSparkPlanExec].
          executedPlan.outputPartitioning.numPartitions === 6)
      }
      checkAnswer(
        query,
