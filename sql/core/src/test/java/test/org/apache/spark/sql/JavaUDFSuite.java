...
public class JavaUDFSuite implements Serializable {
  ...
  public void udf7Test() {
    String originConf = spark.conf().get(SQLConf.DATE_EXTERNAL_TYPE().key());
    try {
      spark.conf().set(SQLConf.DATE_EXTERNAL_TYPE().key(), "LocalDate");
      spark.udf().register(
          "plusDay",
      ...
      spark.conf().set(SQLConf.DATE_EXTERNAL_TYPE().key(), originConf);
    }
  }
}
