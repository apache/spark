...
/**
 * Java test suite for TwsTester utility. Demonstrates how to use TwsTester from Java code to test
 * StatefulProcessor implementations with value state, list state, map state, timers, and initial
 * state.
 */
public class JavaTwsTesterSuite {

  /**
   * Simple processor with timer support for testing timer functionality. Registers a timer on first
   * input and emits output when timer expires.
   */
  static class SimpleTimerProcessor
      extends StatefulProcessor<String, String, Tuple2<String, String>> {
    private transient ValueState<Long> timerState;

    @Override
    public void init(OutputMode outputMode, TimeMode timeMode) {
      timerState = this.getHandle().getValueState("timerState", Encoders.LONG(), TTLConfig.NONE());
    }

    @Override
    public scala.collection.Iterator<Tuple2<String, String>> handleInputRows(
        String key, scala.collection.Iterator<String> rows, TimerValues timerValues) {
    ...
    public scala.collection.Iterator<Tuple2<String, String>> handleExpiredTimer(
        String key, TimerValues timerValues, ExpiredTimerInfo expiredTimerInfo) {
      timerState.clear();
      List<Tuple2<String, String>> result = new ArrayList<>();
  ...
  static class MultiStateProcessor
      extends StatefulProcessor<Integer, String, Tuple2<Integer, Long>> {
    ...
    @Override
    public void init(OutputMode outputMode, TimeMode timeMode) {
      countState = this.getHandle().getValueState("countState", Encoders.LONG(), TTLConfig.NONE());
      itemsList = this.getHandle().getListState("itemsList", Encoders.STRING(), TTLConfig.NONE());
      itemCountMap =
          this.getHandle()
              .getMapState("itemCountMap", Encoders.STRING(), Encoders.LONG(), TTLConfig.NONE());
    }

    @Override
    public scala.collection.Iterator<Tuple2<Integer, Long>> handleInputRows(
        Integer key, scala.collection.Iterator<String> rows, TimerValues timerValues) {
  ...
  public void testBasicProcessing() {
    ...
    List<Tuple2<Integer, String>> input =
        Arrays.asList(
            new Tuple2<>(1, "a"), new Tuple2<>(1, "b"), new Tuple2<>(2, "x"), new Tuple2<>(1, "c"));

    scala.collection.immutable.List<Tuple2<Integer, Long>> output =
        tester.test(CollectionConverters.asScala(input).toList());

    List<Tuple2<Integer, Long>> outputJava = CollectionConverters.asJava(output).stream().toList();
  ...
  public void testValueState() {
    ...
    // Process input
    scala.collection.immutable.List<Tuple2<Integer, Long>> output = tester.testOneRow(1, "a");
  ...
  public void testListState() {
    ...
    // Verify list state
    scala.collection.immutable.List<Object> listState = tester.peekListState("itemsList", 1);
  ...
  public void testTimersWithProcessingTime() {
    ...
    TwsTester.TestClock clock =
        new TwsTester.TestClock(Instant.ofEpochMilli(1000L), java.time.ZoneId.systemDefault());
    ...
    List<Tuple2<String, String>> input = Arrays.asList(new Tuple2<>("key1", "value1"));
  ...
  public void testInitialState() {
    ...
    List<Tuple2<String, Object>> initialState =
        Arrays.asList(new Tuple2<>("key1", 100L), new Tuple2<>("key2", 50L));
    ...
    List<Tuple2<String, String>> input =
        Arrays.asList(
            new Tuple2<>("key1", "a"), new Tuple2<>("key1", "b"), new Tuple2<>("key2", "x"));
    scala.collection.immutable.List<Tuple2<String, Long>> output =
        tester.test(CollectionConverters.asScala(input).toList());

    List<Tuple2<String, Long>> outputJava = CollectionConverters.asJava(output).stream().toList();
  ...
  public void testWithWatermark() {
    SimpleTimerProcessor processor = new SimpleTimerProcessor();
    TwsTester.TestClock clock =
        new TwsTester.TestClock(Instant.ofEpochMilli(10000L), java.time.ZoneId.systemDefault());
    ...
    // Set watermark
    tester.withWatermark((String s) -> new Timestamp(Long.parseLong(s)), "2 seconds");

    // Process input with event times
    List<Tuple2<String, String>> input =
        Arrays.asList(new Tuple2<>("key1", "5000"), new Tuple2<>("key1", "8000"));
  ...
  public void testMultipleBatches() {
    ...
    // First batch
    List<Tuple2<Integer, String>> batch1 =
        Arrays.asList(new Tuple2<>(1, "a"), new Tuple2<>(1, "b"));
    ...
    // Second batch
    List<Tuple2<Integer, String>> batch2 =
        Arrays.asList(new Tuple2<>(1, "c"), new Tuple2<>(2, "x"));
  ...
  public void testExistingJavaStatefulProcessor() {
    ...
    // Verify list state
    scala.collection.immutable.List<Object> listState = tester.peekListState("keyList", 1);
    List<Object> listStateJava = CollectionConverters.asJava(listState).stream().toList();
    assertEquals(2, listStateJava.size());

    // Verify map state
    scala.collection.immutable.Map<Object, Object> mapState = tester.peekMapState("keyCountMap", 1);
    Map<Object, Object> mapStateJava = CollectionConverters.asJava(mapState);
  ...
  public void testExistingProcessorWithInitialState() {
    ...
    List<Tuple2<Integer, Object>> initialState =
        Arrays.asList(new Tuple2<>(1, "init1"), new Tuple2<>(2, "init2"));
    ...
    // Process input
    List<Tuple2<Integer, String>> input = Arrays.asList(new Tuple2<>(1, "a"), new Tuple2<>(1, "b"));
