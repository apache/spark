-- A test suite for NOT ANY GROUP BY in parent side, subquery, and both predicate subquery
...
FROM   (SELECT *
        FROM   t2
        WHERE  NOT t2a = ANY (SELECT t3a
                               FROM   t3
                               WHERE  t3h != t2h)) t2
WHERE  NOT t2b <= ANY (SELECT Min(t2b)
                       FROM   t2
                       WHERE  t2b = t2b
                       GROUP  BY t2c);
...
SELECT t1a,
       max(t1b)
FROM   t1
WHERE  NOT t1c >= ANY (SELECT Max(t2b)
                       FROM   t2
                       WHERE  t1a = t2a
                       GROUP  BY t2a)
GROUP BY t1a;
...
WHERE  t1c >= ANY (SELECT t2b
                   FROM   t2
                   WHERE  NOT t2a >= ANY (SELECT Min(t3a)
                                          FROM   t3
                                          WHERE  t3a = t2a
