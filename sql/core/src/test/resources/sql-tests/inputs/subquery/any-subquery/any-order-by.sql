-- A test suite for ANY ORDER BY in parent side, subquery, and both predicate subquery
...
WHERE    t1c >= ANY (SELECT   t2c
         ...
         OR t1h <= ANY (SELECT t2h
                        FROM   t2
                        WHERE  t1h > t2h)
ORDER BY t1h DESC nulls last;

-- ORDER BY with NOT ANY
-- TC 01.08
SELECT *
FROM   t1
WHERE  NOT t1a = ANY (SELECT t2a
                      FROM   t2)
ORDER  BY t1a;
...
-- GROUP BY and ORDER BY with NOT ANY
-- TC 01.13
SELECT *
FROM   t1
WHERE  NOT t1b > ANY (SELECT   Min(t2b)
                      FROM     t2
                      GROUP BY t2a
                      ORDER BY t2a);
