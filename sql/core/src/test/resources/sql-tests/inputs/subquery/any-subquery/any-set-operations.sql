...
FROM   (SELECT *
        FROM   t2
        WHERE  t2a > ANY (SELECT t1a
                          FROM   t1)
        UNION ALL
        SELECT *
        FROM   t3
        WHERE  t3a = ANY (SELECT t1a
                          FROM   t1)) AS t3
...
SELECT t2a,
               ...
               t2i
FROM   t2
WHERE  t2a >= ANY (SELECT DISTINCT(t1a)
                   FROM   t1
                   WHERE  t1b = t2b)
GROUP  BY t2a,
          t2b,
...
FROM   (SELECT *
        FROM   (SELECT *
                FROM   t2
                WHERE  t2h >= ANY (SELECT t1h
                                   FROM   t1
                                   WHERE  t1a = t2a)
                UNION DISTINCT
                SELECT *
                FROM   t1
                WHERE  t1h >= ANY (SELECT t3h
                                   FROM   t3
                                   UNION
                                   SELECT t1h
                                   FROM   t1)
              ...
              SELECT *
              FROM   t2
              WHERE  t2h <= ANY (SELECT t1i
                                 FROM   t1)) t4
        WHERE  t4.t2b = ANY (SELECT Min(t3b)
                             FROM   t3
                             WHERE  t4.t2a = t3a)) t5;

-- UNION, UNION ALL, UNION DISTINCT, INTERSECT and EXCEPT for NOT ANY
-- TC 01.12
SELECT t2a,
       t2b,
       t2c,
...
FROM   (SELECT *
        ...
        WHERE  NOT t2a >= ANY (SELECT t1a
                               ...
                               SELECT t2a
                               FROM   t2)) AS t3
WHERE  NOT t3.t2a  = ANY (SELECT t1a
                          FROM   t1
                          INTERSECT
                          SELECT t2a
                          FROM   t2)
