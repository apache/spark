-- A test suite for ANY/SOME predicate with GROUP BY in parent side, subquery, and both
...
WHERE  t1c <= ANY (SELECT t2c
                   FROM   t2
                   WHERE  t2a = ANY (SELECT Min(t3a)
                                     FROM   t3
                                     WHERE  t3a = t2a
                                     GROUP  BY t3b)
                   GROUP  BY t2c)
GROUP BY t1a
HAVING SUM(t1b) IS NOT NULL;
