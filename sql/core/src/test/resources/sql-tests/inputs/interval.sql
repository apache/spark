...
SELECT
  ...
  from_csv(to_csv(from_csv('1, 1 day', 'a INT, b interval')), 'a INT, b interval'),
...
SELECT
  ...
  from_json(to_json(from_json('{"a":"1 days"}', 'a interval')), 'a interval');
SELECT
  ...
  to_json(from_json(to_json(map('a', interval 25 month 100 day 130 minute)), 'a interval'));
