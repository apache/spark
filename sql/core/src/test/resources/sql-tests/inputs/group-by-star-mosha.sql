...
SELECT s AS s, COUNT(*) c FROM stuff GROUP BY * HAVING SUM(f) > 0 ORDER BY s;

SELECT SUM(i) si FROM stuff GROUP BY * HAVING si > 2;

SELECT SUM(i) si FROM stuff GROUP BY * HAVING si < 2;

-- negative test, i shouldn't propagate through the aggregate so the having should fail
SELECT SUM(i) si FROM stuff GROUP BY * HAVING i > 2;

-- negative test, i shouldn't propagate through the aggregate so the order by should fail
SELECT SUM(i) si FROM stuff GROUP BY * ORDER BY i DESC;
