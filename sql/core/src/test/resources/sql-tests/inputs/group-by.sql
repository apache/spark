...
-- SPARK-37623: Support ANSI Aggregate Function: regr_slope & regr_intercept
SELECT regr_slope(y, x), regr_intercept(y, x) FROM testRegression;
SELECT regr_slope(y, x), regr_intercept(y, x) FROM testRegression WHERE x IS NOT NULL AND y IS NOT NULL;
SELECT k, regr_slope(y, x), regr_intercept(y, x) FROM testRegression GROUP BY k;
SELECT k, regr_slope(y, x), regr_intercept(y, x) FROM testRegression WHERE x IS NOT NULL AND y IS NOT NULL GROUP BY k;
