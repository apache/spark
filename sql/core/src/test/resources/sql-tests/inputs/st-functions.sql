...
SELECT ST_SetSrid(ST_GeomFromWKB(X'0101000000000000000000f03f0000000000000040'), 3857);
SELECT ST_GeogFromWKB(X'0101000000000000000000f03f0000000000000040');
SELECT CAST(ST_GeogFromWKB(X'0101000000000000000000f03f0000000000000040') AS GEOMETRY(4326));
-- NULL handling.
...
-- Error handling: mismatched SRIDs.
SELECT CAST(ST_GeomFromWKB(X'0101000000000000000000f03f0000000000000040') AS GEOGRAPHY(4326)) AS result;
-- Error handling: coordinates out of geographic bounds.
SELECT CAST(ST_GeomFromWKB(X'010100000000000000000069400000000000005940', 4326) AS GEOGRAPHY(4326)) AS result;
