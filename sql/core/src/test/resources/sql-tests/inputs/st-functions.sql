...
-- GEOGRAPHY and GEOMETRY data types can be cast to/from each other only if SRIDs match.
SELECT CAST(ST_GeogFromWKB(X'0101000000000000000000f03f0000000000000040') AS GEOMETRY(4326)) AS result;
SELECT CAST(ST_GeogFromWKB(X'0101000000000000000000f03f0000000000000040')::GEOMETRY(4326) AS GEOGRAPHY(4326)) AS result;
