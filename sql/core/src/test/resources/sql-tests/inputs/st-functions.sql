...
SELECT ST_GeomFromWKB(NULL) AS result;
SELECT hex(ST_AsBinary(ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040'))) AS result;
SELECT hex(ST_AsBinary(ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040', 0))) AS result;
SELECT hex(ST_AsBinary(ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040', 3857))) AS result;
SELECT hex(ST_AsBinary(ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040', 4326))) AS result;
-- Error handling: invalid SRID.
SELECT ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040', -1) AS result;
SELECT ST_GeomFromWKB(X'0101000000000000000000F03F0000000000000040', 9999) AS result;
