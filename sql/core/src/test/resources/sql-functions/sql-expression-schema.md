...
| org.apache.spark.sql.catalyst.expressions.aggregate.KllMergeAggBigint | kll_merge_agg_bigint | SELECT LENGTH(kll_sketch_to_string_bigint(kll_merge_agg_bigint(sketch_col))) > 0 FROM (SELECT kll_sketch_agg_bigint(col) as sketch_col FROM VALUES (1), (2), (3) tab(col)) t | struct<(length(kll_sketch_to_string_bigint(kll_merge_agg_bigint(sketch_col))) > 0):boolean> |
| org.apache.spark.sql.catalyst.expressions.aggregate.KllMergeAggDouble | kll_merge_agg_double | SELECT LENGTH(kll_sketch_to_string_double(kll_merge_agg_double(sketch_col))) > 0 FROM (SELECT kll_sketch_agg_double(col) as sketch_col FROM VALUES (CAST(1.0 AS DOUBLE)), (CAST(2.0 AS DOUBLE)), (CAST(3.0 AS DOUBLE)) tab(col)) t | struct<(length(kll_sketch_to_string_double(kll_merge_agg_double(sketch_col))) > 0):boolean> |
| org.apache.spark.sql.catalyst.expressions.aggregate.KllMergeAggFloat | kll_merge_agg_float | SELECT LENGTH(kll_sketch_to_string_float(kll_merge_agg_float(sketch_col))) > 0 FROM (SELECT kll_sketch_agg_float(col) as sketch_col FROM VALUES (CAST(1.0 AS FLOAT)), (CAST(2.0 AS FLOAT)), (CAST(3.0 AS FLOAT)) tab(col)) t | struct<(length(kll_sketch_to_string_float(kll_merge_agg_float(sketch_col))) > 0):boolean> |
