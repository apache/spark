...
| org.apache.spark.sql.catalyst.expressions.LikeAll | like_all | SELECT like_all('foo', '%foo%', '%oo') | struct<likeall(foo, %foo%, %oo):boolean> |
...
| org.apache.spark.sql.catalyst.expressions.NotLikeAll | not_like_all | SELECT not_like_all('foo', 'tee', '%yoo%') | struct<notlikeall(foo, tee, %yoo%):boolean> |
