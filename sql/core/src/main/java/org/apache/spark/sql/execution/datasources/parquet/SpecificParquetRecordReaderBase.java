...
public abstract class SpecificParquetRecordReaderBase<T> extends RecordReader<Void, T> {
  ...
  public void initialize(
    ...
    if (inputFile.isDefined() && fileFooter.isDefined() && inputStream.isDefined()) {
      // FIXME: This is a temporary regression which logically reverts SPARK-42388,
      //        wait for https://github.com/apache/parquet-java/pull/3165
      fileReader = new ParquetFileReader(
          inputFile.get(), options, inputStream.get());
    } else {
      fileReader = new ParquetFileReader(
