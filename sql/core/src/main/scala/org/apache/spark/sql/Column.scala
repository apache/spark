...
class Column(protected[sql] val expr: Expression) extends Logging {
  ...
  def when(condition: Column, value: Any): Column = this.expr match {
    case CaseWhen(conditions, values) =>
      withExpr { CaseWhen(conditions :+ lit(condition).expr, values :+ lit(value).expr) }
    case _ =>
  ...
  def otherwise(value: Any): Column = this.expr match {
    case CaseWhen(conditions, values) =>
      if (values.size == conditions.size) {
        withExpr { CaseWhen(conditions, values :+ lit(value).expr) }
      } else {
        throw new IllegalArgumentException(
