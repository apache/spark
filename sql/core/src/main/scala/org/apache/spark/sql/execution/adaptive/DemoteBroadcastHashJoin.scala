...
import org.apache.spark.sql.catalyst.plans.logical.{HintInfo, Join, JoinStrategyHint, LogicalPlan, NO_BROADCAST_HASH}
import org.apache.spark.sql.catalyst.rules.Rule

/**
 * This optimization rule detects a join child that has a high ratio of empty partitions and adds a
 * NO_BROADCAST_HASH hint to avoid it being broadcast, as shuffle join is faster in this case: many
 * tasks complete immediately since one join side is empty.
 */
object DemoteBroadcastHashJoin extends Rule[LogicalPlan] with JoinSelectionHelper {
