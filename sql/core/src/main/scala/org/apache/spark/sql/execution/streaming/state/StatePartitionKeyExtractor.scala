...
/**
 * Trait for extracting partition keys from state key rows. The partition key is the key used by
 * shuffle. This is used for offline state repartitioning.
 */
trait StatePartitionKeyExtractor {
   ...
   *
   * @note
   *   Depending on the implementation, it might not be safe to buffer the returned UnsafeRow
   *   across multiple calls of this method, due to UnsafeRow re-use. If you are holding on to the
   *   row between multiple calls, you should copy the row.
   */
 ...
 *   The schema of the state key row
 */
...
/**
 * State partition key extractor that returns the field at the specified index of the state key
 * row as the partition key row.
 *
 * @param stateKeySchema
 *   The schema of the state key row
 * @param partitionKeyIndex
 *   The index of the field to extract as the partition key
 */
...
/**
 * State partition key extractor that drops the last N fields of the state key row and returns the
 * remaining fields as the partition key row.
 *
 * @param stateKeySchema
 *   The schema of the state key row
 * @param numLastColsToDrop
 *   The number of last columns to drop in the state key
 */
class DropLastNFieldsStatePartitionKeyExtractor(
    stateKeySchema: StructType,
    numLastColsToDrop: Int)
    extends StatePartitionKeyExtractor {
  override lazy val partitionKeySchema: StructType = {
    require(
      numLastColsToDrop < stateKeySchema.length,
