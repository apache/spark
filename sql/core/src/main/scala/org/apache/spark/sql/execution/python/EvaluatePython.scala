...
object EvaluatePython {
  ...
  def needConversionInPython(dt: DataType): Boolean = dt match {
    case DateType | TimestampType | TimestampNTZType | VariantType | _: DayTimeIntervalType |
        _: TimeType | _: GeometryType | _: GeographyType =>
      true
   ...
   */
  def toJava(obj: Any, dataType: DataType, binaryAsBytes: Boolean): Any = {
    (obj, dataType) match {
      case (null, _) => null
  ...
  /**
   * Make a converter that converts `obj` to the type specified by the data type, or returns null
  ...
  def makeFromJava(dataType: DataType): Any => Any = dataType match {
          ...
          case c if c.getClass.isArray =>
            ...
            if (array.length != fields.length) {
              throw new SparkIllegalArgumentException(
                ...
                messageParameters =
                  Map("expected" -> fields.length.toString, "actual" -> array.length.toString))
            }
    ...
    case VariantType =>
      (obj: Any) =>
        nullSafeConvert(obj) { case s: java.util.HashMap[_, _] =>
          new VariantVal(
    ...
    case g: GeographyType =>
      (obj: Any) =>
        nullSafeConvert(obj) { case s: java.util.HashMap[_, _] =>
          ...
          STUtils.stGeogFromWKB(s.get("wkb").asInstanceOf[Array[Byte]])
        }
    ...
    case g: GeometryType =>
      (obj: Any) =>
        nullSafeConvert(obj) { case s: java.util.HashMap[_, _] =>
          ...
          STUtils.stGeomFromWKB(s.get("wkb").asInstanceOf[Array[Byte]], geometrySrid)
        }
   ...
   * Structure: bytes(bytearray_data) where bytearray_data is pickled by razorvine's default
   * pickler.
   */
  ...
  /**
   * This should be called before trying to serialize any above classes un cluster mode, this
   * should be put in the closure
   */
