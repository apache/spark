...
case class DescribeTableCommand(
  ...
  private def describeFormattedTableInfo(table: CatalogTable, buffer: ArrayBuffer[Row]): Unit = {
    ...
    append(buffer, "Location:", table.storage.locationUri.map(_.getPath).getOrElse(""), "")
  ...
  private def describeFormattedDetailedPartitionInfo(
    ...
    append(buffer, "Location:", partition.storage.locationUri.map(_.getPath).getOrElse(""), "")
  ...
  private def showDataSourceTableOptions(metadata: CatalogTable, builder: StringBuilder): Unit = {
    ...
    val dataSourceOptions = metadata.storage.properties.map {
      ...
      if (metadata.tableType == MANAGED) {
        ...
        Some(s"path '${escapeSingleQuotedString(location.getPath)}'")
      }
    }
