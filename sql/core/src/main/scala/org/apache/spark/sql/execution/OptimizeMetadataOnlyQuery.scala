 ...
 *   2. aggregate function on partition columns with DISTINCT. e.g. SELECT col1, count(DISTINCT
 *      col2) FROM tbl GROUP BY col1.
...
case class OptimizeMetadataOnlyQuery(catalog: SessionCatalog) extends Rule[LogicalPlan] {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    plan.transform {
      case a @ Aggregate(
            _,
            aggExprs,
            child @ PhysicalOperation(projectList, filters, PartitionedRelation(partAttrs, rel)),
            _) =>
        ...
        if (AttributeSet((projectList ++ filters).flatMap(_.references)).subsetOf(partAttrs)) {
          ...
          val aggFunctions = aggExprs.flatMap(_.collect { case agg: AggregateExpression =>
            agg
          })
          ...
          if (isAllDistinctAgg) {
            logWarning(
              "Since configuration `spark.sql.optimizer.metadataOnly` is enabled, " +
  ...
  private def replaceTableScanWithPartitionMetadata(
    ...
    val normalizedFilters = partFilters
      .map { e =>
      ...
      case plan if plan eq relation =>
        relation match {
          ...
          case relation: HiveTableRelation =>
            ...
            val timeZoneId = caseInsensitiveProperties
          ...
          case _ =>
            throw SparkException.internalError(
              s"unrecognized table scan node: $relation, " +
