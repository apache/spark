...
class UnivocityParser(
  ...
  // Retrieve the raw malformed row.
  ...
  //   `valueConverters` keeps the positions of input token indices (by its index) to required
  ...
  //   `tokenIndexArr` keeps the positions of input token indices (by its index) to required
  //   fields given the required CSV data schema (by its value). In this case, [2, 1].
  ...
  private def convert(tokens: Array[String]): InternalRow = {
    if (tokens.length != schema.length) {
      ...
      throw BadRecordException(
        () => UTF8String.fromString(getCurrentInput()),
        getPartialResult,
        new RuntimeException("Malformed CSV line"))
    } else {
      var i: Int = 0
...
private[csv] object UnivocityParser {
  ...
  def parseIterator(
    ...
    filteredLines.map(line => parser.parse(line))
  }
}
