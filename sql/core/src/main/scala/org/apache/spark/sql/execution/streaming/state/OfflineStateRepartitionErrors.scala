...
/**
 * Base class for exceptions thrown when an invalid parameter is passed into the repartition
 * operation.
 */
...
class StateRepartitionParameterIsEmptyError(parameter: String)
    extends StateRepartitionInvalidParameterError(parameter, subClass = "IS_EMPTY")
...
class StateRepartitionParameterIsNullError(parameter: String)
    extends StateRepartitionInvalidParameterError(parameter, subClass = "IS_NULL")

/**
 * Base class for exceptions thrown when the checkpoint location is in an invalid state for
 * repartitioning.
 */
...
class StateRepartitionLastBatchFailedError(checkpointLocation: String, lastBatchId: Long)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
...
class StateRepartitionNoBatchFoundError(checkpointLocation: String)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
      subClass = "NO_BATCH_FOUND",
      messageParameters = Map.empty)

class StateRepartitionNoCommittedBatchError(checkpointLocation: String)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
      subClass = "NO_COMMITTED_BATCH",
      messageParameters = Map.empty)

class StateRepartitionOffsetSeqNotFoundError(checkpointLocation: String, batchId: Long)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
...
class StateRepartitionUnsupportedOffsetSeqVersionError(checkpointLocation: String, version: Int)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
      subClass = "UNSUPPORTED_OFFSET_SEQ_VERSION",
      messageParameters = Map("version" -> version.toString))

class StateRepartitionUnsupportedProviderError(checkpointLocation: String, provider: String)
    extends StateRepartitionInvalidCheckpointError(
      checkpointLocation,
