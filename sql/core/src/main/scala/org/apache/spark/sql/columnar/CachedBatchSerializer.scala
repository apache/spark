...
/**
 * Basic interface that all cached batches of data must support. This is primarily to allow for
 * metrics to be handled outside of the encoding and decoding steps in a standard way.
 */
@DeveloperApi
@Since("3.1.0")
trait CachedBatch {
...
trait CachedBatchSerializer extends Serializable {
   ...
   * this is mostly supported by input formats like parquet and orc, but more operations are
   * likely to be supported soon.
   * @param schema
  ...
  /**
   * Attempt to convert a query plan to its columnar equivalence for columnar caching. Called on
   * the query plan that is about to be cached once [[supportsColumnarInput]] returns true on its
   * output schema.
   *
   ...
   *   The plan to convert.
   * @return
   *   The output plan. Could either be a columnar plan if the input plan is convertible, or the
   *   input plan unchanged if no viable conversion can be done.
   */
  @DeveloperApi
  @Since("4.1.0")
  ...
  /**
   * Convert an `RDD[ColumnarBatch]` into an `RDD[CachedBatch]` in preparation for caching the
   * data. This will only be called if `supportsColumnarInput()` returned true for the given
   * schema and the plan up to this point would could produce columnar output without modifying
   * it.
   * @param input
   *   the input `RDD` to be converted.
   * @param schema
  ...
  /**
   * Builds a function that can be used to filter batches prior to being decompressed. In most
   * cases extending [[SimpleMetricsCachedBatchSerializer]] will provide the filter logic
   * necessary. You will need to provide metrics for this to work. [[SimpleMetricsCachedBatch]]
   * provides the APIs to hold those metrics and explains the metrics used, really just min and
   * max. Note that this is intended to skip batches that are not needed, and the actual filtering
   * of individual rows is handled later.
   * @param predicates
   *   the set of expressions to use for filtering.
   * @param cachedAttributes
   *   the schema/attributes of the data that is cached. This can be helpful if you don't store it
   *   with the data.
   * @return
   *   a function that takes the partition id and the iterator of batches in the partition. It
   *   returns an iterator of batches that should be decompressed.
   */
  def buildFilter(
   ...
   *   the fields that should be loaded from the data and the order they should appear in the
   *   output batch.
   * @param conf
   ...
   *   the attributes of the data in the batch.
   * @param selectedAttributes
   *   the field that should be loaded from the data and the order they should appear in the
   *   output rows.
   * @param conf
   *   the configuration for the job.
   * @return
...
/**
 * A [[CachedBatch]] that stores some simple metrics that can be used for filtering of batches
 * with the [[SimpleMetricsCachedBatchSerializer]]. The metrics are returned by the stats value.
 * For each column in the batch 5 columns of metadata are needed in the row.
 */
@DeveloperApi
@Since("3.1.0")
trait SimpleMetricsCachedBatch extends CachedBatch {
  ...
  /**
   * Holds stats for each cached column. The optional `upperBound` and `lowerBound` should be of
   * the same type as the original column. If they are null, then it is assumed that they are not
   * provided, and will not be used for filtering. <ul> <li>`upperBound` (optional)</li>
   * <li>`lowerBound` (Optional)</li> <li>`nullCount`: `Int`</li> <li>`rowCount`: `Int`</li>
   * <li>`sizeInBytes`: `Long`</li> </ul> These are repeated for each column in the original
   * cached data.
   */
...
/**
 * Provides basic filtering for [[CachedBatchSerializer]] implementations. The requirement to
 * extend this is that all of the batches produced by your serializer are instances of
 * [[SimpleMetricsCachedBatch]]. This does not calculate the metrics needed to be stored in the
 * batches. That is up to each implementation. The metrics required are really just min and max
 * values and those are optional especially for complex types. Because those metrics are simple
 * and it is likely that compression will also be done on the data we thought it best to let each
 * implementation decide on the most efficient way to calculate the metrics, possibly combining
 * them with compression passes that might also be done across the data.
 */
@DeveloperApi
@Since("3.1.0")
  ...
  override def buildFilter(
    ...
    @transient lazy val buildFilter: PartialFunction[Expression, Expression] = {
      ...
      case In(a: AttributeReference, list: Seq[Expression])
        ...
        list
          .map(l =>
            statsFor(a).lowerBound <= l.asInstanceOf[Literal] &&
    ...
    val partitionFilters: Seq[Expression] = {
      predicates.flatMap { p =>
        ...
        val boundFilter =
          filter.map(BindReferences.bindReference(_, statsSchema, allowFailures = true))

        boundFilter.foreach(_ =>
          filter.foreach(f =>
            logInfo(
              log"Predicate ${MDC(PREDICATE, p)} generates " +
