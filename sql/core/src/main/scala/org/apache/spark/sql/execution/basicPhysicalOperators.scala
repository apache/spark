...
case class RangeExec(range: org.apache.spark.sql.catalyst.plans.logical.Range)
  ...
  protected override def doProduce(ctx: CodegenContext): String = {
    ...
    val taskContextAccessor = ctx.addMutableState("TaskContext",
      taskContext, s"$taskContext = TaskContext.get();")
    ...
    val numElementsTodoAccessor = ctx.addMutableState("long",
      numElementsTodo, s"$numElementsTodo = 0L;")
    ...
    ctx.addMutableState("scala.collection.Iterator", input, s"$input = inputs[0];", inline = true)

    val localIdx = ctx.freshName("localIdx")
    val localEnd = ctx.freshName("localEnd")
    val range = ctx.freshName("range")
    val shouldStop = if (isShouldStopRequired) {
      s"if (shouldStop()) { $numberAccessor = $value + ${step}L; return; }"
    } else {
    ...
    s"""
      ...
      |
      | while (true) {
      |   long $range = $batchEndAccessor - $numberAccessor;
      |   if ($range != 0L) {
      |     int $localEnd = (int)($range / ${step}L);
      |     for (int $localIdx = 0; $localIdx < $localEnd; $localIdx++) {
      |       long $value = ((long)$localIdx * ${step}L) + $numberAccessor;
      |       ${consume(ctx, Seq(ev))}
      |       $shouldStop
      |     }
      |     $numberAccessor = $batchEndAccessor;
      |   }
      |
      |   $taskContextAccessor.killTaskIfInterrupted();
      |
      |   long $nextBatchTodo;
