 ...
 * checkpoint directory. It supports Auto snapshot repair, which will automatically handle corrupt
 * snapshots and skip them, by using another snapshot version before the corrupt one. If no
 * snapshot exists before the corrupt one, then it will use the 0 version snapshot (represents
 * initial/empty snapshot).
 *
 * @param autoSnapshotRepairEnabled
 *   If true, it will handle corrupt snapshot
 * @param numFailuresBeforeActivating
 *   If auto snapshot repair is enabled, number of failures before activating it
 * @param maxChangeFileReplay
 *   If auto snapshot repair is enabled, maximum difference between the requested snapshot version
 *   and the selected snapshot version
 * @param loggingId
 *   To append to log messages
 */
abstract class AutoSnapshotLoader(
  ...
  /**
   * Attempt to load the specified snapshot version from the checkpoint directory. Should throw an
   * exception if the snapshot is corrupt.
   * @note
   *   Must support loading version 0
   */
  ...
  /**
   * Load the latest snapshot for the specified version from the checkpoint directory. If Auto
   * snapshot repair is enabled, the snapshot version loaded may be lower than the latest snapshot
   * version, if the latest is corrupt.
   *
   * @param versionToLoad
   *   The version to load latest snapshot for
   * @return
   *   The actual loaded snapshot version and if it was due to auto repair
   */
  def loadSnapshot(versionToLoad: Long): (Long, Boolean) = {
    ...
    if (loadedSnapshot.isEmpty) {
      ...
      val selectedRemainingSnapshots =
        remainingEligibleSnapshots.filter(s => versionToLoad - s <= maxChangeFileReplay)

      logInfo(
        log"Attempting to auto repair snapshot by skipping " +
      ...
      for (snapshotVersion <- selectedRemainingSnapshots if loadedSnapshot.isEmpty) {
        ...
        try {
          ...
          logInfo(
            log"Successfully loaded snapshot version " +
          ...
          case NonFatal(e) =>
            logError(
              log"Failed to load snapshot version " +
                ...
                log"the next eligible snapshot version",
              e)
            onLoadSnapshotFromCheckpointFailure()
            lastException = e
        }
      ...
      if (loadedSnapshot.isEmpty) {
        ...
        logError(
          log"Auto snapshot repair failed to load any snapshot:" +
