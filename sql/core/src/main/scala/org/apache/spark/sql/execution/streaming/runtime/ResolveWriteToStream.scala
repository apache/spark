...
object ResolveWriteToStream extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperators {
    case s: WriteToStreamStatement =>
      ...
      if (conf.adaptiveExecutionEnabled) {
        logWarning(
          log"${MDC(CONFIG, SQLConf.ADAPTIVE_EXECUTION_ENABLED.key)} " +
      ...
      if (conf.isUnsupportedOperationCheckEnabled) {
        if (s.trigger.isInstanceOf[RealTimeTrigger]) {
          UnsupportedOperationChecker.checkAdditionalRealTimeModeConstraints(
            s.inputQuery,
            s.outputMode)
        }
  ...
  def resolveCheckpointLocation(s: WriteToStreamStatement): (String, Boolean) = {
    ...
    val checkpointLocation = s.userSpecifiedCheckpointLocation
      .map { userSpecified =>
      ...
      .getOrElse {
        if (s.useTempCheckpointLocation) {
          ...
          logWarning(
            log"Temporary checkpoint location created which is deleted normally when" +
    ...
    logInfo(
      log"Checkpoint root ${MDC(CHECKPOINT_LOCATION, checkpointLocation)} " +
        log"resolved to ${MDC(CHECKPOINT_ROOT, resolvedCheckpointRoot)}.")
    (resolvedCheckpointRoot, deleteCheckpointOnStop)
  }

  /**
   * Checks for duplicate source names across all streaming sources. This validation always runs
   * regardless of source evolution enforcement.
   */
  private def checkDuplicateSourceNames(plan: LogicalPlan): Unit = {
    val allSourceNames = plan.collect {
      case StreamingRelation(_, _, _, sourceIdentifyingName) => sourceIdentifyingName
      case StreamingRelationV2(_, _, _, _, _, _, _, _, sourceIdentifyingName) =>
