...
/**
 * An [[SQLFileCommitProtocol]] implementation backed by an underlying Hadoop OutputCommitter
 * (from the newer mapreduce API, not the old mapred API).
 *
 * Unlike Hadoop's OutputCommitter, this implementation is serializable and
 * allows specifying the actual  Hadoop output committer using an option specified in SQLConf..
 *
...
class SQLHadoopMapReduceCommitProtocol(
   ...
   * [[SQLHadoopMapReduceCommitProtocol#path]] need not be a valid [[org.apache.hadoop.fs.Path]] for
  ...
  protected def setupCommitter(context: TaskAttemptContext): OutputCommitter = {
    ...
    var committer = format.getOutputCommitter(context)
