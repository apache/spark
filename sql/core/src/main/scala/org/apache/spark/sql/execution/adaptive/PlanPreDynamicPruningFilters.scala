...
*/
case class PlanPreDynamicPruningFilters(sparkSession: SparkSession) extends Rule[SparkPlan] {
  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    ...
    plan.transformAllExpressionsWithPruning(_.containsPattern(DYNAMIC_PRUNING_SUBQUERY)) {
      case dpp @ DynamicPruningSubquery(
...
//          HashJoin.rewriteKeyExpr(buildKeys), executedPlan.output)
//        val mode = broadcastMode(packedKeys, executedPlan.output)
            ...
            case BroadcastHashJoinExec(_, _, _, BuildLeft, _, left, _, _) =>
              val childPlan =
                if (left.isInstanceOf[BroadcastQueryStageExec]) {
                  left.asInstanceOf[BroadcastQueryStageExec].plan
                } else {
                  left
                }
              childPlan.sameResult(exchange)
            case BroadcastHashJoinExec(_, _, _, BuildRight, _, _, right, _) =>

              val rightPlan: SparkPlan = right.canonicalized

              val childPlan =
                if (right.isInstanceOf[BroadcastQueryStageExec]) {
                  right.asInstanceOf[BroadcastQueryStageExec].plan
                } else {
                  right
                }
              val boolvalue =  childPlan.sameResult(exchange)
