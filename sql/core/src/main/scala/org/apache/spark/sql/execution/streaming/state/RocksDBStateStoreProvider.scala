...
private[sql] class RocksDBStateStoreProvider
  ...
  class RocksDBStateStore(lastVersion: Long) extends StateStore {
    ...
    override def iterator(colFamilyName: String): Iterator[UnsafeRowPair] = {
      println("I got here inside iterator: " + colFamilyName)
      val kvEncoder = keyValueEncoderMap.get(colFamilyName)
      println("I got inside iterator func, kvEncoder: " + kvEncoder)
  ...
  override def init(
    ...
    println(s"I am here inside RocksDBStateStoreProvider, init params: \n" +
      s"stateStoreId = $stateStoreId, useColumnFamilies=$useColumnFamilies, " +
      s"storeConf = $storeConf, hadoopConf=$hadoopConf")
  ...
  override def getReadStore(version: Long): StateStore = {
    try {
      println("I am inside rocksdbprovider.getReadStore, version: " + version)
      if (version < 0) {
        throw QueryExecutionErrors.unexpectedStateStoreVersion(version)
      }
  ...
  val keyValueEncoderMap = new java.util.concurrent.ConcurrentHashMap[String,
