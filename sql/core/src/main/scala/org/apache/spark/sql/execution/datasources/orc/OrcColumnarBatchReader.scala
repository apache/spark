...
private[orc] class OrcColumnarBatchReader extends RecordReader[Void, ColumnarBatch] with Logging {
  ...
  def setRequiredSchema(
    ...
    if (memMode == MemoryMode.OFF_HEAP) {
      columnVectors = OffHeapColumnVector.allocateColumns(capacity, fullSchema)
    } else {
      columnVectors = OnHeapColumnVector.allocateColumns(capacity, fullSchema)
    }
    columnarBatch = new ColumnarBatch(fullSchema, columnVectors.toArray, capacity)
