...
import org.apache.spark.sql.execution.streaming.state.{NoPrefixKeyStateEncoderType, StateStore, StateStoreErrors}
...
class ListStateImpl[S](
  ...
  store.createColFamilyIfAbsent(stateName, NoPrefixKeyStateEncoderType, KEY_ROW_SCHEMA,
    numColsPrefixKey = 0, VALUE_ROW_SCHEMA, useMultipleValuesPerKey = true)
