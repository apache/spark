...
case class TransformWithStateExec(
  ...
  /**
   * We initialize this processor handle in the driver to run the init function and fetch the
   * schemas of the state variables initialized in this processor.
   * @return
   *   a new instance of the driver processor handle
   */
  ...
  /**
   * This method is used for the driver-side stateful processor after we have collected all the
   * necessary schemas. This instance of the stateful processor won't be used again.
   */
  ...
  /**
   * Fetching the columnFamilySchemas from the StatefulProcessorHandle after init is called.
   */
  override def getColFamilySchemas(
    ...
    val defaultSchema = StateStoreColFamilySchema(
  ...
  override protected def withNewChildrenInternal(
      newLeft: SparkPlan,
      newRight: SparkPlan): TransformWithStateExec = {
    if (hasInitialState) {
  ...
  private def handleInputRows(
      keyRow: UnsafeRow,
      valueRowIter: Iterator[InternalRow]): Iterator[InternalRow] = {
  ...
  private def processTimers(
    ...
    timeMode match {
      case ProcessingTime =>
        ...
        processorHandle
          .getExpiredTimers(batchTimestamp)
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    if (hasInitialState) {
      ...
      if (isStreaming) {
        ...
        child
          .execute()
          .mapPartitionsWithIndex[InternalRow]((i: Int, iter: Iterator[InternalRow]) => {
   ...
   * @param singleIterator
   *   The iterator of rows to process
   * @return
   *   An iterator of rows that are the result of processing the input rows
   */
  ...
  private def processDataWithInitialState(
    ...
    if (processorHandle.getQueryInfo().getBatchId == 0) {
      ...
      val groupedInitialStateIter =
        GroupedIterator(initStateIterator, initialStateGroupingAttrs, initialState.output)
      groupedInitialStateIter.foreach { case (keyRow, valueRowIter) =>
