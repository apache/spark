...
case class SortMergeJoinExec(
  ...
  private def genScanner(ctx: CodegenContext): (String, String) = {
    ...
    val matchesAccessor = ctx.addMutableState(clsName, matches,
  ...
  override def doProduce(ctx: CodegenContext): String = {
    ...
    val leftInputAccessor = ctx.addMutableState("scala.collection.Iterator",
      leftInput, s"$leftInput = inputs[0];")
    val rightInput = ctx.freshName("rightInput")
    val rightInputAccessor = ctx.addMutableState("scala.collection.Iterator",
      rightInput, s"$rightInput = inputs[1];")
