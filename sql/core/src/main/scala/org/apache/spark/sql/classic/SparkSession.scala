...
class SparkSession private(
  ...
  override private[sql] def refreshTable(
      session: org.apache.spark.sql.SparkSession, tableName: String): Unit =
    sessionState.doRefreshTable(session, tableName)
  ...
  override private[sql] def dropTempViewIfExists(
      session: org.apache.spark.sql.SparkSession,
      viewName: String): Boolean =
    sessionState.dropTempViewIfExists(session, viewName)
