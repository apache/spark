...
object LogicalQueryStageStrategy extends Strategy with PredicateHelper {
  ...
  private def isBroadcastStageWithHashedBroadcastMode(plan: LogicalPlan): Boolean = plan match {
    case LogicalQueryStage(_, bqs: BroadcastQueryStageExec) =>
      bqs.broadcast.mode match {
        case HashedRelationBroadcastMode(_, _) => true
        case _ => false
      }
    case _ => false
  }
  ...
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, condition, left, right, hint)
        if isBroadcastStageWithHashedBroadcastMode(left) ||
             isBroadcastStageWithHashedBroadcastMode(right) =>
      val buildSide = if (isBroadcastStageWithHashedBroadcastMode(left)) BuildLeft else BuildRight
      Seq(BroadcastHashJoinExec(
        leftKeys, rightKeys, joinType, buildSide, condition, planLater(left), planLater(right)))

    case j @ ExtractSingleColumnNullAwareAntiJoin(leftKeys, rightKeys)
        // TODO: support IdentityBroadcastMode change to HashedRelationBroadcastMode
        if isBroadcastStageWithHashedBroadcastMode(j.right) =>
    ...
    case j @ Join(left, right, joinType, condition, _)
      ...
      val buildSide = if (isBroadcastStageWithIdentityBroadcastMode(left)) BuildLeft else BuildRight
