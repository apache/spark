 ...
 * logical plan, which effectively transposes a DataFrame by turning rows into columns based on a
 ...
 *   - Non-index columns are cast to a common data type, determined by finding the least common
 *     type that can accommodate all non-index columns.
 *   - The data is sorted by the index column, and rows with `null` index values are excluded from
 *     the transpose operation.
 *   - The transposed DataFrame is constructed by turning the original rows into columns, with the
 *     index column values becoming the new column names and the non-index column values
...
class ResolveTranspose(sparkSession: SparkSession) extends Rule[LogicalPlan] {
      ...
      dataTypes.reduce { (dt1, dt2) =>
        AnsiTypeCoercion.findTightestCommonType(dt1, dt2).getOrElse {
          throw new AnalysisException(
            ...
            messageParameters = Map("dt1" -> toSQLType(dt1), "dt2" -> toSQLType(dt2)))
        }
      }
    }
  }
  ...
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsWithPruning(_.containsPattern(TreePattern.UNRESOLVED_TRANSPOSE)) {
      case t @ UnresolvedTranspose(indices, child)
          if child.resolved && indices.forall(_.resolved) =>
        assert(
          indices.length == 0 || indices.length == 1,
        ...
        val indexColumnAsString = inferredIndexColumn match {
          ...
          case _ =>
            throw new AnalysisException(
              errorClass = "TRANSPOSE_INVALID_INDEX_COLUMN",
              messageParameters = Map("reason" -> s"Index column must be an atomic attribute"))
        }

        // Cast non-index columns to the least common type
        val nonIndexColumns =
          child.output.filterNot(_.exprId == inferredIndexColumn.asInstanceOf[Attribute].exprId)
        ...
        val sortedChild =
          Sort(Seq(SortOrder(inferredIndexColumn, Ascending)), global = true, nonNullChild)
        ...
        if (fullCollectedRows.isEmpty) {
          ...
          val transposedMatrix =
            transposeMatrix(fullCollectedRows, nonIndexColumnNames, nonIndexColumnDataTypes)
          ...
          val valueAttrs = transposedColumnNames.map { value =>
            AttributeReference(value, commonType)()
          }
