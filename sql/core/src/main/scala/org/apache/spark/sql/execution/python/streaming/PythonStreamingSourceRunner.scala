...
class PythonStreamingSourceRunner(func: PythonFunction, outputSchema: StructType)
   ...
   */
  def latestOffset(start: Option[Offset], limit: Option[ReadLimit] = None): String = {
    dataOut.writeInt(LATEST_OFFSET_FUNC_ID)
    start match {
      case Some(s) =>
        dataOut.writeBoolean(true)
        PythonWorkerUtils.writeUTF(s.json(), dataOut)
      case None =>
        dataOut.writeBoolean(false)
    }
    val limitToUse = limit.getOrElse(ReadLimit.allAvailable())
    val limitJson =
      try {
        val maxRowsMethod = limitToUse.getClass.getMethod("maxRows")
        val maxRowsValue = maxRowsMethod.invoke(limitToUse).asInstanceOf[Long]
        s"""{"type": "maxRows", "maxRows": ${maxRowsValue}}"""
      } catch {
        case _: NoSuchMethodException => s"""{"type": "allAvailable"}"""
      }
    PythonWorkerUtils.writeUTF(limitJson, dataOut)
    dataOut.flush()
