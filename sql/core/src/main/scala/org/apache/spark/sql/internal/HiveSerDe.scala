...
object HiveSerDe {
   ...
   *
   * @param source
   *   Currently the source abbreviation can be one of the following: SequenceFile, RCFile, ORC,
   *   PARQUET, and case insensitive.
   * @return
   *   HiveSerDe associated with the specified source
   */
   ...
   *
   * @param serde
   *   Hive SerDe information.
   * @return
   *   Spark data source name associated with the specified Hive Serde.
   */
  ...
  def getDefaultStorage(conf: SQLConf): CatalogStorageFormat = {
    ...
    CatalogStorageFormat.empty.copy(
      inputFormat = defaultHiveSerde
        .flatMap(_.inputFormat)
        .orElse(Some("org.apache.hadoop.mapred.TextInputFormat")),
      outputFormat = defaultHiveSerde
        .flatMap(_.outputFormat)
        .orElse(Some("org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat")),
      serde = defaultHiveSerde
        .flatMap(_.serde)
