  ...
  def unwrapper(
    ...
    dataType match {
      ...
      case ArrayType(tpe, _) =>
        ...
        val un = unwrapper(tpe, conf, ioSchema, level + 1)
      ...
      case MapType(keyType, valueType, _) =>
        ...
        val keyUnwrapper = unwrapper(keyType, conf, ioSchema, level + 2)
        val valueUnwrapper = unwrapper(valueType, conf, ioSchema, level + 2)
        wrapperConvertException(data => {
          val list = data.split(separator)
          list.map { kv =>
            val kvList = kv.split(keyValueSeparator)
            keyUnwrapper(kvList(0)) -> valueUnwrapper(kvList(1))
          }.toMap
        }, converter)
      case StructType(fields) =>
        val separator = ioSchema.getSeparator(level)
        val unwrappers = fields.map(f => unwrapper(f.dataType, conf, ioSchema, level + 1))
        wrapperConvertException(data => {
          val list = data.split(separator)
          Row.fromSeq(list.zip(unwrappers).map {
            case (data: String, unwrapper: (String => Any)) => unwrapper(data)
          })
        }, converter)
