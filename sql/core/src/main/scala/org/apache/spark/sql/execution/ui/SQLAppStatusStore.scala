...
class SQLExecutionUIData(
     ...
     * execution is still running. During execution, aggregate metrics need to be retrieved from
     * the SQL listener instance.
     */
    @JsonDeserialize(keyAs = classOf[JLong])
    val metricValues: Map[Long, String],
    /**
     * A unique identifier for the query. For backward compatibility, queryId is null when we
     * parse event logs generated by old versions of Spark.
     */
