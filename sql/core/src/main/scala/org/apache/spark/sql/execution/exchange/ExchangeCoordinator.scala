...
import org.apache.spark.sql.execution.{ShuffledRowRDD, SkewShuffleRowRDD, SparkPlan}
...
class ExchangeCoordinator(
  ...
  def skewPartitionIdx(
    ...
    val isSkew = skewPartition.flatten.length > 0 && isJoin &&
      skewSize > advisoryTargetPostShuffleInputSize
    (0 until numExchanges).map(k =>
      if (isSkew) {
        ...
        if (otherPos == skewPartition(otherPart).length - 1) {
         ...
         if ( thisPos == skewPartition(k).length - 1) {
           skewPartitionIdx.append((1, skewPartition(k)(thisPos)._2, prePartitionNum(k)))
           partitionNum += prePartitionNum(k)
        }
        (partitionNum, skewPartitionIdx.toArray)
      } else (0, Array[(Int, Int, Int)]((-1, 0, 0)))).toArray
    ...
    if (!estimated) {
      ...
      while (k < numExchanges) {
        ...
        val rdd = if (skewPartitionIndices(k)._1 > 0) {
          ...
          new SkewShuffleRowRDD(shuffleDependencies(k), skewPartitionIndices(k)._2)
