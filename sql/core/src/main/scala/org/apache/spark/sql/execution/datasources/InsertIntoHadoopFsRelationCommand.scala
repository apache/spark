  ...
  override def run(sparkSession: SparkSession, child: SparkPlan): Seq[Row] = {
    ...
    if (doInsertion) {
      ...
      def refreshUpdatedPartitions(updatedPartitionPaths: Set[String]): Unit = {
        val updateAndNewPartitions = updatedPartitionPaths.map(PartitioningUtils.parsePathFragment)
        if (partitionsTrackedByCatalog) {
          ...
          if (updatedPartitions.nonEmpty) {
            AlterTableDropPartitionCommand(
              catalogTable.get.identifier, updatedPartitions.toSeq,
              ifExists = true, purge = false,
              retainData = true /* already deleted */).run(sparkSession)
          }
          if (updateAndNewPartitions.nonEmpty) {
            AlterTableAddPartitionCommand(
              catalogTable.get.identifier, updateAndNewPartitions.toSeq.map(p => (p, customPartitionLocations.get(p))),
