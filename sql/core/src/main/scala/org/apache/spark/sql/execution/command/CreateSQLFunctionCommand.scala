...
case class CreateSQLFunctionCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val newFunction = {
      ...
      val deterministic = deriveDeterminism(analyzedPlan)
  ...
  /**
   * Derive the determinism of the SQL function from its function body.
   */
  private def deriveDeterminism(plan: LogicalPlan): Boolean = {
    val deterministic = plan.deterministic
    if (isDeterministic.exists(_ != deterministic)) {
      logWarning(s"Derived function determinism ($deterministic) differs from the user " +
        s"specified value.")
    }
    deterministic
  }
