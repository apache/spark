 ...
 * }}}
 *
 * being reported as a BIT type is quite common, even though BIT in JDBC is meant for single-bit
 * values. Also, there does not appear to be a standard name for an unbounded string or binary
