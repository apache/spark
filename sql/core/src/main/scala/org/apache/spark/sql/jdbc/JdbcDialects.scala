...
/**
 * :: DeveloperApi :: A database type definition coupled with the jdbc type needed to send null
 * values to the database.
 * @param databaseTypeDefinition
...
/**
 * :: DeveloperApi :: Encapsulates everything (extensions, workarounds, quirks) to handle the SQL
 * dialect of a certain database or jdbc driver. Lots of databases define types that aren't
 * explicitly supported by the JDBC spec. Some JDBC drivers also report inaccurate
 * information---for instance, BIT(n{@literal >}1) being reported as a BIT type is quite common,
 * even though BIT in JDBC is meant for single-bit values. Also, there does not appear to be a
 * standard name for an unbounded string or binary type; we use BLOB and CLOB by default but
 * override with database-specific alternatives when these are absent or do not behave correctly.
 *
 * Currently, the only thing done by the dialect is type mapping. `getCatalystType` is used when
 * reading from a JDBC table and `getJDBCType` is used when writing to a JDBC table. If
 * `getCatalystType` returns `null`, the default type handling is used for the given JDBC type.
 * Similarly, if `getJDBCType` returns `(null, None)`, the default type handling is used for the
 * given Catalyst type.
 */
@DeveloperApi
abstract class JdbcDialect extends Serializable with Logging {
   ...
   *
   * Guidelines for mapping database defined timestamps to Spark SQL timestamps: <ul> <li>
   * TIMESTAMP WITHOUT TIME ZONE if preferTimestampNTZ ->
   * [[org.apache.spark.sql.types.TimestampNTZType]] </li> <li> TIMESTAMP WITHOUT TIME ZONE if
   * !preferTimestampNTZ -> [[org.apache.spark.sql.types.TimestampType]](LTZ) </li> <li>TIMESTAMP
   * WITH TIME ZONE -> [[org.apache.spark.sql.types.TimestampType]](LTZ)</li> <li>TIMESTAMP WITH
   * LOCAL TIME ZONE -> [[org.apache.spark.sql.types.TimestampType]](LTZ)</li> <li> If the
   * TIMESTAMP cannot be distinguished by `sqlType` and `typeName`, preferTimestampNTZ is
   * respected for now, but we may need to add another option in the future if necessary. </li>
   * </ul>
   *
   * @param sqlType
   *   Refers to [[java.sql.Types]] constants, or other constants defined by the target database,
   *   e.g. `-101` is Oracle's TIMESTAMP WITH TIME ZONE type. This value is returned by
   *   [[java.sql.ResultSetMetaData#getColumnType]].
   * @param typeName
   *   The column type name used by the database (e.g. "BIGINT UNSIGNED"). This is sometimes used
   *   to determine the target data type when `sqlType` is not sufficient if multiple database
   *   types are conflated into a single id. This value is returned by
   *   [[java.sql.ResultSetMetaData#getColumnTypeName]].
   * @param size
   *   The size of the type, e.g. the maximum precision for numeric types, length for character
   *   string, etc. This value is returned by [[java.sql.ResultSetMetaData#getPrecision]].
   * @param md
   *   Result metadata associated with this type. This contains additional information from
   *   [[java.sql.ResultSetMetaData]] or user specified options. <ul> <li> `isTimestampNTZ`:
   *   Whether read a TIMESTAMP WITHOUT TIME ZONE value as
   *   [[org.apache.spark.sql.types.TimestampNTZType]] or not. This is configured by
   *   `JDBCOptions.preferTimestampNTZ`. </li> <li> `scale`: The length of fractional part
   *   [[java.sql.ResultSetMetaData#getScale]] </li> </ul>
   * @return
   *   An option the actual DataType (subclasses of [[org.apache.spark.sql.types.DataType]]) or
   *   None if the default type mapping should be used.
   */
  def getCatalystType(
      sqlType: Int,
      typeName: String,
   ...
   * @param t
   *   represents a specific instant in time based on the hybrid calendar which combines Julian
   ...
   * value stored in a remote database. JDBC dialects should override this function to provide
   * implementations that suit their JDBC drivers.
   * @param t
  ...
  /**
   * Create the table if the table does not exist. To allow certain options to append when create
   * a new table, which can be table_options or partition_options. E.g., "CREATE TABLE t (name
   ...
   * @param options
   *   The JDBC options. It contains the create table option, which can be table_options or
   *   partition_options.
   */
  def createTable(
      statement: Statement,
   ...
   * conn. Use "?" as placeholder for each value to be inserted. E.g.
   * `INSERT INTO t ("name", "age", "gender") VALUES (?, ?, ?)`
   *
   * @param table
   ...
   */
  @Since("4.0.0")
  def insertIntoTable(table: String, fields: Array[StructField]): String = {
   ...
   * return a query that is suitable for a particular database. For PostgreSQL, for instance, a
   * different query is used to prevent "TRUNCATE" affecting other tables.
   * @param table
   ...
   *   The connection object
   * @param options
   *   The JDBC options for the connection.
   */
  ...
  private[jdbc] class JDBCSQLBuilder extends V2ExpressionSQLBuilder {
    ...
    override def visitAggregateFunction(
        funcName: String,
        isDistinct: Boolean,
        inputs: Array[Expression]): String = {
      if (isSupportedFunction(funcName)) {
    ...
    override def visitOverlay(inputs: Array[String]): String = {
      if (isSupportedFunction("OVERLAY")) {
        ...
        throw new SparkUnsupportedOperationException(
          errorClass = "_LEGACY_ERROR_TEMP_3177",
          messageParameters =
            Map("class" -> this.getClass.getSimpleName, "funcName" -> "OVERLAY"))
      }
    }
    ...
    override def visitTrim(direction: String, inputs: Array[String]): String = {
      if (isSupportedFunction("TRIM")) {
        ...
        throw new SparkUnsupportedOperationException(
          errorClass = "_LEGACY_ERROR_TEMP_3177",
          messageParameters = Map("class" -> this.getClass.getSimpleName, "funcName" -> "TRIM"))
      }
    }
  }
  ...
  def compileAggregate(aggFunction: AggregateFunc): Option[String] = compileExpression(
    aggFunction)
  ...
  /**
   * Return Some[true] iff `TRUNCATE TABLE` causes cascading default. Some[true] : TRUNCATE TABLE
   * causes cascading. Some[false] : TRUNCATE TABLE does not cause cascading. None: The behavior
   * of TRUNCATE TABLE is unknown (default).
   */
   ...
   * @return
   *   `SparkThrowable + Throwable` or its sub-class.
   */
  ...
  def classifyException(message: String, e: Throwable): AnalysisException = {
    new AnalysisException(
      ...
      messageParameters = Map("url" -> "jdbc:", "message" -> message),
      cause = Some(e))
  }
   ...
   * {@link OracleDialect.OracleSQLQueryBuilder} and {@link MySQLDialect.MySQLSQLQueryBuilder}.
   */
 ...
 *
 * If multiple matching dialects are registered then all matching ones will be tried in reverse
 * order. A user-added dialect will thus be applied first, overwriting the defaults.
 *
 * @note
 *   All new dialects are applied to new jdbc DataFrames only. Make sure to register your dialects
 *   first.
 */
@DeveloperApi
object JdbcDialects {
  ...
  /**
   * Register a dialect for use on all new matching jdbc `org.apache.spark.sql.DataFrame`. Reading
   * an existing dialect will cause a move-to-front.
   *
   * @param dialect
   *   The new dialect.
   */
