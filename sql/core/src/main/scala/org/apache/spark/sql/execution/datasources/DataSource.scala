...
import org.apache.spark.internal.LogKey.{CLASS_NAME, DATA_SOURCE_NAMES, DATA_SOURCE_PROVIDER, PATHS}
        ...
        case sources =>
          ...
          if (provider.equalsIgnoreCase("xml") && sources.size == 2) {
            ...
            logWarning(log"Multiple sources found for ${MDC(DATA_SOURCE_PROVIDER, provider1)} " +
              log"(${MDC(DATA_SOURCE_NAMES, sourceNames.mkString(", "))}), defaulting to the " +
