...
case class MergeRowsExec(
  ...
  override lazy val metrics: Map[String, SQLMetric] = Map(
    "numTargetRowsCopied" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows copied unmodified because they did not match any action"),
    "numTargetRowsInserted" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows inserted"),
    "numTargetRowsDeleted" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows deleted"),
    "numTargetRowsUpdated" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows updated"),
    "numTargetRowsMatchedUpdated" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows updated by a matched clause"),
    "numTargetRowsMatchedDeleted" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows deleted by a matched clause"),
    "numTargetRowsNotMatchedBySourceUpdated" -> SQLMetrics.createMetric(
      sparkContext,
      "number of target rows updated by a not matched by source clause"),
    "numTargetRowsNotMatchedBySourceDeleted" -> SQLMetrics.createMetric(
      sparkContext,
  ...
  case class KeepExec(context: Context, condition: BasePredicate, projection: Projection)
   ...
   * validator assumes the target table is never broadcasted or replicated, which guarantees
