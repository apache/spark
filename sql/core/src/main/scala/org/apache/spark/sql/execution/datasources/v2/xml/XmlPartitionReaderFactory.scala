...
/**
 * A factory used to create XML readers.
 *
 * @param sqlConf SQL configuration.
 * @param broadcastedConf Broadcast serializable Hadoop Configuration.
 * @param dataSchema Schema of XML files.
 * @param readDataSchema Required schema of XML files.
 * @param partitionSchema Schema of partitions.
 * @param options Options for parsing XML files.
 * @param filters The filters pushed down to XML datasource.
 */
case class XmlPartitionReaderFactory(
    sqlConf: SQLConf,
  ...
  override def buildReader(partitionedFile: PartitionedFile): PartitionReader[InternalRow] = {
    ...
    val iter = XmlDataSource(options).readFile(
