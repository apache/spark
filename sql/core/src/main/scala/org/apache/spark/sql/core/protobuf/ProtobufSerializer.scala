...
private[sql] class ProtobufSerializer(
  ...
  def serialize(catalystData: scala.Any): scala.Any = {
    converter.apply(catalystData)
  }

  private val converter: scala.Any => scala.Any = {
    val baseConverter =
      try {
        rootCatalystType match {
          case st: StructType =>
            newStructConverter(st, rootDescriptor, Nil, Nil).asInstanceOf[scala.Any => scala.Any]
        }
      } catch {
  ...
  private type Converter = (SpecializedGetters, Int) => scala.Any
    ...
    (catalystType, fieldDescriptor.getJavaType) match {
      ...
      case (ArrayType(et, containsNull), _) =>
        ...
        (getter, ordinal) => {
          ...
          val result = new Array[scala.Any](len)
          var i = 0
          while (i < len) {
