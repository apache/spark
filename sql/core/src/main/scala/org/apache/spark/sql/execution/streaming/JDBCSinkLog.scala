...
class JDBCSinkLog(parameters: Map[String, String]) {
  ...
  val logParameters : Map[String, String] = parameters + (JDBCOptions.JDBC_TABLE_NAME-> logTableName)
  val logOptions = new JDBCOptions(logParameters)
  ...
  private def status(conn: Connection, batchId: Long): Option[String] = {
    val ps: PreparedStatement = conn.prepareStatement(s"Select ${quoteIdentifier(STATUS)} from $logTableName where ${quoteIdentifier(BATCH_ID)}=?" )
    try {
  ...
  private def deleteIncompleteBatch(conn: Connection, batchId: Long): Unit = {
    batchIdCol.foreach(b=> {
      ...
      val ps: PreparedStatement = conn.prepareStatement(s"delete from $tableName where ${quoteIdentifier(b)}=?" )
      try {
        ps.setLong(1, batchId)
        ps.executeUpdate()
      } finally  {
    ...
    val ps: PreparedStatement = conn.prepareStatement(s"delete from $logTableName where ${quoteIdentifier(BATCH_ID)}=?" )
    try {
      ps.setLong(1, batchId)
      ps.executeUpdate()
    } finally  {
  ...
  private def updateStatus(conn: Connection, batchId: Long, newStatus: String): Unit = {
    ...
    status(conn, batchId) match {
      case None => {
        // insert row
        val ps: PreparedStatement = conn.prepareStatement(s"insert into $logTableName (${quoteIdentifier(BATCH_ID)}, ${quoteIdentifier(STATUS)}) values (?,?)" )
        try {
          ps.setLong(1, batchId)
      ...
      case Some(existingStatus) if (existingStatus != newStatus) => {
        // update row
        val ps: PreparedStatement = conn.prepareStatement(s"update $logTableName set ${quoteIdentifier(STATUS)}=? where ${quoteIdentifier(BATCH_ID)}=?" )
        try {
