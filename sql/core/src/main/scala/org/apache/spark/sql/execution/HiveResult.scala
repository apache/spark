...
import org.apache.spark.sql.execution.command.{DescribeTableOrQueryCommand, ExecutedCommandExec, ShowTablesCommand}
...
object HiveResult {
  ...
  def hiveResultString(executedPlan: SparkPlan): Seq[String] = executedPlan match {
    case ExecutedCommandExec(desc: DescribeTableOrQueryCommand) =>
