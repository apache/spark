...
object XSDToSchema extends Logging {
  ...
  /**
   * Reads a schema from an XSD path. Note that if the schema consists of one complex parent type
   * which you want to use as the row tag schema, then you will need to extract the schema of the
   * single resulting struct in the resulting StructType, and use its StructType as your schema.
   *
   * @param xsdPath
   *   XSD path
   * @return
  ...
  /**
   * Reads a schema from an XSD as a string. Note that if the schema consists of one complex
   * parent type which you want to use as the row tag schema, then you will need to extract the
   * schema of the single resulting struct in the resulting StructType, and use its StructType as
   * your schema.
   *
   * @param xsdString
   *   XSD as a string
      ...
      case simpleType: XmlSchemaSimpleType =>
        val schemaType = simpleType.getContent match {
          case restriction: XmlSchemaSimpleTypeRestriction =>
            ...
            val typeName = xmlSchema.getSchemaTypes.asScala
              .get(qName)
              .map { s =>
                s.asInstanceOf[XmlSchemaSimpleType]
                  .getContent
                  .asInstanceOf[XmlSchemaSimpleTypeRestriction]
                  .getBaseTypeName
              }
              .getOrElse(qName)
            ...
            typeName match {
              ...
              case Constants.XSD_DECIMAL =>
                ...
                val fracDigits = facets
                  .collectFirst { case facet: XmlSchemaFractionDigitsFacet =>
                    facet.getValue.toString.toInt
                  }
                  .getOrElse(18)
                val totalDigits = facets
                  .collectFirst { case facet: XmlSchemaTotalDigitsFacet =>
                    facet.getValue.toString.toInt
                  }
                  .getOrElse(38)
                DecimalType(totalDigits, math.min(totalDigits, fracDigits))
              case Constants.XSD_UNSIGNEDLONG | Constants.XSD_INTEGER |
                  Constants.XSD_NEGATIVEINTEGER | Constants.XSD_NONNEGATIVEINTEGER |
                  Constants.XSD_NONPOSITIVEINTEGER | Constants.XSD_POSITIVEINTEGER =>
                DecimalType(38, 0)
              case Constants.XSD_DOUBLE => DoubleType
              case Constants.XSD_FLOAT => FloatType
              case Constants.XSD_BYTE => ByteType
              case Constants.XSD_SHORT | Constants.XSD_UNSIGNEDBYTE => ShortType
              case Constants.XSD_INT | Constants.XSD_UNSIGNEDSHORT => IntegerType
              case Constants.XSD_LONG | Constants.XSD_UNSIGNEDINT => LongType
          ...
          case content: XmlSchemaSimpleContent =>
            ...
            content.getContent match {
              case extension: XmlSchemaSimpleContentExtension =>
                val baseStructField = getStructField(
                  xmlSchema,
  ...
  private def getStructFieldsFromParticle(
      ...
      xmlSchema: XmlSchema): Seq[StructField] = {
    particle match {
      // xs:all
      case all: XmlSchemaAll =>
        all.getItems.asScala.map { case element: XmlSchemaElement =>
