...
object FilePartitionUtil extends Logging {
  ...
  def compute(
      ...
      ignoreMissingFiles: Boolean = false): Iterator[InternalRow] = {
  ...
  def getPreferredLocations(split: FilePartition): Seq[String] = {
    val files = split.files
