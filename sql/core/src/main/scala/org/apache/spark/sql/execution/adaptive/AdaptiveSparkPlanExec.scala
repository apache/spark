...
import org.apache.spark.sql.catalyst.rules.{PlanChangeLogger, Rule, RuleContextBase}
...
case class AdaptiveSparkPlanExec(
  ...
  @transient private var ruleContext = AdaptiveRuleContext(
    isSubquery = isSubquery,
    isFinalStage = false)

  private def withRuleContextConfigs[T](f: => T): T = {
    val configs = ruleContext.configs()
  ...
  private def applyPhysicalRulesWithRuleContext(
    ...
    val newPlan = plan
    withRuleContextConfigs {
      applyPhysicalRules(newPlan, rules, loggerAndBatchName, Some(ruleContext))
    }
  }
  ...
  private def optimizeQueryStage(
      plan: SparkPlan,
      isFinalStage: Boolean): SparkPlan = withRuleContextConfigs {
    ...
    val optimized = rules.foldLeft(plan) { case (latestPlan, rule) =>
      val applied = rule.apply(latestPlan, Some(ruleContext))
  ...
  private def reOptimize(logicalPlan: LogicalPlan): Option[(SparkPlan, LogicalPlan)] = {
    try {
      ...
      val optimized = withRuleContextConfigs { optimizer.execute(logicalPlan, Some(ruleContext)) }
      val sparkPlan = withRuleContextConfigs {
        context.session.sessionState.planner.plan(
          ReturnAnswer(optimized), Some(ruleContext)).next()
      }
      val newPlan = applyPhysicalRulesWithRuleContext(
