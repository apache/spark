...
case class AdaptiveSparkPlanExec(
  ...
  private def updateLogicalPlan(
    ...
    newStages.foreach { case (exhange: Exchange, stage: QueryStageExec) =>
      // Get the corresponding logical node for `exchange`. If `exchange` has been transformed from
      // a `Repartition`, it should have `logicalLink` available by itself; otherwise traverse down
