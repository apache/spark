  ...
  private def updateLogicalPlan(
    ...
    newStages.foreach {
      case stage if currentPhysicalPlan.find(_.eq(stage)).isDefined =>
        ...
        // Fix the temp link in case `stage` is not `physicalNode.get`.
...
object AdaptiveSparkPlanExec {
   ...
   *
   * Temp logical links are set for each query stage after its creation. During re-planning, the
   * query planner will set the usual logical links. When getting logical links, we prioritize the
   * temp logical links over the usual logical links because in case of a conflict, the usual
   * logical link must have come from a new physical plan that was adopted. Once a new physical
   * plan is adopted and becomes the current plan, we clear the temp logical links so that the
   * usual logical links will take effect.
   */
