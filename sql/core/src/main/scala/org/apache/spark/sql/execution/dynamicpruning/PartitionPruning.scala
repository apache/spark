  ...
  def getFilterableTableScan(a: Expression, plan: LogicalPlan): Option[LogicalPlan] = {
    ...
    srcInfo.flatMap {
      ...
      case (resExp, r @ ExtractV2Scan(scan: FileScan)) =>
        ...
        // SupportsRuntimeCatalystFiltering, so neither case above catches it. Partition columns
        // are filterable at runtime via FileScan.planInputPartitionsWithRuntimeFilters, so DPP
        // is eligible on them.
