...
/**
 * Dynamic partition pruning optimization is performed based on the type and selectivity of the
 * join operation. During query optimization, we insert a predicate on the filterable table using
 * the filter from the other side of the join and a custom wrapper called DynamicPruning.
 *
 * The basic mechanism for DPP inserts a duplicated subquery with the filter from the other side,
 * when the following conditions are met: (1) the table to prune is filterable by the JOIN key (2)
 * the join operation is one of the following types: INNER, LEFT SEMI, LEFT OUTER (partitioned on
 * right), or RIGHT OUTER (partitioned on left)
 *
 * In order to enable partition pruning directly in broadcasts, we use a custom DynamicPruning
 * clause that incorporates the In clause with the subquery and the benefit estimation. During
 * query planning, when the join type is known, we use the following mechanism: (1) if the join is
 * a broadcast hash join, we replace the duplicated subquery with the reused results of the
 * broadcast, (2) else if the estimated benefit of partition pruning outweighs the overhead of
 * running the subquery query twice, we keep the duplicated subquery (3) otherwise, we drop the
 * subquery.
 */
object PartitionPruning extends Rule[LogicalPlan] with PredicateHelper with JoinSelectionHelper {
   ...
   *
   * This methods tries to find either a v1 or Hive serde partitioned scan for a given partition
   * column or a v2 scan that support runtime filtering on a given attribute.
   */
  def getFilterableTableScan(a: Expression, plan: LogicalPlan): Option[LogicalPlan] = {
    ...
    srcInfo.flatMap {
      ...
      case (resExp, r @ DataSourceV2ScanRelation(_, scan: SupportsRuntimeV2Filtering, _, _, _)) =>
        val filterAttrs =
          V2ExpressionUtils.resolveRefs[Attribute](scan.filterAttributes.toImmutableArraySeq, r)
   ...
   *   - to be able to identify this filter during query planning, we use a custom DynamicPruning
   *     expression that wraps a regular In expression
   *   - we also insert a flag that indicates if the subquery duplication is worthwhile and it
   *     should run regardless of the join strategy, or is too expensive and it should be run only
   *     if we can reuse the results of a broadcast
   */
  private def insertPredicate(
    ...
    require(
      filteringKeys.size == 1,
      "DPP Filters should only have a single broadcasting key " +
  ...
  /**
   * Given an estimated filtering ratio we assume the partition pruning has benefit if the size in
   * bytes of the partitioned plan after filtering is greater than the size in bytes of the plan
   * on the other side of the join. We estimate the filtering ratio using column statistics if
   * they are available, otherwise we use the config value of
  ...
  private def pruningHasBenefit(
    ...
    val filterRatio = (partExpr.references.toList, otherExpr.references.toList) match {
      ...
      case (leftAttr :: Nil, rightAttr :: Nil) if conf.dynamicPartitionPruningUseStats =>
  ...
  /**
   * Calculates a heuristic overhead of a logical plan. Normally it returns the total size in
   * bytes of all scan relations. We don't count in-memory relation which uses only memory.
   */
  private def calculatePlanOverhead(plan: LogicalPlan): Float = {
    val (cached, notCached) = plan
      .collectLeaves()
      .partition(p =>
        p match {
    ...
    val cachedOverhead = cached
      .map {
        case m: InMemoryRelation
  ...
  /**
   * To be able to prune partitions on a join key, the filtering side needs to meet the following
   * requirements: (1) it can not be a stream (2) it needs to contain a selective predicate used
   * for filtering
   */
    ...
    plan transformUp {
      ...
      case j @ Join(left, right, joinType, Some(condition), hint) =>
        ...
        splitConjunctivePredicates(condition).foreach {
          case EqualTo(a: Expression, b: Expression) if fromDifferentSides(a, b) =>
            val (l, r) =
            ...
            if (filterableScan.isDefined && canPruneLeft(joinType) &&
              ...
              if (filterableScan.isDefined && canPruneRight(joinType) &&
                ...
                newRight =
                  insertPredicate(r, newRight, Seq(l), left, leftKeys, filterableScan.get)
              }
            }
          case _ =>
        }
