...
abstract class StatePartitionReaderBase(
  ...
  protected val keySchema = SchemaUtil.getSchemaAsDataType(schema, "key").asInstanceOf[StructType]
  protected val valueSchema =
    SchemaUtil.getSchemaAsDataType(schema, "value").asInstanceOf[StructType]
  println("I am here inside StatePartitionReaderBase, keySchema: " + keySchema)
  println("I am here inside StatePartitionReaderBase, valueSchema: " + valueSchema)
  ...
  protected lazy val provider: StateStoreProvider = {
    ...
    val numColsPrefixKey = if (stateStoreMetadata.isEmpty) {
      ...
      println("stateStoreMetadata here: " + stateStoreMetadata.head)
      println("stateStoreMetadata properties: " + stateStoreMetadata.head.operatorPropertiesJson)
    ...
    // TODO do we need to support RangeScan(do we need to support reading timer rows)
...
class StatePartitionReader(
    ...
    stateStoreMetadata: Array[StateMetadataTableEntry],
    stateVarName: Option[String] = None)
  extends StatePartitionReaderBase(storeConf, hadoopConf, partition,
    schema, stateStoreMetadata, stateVarName) {
  ...
  override lazy val iter: Iterator[InternalRow] = {
    if (transformWithState) {
      // TODO we can remove create new col family after integration with VCF
