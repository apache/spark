...
/**
 * State Rewriter is used to rewrite the state stores for a stateful streaming query. It reads
 * state from a checkpoint location, optionally applies transformation to the state, and then
 * writes the state back to a (possibly different) checkpoint location for a new batch ID.
 *
 * Example use case is for offline state repartitioning. Can also be used to support other use
 * cases.
 *
 * @param sparkSession
 *   The active Spark session.
 * @param readBatchId
 *   The batch ID for reading state.
 * @param writeBatchId
 *   The batch ID to which the (transformed) state will be written.
 * @param resolvedCheckpointLocation
 *   The resolved checkpoint path where state will be written.
 * @param hadoopConf
 *   Hadoop configuration for file system operations.
 * @param readResolvedCheckpointLocation
 *   Optional separate checkpoint location to read state from. If None, reads from
 *   resolvedCheckpointLocation.
 * @param transformFunc
 *   Optional transformation function applied to each operator's state DataFrame. If None, state
 *   is written as-is.
 * @param writeCheckpointMetadata
 *   Optional checkpoint metadata for the resolvedCheckpointLocation. If None, will create a new
 *   one for resolvedCheckpointLocation. Helps us to reuse already cached checkpoint log entries,
...
class StateRewriter(
    ...
    extends Logging {
  require(
    readResolvedCheckpointLocation.isDefined || readBatchId < writeBatchId,
  ...
  private val stateRootLocation =
    new Path(resolvedCheckpointLocation, StreamingCheckpointConstants.DIR_NAME_STATE).toString
  ...
  def run(): Option[Map[Long, Array[Array[String]]]] = {
    logInfo(
      log"Starting state rewrite for " +
    ...
    logInfo(
      log"State rewrite completed in ${MDC(DURATION, timeTakenMs)} ms for " +
        log"checkpointLocation=${MDC(CHECKPOINT_LOCATION, resolvedCheckpointLocation)}")
    checkpointIds
  }
  ...
  private def extractCheckpointIdsPerPartition(
    ...
    Option(checkpointInfos.map { case (operator, storesSeq) =>
  ...
  private def rewriteStore(
    ...
    require(
      updatedStateDf.schema == stateDf.schema,
    ...
    val schemaProvider =
      createStoreSchemaProviderIfTWS(opMetadata.operatorInfo.operatorName, storeSchemaFiles)
    ...
    updatedStateDf.queryExecution.toRdd
      .mapPartitions { partitionIter: Iterator[InternalRow] =>
  ...
  private def createConfsFromOffsetLog(): (StateStoreConf, SQLConf) = {
    val offsetLog = writeCheckpointMetadata
      .getOrElse(new StreamingQueryCheckpointMetadata(sparkSession, resolvedCheckpointLocation))
      .offsetLog
  ...
  private def getWriterColFamilyInfoMap(
    ...
    getLatestColFamilyToSchemaMap(operatorId, storeMetadata, schemaFiles)
      .map { case (colFamilyName, schema) =>
        colFamilyName -> StatePartitionWriterColumnFamilyInfo(
          schema,
          useMultipleValuesPerKey = twsStateVariables
            .get(colFamilyName)
            .map(_.stateVariableType == StateVariableType.ListState)
            .getOrElse(false))
      }
  }
  ...
  private def getLatestColFamilyToSchemaMap(
    ...
    manager
      .readSchemaFile()
      .map { schema =>
        schema.colFamilyName -> createKeyEncoderSpecIfAbsent(schema, storeMetadata)
      }
      .toMap
  }
    ...
    colFamilySchema.keyStateEncoderSpec match {
      ...
      case None if storeMetadata.isInstanceOf[StateStoreMetadataV1] =>
        ...
        if (storeMetadata.numColsPrefixKey > 0) {
          colFamilySchema.copy(keyStateEncoderSpec = Some(
...
/**
 * Base class for exceptions thrown when the checkpoint location is in an invalid state for state
 * rewriting.
 */
...
private[state] class StateRewriterUnsupportedStoreMetadataVersionError(checkpointLocation: String)
    extends StateRewriterInvalidCheckpointError(
      checkpointLocation,
