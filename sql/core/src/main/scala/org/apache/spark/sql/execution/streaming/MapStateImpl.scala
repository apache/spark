...
import org.apache.spark.sql.execution.streaming.state.{PrefixKeyScanStateEncoderType, StateStore, StateStoreErrors, UnsafeRowPair}
...
class MapStateImpl[K, V](
  ...
  store.createColFamilyIfAbsent(stateName, PrefixKeyScanStateEncoderType, schemaForCompositeKeyRow,
    numColsPrefixKey = 1, schemaForValueRow)
