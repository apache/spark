...
trait MetadataLog[T] {
  ...
  /**
   * Store the metadata for the specified batchId and return `true` if successful. If the
   * batchId's metadata has already been stored, this method will return `false`.
   */
   ...
   * is `None`, just return all batches before endId (inclusive).
   */
  ...
  /**
   * Removes all the log entry earlier than thresholdBatchId (exclusive). This operation should be
   * idempotent.
   */
