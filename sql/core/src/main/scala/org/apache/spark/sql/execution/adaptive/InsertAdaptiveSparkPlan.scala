...
case class InsertAdaptiveSparkPlan(
    adaptiveExecutionContext: AdaptiveExecutionContext) extends RuleWithContext[SparkPlan] {

  override def applyWithContext(plan: SparkPlan, ruleContext: Option[RuleContextBase]): SparkPlan =
    applyInternal(plan, false, ruleContext)

  private def applyInternal(
      plan: SparkPlan,
      isSubquery: Boolean,
      ruleContext: Option[RuleContextBase]): SparkPlan = plan match {
    ...
    case _ if shouldApplyAQE(plan, isSubquery) =>
      if (supportAdaptive(plan)) {
        try {
          ...
          val subqueryMap = buildSubqueryMap(plan, ruleContext)
          val planSubqueriesRule = PlanAdaptiveSubqueries(subqueryMap)
          val preprocessingRules = Seq(
            planSubqueriesRule)
          // Run pre-processing rules.
          val newPlan = AdaptiveSparkPlanExec.applyPhysicalRules(
            plan, preprocessingRules, ruleContext = ruleContext)
   ...
   */
  private def buildSubqueryMap(
      plan: SparkPlan,
      ruleContext: Option[RuleContextBase]): Map[Long, SparkPlan] = {
    ...
    plan.foreach(_.expressions.filter(_.containsPattern(PLAN_EXPRESSION)).foreach(_.foreach {
      case e @ (_: expressions.ScalarSubquery | _: ListQuery | _: DynamicPruningSubquery) =>
        ...
        if (!subqueryMap.contains(subquery.exprId.id)) {
          val executedPlan = compileSubquery(subquery.plan, ruleContext)
  ...
  def compileSubquery(plan: LogicalPlan, ruleContext: Option[RuleContextBase]): SparkPlan = {
