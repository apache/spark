...
case class InMemoryTableScanExec(
  ...
  /**
   * If true, get data from ColumnVector in ColumnarBatch, which are generally faster. If false,
   * get data from UnsafeRow build from CachedBatch
   */
  ...
  private def updateAttribute(expr: Expression): Expression = {
    ...
    expr.transform { case attr: Attribute =>
      attrMap.getOrElse(attr, attr)
    }
  }
  ...
  /**
   * This method is only used by AQE which executes the actually cached RDD that without filter
   * and serialization of row/columnar.
   */
