...
import org.apache.spark.sql.execution.joins.{BroadcastHashJoinExec, BroadcastNearestByJoinExec, BroadcastNestedLoopJoinExec}
...
object ValidateSparkPlan extends Rule[SparkPlan] {
  ...
  private def validate(plan: SparkPlan): Unit = plan match {
    ...
    case b: BroadcastNearestByJoinExec =>
      if (!b.right.isInstanceOf[BroadcastQueryStageExec]) {
        validate(b.right)
      }
      validate(b.left)
