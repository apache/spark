...
// scalastyle:on
  ...
  def hll_sketch_agg(e: Column, lgConfigK: Int, tgtHllType: String): Column =
    ...
    new HllSketchAgg(e.expr, Literal(lgConfigK), Literal(tgtHllType))
  }
  ...
  def hll_sketch_agg(columnName: String, lgConfigK: Int, tgtHllType: String): Column = {
    hll_sketch_agg(Column(columnName), lgConfigK, tgtHllType)
  }

  /**
   * Aggregate function: returns the compact binary representation of the Datasketches
   * HllSketch configured with lgConfigK arg and default tgtHllType value.
   *
   * @group agg_funcs
   * @since 3.5.0
   */
  ...
  /**
   * Aggregate function: returns the compact binary representation of the Datasketches
   * HllSketch configured with default lgConfigK and tgtHllType values.
   *
   * @group agg_funcs
   * @since 3.5.0
   */
   ...
   * @since 3.5.0
   */
  def hll_union_agg(e: Column, lgMaxK: Int): Column = withAggregateFunction {
    new HllUnionAgg(e.expr, lgMaxK)
  }
  ...
  def hll_union_agg(columnName: String, lgMaxK: Int): Column = {
    hll_union_agg(Column(columnName), lgMaxK)
  }

  /**
   * Aggregate function: returns the compact binary representation of the Datasketches
   * HllSketch, generated by merging previously created Datasketches HllSketch instances
   * via a Datasketches Union instance configured with default lgMaxK arg.
   *
   * @group agg_funcs
   * @since 3.5.0
   */
  ...
  def hll_union(c1: Column, c2: Column): Column = withExpr {
    HllUnion(c1.expr, c2.expr)
  }

  /**
   * Merges two binary representations of Datasketches HllSketch objects, using a
   * Dataskethes Union object configured with an lgMaxK equal to the min of the
   * HllSketch object's lgConfigK values.
   *
   * @group misc_funcs
   * @since 3.5.0
