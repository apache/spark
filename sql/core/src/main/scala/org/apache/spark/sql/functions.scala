...
// scalastyle:on
  ...
  private[spark] def udfInternal[A1, RT: TypeTag](f: Function1[A1, RT]): UserDefinedFunction = {
    val ScalaReflection.Schema(dataType, nullable) = ScalaReflection.schemaFor[RT]
    val udf = UserDefinedFunction(f, dataType, None)
    if (nullable) udf else udf.asNonNullable()
  }

  private[spark] def udfInternal[A1, A2, RT: TypeTag](
  ...
  (0 to 10).foreach { x =>
    ...
    val argSchema = (1 to x).foldRight("Nil")((i, s) => {s"ScalaReflection.schemaFor(typeTag[A$i]) :: $s"})
    println(s"""
      |/**
