 ...
 * captured `SQLConf`. Please notice that this means we may miss configurations set after the
 * creation of this RDD and before its execution.
 *
 * @param sqlRDD
 *   the `RDD` generated by the SQL plan
 * @param conf
 *   the `SQLConf` to apply to the execution of the SQL plan
 */
class SQLExecutionRDD(var sqlRDD: RDD[InternalRow], @transient conf: SQLConf)
