...
case class PlanAdaptiveSubqueries(subqueryMap: Map[Long, SparkPlan]) extends Rule[SparkPlan] {
  ...
  def apply(plan: SparkPlan): SparkPlan = {
    plan.transformAllExpressionsWithPruning(
      ...
      case expressions.ScalarSubquery(_, _, exprId, _, _, _, _) =>
        val subquery =
          SubqueryExec.createForScalarSubquery(s"subquery#${exprId.id}", subqueryMap(exprId.id))
      ...
      case expressions.DynamicPruningSubquery(
            value,
            buildPlan,
            buildKeys,
            broadcastKeyIndices,
