...
object ParquetUtils {
  ...
  private[sql] def createAggInternalRowFromFooter(
    ...
    val aggSchemaWithoutGroupby = if (groupByColNums > 0) {
      aggSchema.apply(aggSchema.fieldNames.drop(groupByColNums).toSet)
    } else {
      aggSchema
    }

    val schemaConverter = new ParquetToSparkSchemaConverter
    val converter = new ParquetRowConverter(schemaConverter, parquetSchema, aggSchemaWithoutGroupby,
    ...
    if (groupByColNums > 0) {
      new JoinedRow().apply(partitionColValue, converter.currentRecord)
    } else {
  ...
  private[sql] def createAggColumnarBatchFromFooter(
      ...
      offHeap: Boolean,
      partitionColValue: InternalRow,
      datetimeRebaseMode: LegacyBehaviorPolicy.Value): ColumnarBatch = {
