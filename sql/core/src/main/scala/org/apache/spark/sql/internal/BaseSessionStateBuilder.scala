...
import org.apache.spark.sql.catalyst.analysis.{Analyzer, EvalSubqueriesForTimeTravel, ExecuteExecutableDuringAnalysis, FunctionRegistry, InvokeProcedures, ReplaceCharWithVarchar, ResolveDataSource, ResolveSessionCatalog, ResolveTranspose, TableFunctionRegistry}
  ...
  protected def analyzer: Analyzer = new Analyzer(catalogManager) {
    ...
    override val extendedResolutionRules: Seq[Rule[LogicalPlan]] =
      new ResolveDataSource(session) +:
        ...
        ExecuteExecutableDuringAnalysis(session) +:
