...
/**
 * A collation in Spark, as returned by the `listCollations` method defined in [[Catalog]].
 *
 * @param catalog name of the catalog that the collation belongs to.
 * @param schema the schema that the collation belongs to.
 * @param collationName name of the collation
 * @param language language of the collation
 * @param country country of the collation
 * @param icuVersion icu version of the collation
 * @param padAttribute pad attribute of the collation
 * @param accentSensitivity accent sensitivity of the collation
 * @param caseSensitivity case sensitivity of the collation
 * @since 4.0.0
 */
class CollationMetadata(
    ...
    @Nullable val language: String,
    @Nullable val country: String,
    @Nullable val icuVersion: String,
    val padAttribute: String,
    val accentSensitivity: String,
    val caseSensitivity: String)
  extends DefinedByConstructorParams {
  ...
  override def toString: String =
    "Collation[" +
      ...
      Option(language).map(l => s"language='$l'").getOrElse("") +
      Option(country).map(c => s"country='$c'").getOrElse("") +
      s"accentSensitivity='$accentSensitivity', " +
      s"caseSensitivity='$caseSensitivity'," +
      Option(icuVersion).map(iv => s"icuVersion='$iv'").getOrElse("") +
      s"padAttribute='$padAttribute']"
}
