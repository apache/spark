...
abstract class OutputWriterFactory extends Serializable {
   ...
   * @param dataSchema
   *   Schema of the rows to be written. Partition columns are not included in the schema if the
   *   relation being written is partitioned.
   * @param context
 ...
 * constructor. An [[OutputWriter]] instance is created and initialized when a new output file is
 * opened on executor side. This instance is used to persist rows to this single output file.
 */
