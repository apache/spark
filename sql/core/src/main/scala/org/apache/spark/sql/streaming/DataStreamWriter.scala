...
final class DataStreamWriter[T] private[sql](ds: Dataset[T]) {
  ...
  /**
   * :: Experimental ::
   * Saves the content of the streaming [[DataFrame]] in Parquet format at the specified path.
   ...
   */
  @Experimental
  def parquet(path: String): ContinuousQuery = {
    format("parquet").save(path)
  }
   ...
   */
  @Experimental
  def save(path: String): ContinuousQuery = {
