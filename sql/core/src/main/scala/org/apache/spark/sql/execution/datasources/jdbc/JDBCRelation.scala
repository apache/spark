...
import org.apache.spark.internal.LogKeys.{CLAUSE, LOWER_BOUND, NEW_VALUE, OLD_VALUE, PARTITIONS_SIZE,  UPPER_BOUND}
...
private[sql] object JDBCRelation extends Logging {
  ...
  def columnPartition(
    ...
    logInfo(log"Number of partitions: ${MDC(PARTITIONS_SIZE, numPartitions)}, WHERE clauses of " +
      log"these partitions: " +
      log"${MDC(CLAUSE, partitions.map(_.asInstanceOf[JDBCPartition].whereClause).mkString(", "))}")
    partitions
  }
