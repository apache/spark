 ...
 * Every version checkpoint is saved in specific directory structure that allows successive
 ...
 *   a RocksDB instance at the corresponding version. This is unlike the [version].delta files of
 *   HDFSBackedStateStore where previous delta files needs to be read to be recovered.
...
class RocksDBFileManager(
  ...
  def saveCheckpointToDfs(checkpointDir: File, version: Long, numKeys: Long): Unit = {
    ...
    val rocksDBFiles = saveImmutableFilesToDfs(version, checkpointDir, localImmutableFiles)
  ...
  private def rememberImmutableFiles(
    version: Long, rocksDBFiles: Seq[RocksDBImmutableFile]): Unit = {
    versionToRocksDBFiles.put(version, rocksDBFiles)
  }
   ...
   */
  private def listRocksDBFiles(localDir: File): Seq[File] = {
    ...
    topLevelFiles ++ archivedLogFiles
  }
}
