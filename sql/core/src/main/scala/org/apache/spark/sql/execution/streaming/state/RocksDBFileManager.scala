 ...
 * version, checkpoint is saved in specific directory structure that allows successive versions to
 * reuse to SST data files and archived log files. This allows each commit to be incremental, only
 * new SST files and archived log files generated by RocksDB will be uploaded. The directory
 * structures on local disk and in DFS are as follows.
 *
 * Local checkpoint dir structure ------------------------------ RocksDB generates a bunch of
 * files in the local checkpoint directory. The most important among them are the SST files; they
 * are the actual log structured data files. Rest of the files contain the metadata necessary for
 * RocksDB to read the SST files and start from the checkpoint. Note that the SST files are hard
 * links to files in the RocksDB's working directory, and therefore successive checkpoints can
 * share some of the SST files. So these SST files have to be copied to DFS in shared directory
 * such that different committed versions can save them.
 *
 * We consider both SST files and archived log files as immutable files which can be shared
 * between different checkpoints.
 *
 * localCheckpointDir \| +-- OPTIONS-000005 +-- MANIFEST-000008 +-- CURRENT +-- 00007.sst +--
 * 00011.sst +-- archive \| +-- 00008.log \| +-- 00013.log ...
 *
 * DFS directory structure after saving to DFS as version 10
 * ----------------------------------------------------------- The SST and archived log files are
 * given unique file names and copied to the shared subdirectory. Every version maintains a
 * mapping of local immutable file name to the unique file name in DFS. This mapping is saved in a
 * JSON file (named `metadata`), which is zipped along with other checkpoint files into a single
 * file `[version].zip`.
 *
 * dfsRootDir \| +-- SSTs \| +-- 00007-[uuid1].sst \| +-- 00011-[uuid2].sst +-- logs \| +--
 * 00008-[uuid3].log \| +-- 00013-[uuid4].log +-- 10.zip \| +-- metadata <--- contains mapping
 * between 00007.sst and [uuid1].sst, and the mapping between 00008.log and [uuid3].log \| +--
 * OPTIONS-000005 \| +-- MANIFEST-000008 \| +-- CURRENT \| ... \| +-- 9.zip +-- 8.zip ...
 *
 * Note the following.
 *   - Each [version].zip is a complete description of all the data and metadata needed to recover
 *     a RocksDB instance at the corresponding version. The SST files and log files are not
 *     included in the zip files, they can be shared cross different versions. This is unlike the
 ...
 *     as each task would upload a different copy of the generated immutable files and atomically
 *     update the [version].zip.
...
class RocksDBFileManager(
  ...
  def deepCopy(): RocksDBFileManager = {
    val newFileManager =
      new RocksDBFileManager(dfsRootDir, localTempDir, hadoopConf, codecName, loggingId)
  ...
  /**
   * Delete old versions by deleting the associated version and SST files. At a high-level, this
   * method finds which versions to delete, and which SST files that were last used in those
   * versions. It's safe to delete these SST files because a SST file can be reused only in
   * successive versions. Therefore, if a SST file F was last used in version V, then it won't be
   * used in version V+1 or later, and if version V can be deleted, then F can safely be deleted
   * as well.
   *
  ...
  private def saveImmutableFilesToDfs(
    ...
    val immutableFiles = localFiles.map { localFile =>
      ...
      if (existingDfsFile.isDefined && existingDfsFile.get.sizeBytes == localFile.length()) {
        ...
        fs.copyFromLocalFile(new Path(localFile.getAbsoluteFile.toURI), dfsFile)
  ...
  private def removeLocallyDeletedSSTFilesFromDfsMapping(localFiles: Seq[File]): Unit = {
    ...
    val mappingsToClean = localFilesToDfsFiles.asScala.keys
