...
class RocksDBFileManager(
  ...
  def loadCheckpointFromDfs(
    ...
    val metadata = if (version == 0) {
      ...
      // TODO: We are using fs here to read the file, checksum verification wouldn't happen
      //  for the file if enabled, since it is not using fm to open it.
  ...
  def deleteOldVersions(numVersionsToRetain: Int, minVersionsToDelete: Long = 0): Unit = {
    ...
    if (checksumFiles.nonEmpty) {
      // Set of the zip and changelog files that were deleted
      val deletedStoreFiles = snapshotVersionsAndUniqueIdsToDelete
        .map{ case (v, id) => dfsBatchZipFile(v, id) } ++
        changelogVersionsAndUniqueIdsToDelete
          .map{ case (v, id) => dfsChangelogFile(v, id) }
