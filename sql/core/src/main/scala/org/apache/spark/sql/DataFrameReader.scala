...
class DataFrameReader private[sql](sparkSession: SparkSession) extends Logging {
  ...
  def schema(schema: StructType): DataFrameReader = {
    ...
    extraOptions += (DataSourceUtils.USER_SPECIFIED_SCHEMA_KEY -> schema.json)
    this
  }
  ...
  def load(paths: String*): DataFrame = {
    ...
    val tableOptional = if (!shouldUseV1Source) {
      if (classOf[PathCatalog].isAssignableFrom(cls)) {
        loadTableFromPathCatalog(cls, paths.toSeq, pathsOption)
        ...
        val table = userSpecifiedSchema match {
          case Some(schema) => provider.getTable(finalOptions, schema)
          case _ => provider.getTable(finalOptions)
        }
        Some(table)
      } else {
        None
  ...
  private def loadTableFromPathCatalog(
    ...
    val ident = catalog.getTableIdentifier(
      paths.toArray ++ extraOptions.get("path").map(Array(_)).getOrElse(Array.empty))
