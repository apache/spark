...
import java.io.{InputStream, InputStreamReader, IOException, OutputStream}
...
abstract class CompactibleFileStreamLog[T <: AnyRef : ClassTag](
  ...
  override def deserialize(in: InputStream): Array[T] = {
    ...
    if (content(0) == 'v') {
      ...
      if (indexOfNewLine > 0) {
        val version = parseVersion(content.substring(0, indexOfNewLine), metadataLogVersion)
        Array(Serialization.read[T](content.substring(indexOfNewLine + 1)))
      } else {
        throw new IllegalStateException(
          s"Incomplete log file.")
      }
    } else {
      // The log was generated by Spark 2.1.0
      Array(Serialization.read[T](content))
    }
  }
