...
/**
 * Object that contains metadata about the external database. The metadata is static database
 * information such as the version, or the version of the JDBC driver.
 *
...
object JDBCDatabaseMetadata extends Logging {
   ...
   *
   * @param f
   *   A function that retrieves an integer value from DatabaseMetaData.
   * @return
  ...
  /**
   * Creates a DatabaseMetadata instance from a JDBC Connection, handling errors for each field
   * individually.
   *
   * @param getConnection
   *   A JDBC connection factory.
   * @return
   *   A new instance of DatabaseMetadata containing the version metadata.
   */
