...
private[v2] trait V2ViewPreparation extends LeafV2CommandExec {
  ...
  protected def buildTableInfo(): TableInfo = {
    ...
    // Reject permanent views referencing temporary objects. Temp-function references are not
    // detected here because v2 `CreateView` does not extend AnalysisOnlyCommand and therefore
    // does not capture `referredTempFunctionNames` from AnalysisContext; temp views and temp
    // variables are still caught via plan inspection. TODO(SPARK-...): capture temp functions.
    verifyTemporaryObjectsNotExists(
      isTemporary = false, legacyName, query, referredTempFunctions = Nil)
