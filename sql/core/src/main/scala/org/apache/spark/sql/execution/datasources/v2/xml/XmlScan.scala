...
case class XmlScan(
  ...
  private val xmlOptions = new XmlOptions(
  ...
  override def isSplitable(path: Path): Boolean = {
    XmlDataSource(xmlOptions).isSplitable && super.isSplitable(path)
  }
  ...
  override def getFileUnSplittableReason(path: Path): String = {
    ...
    if (!super.isSplitable(path)) {
      ...
      "the xml datasource is set multiLine mode"
    }
  }

  override def createReaderFactory(): PartitionReaderFactory = {
    // Check a field requirement for corrupt records here to throw an exception in a driver side
    ExprUtils.verifyColumnNameOfCorruptRecord(dataSchema, xmlOptions.columnNameOfCorruptRecord)

    if (readDataSchema.length == 1 &&
      readDataSchema.head.name == xmlOptions.columnNameOfCorruptRecord) {
    ...
    XmlPartitionReaderFactory(sparkSession.sessionState.conf, broadcastedConf,
      dataSchema, readDataSchema, readPartitionSchema, xmlOptions,
      pushedFilters.toImmutableArraySeq)
  }

  override def equals(obj: Any): Boolean = obj match {
    case j: XmlScan => super.equals(j) && dataSchema == j.dataSchema && options == j.options &&
