...
import org.apache.spark.sql.catalyst.plans.logical.{DefaultValueExpression, DropDefaultColumnValue, LogicalPlan}
...
case class AlterTableChangeColumnCommand(
    ...
    newDefaultValue: Option[Expression] = None) extends LeafRunnableCommand {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val newDataSchema = table.dataSchema.fields.map { field =>
      if (field.name == originColumn.name) {
        ...
        if (newDefaultValue.isDefined) {
          newDefaultValue.get match {
            case DropDefaultColumnValue => withNewComment.clearCurrentDefaultValue()
            case d: DefaultValueExpression =>
              withNewComment.withCurrentDefaultValue(d.originalSQL)
            case other => throw SparkException.internalError(
              "Unexpected expression in AlterTableChangeColumnCommand.defaultExpression: " + other)
          }
        } else {
          withNewComment
        }
