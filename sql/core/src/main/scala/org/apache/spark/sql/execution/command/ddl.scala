...
case class DescribeDatabaseCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (extended) {
      val properties = allDbProperties -- NAMESPACE_RESERVED_PROPERTIES
      val propertiesStr =
...
case class AlterTableSetPropertiesCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val newTable = table.copy(
      ...
      comment = properties.get(PROP_COMMENT).orElse(table.comment))
...
case class AlterTableUnsetPropertiesCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (!ifExists) {
      propKeys.foreach { k =>
        if (!table.properties.contains(k) && k != PROP_COMMENT) {
          throw new AnalysisException(
    ...
    val tableComment = if (propKeys.contains(PROP_COMMENT)) None else table.comment
