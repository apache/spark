...
import org.apache.spark.sql.execution.datasources.csv.UnivocityParser._
...
private[csv] object CSVInferSchema {
  ...
  def infer(
    ...
    import csv.sqlContext.implicits._

    val filtered = csv.mapPartitions(filterCommentAndEmpty(_, options))
    val firstLine: String = filtered.first()
    ...
    val fields = if (options.inferSchemaFlag) {
      val tokenRdd = filtered.mapPartitions { iter =>
        val parser = new CsvParser(options.asParserSettings)
        if (options.headerFlag) {
          iter.filterNot(_ == firstLine).map(parser.parseLine)
        } else {
          iter.map(parser.parseLine)
        }
      }.rdd
