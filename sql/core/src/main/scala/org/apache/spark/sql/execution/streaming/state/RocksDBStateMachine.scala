 ...
 *
 * This class enforces proper state transitions and ensures thread-safety for accessing RocksDB
 * instances. It prevents concurrent modifications to the same native RocksDB instance by using a
 * stamp-based locking mechanism.
 *
 * State Lifecycle:
 ...
 *
 * Stamps: Each time a RocksDB instance is acquired, a unique stamp is generated. This stamp must
 * be presented when performing operations on the RocksDB instance and when releasing it. This
 * ensures that only the stamp owner that acquired the RocksDB instance can release it or perform
 * operations.
 */
class RocksDBStateMachine(stateStoreId: StateStoreId, rocksDBConf: RocksDBConf) extends Logging {
  ...
  /**
   * Map defining all valid state transitions in the state machine. Key: (currentState, operation)
   * -> Value: nextState
   *
   * Valid transitions:
  ...
  /**
   * Returns information about the thread that currently has the RocksDB instance acquired. This
   * method is exposed for testing purposes only.
   *
   * @return
   *   Some(AcquiredThreadInfo) if a thread currently has the RocksDB instance acquired, None if
   *   the RocksDB instance is in RELEASED state
   */
  ...
  /**
   * This method is marked "WithLock" because it MUST only be called when the caller already holds
   * the stateMachineLock. Calling this method without holding the lock will result in race
   * conditions and data corruption.
   *
   * @return
   *   A new unique stamp value
   */
  @GuardedBy("stateMachineLock")
  private def incAndGetStampWithLock: Long = {
    ...
    logInfo(
      log"New stamp: ${MDC(LogKeys.STAMP, currentValidStamp.get())} issued for " +
  ...
  /**
   * This method is marked "WithLock" because it MUST only be called when the caller already holds
   * the stateMachineLock. The method uses stateMachineLock.wait() which requires the calling
   * stamp owner to own the monitor. Calling this without holding the lock will throw
   * IllegalMonitorStateException.
   *
   * @param operation
   *   The operation being attempted (used for error reporting)
   * @throws QueryExecutionErrors.unreleasedThreadError
   *   if timeout occurs
   */
  @GuardedBy("stateMachineLock")
  private def awaitNotAcquiredWithLock(operation: OPERATION): Unit = {
    ...
    if (state == ACQUIRED) {
      ...
      throw QueryExecutionErrors.unreleasedThreadError(
        loggingId,
        operation.toString,
        newAcquiredThreadInfo.toString(),
        acquiredThreadInfo.toString(),
        timeWaitedMs,
        stackTraceOutput)
    }
  }
  ...
  /**
   * Return the task ID from the active TaskContext if exists for logging purposes. Returns
   * "undefined" if no TaskContext is active.
   */
   ...
   * instances. It uses a map-based approach to define valid state transitions, making the state
   * machine logic cleaner and more maintainable.
   *
   ...
   *
   * @param operation
   *   The requested state operation (LOAD, RELEASE, CLOSE, or MAINTENANCE)
   * @return
   *   A tuple of (oldState, newState) representing the state before and after operation
   * @throws StateStoreInvalidStateMachineTransition
   *   if the requested operation is not allowed from the current state
   */
  @GuardedBy("stateMachineLock")
  private def validateAndTransitionState(operation: OPERATION): (STATE, STATE) = {
    ...
    logInfo(
      log"Transitioned state from ${MDC(LogKeys.STATE_STORE_STATE, oldState)} " +
  ...
  /**
   * Verifies that the provided stamp matches the current valid stamp. This ensures that
   * operations are performed by the task that acquired the RocksDB instance.
   *
   * @param stamp
   *   The stamp to verify against the current valid stamp
   * @throws StateStoreInvalidStamp
   *   if the stamp does not match the current valid stamp
   */
  def verifyStamp(stamp: Long): Unit = {
    val currentStamp = currentValidStamp.get()
    if (stamp != currentStamp) {
      logWarning(
        log"Invalid stamp=${MDC(LogKeys.STAMP, stamp)} " +
  ...
  /**
   * Releases the RocksDB instance, transitioning it from ACQUIRED to RELEASED state. This can
   * only be called by the stamp owner that acquired the RocksDB instance.
   *
   * @param stamp
   *   The stamp that was returned when the RocksDB instance was acquired
   * @param throwEx
   *   Whether to throw an exception if the stamp is invalid (default: true)
   * @return
   *   true if the RocksDB instance was successfully released, false if stamp was invalid and
   *   throwEx=false
   * @throws StateStoreInvalidStamp
   *   if stamp is invalid and throwEx=true
   * @throws StateStoreInvalidStateMachineTransition
   *   if the current state doesn't allow release
   */
  def releaseStamp(stamp: Long, throwEx: Boolean = true): Boolean =
    stateMachineLock.synchronized {
  ...
  /**
   * Acquires the RocksDB instance for exclusive use by the calling task. Transitions the state
   * from RELEASED to ACQUIRED.
   *
   ...
   *
   * @return
   *   A unique stamp that must be used for subsequent operations and release
   * @throws StateStoreInvalidStateMachineTransition
   *   if the RocksDB instance is in CLOSED state
   * @throws QueryExecutionErrors.unreleasedThreadError
   *   if timeout occurs waiting for another thread
   */
  ...
  /**
   * This verifies that it is in a state that allows maintenance to be performed. This operation
   * is allowed in both RELEASED and ACQUIRED states.
   *
   * @throws StateStoreInvalidStateMachineTransition
   *   if the RocksDB instance is in CLOSED state
   */
  ...
  /**
   * Closes the RocksDB instance permanently, transitioning it to CLOSED state. Once closed, the
   * RocksDB instance cannot be used again and all future operations will fail.
   *
   * This method will block if another task currently has a stamp for the RocksDB instance,
   * waiting up to the configured timeout before throwing an exception.
   *
   * @throws StateStoreInvalidStateMachineTransition
   *   if called multiple times (idempotent)
   * @throws QueryExecutionErrors.unreleasedThreadError
   *   if timeout occurs waiting for another thread
   */
  def close(): Boolean = stateMachineLock.synchronized {
    ...
    if (state == CLOSED) {
      ...
      logInfo(
        log"Finished waiting to acquire lock," +
