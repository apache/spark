  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (viewType == LocalTempView) {
      ...
      val tableDefinition = createTemporaryViewRelation(
        ...
        collation)
      catalog.createTempView(name.table, tableDefinition,
        ignoreIfExists = allowExisting, overrideIfExists = replace)
    } else if (viewType == GlobalTempView) {
      val db = getSqlConf(sparkSession).getConf(StaticSQLConf.GLOBAL_TEMP_DATABASE)
      ...
      val tableDefinition = createTemporaryViewRelation(
        ...
        aliasedPlan,
        referredTempFunctions)
      catalog.createGlobalTempView(name.table, tableDefinition,
        ignoreIfExists = allowExisting, overrideIfExists = replace)
...
case class AlterViewAsCommand(
    ...
    isAnalyzed: Boolean = false,
    referredTempFunctions: Seq[String] = Seq.empty)
  extends RunnableCommand with AnalysisOnlyCommand with CTEInChildren with SessionStateHelper {
  ...
  override def run(session: SparkSession): Seq[Row] = {
    val isTemporary = sessionState(session).catalog.isTempView(name)
  ...
  private def alterTemporaryView(session: SparkSession, analyzedPlan: LogicalPlan): Unit = {
    val catalog = sessionState(session).catalog
    ...
    val tableDefinition = createTemporaryViewRelation(
      ...
      referredTempFunctions)
    sessionState(session).catalog.alterTempViewDefinition(name, tableDefinition)
  }

  private def alterPermanentView(session: SparkSession, analyzedPlan: LogicalPlan): Unit = {
    val viewMeta = sessionState(session).catalog.getTableMetadata(name)
    ...
    sessionState(session).catalog.alterTable(updatedViewMeta)
  }
...
case class AlterViewSchemaBindingCommand(name: TableIdentifier, viewSchemaMode: ViewSchemaMode)
  extends LeafRunnableCommand with SessionStateHelper {
...
case class ShowViewsCommand(
    ...
    override val output: Seq[Attribute]) extends LeafRunnableCommand with SessionStateHelper {
...
object ViewHelper extends SQLConfHelper with Logging with CapturesConfig with SessionStateHelper {
  ...
  def generateViewProperties(
    ...
    val manager = sessionState(session).catalogManager
  ...
  def createTemporaryViewRelation(
    ...
    val uncache = rawTempView.exists(needsToUncache(_, aliasedPlan))
    val storeAnalyzedPlanForView = getSqlConf(session).storeAnalyzedPlanForView ||
  ...
  def prepareTable(
    ...
    val aliasedSchema = CharVarcharUtils.getRawSchema(
      aliasPlan(session, analyzedPlan, userSpecifiedColumns).schema, getSqlConf(session))
    val newProperties = generateViewProperties(
  ...
  def aliasPlan(
    ...
    if (userSpecifiedColumns.isEmpty) {
      ...
      sessionState(session).executePlan(Project(projectList, analyzedPlan)).analyzed
    }
  }
}
