...
case class CreateViewCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    verifyTemporaryObjectsNotExists(sparkSession, isTemporary, name, analyzedPlan)
...
case class AlterViewAsCommand(
  ...
  override def run(session: SparkSession): Seq[Row] = {
    ...
    verifyTemporaryObjectsNotExists(session, isTemporary, name, query)
...
object ViewHelper extends SQLConfHelper with Logging {
  ...
  def verifyTemporaryObjectsNotExists(
    ...
    if (!isTemporary) {
      val (tempViews, tempFunctions) = collectTemporaryObjects(spark, child)
  ...
  private def collectTemporaryObjects(
      spark: SparkSession, child: LogicalPlan): (Seq[Seq[String]], Seq[String]) = {
    ...
    def collectTempFunctions(): Seq[String] = {
      // All the referred temporary functions will be added to `AnalysisContext` when
      // it's initialized by `lookupFunction`.
      spark.sessionState.analyzer.getAnalysisContext.referredTempFunctionNames.toSeq
    }
    (collectTempViews(child), collectTempFunctions())
  }
  ...
  private def prepareTemporaryView(
    ...
    val catalog = session.sessionState.catalog
    val (tempViews, tempFunctions) = collectTemporaryObjects(session, analyzedPlan)
