...
case class CreateViewCommand(
    ...
    referredTempVariables: Seq[String] = Seq.empty)
  ...
  def markAsAnalyzed(analysisContext: AnalysisContext): LogicalPlan = {
    copy(
      ...
      referredTempVariables = analysisContext.referredTempVariableNames.toSeq)
  }
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    verifyTemporaryObjectsNotExists(isTemporary, name, analyzedPlan,
      referredTempFunctions, referredTempVariables)
...
case class AlterViewAsCommand(
  ...
  override def run(session: SparkSession): Seq[Row] = {
    ...
    verifyTemporaryObjectsNotExists(isTemporary, name, query,
      referredTempFunctions, referredTempVariables)
...
object ViewHelper extends SQLConfHelper with Logging {
  ...
  private def referredTempNamesToProps(
      ...
      variablesNames: Seq[String]): Map[String, String] = {
    val viewNamesJson =
    ...
    val variablesNamesJson = JArray(variablesNames.map(JString).toList)
  ...
  def generateViewProperties(
      ...
      tempVariableNames: Seq[String] = Seq.empty): Map[String, String] = {
  ...
  def verifyTemporaryObjectsNotExists(
      ...
      referredTempVariables: Seq[String]): Unit = {
    ...
    if (!isTemporary) {
      ...
      referredTempVariables.foreach { varName =>
  ...
  def createTemporaryViewRelation(
      ...
      referredTempFunctions: Seq[String],
      referredTempVariables: Seq[String]): TemporaryViewRelation = {
  ...
  private def prepareTemporaryView(
      ...
      tempVariables: Seq[String]): CatalogTable = {
