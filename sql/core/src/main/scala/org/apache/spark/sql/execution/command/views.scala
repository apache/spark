...
import org.apache.spark.sql.catalyst.analysis.{AnalysisContext, GlobalTempView, LocalTempView, ResolvedIdentifier, SchemaEvolution, SchemaUnsupported, ViewSchemaMode, ViewType}
...
import org.apache.spark.sql.catalyst.plans.logical.{AnalysisOnlyCommand, CreateTempView, CreateView, CTEInChildren, CTERelationDef, LogicalPlan, Project, View, WithCTE}
...
/**
 * Post-analysis check for v2 CREATE VIEW: rejects permanent views that reference temporary
 * objects and rejects view bodies with auto-generated aliases. `referredTempFunctions` is
 * captured by [[CreateView.markAsAnalyzed]] before this rule runs. The v1 counterpart
 * [[CreateViewCommand]] keeps its existing exec-time checks — Dataset-built commands bypass
 * the analyzer's re-capture path, so the exec-time safety net must stay for v1.
 */
object CheckViewReferences extends (LogicalPlan => Unit) {
  ...
  override def apply(plan: LogicalPlan): Unit = plan.foreach {
    case cv: CreateView if cv.isAnalyzed =>
      val ident = cv.child.asInstanceOf[ResolvedIdentifier]
      val legacyName = TableIdentifier(
        table = ident.identifier.name(),
        database = ident.identifier.namespace().lastOption,
        catalog = Some(ident.catalog.name()))
      verifyTemporaryObjectsNotExists(
        isTemporary = false, legacyName, cv.query, cv.referredTempFunctions)
      verifyAutoGeneratedAliasesNotExists(cv.query, isTemporary = false, legacyName)
