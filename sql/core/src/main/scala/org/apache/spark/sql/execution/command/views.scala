...
case class CreateViewCommand(
  ...
  private def sessionState(sparkSession: SparkSession): SessionState = sparkSession.sessionState
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val state: SessionState = sessionState(sparkSession)
    ...
    if (viewType == LocalTempView) {
      ...
      val db = state.conf.getConf(StaticSQLConf.GLOBAL_TEMP_DATABASE)
  ...
  private def prepareTable(session: SparkSession, analyzedPlan: LogicalPlan): CatalogTable = {
    ...
    val aliasedSchema = CharVarcharUtils.getRawSchema(
      aliasPlan(session, analyzedPlan).schema, sessionState(session).conf)
...
case class AlterViewAsCommand(
  ...
  override def run(session: SparkSession): Seq[Row] = {
    ...
    val isTemporary = sessionState.catalog.isTempView(name)
  ...
  private def alterTemporaryView(session: SparkSession, analyzedPlan: LogicalPlan): Unit = {
    ...
    val tableDefinition = createTemporaryViewRelation(
      ...
      referredTempFunctions)
    sessionState.catalog.alterTempViewDefinition(name, tableDefinition)
  }

  private def alterPermanentView(session: SparkSession, analyzedPlan: LogicalPlan): Unit = {
    val sessionState: SessionState = session.sessionState

    val viewMeta = sessionState.catalog.getTableMetadata(name)
    ...
    sessionState.catalog.alterTable(updatedViewMeta)
  }
...
object ViewHelper extends SQLConfHelper with Logging {
  ...
  def createTemporaryViewRelation(
    ...
    val sessionState: SessionState = session.sessionState
    val storeAnalyzedPlanForView = sessionState.conf.storeAnalyzedPlanForView ||
