...
/**
 * An example runner for multinomial logistic regression.
 *
 * Run with
 * {{{
 * bin/run-example ml.MultinomialLogisticRegressionExample [options]
 * }}}
 * A synthetic dataset can be found at `data/mllib/sample_libsvm_data.txt` which can be
 * trained by
 * {{{
 * bin/run-example ml.MultinomialLogisticRegressionExample --regParam 0.3 --elasticNetParam 0.8 \
 *   data/mllib/sample_libsvm_data.txt
 * }}}
 * If you use it as a template to create your own app, please use `spark-submit` to submit your app.
 */
// scalastyle:off println
object MultinomialLogisticRegressionExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val params = parser.parse(args, defaultParams).getOrElse(sys.exit(1))

    val spark = SparkSession
      .builder()
      .appName(s"MultinomialLogisticRegressionExample")
      .getOrCreate()

    println(s"MultinomialLogisticRegressionExample with parameters:\n$params")

    val (training, test) = DecisionTreeExample.loadDatasets(params.input,
    ...
    val pipeline = new Pipeline().setStages(Array(labelIndexer, multiLogReg))
    ...
    DecisionTreeExample.evaluateClassificationModel(model, training, "indexedLabel")
    println("Test data results:")
    DecisionTreeExample.evaluateClassificationModel(model, test, "indexedLabel")
