...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("StructuredSessionization").getOrCreate()
    ...
    events = lines.select(
        explode(split(lines.value, " ")).alias("sessionId"), lines.timestamp.alias("eventTime")
    )

    # Group the data by window and word and compute the count of each group
    windowedCounts = (
        events.groupBy(
            session_window(events.eventTime, "10 seconds").alias("session"), events.sessionId
        )
