...
import org.apache.spark.streaming.akka.JavaActorHelper;
...
public class JavaActorWordCount {
  ...
  public static void main(String[] args) {
    ...
    ActorSystemFactory actorSystemFactory = new ActorSystemFactory() {
      @Override
      public ActorSystem create() {
        return JavaGlobalActorSystem.getActorSystem();
      }
    };

    String feederActorURI = "akka.tcp://test@" + host + ":" + port + "/user/FeederActor";

    JavaDStream<String> lines1 = AkkaUtils.<String>createStream(
        jssc,
        actorSystemFactory,
        Props.create(JavaSampleActorReceiver.class, feederActorURI), "SampleReceiver1");
    JavaDStream<String> lines2 = AkkaUtils.<String>createStream(
        jssc,
        actorSystemFactory,
        Props.create(JavaSampleActorReceiver.class, feederActorURI), "SampleReceiver2");

    JavaDStream<String> lines = lines1.union(lines2);
...
class JavaGlobalActorSystem {
  ...
  public synchronized static ActorSystem getActorSystem() {
    if (actorSystem == null) {
      ...
      actorSystem = ActorSystem.create("test", ConfigFactory.parseMap(akkaConf));
    }
    return actorSystem;
  }
}
