...
object BuildCommons {
  ...
  val allProjects@Seq(bagel, catalyst, core, graphx, hive, hiveThriftServer, mllib, repl,
    ...
    streamingMqtt, streamingTwitter, streamingZeromq, launcher, managedMemory) =
    Seq("bagel", "catalyst", "core", "graphx", "hive", "hive-thriftserver", "mllib", "repl",
      "sql", "network-common", "network-shuffle", "streaming", "streaming-flume-sink",
      "streaming-flume", "streaming-kafka", "streaming-mqtt", "streaming-twitter",
      "streaming-zeromq", "launcher", "managed-memory").map(ProjectRef(buildLocation, _))
...
object SparkBuild extends PomBuild {
  ...
  allProjects.filterNot(x => Seq(spark, hive, hiveThriftServer, catalyst, repl,
    networkCommon, networkShuffle, networkYarn, launcher, managedMemory).contains(x)).foreach {
      x => enable(MimaBuild.mimaSettings(sparkHome, x))(x)
    }

  /* ManagedMemory settings */
  enable(ManagedMemory.settings)(managedMemory)
...
object Unidoc {
  ...
  private def ignoreUndocumentedPackages(packages: Seq[Seq[File]]): Seq[Seq[File]] = {
    packages
      ...
      .map(_.filterNot(_.getCanonicalPath.contains("org/apache/spark/managedmemory")))
...
object TestSettings {
  ...
  lazy val settings = Seq (
    ...
    javaOptions in Test += "-Dspark.managedMemory.exceptionOnMemoryLeak=true",
