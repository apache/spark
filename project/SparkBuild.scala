...
object BuildCommons {
  ...
  val sqlProjects@Seq(catalyst, sql, hive, hiveThriftServer, hiveThriftServerV2, tokenProviderKafka010, sqlKafka010, avro) = Seq(
    "catalyst", "sql", "hive", "hive-thriftserver", "hive-thriftserverV2", "token-provider-kafka-0-10", "sql-kafka-0-10", "avro"
...
object SparkBuild extends PomBuild {
  ...
  val mimaProjects = allProjects.filterNot { x =>
    Seq(
      spark, hive, hiveThriftServer, hiveThriftServerV2, catalyst, repl, networkCommon, networkShuffle, networkYarn,
