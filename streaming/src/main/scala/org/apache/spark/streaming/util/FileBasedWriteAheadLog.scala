...
import org.apache.spark.internal.LogKeys.{NUM_FILES, NUM_RETRY, PATH, PATHS, THRESH_TIME, WRITE_AHEAD_LOG_INFO}
...
private[streaming] class FileBasedWriteAheadLog(
  ...
  def readAll(): JIterator[ByteBuffer] = synchronized {
    ...
    logInfo(log"Reading from the logs:\n${MDC(PATHS, logFilesToRead.mkString("\n"))}")
  ...
  def clean(threshTime: Long, waitForCompletion: Boolean): Unit = {
    ...
    logInfo(log"Attempting to clear ${MDC(NUM_FILES, oldLogFiles.size)} old log files in " +
      log"${MDC(PATH, logDirectory)} older than ${MDC(THRESH_TIME, threshTime)}: " +
      log"${MDC(PATHS, oldLogFiles.map { _.path }.mkString("\n"))}")
    ...
    def deleteFile(walInfo: LogInfo): Unit = {
      ...
      logInfo(log"Cleared log files in ${MDC(PATH, logDirectory)} older than " +
        log"${MDC(THRESH_TIME, threshTime)}")
    }
  ...
  private def initializeOrRecover(): Unit = synchronized {
    ...
    try {
      ...
      if (fileSystem.getFileStatus(logDirectoryPath).isDirectory) {
        ...
        logInfo(log"Recovered ${MDC(NUM_FILES, logFileInfo.size)} write ahead log files from " +
