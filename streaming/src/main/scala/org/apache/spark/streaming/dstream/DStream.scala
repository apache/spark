...
abstract class DStream[T: ClassTag] (
  ...
  private def doCompute(time: Time): Option[RDD[T]] = {
    ...
    try {
      ...
      // created the DStream. Note that this is equivalent to {{RDDOperationScope.withScope}}
   ...
   */
  private[streaming] def withScope[U](body: => U): U = {
    RDDOperationScope.withScope[U](ssc.sparkContext)(body)
  }
  ...
  def map[U: ClassTag](mapFunc: T => U): DStream[U] = withScope {
   ...
   */
  def flatMap[U: ClassTag](flatMapFunc: T => Traversable[U]): DStream[U] = withScope {
    new FlatMappedDStream(this, context.sparkContext.clean(flatMapFunc))
  }

  /** Return a new DStream containing only the elements that satisfy a predicate. */
  def filter(filterFunc: T => Boolean): DStream[T] = withScope {
   ...
   * an array.
   */
  def glom(): DStream[Array[T]] = withScope {
   ...
   */
  def repartition(numPartitions: Int): DStream[T] = withScope {
   ...
   * of this DStream.
   */
  def reduce(reduceFunc: (T, T) => T): DStream[T] = withScope {
   ...
   * of this DStream.
   */
  def count(): DStream[Long] = withScope {
    this.map(_ => (null, 1L))
  ...
  def foreach(foreachFunc: RDD[T] => Unit): Unit = withScope {
   ...
   */
  @deprecated("use foreachRDD", "0.9.0")
  def foreach(foreachFunc: (RDD[T], Time) => Unit): Unit = withScope {
    this.foreachRDD(foreachFunc)
  }
   ...
   * 'this' DStream will be registered as an output stream and therefore materialized.
   */
  def foreachRDD(foreachFunc: RDD[T] => Unit): Unit = withScope {
   ...
   * 'this' DStream will be registered as an output stream and therefore materialized.
   */
  def foreachRDD(foreachFunc: (RDD[T], Time) => Unit): Unit = withScope {
   ...
   */
  def transform[U: ClassTag](transformFunc: RDD[T] => RDD[U]): DStream[U] = withScope {
    // because the DStream is reachable from the outer object here, and because 
    // DStreams can't be serialized with closures, we can't proactively check 
    // it for serializability and so we pass the optional false to SparkContext.clean
   ...
   * on each RDD of 'this' DStream.
   */
  def transform[U: ClassTag](transformFunc: (RDD[T], Time) => RDD[U]): DStream[U] = withScope {
    // because the DStream is reachable from the outer object here, and because 
    // DStreams can't be serialized with closures, we can't proactively check 
    // it for serializability and so we pass the optional false to SparkContext.clean
    val cleanedF = context.sparkContext.clean(transformFunc, false)
   ...
   * operator, so this DStream will be registered as an output stream and there materialized.
   */
  def print(num: Int): Unit = withScope {
   ...
   */
  def window(windowDuration: Duration): DStream[T] = withScope {
   ...
   */
  def window(windowDuration: Duration, slideDuration: Duration): DStream[T] = withScope {
   ...
   *                       DStream's batching interval
   */
  def countByWindow(windowDuration: Duration, slideDuration: Duration): DStream[Long] = withScope {
   ...
   */
  def union(that: DStream[T]): DStream[T] = withScope {
   ...
   */
  def slice(interval: Interval): Seq[RDD[T]] = withScope {
   ...
   */
  def slice(fromTime: Time, toTime: Time): Seq[RDD[T]] = withScope {
    if (!isInitialized) {
      throw new SparkException(this + " has not been initialized")
    }
   ...
   */
  def saveAsObjectFiles(prefix: String, suffix: String = ""): Unit = withScope {
   ...
   */
  def saveAsTextFiles(prefix: String, suffix: String = ""): Unit = withScope {
    val saveFunc = (rdd: RDD[T], time: Time) => {
      val file = rddToFileName(prefix, suffix, time)
