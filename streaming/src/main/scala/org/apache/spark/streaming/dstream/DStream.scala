...
import org.apache.spark.rdd.{BlockRDD, PairRDDFunctions, RDD, RDDLike}
...
abstract class DStream[T: ClassTag] (
  ...
  ) extends RDDLike[T, DStream] with Serializable with Logging {
  ...
  def persist(): this.type = persist(StorageLevel.MEMORY_ONLY_SER)

  /** Persist RDDs of this DStream with the default storage level (MEMORY_ONLY_SER) */
  def cache(): this.type = persist()

  /** Persists RDDs of this DStream with the storage level (NONE) */
  def unpersist (blocking: Boolean = true): this.type = persist(StorageLevel.NONE)
   ...
   */
  def flatMap[R: ClassTag](f: T => Traversable[R]): DStream[R] = {
    new FlatMappedDStream(this, context.sparkContext.clean(f))
  }
   ...
   */
  def repartition(numPartitions: Int)(implicit ord: Ordering[T] = null): DStream[T] =
    this.transform(_.repartition(numPartitions)(ord))
  ...
  def foreach(f: T => Unit): Unit = {
    this.foreachRDD((rdd: RDD[T]) => rdd.foreach(f))
  }

  /**
   * Apply a function to each partition in each RDD in this DStream.  This is an ouput operator, 
   * so 'this' DStream will be registered as an output stream, and therefore materialized.
   */
  def foreachPartition (f: Iterator[T] => Unit): Unit = {
    this.foreachRDD((rdd: RDD[T]) => rdd.foreachPartition(f))
  }
