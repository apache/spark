...
/**
 * This abstract class exists to work around a Scala compiler bug related to incorrect type erasure
 * for methods whose implementations are inherited from generic traits (SI-8905), which caused
 * NoSuchMethodErrors at runtime when calling these inherited methods from certain classes
 * (for example, this broke JavaDoubleRDD.max(); see SPARK-3266).
 */
private[spark] abstract class AbstractJavaDStream[T, This <: JavaDStreamLike[T, This, R], R <: JavaRDDLike[T, R]]
