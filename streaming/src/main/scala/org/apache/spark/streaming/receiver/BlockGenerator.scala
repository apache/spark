...
private[streaming] trait BlockGeneratorListener {
  ...
  /**
   * Called after the first data item is added into the BlockGenerator.
   */
  def onAddFirstDataItem(data: Any, timestamp: Any): Unit
  ...
  def addData(data: Any): Unit = {
    if (state == Active) {
      ...
      synchronized {
        if (state == Active) {
          currentBuffer += data
          if (currentBuffer.size == 1) {
            logInfo("First data item added into block buffer")
            val firstBlockDataItemTimestamp = System.currentTimeMillis
            listener.onAddFirstDataItem(data, firstBlockDataItemTimestamp)
          }
        } else {
          throw new SparkException(
