...
class StreamingContextSuite extends FunSuite with BeforeAndAfter with Timeouts with Logging {
  ...
  test("from conf with settings") {
    ...
    assert(Utils.timeStringAsS(ssc.conf.get("spark.cleaner.ttl", "-1")) === 10)
  }
  ...
  test("from checkpoint") {
    ...
    assert(
      Utils.timeStringAsS(cp.sparkConfPairs.toMap.getOrElse("spark.cleaner.ttl", "-1")) === 10)
    ssc1.stop()
    val newCp = Utils.deserialize[Checkpoint](Utils.serialize(cp))
    assert(Utils.timeStringAsS(newCp.createSparkConf().get("spark.cleaner.ttl", "-1")) === 10)
