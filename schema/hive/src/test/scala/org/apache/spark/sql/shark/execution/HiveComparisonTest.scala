...
abstract class HiveComparisonTest extends FunSuite with BeforeAndAfterAll with GivenWhenThen with Logging {
   ...
   * Format `shardId:numShards`. Shard ids should be zero indexed.  E.g. -Dshark.hive.testshard=0:4.
   */
  val shardInfo = Option(System.getProperty("shark.hive.shard")).map {
   ...
   *
   * For example when `-Dshark.hive.skiptests=passed,hiveFailed` is specified and test cases listed
   * in [[passedDirectory]] or [[hiveFailedDirectory]] will be skipped.
   */
  val skipDirectories =
    Option(System.getProperty("shark.hive.skiptests"))
      .toSeq
      .flatMap(_.split(","))
      .map(name => new File(targetDir, s"$suiteName.$name"))

  val runOnlyDirectories =
    Option(System.getProperty("shark.hive.runonlytests"))
      .toSeq
      .flatMap(_.split(","))
      .map(name => new File(targetDir, s"$suiteName.$name"))
  ...
  protected def prepareAnswer(
    sharkQuery: TestShark.type#SqlQueryExecution,
    answer: Seq[String]): Seq[String] = {
    val orderedAnswer = sharkQuery.logical match {
      ...
      try {
        ...
        val hiveResults: Seq[Seq[String]] =
          if (hiveCachedResults.size == queryList.size) {
            ...
            val sharkQueries = queryList.map(new TestShark.SqlQueryExecution(_))
            // Make sure we can at least parse everything before attempting hive execution.
            sharkQueries.foreach(_.logical)
        ...
        val catalystResults = queryList.zip(hiveResults).map { case (queryString, hive) =>
          val query = new TestShark.SqlQueryExecution(queryString)
        ...
        (queryList, hiveResults, catalystResults).zipped.foreach {
          case (query, hive, (sharkQuery, catalyst)) =>
            ...
            if ((!sharkQuery.logical.isInstanceOf[ExplainCommand]) && preparedHive != catalyst) {
        ...
        case originalException: Exception =>
          if (System.getProperty("shark.hive.canarytest") != null) {
            ...
            try {
              new TestShark.SqlQueryExecution("SELECT key FROM src").stringResult()
