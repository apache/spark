...
class CommitterBindingSuite extends SparkFunSuite {
  ...
  test("BindingParquetOutputCommitter binds to the inner committer") {
    ...
    // MAPREDUCE-7403 only arrived in hadoop 3.3.5; this test case
...
private[cloud] object CommitterBindingSuite extends Logging {
  ...
  def enablePathCommitter(conf: SparkConf,
    ...
    (manifestCommitterFound, reportsDir)
  }

  /**
   * Is the manifest committer present.
   */
  lazy val manifestCommitterFound: Boolean = {
    try {
      getClass.getClassLoader.loadClass(PathOutputCommitProtocol.MANIFEST_COMMITTER_FACTORY)
      true
    } catch {
