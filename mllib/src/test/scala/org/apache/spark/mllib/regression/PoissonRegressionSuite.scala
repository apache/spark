...
import org.apache.spark.mllib.optimization.PoissonNLLGradient
import org.apache.spark.mllib.optimization._
import org.apache.spark.mllib.util.{PoissonRegressionDataGenerator, LocalSparkContext}

class PoissonRegressionSuite
  extends FunSuite
  with LocalSparkContext {

  def validatePrediction(predictions: Seq[Double], input: Seq[LabeledPoint], tolerance: Double) = {
    val sumSquaredError = (predictions zip input).foldLeft[Double](0.0) {
      case (sum, (predict: Double, labeledPoint: LabeledPoint)) =>
        sum + (predict - labeledPoint.label) * (predict - labeledPoint.label)
    }
    val measSquaredError = sumSquaredError / predictions.length
    assert(measSquaredError < tolerance)
  }
  ...
  test("Getting gradient and loss func value of a point data.") {
    val weight = Vectors.dense(Array(1.0, 1.0, 1.0))
    val data = Vectors.dense(Array(1.0, 2.0, 3.0))
    val label = 10.0
    val gradient = new PoissonNLLGradient()
    val (grad, loss) = gradient.compute(data, label, weight)
    
    assert(math.abs(loss - 343.4287934927351) < 1e-6)
    assert(math.abs(grad(0) - 393.4287934927351) < 1e-6)
    assert(math.abs(grad(1) - 786.8575869854702) < 1e-6)
    assert(math.abs(grad(2) - 1180.286380478205) < 1e-6)
  }
  
  test("The accumulative version of gradient and loss of log-linear likelihood.") {
    val weight = Vectors.dense(Array(1.0, 1.0, 1.0))
    val data = Vectors.dense(Array(1.0, 2.0, 3.0))
    val label = 10.0
    val grad = Vectors.dense(Array(0.0, 0.0, 0.0))
    val gradient = new PoissonNLLGradient()
    
    val loss = gradient.compute(data, label, weight, grad)
  ...
  test("Modeling generated count data with Poisson regression using L-BFGS.") {
    ...
    val generatedDataSet = PoissonRegressionDataGenerator
      .generatePoissonRegRDD(sc, 500, numFeatures, true)
    ...
    posReg.optimizer
      ...
      .setRegParam(0.0)
    val prModel = posReg.run(generatedDataSet, Vectors.dense(new Array[Double](numFeatures)))
    
    val predicts = generatedDataSet map { labeledPoint =>
      math rint prModel.predict(labeledPoint.features)
    }
    validatePrediction(predicts.collect(), generatedDataSet.collect(), 1.0)
  }
  ...
  test("Modeling generated count data with Poisson regression using SGD.") {
    ...
    posReg.optimizer
      .setNumIterations(800)
      .setRegParam(0.0)
      .setStepSize(1e-2)
      .setMiniBatchFraction(0.8)
    val prModel = posReg.run(generatedDataSet, Vectors.dense(Array.fill[Double](numFeatures)(0.5)))
    
    val predicts = generatedDataSet map { labeledPoint =>
      math rint prModel.predict(labeledPoint.features)
    }
    validatePrediction(predicts.collect(), generatedDataSet.collect(), 5.0)
  }
  ...
  test("Modeling real-world count data with Poisson regression using L-BFGS.") {
    ...
    validatePrediction(predicts.collect(), realDataSet.collect(), 10.0)
  }
