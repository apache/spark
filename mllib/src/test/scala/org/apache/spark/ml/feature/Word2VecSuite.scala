...
class Word2VecSuite extends SparkFunSuite with MLlibTestSparkContext with DefaultReadWriteTest {
  ...
  test("getVectors") {
    ...
    val realVectors = model.getVectors.sort("word").select("vector").mapRows {
      case Row(v: Vector) => v
    }.collect()
    // These expectations are just magic values, characterizing the current
    // behavior.  The test needs to be updated to be more general, see SPARK-11502
  ...
  test("findSynonyms") {
    ...
    val (synonyms, similarity) = model.findSynonyms("a", 2).mapRows {
  ...
  test("window size") {
    ...
    val (synonyms, similarity) = model.findSynonyms("a", 6).mapRows {
      case Row(w: String, sim: Double) => (w, sim)
    }.collect().unzip
    ...
    val (synonymsLarger, similarityLarger) = model.findSynonyms("a", 6).mapRows {
      case Row(w: String, sim: Double) => (w, sim)
    }.collect().unzip
