...
class FRegressionSelectorSuite extends MLTest with DefaultReadWriteTest {
  ...
  test("params") {
    ParamsSuite.checkParams(new FRegressionSelector)
  }

  test("Test FValue regression selector: numTopFeatures") {
    val selector = new FRegressionSelector()
  ...
  test("Test F Value regression selector: percentile") {
    val selector = new FRegressionSelector()
  ...
  private def testSelector(selector: FRegressionSelector, data: Dataset[_]):
      FRegressionSelectorModel = {
...
object FRegressionSelectorSuite {
