...
class LogisticRegressionSuite
  ...
  test("binary logistic regression with intercept with L2 regularization with bound") {
    ...
    // The solution is generated by https://github.com/yanboliang/bound-optimization.
    val coefficientsExpectedStd = Vectors.dense(-0.06985003, 0.0, -0.04794278, -0.10168595)
    val interceptExpectedStd = 0.457501411067
    val coefficientsExpected = Vectors.dense(-0.0494524, 0.0, -0.11360797, -0.06313577)
    val interceptExpected = 0.537229668794
  ...
  test("multinomial logistic regression with intercept without regularization with bound") {
    ...
    model1.coefficientMatrix.colIter.zip(coefficientsExpected1.colIter)
    ...
    model2.coefficientMatrix.colIter.zip(coefficientsExpected1.colIter)
      .foreach { case (col1: Vector, col2: Vector) =>
        (col1.asBreeze - col2.asBreeze).toArray.toSeq.sliding(2).foreach {
          case Seq(v1, v2) => assert(v1 ~= v2 absTol 1E-3)
        }
    ...
    model3.coefficientMatrix.colIter.zip(coefficientsExpected3.colIter)
      .foreach { case (col1: Vector, col2: Vector) =>
        (col1.asBreeze - col2.asBreeze).toArray.toSeq.sliding(2).foreach {
          case Seq(v1, v2) => assert(v1 ~= v2 absTol 1E-3)
        }
    ...
    model4.coefficientMatrix.colIter.zip(coefficientsExpected3.colIter)
      .foreach { case (col1: Vector, col2: Vector) =>
        (col1.asBreeze - col2.asBreeze).toArray.toSeq.sliding(2).foreach {
          case Seq(v1, v2) => assert(v1 ~= v2 absTol 1E-3)
        }
    ...
    model5.coefficientMatrix.colIter.zip(coefficientsExpected5.colIter)
      .foreach { case (col1: Vector, col2: Vector) =>
        (col1.asBreeze - col2.asBreeze).toArray.toSeq.sliding(2).foreach {
          case Seq(v1, v2) => assert(v1 ~= v2 absTol 1E-3)
        }
    ...
    model6.coefficientMatrix.colIter.zip(coefficientsExpected5.colIter)
      .foreach { case (col1: Vector, col2: Vector) =>
        (col1.asBreeze - col2.asBreeze).toArray.toSeq.sliding(2).foreach {
          case Seq(v1, v2) => assert(v1 ~= v2 absTol 1E-3)
        }
  ...
  test("multinomial logistic regression with intercept with L2 regularization with bound") {
    ...
    val interceptsExpectedStd = Vectors.dense(2.52055893, 1.0, 2.560682)
    val coefficientsExpected = new DenseMatrix(3, 4, Array(
    ...
    assert(model1.coefficientMatrix ~== coefficientsExpectedStd relTol 0.01)
    assert(model1.interceptVector ~== interceptsExpectedStd relTol 0.01)
