...
class UnivariateFeatureSelectorSuite extends MLTest with DefaultReadWriteTest {
  ...
  test("Test fdr f_classif") {
    ...
    */
    ...
    val model1 = selector.fit(dataset_f_classif1)
    MLTestingUtils.checkCopyAndUids(selector, model1)

    val model2 = selector.fit(dataset_f_classif2)
    MLTestingUtils.checkCopyAndUids(selector, model2)
  }

  test("Test fdr f_regression") {
    // use the following sklean program to verify the test
    // scalastyle:off
    /* import numpy as np
    ...
    */
    ...
    val selector = new UnivariateFeatureSelector()
      ...
      .setLabelType("continuous")
      .setSelectionMode("fdr")
      .setSelectionThreshold(0.1)

    val model1 = testSelector(selector, dataset_f_regression1)
    MLTestingUtils.checkCopyAndUids(selector, model1)

    val model2 = testSelector(selector, dataset_f_regression2)
    MLTestingUtils.checkCopyAndUids(selector, model2)
  }
