...
class LinearRegressionSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("Robust regression with intercept without regularization") {
    val trainer = new RobustRegression
    val model = trainer.fit(dataset)
  ...
  test("Robust regression without intercept without regularization") {
    val trainer = (new RobustRegression).setFitIntercept(false)
    val model = trainer.fit(dataset)
    val modelWithoutIntercept = trainer.fit(datasetWithoutIntercept)
  ...
  test("Robust regression with intercept with L1 regularization") {
    val trainer = (new RobustRegression).setElasticNetParam(1.0).setRegParam(0.57)
    val model = trainer.fit(dataset)
  ...
  test("Robust regression with intercept with L2 regularization") {
    val trainer = (new RobustRegression).setElasticNetParam(0.0).setRegParam(2.3)
    val model = trainer.fit(dataset)
  ...
  test("Robust regression with intercept with ElasticNet regularization") {
    val trainer = (new RobustRegression).setElasticNetParam(0.3).setRegParam(1.6)
    val model = trainer.fit(dataset)
  ...
  test("Robust regression model training summary") {
    val trainer = new RobustRegression
    val model = trainer.fit(dataset)
    ...
    // Residuals in [[RobustRegressionResults]] should equal those manually computed
    val expectedResiduals = dataset.select("features", "label")
      .map { case Row(features: DenseVector, label: Double) =>
      val prediction =
        features(0) * model.weights(0) + features(1) * model.weights(1) + model.intercept
  ...
  test("Robust regression model testset evaluation summary") {
    val trainer = new RobustRegression
    val model = trainer.fit(dataset)
