...
class LinearRegressionSuite
  ...
  test("linear regression with intercept without regularization") {
    ...
    // Bound constrained optimization with infinite bound
    Seq("auto", "l-bfgs").foreach { solver =>
      val lowerBoundOfCoefficients = Vectors.dense(Array.fill(2)(Double.NegativeInfinity))
      val upperBoundOfCoefficients = Vectors.dense(Array.fill(2)(Double.PositiveInfinity))
      ...
      // The result should be the same as non-bound constrained optimization
      val interceptExpected = 6.298698
      val coefficientsExpected = Vectors.dense(4.700706, 7.199082)
    ...
    // Bound constrained optimization with concrete bound value
    Seq("auto", "l-bfgs").foreach { solver =>
      val lowerBoundOfCoefficients = Vectors.dense(Array(Double.NegativeInfinity, 6.0))
      val upperBoundOfCoefficients = Vectors.dense(Array(0.0, 10.0))
      val trainer1 = new LinearRegression()
        .setLowerBoundOfCoefficients(lowerBoundOfCoefficients)
        .setUpperBoundOfCoefficients(upperBoundOfCoefficients)
        .setSolver(solver)
      ...
      // The result is generated by https://github.com/yanboliang/bound-optimization
      val interceptExpected = 10.513031
      val coefficientsExpected = Vectors.dense(0.0, 7.194686)
  ...
  test("linear regression without intercept without regularization") {
    ...
    Seq("auto", "l-bfgs").foreach { solver =>
      ...
      // The result should be the same as non-bound constrained optimization
      val coefficientsExpected = Vectors.dense(6.973403, 5.284370)
    ...
    Seq("auto", "l-bfgs").foreach { solver =>
      ...
      // The result is generated by https://github.com/yanboliang/bound-optimization
      val coefficientsExpected = Vectors.dense(0.0, 6.0)
  ...
  test("linear regression with intercept with L2 regularization") {
    ...
    Seq("auto", "l-bfgs").foreach { solver =>
      ...
      // The result should be the same as non-bound constrained optimization
      val interceptExpected1 = 5.260103
      val coefficientsExpected1 = Vectors.dense(3.725522, 5.71120)

      assert(model1.intercept ~== interceptExpected1 relTol 1E-2)
      assert(model1.coefficients ~= coefficientsExpected1 relTol 1E-2)

      // The result should be the same as non-bound constrained optimization
      val interceptExpected2 = 5.790885
      val coefficientsExpected2 = Vectors.dense(3.432373, 5.919196)

      assert(model2.intercept ~== interceptExpected2 relTol 1E-2)
      assert(model2.coefficients ~= coefficientsExpected2 relTol 1E-2)
    }
    ...
    Seq("auto", "l-bfgs").foreach { solver =>
      ...
      // The result is generated by https://github.com/yanboliang/bound-optimization
      val interceptExpected1 = 8.975608
      val coefficientsExpected1 = Vectors.dense(0.0, 6.0)

      assert(model1.intercept ~== interceptExpected1 relTol 1E-2)
      assert(model1.coefficients ~= coefficientsExpected1 relTol 1E-2)

      // The result is generated by https://github.com/yanboliang/bound-optimization
      val interceptExpected2 = 8.975608
      val coefficientsExpected2 = Vectors.dense(0.0, 6.0)
  ...
  test("linear regression without intercept with L2 regularization") {
    ...
    Seq("auto", "l-bfgs").foreach { solver =>
      ...
      // The result should be the same as non-bound constrained optimization
      val interceptExpected1 = 0.0
      val coefficientsExpected1 = Vectors.dense(5.493430, 4.223082)

      assert(model1.intercept ~== interceptExpected1 relTol 1E-2)
      assert(model1.coefficients ~= coefficientsExpected1 relTol 1E-2)

      // The result should be the same as non-bound constrained optimization
      val interceptExpected2 = 0.0
      val coefficientsExpected2 = Vectors.dense(5.244324, 4.203106)
