...
class BlockMatrix @Since("1.3.0") (
  ...
  private[distributed] def simulateMultiply(
    ...
    val rightCounterpartsHelper = rightMatrix.groupBy(_._1).view.mapValues(_.map(_._2))
    ...
    val leftCounterpartsHelper = leftMatrix.groupBy(_._2).view.mapValues(_.map(_._1))
