...
class GradientDescent private[spark] (private var gradient: Gradient, private var updater: Updater)
  ...
  def setStepSize(step: Double): this.type = {
    require(step > 0, s"Initial step size must be greater than 0, but got ${step}")
    this.stepSize = step
    this
  }
  ...
  def setMiniBatchFraction(fraction: Double): this.type = {
    require(fraction > 0 && fraction <= 1.0, s"Fraction for mini-batch SGD must be in range" +
      s" (0, 1], but got ${fraction}")
  ...
  def setNumIterations(iters: Int): this.type = {
    require(iters > 0, s"Number of iterations must be greater than 0," +
      s" but got ${iters}")
  ...
  def setConvergenceTol(tolerance: Double): this.type = {
    require(tolerance >= 0.0 && tolerance <= 1.0, s"Convergence tolerance must be in range" +
      s" [0, 1], but got ${tolerance}")
