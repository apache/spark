    ...
    val distinctFields = dataFields.groupBy(dataField => dataField.getName.getValue).map{case ( fieldName, dataFieldsArr) => {
      val finalDataFields: DataField = dataFieldsArr.head.getOpType match {
        ...
        case OpType.CATEGORICAL => dataFieldsArr.toList.reduce((a,b) => {
          val valuesSet = (a.getValues.asScala.toList ++ b.getValues.asScala.toList).groupBy(v => v.getValue)
          val blist = valuesSet.map { case (string, valueList) => valueList(0) }.toList.asJava
