...
object PAM {
  ...
  /**
   * Trains a PAM model using the given set of parameters.
   *
   * @param data training points stored as `RDD[Array[Double]]`
   * @param k number of clusters
   * @param maxIterations max number of iterations
   * @param minIterations min number of iterations
   * @param runs number of parallel runs, defaults to 1. The best model is returned.
   * @param initializationMode initialization model, either "random" or "PAM_PARALLEL" (default).
   */
  def train(
             data: RDD[Vector],
             k: Int,
             ...
             minIterations: Int,
             runs: Int,
             initializationMode: String): PAMModel = {
    new PAM().setK(k)
  ...
  /**
   * Trains a PAM model using specified parameters and the default values for unspecified.
   */
  def train(
             data: RDD[Vector],
             k: Int,
             maxIterations: Int): PAMModel = {
    train(data, k, maxIterations, 5, 1, PAM_PARALLEL)
  }
  ...
  def train(
             ...
             maxIterations: Int,
             runs: Int): PAMModel = {
    train(data, k, maxIterations, 5, runs, PAM_PARALLEL)
  }
