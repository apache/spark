 ...
 * incoming data from a DStream (see `SVMWithSGD` for model equation)
 *
 ...
 */
@Experimental
class StreamingSVMWithSGD private[mllib] (
  ...
  /**
   * Construct a StreamingSVM object with default parameters:
  ...
  protected val algorithm = new SVMWithSGD(stepSize, numIterations, regParam, miniBatchFraction)
