...
class Word2Vec extends Serializable with Logging {
  ...
  def setMaxSentenceLength(maxSentenceLength: Int): this.type = {
    require(maxSentenceLength > 0,
      s"Maximum length of sentences must be greater than 0 but got ${maxSentenceLength}")
  ...
  def setVectorSize(vectorSize: Int): this.type = {
    require(vectorSize > 0,
      s"vector size must be greater than 0 but got ${vectorSize}")
  ...
  def setLearningRate(learningRate: Double): this.type = {
    require(learningRate > 0,
      s"Initial learning rate must be greater than 0 but got ${learningRate}")
  ...
  def setWindowSize(window: Int): this.type = {
    require(window > 0,
      s"Window of words must be greater than 0 but got ${window}")
    this.window = window
    this
  }
