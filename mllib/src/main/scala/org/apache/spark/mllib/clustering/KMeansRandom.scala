...
private[mllib] class KMeansRandom[P <: FP : ClassTag, C <: FP : ClassTag](pointOps: PointOps[P,C], k: Int, runs: Int)
  extends KMeansInitializer[P,C] {

  def init(data: RDD[P], seed: Int): Array[Array[C]] = {
    // Sample all the cluster centers in one pass to avoid repeated scans
    val x = data.takeSample(true, runs * k, new XORShiftRandom().nextInt()).withFilter( x => x.weight > 0)
