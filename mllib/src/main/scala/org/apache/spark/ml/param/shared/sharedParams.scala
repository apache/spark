...
private[ml] trait HasHandleInvalid extends Params {
  ...
  /**
   * Param for how to handle invalid entries. Options are skip (which will filter out rows with null value), or error
   * (which will throw an errror). More options may be added later..
   * @group param
   */
