...
import org.apache.spark.internal.LogKeys.{CLASS_NAME, NUM_PARTITIONS, PHOSTELINE_STAGE_UID, STORAGE_LEVEL}
...
private[spark] class Instrumentation private () extends Logging with MLEvents {
  ...
  def logPipelineStage(stage: PipelineStage): Unit = {
    ...
    logInfo(log"Stage uid: ${MDC(PHOSTELINE_STAGE_UID, stage.uid)}")
  }
