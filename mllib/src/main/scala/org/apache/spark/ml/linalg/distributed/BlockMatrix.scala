...
class BlockMatrix @Since("2.1.0") (
    @Since("2.1.0") val blocks: RDD[((Int, Int), Matrix)],
    @Since("2.1.0") val rowsPerBlock: Int,
    @Since("2.1.0") val colsPerBlock: Int,
  ...
  private lazy val blockInfo = blocks.mapValues(block => (block.numRows, block.numCols)).collect()
  ...
  private[distributed] def simulateMultiply(
    ...
    val leftMatrix = blockInfo.map(_._1)
    val rightMatrix = other.blockInfo.map(_._1)
  ...
  private def multiply(
    ...
    require(numCols() == other.numRows(), "The number of columns of A and the number of rows " +
      ...
      "initializing them.")
    require(colsPerBlock == other.rowsPerBlock,
