 ...
 * and numeric columns will be cast to doubles.
 * If the label column is of type string, it will be first transformed to double with `StringIndexer`.
 * If the label column does not exist in the DataFrame, the output label column will be created
 * from the specified response variable in the formula.
 */
@Experimental
