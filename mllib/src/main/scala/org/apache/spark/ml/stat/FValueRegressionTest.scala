...
object FValueRegressionTest {
  ...
  def test(dataset: DataFrame, featuresCol: String, labelCol: String): DataFrame = {
    ...
    val testResults = SelectionTest.fValueRegressionTest(dataset, featuresCol, labelCol)
    ...
    spark.createDataFrame(Seq((pValues, degreesOfFreedom, fValues)))
  }
}
