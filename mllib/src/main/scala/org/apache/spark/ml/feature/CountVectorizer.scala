...
class CountVectorizer @Since("1.5.0") (@Since("1.5.0") override val uid: String)
  ...
  override def fit(dataset: Dataset[_]): CountVectorizerModel = {
    ...
    val maxDfOpt = if (isSet(maxDF)) {
      if ($(maxDF) >= 1.0) {
        Some($(maxDF))
      } else {
        Some($(maxDF) * input.cache().count())
      }
    } else {
      None
    }
    val filterDf = if (maxDfOpt.isDefined) {
      val maxDf = maxDfOpt.get
      (df: Double) => df <= maxDf && df >= minDf
    } else {
      (df: Double) => df >= minDf
    }
