...
// scalastyle:on
   ...
   * Aggregate function: returns the most frequent value in a group.
   *
   * If there are multiple values with the greatest frequency only one value will be returned. The
   * value will be chosen based on optional sort direction. Use ascending order to get smallest
