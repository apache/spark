...
class SparkSession private[sql] (
   ...
   *   can be also a Column` of a literal or a foldable expression created by `map()`, `array()`,
   *   `struct()`, `map_from_arrays()` or `map_from_entries()`, in that case it is taken as is.
   *
   * @since 3.4.0
   */
  @Experimental
