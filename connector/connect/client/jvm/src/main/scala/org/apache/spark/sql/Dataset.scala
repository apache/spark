...
class Dataset[T] private[sql] (
  ...
  /**
   * Gets the top 20 rows of Dataset as string in a tabular form. Strings more than 20 characters
   ...
   */
  // scalastyle:off println
  def getString(numRows: Int, truncate: Boolean): String = {
    val truncateValue = if (truncate) 20 else 0
    getString(numRows, truncateValue, vertical = false)
  }
