...
class AmmoniteReplE2ESuite extends ConnectFunSuite with RemoteSparkSession {
  ...
  test("SPARK-56448: restarting spark-connect-scala-client --remote does not throw NPE") {
    // First invocation
    val (exit1, _, stderr1) = runSparkShell()
    assert(exit1 == 0, s"First spark-connect-scala-client failed (exit=$exit1): $stderr1")

    // Second invocation -- without the fix, this would NPE from stale Ammonite cache
    val (exit2, _, stderr2) = runSparkShell()
    assert(exit2 == 0, s"Second spark-connect-scala-client failed (exit=$exit2): $stderr2")
  }
}
