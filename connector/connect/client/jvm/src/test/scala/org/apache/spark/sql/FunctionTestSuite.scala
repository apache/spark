...
class FunctionTestSuite extends ConnectFunSuite {
  ...
  testEquals("col/column",
    a,
    column("a"))
  testEquals("asc/asc_nulls_first",
    asc("a"),
    asc_nulls_first("a"))
  testEquals("desc/desc_nulls_last",
    desc("a"),
    desc_nulls_last("a"))
  testEquals("approx_count_distinct",
    approxCountDistinct(a),
    approxCountDistinct("a"),
    approx_count_distinct("a"),
    approx_count_distinct(a))
  testEquals("approx_count_distinct rsd",
  ...
  testEquals("avg/mean",
    ...
    mean(a),
    mean("a"))
  testEquals("collect_list",
    collect_list("a"),
    collect_list(a))
  testEquals("collect_set",
    collect_set("a"),
    collect_set(a))
  testEquals("corr",
    corr("a", "b"),
    corr(a, b))
  testEquals("count_distinct",
    countDistinct(a, b, c),
    countDistinct("a", "b", "c"),
    count_distinct(a, b, c))
  testEquals("covar_pop",
    covar_pop(a, b),
    covar_pop("a", "b"))
  testEquals("covar_samp",
    covar_samp(a, b),
    covar_samp("a", "b"))
  testEquals("first",
    first("a"),
  ...
  testEquals("stddev",
    stddev("a"),
    stddev(a))
  testEquals("stddev_samp",
    stddev_samp("a"),
    stddev_samp(a))
  testEquals("stddev_pop",
    stddev_pop("a"),
    stddev_pop(a))
  testEquals("sum",
    sum("a"),
    sum(a))
  testEquals("sum_distinct",
    sumDistinct("a"),
    sumDistinct(a),
    sum_distinct(a))
  testEquals("variance",
  ...
  testEquals("array",
    array(a, b, c),
    array("a", "b", "c"))
  testEquals("monotonicallyIncreasingId",
  ...
  testEquals("hypot",
    ...
    hypot(a, "b"),
    hypot("a", "b"))
  testEquals("hypot right fixed",
    hypot(lit(3d), a),
    hypot(lit(3d), "a"),
    hypot(3d, a),
    hypot(3d, "a"))
  testEquals("hypot left fixed",
    hypot(a, lit(4d)),
    hypot(a, 4d),
    hypot("a", lit(4d)),
    hypot("a", 4d))
  ...
  testEquals("pow",
    ...
    pow("a", b),
    pow("a", "b"))
  testEquals("pow left fixed",
    pow(lit(7d), b),
    pow(lit(7d), "b"),
    pow(7d, b),
    pow(7d, "b"))
  testEquals("pow right fixed",
    pow(a, lit(9d)),
    pow(a, 9d),
    pow("a", lit(9d)),
    pow("a", 9d))
  ...
  testEquals("bround",
    bround(a),
    bround(a, 0))
  testEquals("shiftleft",
    shiftLeft(a, 2),
    shiftleft(a, 2))
  testEquals("shiftright",
    shiftRight(a, 3),
    shiftright(a, 3))
  testEquals("shiftrightunsigned",
    shiftRightUnsigned(a, 3),
    shiftrightunsigned(a, 3))
  testEquals("signum",
    signum("a"),
    signum(a))
