...
import org.apache.spark.sql.catalyst.plans.logical.{AttachDistributedSequence, CollectMetrics, CommandResult, Deduplicate, Except, Intersect, LocalRelation, LogicalPlan, Sample, Sort, SubqueryAlias, Union, Unpivot, UnresolvedHint}
...
class SparkConnectPlanner(val session: SparkSession) {
  ...
  def transformRelation(rel: proto.Relation): LogicalPlan = {
    val plan = rel.getRelTypeCase match {
      ...
      case proto.Relation.RelTypeCase.WITH_SEQUENCE_COLUMN =>
        transformWithSequenceColumn(rel.getWithSequenceColumn)
  ...
  private def transformWithSequenceColumn(rel: proto.WithSequenceColumn): LogicalPlan = {
    AttachDistributedSequence(
      AttributeReference(rel.getName, LongType, nullable = false)(),
      transformRelation(rel.getInput))
  }
