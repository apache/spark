...
class KafkaOffsetRangeCalculatorSuite extends SparkFunSuite {
  ...
  def testWithMinPartitions(name: String, minPartition: Int)(
    ...
    val options = new CaseInsensitiveStringMap(
      Map("minPartitions" -> minPartition.toString).asJava)
  ...
  test("with no minPartition: N TopicPartitions to N offset ranges") {
    ...
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 2))) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None)))

    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 2)), executorLocations = Seq("location")) ==
  ...
  test("with no minPartition: empty ranges ignored") {
    val calc = KafkaOffsetRangeCalculator(CaseInsensitiveStringMap.empty())
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 2), KafkaOffsetRange(tp2, 1, 1))) ===
        Seq(KafkaOffsetRange(tp1, 1, 2, None)))
  }
  ...
  testWithMinPartitions("1 TopicPartition to N offset ranges", 4) { calc =>
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 5))) ==
        Seq(
          KafkaOffsetRange(tp1, 1, 2, None),
    ...
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 5)), executorLocations = Seq("location")) ==
        Seq(
          KafkaOffsetRange(tp1, 1, 2, None),
          KafkaOffsetRange(tp1, 2, 3, None),
          KafkaOffsetRange(tp1, 3, 4, None),
    ...
    ) // location pref not set when minPartition is set
  }

  testWithMinPartitions("N skewed TopicPartitions to M offset ranges", 4) { calc =>
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 5), KafkaOffsetRange(tp2, 1, 21))) ===
        Seq(
  ...
  testWithMinPartitions("SPARK-30656: ignore empty ranges and split the rest", 4) { calc =>
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 1), KafkaOffsetRange(tp2, 1, 21))) ===
        Seq(
  ...
  testWithMinPartitions("SPARK-30656: N very skewed TopicPartitions to M offset ranges", 4) {
    calc =>
      assert(
        calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 2), KafkaOffsetRange(tp2, 1, 1001))) ===
          Seq(
            KafkaOffsetRange(tp1, 1, 2, None),
  ...
  testWithMinPartitions(
    ...
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 5), KafkaOffsetRange(tp2, 1, 21))) ===
        Seq(KafkaOffsetRange(tp1, 1, 5, None), KafkaOffsetRange(tp2, 1, 21, None)))
  }

  testWithMinPartitions("range inexact multiple of minPartitions", 3) { calc =>
    assert(
      calc.getRanges(Seq(KafkaOffsetRange(tp1, 1, 11))) ==
        Seq(
  ...
  testWithMinPartitions("SPARK-36576: 0 small unsplit ranges and 3 large split ranges", 9) {
    calc =>
      assert(
        calc.getRanges(
          Seq(
  ...
  testWithMinPartitions("SPARK-36576: 1 small unsplit range and 2 large split ranges", 6) {
    calc =>
      assert(
        calc.getRanges(
          Seq(
  ...
  testWithMinPartitions("SPARK-36576: 2 small unsplit ranges and 1 large split range", 6) {
    calc =>
      assert(
        calc.getRanges(
          Seq(
