...
class KafkaDataConsumerSuite
  ...
  test("SPARK-54039: KafkaDataConsumer logs include TaskContext information") {
    prepareTestTopicHavingTestMessages(topic)
    val topicPartition = new TopicPartition(topic, 0)
    val kafkaParams = getKafkaParams()

    try {
      // Set up TaskContext with known taskId and partitionId for validation
      val taskAttemptId = 12345L
      val partitionId = 42
      val context = new TaskContextImpl(0, partitionId, 0, taskAttemptId, 0, 1, null, null, null)
      TaskContext.setTaskContext(context)

      // Acquire consumer and perform read operation
      val consumer = KafkaDataConsumer.acquire(topicPartition, kafkaParams)

      // Read some records to generate metrics
      val range = consumer.getAvailableOffsetRange()
      if (range.latest > range.earliest) {
        consumer.get(range.earliest, range.earliest + 1, 10000, failOnDataLoss = false)
      }

      // Release should trigger logging with taskId and partitionId
      // The log message should include:
      // - taskId=12345
      // - partitionId=42
      consumer.release()

      // Verify consumer was properly released
      assert(consumer._consumer.isEmpty)
    } finally {
      TaskContext.unset()
    }
  }

  test("SPARK-54039: KafkaDataConsumer logs without TaskContext information") {
    prepareTestTopicHavingTestMessages(topic)
    val topicPartition = new TopicPartition(topic, 0)
    val kafkaParams = getKafkaParams()

    try {
      // No TaskContext set - simulates non-task execution context
      TaskContext.unset()
      ...
      // Release should trigger logging without taskId and partitionId
      // The log message should end with "ms." instead of "ms for taskId=..."
      consumer.release()
