        ...
        (logicalDataType, catalystType) match {
          ...
          case (_: YearMonthIntervalType, _) if preventReadingIncorrectType =>
            throw QueryCompilationErrors.avroIncompatibleReadError(toFieldStr(avroPath),
              toFieldStr(catalystPath), logicalDataType.catalogString, catalystType.catalogString)
        ...
        (logicalDataType, catalystType) match {
          ...
          case (_, dt: DecimalType) =>
            ...
            if (preventReadingIncorrectType &&
              ...
              throw QueryCompilationErrors.avroIncompatibleReadError(toFieldStr(avroPath),
                toFieldStr(catalystPath), logicalDataType.catalogString, dt.catalogString)
            }
