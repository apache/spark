...
private[v2] trait V2JDBCTest
  ...
  test("SPARK-57988: IS [NOT] NULL over an IN operand is pushed down") {
    ...
    withTable(tbl) {
      ...
      // An IN operand must be parenthesized as `(a IN (...)) IS [NOT] NULL`; the unwrapped
      // rendering `a IN (...) IS NOT NULL` is a syntax error on engines such as PostgreSQL.
      // The NULL row exercises the IS NULL branch: (NULL IN (...)) is NULL, so it matches
      // IS NULL and not IS NOT NULL. Dialects without a boolean type (e.g. SQL Server) cannot
      // push this, but Spark evaluates it and returns the same rows.
      val df1 = sql(s"SELECT b FROM $tbl WHERE (a IN (10000, 20000)) IS NOT NULL")
      checkFilterPushed(df1, supportsIsNullOverPredicate)
      assert(df1.collect().map(_.getString(0)).sorted === Array("x", "y"))
